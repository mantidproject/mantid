...
from sans.common.enums import (ReductionDimensionality, OutputMode, SaveType, SANSInstrument, RangeStepType,
                               ReductionMode, FitType)
from sans.gui_logic.gui_common import (get_reduction_mode_from_gui_selection, get_reduction_mode_strings_for_gui,
                                       ...
                                       load_property, set_setting, get_instrument_from_gui_selection)
...
class SANSDataProcessorGui(QMainWindow, Ui_SansDataProcessorWindow):
    ...
    COLUMN_NAMES = [
        "Sample Scatter", "Sample Transmission", "Sample Direct", "Can Scatter", "Can Transmission", "Can Direct",
        "Output Name", "User File", "Sample Thickness", "Options", "Sample Shape", "Sample Height", "Sample Width",
    ...
    def __init__(self, parent=None, window_flags=None):
        ...
        SANSDataProcessorGui.INSTRUMENTS = ",".join(
            item.value
            for item in [SANSInstrument.SANS2D, SANSInstrument.LOQ, SANSInstrument.LARMOR, SANSInstrument.ZOOM])
    ...
    def setup_layout(self):
        ...
        self.q_xy_step_type_combo_box.currentIndexChanged.connect(self._on_q_xy_step_type_has_changed)
    ...
    def _on_user_file_load(self):
        ...
        """
        # Load the user file
        load_file(self.user_file_line_edit, "TOML Files (*.toml, *.TOML);; Text Files (*.txt)", self.__generic_settings,
                  self.__path_key, self.get_user_file_path)
    ...
    def _on_load_pixel_adjustment_det_1(self):
        load_file(self.pixel_adjustment_det_1_line_edit, "*.*", self.__generic_settings, self.__path_key,
                  self.get_pixel_adjustment_det_1)
    ...
    def _on_load_pixel_adjustment_det_2(self):
        load_file(self.pixel_adjustment_det_2_line_edit, "*.*", self.__generic_settings, self.__path_key,
                  self.get_pixel_adjustment_det_2)
    ...
    def _on_load_wavelength_adjustment_det_1(self):
        load_file(self.wavelength_adjustment_det_1_line_edit, "*.*", self.__generic_settings, self.__path_key,
    ...
    def _on_load_wavelength_adjustment_det_2(self):
        load_file(self.wavelength_adjustment_det_2_line_edit, "*.*", self.__generic_settings, self.__path_key,
    ...
    def _on_load_transmission_roi_files(self):
        load_file(self.transmission_roi_files_line_edit, "*.*", self.__generic_settings, self.__path_key,
                  self.get_transmission_roi_files)
    ...
    def _on_load_transmission_mask_files(self):
        load_file(self.transmission_mask_files_line_edit, "*.*", self.__generic_settings, self.__path_key,
                  self.get_transmission_mask_files)

    def get_moderator_file(self):
        return str(self.q_resolution_moderator_file_line_edit.text())

    def _on_load_moderator_file(self):
        load_file(self.q_resolution_moderator_file_line_edit, "*.*", self.__generic_settings, self.__path_key,
                  self.get_moderator_file)
    ...
    def _on_load_mask_file(self):
        load_file(self.mask_file_input_line_edit, "*.*", self.__generic_settings, self.__mask_file_input_path_key,
                  self.get_mask_file)
