...
class TOFTOFScriptElement(BaseScriptElement):
    ...
    def validate_inputs(self):
        ...
        if any([
                self.saveSofTWNxspe, self.saveSofTWNexus, self.saveSofTWAscii, self.saveSofQWNexus, self.saveSofQWAscii
    ...
    def merge_runs(self, ws_raw, raw_runs, outws, comment, temperature=None):
        ...
        self.l(
            "AddSampleLog({}, LogName='temperature', LogText=str(temperature), LogType='Number', LogUnit='K')".format(
                outws))
    ...
    def save_wsgroup(self, wsgroup, suffix, spectrumMetaData, saveFormats):
        ...
        if 'ascii' in saveFormats:
            self.l("    SaveAscii(ws, join(r'{}', name + '.txt'), SpectrumMetaData='{}')".format(
                self.saveDir, spectrumMetaData))
        self.l()
    ...
    def to_script(self):
        ...
        if self.vanRuns:
            ...
            if self.ecRuns:
                self.l("{} = GroupWorkspaces({}list({}.getNames()))".format(
                    gData, self.group_list([wsVanNorm, wsECNorm2], ' + '), gDataSource))
            else:
                self.l("{} = GroupWorkspaces({}list({}.getNames()))".format(gData, self.group_list([wsVanNorm], ' + '),
                                                                            gDataSource))
        else:
            if self.ecRuns:
                self.l("{} = GroupWorkspaces({}list({}.getNames()))".format(gData, self.group_list([wsECNorm2], ' + '),
                                                                            gDataSource))
            else:
        ...
        if self.binQon and self.binEon:
            ...
            self.l("{} = SofQW3({}, QAxisBinning=rebinQ, EMode='Direct', EFixed=Ei, ReplaceNaNs=False)".format(
                gDataBinQ, gLast))
            if self.replaceNaNs:
        ...
        saveFormats = set(
            compress(self.allowed_save_formats, [self.saveSofTWNxspe, self.saveSofTWNexus, self.saveSofTWAscii]))
