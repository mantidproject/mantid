...
class TomlParser(object):
    ...
    def get_toml_parser(self, toml_file_path, data_info) -> IStateParser:
        parsed_dict = self._lib_impl.get_user_file_dict(toml_file_path)
        return self.get_versioned_parser(parsed_dict, data_info=data_info)

    def parse_toml_file(self, toml_file_path, data_info) -> AllStates:
        parser = self.get_toml_parser(toml_file_path=toml_file_path, data_info=data_info)
        return parser.get_all_states()

    @staticmethod
    def get_versioned_parser(toml_dict, data_info) -> IStateParser:
        """
        ...
        if version == 0:
            return TomlV1Parser(toml_dict, data_info=data_info)
        else:
