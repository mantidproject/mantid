...
class ConvertToQ(object):
    def __init__(self, state_convert_to_q, wavelength_adj_workspace=None, pixel_adj_workspace=None,
                 wavelength_and_pixel_adj_workspace=None):
        self._wavelength_and_pixel_adj_workspace = wavelength_and_pixel_adj_workspace
        self._pixel_adj_workspace = pixel_adj_workspace
        self._wavelength_adj_workspace = wavelength_adj_workspace
        self._state = state_convert_to_q

    def convert_workspace(self, workspace, output_summed_parts=False):
        """
        ...
        if reduction_dimensionality is ReductionDimensionality.OneDim:
            output_workspace, sum_of_counts_workspace, sum_of_norms_workspace = self._run_q_1d(workspace,
                                                                                               output_summed_parts)
        else:
            output_workspace, sum_of_counts_workspace, sum_of_norms_workspace = self._run_q_2d(workspace,
                                                                                               output_summed_parts)
    ...
    def _run_q_1d(self, workspace, output_summed_parts):
        convert_to_q = self._state

        if convert_to_q.use_q_resolution:
            q_resolution_workspace = self._create_q_resolution_workspace(convert_to_q=convert_to_q, workspace=workspace)
        else:
        ...
        if self._wavelength_adj_workspace:
            q1d_options.update({"WavelengthAdj": self._wavelength_adj_workspace})

        if self._pixel_adj_workspace:
            q1d_options.update({"PixelAdj": self._pixel_adj_workspace})

        if self._wavelength_and_pixel_adj_workspace:
            q1d_options.update({"WavePixelAdj": self._wavelength_and_pixel_adj_workspace})
        ...
        if output_summed_parts:
            sum_of_counts_workspace, sum_of_norms_workspace = self._get_partial_output(q1d_alg, do_clean=False)
            return reduced_workspace, sum_of_counts_workspace, sum_of_norms_workspace
        else:
            return reduced_workspace, None, None

    def _run_q_2d(self, workspace, output_summed_parts):
        """
        ...
        qxy_options = {"InputWorkspace": workspace,
                       ...
                       "ExtraLength": gravity_extra_length}
        if self._wavelength_adj_workspace:
            qxy_options.update({"WavelengthAdj": self._wavelength_adj_workspace})

        if self._pixel_adj_workspace:
            qxy_options.update({"PixelAdj": self._pixel_adj_workspace})
        ...
        # Get the partial workspaces
        if output_summed_parts:
            sum_of_counts_workspace, sum_of_norms_workspace = self._get_partial_output(qxy_alg, do_clean=True)
            return reduced_workspace, sum_of_counts_workspace, sum_of_norms_workspace
        else:
            return reduced_workspace, None, None

    def _get_partial_output(self, alg, do_clean=False):
    ...
    def _create_q_resolution_workspace(convert_to_q, workspace):
        ...
        sigma_moderator = ConvertToQ._load_sigma_moderator_workspace(file_name)

        # Get the aperture diameters
        a1, a2 = ConvertToQ._get_aperture_diameters(convert_to_q)
