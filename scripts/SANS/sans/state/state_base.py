...
class EnumParameter(TypedParameter):
    ...
    def _type_check(self, value):
        msg = "Trying to set {0} which expects a value of type {1}." \
              " Got a value of {2} which is of type: {3}"
        # For OSX, enum values are type EnumValues, so isinstance check won't work
        # We can instead check that EnumValues.enum is parameter_type
        try:
            value = value.enum
        except AttributeError:
            # We're not on OSX / don't have an EnumValue object
            if not isinstance(value, self.parameter_type):
                raise TypeError(msg.format(self.name, self.parameter_type, value, type(value)))
        else:
            if not value == self.parameter_type:
                raise TypeError(msg.format(self.name, self.parameter_type, value, type(value)))
