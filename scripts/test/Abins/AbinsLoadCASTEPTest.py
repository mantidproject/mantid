...
class LoadCASTEPTest(unittest.TestCase, abins.GeneralLoadAbInitioTester):

    # simple tests
    def test_non_existing_file(self):
        with self.assertRaises(IOError):
            bad_castep_reader = abins.LoadCASTEP(input_ab_initio_filename="NonExistingFile.txt")
            bad_castep_reader.read_vibrational_or_phonon_data()

        with self.assertRaises(ValueError):
            # noinspection PyUnusedLocal
            abins.LoadCASTEP(input_ab_initio_filename=1)
    ...
    def test_gamma_sum_correction(self):
        self.check(name=self._gamma_sum, loader=abins.LoadCASTEP)
    ...
    def test_gamma_no_sum_correction(self):
        self.check(name=self._gamma_no_sum, loader=abins.LoadCASTEP)
    ...
    def test_sum_correction_single_crystal(self):
        self.check(name=self._many_k_sum, loader=abins.LoadCASTEP)
    ...
    def test_no_sum_correction_single_crystal(self):
        self.check(name=self._many_k_no_sum, loader=abins.LoadCASTEP)
    ...
    def test_isotopes(self):
        self.check(name=self._li7_d2, loader=abins.LoadCASTEP)
