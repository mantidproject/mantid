...
class TestTubeCalib(unittest.TestCase):
    ...
    def setUpClass(cls):  # called only before running all tests in the test case
        ...
        cls.corelli = {"tube_length": 0.900466, "pixels_per_tube": 256, "workspace": workspace}  # in meters
    ...
    def test_correct_tube_to_ideal_tube(self):
        ...
        fitted_coordinates = correct_tube_to_ideal_tube(
            data["tube_points"], data["ideal_tube_coordinates"], data["detector_count"], parameters_table="parameters"
        )

        # Verify the fitted coordinates are the ideal_tube_coordinates
        assert_allclose([fitted_coordinates[int(n)] for n in data["tube_points"]], data["ideal_tube_coordinates"], atol=0.0001)
        ...
        # a bit of clean-up
        DeleteWorkspaces(["PolyFittingWorkspace", "QF_NormalisedCovarianceMatrix", "QF_Parameters", "QF_Workspace", "parameters"])

    def test_getCalibratedPixelPositions(self):
        data = self.y_quad_data
        # calibrate the first tube of bank 20 in the corelli input workspace
        detector_ids, detector_positions = getCalibratedPixelPositions(
            self.corelli["workspace"],
            data["tube_points"],
        ...
        # a bit of clean-up
        DeleteWorkspaces(["parameters", "PolyFittingWorkspace", "QF_NormalisedCovarianceMatrix", "QF_Parameters", "QF_Workspace"])
