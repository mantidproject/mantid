...
class SampleTransmissionCalculatorPresenterTest(unittest.TestCase):
    ...
    def test_calculate_with_invalid(self):
        view = mock.Mock()
        view.get_input_dict.return_value = {'val_1': 1.0, 'val_2': 2.0}
        model = mock.Mock()
        model.validate.return_value = {'val_1': 'error_string_1', 'val_2': 'error_string_2'}
        self.presenter = SampleTransmissionCalculatorPresenter(view, model)

        self.presenter.calculate()
        call_list = [mock.call('val_1'), mock.call('val_2')]
    ...
    def test_calculate_with_valid(self):
        ...
        calculated_values = {'x': [1.0, 2.0], 'y': [1.0, 2.0], 'scattering': 1.0}
        model.calculate.return_value = calculated_values
        statistics = {"Min": 1.0, "Max": 2.0, "Mean": 1.5, "Median": 1.0, "Std. Dev.": 1.0}
