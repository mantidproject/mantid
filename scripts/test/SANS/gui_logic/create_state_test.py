...
from sans.common.enums import (SANSInstrument, SANSFacility, SaveType, ReductionDimensionality)
...
class GuiCommonTest(unittest.TestCase):
    def setUp(self):
        ...
        self.run_tab_model = RunTabModel({})
    ...
    def test_create_states_returns_correct_number_of_states(self):
        states, errors = create_states(self.run_tab_model, self.table_model, SANSInstrument.LOQ, SANSFacility.ISIS,
                                       row_index=[0, 1])
    ...
    def test_create_state_from_user_file_if_specified(self, create_gui_state_mock):
        create_gui_state_mock.returns = RunTabModel({})
        ...
        states, errors = create_states(self.run_tab_model, table_model, SANSInstrument.LOQ, SANSFacility.ISIS,
                                       row_index=[0, 1, 2])

        self.assertEqual(len(states), 1)
        create_gui_state_mock.assert_called_once_with('MaskLOQData.txt', self.run_tab_model)

    def test_create_gui_state_from_userfile_adds_save_format_from_gui(self):
        gui_state = RunTabModel({})
    ...
    def test_create_gui_state_overrides_reduction_mode_from_gui(self):
        gui_state = RunTabModel({})
        gui_state.reduction_dimensionality = ReductionDimensionality.TWO_DIM

        row_state = create_gui_state_from_userfile('MaskLOQData.txt', gui_state)

        self.assertEqual(ReductionDimensionality.TWO_DIM, row_state.reduction_dimensionality)
