...
from sans.user_file.settings_tags import (DetectorId, BackId, range_entry, back_single_monitor_entry,
                                          single_entry_with_detector, mask_angle_entry, LimitsId, simple_range, MaskId,
                                          mask_block, mask_block_cross, mask_line, range_entry_with_detector, SampleId,
                                          SetId, set_scales_entry, position_entry, TransId, TubeCalibrationFileId,
                                          QResolutionId, FitId, fit_general, MonId, monitor_length, monitor_file,
                                          GravityId, OtherId, monitor_spectrum, PrintId, det_fit_range, q_rebin_values)
from sans.user_file.user_file_parser import (InstrParser, DetParser, LimitParser, MaskParser, SampleParser, SetParser,
                                             TransParser, TubeCalibFileParser, QResolutionParser, FitParser,
                                             GravityParser, MaskFileParser, MonParser, PrintParser, BackParser,
                                             SANS2DParser, LOQParser, UserFileParser, LARMORParser, CompatibilityParser)
    ...
    for key in keys_result:
        ...
        if result[key] != expected[key]:
            assert_true(result[key] == expected[key],
                        "For key {}, {} does not equal {}".format(key, result[key], expected[key]))
...
class InstrParserTest(unittest.TestCase):
    ...
    def test_that_instruments_are_parsed_correctly(self):
        valid_settings = {
            "SANS2D": {
                DetectorId.INSTRUMENT: SANSInstrument.SANS2D
            },
            "LOQ": {
                DetectorId.INSTRUMENT: SANSInstrument.LOQ
            },
            "ZOOM": {
                DetectorId.INSTRUMENT: SANSInstrument.ZOOM
            },
            "LARMOR": {
                DetectorId.INSTRUMENT: SANSInstrument.LARMOR
            }
        }
...
class DetParserTest(unittest.TestCase):
    ...
    def test_that_reduction_mode_is_parsed_correctly(self):
        ...
        valid_settings = {
            ...
            " dEt/ BOtH": {
                DetectorId.REDUCTION_MODE: ReductionMode.ALL
            },
            "DeT /merge ": {
                DetectorId.REDUCTION_MODE: ReductionMode.MERGED
            },
            " DEt / MERGED": {
                DetectorId.REDUCTION_MODE: ReductionMode.MERGED
            }
    ...
    def test_that_merge_option_is_parsed_correctly(self):
        valid_settings = {
            "DET/RESCALE 123": {
                DetectorId.RESCALE: 123
            },
            "dEt/ shiFt 48.5": {
                DetectorId.SHIFT: 48.5
            },
            "dET/reSCale/FIT   23 34.6 ": {
                DetectorId.RESCALE_FIT: det_fit_range(start=23, stop=34.6, use_fit=True)
            },
            "dEt/SHIFT/FIT 235.2  341   ": {
                DetectorId.SHIFT_FIT: det_fit_range(start=235.2, stop=341, use_fit=True)
            }
        }
    ...
    def test_that_detector_setting_is_parsed_correctly(self):
        valid_settings = {
            "Det/CORR/REAR/X 123": {
                DetectorId.CORRECTION_X: single_entry_with_detector(entry=123, detector_type=DetectorType.LAB)
            },
            # noqa
            "DEt/CORR/ frOnt/X +95.7": {
                DetectorId.CORRECTION_X: single_entry_with_detector(entry=95.7, detector_type=DetectorType.HAB)
            },
            "DeT/ CORR / ReAR/ y 12.3": {
                DetectorId.CORRECTION_Y: single_entry_with_detector(entry=12.3, detector_type=DetectorType.LAB)
            },
            " DET/CoRR/fROnt/Y -957": {
                DetectorId.CORRECTION_Y: single_entry_with_detector(entry=-957, detector_type=DetectorType.HAB)
            },
            "DeT/ CORR /reAR/Z 12.3": {
                DetectorId.CORRECTION_Z: single_entry_with_detector(entry=12.3, detector_type=DetectorType.LAB)
            },
            " DET/CoRR/FRONT/ Z -957": {
                DetectorId.CORRECTION_Z: single_entry_with_detector(entry=-957, detector_type=DetectorType.HAB)
            },
            "DeT/ CORR /reAR/SIDE 12.3": {
                DetectorId.CORRECTION_TRANSLATION: single_entry_with_detector(entry=12.3,
                                                                              detector_type=DetectorType.LAB)
            },
            " DET/CoRR/FRONT/ SidE -957": {
                DetectorId.CORRECTION_TRANSLATION: single_entry_with_detector(entry=-957,
    ...
    def test_that_DET_OVERLAP_option_is_parsed_correctly(self):
        valid_settings = {
            "DET/OVERLAP 0.13 0.15": {
                DetectorId.MERGE_RANGE: det_fit_range(start=0.13, stop=0.15, use_fit=True)
            },
            "DeT/OverLAP 0.13 0.15": {
                DetectorId.MERGE_RANGE: det_fit_range(start=0.13, stop=0.15, use_fit=True)
            }
...
class LimitParserTest(unittest.TestCase):
    ...
    def test_that_angle_limit_is_parsed_correctly(self):
        valid_settings = {
            "L/PhI 123   345.2": {
                LimitsId.ANGLE: mask_angle_entry(min=123, max=345.2, use_mirror=True)
            },
            "L/PHI / NOMIRROR 123 -345.2": {
                LimitsId.ANGLE: mask_angle_entry(min=123, max=-345.2, use_mirror=False)
            }
        }
    ...
    def test_that_event_time_limit_is_parsed_correctly(self):
        valid_settings = {
            "L  / EVEnTStime 0,-10,32,434,34523,35": {
                LimitsId.EVENTS_BINNING: "0.0,-10.0,32.0,434.0,34523.0,35.0"
            },
            "L / Eventstime 0 -10 32 434 34523 35": {
                LimitsId.EVENTS_BINNING: "0.0,-10.0,32.0,434.0,34523.0,35.0"
            }
    ...
    def test_that_cut_limits_are_parsed_correctly(self):
        valid_settings = {
            "L/Q/RCUT 234.4": {
                LimitsId.RADIUS_CUT: 234.4
            },
            "L /q / RcUT -234.34": {
                LimitsId.RADIUS_CUT: -234.34
            },
            "l/Q/WCUT 234.4": {
                LimitsId.WAVELENGTH_CUT: 234.4
            },
            "L /q / wcUT -234.34": {
                LimitsId.WAVELENGTH_CUT: -234.34
            }
        }
    ...
    def test_that_radius_limits_are_parsed_correctly(self):
        valid_settings = {
            "L/R 234 235": {
                LimitsId.RADIUS: range_entry(start=234, stop=235)
            },
            "L / r   -234   235": {
                LimitsId.RADIUS: range_entry(start=-234, stop=235)
            },
    ...
    def test_that_q_limits_are_parsed_correctly(self):
        valid_settings = {
            "L/Q 12 34": {
                LimitsId.Q: q_rebin_values(min=12., max=34., rebin_string="12.0,34.0")
            },
            "L/Q 12 34 2.7": {
                LimitsId.Q: q_rebin_values(min=12., max=34., rebin_string="12.0,2.7,34.0")
            },
            "L/Q -12 34.6 2.7/LOG": {
                LimitsId.Q: q_rebin_values(min=-12., max=34.6, rebin_string="-12.0,-2.7,34.6")
            },
            "L/q -12 3.6 2 /LIN": {
                LimitsId.Q: q_rebin_values(min=-12., max=3.6, rebin_string="-12.0,2.0,3.6")
            },
            "L/q -12   0.41  23 -34.8 3.6": {
                LimitsId.Q: q_rebin_values(min=-12., max=3.6, rebin_string="-12.0,0.41,23.0,"
                                           "-34.8,3.6")
            },
            # noqa
            "L/q -12   0.42  23 -34.8 3.6 /LIn": {
                LimitsId.Q: q_rebin_values(min=-12., max=3.6, rebin_string="-12.0,0.42,"
                                           "23.0,34.8,"
                                           "3.6")
            },
            "L/q -12   0.43     23 -34.8 3.6": {
                LimitsId.Q: q_rebin_values(min=-12., max=3.6, rebin_string="-12.0,0.43,"
                                           "23.0,-34.8,"
                                           "3.6")
            },
            "L/q -12   0.44  23  ,34.8,3.6  /Log": {
                LimitsId.Q: q_rebin_values(min=-12., max=3.6, rebin_string="-12.0,"
                                           "-0.44,"
                                           "23.0,"
                                           "-34.8,3.6")
            },
            "L/q -12  , 0.45 , 23  ,34.8 ,3.6, .123, 5.6  /Log": {
                LimitsId.Q: q_rebin_values(min=-12.,
                                           max=5.6,
                                           rebin_string="-12.0,-0.45,23.0,"
                                           "-34.8,3.6,"
                                           "-0.123,5.6")
            },
            "L/q -12  , 0.46 , 23  ,34.8 ,3.6, -.123, 5.6": {
                LimitsId.Q: q_rebin_values(min=-12., max=5.6, rebin_string="-12.0,0.46,23.0,"
                                           "34.8,3.6,"
                                           "-0.123,5.6")
            },
            "L/q -12   0.47   23 34.8  3.6, -.123    5.6": {
                LimitsId.Q: q_rebin_values(min=-12., max=5.6, rebin_string="-12.0,0.47,23.0,"
                                           "34.8,3.6,"
                                           "-0.123,5.6")
            }
        }
    ...
    def test_that_qxy_limits_are_parsed_correctly(self):
        valid_settings = {
            "L/QXY 12 34": {
                LimitsId.QXY: simple_range(start=12, stop=34, step=None, step_type=None)
            },
            "L/QXY 12 34 2.7": {
                LimitsId.QXY: simple_range(start=12, stop=34, step=2.7, step_type=RangeStepType.LIN)
            },
            "L/QXY -12 34.6 2.7/LOG": {
                LimitsId.QXY: simple_range(start=-12, stop=34.6, step=2.7, step_type=RangeStepType.LOG)
            },
            "L/qxY -12 3.6 2 /LIN": {
                LimitsId.QXY: simple_range(start=-12, stop=3.6, step=2, step_type=RangeStepType.LIN)
            }
        }
        """
    ...
    def test_that_wavelength_limits_are_parsed_correctly(self):
        valid_settings = {
            "L/WAV 12 34": {
                LimitsId.WAVELENGTH: simple_range(start=12, stop=34, step=None, step_type=None)
            },
            "L/waV 12 34 2.7": {
                LimitsId.WAVELENGTH: simple_range(start=12, stop=34, step=2.7, step_type=RangeStepType.LIN)
            },
            "L/wAv -12 34.6 2.7/LOG": {
                LimitsId.WAVELENGTH: simple_range(start=-12, stop=34.6, step=2.7, step_type=RangeStepType.LOG)
            },
            "L/WaV -12 3.6 2 /LIN": {
                LimitsId.WAVELENGTH: simple_range(start=-12, stop=3.6, step=2, step_type=RangeStepType.LIN)
            }
        }
...
class MaskParserTest(unittest.TestCase):
    ...
    def test_that_masked_line_is_parsed_correctly(self):
        valid_settings = {
            "MASK/LiNE 12  23.6": {
                MaskId.LINE: mask_line(width=12, angle=23.6, x=None, y=None)
            },
            "MASK/LiNE 12  23.6 2 346": {
                MaskId.LINE: mask_line(width=12, angle=23.6, x=2, y=346)
            }
        }
        invalid_settings = {
    ...
    def test_that_masked_time_is_parsed_correctly(self):
        valid_settings = {
            "MASK/TIME 23 35": {
                MaskId.TIME: range_entry_with_detector(start=23, stop=35, detector_type=None)
            },
            "MASK/T 23 35": {
                MaskId.TIME: range_entry_with_detector(start=23, stop=35, detector_type=None)
            },
            "MASK/REAR/T 13 35": {
                MaskId.TIME_DETECTOR: range_entry_with_detector(start=13, stop=35, detector_type=DetectorType.LAB)
            },
            "MASK/FRONT/TIME 33 35": {
                MaskId.TIME_DETECTOR: range_entry_with_detector(start=33, stop=35, detector_type=DetectorType.HAB)
            },
            "MASK/TIME/REAR 13 35": {
                MaskId.TIME_DETECTOR: range_entry_with_detector(start=13, stop=35, detector_type=DetectorType.LAB)
            },
            "MASK/T/FRONT 33 35": {
                MaskId.TIME_DETECTOR: range_entry_with_detector(start=33, stop=35, detector_type=DetectorType.HAB)
    ...
    def test_that_clear_mask_is_parsed_correctly(self):
        ...
        invalid_settings = {
            "MASK/CLEAR/TIME/test": RuntimeError,
    ...
    def test_that_single_spectrum_mask_is_parsed_correctly(self):
        valid_settings = {
            "MASK S 12  ": {
                MaskId.SINGLE_SPECTRUM_MASK: 12
            },
            "MASK S234": {
                MaskId.SINGLE_SPECTRUM_MASK: 234
            }
        }
    ...
    def test_that_single_spectrum_range_is_parsed_correctly(self):
        valid_settings = {
            "MASK S 12 >  S23  ": {
                MaskId.SPECTRUM_RANGE_MASK: range_entry(start=12, stop=23)
            },
            "MASK S234>S1234": {
                MaskId.SPECTRUM_RANGE_MASK: range_entry(start=234, stop=1234)
            }
        }
    ...
    def test_that_single_vertical_strip_mask_is_parsed_correctly(self):
        valid_settings = {
            ...
            "MASK/mAin V234": {
                MaskId.VERTICAL_SINGLE_STRIP_MASK: single_entry_with_detector(entry=234, detector_type=DetectorType.LAB)
            },
            "MASK / LaB V  234": {
                MaskId.VERTICAL_SINGLE_STRIP_MASK: single_entry_with_detector(entry=234, detector_type=DetectorType.LAB)
            },
            ...
            "MASK/HAB V234": {
                MaskId.VERTICAL_SINGLE_STRIP_MASK: single_entry_with_detector(entry=234, detector_type=DetectorType.HAB)
            }
        }
    ...
    def test_that_range_vertical_strip_mask_is_parsed_correctly(self):
        valid_settings = {
            "MASK V  12 >  V23  ": {
                MaskId.VERTICAL_RANGE_STRIP_MASK: range_entry_with_detector(start=12,
                                                                            stop=23,
                                                                            detector_type=DetectorType.LAB)
            },
            "MASK V123>V234": {
                MaskId.VERTICAL_RANGE_STRIP_MASK: range_entry_with_detector(start=123,
                                                                            stop=234,
                                                                            detector_type=DetectorType.LAB)
            },
            "MASK / Rear V123>V234": {
    ...
    def test_that_single_horizontal_strip_mask_is_parsed_correctly(self):
        valid_settings = {
            "MASK H 12  ": {
                MaskId.HORIZONTAL_SINGLE_STRIP_MASK: single_entry_with_detector(entry=12,
                                                                                detector_type=DetectorType.LAB)
            },
            "MASK / Rear H  12  ": {
                MaskId.HORIZONTAL_SINGLE_STRIP_MASK: single_entry_with_detector(entry=12,
            ...
            "MASK/mAin H234": {
                MaskId.HORIZONTAL_SINGLE_STRIP_MASK: single_entry_with_detector(entry=234,
                                                                                detector_type=DetectorType.LAB)
            },
            "MASK / LaB H  234": {
                MaskId.HORIZONTAL_SINGLE_STRIP_MASK: single_entry_with_detector(entry=234,
    ...
    def test_that_range_horizontal_strip_mask_is_parsed_correctly(self):
        valid_settings = {
            "MASK H  12 >  H23  ": {
                MaskId.HORIZONTAL_RANGE_STRIP_MASK: range_entry_with_detector(start=12,
                                                                              stop=23,
                                                                              detector_type=DetectorType.LAB)
            },
            "MASK H123>H234": {
                MaskId.HORIZONTAL_RANGE_STRIP_MASK: range_entry_with_detector(start=123,
                                                                              stop=234,
                                                                              detector_type=DetectorType.LAB)
            },
            "MASK / Rear H123>H234": {
    ...
    def test_that_block_mask_is_parsed_correctly(self):
        valid_settings = {
            "MASK H12>H23 + V14>V15 ": {
                MaskId.BLOCK: mask_block(horizontal1=12,
                                         horizontal2=23,
                                         vertical1=14,
                                         vertical2=15,
                                         detector_type=DetectorType.LAB)
            ...
            "MASK/ HAB V12>V23 + H14>H15 ": {
                MaskId.BLOCK: mask_block(horizontal1=14,
                                         horizontal2=15,
                                         vertical1=12,
                                         vertical2=23,
                                         detector_type=DetectorType.HAB)
            ...
            "MASK  V12 + H 14": {
                MaskId.BLOCK_CROSS: mask_block_cross(horizontal=14, vertical=12, detector_type=DetectorType.LAB)
            },
            "MASK/HAB H12 + V 14": {
                MaskId.BLOCK_CROSS: mask_block_cross(horizontal=12, vertical=14, detector_type=DetectorType.HAB)
            }
        }
...
class SetParserTest(unittest.TestCase):
    ...
    def test_that_centre_is_parsed_correctly(self):
        valid_settings = {
            "SET centre 23 45": {
                SetId.CENTRE: position_entry(pos1=23, pos2=45, detector_type=DetectorType.LAB)
            },
            "SET centre /main 23 45": {
                SetId.CENTRE: position_entry(pos1=23, pos2=45, detector_type=DetectorType.LAB)
            },
            "SET centre / lAb 23 45": {
                SetId.CENTRE: position_entry(pos1=23, pos2=45, detector_type=DetectorType.LAB)
            },
            "SET centre / hAb 23 45": {
                SetId.CENTRE_HAB: position_entry(pos1=23, pos2=45, detector_type=DetectorType.HAB)
            },
            "SET centre /FRONT 23 45": {
                SetId.CENTRE_HAB: position_entry(pos1=23, pos2=45, detector_type=DetectorType.HAB)
            },
            "SET centre /FRONT 23 45 55 67": {
                SetId.CENTRE_HAB: position_entry(pos1=23, pos2=45, detector_type=DetectorType.HAB)
            },
        }
...
class TransParserTest(unittest.TestCase):
    ...
    def test_mon_shift_is_parsed_correctly(self):
        valid_settings = {
            "TRANS/SHIFT = 4000 5": {
                TransId.SPEC_5_SHIFT: 4000
            },
            "TRANS/SHIFT=4000 4": {
                TransId.SPEC_4_SHIFT: 4000
            },
            "TRANS/SHIFT=4000 5": {
                TransId.SPEC_5_SHIFT: 4000
            },
            "TRANS/SHIFT=-100 4": {
                TransId.SPEC_4_SHIFT: -100
            },
            "TRANS/ SHIFT=4000 5": {
                TransId.SPEC_5_SHIFT: 4000
            },
    ...
    def test_that_trans_spec_shift_is_parsed_correctly(self):
        valid_settings = {
            "TRANS/TRANSPEC=4/SHIFT=23": {
                TransId.SPEC_4_SHIFT: 23,
                TransId.SPEC: 4
            },
            "TRANS/TRANSPEC =4/ SHIFT = 23": {
                TransId.SPEC_4_SHIFT: 23,
    ...
    def test_that_roi_is_parsed_correctly(self):
        valid_settings = {
            "TRANS/ROI =testFile.xml": {
                TransId.ROI: ["testFile.xml"]
            },
            "TRANS/ROI =testFile.xml, "
            "TestFile2.XmL,testFile4.xml": {
                TransId.ROI: ["testFile.xml", "TestFile2.XmL", "testFile4.xml"]
            }
        }
        invalid_settings = {
    ...
    def test_that_mask_is_parsed_correctly(self):
        valid_settings = {
            "TRANS/Mask =testFile.xml": {
                TransId.MASK: ["testFile.xml"]
            },
            "TRANS/ MASK =testFile.xml, "
            "TestFile2.XmL,testFile4.xml": {
                TransId.MASK: ["testFile.xml", "TestFile2.XmL", "testFile4.xml"]
            }
        }
        invalid_settings = {
    ...
    def test_that_workspaces_are_parsed_correctly(self):
        valid_settings = {
            "TRANS/SampleWS =testworksaoe234Name": {
                TransId.SAMPLE_WORKSPACE: "testworksaoe234Name"
            },
            "TRANS/ SampleWS = testworksaoe234Name": {
                TransId.SAMPLE_WORKSPACE: "testworksaoe234Name"
            },
            "TRANS/ CanWS =testworksaoe234Name": {
                TransId.CAN_WORKSPACE: "testworksaoe234Name"
            },
            "TRANS/ CANWS = testworksaoe234Name": {
                TransId.CAN_WORKSPACE: "testworksaoe234Name"
            }
    ...
    def test_that_tube_calibration_file_is_parsed_correctly(self):
        valid_settings = {
            "TUBECALIbfile= calib_file.nxs": {
                TubeCalibrationFileId.FILE: "calib_file.nxs"
            },
            " tUBECALIBfile=  caAlib_file.Nxs": {
                TubeCalibrationFileId.FILE: "caAlib_file.Nxs"
            }
        }
...
class QResolutionParserTest(unittest.TestCase):
    ...
    def test_that_q_resolution_float_values_are_parsed_correctly(self):
        valid_settings = {
            "QRESOL/deltaR = 23.546": {
                QResolutionId.DELTA_R: 23.546
            },
            "QRESOL/ Lcollim = 23.546": {
                QResolutionId.COLLIMATION_LENGTH: 23.546
            },
            "QRESOL/ a1 = 23.546": {
                QResolutionId.A1: 23.546
            },
            "QRESOL/ a2 =  23": {
                QResolutionId.A2: 23
            },
            "QRESOL /  H1 = 23.546 ": {
                QResolutionId.H1: 23.546
            },
            "QRESOL /h2 = 23.546 ": {
                QResolutionId.H2: 23.546
            },
            "QRESOL /  W1 = 23.546 ": {
                QResolutionId.W1: 23.546
            },
            "QRESOL /W2 = 23.546 ": {
                QResolutionId.W2: 23.546
            }
        }
...
class FitParserTest(unittest.TestCase):
    ...
    def test_that_general_fit_is_parsed_correctly(self):
        valid_settings = {
            "FIT/ trans / LIN 123 3556": {
                FitId.GENERAL: fit_general(start=123,
                                           stop=3556,
            ...
            "FIT/Trans/ Log": {
                FitId.GENERAL: fit_general(
                    start=None,
                    stop=None,
                    fit_type=FitType.LOGARITHMIC,  # noqa
                    data_type=None,
                    polynomial_order=0)
            },
            ...
            "FIT/Trans/Sample/Log 23.4 56.7": {
                FitId.GENERAL: fit_general(start=23.4,
                                           stop=56.7,
                                           fit_type=FitType.LOGARITHMIC,
            ...
            "FIT/Trans / can/polynomiAL 5 23 45": {
                FitId.GENERAL: fit_general(start=23,
                                           stop=45,
                                           fit_type=FitType.POLYNOMIAL,
                                           data_type=DataType.CAN,
                                           polynomial_order=5)
    ...
    def test_that_monitor_times_are_parsed_correctly(self):
        valid_settings = {
            "FIT/monitor 12 34.5": {
                FitId.MONITOR_TIMES: range_entry(start=12, stop=34.5)
            },
            "Fit / Monitor 12.6 34.5": {
                FitId.MONITOR_TIMES: range_entry(start=12.6, stop=34.5)
            }
        }
...
class CompatibilityParserTest(unittest.TestCase):
    ...
    def test_that_compatibility_on_off_is_parsed_correctly(self):
        valid_settings = {
            "COMPATIBILITY on ": {
                OtherId.USE_COMPATIBILITY_MODE: True
            },
            "COMPATIBILITY   OFF ": {
                OtherId.USE_COMPATIBILITY_MODE: False
            }
        }
...
class MaskFileParserTest(unittest.TestCase):
    ...
    def test_that_gravity_on_off_is_parsed_correctly(self):
        valid_settings = {
            "MaskFile= test.xml,   testKsdk2.xml,tesetlskd.xml": {
                MaskId.FILE: ["test.xml", "testKsdk2.xml", "tesetlskd.xml"]
            }
        }

        invalid_settings = {
            "MaskFile=": RuntimeError,
...
class MonParserTest(unittest.TestCase):
    ...
    def test_that_length_is_parsed_correctly(self):
        valid_settings = {
            "MON/length= 23.5 34": {
                MonId.LENGTH: monitor_length(length=23.5, spectrum=34, interpolate=False)
            },
            "MON/length= 23.5 34  / InterPolate": {
                MonId.LENGTH: monitor_length(length=23.5, spectrum=34, interpolate=True)
            }
        }
    ...
    def test_that_flat_files_are_parsed_correctly(self):
        valid_settings = {
            ...
            "MON/ flAt  = filE.Ext ": {
                MonId.FLAT: monitor_file(file_path="filE.Ext", detector_type=DetectorType.LAB)
            },
    ...
    def test_that_hab_files_are_parsed_correctly(self):
        valid_settings = {
            ...
            "MON/ hAB  = filE.Ext ": {
                MonId.DIRECT: [monitor_file(file_path="filE.Ext", detector_type=DetectorType.HAB)]
            },
    ...
    def test_that_hab_files_are_parsed_correctly2(self):
        valid_settings = {
            "MON/Spectrum = 123 ": {
                MonId.SPECTRUM: monitor_spectrum(spectrum=123, is_trans=False, interpolate=False)
            },
            "MON/trans/Spectrum = 123 ": {
                MonId.SPECTRUM: monitor_spectrum(spectrum=123, is_trans=True, interpolate=False)
            },
            "MON/trans/Spectrum = 123 /  interpolate": {
                MonId.SPECTRUM: monitor_spectrum(spectrum=123, is_trans=True, interpolate=True)
            },
            "MON/Spectrum = 123 /  interpolate": {
                MonId.SPECTRUM: monitor_spectrum(spectrum=123, is_trans=False, interpolate=True)
            }
        }
        invalid_settings = {}
...
class PrintParserTest(unittest.TestCase):
    ...
    def test_that_print_is_parsed_correctly(self):
        valid_settings = {
            "PRINT OdlfP slsk 23lksdl2 34l": {
                PrintId.PRINT_LINE: "OdlfP slsk 23lksdl2 34l"
            },
            "PRiNt OdlfP slsk 23lksdl2 34l": {
                PrintId.PRINT_LINE: "OdlfP slsk 23lksdl2 34l"
            },
...
class BackParserTest(unittest.TestCase):
    ...
    def test_that_single_monitors_is_parsed_correctly(self):
        valid_settings = {
            "BACK / M3 /times  123 34": {
                BackId.SINGLE_MONITORS: back_single_monitor_entry(monitor=3, start=123, stop=34)
            },
            "BACK / M3 123 34": {
                BackId.SINGLE_MONITORS: back_single_monitor_entry(monitor=3, start=123, stop=34)
            }
    ...
    def test_that_trans_mon_is_parsed_correctly(self):
        valid_settings = {
            "BACK / TRANS  123 344": {
                BackId.TRANS: range_entry(start=123, stop=344)
            },
            "BACK / tranS 123 34": {
                BackId.TRANS: range_entry(start=123, stop=34)
            }
        }
...
class UserFileParserTest(unittest.TestCase):
    def test_that_correct_parser_is_selected_(self):
        ...
        assert_valid_result(result, {
            FitId.GENERAL: fit_general(
                start=123, stop=3556, fit_type=FitType.LINEAR, data_type=None, polynomial_order=0)
        }, self.assertTrue)
