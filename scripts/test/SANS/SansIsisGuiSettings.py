...
class Sans2DIsisGuiSettings(unittest.TestCase):
    ...
    def test_read_set_radius(self):  # GUI: rad_min, rad_max
        ...
        i.ReductionSingleton().user_settings.readLimitValues('L/R %f %f' % (min_value, max_value),
    ...
    def test_fit(self):
        ...
        def checkNotEqualOption(option, selector):
            self.assertTrue(
                not i.ReductionSingleton().transmission_calculator.fitMethod(selector) == str(option[0]).upper())
            if (option[1] is not None):
    ...
    def test_scaling_options(self):
        ...
        def testScalingValues(scale, shift, qMin=None, qMax=None, fitScale=False, fitShift=False):
            ...
            if qMin is not None:
                self.assertTrue(
                    i.ReductionSingleton().instrument.getDetector('FRONT').rescaleAndShift.qRangeUserSelected)
