...
class TextureCorrectionModelTest(unittest.TestCase):
    ...
    @patch(correction_model_path + ".ConvertUnits")
    @patch(correction_model_path + ".CloneWorkspace")
    def test_calc_divergence(self, mock_clone_ws, mock_convert, mock_ads, mock_get_thetas):
        ...
        y_calls = mock_out_ws.setY.call_args_list
        self.assertEqual(len(y_calls), 2)

        e_calls = mock_out_ws.setE.call_args_list
        self.assertEqual(len(e_calls), 2)

        for i, call_args in enumerate(y_calls):
