...
class TextureCorrectionModelTest(unittest.TestCase):
    ...
    @patch(correction_model_path + ".TextureCorrectionModel.calc_scattering_centres")
    ...
    def test_calc_all_corrections(
        ...
        mock_calc_scattering_centres.assert_called_once_with(self.mock_ws, abs_args["gauge_vol_preset"])
...
class TextureCorrectionModelScatteringCentreTest(unittest.TestCase):
    ...
    def test_calc_scattering_centres_attaches_a_centre_per_detector(self):
        DefineGaugeVolume(self.ws, get_cube_xml("gauge", 0.004))

        self.model.calc_scattering_centres(self.ws_name)
    ...
    def test_calc_scattering_centres_without_a_gauge_volume_uses_the_whole_sample(self):
        self.model.calc_scattering_centres(self.ws_name)

        run = ADS.retrieve(self.ws_name).getRun()
        self.assertEqual(len(run.getProperty("ScatteringCentreDetIDs").value), self.ws.getNumberHistograms())

    @patch(correction_model_path + ".logger")
    def test_calc_scattering_centres_warns_and_attaches_nothing_on_failure(self, mock_logger):
        ...
        self.model.calc_scattering_centres("_no_material_ws")
