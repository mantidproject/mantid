...
class ISISPowderCommonTest(unittest.TestCase):
    ...
    def test_subtract_summed_runs(self):
        ...
        returned_ws = common.subtract_summed_runs(ws_to_correct=no_scale_ws, instrument=MockPolarisInstrument(),
                                                  empty_sample_ws_string=sample_empty_number)
        y_values = returned_ws.readY(0)
        for i in range(0, returned_ws.blocksize(), 1000):
            self.assertAlmostEqual(y_values[i], 0)

        # Check what happens when we specify scale as a half
        scaled_ws = common.subtract_summed_runs(ws_to_correct=original_ws, instrument=MockPolarisInstrument(),
                                                scale_factor=0.75, empty_sample_ws_string=sample_empty_number)
        scaled_y_values = scaled_ws.readY(0)
        self.assertAlmostEqual(scaled_y_values[1100], 485.25)
        self.assertAlmostEqual(scaled_y_values[5000], 134.75)
        self.assertAlmostEqual(scaled_y_values[7700], 2.5)
...
class MockPolarisInstrument(object):
