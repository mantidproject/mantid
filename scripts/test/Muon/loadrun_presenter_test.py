...
class LoadRunWidgetPresenterTest(unittest.TestCase):
    ...
    # All validation of user input is handled by a Regex which is tested elsewhere
    # so we assume user can only enter valid runs

    def test_user_can_load_single_run_in_single_file_mode(self):
        view = LoadRunWidgetView()
        model = LoadRunWidgetModel(MuonLoadData())
        presenter = LoadRunWidgetPresenter(view, model)
        presenter.enable_multiple_files(False)

        model.load_workspace_from_filename = mock.Mock(return_value=([1, 2, 3],
                                                                     "EMU00001234.nxs",
                                                                     1234))
        view.set_run_edit_text("1234")

        presenter.handle_run_changed_by_user()
        self.Runner(presenter._load_thread)

        self.assertEqual(presenter.filenames, ["EMU00001234.nxs"])
        self.assertEqual(presenter.runs, [1234])
        self.assertEqual(presenter.workspaces, [[1, 2, 3]])

    def test_data_cleared_and_warning_message_displayed_if_user_enters_multiple_files_in_single_file_mode(self):
        view = LoadRunWidgetView()
        model = LoadRunWidgetModel()
        presenter = LoadRunWidgetPresenter(view, model)
        presenter.enable_multiple_files(False)
        view.warning_popup = mock.Mock()

        view.set_run_edit_text("1234,1235,1236")
        ...
        view.warning_popup.assert_called_once()

    def test_user_input_run_number_initiates_a_load(self):
        pass
...
class LoadRunWidgetIncrementDecrementSingleFileModeTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.model.load_workspace_from_filename = mock.Mock(return_value=(self._loaded_workspace,
    ...
    def test_that_decrement_run_attempts_to_load_the_correct_run(self):
        ...
        fname, _ = self.model.load_workspace_from_filename.call_args
        self.assertEqual(os.path.basename(fname[0]), new_filename)
    ...
    def test_that_decrement_run_loads_the_data_correctly_by_overwriting_previous(self):
    ...
    def test_that_increment_run_loads_the_data_correctly_by_overwriting_previous(self):
        new_run = self._loaded_run + 1
        new_filename = "EMU00001235.nxs"
        self.model.load_workspace_from_filename = mock.Mock(return_value=([1], new_filename, new_run))
...
#
# import Muon.GUI.Common.message_box as box
# box.warning = mock.Mock()
...
class LoadRunWidgetLoadCurrentRunTest(unittest.TestCase):
    ...
    def test_load_current_run_clears_data_if_fails_to_load(self):
