...
class EAPlotDataPaneModelTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        test_group = EAGroup(group_name="9999; Detector 1", detector="Detector 1", run_number="9999")
        self.context.group_context._groups = [test_group]
        self.context.group_context._selected_groups = ["9999; Detector 1"]

    def test_get_workspace_to_plot_counts(self):
        self.context.group_context["9999; Detector 1"].get_counts_workspace_for_run = mock.MagicMock()

        self.model.get_count_workspaces_to_plot("9999; Detector 1", True)

        self.context.group_context["9999; Detector 1"].get_counts_workspace_for_run.assert_called_once()

    def test_get_workspace_to_plot_counts_rebin(self):
        group = self.context.group_context["9999; Detector 1"]
        ...
        self.model.get_count_workspaces_to_plot("9999; Detector 1", False)
    ...
    def test_get_workspace_and_indices_for_group_or_pair_returns_correctly(self):
        self.model.get_count_workspaces_to_plot = mock.Mock(return_value=["9999; Detector 1"])
        expected_workspaces = ["9999; Detector 1"]
        expected_indices = [2]

        workspaces, indices = self.model.get_workspace_and_indices_for_group("9999; Detector 1", True, "Total")
