...
class SData(GeneralData):
    ...
    def set(self, items=None):
        ...
        for item in items["atoms_data"]:
            ...
            if sorted(items["atoms_data"][item].keys()) != sorted(AbinsParameters.all_keywords_atoms_s_data):
                raise ValueError("Invalid structure of the dictionary.")

            if not items["atoms_data"][item]["symbol"] in AbinsParameters.all_symbols:
