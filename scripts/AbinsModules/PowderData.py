...
class PowderData(GeneralData):
    ...
    def __init__(self, temperature=None, num_atoms=None):
        ...
        self._data = {"msd": np.zeros(self._num_atoms, dtype=Constants.float_type), # value of mean square displacements
                      "dw":  np.zeros(self._num_atoms, dtype=Constants.float_type)} # Debye-Waller factor for that mean square displacements
    ...
    def _append(self, num_atom=None, powder_atom=None):
        ...
        if not isinstance(powder_atom, dict):
            raise ValueError("Invalid value. Dictionary with the following entries : %s"%Constants.all_keyword_msd_data + " was expected")

        if sorted(powder_atom.keys()) != sorted(Constants.all_keyword_msd_data):
    ...
    def _check_items(self, items=None):
         ...
         if sorted(items.keys()) != sorted(Constants.all_keyword_msd_data):
            raise ValueError("Invalid structure of the dictionary.")
