...
class FrequencyPowderGenerator(object):
    ...
    def rebin_array(self, array=None):
        ...
        if array.size < self._bin_size:
            ...
            return np.extract(array > AbinsConstants.acoustic_phonon_threshold, array)

        inds = np.digitize(array, self._bins)
        new_array = sorted(np.asarray([array[inds == i].mean() for i in range(1, self._bin_size)]))
        new_array = np.extract(np.isfinite(new_array), new_array) # remove nan from array
        # noinspection PyTypeChecker
        return np.extract(new_array > AbinsConstants.acoustic_phonon_threshold, new_array ) # remove acoustic frequencies
    ...
    def construct_freq_combinations(self, previous_array=None, fundamentals_array=None, quantum_order=None):
        ...
        if quantum_order == AbinsConstants.fundamentals or quantum_order == AbinsConstants.first_overtone:
