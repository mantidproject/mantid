...
class AtomsDaTa(GeneralData):
    ...
    def append(self, item=None):
        ...
        if not sorted(item.keys()) == sorted(Constants.all_keywords_atoms_data):
            raise ValueError("Invalid structure of the dictionary to be added.")

        # "symbol"
        if not item["symbol"] in Constants.all_symbols:
    ...
    def set(self, items=None):
        ...
        if len(list) != self._num_atoms:
            raise ValueError("Inconsistent size of new data and number of atoms.")
