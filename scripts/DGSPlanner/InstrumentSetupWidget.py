...
class InstrumentSetupWidget(QtWidgets.QWidget):
    ...
    def __init__(self, parent=None):
        ...
        #instrument selector
        self.instrumentList = [
            'ARCS', 'CHESS', 'CNCS', 'DNS', 'EXED', 'FOCUS', 'HET', 'HYSPEC', 'LET', 'MAPS', 'MARI', 'MERLIN', 'SEQUOIA'
        ]
    ...
    def updateFigure(self):
        ...
        for i in range(3):
            circle = numpy.array([
                mantid.kernel.Quat(0, 0, 0.5 * numpy.sin(t), 0.5 * numpy.cos(t))
                for t in numpy.arange(0, 1.51 * numpy.pi, 0.1 * numpy.pi)
            ])
    ...
    def loadMaskFromFile(self):
        fileName = QtWidgets.QFileDialog.getOpenFileName(self, "Open Mask File", '',
