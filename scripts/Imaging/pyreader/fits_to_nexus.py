import Fits_Reader as fits
import h5py
import numpy as np
import logging
import argparse

def fits_to_nexus(input_dir,prefix,white,dark,output_file):
	"""
	...
	print 'Reading the fits files in the directory'
	data, white, dark, theta = fits.fits_reader(input_dir,prefix,white,dark)
	try:
...
#dark = np.reshape(dark,(1,)+dark.shape)
		...
		print 'Warning: Cannot create a input.nxs temporary file in the data directory. input.nxs will speed up loading on next run'
