...
def _read_img(filename, file_extension=None):
    ...
    if file_extension in ['fits', 'fit']:
        ...
        raise ValueError(
            "Don't know how to load a file with this extension: {0}".format(
                file_extension))
...
def _read_listed_files(files, slice_img_shape, file_extension, dtype):
    ...
    """
    data = np.zeros(
        (len(files), slice_img_shape[0], slice_img_shape[1]), dtype=dtype)
...
def get_flat_dark_stack(field_path, field_prefix, file_prefix, file_extension,
...
def read_stack_of_images(sample_path,
...
def save_recon_as_vertical_slices(recon_data,
                                  output_dir,
                                  img_format='tiff',
                                  name_prefix='out_recon_slice',
                                  zero_fill=6):
    """
...
def save_recon_as_horizontal_slices(recon_data,
                                    out_horiz_dir,
                                    img_format='tiff',
                                    name_prefix='out_recon_horiz_slice',
                                    zero_fill=6):
    """
    ...
    for idx in range(0, recon_data.shape[1]):
        write_image(
            ...
            os.path.join(out_horiz_dir,
                         name_prefix + str(idx).zfill(zero_fill)),
            img_format=img_format,
            dtype='uint16')
...
def self_save_zipped_scripts(output_path, this_path):
    ...
    print("Saving myself (reconstruction scripts) from: {0} in: {1}".format(
        scripts_path, os.path.abspath(output_path)))
    import zipfile
    # os.path.join(output_path, ... )
    RECON_SCRIPTS_PKG_NAME = '0.reconstruction_scripts.zip'
    with zipfile.ZipFile(
            os.path.join(output_path, RECON_SCRIPTS_PKG_NAME), 'w',
