...
for i in workspace_list:
    Fit(Function='name=ExpDecayMuon,A=4306.05,Lambda=0.458289', InputWorkspace='%s' % i,
        IgnoreInvalidData=True, Output='%s_fit' % i, OutputCompositeMembers=True, ConvolveMembers=True)
...
for i in workspace_list:
    Fit(Function='name=ExpDecayMuon,A=4306.05,Lambda=0.458289', InputWorkspace='%s_fit' % i,
        IgnoreInvalidData=True, Output='%s_fit_fit' % i, OutputCompositeMembers=True, ConvolveMembers=True)
    RenameWorkspace(InputWorkspace='%s_fit_fit_Workspace' % i, OutputWorkspace='%s_fit' % i)
    Fit(Function='name=ExpDecayMuon,A=4306.05,Lambda=0.458289', InputWorkspace='%s_fit_rebin' % i,
        IgnoreInvalidData=True, Output='%s_fit_rebin_fit' % i, OutputCompositeMembers=True, ConvolveMembers=True)
