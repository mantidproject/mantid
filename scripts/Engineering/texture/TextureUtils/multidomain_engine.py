...
# MultiDomainFunction fitting engine for the texture peak-fitting workflow.
#
# The original iterative MultiDomainFunction fit: a summed-spectrum fit seeds the peak centre, then
# each (workspace, peak) is fit serially on progressively finer rebunched copies for SNR, with the
# shared broadening params tied across domains on the first fit and held fixed per-domain thereafter.
# Fits in TOF.  Retained for comparison/validation and for its unweighted-cost behaviour.
#
...
# ~~~~~~~~~~~~~~~~~~~~~ MultiDomainFunction per-spectrum fit setup ~~~~~~~~~~~~~~~~~~~~~
...
def get_initial_fit_function_and_kwargs_from_specs(
    ...
    # save the width parameter name for future constraints
    ...
    for ispec in ispecs:
        ...
        tof_peak = _d_to_tof(peak, diff_consts)
        tof_start = _d_to_tof(x_start, diff_consts)
        tof_end = _d_to_tof(x_end, diff_consts)
        ...
        x0_lower = _d_to_tof(x0_window[0], diff_consts)
        x0_upper = _d_to_tof(x0_window[1], diff_consts)
        ...
        peak_func.addConstraints(f"{x0_lower} < {_PEAK_CENTRE_PARAM} < {x0_upper}")
        peak_func.addConstraints(f"0 < {_PEAK_INTENSITY_PARAM} < {intens * 5}")
        if width_par_name:
    ...
    available_params = _param_names(base_peak_func)
...
def rerun_fit_with_new_ws(
    ...
    for idom in range(mdf.nFunctions()):
        ...
        intens = max(peak.getParameterValue(_PEAK_INTENSITY_PARAM), 1)
        ...
        if last_fit_ic and is_final:
            ...
            new_peak.setParameter(_PEAK_CENTRE_PARAM, x0)
        ...
        if not is_final:
            ...
            new_peak.addConstraints(f"{max(intens / 2, 1e-6)}<{_PEAK_INTENSITY_PARAM}<{intens * 2}")
        new_peak.addConstraints(f"{x0 * (1 - x0_frac_move)}<{_PEAK_CENTRE_PARAM}<{x0 * (1 + x0_frac_move)}")
...
# ~~~~~~~~~~~~~~~~~~~~~~~~ MultiDomainFunction fitting engine ~~~~~~~~~~~~~~~~~~~~~~~~


def _build_fit_ws_stack(
    ws_tof: Workspace2D,
    smooth_vals: Sequence[int],
    tied_bkgs: Sequence[bool],
    final_fit_raw: bool,
    last_fit_ic: bool,
    peak_func_name: str,
) -> Tuple[List, List[bool]]:
    """Build the ordered stack of workspaces to fit iteratively (progressively finer rebunched copies
    for SNR, then optionally the raw ws, then an extra raw copy when the final fit switches to
    IkedaCarpenterPV) together with the per-fit 'tie background' flags."""
    fit_wss = []
    bkg_is_tied = []
    ...
    # if the peak func isn't already Ikeda Carpenter, and last_fit_ic is true, add another ws_tof to be fit
    if last_fit_ic and peak_func_name != "IkedaCarpenterPV":
        ...
        bkg_is_tied.append(True)
    return fit_wss, bkg_is_tied


def _run_iterative_multidomain_fit(
    ws: Workspace2D,
    fit_wss: Sequence,
    bkg_is_tied: Sequence[bool],
    peak: float,
    x_window: Tuple[float, float],
    x0_window: Tuple[float, float],
    parameters_to_tie: Sequence[str] | None,
    subsequent_fit_param_fix: Sequence[str] | None,
    peak_func_name: str,
    bg_func_name: str,
    fit_kwargs: dict,
    max_fit_iters: int,
    last_fit_ic: bool,
) -> Tuple[Fit, Workspace2D, Sequence[float]]:
    """Fit the first (coarsest) workspace in the stack, then re-fit on each subsequent workspace,
    carrying the fitted function forward.  Returns the final Fit object, the last workspace fitted
    (used to retrieve its parameter table), and the per-spectrum intensity estimates from the
    initial fit setup."""
    ...
    initial_function, md_fit_kwargs, intensity_estimates = get_initial_fit_function_and_kwargs_from_specs(
        ws, fit_ws, peak, x_window, x0_window, parameters_to_tie, peak_func_name, bg_func_name, bkg_is_tied[fit_num]
    )
    fit_object = Fit(
    ...
    return fit_object, fit_ws, intensity_estimates


def _populate_multidomain_output_table(
    out_tab,
    ws: Workspace2D,
    fit_ws: Workspace2D,
    intensity_estimates: Sequence[float],
    fit_mask: np.ndarray,
    no_fit_value_dict: dict | None,
    nan_replacement: str | None,
) -> None:
    """Build the per-(ws, peak) output table (columns wsindex, I_est, then for each peak-function
    parameter p: p, p_err, p/p_err) from the flat multidomain Fit parameter table produced for
    fit_ws.  Spectra flagged False in fit_mask get default/unfit values.  X0 is converted from TOF
    back to d-spacing per spectrum (the multidomain engine fits in TOF)."""
...
def _fit_all_peaks_multidomain(
    wss: Sequence[str],
    ...
    """MultiDomainFunction-based (serial, per (ws, peak)) implementation of fit_all_peaks.

    The original iterative fit: a summed-spectrum fit seeds the peak centre, then each (ws, peak) is
    fit on progressively finer rebunched copies for SNR (tied-then-fixed broadening) before the final
    raw fit.  Fits in TOF.  Retained for comparison/validation and for the unweighted-cost behaviour."""
    ...
    for iws, wsname in enumerate(wss):
        ...
        for ipeak, peak in enumerate(peaks):
            ...
            # approach will be to use iterative fits and these iterations can have optionally 'rebunched' data to improve SNR
            fit_wss, bkg_is_tied = _build_fit_ws_stack(ws_tof, smooth_vals, tied_bkgs, final_fit_raw, last_fit_ic, peak_func_name)

            # low level information
            logger.information(f"Workspace: {wsname}, Peak: {peak}")

            # set up window bounds and the output table / file path
            ...
            # run the initial fit and any subsequent (finer / raw / IC) fits across the ws stack
            fit_object, fit_ws, intensity_estimates = _run_iterative_multidomain_fit(
                ws,
                fit_wss,
                bkg_is_tied,
                peak,
            ...
            *_, i_over_sigma, _ = calc_intens_and_sigma_arrays(fit_result)
            ...
            # populate and save the final table
            _populate_multidomain_output_table(out_tab, ws, fit_ws, intensity_estimates, fit_mask, no_fit_value_dict, nan_replacement)
