...
def fetch_correction_workspaces(vanadium_path, instrument, rb_num=""):
    ...
    if path.exists(curves_path) and path.exists(integ_path) and not force_recalc:  # Check if the cached files exist.
        try:
            ...
            if rb_num:
                user_integ, user_curves = _generate_saved_workspace_file_paths(vanadium_number, rb_num=rb_num)
            ...
            logger.error("Problem loading existing vanadium calculations. Creating new files. Description: " + str(e))
...
def _calculate_vanadium_correction(vanadium_path):
    ...
    try:
        ...
        logger.error("Error when loading vanadium sample data. "
                     "Could not run Load algorithm with vanadium run number: " + str(vanadium_path) +
...
def _save_correction_files(integration_workspace, integration_path, curves_workspace, curves_path):
    ...
    try:
        ...
        logger.error("Vanadium Correction files could not be saved to the filesystem. Description: " + str(e))
        return
...
def _generate_saved_workspace_file_paths(vanadium_number, rb_num=""):
    ...
    if rb_num:
        vanadium_dir = path.join(path_handling.get_output_path(), "User", rb_num, VANADIUM_DIRECTORY_NAME)
    else:
