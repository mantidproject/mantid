...
class PawleyPatternBase(MtdFuncMixin, OutputTableMixin, ABC):
    def __init__(
        self,
        ws: Workspace2D,
        phases: Phase | Sequence[Phase],
        profile: PeakProfile,
        bg_func: FunctionWrapper | None = None,
        global_param_bound_frac: float | None = None,
        param_bounds_frac: float | None = None,
    ...
    def eval_2d(self, params: np.ndarray[float]) -> Workspace2D:
        ...
        if self.apply_lorentz_correction:
            ...
            for ispec in range(ws_sim.getNumberHistograms()):
                ws_sim.setY(ispec, ws_sim.y(ispec) * np.sin(si.twoTheta(ispec) / 2))
...
<<<<<<< HEAD
                    ...
                    ws_sim.setSharedY(ispec, scale * ycalc + bg)
=======
...
>>>>>>> df975b5251d (iterate the scale and background it the 2d reconstruction)
            else:
                # Subsequent calls: both scale and bg are locked.
                for ispec in range(self.ws.getNumberHistograms()):
                    ws_sim.setSharedY(ispec, self.scales[ispec] * ws_sim.readY(ispec) + self.bgs[ispec])
        return ws_sim
...
class PawleyPattern2D(Poldi2DEvalMixin, PawleyPatternBase):
    ...
    def create_no_constriants_fit(self, global_param_bound_frac: float = None, param_bounds_abs_min: float = 1e-6) -> PawleyPattern2DNoConstraints:
