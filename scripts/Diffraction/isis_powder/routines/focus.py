...
def apply_per_detector_placzek(
    ...
    input_workspace = _apply_vanadium_corrections_per_detector(
        instrument=instrument, input_workspace=input_workspace, perform_vanadium_norm=perform_vanadium_norm, vanadium_splines=vanadium_path
    )
...
def _apply_vanadium_corrections_per_detector(
    instrument, input_workspace: Workspace2D, perform_vanadium_norm, vanadium_splines: Workspace2D
):

    input_workspace = mantid.ConvertUnits(InputWorkspace=input_workspace, OutputWorkspace=input_workspace, Target="TOF")
    # Remove Masked and Monitor spectra
    input_workspace, vanadium_splines = _prepare_for_correction(data_workspace=input_workspace, correction_workspace=vanadium_splines)
    processed_spectra = mantid.Divide(LHSWorkspace=input_workspace, RHSWorkspace=vanadium_splines, AllowDifferentNumberSpectra=True)
    processed_spectra = mantid.ReplaceSpecialValues(InputWorkspace=processed_spectra, NaNValue=0, InfinityValue=0)
    return processed_spectra
...
def _apply_placzek_corrections(
    ...
    input_workspace, self_scattering_correction = _prepare_for_correction(
        data_workspace=input_workspace, correction_workspace=self_scattering_correction
    )
