...
class WishPowder:
    def __init__(self, calib_dir, user_dir, sample_env=SAMPENV.STICK,
                 per_detector_van=False, van_runno=None, empty_runno=None, default_ext="raw", sum_equivalent_banks=False, subtract_empty=False):
        ...
        self.van_sample = {"Material": {"ChemicalFormula": "V", "AttenuationXSection": 4.8756,"ScatteringXSection": 5.16, "SampleNumberDensity": 0.07118},
                           "Geometry": {"Shape": "Cylinder", "Height": 4.0, "Radius": 0.15, "Center": [0.0, 0.0, 0.0]}}
        self.sample = {} # "Geometry": {"Shape": "Cylinder", "Center": [0.0, 0.0, 0.0]}}
        self.cylinder_abs_kwargs = {"NumberOfSlices": 10, "NumberOfAnnuli": 10, "NumberOfWavelengthPoints": 25,
                                    "ExpMethod": "Normal"}
    ...
    def _get_mask_filepath(self):
        return self._get_filepath_in_calib_dir(f"mask.xml")

    def _get_cal_filepath(self):
        return self._get_filepath_in_calib_dir(f"calibration.xml")
    ...
    def normalise_by_monitor(self, wsname):
        ...
        mantid.ReplaceSpecialValues(InputWorkspace=wsname, OutputWorkspace=wsname, NaNValue=0.0, NaNError=0.0,
    ...
    def correct_for_attenuation(self, wsname):
        ws_trans = mantid.CylinderAbsorption(InputWorkspace=wsname,
                                             OutputWorkspace=f"{wsname}_transmission",
                                             **self.cylinder_abs_kwargs)
    ...
    def _get_mask_ws(self, wsname):
        if self.mask_ws is None:
            self.mask_ws = mantid.LoadMask(Instrument="WISH", InputFile=self._get_mask_filepath(),
                                          ReferenceWorkspace=wsname, OutputWorkspace=wsname + "_mask")
        return self.mask_ws

    def get_group_ws(self, ws):
        if self.grp_ws is None:
            self.grp_ws = mantid.LoadDetectorsGroupingFile(InputFile=self.grp_ws_fpath, InputWorkspace=ws,
    ...
    def focus(self, wsname):
        ...
        if not self.per_detector:
            ...
            mantid.ReplaceSpecialValues(InputWorkspace=wsname_foc, OutputWorkspace=wsname_foc, NaNValue=0.0, NaNError=0.0,
                                        InfinityValue=0.0, InfinityError=0.0)
    ...
    def _replace_data_with_wavelength_focussed(wsname, ngroups=10):
        ...
        for igroup in range(ngroups):
            ...
            ws_specs = mantid.ExtractSpectra(InputWorkspace=wsname, OutputWorkspace=wsname + "_specs",
                                                 WorkspaceIndexList=ispecs, EnableLogging=False)
            # rebin spectra in group to match so can be summed
            [ws_specs.applyBinEdgesFromAnotherWorkspace(ws_specs, 0, ispec) for ispec in
             range(1, ws_specs.getNumberHistograms())]
    ...
    def _crop_focussed_in_dspac(ws_foc):
        ...
        for ispec in range(WishPowder.retreive(ws_foc).getNumberHistograms()):
            ws_foc = mantid.MaskBins(InputWorkspace=ws_foc, OutputWorkspace=ws_foc.name(),
    ...
    def _combine_focussed_banks(ws_foc):
        ws_foc_1to5 = mantid.ExtractSpectra(InputWorkspace=ws_foc, OutputWorkspace=f"{ws_foc.name()}_1to5",
                                            WorkspaceIndexList=range(0, 5))
        ws_foc_6to10 = mantid.ExtractSpectra(InputWorkspace=ws_foc, OutputWorkspace=f"{ws_foc.name()}_6to10",
                                             WorkspaceIndexList=range(5, 10))
        # ensure bin edges in equivalent banks are the same (could be different due to calibration)
        ws_foc_6to10 = mantid.RebinToWorkspace(WorkspaceToRebin=ws_foc_6to10, WorkspaceToMatch=ws_foc_1to5,
                                               OutputWorkspace=ws_foc_6to10.name())
