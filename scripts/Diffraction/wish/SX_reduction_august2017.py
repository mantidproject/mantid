    ...
    for p in Pklist:
        ...
        for j, k in enumerate(w):
            for i in range(0, k.rowCount()):
                ...
                if k.row(i)["Wavelength"] >= 1.1 and k.row(i)["Intens"] > 0 and ratio > 3.0 and k.row(i)["DSpacing"] >= 1.0:
                    d = (
                        k.row(i)["h"],
                        k.row(i)["k"],
                        k.row(i)["l"],
...
def ProcessVana(rnum, cycle):
    ...
    mantid.LoadRaw(
        Filename="/archive/NDXWISH/Instrument/data/cycle_" + cycle + "/WISH000" + str(rnum) + ".raw",
    ...
    mantid.SetSampleMaterial(
        "Vana", SampleNumberDensity=0.0719, ScatteringXSection=5.197, AttenuationXSection=4.739, ChemicalFormula="V0.95 Nb0.05"
    )
...
def Norm_data(rnum, cycle):
    ...
    mantid.CropWorkspace(InputWorkspace="WISH000" + str(rnum), OutputWorkspace="WISH000" + str(i), XMin=6000, XMax=99000)
    mantid.ConvertUnits(InputWorkspace="WISH000" + str(rnum), OutputWorkspace="WISH000" + str(rnum), Target="Wavelength")
    ...
    mantid.CropWorkspace(InputWorkspace="WISH000" + str(rnum), OutputWorkspace="WISH000" + str(rnum), XMin=0.75, XMax=9.3)
    mantid.RebinToWorkspace(WorkspaceToRebin="Vana_smoot1", WorkspaceToMatch="WISH000" + str(rnum), OutputWorkspace="Vana_smoot1")
...
def Find_Peaks(rnum, long_cell, threshold, MaxPeaks, centroid_radius, edge_pixel):
    ...
    mantid.FindPeaksMD(
        InputWorkspace="WISH000" + str(rnum) + "_MD",
...
def Find_UB_FFT(rnum, MinD, MaxD, Tolerance, Centering, CellType):
    mantid.FindUBUsingFFT(PeaksWorkspace="WISH000" + str(rnum) + "_find_peaks", MinD=MinD, MaxD=MaxD, Tolerance=Tolerance)
    mantid.SelectCellOfType(PeaksWorkspace="WISH000" + str(rnum) + "_find_peaks", Centering=Centering, Apply=True, CellType=CellType)
    ...
    mantid.OptimizeLatticeForCellType(
        PeaMinWavelengthksWorkspace="WISH000" + str(rnum) + "_find_peaks", Apply=True, CellType=CellType, Tolerance=Tolerance
    )


def Find_UB_Latt(rnum, a, b, c, alpha, beta, gamma, Tolerance, Centering, CellType):
    mantid.FindUBUsingLatticeParameters("WISH000" + str(rnum) + "_find_peaks", a=a, b=b, c=c, alpha=alpha, beta=beta, gamma=gamma)
    mantid.SelectCellOfType(PeaksWorkspace="WISH000" + str(rnum) + "_find_peaks", Centering=Centering, Apply=True, CellType=CellType)
    # OptimizeCrystalPlacement(PeaksWorkspace='WISH000'+str(rnum)+'_find_peaks', ModifiedPeaksWorkspace='WISH000'
    # +str(rnum)+'_find_peaks', AdjustSampleOffsets=True)
    mantid.OptimizeLatticeForCellType(
        PeaksWorkspace="WISH000" + str(rnum) + "_find_peaks", Apply=True, CellType=CellType, Tolerance=Tolerance
    )


def Predict_peak(rnum, MinWavelength, MinDSpacing, centroid_radius, ReflectionCondition):
    PredictPeaks(
        InputWorkspace="WISH000" + str(rnum) + "_find_peaks",
...
def Integrate_elips(rnum, RegionRadius, Scale, wDir, MinDSpacing, MinWavelength, UseOnePercentBackgroundCorrection):
    ...
    ConvertUnits(InputWorkspace="WISH000" + str(rnum) + "_Lorentz", OutputWorkspace="WISH000" + str(rnum) + "_Lorentz", Target="Wavelength")
    LorentzCorrection(InputWorkspace="WISH000" + str(rnum) + "_Lorentz", OutputWorkspace="WISH000" + str(rnum) + "_Lorentz")
    IntegrateEllipsoidsTwoStep(
        InputWorkspace="WISH000" + str(rnum) + "_Lorentz",
        PeaksWorkspace="WISH000" + str(rnum) + "_peaks",
...
for i in range(38423, 38427):
    ...
    PeakIntensityVsRadius(
        InputWorkspace="WISH000" + str(i) + "_MD",
