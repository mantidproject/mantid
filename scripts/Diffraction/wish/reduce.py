...
class Wish:
    ...
    def __init__(self, cal_directory, output_folder, delete_ws, user_directory="/archive/ndxwish/Instrument/data/cycle_", absorb=True):
        self.absorb = absorb
    ...
    def read(self, number, panel, extension):
        ...
        simple.NormaliseToMonitor(
            InputWorkspace=output + "norm1",
        ...
        simple.ReplaceSpecialValues(
            InputWorkspace=output, OutputWorkspace=output, NaNValue=0.0, NaNError=0.0, InfinityValue=0.0, InfinityError=0.0
        )
        return output
    ...
    def load_multi_run_part(self, extension, run, panel):
        ...
        simple.LoadRaw(
            Filename=filename, OutputWorkspace=output1, SpectrumMin=str(spectra_min), SpectrumMax=str(spectra_max), LoadLogFiles="0"
        )
        return output1
    ...
    def read_event_nexus(self, number, output, panel):
        ...
        spectra_min, spectra_max = self.return_panel.get(panel)
        simple.CropWorkspace(
            InputWorkspace=output, OutputWorkspace=output, StartWorkspaceIndex=spectra_min - 6, EndWorkspaceIndex=spectra_max - 6
        )
    ...
    def process_run(
        ...
        if panel == 0:
            for panel_i in range(Wish.NUM_PANELS):
                ...
                simple.CropWorkspace(InputWorkspace=focused_ws, OutputWorkspace=focused_ws, XMin=d_min, XMax=d_max)
                logger.notice("will try to load a vanadium with the name: {}".format(self.get_vanadium(panel_i, cycle_vana)))
                self.apply_vanadium_corrections(cycle_vana, panel_i, focused_ws)
                simple.SaveGSS(
                    InputWorkspace=focused_ws, Filename=save_location.format(number, panel_i, extension, "gss"), Append=False, Bank=1
                )
            ...
            simple.SaveGSS(InputWorkspace=focused_ws, Filename=save_location.format(number, panel, extension, "gss"), Append=False, Bank=1)
    ...
    def apply_vanadium_corrections(self, cyclevana, i, focused_ws):
        ...
        simple.ReplaceSpecialValues(
            InputWorkspace=focused_ws, OutputWorkspace=focused_ws, NaNValue=0.0, NaNError=0.0, InfinityValue=0.0, InfinityError=0.0
        )
    ...
    def create_vanadium_run(self, van_run_number, empty_run_number, panels):
        ...
        for panel in panels:
            self.process_vanadium(van_run_number, empty_run_number, panel, 4, 0.14999999999999999, vanadium_cycle, empty_cycle)
    ...
    def process_incidentmon(self, number, extension, spline_terms=20):
        if isinstance(number, int):
            ...
            if extension[:9] == "nxs_event":
                ...
                simple.Rebin(InputWorkspace=temp, OutputWorkspace=temp, Params="6000,-0.00063,110000", PreserveEvents=False)
        ...
        for reg in range(0, 4):
            simple.MaskBins(InputWorkspace=works, OutputWorkspace=works, XMin=ex_regions[reg, 0], XMax=ex_regions[reg, 1])
    ...
    def reduce(self, run_numbers, panels):
        ...
        for run in run_numbers:
            ...
            for panel in panels:
                ...
                simple.ConvertUnits(InputWorkspace=wout, OutputWorkspace="{}-d".format(wout), Target="dSpacing", EMode="Elastic")
    ...
    def save_combined_panel(self, run, panel):
        ...
        simple.RebinToWorkspace(WorkspaceToRebin=input_ws2, WorkspaceToMatch=input_ws1, OutputWorkspace=input_ws2, PreserveEvents="0")
        simple.Plus(LHSWorkspace=input_ws1, RHSWorkspace=input_ws2, OutputWorkspace=combined_ws)
        simple.ConvertUnits(InputWorkspace=combined_ws, OutputWorkspace=combined_ws + "-d", Target="dSpacing", EMode="Elastic")

        simple.SaveGSS(combined_ws, os.path.join(self.user_directory, combined_save.format("raw.gss")), Append=False, Bank=1)
...
def shared_load_files(extension, filename, ws, spectrum_max, spectrum_min, is_monitor):
    ...
    if extension == "nxs":
        ...
        simple.LoadRaw(Filename=filename, OutputWorkspace=ws, SpectrumMin=spectrum_min, SpectrumMax=spectrum_max, LoadLogFiles="0")
