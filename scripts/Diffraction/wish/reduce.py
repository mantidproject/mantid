...
def WISH_read(number, panel, ext):
    if type(number) is int:
        ...
        if (panel != 0):
            ...
            output = "w" + str(number)
        if (ext == "raw"):
            LoadRaw(Filename=filename, OutputWorkspace=output, SpectrumMin=str(min), SpectrumMax=str(max),
                    LoadLogFiles="0")
            MaskBins(InputWorkspace=output, OutputWorkspace=output, XMin=99900, XMax=106000)
        ...
        if (ext == "nxs_event"):
            ...
            min, max = WISH_returnpanel(panel)
            CropWorkspace(InputWorkspace=output, OutputWorkspace=output, StartWorkspaceIndex=min - 6,
        ...
        if (ext[0:10] == "nxs_event_"):
            ...
            if (tmax == "end"):
                LoadEventNexus(Filename=filename, OutputWorkspace=output, FilterByTimeStart=tmin, LoadMonitors='1',
                               MonitorsAsEvents='1', FilterMonByTimeStart=tmin)
            else:
                LoadEventNexus(Filename=filename, OutputWorkspace=output, FilterByTimeStart=tmin, FilterByTimeStop=tmax,
        ...
        if (ext == "nxs"):
            LoadNexus(Filename=filename, OutputWorkspace=output, SpectrumMin=str(min), SpectrumMax=str(max))
            MaskBins(InputWorkspace=output, OutputWorkspace=output, XMin=99900, XMax=106000)
        ...
        LoadRaw(Filename=filename, OutputWorkspace=output1, SpectrumMin=str(min), SpectrumMax=str(max),
                LoadLogFiles="0")
        filename = WISH_getfilename(n2, ext)
        print "reading filename..." + filename
        min, max = WISH_returnpanel(panel)
        output2 = "w" + str(n2) + "-" + str(panel)
        LoadRaw(Filename=filename, OutputWorkspace=output2, SpectrumMin=str(min), SpectrumMax=str(max),
                LoadLogFiles="0")
        MergeRuns(output1 + "," + output2, output)
        DeleteWorkspace(output1)
        DeleteWorkspace(output2)
    ConvertUnits(InputWorkspace=output, OutputWorkspace=output, Target="Wavelength", Emode="Elastic")
    lmin, lmax = WISH_getlambdarange()
    CropWorkspace(InputWorkspace=output, OutputWorkspace=output, XMin=lmin, XMax=lmax)
    ...
    NormaliseToMonitor(InputWorkspace=output + "norm1", OutputWorkspace=output + "norm2", MonitorWorkspace=monitor,
                       IntegrationRangeMin=0.7, IntegrationRangeMax=10.35)
    DeleteWorkspace(output)
    DeleteWorkspace(output + "norm1")
    RenameWorkspace(InputWorkspace=output + "norm2", OutputWorkspace=output)
    ConvertUnits(InputWorkspace=output, OutputWorkspace=output, Target="TOF", EMode="Elastic")
    ReplaceSpecialValues(InputWorkspace=output, OutputWorkspace=output, NaNValue=0.0, NaNError=0.0, InfinityValue=0.0,
                         InfinityError=0.0)
    return output
...
def WISH_focus_onepanel(work, focus, panel):
    AlignDetectors(InputWorkspace=work, OutputWorkspace=work, CalibrationFile=WISH_cal(panel))
...
def WISH_split(focus):
    for i in range(0, 11):
        ...
        ExtractSingleSpectrum(InputWorkspace=focus, OutputWorkspace=out, WorkspaceIndex=i)
    DeleteWorkspace(focus)
    return
...
def WISH_process(number, panel, ext, SEsample="WISHcryo", emptySEcycle="09_4", SEvana="candlestick", cyclevana="09_4",
    ...
    if (absorb):
        ConvertUnits(InputWorkspace=w, OutputWorkspace=w, Target="Wavelength", EMode="Elastic")
        ...
        Divide(LHSWorkspace=w, RHSWorkspace="T", OutputWorkspace=w)
        DeleteWorkspace("T")
        ConvertUnits(InputWorkspace=w, OutputWorkspace=w, Target="TOF", EMode="Elastic")
    ...
    if (panel == 1):
        CropWorkspace(InputWorkspace=wfocname, OutputWorkspace=wfocname, XMin=0.80, XMax=53.3)
    elif (panel == 2):
        CropWorkspace(InputWorkspace=wfocname, OutputWorkspace=wfocname, XMin=0.50, XMax=13.1)
    elif (panel == 3):
        CropWorkspace(InputWorkspace=wfocname, OutputWorkspace=wfocname, XMin=0.50, XMax=7.77)
    elif (panel == 4):
        CropWorkspace(InputWorkspace=wfocname, OutputWorkspace=wfocname, XMin=0.40, XMax=5.86)
    elif (panel == 5):
        CropWorkspace(InputWorkspace=wfocname, OutputWorkspace=wfocname, XMin=0.35, XMax=4.99)
    elif (panel == 6):
        CropWorkspace(InputWorkspace=wfocname, OutputWorkspace=wfocname, XMin=0.35, XMax=4.99)
    elif (panel == 7):
        CropWorkspace(InputWorkspace=wfocname, OutputWorkspace=wfocname, XMin=0.40, XMax=5.86)
    ...
    if (panel == 0):
        for i in range(1, 11):
            ...
            RebinToWorkspace(WorkspaceToRebin="empty", WorkspaceToMatch=wfocname, OutputWorkspace="empty")
            ...
            RebinToWorkspace(WorkspaceToRebin="vana", WorkspaceToMatch=wfocname, OutputWorkspace="vana")
            Divide(LHSWorkspace=wfocname, RHSWorkspace="vana", OutputWorkspace=wfocname)
            DeleteWorkspace("vana")
            ConvertUnits(InputWorkspace=wfocname, OutputWorkspace=wfocname, Target="TOF", EMode="Elastic")
            ReplaceSpecialValues(InputWorkspace=wfocname, OutputWorkspace=wfocname, NaNValue=0.0, NaNError=0.0,
                                 InfinityValue=0.0, InfinityError=0.0)
            SaveGSS(InputWorkspace=wfocname, Filename=WISH_userdir() + str(number) + "-" + str(i) + ext + ".gss",
                    Append=False, Bank=1)
...
def WISH_monitors(rb, ext):
    ...
    NormaliseByCurrent(InputWorkspace=wout, OutputWorkspace=wout)
...
def WISH_process_incidentmon(number, ext, spline_terms=20, debug=False):
    if type(number) is int:
        ...
        if (ext[0:9] == "nxs_event"):
            ...
            Rebin(InputWorkspace=temp, OutputWorkspace=temp, Params='6000,-0.00063,110000', PreserveEvents=False)
            ExtractSingleSpectrum(InputWorkspace=temp, OutputWorkspace=works, WorkspaceIndex=3)
    else:
        n1, n2 = split_string(number)
        ...
        MergeRuns(InputWorkspaces=works1 + "," + works2, OutputWorkspace=works)
        DeleteWorkspace(works1)
        DeleteWorkspace(works2)
    ConvertUnits(InputWorkspace=works, OutputWorkspace=works, Target="Wavelength", Emode="Elastic")
    lmin, lmax = WISH_getlambdarange()
    ...
    if (debug):
        ...
        p.plot(x, y)
        p.show()
    SmoothData(InputWorkspace=works, OutputWorkspace=works, NPoints=40)
...
def Removepeaks_spline_smooth_empty(works, panel, debug=False):
    ...
    if (debug):
        ...
        p.plot(x, y)
    if (splineterms != 0):
        SplineBackground(InputWorkspace=works, OutputWorkspace=works, WorkspaceIndex=0, NCoeff=splineterms)
    if (debug):
        x, y, z = getnarray(works, 0)
        p.plot(x, y)
    SmoothData(InputWorkspace=works, OutputWorkspace=works, NPoints=smoothterms)
    if (debug):
        x, y, z = getnarray(works, 0)
        p.plot(x, y)
        p.show()
...
def minus_emptycans(runno, empty):
    ...
    for p in panel_list:
        Minus(LHSWorkspace='w' + str(runno) + p, RHSWorkspace='w' + str(empty) + p,
              OutputWorkspace='w' + str(runno) + 'minus' + str(empty) + p)
        ConvertUnits(InputWorkspace='w' + str(runno) + 'minus' + str(empty) + p,
                     OutputWorkspace='w' + str(runno) + 'minus' + str(empty) + p + '-d', Target='dSpacing')
        SaveGSS("w" + str(runno) + 'minus' + str(empty) + p, WISH_userdir() + str(i) + p + ".gss", Append=False, Bank=1)
...
def main(input_file, output_dir):
    ...
    for j in range(1, 11):
        wout = WISH_process(i, j, "raw", "candlestick", "11_4", "candlestick", "11_4", absorb=False, nd=0.0, Xs=0.0,
                            Xa=0.0, h=4.0, r=0.4)
        ConvertUnits(InputWorkspace=wout, OutputWorkspace=wout + "-d", Target="dSpacing", EMode="Elastic")
    ...
    RebinToWorkspace(WorkspaceToRebin="w" + str(i) + "-6foc", WorkspaceToMatch="w" + str(i) + "-5foc",
                     OutputWorkspace="w" + str(i) + "-6foc", PreserveEvents='0')
    Plus(LHSWorkspace="w" + str(i) + "-5foc", RHSWorkspace="w" + str(i) + "-6foc",
         OutputWorkspace="w" + str(i) + "-5_6foc")
    ConvertUnits(InputWorkspace="w" + str(i) + "-5_6foc", OutputWorkspace="w" + str(i) + "-5_6foc" + "-d",
                 Target="dSpacing", EMode="Elastic")
    SaveGSS("w" + str(i) + "-5_6foc", WISH_userdir() + str(i) + "-5_6raw" + ".gss", Append=False, Bank=1)
    SaveFocusedXYE("w" + str(i) + "-5_6foc", WISH_userdir() + str(i) + "-5_6raw" + ".dat")
    SaveNexusProcessed("w" + str(i) + "-5_6foc", WISH_userdir() + str(i) + "-5_6raw" + ".nxs")
    RebinToWorkspace(WorkspaceToRebin="w" + str(i) + "-7foc", WorkspaceToMatch="w" + str(i) + "-4foc",
                     OutputWorkspace="w" + str(i) + "-7foc", PreserveEvents='0')
    Plus(LHSWorkspace="w" + str(i) + "-4foc", RHSWorkspace="w" + str(i) + "-7foc",
         OutputWorkspace="w" + str(i) + "-4_7foc")
    ConvertUnits(InputWorkspace="w" + str(i) + "-4_7foc", OutputWorkspace="w" + str(i) + "-4_7foc" + "-d",
                 Target="dSpacing", EMode="Elastic")
    SaveGSS("w" + str(i) + "-4_7foc", WISH_userdir() + str(i) + "-4_7raw" + ".gss", Append=False, Bank=1)
    SaveFocusedXYE("w" + str(i) + "-4_7foc", WISH_userdir() + str(i) + "-4_7raw" + ".dat")
    SaveNexusProcessed("w" + str(i) + "-4_7foc", WISH_userdir() + str(i) + "-4_7raw" + ".nxs")
    RebinToWorkspace(WorkspaceToRebin="w" + str(i) + "-8foc", WorkspaceToMatch="w" + str(i) + "-3foc",
                     OutputWorkspace="w" + str(i) + "-8foc", PreserveEvents='0')
    Plus(LHSWorkspace="w" + str(i) + "-3foc", RHSWorkspace="w" + str(i) + "-8foc",
         OutputWorkspace="w" + str(i) + "-3_8foc")
    ConvertUnits(InputWorkspace="w" + str(i) + "-3_8foc", OutputWorkspace="w" + str(i) + "-3_8foc" + "-d",
                 Target="dSpacing", EMode="Elastic")
    SaveGSS("w" + str(i) + "-3_8foc", WISH_userdir() + str(i) + "-3_8raw" + ".gss", Append=False, Bank=1)
    SaveFocusedXYE("w" + str(i) + "-3_8foc", WISH_userdir() + str(i) + "-3_8raw" + ".dat")
    SaveNexusProcessed("w" + str(i) + "-3_8foc", WISH_userdir() + str(i) + "-3_8raw" + ".nxs")
    RebinToWorkspace(WorkspaceToRebin="w" + str(i) + "-9foc", WorkspaceToMatch="w" + str(i) + "-2foc",
                     OutputWorkspace="w" + str(i) + "-9foc", PreserveEvents='0')
    Plus(LHSWorkspace="w" + str(i) + "-2foc", RHSWorkspace="w" + str(i) + "-9foc",
         OutputWorkspace="w" + str(i) + "-2_9foc")
    ConvertUnits(InputWorkspace="w" + str(i) + "-2_9foc", OutputWorkspace="w" + str(i) + "-2_9foc" + "-d",
                 Target="dSpacing", EMode="Elastic")
    SaveGSS("w" + str(i) + "-2_9foc", WISH_userdir() + str(i) + "-2_9raw" + ".gss", Append=False, Bank=1)
    SaveFocusedXYE("w" + str(i) + "-2_9foc", WISH_userdir() + str(i) + "-2_9raw" + ".dat")
    SaveNexusProcessed("w" + str(i) + "-2_9foc", WISH_userdir() + str(i) + "-2_9raw" + ".nxs")
    RebinToWorkspace(WorkspaceToRebin="w" + str(i) + "-10foc", WorkspaceToMatch="w" + str(i) + "-1foc",
                     OutputWorkspace="w" + str(i) + "-10foc", PreserveEvents='0')
    Plus(LHSWorkspace="w" + str(i) + "-1foc", RHSWorkspace="w" + str(i) + "-10foc",
         OutputWorkspace="w" + str(i) + "-1_10foc")
    ConvertUnits(InputWorkspace="w" + str(i) + "-1_10foc", OutputWorkspace="w" + str(i) + "-1_10foc" + "-d",
                 Target="dSpacing", EMode="Elastic")
    SaveGSS("w" + str(i) + "-1_10foc", WISH_userdir() + str(i) + "-1_10raw" + ".gss", Append=False, Bank=1)
    SaveFocusedXYE("w" + str(i) + "-1_10foc", WISH_userdir() + str(i) + "-1_10raw" + ".dat")
    SaveNexusProcessed("w" + str(i) + "-1_10foc", WISH_userdir() + str(i) + "-1_10raw" + ".nxs")
