...
def get_masked_pixels(nx_low, nx_high, ny_low, ny_high, workspace, component_name=None):
    ...
    Logger("hfir_instrument").debug("Masking pixels: nx_low=%s, nx_high=%s, ny_low=%s, ny_high=%s for component %s of type=%s." % (nx_low, nx_high, ny_low, ny_high, component_name, component.type()))
    ...
    if component.type() == 'RectangularDetector':
        ...
        while i < component.nelements() * component.idstep() - component.idstep() + nx_low + component.idstart():
            IDs.append(i)
            i -= 1
