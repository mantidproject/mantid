...
def Jacobsen(h1, Xm_1, h2, Xm_2):
    ...
    ast = (2 * np.pi) / a  #recip lattice par(note this is the mantid convention: no 2 pi)
...
def guessIndx(d, tol):
    #guessIndx accepts n d-spacings and returns guess at indexing class for diamond
...
def UBMatrixGen(fileName):
    ...
    for i in range(N):
        ...
        if i == nref1:
            ...
            if np.linalg.norm(hklhit) == 0:
                ...
                h[i] = hklhit[0]
                print('{0:0.0f} |   {1:0.0f} {2:0.0f} {3:0.0f} |    {4:0.3f} | {5:0.3f}'.format(i, h[i][0], h[i][1], h[i][2], beta[i], calcang))
    ...
    for i in range(N):   # decide if reflection is indexed to being within an integer by less then the tolerance
        dif = np.abs(hindx[i]-np.round(hindx[i]))    #difference with nearst integer
