...
def START(npts, PULSES_npulse, RUNDATA_res, MAXPAGE_n, TZERO_fine, mylog):
    ...
    hpulsew = 0.05  # subject to accelerator tune, pulse slicers, etc
    pulse2t = 0.324  # at 800MeV, different at 700MeV
    if PULSES_npulse == 1:
        pulse2t = 0.0
    fperchan = 1.0 / (RUNDATA_res * float(npts) * 2.0)
    ww = np.arange(MAXPAGE_n) * fperchan * 2.0 * math.pi
    # GW(I) is parabolic proton pulse ft GW(0)=1
    gw = (3 / hpulsew) * (
        np.sin(ww * hpulsew) / (hpulsew**2 * ww**3) - np.cos(ww * hpulsew) / (hpulsew * ww**2)
    )  # may throw warning about div/0 at i=0
    gw[0] = 1.0
    aa = gw / (1 + (ww * Tpion) ** 2)
    ...
    PULSESHAPE_convol = PULSESHAPE_convolr + 1.0j * PULSESHAPE_convoli
    ...
    # only for now)
    PULSESHAPE_convol = PULSESHAPE_convol * np.exp(1.0j * (TZERO_fine + Tpion) * ww)
