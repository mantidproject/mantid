...
def MULTIMAX(
    ...
    for j in range(OuterIter):  # outer "alpha chop" iterations?
        ...
        if FLAGS_fitdead:
            ...
            (DETECT_c, DETECT_d) = MODBAK(
                RUNDATA_hists, datum, sigma, DETECT_a, DETECT_b, DETECT_e, DETECT_d, MAXPAGE_f, PULSESHAPE_convol, SAVETIME_i2, mylog
            )

        if FLAGS_fixphase:
            (SENSE_phi, DETECT_a, DETECT_b, AMPS_amp) = MODAMP(
                RUNDATA_hists, datum, sigma, MISSCHANNELS_mm, FASE_phase, MAXPAGE_f, PULSESHAPE_convol, DETECT_e, SAVETIME_i2, mylog
            )
        else:
            (SENSE_phi, DETECT_a, DETECT_b, AMPS_amp) = MODAB(
                RUNDATA_hists, datum, sigma, MISSCHANNELS_mm, MAXPAGE_f, PULSESHAPE_convol, DETECT_e, SAVETIME_i2, mylog
            )
    ...
    (OUTSPEC_test, OUTSPEC_guess) = OUTSPEC(
        datum,
        MAXPAGE_f,
        sigma,
        datt,
