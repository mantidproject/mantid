...
def INPUT(nhists, ngroups, npts, CHANNELS_itzero, CHANNELS_i1stgood, CHANNELS_itotal, RUNDATA_res, RUNDATA_frames,
          GROUPING_group, DATALL_rdata, FAC_factor, SENSE_taud, mylog):
    ...
    for j in range(nhists):
        ...
        if (idest >= 0):
            mylog.debug("adding rdata[{0},{1}:{2}] to datum[{3}:{4},{5}]".format(j, CHANNELS_i1stgood, CHANNELS_itotal,
                                                                                 i1, i2, idest))
