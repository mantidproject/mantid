...
def MAXENT(datum, sigma, flat, base, itermax, sumfix, SAVETIME_ngo, MAXPAGE_n, MAXPAGE_f, PULSESHAPE_convol, DETECT_a,
    ...
    while (HERITAGE_iter <= itermax
           and (HERITAGE_iter <= 1 or not (test < 0.02 and abs(SPACE_chisq / SPACE_chizer - 1) < 0.01))):  # label 6
        mylog.debug("start loop, iter={} ngo={} test={} chisq={}".format(HERITAGE_iter, SAVETIME_ngo, test,
                                                                         SPACE_chisq / SPACE_chizer))
        mylog.debug("entering loop with spectrum from {0} to {1}".format(np.amin(MAXPAGE_f), np.amax(MAXPAGE_f)))
        ...
        s = -np.sum(MAXPAGE_f * np.log(MAXPAGE_f /
                                       (base * math.e))) / (SPACE_blank * math.e)  # spotted missing minus sign!
        a = s * SPACE_blank * math.e / SPACE_xsum
        mylog.notice("{:3}    {:10.4}  {:10.4}  {:10.4}  {:10.4}  {:10.4}".format(HERITAGE_iter, test, s, SPACE_chtarg,
                                                                                  SPACE_chisq, SPACE_xsum))
        SPACE_beta = np.array([-0.5 * SPACE_c1[0] / SPACE_c2[0, 0], 0.0, 0.0])
        warningMsg(SPACE_beta, "SPACE_beta", mylog)
        if (HERITAGE_iter != 0):
            (sigma, SPACE_chtarg, SPACE_beta, FAC_factor,
             FAC_facfake) = MOVE(sigma, SPACE_chisq, SPACE_chizer, SPACE_xsum, SPACE_c1, SPACE_c2, SPACE_s1, SPACE_s2,
                                 SPACE_blank, FAC_factor, FAC_facfake, mylog)
        # do 23,24
