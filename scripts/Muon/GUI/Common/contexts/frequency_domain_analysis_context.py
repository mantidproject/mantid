...
class FrequencyDomainAnalysisContext(MuonContext):
    ...
    def get_workspace_names_for_FFT_analysis(self, use_raw=True):
        ...
        workspace_options = self.get_names_of_time_domain_workspaces_to_fit(runs='All',
                                                                            group_and_pair=groups_and_pairs,
                                                                            rebin=not use_raw)
        return workspace_options

    def get_names_of_workspaces_to_fit(self, runs='', group_and_pair='', phasequad=False, rebin=False, freq="None"):

        return self.get_names_of_frequency_domain_workspaces_to_fit(runs=runs,
    ...
    def get_names_of_frequency_domain_workspaces_to_fit(self, runs='', group_and_pair='', frequency_type="None"):
        ...
        names = self._frequency_context.get_frequency_workspace_names(run_list, group, pair, frequency_type)
        return names
