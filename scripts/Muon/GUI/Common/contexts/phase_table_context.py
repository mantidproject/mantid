...
class PhaseTableContext(object):
    ...
    def add_phase_table(self, workspace):
        self.phase_tables = [
            phase_table for phase_table in self.phase_tables if phase_table.workspace_name != workspace.workspace_name
        ]
        self.phase_tables.append(workspace)

    def get_phase_table_list(self, instrument):
        return [
            phase_table.workspace_name for phase_table in self.phase_tables if instrument in phase_table.workspace_name
        ]
    ...
    def get_phase_quad(self, instrument, run):
        return [
            phase_quad.workspace_name for key, phase_quad in self.phase_quad.items()
