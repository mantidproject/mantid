...
#
QLab = mantid.ConvertToDiffractionMDWorkspace(
...
#
peaks_qLab = mantid.FindPeaksMD(InputWorkspace="QLab", MaxPeaks=300, DensityThresholdFactor=10, PeakDistanceThreshold=1.0)
...
if use_cubic_lat_par:
    mantid.FindUBUsingLatticeParameters(
        PeaksWorkspace=peaks_qLab, a=5.6402, b=5.6402, c=5.6402, alpha=90, beta=90, gamma=90, NumInitial=25, Tolerance=0.12
    )
    print("\nCubic cell found directly from FindUBUsingLatticeParameters")

if use_Niggli_lat_par:
    mantid.FindUBUsingLatticeParameters(
        PeaksWorkspace=peaks_qLab, a=3.9882, b=3.9882, c=3.9882, alpha=60, beta=60, gamma=60, NumInitial=25, Tolerance=0.12
    )
...
peaks_qLab_Integrated = mantid.IntegratePeaksMD(
    InputWorkspace=QLab, PeaksWorkspace=peaks_qLab, PeakRadius=0.2, BackgroundInnerRadius=0.3, BackgroundOuterRadius=0.4
)

binned = mantid.BinMD(
    InputWorkspace=QLab, AlignedDim0="Q_lab_x,-15,15,200", AlignedDim1="Q_lab_y,-15,15,200", AlignedDim2="Q_lab_z,-15,15,200"
)
