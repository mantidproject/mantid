...
class MantidGeom(object):
        ...
        for i in range(len(r)):
            for j in range(len(r[i])):
                if str(r[i][j]) != "nan":
                    ...
                    location_element = self._append_child(
                        "location", basecomponent, r=str(r[i][j]), t=str(theta[i][j]), p=str(phi[i][j]), name=str(names[i][j])
                    )
    ...
    def addCylinderPixel(self, name, center_bottom_base, axis, pixel_radius, pixel_height, is_type="detector"):
        ...
        self._append_child(
            "centre-of-bottom-base", cylinder, r=str(center_bottom_base[0]), t=str(center_bottom_base[1]), p=str(center_bottom_base[2])
        )
    ...
    def addDetectorIds(self, idname, idlist):
        ...
        if len(idlist) % 3 != 0:
            raise IndexError(
                "Please specify list as [start1, end1, step1, " + "start2, end2, step2, ...]. If no step is" + "required, use None."
            )
        ...
        for i in range(num_ids):
            if idlist[(i * 3) + 2] is None:
                ...
                self._append_child(
                    "id", id_element, start=str(idlist[(i * 3)]), step=str(idlist[(i * 3) + 2]), end=str(idlist[(i * 3) + 1])
                )
