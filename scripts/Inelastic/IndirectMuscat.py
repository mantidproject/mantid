...
def CalcSqw(q0, nw2, nel, dw, w0):
    ...
    for i in range(0, nq):
        ...
        if i == 0:
            ...
            Qaxis += ',' + str(q0[i])
    CreateWorkspace(OutputWorkspace='S(Q,w)',
                    DataX=xSqw,
                    DataY=ySqw,
                    DataE=eSqw,
                    Nspec=nq,
...
def CreateSqw(disp, coeff, grid, Verbose):
    ...
    CreateWorkspace(OutputWorkspace=disp, DataX=q0, DataY=w0, DataE=e0, Nspec=1, UnitX='MomentumTransfer')
    nw2 = 2 * nw + 1
    nel = nw + 1
...
# start output of Totals
    ...
    msname = sname + '_MS'
    CreateWorkspace(OutputWorkspace=msname + '_Totals',
                    DataX=xTot,
                    DataY=yTot,
                    DataE=eTot,
                    Nspec=nt,
    ...
    CreateWorkspace(OutputWorkspace=msname + '_1',
                    DataX=xMs,
                    DataY=yMsc1,
                    DataE=eMs,
                    Nspec=mang,
                    ...
                    VerticalAxisUnit='MomentumTransfer',
                    VerticalAxisValues=Qaxis)
    CreateWorkspace(OutputWorkspace=msname + '_M',
                    DataX=xMs,
                    DataY=yMscM,
                    DataE=eMs,
                    Nspec=mang,
                    ...
                    VerticalAxisUnit='MomentumTransfer',
                    VerticalAxisValues=Qaxis)
    CreateWorkspace(OutputWorkspace=msname + '_R',
                    DataX=xMs,
                    DataY=yMr,
                    DataE=eMs,
                    Nspec=mang,
