...
def ResNormRun(vname, rname, erange, nbin, Plot='None', Save=False):
    ...
    for m in range(0, nvan):
        ...
        if m == 0:
            ...
            CreateWorkspace(OutputWorkspace='Data',
                            DataX=dataX,
                            DataY=yout[:nd],
                            DataE=eout[:nd],
                            NSpec=1,
                            UnitX='DeltaE')
            CreateWorkspace(OutputWorkspace='Fit',
                            DataX=dataX,
                            DataY=yfit[:nd],
                            DataE=np.zeros(nd),
                            NSpec=1,
            ...
            CreateWorkspace(OutputWorkspace='__datmp',
                            DataX=dataX,
                            DataY=yout[:nd],
                            DataE=eout[:nd],
                            NSpec=1,
            ...
            CreateWorkspace(OutputWorkspace='__f1tmp',
                            DataX=dataX,
                            DataY=yfit[:nd],
                            DataE=np.zeros(nd),
                            NSpec=1,
    ...
    CreateWorkspace(OutputWorkspace=resnorm_stretch,
                    DataX=xPar,
                    DataY=yPar2,
                    DataE=xPar,
                    NSpec=1,
