...
def ResNormRun(vname, rname, erange, nbin, Plot="None", Save=False):
    ...
    for m in range(0, nvan):
        ...
        nd, xout, yout, eout, yfit, pfit = resnorm.resnorm(numb, Xv, Yv, Ev, reals, Xdat, Xb, Yb, wrks, wrkr, lwrk)
        ...
        if m == 0:
            ...
            CreateWorkspace(OutputWorkspace="Data", DataX=dataX, DataY=yout[:nd], DataE=eout[:nd], NSpec=1, UnitX="DeltaE")
            CreateWorkspace(OutputWorkspace="Fit", DataX=dataX, DataY=yfit[:nd], DataE=np.zeros(nd), NSpec=1, UnitX="DeltaE")
        else:
            yPar1 = np.append(yPar1, pfit[0])
            yPar2 = np.append(yPar2, pfit[1])

            CreateWorkspace(OutputWorkspace="__datmp", DataX=dataX, DataY=yout[:nd], DataE=eout[:nd], NSpec=1, UnitX="DeltaE")
            ConjoinWorkspaces(InputWorkspace1="Data", InputWorkspace2="__datmp", CheckOverlapping=False)
            CreateWorkspace(OutputWorkspace="__f1tmp", DataX=dataX, DataY=yfit[:nd], DataE=np.zeros(nd), NSpec=1, UnitX="DeltaE")
    ...
    CreateWorkspace(OutputWorkspace=resnorm_intesity, DataX=xPar, DataY=yPar1, DataE=xPar, NSpec=1, UnitX="MomentumTransfer")
    CreateWorkspace(OutputWorkspace=resnorm_stretch, DataX=xPar, DataY=yPar2, DataE=xPar, NSpec=1, UnitX="MomentumTransfer")
