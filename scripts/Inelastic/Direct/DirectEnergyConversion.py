...
#pylint: disable=bare-except
        ...
        if self.use_hard_mask_only:
            ...
            if diag_mask is None:
                ...
                diag_mask = LoadMask(Instrument=idf_file,
                                     InputFile=self.hard_mask_file,
        ...
        if diag_sample is not None:
            ...
            if sample_mask is None:
                ...
                if (PropertyManager.incident_energy.multirep_mode() and self.normalise_method == 'monitor-2')\
                    ...
                    wb_normalization_method = white_integrals.getRun().getLogData(
                        'DirectInelasticReductionNormalisedBy').value
...
#pylint: disable=bare-except
        ...
        if len(workspace_defined_prop) > 0:
            prop_man.log("****************************************************************")
            prop_man.log('*** Sample run {0} properties change default reduction properties: '.format(
...
#  -- code below uses current energy state from PropertyManager.incident_energy
        ...
        for ind, ei_guess in enumerate(AllEn):
            ...
            if self._multirep_mode:
                ...
                ws_base = PropertyManager.sample_run.chop_ws_part(ws_base, tof_range, self._do_early_rebinning, cut_ind,
                                                                  num_ei_cuts)
                prop_man.log(
                    "*** Processing multirep chunk: #{0}/{1} for provisional energy: {2} meV".format(
                        cut_ind, num_ei_cuts, ei_guess), 'notice')
            ...
            if mono_van_cache_num is not None or self.mono_correction_factor or cashed_mono_int:
                deltaE_ws_sample, mono_ws_base = self._do_abs_corrections(deltaE_ws_sample, cashed_mono_int, ei_guess,
...
# Handles cleanup of the convert_to_energy method
    ...
    def clean_up_convert_to_energy(self, start_time):
        ...
        self.prop_man.log(
            "*** Elapsed time : {0:>9.2f} sec                       *********".format(end_time - start_time), 'notice')
        self.prop_man.log("****************************************************************")
...
#pylint: disable=too-many-arguments
    ...
    def _do_abs_corrections(self, deltaE_ws_sample, cashed_mono_int, ei_guess, mono_ws_base, tof_range, cut_ind,
                            num_ei_cuts):
...
#pylint: disable=attribute-defined-outside-init
    ...
    def _run_diagnostics(self, prop_man):
        ...
        masking = self.diagnose(PropertyManager.wb_run,
                                PropertyManager.mask_run,
                                second_white=None,
            ...
            if self.mono_correction_factor is None:
                if self.use_sam_msk_on_monovan:
                    ...
                    if not self.use_hard_mask_only:
                        ...
                        masking2 = self.diagnose(PropertyManager.wb_for_monovan_run,
                                                 PropertyManager.monovan_run,
                                                 second_white=None,
                                                 print_diag_results=True)
                        masking += masking2
    ...
    def _do_bleed_corrections(self, sample_run, nchunk):
        ...
        CUR_bleed_masks, failures = diagnostics.do_bleed_test(sample_run, self.prop_man.bleed_maxrate,
                                                              self.prop_man.bleed_pixels)
        if failures > 0:
...
#-------------------------------------------------------------------------------
    ...
    def sum_monitors_spectra(self, monitor_ws, ei_mon_spectra):
        ...
        if len(existing_list) == 0:
            ...
            if n_other_mon > 0:
                ExtractSpectra(InputWorkspace=monitor_ws,
                               OutputWorkspace='_OtherMon',
            ...
            AddSampleLog(monitor_ws,
                         LogName='CombinedSpectraIDList',
                         LogText=str(monitors_left + spectra_list1 + spectra_list2),
                         LogType='String')
        else:
            pass
...
#-------------------------------------------------------------------------------
    ...
    def get_ei(self, data_run, ei_guess):
        ...
        if monitor_ws is None:
            raise RuntimeError("Can not find monitors workspace for workspace {0}, run N{1}".format(
                data_ws.name(), data_ws.getRunNumber()))
        ...
        if self.prop_man.normalise_method == 'monitor-2' and not separate_monitors:
            ...
            #pylint: disable=attribute-defined-outside-init
            self._mon2_norm_time_range = self.get_TOF_for_energies(monitor_ws, energy_rage, [self.mon2_norm_spec], None,
                                                                   self._debug_mode)
        #end
        ...
        MoveInstrumentComponent(Workspace=resultws_name,
    ...
    def _normalize_to_monitor1(self, run, old_name, range_offset=0.0, external_monitor_ws=None):
        ...
        if not mon_ws:  # no monitors
            if self.__in_white_normalization:  # we can normalize wb integrals by current separately as they often do not
                ...
                ws = run.get_workspace()
                raise RuntimeError(
                    'Normalise by monitor-1:: Workspace {0} for run {1} does not have monitors in it'.format(
                        ws.name(), run.run_number()))
        ...
        NormaliseToMonitor(InputWorkspace=old_name,
    ...
    def _normalize_to_monitor2(self, run, old_name, range_offset=0.0, external_monitor_ws=None):
        ...
        if self.__in_white_normalization:  # we normalize wb integrals by current separately as they often do not
            ...
            if not mon_ws:  # no monitors
                ws = run.get_workspace()
                raise RuntimeError(
                    'Normalize by monitor-2:: Workspace {0} for run {1} does not have monitors in it'.format(
                        ws.name(), run.run_number()))
        #
    ...
    def find_tof_range_for_multirep(self, workspace):
        ...
        if not spectra_id or len(spectra_id) == 0:
            self.prop_man.log(
                ...
                "    This is correct only if all detectors are equidistant from the sample", 'warning')
            spectra_id = [1]
...
#pylint: disable=too-many-locals
    ...
    def get_TOF_for_energies(self, workspace, energy_list, specID_list, ei=None, debug_mode=False):
            ...
            if monitor_ws is None:  # no shifting to monitor position
                ...
                #pylint: disable=bare-except
                    ...
                    self.prop_man.log(
                        ...
                        " {0}meV, binning: {1}\n"
                        "    Continuing under assumption that incident neutrons arrive at source at time=0".format(
                            ei_guess, en_bin), 'warning')
        else:
            mon1_peak = 0
        #end if
        ...
        for specID in specID_list:
            ...
            if ei:
                CreateWorkspace(OutputWorkspace=range_ws_name,
                                NSpec=1,
                                DataX=energy_list,
                                DataY=y,
                                UnitX='DeltaE',
                                ParentWorkspace=template_ws_name)
                if src_name:
                    MoveInstrumentComponent(Workspace=range_ws_name,
                                            ComponentName=src_name,
                                            X=mon1_pos.getX(),
                                            Y=mon1_pos.getY(),
                                            Z=mon1_pos.getZ(),
                                            RelativePosition=False)
                range_ws = ConvertUnits(InputWorkspace=range_ws_name,
                                        OutputWorkspace=range_ws_name,
                                        Target='TOF',
                                        EMode='Direct',
                                        EFixed=ei)
            else:
        ...
        for file_format in formats:
            for case in common.switch(file_format):
                if case('nxspe'):
                    ...
                    SaveNXSPE(InputWorkspace=name_supported,
                              Filename=filename,
                              KiOverKfScaling=prop_man.apply_kikf_correction,
                              psi=prop_man.psi)
                    if name_supported != name_orig:
                ...
                if case():  # default, could also just omit condition or 'if True'
                    prop_man.log(
                        "Unknown file format {0} requested to save results. No saving performed this format".format(
                            file_format))
...
#pylint: disable=too-many-arguments
    ...
    def apply_absolute_normalization(self,
                                     sample_ws,
                                     monovan_run=None,
                                     ei_guess=None,
                                     wb_mono=None,
                                     abs_norm_factor_is=None):
        ...
        prop_man = self.prop_man
        prop_man.log('*** Running absolute units corrections ****************************************************',
                     'notice')
        ...
        if prop_man.mono_correction_factor:
            ...
            prop_man.log('*** Evaluating the integral from the monovan run and calculate the correction factor ******',
                         'notice')
            prop_man.log(
                '    Using absolute units vanadium integration range : [{0:8f}:{1:8f}]         ******'.format(
                    mvir[0], mvir[1]), 'notice')
            if not abs_norm_factor_is:
                abs_norm_factor_is = 'calculated'

            # convert to monovanadium to energy
            deltaE_wkspace_monovan = self.mono_sample(monovan_run, ei_guess, wb_mono, self.monovan_mapfile,
                                                      self.spectra_masks)
...
*** LibISIS: {1:10.4f} Poisson: {2:10.4f}  TGP: {3:10.4f} """.format(anf_LibISIS, anf_SS2, anf_Puas, anf_TGP), 'notice')
            prop_man.log('*** If these factors are substantially different, something is wrong                    ***',
                         'notice')
        ...
        #end
        prop_man.log(
            '*** Using {0} value : {1} of absolute units correction factor (TGP)'.format(
                abs_norm_factor_is, absnorm_factor), 'notice')
        prop_man.log('*******************************************************************************************',
                     'notice')
...
#pylint: disable=too-many-locals
    ...
    def get_abs_normalization_factor(self, monovan_run, ei_monovan):
        ...
        data_ws = Integration(InputWorkspace=mono_vs,
                              OutputWorkspace='van_int',
                              RangeLower=minmax[0],
                              RangeUpper=minmax[1],
                              IncludePartialBins='1')
        ...
        if weight_sum == 0.0:
            prop_man.log("WB integral has been calculated incorrectly, look at van_int workspace: {0}".format(ws_name),
                         'error')
            raise ArithmeticError(
                "Division by 0 weight when calculating WB integrals from workspace {0}".format(ws_name))
    ...
    def _do_mono_ISIS(self, data_run, ei_guess, white_run=None, map_file=None, spectra_masks=None, Tzero=None):
        ...
        if self.check_background:
            ...
            if isinstance(result_ws, api.IEventWorkspace) or PropertyManager.incident_energy.multirep_mode():
                ...
                bkgr_ws = None
                CalculateFlatBackground(InputWorkspace=result_ws,
        ...
        if energy_bins:
            Rebin(InputWorkspace=result_name,
            ...
            if bkgr_ws:
                ...
                RemoveBackground(InputWorkspace=result_name,
                                 OutputWorkspace=result_name,
                                 BkgWorkspace=bkgr_ws,
                                 EMode='Direct')
...
#pylint disable=too-many-arguments
    ...
    def _do_mono(self, run, ei_guess, white_run=None, map_file=None, spectra_masks=None, Tzero=None):
        ...
        if prop_man.check_background and (hasattr(prop_man, 'nullify_negative_signal')
                                          and prop_man.nullify_negative_signal):
...
#-------------------------------------------------------------------------------
    ...
    def _get_wb_inegrals(self, run):
        ...
        delta = 2.0 * (upp - low)
        white_ws = Rebin(InputWorkspace=old_name,
                         OutputWorkspace=old_name,
                         Params=[low, delta, upp],
                         IgnoreBinErrors=True)
