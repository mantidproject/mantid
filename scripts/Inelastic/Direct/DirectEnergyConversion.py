...
class DirectEnergyConversion(object):
    ...
    def diagnose(self, white, diag_sample=None, **kwargs):
        ...
        if diag_sample is not None:
            ...
            if sample_mask is None:
                ...
                if (
                    PropertyManager.incident_energy.multirep_mode() and self.normalise_method == "monitor-2"
                ...
                diagnostics.normalise_background(background_int, white_integrals, diag_params.get("second_white", None))
    ...
    def convert_to_energy(
        self, wb_run=None, sample_run=None, ei_guess=None, rebin=None, map_file=None, monovan_run=None, wb_for_monovan_run=None, **kwargs
    ):
        ...
        self.prop_man.set_input_parameters_ignore_nan(
            ...
            incident_energy=ei_guess,
            energy_bins=rebin,
            map_file=map_file,
            monovan_run=monovan_run,
            wb_for_monovan_run=wb_for_monovan_run,
        )
        #
        self.prop_man.set_input_parameters(**kwargs)
        ...
        if len(workspace_defined_prop) > 0:
            prop_man.log("****************************************************************")
            prop_man.log(
                "*** Sample run {0} properties change default reduction properties: ".format(
                    PropertyManager.sample_run.get_workspace().name()
                )
            )
        ...
        for ind, ei_guess in enumerate(AllEn):
            ...
            if self._multirep_mode:
                ...
                prop_man.log(
                    "*** Processing multirep chunk: #{0}/{1} for provisional energy: {2} meV".format(cut_ind, num_ei_cuts, ei_guess),
                    "notice",
                )
            ...
            deltaE_ws_sample = self.mono_sample(PropertyManager.sample_run, ei_guess, PropertyManager.wb_run, self.map_file, masking)
            #
            ...
            if mono_van_cache_num is not None or self.mono_correction_factor or cashed_mono_int:
                deltaE_ws_sample, mono_ws_base = self._do_abs_corrections(
                    deltaE_ws_sample, cashed_mono_int, ei_guess, mono_ws_base, tof_range, cut_ind, num_ei_cuts
                )
            else:
    ...
    def clean_up_convert_to_energy(self, start_time):
        ...
        self.prop_man.log("*** Elapsed time : {0:>9.2f} sec                       *********".format(end_time - start_time), "notice")
        self.prop_man.log("****************************************************************")
    ...
    def calc_incident_energies(self, PropertyManager, prop_man, sample_ws):
        if PropertyManager.incident_energy.autoEi_mode():
            ...
            try:
                ...
                prop_man.log("*** Error while calculating autoEi: {0}. See algorithm log for details.".format(str(er)))
                return False
        else:
            return True
    ...
    def _do_abs_corrections(self, deltaE_ws_sample, cashed_mono_int, ei_guess, mono_ws_base, tof_range, cut_ind, num_ei_cuts):
        ...
        if self.mono_correction_factor:  # Correction provided.  Just apply it
            deltaE_ws_sample = self.apply_absolute_normalization(
                deltaE_ws_sample, PropertyManager.monovan_run, ei_guess, PropertyManager.wb_for_monovan_run, " provided "
            )
        elif cashed_mono_int:  # Correction cashed from previous run
            # pylint: disable=attribute-defined-outside-init
            self.mono_correction_factor = cashed_mono_int
            deltaE_ws_sample = self.apply_absolute_normalization(
                deltaE_ws_sample, PropertyManager.monovan_run, ei_guess, PropertyManager.wb_for_monovan_run, " -cached- "
            )
            # pylint: disable=attribute-defined-outside-init
            self.mono_correction_factor = None
        else:  # Calculate corrections
            if self._multirep_mode:
                mono_ws_base = PropertyManager.monovan_run.chop_ws_part(
                    mono_ws_base, tof_range, self._do_early_rebinning, cut_ind, num_ei_cuts
                )
                # its pointless to do bleed for monovan run
                # self._do_bleed_corrections(PropertyManager.monovan_run,cut_ind)

            deltaE_ws_sample = self.apply_absolute_normalization(
                deltaE_ws_sample, PropertyManager.monovan_run, ei_guess, PropertyManager.wb_for_monovan_run, "calculated"
            )
    ...
    def _run_diagnostics(self, prop_man):
        ...
        masking = self.diagnose(PropertyManager.wb_run, PropertyManager.mask_run, second_white=None, print_diag_results=True)
            ...
            if self.mono_correction_factor is None:
                if self.use_sam_msk_on_monovan:
                    ...
                    if not self.use_hard_mask_only:
                        ...
                        masking2 = self.diagnose(
                            PropertyManager.wb_for_monovan_run, PropertyManager.monovan_run, second_white=None, print_diag_results=True
                        )
                        masking += masking2
    ...
    # pylint: disable=too-many-arguments
    def mono_sample(self, mono_run, ei_guess, white_run=None, map_file=None, spectra_masks=None, result_name=None, Tzero=None):
        ...
        mono_s = self._do_mono(mono_run, ei_guess, white_run, map_file, spectra_masks, Tzero)
    ...
    def sum_monitors_spectra(self, monitor_ws, ei_mon_spectra):
        ...
        if len(existing_list) == 0:
            ...
            if n_other_mon > 0:
                ExtractSpectra(InputWorkspace=monitor_ws, OutputWorkspace="_OtherMon", WorkspaceIndexList=spectra_needed)
            else:
                pass
            ...
            AddSampleLog(
                monitor_ws, LogName="CombinedSpectraIDList", LogText=str(monitors_left + spectra_list1 + spectra_list2), LogType="String"
            )
        else:
            pass
    ...
    def _process_spectra_list(self, workspace, spectra_list, target_ws_name="SpectraWS"):
        ...
        if len(spectra_list) == 1:
            ExtractSingleSpectrum(InputWorkspace=workspace, OutputWorkspace=target_ws_name, WorkspaceIndex=wsIDs[0])
        else:
            SumSpectra(InputWorkspace=workspace, OutputWorkspace=target_ws_name, ListOfWorkspaceIndices=wsIDs)
    ...
    def get_ei(self, data_run, ei_guess):
        ...
        if monitor_ws is None:
            raise RuntimeError("Can not find monitors workspace for workspace {0}, run N{1}".format(data_ws.name(), data_ws.getRunNumber()))
        ...
        ei, mon1_peak, _, _ = GetEi(
            InputWorkspace=monitor_ws,
            Monitor1Spec=ei_mon_spectra[0],
        ...
        if self.prop_man.normalise_method == "monitor-2" and not separate_monitors:
            ...
            # pylint: disable=attribute-defined-outside-init
            self._mon2_norm_time_range = self.get_TOF_for_energies(monitor_ws, energy_rage, [self.mon2_norm_spec], None, self._debug_mode)
        # end
    ...
    def remap(self, result_ws, spec_masks, map_file):
        ...
        if map_file is not None:
            GroupDetectors(InputWorkspace=ws_name, OutputWorkspace=ws_name, MapFile=map_file, KeepUngroupedSpectra=0, Behaviour="Average")
    ...
    def _normalize_to_monitor1(self, run, old_name, range_offset=0.0, external_monitor_ws=None):
        ...
        if not mon_ws:  # no monitors
            if self.__in_white_normalization:  # we can normalize wb integrals by current separately as they often do not
                ...
                ws = run.get_workspace()
                raise RuntimeError(
                    "Normalise by monitor-1:: Workspace {0} for run {1} does not have monitors in it".format(ws.name(), run.run_number())
                )
        ...
        NormaliseToMonitor(
            InputWorkspace=old_name,
            OutputWorkspace=old_name,
    ...
    def _normalize_to_monitor2(self, run, old_name, range_offset=0.0, external_monitor_ws=None):
        ...
        if self.__in_white_normalization:  # we normalize wb integrals by current separately as they often do not
            ...
            if not mon_ws:  # no monitors
                ws = run.get_workspace()
                raise RuntimeError(
                    "Normalize by monitor-2:: Workspace {0} for run {1} does not have monitors in it".format(ws.name(), run.run_number())
                )
        #
        ...
        if self._mon2_norm_time_range:  # range has been found during ei-calculations
            ...
            if mon_ws_name.find("_shifted") != -1:
                ...
                raise RuntimeError(
                    """Instrument have been shifted but no time range has been identified.
    ...
    def find_tof_range_for_multirep(self, workspace):
        ...
        if not spectra_id or len(spectra_id) == 0:
            self.prop_man.log(
                """*** WARNING! Multirep mode used but no closest and furthest spectra numbers
    ...
    def get_TOF_for_energies(self, workspace, energy_list, specID_list, ei=None, debug_mode=False):
        ...
        if ei:
            ...
            if monitor_ws is None:  # no shifting to monitor position
                ...
                try:
                    ei, mon1_peak, mon1_index, _ = GetEi(
                        InputWorkspace=monitor_ws,
                        Monitor1Spec=mon_1_spec_ID,
                ...
                # pylint: disable=bare-except
                    ...
                    self.prop_man.log(
                        "*** WARNING: message from multirep chunking procedure: get_TOF_for_energies:\n"
                        "    not able to identify energy peak looking for TOF range for incident energy:"
                        " {0}meV, binning: {1}\n"
                        "    Continuing under assumption that incident neutrons arrive at source at time=0".format(ei_guess, en_bin),
                        "warning",
                    )
        else:
            mon1_peak = 0
        ...
        for specID in specID_list:
            ...
            if ei:
                CreateWorkspace(
                    OutputWorkspace=range_ws_name, NSpec=1, DataX=energy_list, DataY=y, UnitX="DeltaE", ParentWorkspace=template_ws_name
                )
                if src_name:
                    MoveInstrumentComponent(
                        Workspace=range_ws_name,
                        ComponentName=src_name,
                        X=mon1_pos.getX(),
                        Y=mon1_pos.getY(),
                        Z=mon1_pos.getZ(),
                ...
                range_ws = ConvertUnits(
                    InputWorkspace=range_ws_name, OutputWorkspace=range_ws_name, Target="TOF", EMode="Direct", EFixed=ei
                )
            else:
                CreateWorkspace(
    ...
    def save_results(self, workspace, save_file=None, formats=None):
        ...
        if save_file is None:
            if workspace is None:
                self.prop_man.log("DirectEnergyConversion:save_results: Nothing to save", "warning")
                return
            else:
        ...
        for file_format in formats:
            for case in common.switch(file_format):
                if case("nxspe"):
                    ...
                    SaveNXSPE(
                        InputWorkspace=name_supported, Filename=filename, KiOverKfScaling=prop_man.apply_kikf_correction, psi=prop_man.psi
                    )
                    if name_supported != name_orig:
    ...
    def apply_absolute_normalization(self, sample_ws, monovan_run=None, ei_guess=None, wb_mono=None, abs_norm_factor_is=None):
        ...
        if prop_man.mono_correction_factor:
            ...
            prop_man.log(
                "    Using absolute units vanadium integration range : [{0:8f}:{1:8f}]         ******".format(mvir[0], mvir[1]), "notice"
            )
            if not abs_norm_factor_is:
                abs_norm_factor_is = "calculated"

            # convert to monovanadium to energy
            deltaE_wkspace_monovan = self.mono_sample(monovan_run, ei_guess, wb_mono, self.monovan_mapfile, self.spectra_masks)
...
*** LibISIS: {1:10.4f} Poisson: {2:10.4f}  TGP: {3:10.4f} """.format(anf_LibISIS, anf_SS2, anf_Puas, anf_TGP),
        ...
        # end
        prop_man.log(
            "*** Using {0} value : {1} of absolute units correction factor (TGP)".format(abs_norm_factor_is, absnorm_factor), "notice"
        )
        prop_man.log("*******************************************************************************************", "notice")
    ...
    def get_abs_normalization_factor(self, monovan_run, ei_monovan):
        ...
        data_ws = Integration(
            InputWorkspace=mono_vs, OutputWorkspace="van_int", RangeLower=minmax[0], RangeUpper=minmax[1], IncludePartialBins="1"
        )
        ...
        weight_sum = sum([1.0 / e for e in error])
        ...
        weight_sum = sum([1.0 / (e * e) for e in error])
...
--------> Abs norm factors: TGP    : {11}\n""".format(
    ...
    def _do_mono_ISIS(self, data_run, ei_guess, white_run=None, map_file=None, spectra_masks=None, Tzero=None):
        ...
        if self.check_background:
            ...
            if isinstance(result_ws, api.IEventWorkspace) or PropertyManager.incident_energy.multirep_mode():
                ...
                CalculateFlatBackground(
                    InputWorkspace=result_ws,
                    OutputWorkspace=result_ws,
                    StartX=bkg_range_min,
                    EndX=bkg_range_max,
                    WorkspaceIndexList="",
                    Mode="Mean",
                    OutputMode="Subtract Background",
                    SkipMonitors="1",
                    NullifyNegativeValues="0",
                )
        else:
            bkgr_ws = None
            result_ws = data_run.get_workspace()
        ...
        if energy_bins:
            Rebin(InputWorkspace=result_name, OutputWorkspace=result_name, Params=energy_bins, PreserveEvents=False, IgnoreBinErrors=True)
            if bkgr_ws:
    ...
    def _find_or_build_bkgr_ws(self, run, bkg_range_min=None, bkg_range_max=None, time_shift=0):
        ...
        if "bkgr_ws_source" in mtd:
            ...
            if time_shift != 0:  # Workspace has probably been shifted, so to have
                ...
                # Adjust the TOF such that the first monitor peak is at t=0
                ScaleX(
                    InputWorkspace=bkgr_ws,
                    OutputWorkspace="bkgr_ws",
                    Operation="Add",
                    Factor=time_shift,
                    InstrumentParameter="DelayTime",
                    Combine=True,
            ...
            bkgr_ws = Rebin(
                result_ws,
                Params=[bkg_range_min, (bkg_range_max - bkg_range_min) * 1.001, bkg_range_max],
                PreserveEvents=False,
                IgnoreBinErrors=True,
            )
    ...
    def _do_mono(self, run, ei_guess, white_run=None, map_file=None, spectra_masks=None, Tzero=None):
        """
        ...
        if self._do_ISIS_reduction:
            self._do_mono_ISIS(run, ei_guess, white_run, map_file, spectra_masks, Tzero)
        else:
            result_name = run.set_action_suffix("_spe")
            self._do_mono_SNS(run, result_name, ei_guess, white_run, map_file, spectra_masks, Tzero)
            run.synchronize_ws()
        ...
        if prop_man.energy_bins:
            ...
            Rebin(InputWorkspace=result_name, OutputWorkspace=result_name, Params=bins, IgnoreBinErrors=True)
        ...
        if prop_man.check_background and (hasattr(prop_man, "nullify_negative_signal") and prop_man.nullify_negative_signal):
    ...
    def _get_wb_inegrals(self, run):
        ...
        delta = 2.0 * (upp - low)
        white_ws = Rebin(InputWorkspace=old_name, OutputWorkspace=old_name, Params=[low, delta, upp], IgnoreBinErrors=True)
