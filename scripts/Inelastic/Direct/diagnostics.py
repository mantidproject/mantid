...
def diagnose(white_int, **kwargs):
    ...
    if hardmask_file is not None:
        ...
        hm_ws = LoadMask(
            Instrument=kwargs.get("instr_name", ""), InputFile=parser.hard_mask_file, OutputWorkspace="hard_mask_ws", RefWorkspace=ref_ws
        )
    ...
    if not parser.use_hard_mask_only:
        ...
        if white_mask:
            ...
            __white_masks, num_failed = do_white_test(
                white_int,
                parser.tiny,
                parser.huge,
                parser.van_out_lo,
        ...
        if "second_white" in kwargs:  # NOT IMPLEMENTED
            ...
            __second_white_masks, num_failed = do_second_white_test(
                white_int,
                parser.second_white,
                parser.tiny,
                parser.huge,
        ...
        if kwargs.get("sample_counts", None) is not None and kwargs.get("samp_zero", False):
            ...
            maskZero, zero_count_failures = FindDetectorsOutsideLimits(
                InputWorkspace=parser.sample_counts,
        ...
        if hasattr(parser, "background_int"):
            ...
            __bkgd_mask, failures = do_background_test(
                parser.background_int, parser.samp_lo, parser.samp_hi, parser.samp_sig, parser.samp_zero, start_index, end_index
            )
...
def add_masking(input_ws, mask_ws, start_index=None, end_index=None):
    ...
    on the mask_ws.
    """
    MaskDetectors(Workspace=input_ws, MaskedWorkspace=mask_ws, StartWorkspaceIndex=start_index, EndWorkspaceIndex=end_index)


# -------------------------------------------------------------------------------


def do_white_test(white_int, tiny, large, out_lo, out_hi, median_lo, median_hi, sigma, start_index=None, end_index=None):
    """
    ...
    white_masks, num_failed = FindDetectorsOutsideLimits(
        white_int, StartWorkspaceIndex=start_index, EndWorkspaceIndex=end_index, HighThreshold=large, LowThreshold=tiny
    )

    MaskDetectors(Workspace=white_int, MaskedWorkspace=white_masks, StartWorkspaceIndex=start_index, EndWorkspaceIndex=end_index)
    DeleteWorkspace(Workspace=white_masks)

    white_masks, failed_median = MedianDetectorTest(
        white_int,
        StartWorkspaceIndex=start_index,
        EndWorkspaceIndex=end_index,
...
def do_second_white_test(
    white_counts, comp_white_counts, tiny, large, out_lo, out_hi, median_lo, median_hi, sigma, variation, start_index=None, end_index=None
):
    """
    ...
    __second_white_tests, failed = do_white_test(comp_white_counts, tiny, large, median_lo, median_hi, sigma, start_index, end_index)
    # and now compare it with the first
    effic_var, num_failed = DetectorEfficiencyVariation(
        WhiteBeamBase=white_counts,
...
def do_bleed_test(sample_run, max_framerate, ignored_pixels):
    ...
    if max_framerate is None:  # get defaults
        max_framerate = float(component_info.getNumberParameter(root, "max-tube-framerate")[0])
    if ignored_pixels is None:  # get defaults
        ignored_pixels = int(component_info.getNumberParameter(root, "num-ignored-pixels")[0])
    else:
    ...
    bleed_test, num_failed = CreatePSDBleedMask(
        InputWorkspace=data_ws, OutputWorkspace=ws_name, MaxTubeFramerate=max_framerate, NIgnoredCentralPixels=ignored_pixels
    )
