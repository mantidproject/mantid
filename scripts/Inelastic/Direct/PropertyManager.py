...
class PropertyManager(NonIDF_Properties):
    ...
    map_file = MapMaskFile(
        "map_file",
        ".map",
        """Mapping file for the sample run.\n
            ...
            if key not in old_changes_list:
                ...
                # pylint: disable=bare-except
                    ...
                    # pylint: disable=bare-except
                        ...
                        cur_val = "Undefined"
                    self.log(
                        "Retrieving or reapplying script property {0} failed. Property value remains: {1}".format(key, cur_val), "warning"
                    )
                    continue
    ...
    def perform_simplfied_ws_comparison(self, new_val, cur_val):
        if isinstance(new_val, api.Workspace) and isinstance(cur_val, api.Workspace):
            ...
            if (
                ...
                and new_val.getNumberHistograms() == cur_val.getNumberHistograms()
                and new_val.getNEvents() == cur_val.getNEvents()
                and new_val.getAxis(0).getUnit().unitID() == cur_val.getAxis(0).getUnit().unitID()
            ):
                new_val = 1
                cur_val = 1
    ...
    def update_property_value(self, val, public_name, param_list):
        ...
        # pylint: disable=bare-except
            ...
            self.log(
                "Can not retrieve property {0} value from existing reduction parameters. Ignoring this property".format(public_name),
                "warning",
            )
            return True
