...
class anAbsorptionShape(object):
    ...
    def material(self, value):
        ...
        if isinstance(value, (list, tuple)):
            if len(value) == 2:
                ...
                raise TypeError('*** If material is defined by list or tuple,'
    ...
    def _set_list_property(self, value, shape_name, mandatory_prop_list, opt_prop_list, opt_val_list):
        ...
        if n_elements < len(mandatory_prop_list):
            raise TypeError('*** {0} shape parameter needes at least {1} imput parameters namely: {2}'.format(
        ...
        if isinstance(value, (list, tuple)):
            ...
            raise TypeError('*** {0} shape parameter accepts only list or tuple containing from {1} to {2}'
...
class FlatPlate(anAbsorptionShape):
    ...
    @shape.setter
    def shape(self, value):
        shape_dict = self._set_list_property(value, 'FlatPlate', ['Height', 'Width', 'Thick'], ['Center', 'Angle'],
                                             [[0., 0., 0.], 0.])  # noqa: E127
...
class HollowCylinder(anAbsorptionShape):
    ...
    @shape.setter
    def shape(self, value):
        shape_dict = self._set_list_property(value, 'HollowCylinder', ['Height', 'InnerRadius', 'OuterRadius'],
...
class Sphere(anAbsorptionShape):
    ...
    @shape.setter
    def shape(self, value):
        shape_dict = self._set_list_property(value, 'Sphere', ['Radius'], ['Center'], [[0., 0., 0.]])  # noqa: E127
    ...
    def _fast_abs_corrections(self, correction_base_ws, kwarg={}):
        ...
        if kw['Emode'].lower() == 'elastic':
            adsrbtn_correctios = SphericalAbsorption(correction_base_ws,
                                                     SphericalSampleRadius=self._ShapeDescription['Radius'])
        else:
