...
import ui_errorreport_outside
from PyQt4.QtCore import pyqtSignal
from mantidqtpython import MantidQt


class CrashReportPage(QtGui.QWidget, ui_errorreport_outside.Ui_Errorreport):
...
#  These are the options along the bottom
    ...
    def fullShare(self):
        self.action.emit(False, 0, self.input_name, self.input_email)
        self.close()

    def nonIDShare(self):
        self.action.emit(False, 1, self.input_name, self.input_email)
        self.close()

    def noShare(self):
        self.action.emit(False, 2, self.input_name, self.input_email)
        self.close()
