...
class MARIReductionFromFileCache(ISISDirectInelasticReduction):
    ...
    def runTest(self):
        ...
        targ_path = config['defaultsave.directory']
        file_to_clear = os.path.join(targ_path,'MAR11002.raw')
...
class MARIReductionWaitAndSum(ISISDirectInelasticReduction):
    ...
    def prepare_test_file(self):
      ...
      """
        ...
        if self._counter>= 3:
            ...
            targ_path = config['defaultsave.directory']
            targ_file = os.path.join(targ_path,'MAR11666.raw')
            shutil.copy2(source ,targ_file )
    ...
    def runTest(self):
      ...
      """
        ...
        self.red.reducer.prop_man.fix_file_extension = True
        self.red.reducer.prop_man.data_file_ext = '.raw'
        self._file_to_clear = os.path.join(targ_path,'MAR11666.raw')
        if os.path.exists(self._file_to_clear):
            os.remove(self._file_to_clear)
            self._file_to_clear = ''
        ...
        self.red.reducer.prop_man.sample_run=[11001,11666]
        #pylint: disable=unused-variable
