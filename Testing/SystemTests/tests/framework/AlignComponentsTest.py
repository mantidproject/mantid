...
class DetectorIDTest(MantidSystemTest):
    ...
    def runTest(self):
        ...
        LoadAscii(Filename="SNAP_51984_peakcenters_tof.dat", Separator="Tab", Unit="Dimensionless", OutputWorkspace=prefix + "peak_centers")
        LoadEmptyInstrument(InstrumentName="SNAP", OutputWorkspace=prefix + "snap")

        with capture_logs(level="debug") as logs:
            AlignComponents(
                PeakCentersTofTable=prefix + "peak_centers",
