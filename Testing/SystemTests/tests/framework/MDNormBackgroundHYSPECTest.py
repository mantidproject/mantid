...
class MDNormBackgroundHYSPECTest(systemtesting.MantidSystemTest):
    ...
    def prepare_background(input_md, reference_sample_mde, background_md: str):
        ...
        for i in range(data_MDE.getNumExperimentInfo()):
            ...
            AddSampleLogMultiple(Workspace=dgs_data, LogNames="phi, chi, omega", LogValues="{},{},{}".format(phi, chi, omega))
    ...
    def run_old_solution(self, input_sample_md, background_seed_md, prefix, temp_norm_result=None):
        ...
        MDNorm(
            InputWorkspace=input_sample_md,
        ...
        self.prepare_background(input_md=background_seed_md, reference_sample_mde=input_sample_md, background_md="background_MDE")
    ...
    def runTest(self):
        ...
        dgs_red_group_name1 = self.prepare_md(
            input_ws_name="sum", merged_md_name="merged2", min_log_value=13, max_log_value=15, log_step=LOG_VALUE_STEP, prefix="step2"
        )
        # Old solution
        benchmark2 = self.run_old_solution("merged2", f"{dgs_red_group_name1}_2", "oldstep2", benchmark)
        # 1-2 Test new solutions
        self.prepare_single_exp_info_background(input_md_name=f"{dgs_red_group_name1}_2", output_md_name="bkgd_md2", target_qframe="Q_lab")
        r = MDNorm(
    ...
    def normalize_with_background(
        ...
        # prepare sample MD
        self.prepare_md(
            input_ws_name=event_ws_name, merged_md_name="merged", min_log_value=log_value_range[0], max_log_value=log_value_range[1]
        )
        # Prepare background workspace
        # old way - use reduced_1 as the background
        self.prepare_single_exp_info_background(input_md_name="reduced_1", output_md_name="background_MDE", target_qframe="Q_lab")
    ...
    def test_property_setup(input_mde: str, reduced_background: str):
        ...
        try:
            # Create background workspace in Q lab
            ConvertToMD(
                InputWorkspace=reduced_background,
                QDimensions="Q3D",
                Q3DFrames="Q_sample",
