...
from mantid.simpleapi import CreateWorkspace, Fit, Load, LoadInstrument, LoadSINQFile, PoldiAnalyseResiduals, PoldiTruncateData
import numpy as np
    ...
    def analyseResults(self, filenames):
        for dataFile in filenames:
            ...
            self.assertEqual(
                ...
                "Number of d-values does not match for %s \
                             (is: %i, should: %i)"
            ...
            self.assertLessThan(
                relativeSlopeError, 5e-3, "Relative error of slope is too large for %s (is: %d)" % (dataFile, relativeSlopeError)
            )
