...
class MolDynCdlTest(systemtesting.MantidSystemTest):
    def runTest(self):
        ms.MolDyn(Data="DISF_NaF.cdl", Functions=["Fqt-total", "Sqw-total"], OutputWorkspace="ISISIndirectSimulationTest_MolDynCdl")

    def validate(self):
        self.tolerance = 1e-2
        self.disableChecking.append("Instrument")

        ms.Load(Filename="ISISIndirectSimulation_MolDynCDL.nxs", OutputWorkspace="ISISIndirectSimulation_MolDynCDL")
        ms.Load(Filename="ISISIndirectSimulation_MolDynCDL_SQW.nxs", OutputWorkspace="ISISIndirectSimulation_MolDynCDL_SQW")
...
class MolDynDatTest(systemtesting.MantidSystemTest):
    def runTest(self):
        ms.MolDyn(Data="WSH_test.dat", OutputWorkspace="WSH_test_iqt")
