...
class ConvertQtoHKLMDHistoTest_match_ConvertWANDSCDtoQ_test(systemtesting.MantidSystemTest):
    ...
    def runTest(self):
        ...
        data_norm = ConvertHFIRSCDtoMDE(
            ConvertWANDSCDtoQTest_data, Wavelength=1.486, MinValues="-6.04,-6.04,-6.04", MaxValues="6.04,6.04,6.04"
        )

        HKL = ConvertQtoHKLMDHisto(
            data_norm, PeaksWorkspace=peaks, Uproj="1,1,0", Vproj="-1,1,0", Extents="-6.04,6.04,-6.04,6.04,-6.04,6.04", Bins="151,151,151"
        )

        for i in range(HKL.getNumDims()):
            print(HKL.getDimension(i).getUnits(), Q.getDimension(i).getUnits())
            np.testing.assert_equal(HKL.getDimension(i).getUnits(), Q.getDimension(i).getUnits())
        ...
        x, y, z = np.meshgrid(
            np.linspace(xaxis.getMinimum(), xaxis.getMaximum(), xaxis.getNBins()),
            np.linspace(yaxis.getMinimum(), yaxis.getMaximum(), yaxis.getNBins()),
            np.linspace(zaxis.getMinimum(), zaxis.getMaximum(), zaxis.getNBins()),
            indexing="ij",
            copy=False,
        )
        ...
        np.testing.assert_almost_equal(x[~np.isnan(Q_data)].mean(), x[~np.isclose(hkl_data, 0)].mean(), decimal=2)
        np.testing.assert_almost_equal(y[~np.isnan(Q_data)].mean(), y[~np.isclose(hkl_data, 0)].mean(), decimal=2)
        np.testing.assert_almost_equal(z[~np.isnan(Q_data)].mean(), z[~np.isclose(hkl_data, 0)].mean(), decimal=1)
