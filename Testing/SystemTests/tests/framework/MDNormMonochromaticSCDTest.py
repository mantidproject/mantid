...
class MDNormMonochromaticSCDDEMANDRLUTest(systemtesting.MantidSystemTest):
    """Same as MDNormMonochromaticSCDDEMANDTest, but with RLU=True: exercises the
    reciprocal-lattice-unit binning path for monochromatic-SCD input, including the
    box-tree-derived maxQ estimate used to set default Q-dimension extents (since there
    is no time-of-flight trajectory, and hence no MDNorm_low/MDNorm_high, to derive it
    from). Data and normalization are still identical, so every non-empty output bin
    must still be exactly 1.0, regardless of RLU."""

    def requiredFiles(self):
        return ["HB3A_exp0722_scan0220.nxs"]

    def runTest(self):
        ws_name = "MDNormMonochromaticSCDDEMANDRLUTest"
        HB3AAdjustSampleNorm(
            Filename="HB3A_exp0722_scan0220.nxs",
            VanadiumFile="HB3A_exp0722_scan0220.nxs",
            NormaliseBy="None",
...
class MDNormMonochromaticSCDWANDRLUTest(systemtesting.MantidSystemTest):
    """Same as MDNormMonochromaticSCDWANDTest, but with RLU=True: exercises the
    reciprocal-lattice-unit binning path for monochromatic-SCD input, including the
    box-tree-derived maxQ estimate used to set default Q-dimension extents. Data and
    normalization are still identical, so every non-empty output bin must still be
    exactly 1.0, regardless of RLU."""
    ...
    def runTest(self):
        ws_name = "MDNormMonochromaticSCDWANDRLUTest"
        LoadMD("HB2C_WANDSCD_data.nxs", OutputWorkspace=ws_name + "_raw")
        SetGoniometer(ws_name + "_raw", Axis0="s1,0,1,0,1", Average=False)
