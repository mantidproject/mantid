...
def twoAngleFit(workspace_name, scalefactor, expected_fit_params, expected_fit_covariance):
    ...
    function_name = 'name=ReflectivityMulf, nlayer=1, Theta=2.3, ScaleFactor=1, AirSLD=0, BulkSLD=0, Roughness=0,' \
                    ' BackGround=6.8e-06,Resolution=5.0, SLD_Layer0=1.0e-6, d_Layer0=20.0, Rough_Layer0=0.0, ' \
                    'constraints=(0<SLD_Layer0, 0<d_Layer0),ties=(Theta=2.3, AirSLD=0, BulkSLD=0, Resolution=5.0, ' \
                    'ScaleFactor=1.0, Roughness=0, Rough_Layer0=0)'
...
def testFittingOfReducedData(run1_number, run2_number, expected_fit_params, expected_fit_covariance):
    ...
    function_name='name=ReflectivityMulf, nlayer=0, Theta=2.3, ScaleFactor=1.0, AirSLD=0, BulkSLD=6.35e-6, ' \
                  'Roughness=2.5,BackGround=3.0776e-06, Resolution=5.0, ties=(Theta=2.3, AirSLD=0, BulkSLD=6.35e-6,' \
                  ' Resolution=5.0, Roughness=2.5)'
    Fit(Function=function_name,
