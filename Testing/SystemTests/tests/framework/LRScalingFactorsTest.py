...
class LRPrimaryFractionTest(systemtesting.MantidSystemTest):
    ...
    def validate(self):
        ...
        reference = [[7.82, 3.20e-05, 0.054, 3.3e-06], [24.42, 0.000105, 0.225, 1.45e-05]]
        ...
        for line in filed.readlines():
            ...
            if not (
                abs(reference[item_number][0] - data_a) < tolerances[item_number][0]
                ...
                logger.error(
                    "Expected %5.3g %5.3g %5.3g %5.3g"
                    % (reference[item_number][0], reference[item_number][1], reference[item_number][2], reference[item_number][3])
                )
                return False
            item_number += 1
        filed.close()
