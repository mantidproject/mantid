...
class ReflectometryISIS(systemtesting.MantidSystemTest, metaclass=ABCMeta):
    ...
    def runTest(self):
        ...
        CropWorkspace(InputWorkspace=X,
                      OutputWorkspace='Io',
                      XMin=0.8,
                      XMax=14.5,
                      StartWorkspaceIndex=2,
                      EndWorkspaceIndex=2)
        ...
        Divide(LHSWorkspace=D,
               RHSWorkspace=Io,
               OutputWorkspace='I',
               AllowDifferentNumberSpectra='1',
               ClearRHSWorkspace='1')
        I = mtd['I'][0]

        # Move the detector so that the detector channel matching the reflected beam is at 0,0
        MoveInstrumentComponent(Workspace=I,
                                ComponentName="lineardetector",
                                X=0,
                                Y=0,
                                Z=-PIX * ((SC - avgDB) / 2.0 + avgDB))
        ...
        # Check the outputs
        qxqy_comparison = CompareMDWorkspaces(Workspace1='QxQy_rebinned',
                                              Workspace2='QxQy_benchmark',
                                              Tolerance=0.01,
                                              CheckEvents=False)
        kikf_comparison = CompareMDWorkspaces(Workspace1='KiKf_rebinned',
                                              Workspace2='KiKf_benchmark',
                                              Tolerance=0.01,
                                              CheckEvents=False)
        pipf_comparison = CompareMDWorkspaces(Workspace1='PiPf_rebinned',
                                              Workspace2='PiPf_benchmark',
                                              Tolerance=0.01,
                                              CheckEvents=False)
