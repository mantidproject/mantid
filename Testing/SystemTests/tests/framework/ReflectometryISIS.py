...
class ReflectometryISIS(systemtesting.MantidSystemTest, metaclass=ABCMeta):
    ...
    def runTest(self):
        ...
        MoveInstrumentComponent(Workspace=result_workspace, ComponentName="lineardetector", X=0, Y=0, Z=-PIX * ((SC - avgDB) / 2.0 + avgDB))
        ...
        QxQy, _QxQy_vertexes = ConvertToReflectometryQ(
            InputWorkspace="SignedTheta_vs_Wavelength", OutputDimensions="Q (lab frame)", Extents="-0.0005,0.0005,0,0.12", Version=1
        )
        KiKf, _KiKF_vertexes = ConvertToReflectometryQ(
            InputWorkspace="SignedTheta_vs_Wavelength", OutputDimensions="K (incident, final)", Extents="0,0.05,0,0.05", Version=1
        )
        PiPf, _PiPf_vertexes = ConvertToReflectometryQ(
            InputWorkspace="SignedTheta_vs_Wavelength", OutputDimensions="P (lab frame)", Extents="0,0.1,-0.02,0.15", Version=1
        )
