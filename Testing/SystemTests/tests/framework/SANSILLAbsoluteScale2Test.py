...
class D11_AbsoluteScale_Test(systemtesting.MantidSystemTest):
    ...
    def runTest(self):
        ...
        for s in range(sens.getNumberHistograms()):
            sens.setSharedX(s, np.array([5.73, 6.27]))
            reference.setSharedX(s, np.array([5.73, 6.27]))
            water_with_reference.setSharedX(s, np.array([5.73, 6.27]))
            water_with_sens_flux.setSharedX(s, np.array([5.73, 6.27]))
