...
class SphinxWarnings(systemtesting.MantidSystemTest):
    def __init__(self):
        ...
        self.allowedCategories = [
            'Arithmetic', 'CorrectionFunctions', 'Crystal', 'DataHandling', 'Diagnostics', 'Diffraction', 'Events',
            'Examples', 'ILL', 'ISIS', 'Inelastic', 'MDAlgorithms', 'MPI', 'Muon', 'Optimization', 'PythonAlgorithms',
            'Quantification', 'Reflectometry', 'Remote', 'SANS', 'Simulation', 'SINQ', 'Sample', 'Transforms',
            'Utility', 'Workflow'
        ]
        self.errorMessage = ""
                ...
                if mantid.api.DeprecatedAlgorithmChecker(name, version).isDeprecated() == '':
                    ...
                    for cat in alg.categories():
                        if cat.split("\\")[0] not in self.allowedCategories:
                            self.errorMessage += name + " " + str(version) + " Category: " + cat.split(
                                "\\")[0] + " is not in the allowed list."
                    ...
                    for prop in properties:
                        ...
                        if len(result) > 0:
                            self.errorMessage += name + " " + str(
                                version) + " Property: " + propName + " Documentation: " + result + "\n"
