...
class LoadTests(unittest.TestCase):
    ...
    def test_extra_properties_passed_to_loader_for_multiple_files(self):
        data = Load(
            "EQSANS_1466_event.nxs,EQSANS_3293_event.nxs", OutputWorkspace=self.wsname, BankName="bank1", SingleBankPixelsOnly=False
        )

        self.assertTrue(isinstance(data, WorkspaceGroup))
        self.assertEqual(2, data.getNumberOfEntries())
        # Test number of events in each
        self.assertEqual(740, data[0].getNumberEvents())
    ...
    def test_plus_operator_sums_multiple_set_files_to_give_group(self):
        ...
        for name in deleted_names:
            self.assertTrue(
                name not in AnalysisDataService,
            )
