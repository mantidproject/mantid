    ...
    def _add_libs_from_prefix(prefix_path):
        ...
        for lib_dir in ("lib", "lib64"):
            if (prefix_path / lib_dir).is_dir():
                site_packages = next((prefix_path / lib_dir).iterdir()) / "site-packages"
        ...
        if package_dirs:
            ...
            if not prefix_path.is_dir():
                raise FileNotFoundError(f"Install prefix {prefix_path} does not exist.")
            else:
                directory_contents = list(prefix_path.iterdir())
                raise FileNotFoundError(
                    ("Could not find site-packages for temporary dir. " "Here are the directory contents: ") + "; ".join(directory_contents)
                )

    @classmethod
    def _install_euphonic_to_tmp_prefix(cls, tmp_prefix, verbose=False):
        ...
        if "--system" in subprocess.check_output([sys.executable, "-m", "pip", "install", "-h"]).decode():
            compatibility_args = ["--system"]
        else:
            compatibility_args = []

        process = subprocess.run(
            [sys.executable, "-m", "pip", "install", "--prefix", tmp_prefix] + compatibility_args + ["pip", "packaging"],
        ...
        if list(prefix_path.iterdir()):
            ...
            process_pythonpath = ":".join([str(dir) for dir in tmp_site_packages] + [os.environ["PYTHONPATH"]])
        ...
        if version.parse(scipy.version.version) < version.parse("1.0.1"):
            process = subprocess.run(
                [sys.executable, "-m", "pip", "install", "--ignore-installed", "--no-deps", "--prefix", tmp_prefix, "scipy==1.0", "pytest"],
                stdout=subprocess.PIPE,
                stderr=subprocess.STDOUT,
                env=env,
            )
        ...
        process = subprocess.run(
            [sys.executable, "-m", "pip", "install", "--prefix", tmp_prefix, "euphonic[phonopy_reader]==0.6.5"],
            stdout=subprocess.PIPE,
            stderr=subprocess.STDOUT,
            env=env,
        )
