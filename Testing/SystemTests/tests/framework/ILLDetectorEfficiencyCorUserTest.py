...
class IN5(systemtesting.MantidSystemTest):
    def runTest(self):
        ...
        def tube_corr(x):
            return 1.0 - numpy.exp(-5.6 / numpy.sqrt(x))
...
class IN6(systemtesting.MantidSystemTest):
    def runTest(self):
        ...
        def det_corr(x):
            ...
            c[high] = 0.94 * (1.0 - numpy.exp(-3.284 / numpy.sqrt(x[high])))
            c[low] = numpy.exp(-0.0565 / numpy.sqrt(x[low])) * (1.0 - numpy.exp(-3.284 / numpy.sqrt(x[low])))
            return c
