...
def main():
    info = [
        ...
        "all of the tests that haven't been run will be marked as skipped in the",
        "full logs.",
    ]
    ...
    parser.add_argument(
        "-x", "--executable", dest="executable", help="The executable path used to run each test. Default is the sys.executable"
    )
    parser.add_argument("-a", "--exec-args", dest="execargs", help='Arguments passed to executable for each test Default=" --classic"')
    parser.add_argument("--frameworkLoc", help="location of the system test framework (default=%s)" % DEFAULT_FRAMEWORK_LOC)
    parser.add_argument("--qt_api", help="The qt version to use for the system test (default={0}).".format(DEFAULT_QT_API))
    parser.add_argument(
        "--disablepropmake",
        action="store_false",
        dest="makeprop",
    ...
    parser.add_argument(
        "-R", "--tests-regex", dest="testsInclude", help="String specifying which tests to run. Simply uses 'string in testname'."
    )
    parser.add_argument(
        "-E", "--excluderegex", dest="testsExclude", help="String specifying which tests to not run. Simply uses 'string in testname'."
    )
    ...
    parser.add_argument("--output-on-failure", dest="output_on_failure", action="store_true", help="Print full log for failed tests.")
    parser.add_argument("-N", "--dry-run", dest="dry_run", action="store_true", help="Do not run tests just print what would be run.")
    parser.add_argument("--showskipped", dest="showskipped", action="store_true", help="List the skipped tests.")
    parser.add_argument("-d", "--datapaths", dest="datapaths", help="A semicolon-separated list of directories to search for data")
    parser.add_argument("-s", "--savedir", dest="savedir", help="A directory to use for the Mantid save path")
    parser.add_argument("--archivesearch", dest="archivesearch", action="store_true", help="Turn on archive search for file finder.")
    parser.add_argument(
        "--exclude-in-pull-requests",
    ...
    parser.set_defaults(
        frameworkLoc=DEFAULT_FRAMEWORK_LOC,
        qt_api=DEFAULT_QT_API,
        executable=sys.executable,
        execargs=" --classic",  # this default is set in the systemtest.bat runner script anyway
        makeprop=True,
    ...
    mtdconf = systemtesting.MantidFrameworkConfig(
        loglevel=options.loglevel, data_dirs=data_paths, save_dir=save_dir, archivesearch=options.archivesearch
    )
    if options.makeprop:
        mtdconf.config()
    ...
    # Variables updated as tests run, used in result collation
    status_dict = dict()
    ...
    if options.ncores == 1:
        ...
        if not options.dry_run:
            ...
            for ip in range(options.ncores):
                processes.append(
                    Process(
                        target=systemtesting.testThreadsLoop,
                        args=(
                            mtdconf,
                            options,
                            tests_dict,
    ...
    if options.dry_run:
        ...
        if skipped_tests == total_tests:
            ...
            percent = 1.0 - float(failed_tests) / float(total_tests - skipped_tests)
            percent = int(100.0 * percent)
            print(
                "%d%s tests passed, %d tests failed out of %d (%d skipped)"
