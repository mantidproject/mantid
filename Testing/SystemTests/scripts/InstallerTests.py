...
parser.add_argument(
    "-j", "--parallel", dest="ncores", type=int, default=1, help="The number of instances to run in parallel, like the -j option in ctest."
)
parser.add_argument("-n", dest="doInstall", action="store_false", help="Run tests without installing Mantid (it must be already installed)")
...
parser.add_argument("-R", dest="test_regex", metavar="regexp", default=None, help="Optionally only run the test matched by the regex")
parser.add_argument("-E", dest="test_exclude", metavar="exclude", default=None, help="String specifying which tests to not run")
...
parser.add_argument("-l", dest="log_level", metavar="level", default="information", choices=log_levels, help="Log level " + str(log_levels))
