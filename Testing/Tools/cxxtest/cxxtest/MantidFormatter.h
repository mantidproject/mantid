...
namespace CxxTest
{
    ...
    class MantidFormatter : public TestListener
    {
    public:
        MantidFormatter( OutputStream *o, const char *preLine = ":", const char *postLine = "" ) :
            _dotting( true ),
            _reported( false ),
            _o(o),
            _preLine(preLine),
        ...
        int run()
        {
            ...
            float ticksElapsed = float(end - start)/float(CLOCKS_PER_SEC);
        ...
        {
            (*_o) << sd.suiteName() << endl;
            (*_o) << "Running " << sd.numTests() << (sd.numTests() == 1 ? " test" : " tests");
            (*_o).flush();
            _reported = false;
        }
        ...
        void leaveSuite( const SuiteDescription & sd ) 
        {
            ...
            newLine();
            (*_o) << "Failed " << tracker().suiteFailedTests() << " of " << sd.numTests() << endl;
            (*_o).flush();
        }
        ...
        void leaveWorld( const WorldDescription &desc )
        {
            ...
            (*_o) << "Total Failed " << tracker().failedTests() << " of " << totalTests << endl;
