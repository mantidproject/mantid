...
def parseCommandline(args):
    ...
    if imported_fog:
        parser.add_option("--fog", action="store_true", dest="fog", default=False, help="Use new FOG C++ parser")
    parser.add_option("-v",
                      "--version",
                      action="store_true",
                      dest="version",
    ...
    parser.add_option("", "--gui", dest="gui", metavar="CLASS", help="Like --runner, with GUI component")
    parser.add_option("",
                      "--error-printer",
                      action="store_true",
                      dest="error_printer",
    ...
    parser.add_option(
        ...
        default="",
        help=
        "The value of this option is an XML filename to which the XML summary is written.  The default XML filename is TEST-<world>.xml, where <world> is the value of the --world option."
    )
    parser.add_option("-w",
                      "--world",
                      dest="world",
    ...
    parser.add_option("", "--root", action="store_true", dest="root", default=False, help="Write CxxTest globals")
    parser.add_option("", "--part", action="store_true", dest="part", default=False, help="Don't write CxxTest globals")
    parser.add_option("",
                      "--no-static-init",
                      action="store_true",
                      dest="noStaticInit",
    ...
    parser.add_option("", "--factor", action="store_true", dest="factor", default=False, help="Mystery option")
...
def writeDynamicDescription(output, suite):
    ...
    if not options.noStaticInit:
        output.write('( %s, %s, "%s", %s, %s, %s, %s )' % (suite['cfile'], suite['line'], suite['name'], suite['tlist'],
    ...
    for suite in suites:
        ...
        if isDynamic(suite):
            ...
            output.write('  %s.initialize( %s, %s, "%s", %s, %s, %s, %s );\n' %
                         (suite['dobject'], suite['cfile'], suite['line'], suite['name'], suite['tlist'],
                          suite['object'], suite['create'], suite['destroy']))
        else:
            output.write(
                '  %s.initialize( %s, %s, "%s", %s, %s );\n' %
                (suite['dobject'], suite['cfile'], suite['line'], suite['name'], suite['object'], suite['tlist']))
