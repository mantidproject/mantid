...
fixture that fabricates IMAT run data. It is not itself a test: like ``AutomatedUITestBase`` it
defines no ``test_*`` method and no ``runTest``, which is what keeps ``unittest``'s loader from
collecting it out of the modules that import it.
"""
...
class EngDiffGuiTestBase(AutomatedUITestBase):
    ...
    def required_files(self):
        """Data files this test cannot run without, as names the ``FileFinder`` can resolve.

        Declared rather than checked inline so the skip happens before any of the setup below runs -
        there is no point fabricating runs and building the interface for a test that is about to
        be skipped.
        """
        return ()
