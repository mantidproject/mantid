    ...
    for year in range(year_start, year_end + 1):
        ...
        for month in range(1, 13):
            ...
            for line in freading:
                ...
                (facility_changed, found_changed) = _assign_change_to_facility(domains=domains,
                                                                               changes=email_changes,
                                                                               year=year,
                                                                               date_key=date_key,
                                                                               facility_dict=facility_changed,
                                                                               increment=int(changed))
                (facility_added, found_added) = _assign_change_to_facility(domains=domains,
                                                                           changes=email_changes,
                                                                           year=year,
                                                                           date_key=date_key,
                                                                           facility_dict=facility_added,
                                                                           increment=int(added))
                (facility_removed, found_removed) = _assign_change_to_facility(domains=domains,
                                                                               changes=email_changes,
                                                                               year=year,
                                                                               date_key=date_key,
            ...
            for line in f2reading:
                ...
                (found, facility_commits) = _assign_change_to_facility(domains=domains,
                                                                       changes=email_commits,
                                                                       year=year,
                                                                       facility_dict=facility_commits,
                                                                       date_key=date_key,
                                                                       increment=1)
                if not found:
