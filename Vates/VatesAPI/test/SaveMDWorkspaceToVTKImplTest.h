...
public:
  ...
  void test_that_vector_of_threshold_strings_has_all_values() {
    ...
    // Act
    const auto thresholds = saveMDToVTK.getAllowedThresholdsInStringRepresentation();
  ...
  void test_that_saves_MD_Histo_workspace_to_vts_file() {
    ...
    auto workspace = MDEventsTestHelper::makeFakeMDHistoWorkspace(1.0, numDims, numBins);

    const int recursionDepth = 5; // Note that this does not matter since we are testing MDHisto

    const auto normalizations = saveMDToVTK.getAllowedNormalizationsInStringRepresentation();
    const auto normalization = saveMDToVTK.translateStringToVisualNormalization(normalizations[0]);

    const auto thresholds = saveMDToVTK.getAllowedThresholdsInStringRepresentation();
    const auto threshold = saveMDToVTK.translateStringToThresholdRange(thresholds[0]);
    ...
    // Act
    saveMDToVTK.saveMDWorkspace(workspace, filename, normalization, threshold, recursionDepth);
