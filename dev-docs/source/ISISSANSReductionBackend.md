...
transmissions, [SANSWideAngleCorrection](inv:mantid#algm-sanswideanglecorrection-v1).
...
- [CropToComponent](inv:mantid#algm-croptocomponent-v1)
- [Divide](inv:mantid#algm-divide-v1) (by Sample Volume)
- [Multiply](inv:mantid#algm-multiply-v1) (by Absolute Scale)
- [MoveInstrumentComponent](inv:mantid#algm-moveinstrumentcomponent-v1)
- *Normalize To SANS Monitor*
- [Q1D](inv:mantid#algm-q1d-v1) or [Qxy](inv:mantid#algm-qxy-v1)
- [RotateInstrumentComponent](inv:mantid#algm-rotateinstrumentcomponent-v1)
- [SANSConvertToWavelengthAndRebin](inv:mantid#algm-sansconverttowavelengthandrebin-v1)
- [SANSLoad](inv:mantid#algm-sansload-v1)
- *SANSSave*
- *Slice SANS Event*
- *Workspace Masking*
...
3.  Get the corrected transmission workspace. The sub-steps are:
    ...
    2.  Extract the transmission detector ids with
        [ExtractSpectra](inv:mantid#algm-extractspectra-v1)
    3.  Perform prompt peak correction
    4.  Perform flat background correction to monitors (if applicable)
        using [CalculateFlatBackground](inv:mantid#algm-calculateflatbackground-v1)
    5.  Perform flat background correction to other detectors (if
        applicable) using
        [CalculateFlatBackground](inv:mantid#algm-calculateflatbackground-v1)
...
4.  Get the corrected direct workspace. The sub-steps are:
    ...
    3.  Perform prompt peak correction (if applicable) using
        [RemoveBins](inv:mantid#algm-removebins-v1)
    4.  Perform flat background correction to monitors (if applicable)
        using [CalculateFlatBackground](inv:mantid#algm-calculateflatbackground-v1)
    5.  Perform flat background correction to other detectors (if
        applicable) using
...
5.  Perform fitting for the transmission calculation. The sub-steps are:
    1.  Use incident monitor, wavelength settings, transmission detector
        ...
        [CalculateTransmission](inv:mantid#algm-calculatetransmission-v1)
    2.  Execute [CalculateTransmission](inv:mantid#algm-calculatetransmission-v1)
...
1.  Calculate the momentum transfer resolution workspace using
    [TOFSANSResolutionByPixel](inv:mantid#algm-tofsansresolutionbypixel-v1) (if
    applicable)
2.  Set data workspace, adjustment workspaces, momentum transfer
    ...
    [Q1D](inv:mantid#algm-q1d-v1)
3.  Execute [Q1D](inv:mantid#algm-q1d-v1)
...
1.  Set data workspace, adjustment workspaces, momentum transfer
    resolution workspace (if applicable), radius and wavelength
    cut-offs, momentum transfer limits and the gravity correction on
    [Qxy](inv:mantid#algm-qxy-v1)
2.  Execute [Qxy](inv:mantid#algm-qxy-v1)
...
1.  Unit conversion from time-of-flight units to wavelength units using
    [ConvertUnits](inv:mantid#algm-convertunits-v1)
2.  Performs a rebin operation using either [Rebin](inv:mantid#algm-rebin-v1) or
    [InterpolatingRebin](inv:mantid#algm-interpolatingrebin-v1)
...
1.  Create the wavelength-adjustment workspace. The sub-steps are:
    ...
    3.  Load the wavelength-adjustment file using
        [LoadRKH](inv:mantid#algm-loadrkh-v1)
    4.  Provide all of the above workspaces with the same binning using
        [Rebin](inv:mantid#algm-rebin-v1) and multiply them using
        [Multiply](inv:mantid#algm-multiply-v1)
2.  Create the pixel-adjustment workspace. The sub-states are:
    1.  Load the pixel-adjustment file using [LoadRKH](inv:mantid#algm-loadrkh-v1)
    2.  Crop the pixel-adjustment workspace to the desired detector
        using [CropToComponent](inv:mantid#algm-croptocomponent-v1)
3.  Set the pixel-adjustment and wavelength-adjustment workspaces on the
    output of the algorithm

### *SANSLoad*

The [SANSLoad](inv:mantid#algm-sansload-v1) algorithm is responsible for loading data
...
2.  Apply time bin masking. The sub-steps are:
    1.  Apply general time bin masks using [MaskBins](inv:mantid#algm-maskbins-v1)
    2.  Apply detector specific time bin masks using
        [MaskBins](inv:mantid#algm-maskbins-v1)
...
The [MoveInstrumentComponent](inv:mantid#algm-moveinstrumentcomponent-v1) algorithm
and [RotateInstrumentComponent](inv:mantid#algm-rotateinstrumentcomponent-v1) are
...
wavelength correction in [Q1D](inv:mantid#algm-q1d-v1) or [Qxy](inv:mantid#algm-qxy-v1). The user can provide
...
2.  Extract the monitor spectrum using
    [ExtractSingleSpectrum](inv:mantid#algm-extractsinglespectrum-v1) into a monitor
    workspace
3.  Apply the scale factor to the monitor workspace using
    [Scale](inv:mantid#algm-scale-v1)
...
The [SANSSave](inv:mantid#algm-sanssave-v1) algorithm performs two steps:
...
3.  Slice the scatter workspace using the start and end time and
    [FilterByTime](inv:mantid#algm-filterbytime-v1)
...
[SANSStitch](inv:mantid#algm-sansstitch-v1), again only if the reduction has been set
up to do this.
...
3.  If we are dealing with an even-mode workspace and the compatibility
    ...
    binning is applied using [Rebin](inv:mantid#algm-rebin-v1) or
    [RebinToWorkspace](inv:mantid#algm-rebintoworkspace-v1), respectively.
...
5.  The data workspace is masked using various modes. Note that using
    [MoveInstrumentComponent](inv:mantid#algm-moveinstrumentcomponent-v1). The
...
7.  Scale the data set using [Multiply](inv:mantid#algm-multiply-v1). This will
    multiply the data set with the absolute scale and divide
    [Divide](inv:mantid#algm-divide-v1) by the sample volume.
...
9.  Convert the data workspace into histogram-mode using
    [RebinToWorkspace](inv:mantid#algm-rebintoworkspace-v1). This is only relevant
...
10. The final step, the conversion to momentum transfer units, either
    uses [Q1D](inv:mantid#algm-q1d-v1) or [Qxy](inv:mantid#algm-qxy-v1) depending on the setting
