...
.. csv-table:: Larger tasks
    ...
    **Data and Workspace Menus**
    ...
    ,Instrument Viewer, "Open the :ref:`Instrument View <mantid:InstrumentViewer>` (``Show Instrument``) for instruments from different facilities. Try the different projections, pick detectors and banks, overlay a peaks workspace, and add a shape to make a mask and a region of interest. Also check the legacy Instrument Viewer (``Show Instrument (Legacy)``) still opens."
