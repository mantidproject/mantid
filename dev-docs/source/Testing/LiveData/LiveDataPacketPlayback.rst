...
file, depending on the settings in ``adara_player_conf.yml``. Usually it will be convenient to save the packet files for
...
such files with ``mv`` does this automatically, but ``cp -p`` must be used if the files are copied.  Files like this,
...
address for the playback ``server`` resolves to ``${XDG_RUNTIME_DIR}/sock-adara_player``.  For testing, a loopback
address such as ``127.0.0.1:12345`` may be used, but for security reasons, especially on multi-user systems, the UDS
...
#) Create a *base* directory for the saved packet data.  Note that each time the client application connects to the
...
If the stream being played back was saved using ``record``, the playback order will be uniquely determined by the
...
* ``yaml``: the order of the packet files is specified in the ``sessions.yml`` file;  or, if no ``sessions.yml`` file
  has been specified, the *directory* order will be used;
...
*device-descriptor* data.  Although the server saves the complete data corresponding to any packet stream, we need to do
...
because of the way the ADARA-server saves its file data, often there will be duplicate *metadata* sections in the group
of files corresponding to a single run.  Originally the ``SNSLiveEventDataListener`` treated this case as an error, but
in the current version, instead this will produce a *warning* in the logs.
...
- At the moment, I recommend simply *ignoring* any metadata-only files.  Including these files in the playback generates
  "duplicate device-descriptor for XX" and "XX is not a time-series property" errors (i.e. because "XX" has not seen its
...
3\. **IMPORTANT reminder**\ : Set the ``base_path`` in ``${SCRIPTS}/adara_player_sessions.yml`` to the value of
``${INPUT_DATA}``.


4\. **IMPORTANT reminder**\ : Set the ``logging.filename`` in
    ``${ADARA_PLAYER_UTILS}/packet_playback/adara_player_conf.yml`` using the value from ``${LOGS}``.  Note that the
...
corresponding command to play this back is just ``adara_player ${OUTPUT_DATA}`` -- only specifying the root of the tree.
