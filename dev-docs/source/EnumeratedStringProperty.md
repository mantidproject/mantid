...
`EnumeratedStringProperty` allows the use of `EnumeratedString` objects
within the Property structure framework.

# How to use the EnumeratedStringProperty

Include the `EnumeratedStringProperty.h` header file. Set up the
`EnumeratedStringProperty` as follows:
...
Determining and using pre-set mode, if present, or using the default
setting:
...
Please see examples of usage in `Rebin.cpp`, `CalculateDIFC.cpp`, and
`AddSampleLog.cpp`.
