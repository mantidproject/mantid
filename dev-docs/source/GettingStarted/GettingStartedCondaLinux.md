...
This will start `gdb` with the appropriate command, you can then use the run command `r` within `gdb` to start Mantid.
If you wish to launch Workbench more directly then you will need to include the `--single-process` flag for your python process, otherwise you will not be able to use most breakpoints that you set.
For example:
