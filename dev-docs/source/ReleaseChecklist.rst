...
* Set the value of the `github actions variable <https://github.com/mantidproject/mantid/settings/variables/actions>`__ ``BRANCH_TO_PUBLISH`` to ``release-next``. This will set the target branch used by `github workflow <https://github.com/mantidproject/mantid/blob/main/.github/workflows/pixi-update.yml>`__ for updating pixi lock file. And the same variable is also used to `determine the milestone <https://github.com/mantidproject/mantid/blob/main/.github/workflows/add-milestone.yml>`__ for pull requests without an assigned milestone.
