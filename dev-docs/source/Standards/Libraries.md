...
- Use libraries from [std](http://en.cppreference.com/w/cpp) where
  possible.
- [Boost](http://www.boost.org) header-only libraries are also always
  available. The following compiled Boost libraries are available:
  - regex
  - date_time
  - python
  - serialization
...
Prefer `boost::regex` over `std::regex` (bugs in `std::regex` until gcc
5 and some platforms still use gcc 4.8)

### String Algorithms (chop, ends_with etc)

- For any string functions that are not implemented in the C++ standard,
