...
`testworkspace1D_dist` of Suite `RebinTest` from the command line using
gdb would be:
...
2.  Create a new Debug Configuration. I called mine "DataHandling Test
    Debug".
3.  For me, it worked best using the "Standard Create Process Launcher"
    (bottom option on the "Main" tab)
4.  Set the C/C++ application to the path to the test executable, e.g.
    bin/DataHandlingTest
5.  Under the "Arguments" tab, add the name of the test class you want
    to debug under "Program arguments", e.g. LoadTest
    1.  To only run one test in a class, you can add the particular test
        to run, e.g. "LoadTest test_exec"
6.  Under "Common" you can put the debug config in your "favorites"
    menu.
...
<figure>
<img src="images/VisualStudioTestDebugProperties.png"
alt="images/VisualStudioTestDebugProperties.png" />
<figcaption>VisualStudioTestDebugProperties.png</figcaption>
</figure>
