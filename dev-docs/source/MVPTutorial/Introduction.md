...
creating easy to maintain GUIs (graphical user interfaces). In general:
...
It is important to note that the view should be simple (no logic) and is
usually not tested. The model can be tested in a similar way to other
Python tests. The model and view never communicate with each other
directly. The presenter will extract relevant information from the view
and pass it to the model. The presenter may then pass the result of the
model to the view. The presenter will contain the GUI logic and is
tested using **mocking**.
