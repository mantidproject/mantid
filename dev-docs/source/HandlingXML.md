...
useful introductory presentation can be found
[here.](http://pocoproject.org/slides/170-XML.pdf)
...
equivalent to the value of `i` given to it, and its `length` method a
cost of `n`, where `n` is the number of nodes in the list.
...
Even if the compiler is smart enough to optimise `pElems->length()` to a
single call, we still have N² + N performance at best. Instead, we
