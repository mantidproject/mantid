...
MantidQwtIMDWorkspaceData::MantidQwtIMDWorkspaceData(
    ...
    : m_workspace(workspace), m_logScale(logScale), m_minY(DBL_MAX),
      m_minPositive(DBL_MAX), m_maxY(-DBL_MAX), m_preview(false),
      m_start(start), m_end(end), m_normalization(normalize),
...
void MantidQwtIMDWorkspaceData::cacheLinePlot() {
  ...
  //  std::cout << "MantidQwtIMDWorkspaceData found " << m_Y.size() << "
  //  points\n";
...
void MantidQwtIMDWorkspaceData::setLogScale(bool on) { m_logScale = on; }
...
void MantidQwtIMDWorkspaceData::setWaterfallPlot(bool on) {
  m_isWaterfall = on;
}
    ...
    if (m_transform) {
      ...
      if (auto mdew =
        ...
        for (size_t i = 0; i < mdew->getNumDims(); ++i) {
          if (mdew->getDimension(i)->getNBins() ==
              controller->getSplitInto(i)) {
