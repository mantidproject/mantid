...
SliceDefinition get_slice_definition(const size_t numberOfDimensions, const Mantid::Kernel::VMD_t minValue, const Mantid::Kernel::VMD_t maxValue) {
...
std::vector<Mantid::Geometry::MDHistoDimension_sptr> get_dimensions_collection(const size_t numberOfDimensions,
  ...
  for (size_t index = 0; index < numberOfDimensions; ++index) {
    ...
    auto dimension = boost::make_shared<Mantid::Geometry::MDHistoDimension>("Distance", "Dist", frame,
      minConverted, maxConverted, numberOfBins);
...
public:
  bool do_test_slice_lies_in_workspace_boundaries(const std::string sliceLiesWithinWorkspaceBoundary) {
    ...
    auto dimensions = get_dimensions_collection(numberOfDimensions, minValue, maxValue, sliceLiesWithinWorkspaceBoundary);
