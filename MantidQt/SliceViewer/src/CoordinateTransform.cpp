...
namespace SliceViewer {
	...
	void NullTransform::transform(Mantid::Kernel::VMD &coords, size_t dimX, size_t dimY) {}
	void NullTransform::checkDimensionsForHKL(Mantid::API::IMDWorkspace_sptr ws, size_t dimX, size_t dimY) {};


	NonOrthogonalTransform::~NonOrthogonalTransform() {}

	NonOrthogonalTransform::NonOrthogonalTransform(Mantid::API::IMDWorkspace_sptr ws, size_t dimX, size_t dimY) : m_dimensionsHKL(true)
	{
	...
	void NonOrthogonalTransform::checkDimensionsForHKL(Mantid::API::IMDWorkspace_sptr ws, size_t dimX, size_t dimY)
	{
	...
	void NonOrthogonalTransform::transform(Mantid::Kernel::VMD &coords, size_t dimX, size_t dimY)
	{
		if (m_dimensionsHKL)
		{
			auto v1 = coords[0];
			auto v2 = coords[1];
			auto v3 = coords[2];
			coords[dimX] = v1 * m_skewMatrix[0 + 3 * dimX] + v2 * m_skewMatrix[1 + 3 * dimX] + v3 * m_skewMatrix[2 + 3 * dimX];
			coords[dimY] = v1 * m_skewMatrix[0 + 3 * dimY] + v2 * m_skewMatrix[1 + 3 * dimY] + v3 * m_skewMatrix[2 + 3 * dimY];
		}
	}
	...
	std::unique_ptr<CoordinateTransform> createCoordinateTransform(Mantid::API::IMDWorkspace_sptr ws, size_t dimX, size_t dimY)
	{
		...
		{
			coordinateTransform = Mantid::Kernel::make_unique<NonOrthogonalTransform>(ws, dimX, dimY);
		}
		else 
		{
