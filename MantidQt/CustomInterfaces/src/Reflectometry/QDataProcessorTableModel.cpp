...
QDataProcessorTableModel::QDataProcessorTableModel(
    ...
    : m_dataCachePeakIndex(-1), m_tWS(tableWorkspace) {
...
void QDataProcessorTableModel::updateDataCache(const int row) const {
  ...
  m_dataCache.push_back(
      QString::fromStdString(tableRow.cell<std::string>(COL_RUNS)));
  m_dataCache.push_back(
      QString::fromStdString(tableRow.cell<std::string>(COL_ANGLE)));
  m_dataCache.push_back(
      QString::fromStdString(tableRow.cell<std::string>(COL_TRANSMISSION)));
  m_dataCache.push_back(
      QString::fromStdString(tableRow.cell<std::string>(COL_QMIN)));
  m_dataCache.push_back(
...
QVariant QDataProcessorTableModel::headerData(int section,
  ...
  if (role == Qt::WhatsThisRole && orientation == Qt::Horizontal) {
    ...
    case COL_ANGLE:
      return QString("<b>Angle used during the run.</b><br />"
                     "<i>optional</i><br />"
                     "Unit: degrees<br />"
                     "If left blank, this is set to the last value for 'THETA' "
                     "in the run's sample log. "
                     "If multiple runs were given in the Run(s) column, the "
                     "first listed run's sample log will be used. "
    ...
    case COL_DQQ:
      return QString("<b>Resolution used when rebinning</b><br />"
                     "<i>optional</i><br />"
                     "If left blank, this is calculated for you using the "
                     "CalculateResolution algorithm. "
                     "<br /><br /><b>Example:</b> <samp>0.9</samp>");
    case COL_SCALE:
      return QString("<b>Scaling factor</b><br />"
                     "<i>required</i><br />"
                     "The created IvsQ workspaces will be Scaled by "
                     "<samp>1/i</samp> where <samp>i</samp> is the value of "
                     "this column."
    ...
    case COL_GROUP:
      return QString(
          ...
          "<i>required</i><br />"
          "The value of this column determines which other rows this row's "
          "output will be stitched with. "
          "All rows with the same group number are stitched together. ");
    case COL_OPTIONS:
      return QString(
          "<b>Override <samp>ReflectometryReductionOneAuto</samp> "
          "properties</b><br />"
          "<i>optional</i><br />"
          "This column allows you to override the properties used when "
          "executing <samp>ReflectometryReductionOneAuto</samp>. "
          "Options are given as key=value pairs, separated by commas. "
          "Values containing commas must be quoted."
          "<br /><br /><b>Example:</b> <samp>StrictSpectrumChecking=0, "
          "RegionOfDirectBeam=\"0,2\", Params=\"1,2,3\"</samp>");
    default:
      return QVariant();
    }
