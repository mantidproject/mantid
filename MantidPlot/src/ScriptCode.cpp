 ...
 */
ScriptCode::ScriptCode(const char *codeStr) : m_code(codeStr), m_offset(0) { convertLineEndingsToUnix(); }
 ...
 */
ScriptCode::ScriptCode(const char *codeStr, const int offset) : m_code(codeStr), m_offset(offset) {
  convertLineEndingsToUnix();
}

/// Code from a QString with zero offset
ScriptCode::ScriptCode(const QString &codeStr) : m_code(codeStr.toStdString()), m_offset(0) {
  convertLineEndingsToUnix();
}

/// Code from a QString with a defined offset
ScriptCode::ScriptCode(const QString &codeStr, const int offset) : m_code(codeStr.toStdString()), m_offset(offset) {
  convertLineEndingsToUnix();
}
