...
void FirstTimeSetup::initLayout() {
  ...
  connect(m_uiForm.lblPrivacyPolicy, SIGNAL(linkActivated(const QString &)), this,
  ...
  m_uiForm.cbFacility->setCurrentIndex(m_uiForm.cbFacility->findText(QString::fromStdString(facility)));

  // set instrument
  std::string instrument = config.getString("default.instrument", true);
  m_uiForm.cbInstrument->updateInstrumentOnSelection(false);
  m_uiForm.cbInstrument->setCurrentIndex(m_uiForm.cbInstrument->findText(QString::fromStdString(instrument)));
  connect(m_uiForm.cbFacility, SIGNAL(currentIndexChanged(const QString &)), this,
          SLOT(facilitySelected(const QString &)));
...
void FirstTimeSetup::openManageUserDirectories() { MantidQt::API::ManageUserDirectories::openManageUserDirectories(); }

void FirstTimeSetup::openReleaseNotes() {
  MantidQt::API::HelpWindow::showPage(this, Mantid::Kernel::MantidVersion::releaseNotes());
}

void FirstTimeSetup::openSampleDatasets() { MantidDesktopServices::openUrl(QUrl("http://download.mantidproject.org")); }
void FirstTimeSetup::openMantidIntroduction() {
  MantidDesktopServices::openUrl(QUrl("http://www.mantidproject.org/Mantid_Basic_Course"));
}
void FirstTimeSetup::openPythonIntroduction() {
  MantidDesktopServices::openUrl(QUrl("http://www.mantidproject.org/Introduction_To_Python"));
}
void FirstTimeSetup::openPythonInMantid() {
  MantidDesktopServices::openUrl(QUrl("http://www.mantidproject.org/Python_In_Mantid"));
}
...
void FirstTimeSetup::openExternalLink(const QString &link) { MantidDesktopServices::openUrl(link); }
