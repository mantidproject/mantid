...
* the window.
*
*	@param wsname The name of the workspace for which the logs will be displayed
*	@param parentContainer The parent container relative to which the window will be centered.
*						Also delegates the parentContainer to the QDialog(QWidget, flags) constructor.
*	@param flags Flags for QT
*				Also delegates the flags to the QDialog(QWidget, flags) constructor
...
SampleLogDialogBase::SampleLogDialogBase(const QString &wsname, 
	QWidget *parentContainer, Qt::WFlags flags, size_t experimentInfoIndex) :
	QDialog(parentContainer, flags), m_parentContainer(parentContainer), 
	m_wsname(wsname.toStdString()), m_experimentInfoIndex(experimentInfoIndex), 
	buttonPlot(nullptr), buttonClose(nullptr), m_spinNumber(nullptr), m_tree(nullptr) {
...
//------------------------------------------------------------------------------------------------
/**
* Initialize everything in the tree. Must be called after a QTreeWidget initialisation
*	@author Martyn Gigg, Tessella Support Services plc
*	@date 05/11/2009
*/
