...
*/
LabelToolLogValuesDialog::LabelToolLogValuesDialog(const QString &wsname, QWidget *parentContainer, Qt::WFlags flags, size_t experimentInfoIndex):
	...
	boost::shared_ptr<Mantid::API::MultipleExperimentInfos> mei = AnalysisDataService::Instance().retrieveWS<MultipleExperimentInfos>(
		m_wsname);
...
*	@throws std::bad_cast :: The exception is throws if the dynamic_cast fails
*
*/
void LabelToolLogValuesDialog::importItem(QTreeWidgetItem *item) {
	...
	if (statValuesText.isEmpty()) {
		...
		logValuesString += statRadioChoice[activeRadioBoxPosition]->text()
			+ "  " + statValuesText;
	}
