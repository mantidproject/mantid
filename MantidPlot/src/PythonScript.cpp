...
int traceLineNumber(PyObject *scriptObj, PyFrameObject *frame, int event, PyObject *arg) {
  ...
  if (event != PyTrace_LINE)
    return retcode;
  PyObject_CallMethod(scriptObj, STR_LITERAL("lineNumberChanged"), STR_LITERAL("O i"), frame->f_code->co_filename,
                      frame->f_lineno);
  return retcode;
}
 ...
 * Constructor
 */
PythonScript::PythonScript(PythonScripting *env, const QString &name, const InteractionType interact, QObject *context)
    : Script(env, name, interact, context), m_interp(env), localDict(nullptr), stdoutSave(nullptr), stderrSave(nullptr),
      m_codeFileObject(nullptr), m_threadID(-1), isFunction(false), m_isInitialized(false), m_pathHolder(name),
      m_recursiveAsyncGIL(PyGILState_UNLOCKED) {
...
void PythonScript::setIdentifier(const QString &name) {
  ...
  if (QFileInfo(identifier).exists()) {
    ...
    PyDict_SetItem(localDict, FROM_CSTRING("__file__"), FROM_CSTRING(scriptPath.toAscii().data()));
  }
}
 ...
 */
void PythonScript::sendLineChangeSignal(int lineNo, bool error) { emit currentLineChanged(lineNo, error); }
...
void PythonScript::generateAutoCompleteList() {
  ...
  PyObject *keywords = PyObject_CallFunctionObjArgs(
      PyDict_GetItemString(m_interp->globalDict(), "_ScopeInspector_GetFunctionAttributes"), localDict, NULL);
...
void PythonScript::emit_error() {
  ...
  if (bool(exception) && PyErr_GivenExceptionMatches(exception, PyExc_SystemExit) &&
...
QString PythonScript::constructSyntaxErrorStr(PyObject *syntaxError) {
  ...
  const QString filename = m_interp->toString(PyObject_GetAttrString(syntaxError, "filename"), true);
  int lineno = static_cast<int>(m_interp->toLong(PyObject_GetAttrString(syntaxError, "lineno")));
  ...
  if (textObject != Py_None) {
    ...
    int offset = static_cast<int>(m_interp->toLong(PyObject_GetAttrString(syntaxError, "offset")));
 ...
 */
void PythonScript::tracebackToMsg(QTextStream &msgStream, PyTracebackObject *traceback, bool root) {
  ...
  QString filename = QString::fromAscii(TO_CSTRING(traceback->tb_frame->f_code->co_filename));
  if (filename == identifier().c_str()) {
...
bool PythonScript::setInt(int val, const char *name) { return interp()->setInt(val, name, localDict); }

bool PythonScript::setDouble(double val, const char *name) { return interp()->setDouble(val, name, localDict); }
...
QVariant PythonScript::evaluateImpl() {
  ...
  if (!qret.isValid()) {
    ...
    if (pystring) {
      PyObject *asUTF8 =
          PyUnicode_EncodeUTF8(PyUnicode_AS_UNICODE(pystring), (int)PyUnicode_GET_DATA_SIZE(pystring), nullptr);
      Py_DECREF(pystring);
      if (asUTF8) {
...
struct TemporaryValue {
  TemporaryValue(long &refVal, long tmp) : initial(refVal), ref(refVal) { ref = tmp; }
...
PyObject *PythonScript::compileToByteCode(bool for_eval) {
  ...
  if (context()) {
    if (context()->inherits("Table")) {
      ...
      // __builtins__.
      PyDict_SetItemString(localDict, "__builtins__", PyDict_GetItemString(interp()->globalDict(), "__builtins__"));
