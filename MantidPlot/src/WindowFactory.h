...
template <class C, class Base> class ProjectWindowInstantiator : public AbstractProjectInstantiator<Base> {
public:
   ...
   * form.
   */
  Base *loadFromProject(const std::string &lines, ApplicationWindow *app, const int fileVersion = -1) const override {
...
public:
   ...
   */
  MantidQt::API::IProjectSerialisable *loadFromProject(const std::string &className, const std::string &lines,
    ...
    if (it != _map.end())
      ...
      throw Kernel::Exception::NotFoundError("WindowFactory: " + className + " is not registered.\n", className);
  }
  ...
  template <class C> void subscribe(const std::string &className) {
    auto instantiator = std::make_unique<ProjectWindowInstantiator<C, MantidQt::API::IProjectSerialisable>>();
