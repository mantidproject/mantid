# * Find jsoncpp include dirs and libraries Use this module by invoking find_package with the form: find_package(JsonCPP
#   [required] [quiet] )
#
# The module sets the following variables JSONCPP_FOUND          - True if headers and libraries were found
# JSONCPP_INCLUDE_DIR    - jsoncpp include directories JSONCPP_LIBRARY        - library files for linking (optimised
# version) JSONCPP_LIBRARY_DEBUG  - library files for linking (debug version) JSONCPP_LIBRARIES      - All required
# libraries, including the configuration type

# Headers
find_path(JSONCPP_INCLUDE_DIR json/reader.h PATH_SUFFIXES jsoncpp)
...
if(EXISTS "${JSONCPP_INCLUDE_DIR}/json/version.h")
  ...
  foreach(_JSONCPP_VERSION_PART MAJOR MINOR PATCH)
    string(REGEX REPLACE ".*# *define +JSONCPP_VERSION_${_JSONCPP_VERSION_PART} +([0-9]+).*" "\\1"
                         JSONCPP_VERSION_${_JSONCPP_VERSION_PART} "${_JSONCPP_VERSION_CONTENTS}"
    )
  endforeach()
...
if(JSONCPP_FOUND AND NOT TARGET JsonCPP::jsoncpp)
  if(JSONCPP_LIBRARY_DLL)
    ...
    set_target_properties(
      JsonCPP::jsoncpp
      PROPERTIES IMPORTED_LOCATION_RELEASE "${JSONCPP_LIBRARY_DLL}"
    ...
    if(JSONCPP_LIBRARY_DEBUG_DLL)
      set_property(
        TARGET JsonCPP::jsoncpp
        APPEND
        PROPERTY IMPORTED_CONFIGURATIONS Debug
      )
      set_target_properties(
        JsonCPP::jsoncpp PROPERTIES IMPORTED_LOCATION_DEBUG "${JSONCPP_LIBRARY_DEBUG_DLL}" IMPORTED_IMPLIB_DEBUG
    ...
    set_target_properties(
      JsonCPP::jsoncpp
      PROPERTIES IMPORTED_LOCATION "${JSONCPP_LIBRARY}"
                 INTERFACE_INCLUDE_DIRECTORIES "${JSONCPP_INCLUDE_DIR}"
                 IMPORTED_LINK_INTERFACE_LANGUAGES "CXX"
    )
  endif()
