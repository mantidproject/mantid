...
#
# The ``GENERATE_EXPORT_HEADER`` function can be used to generate a file suitable for preprocessor inclusion which
# contains EXPORT macros to be used in library classes::
#
# GENERATE_EXPORT_HEADER( LIBRARY_TARGET [BASE_NAME <base_name>] [EXPORT_MACRO_NAME <export_macro_name>]
# [EXPORT_FILE_NAME <export_file_name>] [DEPRECATED_MACRO_NAME <deprecated_macro_name>] [NO_EXPORT_MACRO_NAME
# <no_export_macro_name>] [STATIC_DEFINE <static_define>] [NO_DEPRECATED_MACRO_NAME <no_deprecated_macro_name>]
# [DEFINE_NO_DEPRECATED] [PREFIX_NAME <prefix_name>] [CUSTOM_CONTENT_FROM_VARIABLE <variable>] )
#
# The target properties :prop_tgt:`CXX_VISIBILITY_PRESET <<LANG>_VISIBILITY_PRESET>` and
# :prop_tgt:`VISIBILITY_INLINES_HIDDEN` can be used to add the appropriate compile flags for targets.  See the
# documentation of those target properties, and the convenience variables :variable:`CMAKE_CXX_VISIBILITY_PRESET
# <CMAKE_<LANG>_VISIBILITY_PRESET>` and :variable:`CMAKE_VISIBILITY_INLINES_HIDDEN`.
#
# By default ``GENERATE_EXPORT_HEADER()`` generates macro names in a file name determined by the name of the library.
# This means that in the simplest case, users of ``GenerateExportHeader`` will be equivalent to:
#
# .. code-block:: cmake
#
# set(CMAKE_CXX_VISIBILITY_PRESET hidden) set(CMAKE_VISIBILITY_INLINES_HIDDEN 1) add_library(somelib someclass.cpp)
# generate_export_header(somelib) install(TARGETS somelib DESTINATION ${LIBRARY_INSTALL_DIR}) install(FILES someclass.h
...
#
# The CMake fragment will generate a file in the ``${CMAKE_CURRENT_BINARY_DIR}`` called ``somelib_export.h`` containing
# the macros ``SOMELIB_EXPORT``, ``SOMELIB_NO_EXPORT``, ``SOMELIB_DEPRECATED``, ``SOMELIB_DEPRECATED_EXPORT`` and
# ``SOMELIB_DEPRECATED_NO_EXPORT``. They will be followed by content taken from the variable specified by the
# ``CUSTOM_CONTENT_FROM_VARIABLE`` option, if any. The resulting file should be installed with other headers in the
# library.
#
...
#
# creates the macro ``OTHER_NAME_EXPORT`` instead of ``SOMELIB_EXPORT``, but other macros and the generated file name is
# as default:
#
# .. code-block:: cmake
#
...
# .. code-block:: cmake
#
# add_library(shared_variant SHARED ${lib_SRCS}) add_library(static_variant ${lib_SRCS})
# generate_export_header(shared_variant BASE_NAME libshared_and_static) set_target_properties(static_variant PROPERTIES
# COMPILE_FLAGS -DLIBSHARED_AND_STATIC_STATIC_DEFINE)
#
# This will cause the export macros to expand to nothing when building the static library.
#
# If ``DEFINE_NO_DEPRECATED`` is specified, then a macro ``${BASE_NAME}_NO_DEPRECATED`` will be defined This macro can
...
#
# The ``ADD_COMPILER_EXPORT_FLAGS`` function adds ``-fvisibility=hidden`` to :variable:`CMAKE_CXX_FLAGS
# <CMAKE_<LANG>_FLAGS>` if supported, and is a no-op on Windows which does not need extra compiler flags for exporting
# support. You may optionally pass a single argument to ``ADD_COMPILER_EXPORT_FLAGS`` that will be populated with the
# ``CXX_FLAGS`` required to enable visibility support for the compiler/architecture in use.
#
# This function is deprecated.  Set the target properties :prop_tgt:`CXX_VISIBILITY_PRESET <<LANG>_VISIBILITY_PRESET>`
# and :prop_tgt:`VISIBILITY_INLINES_HIDDEN` instead.
...
macro(_test_compiler_hidden_visibility)
  ...
  # Exclude XL here because it misinterprets -fvisibility=hidden even though the check_cxx_compiler_flag passes
  if(NOT GCC_TOO_OLD
...
macro(_test_compiler_has_deprecated)
  # NOTE:  Some Embarcadero compilers silently compile __declspec(deprecated) without error, but this is not a
  # documented feature and the attribute does not actually generate any warnings.
...
function(add_compiler_export_flags)
  ...
  # Either return the extra flags needed in the supplied argument, or to the CMAKE_CXX_FLAGS if no argument is supplied.
  if(ARGC GREATER 0)
    set(${ARGV0}
        "${EXTRA_FLAGS}"
        PARENT_SCOPE
