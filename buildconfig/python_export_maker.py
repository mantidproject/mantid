...
def write_export_file(headerfile, overwrite):
    ...
    if os.path.exists(exportfile) and not overwrite:
        raise RuntimeError("Export file '%s' already exists, use the --overwrite option to overwrite the file." %
                           exportfile)
...
def write_unittest(headerfile, overwrite):
    ...
    if os.path.exists(filename) and not overwrite:
        raise RuntimeError(
            "A unit test file '%s' already exists, use the --overwrite-test option to overwrite the file." % filename)
...
def main():
    ...
    parser.add_option("--overwrite-test",
                      "-w",
                      dest='overwrite_test',
                      action='store_true',
                      default=False,
