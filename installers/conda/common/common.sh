...
function trim_conda() {
  ...
  if [ -f "$bundle_conda_prefix"/bin_tmp/workbench_launcher_jemalloc.standalone ]; then
    # keep handwritten startup script used on Linux so that we use jemalloc
    cp "$bundle_conda_prefix"/bin_tmp/workbench_launcher_jemalloc.standalone "$bundle_conda_prefix"/bin/workbench_launcher_jemalloc
  fi
