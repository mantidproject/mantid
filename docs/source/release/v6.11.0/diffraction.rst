...
- New algorithm :ref:`SaveMDHistoToVTK <algm-SaveMDHistoToVTK>` that saves a :py:obj:`MDHistoWorkspace <mantid.dataobjects.MDHistoWorkspace>` as a ``VTK`` file so that it can be visualized by ``Paraview``.
