...
- A large number of crashes, caused by opening an Algorithm Dialog and populating it with a :py:obj:`WorkspaceGroup <mantid.api.WorkspaceGroup>`, have been fixed.
...
- :ref:`SliceViewer <sliceviewer>` will now support ``W_MATRIX`` log for nonorthogonal axes with :py:obj:`MDHistoWorkspace <mantid.dataobjects.MDHistoWorkspace>`.This prevents a bug related where basis vectors are used to calculate the projection.
