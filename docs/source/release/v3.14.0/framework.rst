...
- :ref:`CalculateQMinMax <algm-CalculateQMinMax>` can now calculate the dynamic ranges also per detector component.
...
- Fixed a bug in :ref:`ConvertToMatrixWorkspace <algm-ConvertToMatrixWorkspace>` with ``Workspace2D`` as the ``InputWorkspace`` not being cloned to the ``OutputWorkspace``. Added support for ragged workspaces.
