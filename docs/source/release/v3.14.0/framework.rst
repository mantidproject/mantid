...
- The ``usersearch.directories`` configuration key has been removed as it was no longer used. ``datasearch.directories`` has become the defacto-standard key for data file locations.
