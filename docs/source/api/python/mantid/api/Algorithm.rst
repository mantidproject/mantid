...
Algorithms operate primarily on data in :py:obj:`Workspaces <mantid.api.Workspace>`. They
will normally take one or more :py:obj:`Workspaces <mantid.api.Workspace>` as an input,
perform some processing on them and provide an output as another
:py:obj:`~mantid.api.Workspace` (although it is possible to have multiple
outputs).
...
:py:obj:`properties <mantid.kernel.Property>`, that will control how it performs its
...
-  :ref:`Plus <algm-Plus>`
   - An algorithm for adding data in two :py:obj:`Workspaces <mantid.api.Workspace>`
   together
