...
Properties in Mantid are the mechanism by which we pass parameters into :py:obj:`algorithm <mantid.api.Algorithm>`.
There are a number of different types of properties, and these are described below.
...
Properties for holding :py:obj:`Workspaces <mantid.api.Workspace>` are more complicated,
in that they need to hold links both to the workspace name (in the
:py:obj:`AnalysisDataService <mantid.api.AnalysisDataServiceImpl>`) and the workspace
...
workspace's name; other methods interact with a :ref:`shared pointer <Shared Pointer>` to the workspace.
