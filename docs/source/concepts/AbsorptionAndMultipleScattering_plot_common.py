...
def make_sample_workspace(material, num_density, radius, height):
    ...
    SetSample(InputWorkspace=sample_ws,
              Geometry={'Shape': 'Cylinder',
                        'Center': [0.,0.,0.],
                        'Height': float(height),
                        'Radius': float(radius)},
              Material={'ChemicalFormula': material,
...
def get_numerical_results(sample_ws):
    ...
    numerical_abs_corr = 1. / numerical_abs_corr # plots are of 1/correction
...
for name in [sample_ws, mayers_abs, mayers_ms, mayers_abs_corr, mayers_ms_corr,
             carpenter_abs, carpenter_ms, carpenter_abs_corr, carpenter_ms_corr,
             numerical_abs, numerical_ms, numerical_abs_corr, numerical_ms_corr,
             mc_abs, mc_ms, mc_abs_corr, mc_ms_corr]:
