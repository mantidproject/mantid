...
Note that the transmission information can be either collecting via a monitor or
...
This needs to always be specified. The *StateNormalizeToMonitor*. Most parameters
...
radius_cutoff                    Radius above which pixels are not considered  *PositiveFloatParameter*                      Y                               N                0
wavelength_cutoff                Wavelength above which data is not considered *PositiveFloatParameter*                      Y                               N                0
...
for the circular or the rectangular cases needs to be specified.
...
some of the cases the order in which the parameters are set is not always in the
same order and sometimes a parameter can be set multiple times (e.g. via the user file).
These settings are captured in a input dictionary and then processed
...
The state object is constructed via the builder pattern. Each state has its own builder which
which will construct the correct state or sub-state based on the input parameters. Note that
...
hence which sub-state to choose from. This data-driven approach was deliberate since it
for a large amount of the parameters the particular data set defines the concrete values of these
parameters. Note that the coordination of the builders for the different states is performed
by a state director.

Let's have a look at an example of a typical builder. Here we example the builder for scaling.
...
most of the settings are defined in the use file and only few settings are adjusted/provided
...
Future user files will potentially make use of a custom *yaml*-style format. This will
require a new parser which will easily replace the current parser since only a single
interface method (*parse_line* which takes a single line to parse needs to be provided.


Director for *ISISCommandInterface*
************************************
...
in *state_gui_model.py* which is then cosumed by the *GuiStateDirector* in
...
Some of the implementation is placed into *scripts/SANS/sans/algorithm_detail*.
...
5. Perform fitting for the transmission calculation. The sub-steps are:
   ...
   a. Use incident monitor, wavelength settings, transmission detector ids
      and fit settings to initialize :ref:`CalculateTransmission <algm-CalculateTransmission>`
