...
The *ShapeWorkspace* input defines the shape of the *OutputWorkspace*, but not the contents. The *DataWorkspace* provides the data contents in the lower dimensionality cut, which will be replicated over. This algorithm operates on :py:obj:`MDHistoWorkspace <mantid.dataobjects.MDHistoWorkspace>` inputs and provides a :py:obj:`MDHistoWorkspace <mantid.dataobjects.MDHistoWorkspace>` as an output.
