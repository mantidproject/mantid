...
.. testcode:: ExSubLinFit
   ...
   print('Values with subtracted background: {}'.format(np.around(output.y(0))))
...
.. testcode:: ExReturnMean
   ...
   print('Calculated Mean background: {}'.format(np.around(output.y(0))))
   subtracted = input - output
   print('Background subtracted: {}'.format(np.around(subtracted.y(0))))
...
.. testcode:: ExReturnMovingAverage
   ...
   print('Background using moving window average: {0:.4}'.format(output.y(0)[0]))
   print('True minimum: {0:.4}'.format(np.amin(input.y(0))))
