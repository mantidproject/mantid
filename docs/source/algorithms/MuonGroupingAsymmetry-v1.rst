...
.. testcode:: SinglePeriod
    ...
    input_workspace = simpleapi.CreateWorkspace(dataX, dataY, NSpec=1)
    input_workspace.getSpectrum(0).setDetectorID(1)
    # Workspace must have the number of good frames set
    simpleapi.AddSampleLog(Workspace=input_workspace, LogName='goodfrm', LogText="10")

    pre_processed_workspace = simpleapi.MuonPreProcess(InputWorkspace=input_workspace)

    output_workspace = simpleapi.MuonGroupingAsymmetry(InputWorkspace=pre_processed_workspace,
                                                       GroupName="fwd",
                                                       Grouping=[1])
...
.. testcode:: MultiPeriod
    ...
    dataY_period2 = [30, 40, 50, 40, 30] + \
                    [40, 50, 60, 50, 40]
    ws1 = simpleapi.CreateWorkspace(dataX, dataY_period1, NSpec=2)
    ws2 = simpleapi.CreateWorkspace(dataX, dataY_period2, NSpec=2)
    simpleapi.AddSampleLog(Workspace=ws1, LogName='goodfrm', LogText="10")
    simpleapi.AddSampleLog(Workspace=ws2, LogName='goodfrm', LogText="10")
    for i in range(2):
    ...
    output_workspace = simpleapi.MuonGroupingAsymmetry(InputWorkspace=multi_period_data,
                                                       GroupName="fwd",
                                                       Grouping=[1, 2],
                                                       SummedPeriods=[1, 2])
