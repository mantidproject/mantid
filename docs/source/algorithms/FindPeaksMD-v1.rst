...
event-based data it migth be beneficial to use the `UseNumberOfEventsNormalization` option. This
...
the events are weighted this will result in boxes with signal counts larger than one for peaks and for the majority of background counts this will 1. Hence it is possible to discrimiate between peaks and background. Note that using `UseNumberOfEventsNormalization` does not make sense for all scenarios and infact might produce unuseful results for your particalar case.
