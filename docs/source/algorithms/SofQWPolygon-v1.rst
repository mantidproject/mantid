...
each detector; this algorithm does not utilize the `AngularWidths` optional 
input property. The signal and error of the rebinned data (in :math:`Q-\Delta 
E` space) is then the sum of the contributing pixels in each bin weighted by 
their fractional overlap area. Unlike the more precise 
