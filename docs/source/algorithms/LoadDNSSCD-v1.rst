...
- *SaveHuberTo* should contain a name of the :py:obj:`TableWorkspace <mantid.api.ITableWorkspace>` where sample rotation angles (Huber) read from the data files will be saved. If the specified workspace exists, it will be overwritten.

- *LoadHuberFrom* should contain a name of the :py:obj:`TableWorkspace <mantid.api.ITableWorkspace>`. The workspace must exist and contain one column with the name *Huber(degrees)*, where the sample rotation angles are specified.
