...
Workspace groups are processed member by member. *OutputWorkspace* and *OutputProfileWorkspace* can therefore be returned as workspace groups, although *OutputWorkspace* must be requested whenever a grouped profile is requested. *OutputFitWorkspace* is not supported for a workspace-group input because a fit workspace may not exist for every member when fallback occurs.

Previous Versions
-----------------

For version 2 of the algorithm, see :ref:`FindReflectometryLines-v2 <algm-FindReflectometryLines-v2>`.
