...
Performs an axis-aligned transpose of a :py:obj:`MDHistoWorkspace <mantid.dataobjects.MDHistoWorkspace>`. Default *Axes* setting gives not transpose. It is possible to remove dimensions from the input workspace by omitting those dimension indexes from the *Axes* property. *Axes* are zero-based indexes.
