...
childEnvironment(const fs::path &dirOfExe) {
  ...
  // due to Chromium sandboxing restrictions. It would appear there is no more fine-grained way to control
  // the restrictions: https://doc.qt.io/qt-5/qtwebengine-platform-notes.html
 ...
 *
 * It is a drop-in replacement for the .exe generated by setuptools on Windows
 * and harmonises startup logic into a single place.
 *
 * @param argc The number of arguments passed on the command line
 * @param argv The array of command line arguments
 */
