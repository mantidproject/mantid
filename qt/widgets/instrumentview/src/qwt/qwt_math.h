...
inline double qwtSqr(const double x) { return x * x; }
...
inline QwtDoublePoint qwtPolar2Pos(const QwtDoublePoint &pole, double radius,
                                   double angle) {
  const double x = pole.x() + radius * ::cos(angle);
  const double y = pole.y() - radius * ::sin(angle);
...
inline QwtDoublePoint qwtDegree2Pos(const QwtDoublePoint &pole, double radius,
                                    double angle) {
  return qwtPolar2Pos(pole, radius, angle / 180.0 * M_PI);
}
