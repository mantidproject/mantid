...
*/
void QwtPainter::setDeviceClipping(bool enable) { d_deviceClipping = enable; }
...
*/
void QwtPainter::setSVGMode(bool on) { d_SVGMode = on; }

bool QwtPainter::isSVGMode() { return d_SVGMode; }
...
*/
void QwtPainter::setMetricsMap(const QwtMetricsMap &map) { d_metricsMap = map; }

/*!
   Reset the metrics map to the ratio 1:1
   \sa QwtPainter::setMetricsMap(), QwtPainter::resetMetricsMap()
*/
void QwtPainter::resetMetricsMap() { d_metricsMap = QwtMetricsMap(); }

/*!
  \return Metrics map
*/
const QwtMetricsMap &QwtPainter::metricsMap() { return d_metricsMap; }
...
*/
void QwtPainter::fillRect(QPainter *painter, const QRect &rect,
                          const QBrush &brush) {
  if (!rect.isValid())
    return;
...
*/
void QwtPainter::drawPie(QPainter *painter, const QRect &rect, int a,
                         int alen) {
  const QRect r = d_metricsMap.layoutToDevice(rect, painter);
...
/*!
    Wrapper for QPainter::drawText()
*/
void QwtPainter::drawText(QPainter *painter, const QRect &rect, int flags,
                          const QString &text) {
  QRect textRect = d_metricsMap.layoutToDevice(rect, painter);
#if QT_VERSION < 0x040000
  if (d_SVGMode && (flags == 0 || flags & Qt::AlignVCenter) &&
...
void QwtPainter::drawColoredArc(QPainter *painter, const QRect &rect, int peak,
                                int arc, int interval, const QColor &c1,
                                const QColor &c2) {
  int h1, s1, v1;
  int h2, s2, v2;
...
void QwtPainter::drawFocusRect(QPainter *painter, QWidget *widget,
                               const QRect &rect) {
#if QT_VERSION < 0x040000
  widget->style().drawPrimitive(QStyle::PE_FocusRect, painter, rect,
                                widget->colorGroup());
#else
  ...
  widget->style()->drawPrimitive(QStyle::PE_FrameFocusRect, &opt, painter,
                                 widget);
#endif
}
