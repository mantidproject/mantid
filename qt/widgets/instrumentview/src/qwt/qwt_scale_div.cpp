...
QwtScaleDiv::QwtScaleDiv()
    : d_lowerBound(0.0), d_upperBound(0.0), d_isValid(false) {}
...
QwtScaleDiv::QwtScaleDiv(const QwtDoubleInterval &interval,
    ...
    : d_lowerBound(interval.minValue()), d_upperBound(interval.maxValue()),
      d_isValid(true) {
...
QwtScaleDiv::QwtScaleDiv(double lowerBound, double upperBound,
                         QwtValueList ticks[NTickTypes])
    : d_lowerBound(lowerBound), d_upperBound(upperBound), d_isValid(true) {
  for (int i = 0; i < NTickTypes; i++)
    d_ticks[i] = ticks[i];
}
...
int QwtScaleDiv::operator==(const QwtScaleDiv &other) const {
  if (d_lowerBound != other.d_lowerBound ||
      d_upperBound != other.d_upperBound || d_isValid != other.d_isValid) {
    return false;
  }
...
bool QwtScaleDiv::isValid() const { return d_isValid; }
