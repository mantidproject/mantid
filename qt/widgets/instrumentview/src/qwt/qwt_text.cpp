...
const QwtTextEngine *
QwtTextEngineDict::textEngine(const QString &text,
                              QwtText::TextFormat format) const {
  if (format == QwtText::AutoText) {
    for (EngineMap::const_iterator it = d_map.begin(); it != d_map.end();
         ++it) {
...
const QwtTextEngine *
QwtTextEngineDict::textEngine(QwtText::TextFormat format) const {
...
public:
  PrivateData()
      : renderFlags(Qt::AlignCenter), backgroundPen(Qt::NoPen),
        backgroundBrush(Qt::NoBrush), paintAttributes(0), layoutAttributes(0),
        textEngine(NULL) {}
...
public:
  void invalidate() { textSize = QSize(); }
...
/*!
   ...
   \sa setText()
*/
QString QwtText::text() const { return d_data->text; }
...
/*!
   ...
   \sa setRenderFlags()
*/
int QwtText::renderFlags() const { return d_data->renderFlags; }
...
//! Return the font.
QFont QwtText::font() const { return d_data->font; }
...
QColor QwtText::color() const { return d_data->color; }
...
*/
QPen QwtText::backgroundPen() const { return d_data->backgroundPen; }
...
*/
QBrush QwtText::backgroundBrush() const { return d_data->backgroundBrush; }
...
int QwtText::heightForWidth(int width, const QFont &defaultFont) const {
  ...
  if (d_data->layoutAttributes & MinimumLayout) {
    ...
    d_data->textEngine->textMargins(font, d_data->text, left, right, top,
                                    bottom);

    h = d_data->textEngine->heightForWidth(font, d_data->renderFlags,
                                           d_data->text, width + left + right);
...
QSize QwtText::textSize(const QFont &defaultFont) const {
  ...
  if (!d_layoutCache->textSize.isValid() || d_layoutCache->font != font) {
    d_layoutCache->textSize =
        d_data->textEngine->textSize(font, d_data->renderFlags, d_data->text);
...
void QwtText::draw(QPainter *painter, const QRect &rect) const {
  ...
  d_data->textEngine->draw(painter, expandedRect, d_data->renderFlags,
                           d_data->text);
