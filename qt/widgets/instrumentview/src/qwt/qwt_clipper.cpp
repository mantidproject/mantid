...
private:
  ...
  QPoint intersectEdge(const QPoint &p1, const QPoint &p2, Edge edge) const;
...
public:
  ...
  QwtArray<QwtDoubleInterval> clipCircle(const QwtDoublePoint &,
                                         double radius) const;

private:
  QList<QwtDoublePoint> cuttingPoints(Edge, const QwtDoublePoint &pos,
                                      double radius) const;
  double toAngle(const QwtDoublePoint &, const QwtDoublePoint &) const;
};
#endif

QwtPolygonClipper::QwtPolygonClipper(const QRect &r) : QRect(r) {}

inline void QwtPolygonClipper::addPoint(QwtPolygon &pa, uint pos,
...
QPoint QwtPolygonClipper::intersectEdge(const QPoint &p1, const QPoint &p2,
                                        Edge edge) const {
  int x = 0, y = 0;
  double m = 0;
...
void QwtPolygonClipper::clipEdge(Edge edge, const QwtPolygon &pa,
                                 QwtPolygon &cpa) const {
  if (pa.count() == 0) {
    cpa.resize(0);
    return;
  }
...
inline void QwtPolygonClipperF::addPoint(QwtPolygonF &pa, uint pos,
                                         const QwtDoublePoint &point) const {
  if (uint(pa.size()) <= pos)
    pa.resize(pos + 5);
...
void QwtPolygonClipperF::clipEdge(Edge edge, const QwtPolygonF &pa,
                                  QwtPolygonF &cpa) const {
  if (pa.count() == 0) {
    cpa.resize(0);
    return;
  }
...
QwtCircleClipper::QwtCircleClipper(const QwtDoubleRect &r) : QwtDoubleRect(r) {}

QwtArray<QwtDoubleInterval>
QwtCircleClipper::clipCircle(const QwtDoublePoint &pos, double radius) const {
  ...
  if (points.size() <= 0) {
    ...
    const int in = contains(
        qwtPolar2Pos(pos, radius, angles[0] + (angles[1] - angles[0]) / 2));
    if (in) {
...
double QwtCircleClipper::toAngle(const QwtDoublePoint &from,
                                 const QwtDoublePoint &to) const {
  if (from.x() == to.x())
...
QList<QwtDoublePoint> QwtCircleClipper::cuttingPoints(Edge edge,
...
*/
QwtArray<QwtDoubleInterval>
QwtClipper::clipCircle(const QwtDoubleRect &clipRect,
