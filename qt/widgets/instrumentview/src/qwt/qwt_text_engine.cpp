...
//! Constructor
QwtPlainTextEngine::QwtPlainTextEngine() { d_data = new PrivateData; }

//! Destructor
QwtPlainTextEngine::~QwtPlainTextEngine() { delete d_data; }
...
*/
QSize QwtPlainTextEngine::textSize(const QFont &font, int flags,
                                   const QString &text) const {
  const QFontMetrics fm(font);
  const QRect rect =
      fm.boundingRect(0, 0, QWIDGETSIZE_MAX, QWIDGETSIZE_MAX, flags, text);
...
*/
bool QwtPlainTextEngine::mightRender(const QString &) const { return true; }
...
QSize QwtRichTextEngine::textSize(const QFont &font, int flags,
                                  const QString &text) const {
  QwtRichTextDocument doc(text, flags, font);
...
*/
void QwtRichTextEngine::textMargins(const QFont &, const QString &, int &left,
