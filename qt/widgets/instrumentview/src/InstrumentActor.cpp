...
void InstrumentActor::initMaskHelper() const {
  ...
  try {
    ...
    QMessageBox::warning(nullptr, "Mantid - Warning", "An error occurred when extracting the mask.", "OK");
  }
}
...
void InstrumentActor::calculateIntegratedSpectra(const Mantid::API::MatrixWorkspace &workspace) {
  ...
  std::replace_if(m_specIntegrs.begin(), m_specIntegrs.end(), [](double x) { return !std::isfinite(x); },
