...
public:
  ...
  static PlotFitAnalysisPaneModelTest *createSuite() {
    return new PlotFitAnalysisPaneModelTest();
  }

  static void destroySuite(PlotFitAnalysisPaneModelTest *suite) {
    delete suite;
  }
  ...
  void
  test_that_calculateEstimate_returns_zero_parameters_if_the_workspace_does_not_exist_in_the_ADS() {
    AnalysisDataService::Instance().clear();
    ...
    TS_ASSERT_EQUALS(
        function->asString(),
        "name=FlatBackground,A0=0;name=Gaussian,Height=0,PeakCentre=0,Sigma=0");
    TS_ASSERT(!m_model->hasEstimate());
  }

  void
  test_that_calculateEstimate_returns_an_estimate_if_the_workspace_does_exist_in_the_ADS() {
    const auto function = m_model->calculateEstimate(m_workspaceName, m_range);

    TS_ASSERT_EQUALS(function->asString(),
                     "name=FlatBackground,A0=2;name=Gaussian,Height=0,"
