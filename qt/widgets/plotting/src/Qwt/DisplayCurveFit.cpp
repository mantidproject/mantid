...
std::map<DisplayCurveFit::dcRange, QString> const DisplayCurveFit::m_dcRangeToQString{
    {DisplayCurveFit::dcRange::fit, QString("fit")}, {DisplayCurveFit::dcRange::evaluate, QString("evaluate")}};

/// Constructor
DisplayCurveFit::DisplayCurveFit(QWidget *parent) : API::MantidWidget(parent), m_rangeSelector() {
 ...
 */
DisplayCurveFit::curveTypes DisplayCurveFit::getCurvesForWorkspace(const Mantid::API::MatrixWorkspace_sptr &workspace) {
  QStringList curveNames = m_uiForm.fitPlot->getCurvesForWorkspace(workspace);
  curveNames = curveNames + m_uiForm.residualsPlot->getCurvesForWorkspace(workspace);
 ...
 * workspace
 */
QPair<double, double> DisplayCurveFit::getCurveRange(const Mantid::API::MatrixWorkspace_sptr &workspace) {
  curveTypes typesFound = this->getCurvesForWorkspace(workspace);
  if (typesFound.size() == 0) {
    throw std::runtime_error("No fitting curves associated to workspace" + workspace->getName());
  }
 ...
 */
void DisplayCurveFit::addSpectrum(const curveType &aType, const Mantid::API::MatrixWorkspace_sptr &workspace,
 ...
 */
void DisplayCurveFit::addRangeSelector(const dcRange &adcRange, RangeSelector::SelectType aType) {
  if (m_rangeSelector.find(adcRange) == m_rangeSelector.end()) {
    const QString &dcRangeName(m_dcRangeToQString.at(adcRange));
    m_rangeSelector.emplace(adcRange, m_uiForm.fitPlot->addRangeSelector(dcRangeName, aType));
    switch (adcRange) {
    case dcRange::fit:
...
void DisplayCurveFit::addResidualsZeroline() {
  ...
  auto residualsZeroline = m_uiForm.residualsPlot->addRangeSelector(QString("zeroLine"), RangeSelector::YSINGLE);
...
DisplayCurveFit::curveType DisplayCurveFit::nameToType(const QString &name) const {
  auto foundPair = std::find_if(m_curveTypeToQString.begin(), m_curveTypeToQString.end(),
