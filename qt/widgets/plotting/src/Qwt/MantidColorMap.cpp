 ...
 */
QString MantidColorMap::chooseColorMap(const QString &previousFile, QWidget *parent) {
  ...
  fileselection = QFileDialog::getOpenFileName(parent, "Pick a Colormap", colormapdir, "Colormaps (*.map *.MAP)");
...
QString MantidColorMap::exists(const QString &filename) {
  ...
  if (info.exists())
    ...
    throw std::invalid_argument("Colormap file does not exist " + filename.toStdString());
}
...
MantidColorMap::MantidColorMap()
    : QwtColorMap(QwtColorMap::Indexed), m_scale_type(MantidColorMap::ScaleType::Log10), m_colors(0), m_num_colors(0),
 ...
 */
MantidColorMap::MantidColorMap(const QString &filename, MantidColorMap::ScaleType type)
...
void MantidColorMap::setupDefaultMap() {
  ...
  // wouldn't work.
  std::string colorstring = "0 172 252@0 170 252@0 168 252@0 164 252@0 160 252@0 156 252@0 152 252@0 "
