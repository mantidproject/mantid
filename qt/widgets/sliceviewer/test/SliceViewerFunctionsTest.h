...
SliceDefinition get_slice_definition(const size_t numberOfDimensions, const Mantid::Kernel::VMD_t minValue,
...
std::vector<Mantid::Geometry::MDHistoDimension_sptr> get_dimensions_collection(const size_t numberOfDimensions,
  ...
  for (size_t index = 0; index < numberOfDimensions; ++index) {
    ...
    auto dimension = std::make_shared<Mantid::Geometry::MDHistoDimension>("Distance", "Dist", frame, minConverted,
                                                                          maxConverted, numberOfBins);
...
public:
  bool do_test_slice_lies_in_workspace_boundaries(const std::string &sliceLiesWithinWorkspaceBoundary) {
    // Arrange
    const size_t numberOfDimensions = 3;
    Mantid::Kernel::VMD_t minValue = 1;
    Mantid::Kernel::VMD_t maxValue = 3;

    auto sliceDefinition = get_slice_definition(numberOfDimensions, minValue, maxValue);
    auto dimensions =
        get_dimensions_collection(numberOfDimensions, minValue, maxValue, sliceLiesWithinWorkspaceBoundary);

    // Act
    return doesSliceCutThroughWorkspace(sliceDefinition.min, sliceDefinition.max, dimensions);
  }
