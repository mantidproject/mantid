...
class PeakRepresentationEllipsoidExposeProtectedWrapper : public PeakRepresentationEllipsoid {
public:
  PeakRepresentationEllipsoidExposeProtectedWrapper(
      const Mantid::Kernel::V3D &origin, const std::vector<double> &peakRadius,
      const std::vector<double> &backgroundInnerRadius, const std::vector<double> &backgroundOuterRadius,
      const std::vector<Mantid::Kernel::V3D> &directions,
      std::shared_ptr<Mantid::SliceViewer::EllipsoidPlaneSliceCalculator> calculator)
      : PeakRepresentationEllipsoid(origin, peakRadius, backgroundInnerRadius, backgroundOuterRadius, directions,
                                    std::move(calculator))
...
public:
  void test_getRadius_gets_radius_if_background_is_not_shown() {
    ...
    auto peak = providePeakRepresentation(peakRadii[0], peakRadii[1], peakRadii[2]);

    // Act + Assert
    const double delta = 1e-5;
    TSM_ASSERT_DELTA("The peak radius of the major axis should be shown which is 4.", peakRadii[0],
                     peak.getEffectiveRadius(), delta);
  }
  ...
  void test_getRadius_gets_outer_background_radius_if_background_is_shown() {
    ...
    TSM_ASSERT_DELTA("The peak radius of the major axis should be shown which is 4+2.", expectedEffectiveRadius,
                     peak.getEffectiveRadius(), delta);
  }
  ...
  void test_that_setting_slice_point_to_intersect_produces_valid_drawing_information() {
    ...
    TS_ASSERT_DELTA(expectedOpacityAtDistance, drawingInformationEllipse->peakOpacityAtDistance, delta);
    TSM_ASSERT("Should have an angle of 35.",
               isAngleEitherValueOr180DegreesRoated(expectedAngle, drawingInformationEllipse->angle));
  }
  ...
  void test_getBoundingBox_with_offset_origin() {
    ...
    const double r3 = 3;
    const double angle = 32;
    auto peak = providePeakRepresentation(r1, r2, r3, angle, originX, originY, originZ);
...
private:
  PeakRepresentationEllipsoid providePeakRepresentation(double r1, double r2, double r3, double angle = 0,
    ...
    auto calculator = std::make_shared<Mantid::SliceViewer::EllipsoidPlaneSliceCalculator>();

    return PeakRepresentationEllipsoid(origin, peakRadii, backgroundRadiiInner, backgroundRadiiOuter, directions,
                                       calculator);
  }

  PeakRepresentationEllipsoidExposeProtectedWrapper providePeakRepresentationWrapper(double r1, double r2, double r3,
                                                                                     double angle) {
    ...
    return PeakRepresentationEllipsoidExposeProtectedWrapper(origin, peakRadii, backgroundRadiiInner,
                                                             backgroundRadiiOuter, directions, calculator);
  }
};
