...
----------------------------------------------------------*/
class EXPORT_OPT_MANTIDQT_SLICEVIEWER ProxyCompositePeaksPresenter : public QObject, public UpdateableOnDemand {
public:
  ProxyCompositePeaksPresenter(std::shared_ptr<CompositePeaksPresenter> compositePresenter);
  ProxyCompositePeaksPresenter();
  ~ProxyCompositePeaksPresenter() override;
  size_t size() const;
  void update();

  void setForegroundColor(std::shared_ptr<const Mantid::API::IPeaksWorkspace> ws, const PeakViewColor & /*color*/);
  /// Change the background representation for the peaks of this workspace
  void setBackgroundColor(std::shared_ptr<const Mantid::API::IPeaksWorkspace> ws, const PeakViewColor & /*color*/);
  /// Get the foreground colour corresponding to the workspace
  PeakViewColor getForegroundPeakViewColor(std::shared_ptr<const Mantid::API::IPeaksWorkspace> ws) const;
  /// Get the background colour corresponding to the workspace
  PeakViewColor getBackgroundPeakViewColor(std::shared_ptr<const Mantid::API::IPeaksWorkspace> ws) const;
  /// Determine wheter the background is shown or not.
  bool getShowBackground(std::shared_ptr<const Mantid::API::IPeaksWorkspace> ws) const;
  ...
  void setBackgroundRadiusShown(std::shared_ptr<const Mantid::API::IPeaksWorkspace> ws, const bool shown);
  ...
  void hideInPlot(std::shared_ptr<const Mantid::API::IPeaksWorkspace> peaksWS, const bool hide);
  /// zoom in on a peak.
  void zoomToPeak(std::shared_ptr<const Mantid::API::IPeaksWorkspace> peaksWS, const int peakIndex);
  /// sort the peaks workspace.
  void sortPeaksWorkspace(std::shared_ptr<const Mantid::API::IPeaksWorkspace> peaksWS,
  ...
  bool getIsHidden(std::shared_ptr<const Mantid::API::IPeaksWorkspace> peaksWS) const;
