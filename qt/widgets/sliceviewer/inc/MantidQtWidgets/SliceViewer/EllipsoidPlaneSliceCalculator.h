...
template <class T> typename std::enable_if<!std::numeric_limits<T>::is_integer, bool>::type almost_equal(T x, T y) {
...
createEllipsoidMatrixInXYZFrame(std::vector<Mantid::Kernel::V3D> directions, std::vector<double> radii);
...
getProjectionLengths(const std::vector<Mantid::Kernel::V3D> &directions, std::vector<double> radii);

MantidQt::SliceViewer::PeakBoundingBox EXPORT_OPT_MANTIDQT_SLICEVIEWER
getPeakBoundingBoxForEllipsoid(const std::vector<Mantid::Kernel::V3D> &directions, const std::vector<double> &radii,
                               const Mantid::Kernel::V3D &originEllipsoid);

struct EXPORT_OPT_MANTIDQT_SLICEVIEWER SliceEllipseInfo {
  SliceEllipseInfo(Mantid::Kernel::V3D origin = Mantid::Kernel::V3D(0, 0, 0), double radiusMajorAxis = 0.0,
                   double radiusMinorAxis = 0.0, double angle = 0.0)
...
public:
  SliceEllipseInfo getSlicePlaneInfo(std::vector<Mantid::Kernel::V3D> directions, std::vector<double> radii,
                                     Mantid::Kernel::V3D originEllipsoid, double zPlane) const;
  double getZoomOutFactor() const;

private:
  SliceEllipseInfo getSolutionForEllipsoid(const Kernel::Matrix<double> &m, double zPlane,
