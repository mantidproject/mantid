...
ZMinAndMax getZMinAndMax(const Mantid::API::IMDWorkspace_sptr &workspace,
  ...
  for (size_t dimIndex = 0; dimIndex < numberOfDimensions; ++dimIndex) {
    Mantid::Geometry::IMDDimension_const_sptr dimensionMappedToZ = workspace->getDimension(dimIndex);
...
getDirectionsForEllipticalPeak(const Mantid::Geometry::IPeak &peak,
...
PeakViewFactory::PeakViewFactory(Mantid::API::IMDWorkspace_sptr mdWS, Mantid::API::IPeaksWorkspace_sptr peaksWS,
                                 QwtPlot *plot, QWidget *parent, const int plotXIndex, const int plotYIndex,
                                 const size_t colorNumber)
    : PeakOverlayViewFactoryBase(plot, parent, plotXIndex, plotYIndex, colorNumber), m_mdWS(std::move(mdWS)),
      m_peaksWS(std::move(peaksWS)),
      m_calculator(std::make_shared<Mantid::SliceViewer::EllipsoidPlaneSliceCalculator>()) {
  setForegroundAndBackgroundColors(colorNumber);
}

PeakViewFactory::~PeakViewFactory() {}

std::shared_ptr<PeakOverlayView>
PeakViewFactory::createView(PeaksPresenter *const presenter,
  ...
  for (auto &peakRepresentation : peakRepresentations) {
    ...
    peakRepresentation = createSinglePeakRepresentation(peak, position, transform);
  ...
  return std::make_shared<PeakView>(presenter, m_plot, m_parent, peakRepresentations, m_plotXIndex, m_plotYIndex,
                                    m_foregroundColor, m_backgroundColor, largestEffectiveRadius);
}

PeakRepresentation_sptr
PeakViewFactory::createSinglePeakRepresentation(const Mantid::Geometry::IPeak &peak, Mantid::Kernel::V3D position,
                                                Mantid::Geometry::PeakTransform_const_sptr transform) const {
  ...
  if (shapeName == Mantid::DataObjects::PeakShapeSpherical::sphereShapeName()) {
    ...
    peakRepresentation = createPeakRepresentationCross(position, std::move(transform));
  }
  return peakRepresentation;
}

PeakRepresentation_sptr
PeakViewFactory::createPeakRepresentationCross(Mantid::Kernel::V3D position,
  ...
  return std::make_shared<PeakRepresentationCross>(position, zMinAndMax.zMax, zMinAndMax.zMin);
}

PeakRepresentation_sptr PeakViewFactory::createPeakRepresentationSphere(Mantid::Kernel::V3D position,
                                                                        const Mantid::Geometry::IPeak &peak) const {
  const auto &shape = peak.getPeakShape();
  const auto &sphericalShape = dynamic_cast<const Mantid::DataObjects::PeakShapeSpherical &>(shape);

  // Get the radius
  const auto peakRadius = sphericalShape.radius().get();

  // Get the background inner radius. If it does not exist, default to radius.
  const auto backgroundInnerRadiusOptional = sphericalShape.backgroundInnerRadius();
  const auto backgroundInnerRadius =
      backgroundInnerRadiusOptional.is_initialized() ? backgroundInnerRadiusOptional.get() : peakRadius;

  // Get the background outer radius. If it does not exist, default to radius.
  const auto backgroundOuterRadiusOptional = sphericalShape.backgroundOuterRadius();
  const auto backgroundOuterRadius =
      backgroundOuterRadiusOptional.is_initialized() ? backgroundOuterRadiusOptional.get() : peakRadius;

  return std::make_shared<PeakRepresentationSphere>(position, peakRadius, backgroundInnerRadius, backgroundOuterRadius);
}

PeakRepresentation_sptr PeakViewFactory::createPeakRepresentationEllipsoid(Mantid::Kernel::V3D position,
                                                                           const Mantid::Geometry::IPeak &peak) const {
  const auto &shape = peak.getPeakShape();
  const auto &ellipsoidShape = dynamic_cast<const Mantid::DataObjects::PeakShapeEllipsoid &>(shape);
  ...
  return std::make_shared<PeakRepresentationEllipsoid>(position, abcRadii, abcRadiiBackgroundInner,
                                                       abcRadiiBackgroundOuter, directions, m_calculator);
}

void PeakViewFactory::swapPeaksWorkspace(Mantid::API::IPeaksWorkspace_sptr &peaksWS) { m_peaksWS = peaksWS; }

void PeakViewFactory::setForegroundAndBackgroundColors(const size_t colourNumber) {
  PeakPalette<PeakViewColor> defaultPalette;
  const auto peakColourEnum = defaultPalette.foregroundIndexToColour(static_cast<int>(colourNumber));
  const auto backColourEnum = defaultPalette.backgroundIndexToColour(static_cast<int>(colourNumber));
...
void PeakViewFactory::getNonOrthogonalInfo(NonOrthogonalAxis &info) {
  if (API::requiresSkewMatrix(*m_mdWS)) {
    ...
    Mantid::Kernel::DblMatrix skewMatrixDbl(numberOfDimensions, numberOfDimensions, true);
    ...
    info.dimMissing = API::getMissingHKLDimensionIndex(m_mdWS, info.dimX, info.dimY);
  }
}
} // namespace SliceViewer
} // namespace MantidQt
