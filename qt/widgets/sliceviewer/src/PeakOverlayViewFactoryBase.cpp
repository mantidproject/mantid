...
PeakOverlayViewFactoryBase::PeakOverlayViewFactoryBase(QwtPlot *plot, QWidget *parent, const int plotXIndex,
                                                       const int plotYIndex, const size_t workspaceNumber)
    : PeakOverlayViewFactory(), m_plot(plot), m_parent(parent), m_plotXIndex(plotXIndex), m_plotYIndex(plotYIndex) {
  if (!plot)
  ...
  auto foregroundViewColor = defaultPalette.foregroundIndexToColour(static_cast<int>(workspaceNumber));
  auto backgroundViewColor = defaultPalette.backgroundIndexToColour(static_cast<int>(workspaceNumber));
