...
Constructor
*/
ProxyCompositePeaksPresenter::ProxyCompositePeaksPresenter(std::shared_ptr<CompositePeaksPresenter> composite)
    : m_compositePresenter(std::move(composite)), m_updateableView(nullptr) {
  m_compositePresenter->registerOwningPresenter(this);
}

ProxyCompositePeaksPresenter::ProxyCompositePeaksPresenter() : m_updateableView(nullptr) {}
...
*/
size_t ProxyCompositePeaksPresenter::size() const { return m_compositePresenter->size(); }
...
*/
void ProxyCompositePeaksPresenter::setForegroundColor(std::shared_ptr<const Mantid::API::IPeaksWorkspace> ws,
...
*/
void ProxyCompositePeaksPresenter::setBackgroundColor(std::shared_ptr<const Mantid::API::IPeaksWorkspace> ws,
                                                      const PeakViewColor &color) {
  m_compositePresenter->setBackgroundColor(std::move(ws), std::move(color));
}

PeakViewColor
ProxyCompositePeaksPresenter::getBackgroundPeakViewColor(std::shared_ptr<const Mantid::API::IPeaksWorkspace> ws) const {
  return m_compositePresenter->getBackgroundPeakViewColor(std::move(ws));
}

PeakViewColor
ProxyCompositePeaksPresenter::getForegroundPeakViewColor(std::shared_ptr<const Mantid::API::IPeaksWorkspace> ws) const {
  return m_compositePresenter->getForegroundPeakViewColor(std::move(ws));
}

bool ProxyCompositePeaksPresenter::getShowBackground(std::shared_ptr<const Mantid::API::IPeaksWorkspace> ws) const {
...
*/
std::string ProxyCompositePeaksPresenter::getTransformName() const { return m_compositePresenter->getTransformName(); }

void ProxyCompositePeaksPresenter::setBackgroundRadiusShown(std::shared_ptr<const Mantid::API::IPeaksWorkspace> ws,
                                                            const bool shown) {
  m_compositePresenter->setBackgroundRadiusShown(std::move(ws), shown);
}

void ProxyCompositePeaksPresenter::remove(std::shared_ptr<const Mantid::API::IPeaksWorkspace> peaksWS) {
  m_compositePresenter->remove(std::move(peaksWS));
}

void ProxyCompositePeaksPresenter::hideInPlot(std::shared_ptr<const Mantid::API::IPeaksWorkspace> peaksWS,
                                              const bool hide) {
  m_compositePresenter->setShown(std::move(peaksWS), !hide);
}

void ProxyCompositePeaksPresenter::zoomToPeak(std::shared_ptr<const Mantid::API::IPeaksWorkspace> peaksWS,
                                              const int peakIndex) {
...
void ProxyCompositePeaksPresenter::updatePeaksWorkspace(
    const std::string &toName, std::shared_ptr<const Mantid::API::IPeaksWorkspace> toWorkspace) {
  if (m_updateableView) {
    m_updateableView->updatePeaksWorkspace(toName, toWorkspace);
  }
}

bool ProxyCompositePeaksPresenter::getIsHidden(std::shared_ptr<const Mantid::API::IPeaksWorkspace> peaksWS) const {
...
int ProxyCompositePeaksPresenter::getZoomedPeakIndex() const { return m_compositePresenter->getZoomedPeakIndex(); }

void ProxyCompositePeaksPresenter::editCommand(EditMode editMode,
                                               const std::weak_ptr<const Mantid::API::IPeaksWorkspace> &target) {
  m_compositePresenter->editCommand(editMode, std::move(target));
}

void ProxyCompositePeaksPresenter::setPeakSizeOnProjection(const double fraction) {
  m_compositePresenter->setPeakSizeOnProjection(fraction);
}

void ProxyCompositePeaksPresenter::setPeakSizeIntoProjection(const double fraction) {
