 ...
 */
QPainterPath getTransformedPainterPath(double angle, double transX, double transY, double scaleX, double scaleY,
...
PeakRepresentationEllipsoid::PeakRepresentationEllipsoid(
    ...
    const std::vector<double> &backgroundInnerRadii, const std::vector<double> &backgroundOuterRadii,
    const std::vector<Mantid::Kernel::V3D> &directions,
    std::shared_ptr<Mantid::SliceViewer::EllipsoidPlaneSliceCalculator> calculator)
    : m_originalOrigin(origin), m_originalDirections(directions), m_origin(origin), m_directions(directions),
      m_peakRadii(peakRadii), m_backgroundInnerRadii(backgroundInnerRadii),
      m_backgroundOuterRadii(backgroundOuterRadii), m_opacityMax(0.8), m_opacityMin(0.0),
      m_cachedOpacityAtDistance(0.0), m_angleEllipse(-1.0), m_showBackgroundRadii(false),
      m_calculator(std::move(calculator)) {
  // Get projection lengths onto the xyz axes of the ellipsoid axes
  auto projections = Mantid::SliceViewer::getProjectionLengths(directions, backgroundOuterRadii);
...
void PeakRepresentationEllipsoid::setSlicePoint(const double &z) {
  ...
  if (Mantid::SliceViewer::checkIfCutExists(m_directions, m_backgroundOuterRadii, m_origin, z)) {

    // Handle the case of the outer background
    auto ellipsoidInfoBackgroundOuter =
        m_calculator->getSlicePlaneInfo(m_directions, m_backgroundOuterRadii, m_origin, z);

    // The angle should be the same for all
    m_angleEllipse = ellipsoidInfoBackgroundOuter.angle;
    m_radiiEllipseBackgroundOuter =
        std::vector<double>{ellipsoidInfoBackgroundOuter.radiusMajorAxis, ellipsoidInfoBackgroundOuter.radiusMinorAxis};
    m_originEllipseBackgroundOuter = ellipsoidInfoBackgroundOuter.origin;

    // Handle the peak radius
    if (Mantid::SliceViewer::checkIfCutExists(m_directions, m_peakRadii, m_origin, z)) {

      auto ellipsoidInfoPeaks = m_calculator->getSlicePlaneInfo(m_directions, m_peakRadii, m_origin, z);
      m_radiiEllipse = std::vector<double>{ellipsoidInfoPeaks.radiusMajorAxis, ellipsoidInfoPeaks.radiusMinorAxis};
      m_originEllipse = ellipsoidInfoPeaks.origin;
    }

    // Handle the inner background radius
    if (Mantid::SliceViewer::checkIfCutExists(m_directions, m_backgroundInnerRadii, m_origin, z)) {

      auto ellipsoidInfoBackgroundInner =
          m_calculator->getSlicePlaneInfo(m_directions, m_backgroundInnerRadii, m_origin, z);
      m_radiiEllipseBackgroundInner = std::vector<double>{ellipsoidInfoBackgroundInner.radiusMajorAxis,
    ...
    m_cachedOpacityAtDistance = m_cachedOpacityGradient[2] * distanceAbs + m_opacityMax;
  } else {
...
void PeakRepresentationEllipsoid::movePosition(Mantid::Geometry::PeakTransform_sptr peakTransform) {
  ...
  m_cachedOpacityGradient = peakTransform->transform(m_originalCachedOpacityGradient);
}
 ...
 */
void PeakRepresentationEllipsoid::showBackgroundRadius(const bool show) { m_showBackgroundRadii = show; }
...
PeakBoundingBox PeakRepresentationEllipsoid::getBoundingBox() const {
  ...
  return getPeakBoundingBoxForEllipsoid(m_directions, m_backgroundOuterRadii, m_origin);
}
...
const Mantid::Kernel::V3D &PeakRepresentationEllipsoid::getOrigin() const { return m_originEllipseBackgroundOuter; }

std::shared_ptr<PeakPrimitives>
PeakRepresentationEllipsoid::getDrawingInformation(PeakRepresentationViewInformation /*viewInformation*/) {
  auto drawingInformation = std::make_shared<PeakPrimitivesEllipse>(
      Mantid::Kernel::V3D() /*peakOrigin*/, 0.0 /*peakOpacityAtDistance*/, 0 /* peakLineWidth */,
      0.0 /*peakInnerRadiusMajorAxis*/, 0.0 /*peakInnerRadiusMinorAxis*/, 0.0 /*backgroundOuterRadiusMajorAxis*/,
      0.0 /*backgroundOuterRadiusMinorAxis*/, 0.0 /*backgroundInnerRadiusMajorAxis*/,
  ...
  if (this->m_showBackgroundRadii) {
    if (m_radiiEllipseBackgroundOuter.size() > 0) {
      drawingInformation->backgroundOuterRadiusMajorAxis = m_radiiEllipseBackgroundOuter[0];
      drawingInformation->backgroundOuterRadiusMinorAxis = m_radiiEllipseBackgroundOuter[1];
    } else {
    ...
    if (m_radiiEllipseBackgroundInner.size() > 0) {
      drawingInformation->backgroundInnerRadiusMajorAxis = m_radiiEllipseBackgroundInner[0];
      drawingInformation->backgroundInnerRadiusMinorAxis = m_radiiEllipseBackgroundInner[1];
    } else {
...
void PeakRepresentationEllipsoid::doDraw(QPainter &painter, PeakViewColor &foregroundColor,
  ...
  auto drawingInformationEllipse = std::static_pointer_cast<PeakPrimitivesEllipse>(drawingInformation);
  ...
  if (drawingInformationEllipse->peakInnerRadiusMajorAxis != zeroRadius &&
    ...
    peakRadiusInnerPath.addEllipse(zeroPoint, drawingInformationEllipse->peakInnerRadiusMajorAxis,
                                   drawingInformationEllipse->peakInnerRadiusMinorAxis);

    // Transform the painter path (rotate, translate, scale)
    auto transformedPeakRadiusInnerPath =
        getTransformedPainterPath(drawingInformationEllipse->angle, viewInformation.xOriginWindow,
  ...
  if (m_showBackgroundRadii) {
    if (drawingInformationEllipse->backgroundOuterRadiusMajorAxis != zeroRadius &&
      ...
      backgroundOuterPath.addEllipse(zeroPoint, drawingInformationEllipse->backgroundOuterRadiusMajorAxis,
                                     drawingInformationEllipse->backgroundOuterRadiusMinorAxis);
      auto transformedBackgroundOuterPath =
          getTransformedPainterPath(drawingInformationEllipse->angle, viewInformation.xOriginWindow,
      ...
      backgroundInnerPath.addEllipse(zeroPoint, drawingInformationEllipse->backgroundInnerRadiusMajorAxis,
                                     drawingInformationEllipse->backgroundInnerRadiusMinorAxis);
      auto transformedBackgroundInnerPath =
          getTransformedPainterPath(drawingInformationEllipse->angle, viewInformation.xOriginWindow,
                                    viewInformation.yOriginWindow, scaleX, scaleY, backgroundInnerPath);

      // Subtract inner fill from outer fill
      QPainterPath backgroundRadiusFill = transformedBackgroundOuterPath.subtracted(transformedBackgroundInnerPath);
...
double PeakRepresentationEllipsoid::getZoomOutFactor() const { return m_calculator->getZoomOutFactor(); }
} // namespace SliceViewer
} // namespace MantidQt
