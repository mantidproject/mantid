...
static void addPeak(size_t num, double x, double y, double z, double radius) {
  ...
  FrameworkManager::Instance().exec("FakeMDEventData", 6, "InputWorkspace", "mdew", "PeakParams", mess.str().c_str(),
                                    "RandomSeed", "1234");
}
...
IMDWorkspace_sptr makeDemoData(bool binned = false) {
  ...
  FrameworkManager::Instance().exec("CreateMDWorkspace", 16, "Dimensions", "3", "Extents", "-10,10,-10,10,-10,10",
                                    "Names", "h,k,l", "Units", "lattice,lattice,lattice", "SplitInto", "5",
                                    "SplitThreshold", "100", "MaxRecursionDepth", "20", "OutputWorkspace", "mdew");
  ...
  IMDEventWorkspace_sptr mdew =
      std::dynamic_pointer_cast<IMDEventWorkspace>(AnalysisDataService::Instance().retrieve("mdew"));
  ...
  if (binned) {
    ...
    //        );
    FrameworkManager::Instance().exec("BinMD", 20, "InputWorkspace", "mdew", "OutputWorkspace", "binned", "AxisAligned",
                                      "0", "BasisVector0", "rx, m, 1.0, 0.0, 0.0", "BasisVector1",
                                      "ry, m, 0.0, 1.0, 0.0", "BasisVector2", "rz, m, 0.0, 0.0, 1.0", "ForceOrthogonal",
                                      "1", "Translation", "-5, -5, -5", "OutputExtents", "0, 10, 0,10, 0,10",
                                      "OutputBins", "100, 100, 100");

    FrameworkManager::Instance().exec("BinMD", 12, "InputWorkspace", "mdew", "OutputWorkspace", "binned_al",
    ...
    FrameworkManager::Instance().exec("Rebin", 8, "Params", "40e3, 1e3, 70e3", "PreserveEvents", "1", "InputWorkspace",
                                      "workspace_2d", "OutputWorkspace", "workspace_2d");

    return std::dynamic_pointer_cast<IMDWorkspace>(AnalysisDataService::Instance().retrieve("binned"));
  } else
