...
auto BuildSubtreeItems::buildRecursively(int insertionIndex, RowLocation const &parent, SubtreeConstIterator current,
  ...
  while (current != end) {
    ...
    if (insertionDepth < currentDepth) {
      current = buildRecursively(currentRow.rowRelativeToParent(), parent.child(lastRowInsertedAtThisDepth.row()),
                                 current, end);
    } else if (insertionDepth > currentDepth) {
      return current;
    } else {
      lastRowInsertedAtThisDepth =
          m_adaptedMainModel.insertChildRow(modelIndexAt(parent), insertionIndex, (*current).cells());
      ++insertionIndex;
      ++current;
    }
  }
...
void BuildSubtreeItems::operator()(RowLocation const &parentOfSubtreeRoot, int firstInsertionIndex,
                                   Subtree const &subtree) {
  if (!subtree.empty()) {
    buildRecursively(firstInsertionIndex, parentOfSubtreeRoot, subtree.cbegin(), subtree.cend());
  }
}
