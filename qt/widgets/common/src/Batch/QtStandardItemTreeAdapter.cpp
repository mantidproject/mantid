...
void QtStandardItemTreeModelAdapter::removeRowFrom(QModelIndexForMainModel const &index) {
  if (index.isValid()) {
...
QModelIndexForMainModel QtStandardItemTreeModelAdapter::appendEmptySiblingRow(QModelIndexForMainModel const &index) {
  auto newIndex = appendEmptyChildRow(index.parent());
  return newIndex.sibling(newIndex.row(), index.column());
}

QModelIndexForMainModel QtStandardItemTreeModelAdapter::appendSiblingRow(QModelIndexForMainModel const &index,
                                                                         std::vector<Cell> const &cells) {
  auto newIndex = appendChildRow(index.parent(), cells);
  return newIndex.sibling(newIndex.row(), index.column());
}

QModelIndexForMainModel QtStandardItemTreeModelAdapter::appendEmptyChildRow(QModelIndexForMainModel const &parent) {
  return appendChildRow(parent, emptyRow(m_model.columnCount()));
}

QModelIndexForMainModel QtStandardItemTreeModelAdapter::appendChildRow(QModelIndexForMainModel const &parent,
                                                                       std::vector<Cell> const &cells) {
...
QModelIndexForMainModel QtStandardItemTreeModelAdapter::insertChildRow(QModelIndexForMainModel const &parent, int row,
                                                                       std::vector<Cell> const &cells) {
...
QModelIndexForMainModel QtStandardItemTreeModelAdapter::insertEmptyChildRow(QModelIndexForMainModel const &parent,
                                                                            int row) {
  return insertChildRow(parent, row, emptyRow(m_model.columnCount()));
}

std::vector<Cell> QtStandardItemTreeModelAdapter::emptyRow(int columnCount) const {
  return std::vector<Cell>(columnCount, m_emptyCellStyle);
}

void QtStandardItemTreeModelAdapter::setCellAtCellIndex(QModelIndexForMainModel const &cellIndex, Cell const &cell) {
  auto *item = modelItemFromIndex(m_model, cellIndex);
  applyCellPropertiesToItem(cell, *item);
}

void QtStandardItemTreeModelAdapter::setCellsAtRow(QModelIndexForMainModel const &firstCellIndex,
                                                   std::vector<Cell> const &cells) {
...
QList<QStandardItem *> QtStandardItemTreeModelAdapter::rowItemsFromCells(std::vector<Cell> const &cells) {
...
std::vector<Cell> QtStandardItemTreeModelAdapter::cellsAtRow(QModelIndexForMainModel const &firstCellIndex) const {
...
Cell QtStandardItemTreeModelAdapter::cellFromCellIndex(QModelIndexForMainModel const &index) const {
  auto *cellItem = modelItemFromIndex(m_model, index);
  return extractCellPropertiesFromItem(*cellItem);
}

QStandardItem const *modelItemFromIndex(QStandardItemModel const &model, QModelIndexForMainModel const &index) {
  if (index.isValid()) {
    auto *item = model.itemFromIndex(index.untyped());
    assertOrThrow(item != nullptr, "modelItemFromIndex: Index must point to a valid item.");
    return item;
  } else
    return model.invisibleRootItem();
}

QStandardItem *modelItemFromIndex(QStandardItemModel &model, QModelIndexForMainModel const &index) {
  if (index.isValid()) {
    auto *item = model.itemFromIndex(index.untyped());
    assertOrThrow(item != nullptr, "modelItemFromIndex: Index must point to a valid item.");
    return item;
