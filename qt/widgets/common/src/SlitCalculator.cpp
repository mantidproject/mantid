...
void SlitCalculator::setInstrument(const std::string &instrumentName) {
  ...
  auto filename = Mantid::API::InstrumentFileFinder::getInstrumentFilename(instrumentName, date);
  // parse the XML that we have found for the definition
  Mantid::Geometry::InstrumentDefinitionParser parser = Mantid::Geometry::InstrumentDefinitionParser(
      filename, instrumentName, Mantid::Kernel::Strings::loadFile(filename));
  // retrieving the mangled name of the instrument
  std::string instrumentNameMangled = parser.getMangledName();
  // See if we have a definition already in the InstrumentDataService
  if (Mantid::API::InstrumentDataService::Instance().doesExist(instrumentNameMangled)) {
    // If it does, set the associated instrument to the one we have found.
    this->instrument = Mantid::API::InstrumentDataService::Instance().retrieve(instrumentNameMangled);
  } else {
...
void SlitCalculator::setupSlitCalculatorWithInstrumentValues(
  ...
  if (slit1Component.get() != nullptr && slit2Component.get() != nullptr && sampleComponent.get() != nullptr) {
...
const std::string &SlitCalculator::getCurrentInstrumentName() { return currentInstrumentName; }
