...
void MuonFitPropertyBrowser::init() {
  ...
  m_widgetSplitter->setSizePolicy(QSizePolicy::Policy::Expanding, QSizePolicy::Policy::Expanding);
...
void MuonFitPropertyBrowser::enumChanged(QtProperty *prop) {
  ...
  if (prop == m_groupsToFit) {
    ...
    if (option == CUSTOM_LABEL) {
      ...
      for (auto iter = m_periodBoxes.constBegin(); iter != m_periodBoxes.constEnd(); ++iter) {
        if (option == iter.key()) {
    ...
    if (!m_browser->isItemVisible(m_multiFitSettingsGroup)) {
      ...
      // turn on only the relevant box
      for (auto iter = m_groupBoxes.constBegin(); iter != m_groupBoxes.constEnd(); ++iter) {
...
void MuonFitPropertyBrowser::populateFunctionNames() {
  ...
  for (auto fnName : names) {
    ...
    for (const auto &category : categories) {
      if ((category == "Muon") || (category == "General") || (category == "Background"))
        muon = true;
    }
    if (muon) {
...
void MuonFitPropertyBrowser::doTFAsymmFit() {
  ...
  try {
    ...
    if (m_compositeFunction->nFunctions() > 1) {
      ...
      alg->setProperty("InputFunction", std::dynamic_pointer_cast<IFunction>(m_functionBrowser->getGlobalFunction()));
    } else {
      alg->setProperty("InputFunction", std::dynamic_pointer_cast<IFunction>(m_compositeFunction->getFunction(0)));
    }

    auto unnorm = m_workspacesToFit;
    std::string tmp = UNNORM;
    bool raw = rawData();
    std::for_each(unnorm.begin(), unnorm.end(), [tmp, raw](std::string &wsName) {
      if (wsName.find(UNNORM) == std::string::npos) {
...
std::map<std::string, double> readMultipleNormalization() {
  ...
  if (AnalysisDataService::Instance().doesExist("MuonAnalysisTFNormalizations")) {
    Mantid::API::ITableWorkspace_sptr table = std::dynamic_pointer_cast<Mantid::API::ITableWorkspace>(
...
void MuonFitPropertyBrowser::runFit() {
  ...
  try {
    ...
    if (m_compositeFunction->name() == "MultiBG") {
      ...
      alg->setProperty("Function", std::dynamic_pointer_cast<IFunction>(m_compositeFunction));
    } else {
      alg->setProperty("Function", std::dynamic_pointer_cast<IFunction>(m_compositeFunction->getFunction(0)));
    }
    if (rawData())
 ...
 */
void MuonFitPropertyBrowser::runSequentialFit() { emit sequentialFitRequested(); }

/**
 * Requests checks and updates prior to running a sequential fit
 */
void MuonFitPropertyBrowser::sequentialFit() { emit preFitChecksRequested(true); }
...
void MuonFitPropertyBrowser::setFitWorkspaces(const std::string &input) {
  ...
  if (AnalysisDataService::Instance().doesExist(outputName() + "_Workspace")) {
    ...
    auto outWs = AnalysisDataService::Instance().retrieveWS<MatrixWorkspace>(outputName() + "_Workspace");
    if (inWs && outWs) {
      outWs->copyExperimentInfoFrom(inWs.get());
    }
  } else if (AnalysisDataService::Instance().doesExist(outputName() + "_Workspaces")) {
    // Output workspace was a group
    auto outGroup = AnalysisDataService::Instance().retrieveWS<WorkspaceGroup>(outputName() + "_Workspaces");
    if (outGroup->size() == m_workspacesToFit.size()) {
      for (size_t i = 0; i < outGroup->size(); i++) {
        auto outWs = std::dynamic_pointer_cast<MatrixWorkspace>(outGroup->getItem(i));
        auto inWs = AnalysisDataService::Instance().retrieveWS<MatrixWorkspace>(m_workspacesToFit[i]);
        if (inWs && outWs) {
          outWs->copyExperimentInfoFrom(inWs.get());
        }
      }
...
void MuonFitPropertyBrowser::finishHandleTF(const IAlgorithm *alg) {
  ...
  setFitWorkspaces(static_cast<std::string>(alg->getProperty("UnNormalizedWorkspaceList")));
  ...
  emit changeWindowTitle(QString("Fit Function (") + "Chi-sq " + " = " + QString::number(quality) + ", " + status +
                         ")");
  if (nWorkspaces == 1) {
 ...
 */
void MuonFitPropertyBrowser::setFunction(const IFunction_sptr func) { createCompositeFunction(func); }
...
void MuonFitPropertyBrowser::setWorkspaceNames(const QStringList &wsNames) {
  ...
  std::transform(wsNames.begin(), wsNames.end(), std::back_inserter(m_workspacesToFit),
 ...
 */
bool MuonFitPropertyBrowser::isMultiFittingMode() const { return m_isMultiFittingMode; }
void MuonFitPropertyBrowser::ConvertFitFunctionForMuonTFAsymmetry(bool enabled) {
  ...
  if (AnalysisDataService::Instance().doesExist("MuonAnalysisTFNormalizations") &&
    ...
    alg->initialize();
    IFunction_sptr old = std::dynamic_pointer_cast<IFunction>(m_compositeFunction);
...
std::string MuonFitPropertyBrowser::TFExtension() const { return (m_boolManager->value(m_TFAsymmMode)) ? UNNORM : ""; }
/**
 ...
 */
void MuonFitPropertyBrowser::addFitBrowserWidget(QWidget *widget,
 ...
 */
void MuonFitPropertyBrowser::continueAfterChecks(bool sequential) { emit functionUpdateAndFitRequested(sequential); }
...
void MuonFitPropertyBrowser::setAllPairs() {
  ...
  for (auto iter = m_groupBoxes.constBegin(); iter != m_groupBoxes.constEnd(); ++iter) {
    ...
    const bool isItGroup = std::any_of(m_groupsList.cbegin(), m_groupsList.cend(),
...
void MuonFitPropertyBrowser::clearPeriodCheckboxes() {
  if (m_periodBoxes.size() > 1) {
    for (auto iter = std::next(m_periodBoxes.constBegin()); iter != m_periodBoxes.constEnd(); ++iter) {
      delete (*iter);
    }
    m_periodBoxes.clear();
  }
...
void MuonFitPropertyBrowser::setTFAsymm(bool state) { m_boolManager->setValue(m_TFAsymmMode, state); }
