...
bool MantidTreeWidgetItem::operator<(const QTreeWidgetItem &other) const {
  ...
  // If both should eb sorted and the scheme is set to ByMemorySize ...
    ...
    if(this->getMemorySize() == 0){
      ...
      thisGroupSize = getGroupMemorySize(boost::dynamic_pointer_cast<WorkspaceGroup>(theWorkspace));
    }
    //If workspace other is a group since groups return 0:
    if(mantidOther->getMemorySize() == 0){
      auto theWorkspace = mantidOther->data(0, Qt::UserRole).value<Workspace_sptr>();
      otherGroupSize = getGroupMemorySize(boost::dynamic_pointer_cast<WorkspaceGroup>(theWorkspace));
    }
...
std::size_t MantidTreeWidgetItem::getGroupMemorySize(WorkspaceGroup_sptr group) const {
  ...
  for(auto workspace : workspaces){
    ...
    if(workspace->getMemorySize() == 0){
      total = total + getGroupMemorySize(boost::dynamic_pointer_cast<WorkspaceGroup>(workspace));
      continue;
    }
