...
namespace MantidWidgets {
    ...
    void AlgorithmProgressDialogPresenter::algorithmStartedSlot(Mantid::API::AlgorithmID alg)
    {
        ...
        if (algInstance) {
            auto treeItem = m_view->addAlgorithm(algInstance->name(), algInstance->getProperties());
            m_progressBars.insert(std::make_pair(alg, treeItem));
        }
    }

    void AlgorithmProgressDialogPresenter::updateProgressBarSlot(Mantid::API::AlgorithmID alg, double progress, QString message)
    {
        // if the algorithm isn't contained in the progress bar tree, then pretend it just started
