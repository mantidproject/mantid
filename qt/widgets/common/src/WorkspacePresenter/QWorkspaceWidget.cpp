...
*/
void QWorkspaceWidget::dropEvent(QDropEvent *de) { m_tree->dropEvent(de); }

void QWorkspaceWidget::setupWidgetLayout() {
...
void QWorkspaceWidget::setupLoadButtonMenu() {
...
void QWorkspaceWidget::setupConnections() {
...
*/
void QWorkspaceWidget::setTreeUpdating(const bool state) {
  m_treeUpdating = state;
}

void QWorkspaceWidget::incrementUpdateCount() { m_updateCount.ref(); }
...
WorkspacePresenterWN_wptr QWorkspaceWidget::getPresenterWeakPtr() {
...
*   in the dock.
*/
StringList QWorkspaceWidget::getSelectedWorkspaceNames() const {
...
*   workspaces selected)
*/
Mantid::API::Workspace_sptr QWorkspaceWidget::getSelectedWorkspace() const {
  auto items = m_tree->selectedItems();
...
void QWorkspaceWidget::onLoadAccept() {
...
void QWorkspaceWidget::refreshWorkspaces() {
  m_presenter->notifyFromView(ViewNotifiable::Flag::RefreshWorkspaces);
}

void QWorkspaceWidget::enableDeletePrompt(bool enable) {
  m_promptDelete = enable;
}

bool QWorkspaceWidget::isPromptDelete() const { return m_promptDelete; }

bool QWorkspaceWidget::deleteConfirmation() const {
  return askUserYesNo(
      "Delete Workspaces",
...
void QWorkspaceWidget::deleteWorkspaces(const StringList &wsNames) {
  QStringList names;
  for (auto &ws : wsNames)
    names.append(QString::fromStdString(ws));
...
void QWorkspaceWidget::clearView() { emit signalClearView(); }

void QWorkspaceWidget::sortAscending() {
  m_sortDirection = SortDirection::Ascending;
  m_presenter->notifyFromView(ViewNotifiable::Flag::SortWorkspaces);
}

void QWorkspaceWidget::sortDescending() {
  m_sortDirection = SortDirection::Descending;
  m_presenter->notifyFromView(ViewNotifiable::Flag::SortWorkspaces);
}

void QWorkspaceWidget::chooseByName() {
  m_sortCriteria = SortCriteria::ByName;
  m_presenter->notifyFromView(ViewNotifiable::Flag::SortWorkspaces);
}

void QWorkspaceWidget::chooseByLastModified() {
  m_sortCriteria = SortCriteria::ByLastModified;
  m_presenter->notifyFromView(ViewNotifiable::Flag::SortWorkspaces);
}

void QWorkspaceWidget::excludeItemFromSort(MantidTreeWidgetItem *item) {
...
QWorkspaceWidget::SortDirection QWorkspaceWidget::getSortDirection() const {
  return m_sortDirection;
}

QWorkspaceWidget::SortCriteria QWorkspaceWidget::getSortCriteria() const {
...
void QWorkspaceWidget::saveWorkspaceCollection() {
  m_presenter->notifyFromView(ViewNotifiable::Flag::SaveWorkspaceCollection);
}

void QWorkspaceWidget::handleShowSaveAlgorithm() {
...
QWorkspaceWidget::SaveFileType QWorkspaceWidget::getSaveFileType() const {
  return m_saveFileType;
}

void QWorkspaceWidget::saveWorkspace(SaveFileType type) {
  int version = -1;
...
std::string QWorkspaceWidget::getFilterText() const {
  return m_workspaceFilter->text().toStdString();
}

void QWorkspaceWidget::filterWorkspaces(const std::string &filterText) {
...
*/
void QWorkspaceWidget::createWorkspaceMenuActions() {
...
*/
void QWorkspaceWidget::createSortMenuActions() {
  m_sortCriteria = SortCriteria::ByName;
...
*/
void QWorkspaceWidget::populateChildData(QTreeWidgetItem *item) {
  QVariant userData = item->data(0, Qt::UserRole);
  if (userData.isNull())
    return;
...
*/
void QWorkspaceWidget::updateTree(const TopLevelItems &items) {
...
*/
MantidTreeWidgetItem *QWorkspaceWidget::addTreeEntry(
...
*/
bool QWorkspaceWidget::shouldBeSelected(QString name) const {
...
void QWorkspaceWidget::treeSelectionChanged() {
...
*/
void QWorkspaceWidget::addMatrixWorkspaceMenuItems(
    QMenu *menu,
...
*/
void QWorkspaceWidget::addMDEventWorkspaceMenuItems(
...
void QWorkspaceWidget::addMDHistoWorkspaceMenuItems(
...
* @param WS :: The workspace related to the menu
*/
void QWorkspaceWidget::addPeaksWorkspaceMenuItems(
...
* @param menu :: The menu to store the items
*/
void QWorkspaceWidget::addWorkspaceGroupMenuItems(QMenu *menu) const {
...
* @param menu :: The menu to store the items
*/
void QWorkspaceWidget::addTableWorkspaceMenuItems(QMenu *menu) const {
  menu->addAction(m_showData);
...
*/
void QWorkspaceWidget::addClearMenuItems(QMenu *menu, const QString &wsName) {
...
bool QWorkspaceWidget::hasUBMatrix(const std::string &wsName) {
  bool hasUB = false;
...
*/
void QWorkspaceWidget::filterWorkspaceTree(const QString &text) {
...
void QWorkspaceWidget::onClickDeleteWorkspaces() {
  m_presenter->notifyFromView(ViewNotifiable::Flag::DeleteWorkspaces);
}

void QWorkspaceWidget::clickedWorkspace(QTreeWidgetItem *item, int) {
  Q_UNUSED(item);
}

void QWorkspaceWidget::workspaceSelected() {
...
void QWorkspaceWidget::onClickGroupButton() {
  if (m_groupButton) {
...
void QWorkspaceWidget::popupMenu(const QPoint &pos) {
  m_menuPosition = pos;
  m_presenter->notifyFromView(
      ViewNotifiable::Flag::PopulateAndShowWorkspaceContextMenu);
}

void QWorkspaceWidget::popupContextMenu() {
...
void QWorkspaceWidget::onClickShowData() {
...
*/
void QWorkspaceWidget::saveToProgram() {
...
*/
void QWorkspaceWidget::plotSpectrum(std::string type) {
...
void QWorkspaceWidget::onClickDrawColorFillPlot() {
...
*/
void QWorkspaceWidget::showColourFillPlot() {
...
void QWorkspaceWidget::keyPressEvent(QKeyEvent *e) {
  switch (e->key()) {
  case Qt::Key_Delete:
...
void QWorkspaceWidget::onClickShowDetectorTable() {
  m_presenter->notifyFromView(ViewNotifiable::Flag::ShowDetectorsTable);
}

void QWorkspaceWidget::showDetectorsTable() {
...
void QWorkspaceWidget::showMDPlot() { m_mantidDisplayModel->showMDPlot(); }

void QWorkspaceWidget::onClickShowListData() {
  m_presenter->notifyFromView(ViewNotifiable::Flag::ShowListData);
}

void QWorkspaceWidget::showListData() { m_mantidDisplayModel->showListData(); }

void QWorkspaceWidget::onClickShowSpectrumViewer() {
...
void QWorkspaceWidget::showLogs() { m_mantidDisplayModel->showLogFileWindow(); }

void QWorkspaceWidget::onClickShowSampleMaterial() {
...
*/
void QWorkspaceWidget::onClickConvertToMatrixWorkspace() {
...
*/
void QWorkspaceWidget::onClickConvertMDHistoToMatrixWorkspace() {
  m_presenter->notifyFromView(
...
*/
void QWorkspaceWidget::onClickClearUB() {
