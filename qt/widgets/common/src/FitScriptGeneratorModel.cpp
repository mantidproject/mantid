...
MultiDomainFunction_sptr createMultiDomainFunction(std::size_t const &numberOfDomains) {
  return FunctionFactory::Instance().createInitializedMultiDomainFunction("name=CompositeFunction", numberOfDomains);
}
...
std::size_t getPrefixIndexAt(std::string const &prefix, std::size_t const &index) {
  auto const subStrings = splitStringBy(prefix, "f.");
...
IFunction_sptr getFunctionAtPrefix(std::string const &prefix, IFunction_sptr const &function,
  ...
  try {
    return getFunctionAtPrefix(topPrefix, function->getFunction(firstIndex), isLast);
...
FitDomain::FitDomain(std::string const &prefix, std::string const &workspaceName, WorkspaceIndex workspaceIndex,
                     double startX, double endX)
    : m_multiDomainFunctionPrefix(prefix), m_workspaceName(workspaceName), m_workspaceIndex(workspaceIndex),
      m_startX(startX), m_endX(endX) {}

bool FitDomain::isSameDomain(std::string const &workspaceName, WorkspaceIndex workspaceIndex) const {
...
FitScriptGeneratorModel::FitScriptGeneratorModel() : m_fitDomains(), m_function(nullptr) {}
...
void FitScriptGeneratorModel::removeWorkspaceDomain(std::size_t const &removeIndex,
  ...
  std::transform(m_fitDomains.begin(), m_fitDomains.end(), m_fitDomains.begin(), decrementPrefixes);
}
...
void FitScriptGeneratorModel::addWorkspaceDomain(std::string const &workspaceName, WorkspaceIndex workspaceIndex,
  ...
  addWorkspaceDomain(nextAvailablePrefix(), workspaceName, workspaceIndex, startX, endX);
}

void FitScriptGeneratorModel::addWorkspaceDomain(std::string const &prefix, std::string const &workspaceName,
                                                 WorkspaceIndex workspaceIndex, double startX, double endX) {
  addEmptyCompositeAtPrefix(prefix);
  m_fitDomains.emplace_back(FitDomain(prefix, workspaceName, workspaceIndex, startX, endX));
}

std::size_t FitScriptGeneratorModel::findDomainIndex(std::string const &workspaceName,
...
bool FitScriptGeneratorModel::hasWorkspaceDomain(std::string const &workspaceName,
                                                 WorkspaceIndex workspaceIndex) const {
  return findWorkspaceDomain(workspaceName, workspaceIndex) != m_fitDomains.end();
}

bool FitScriptGeneratorModel::isStartXValid(std::string const &workspaceName, WorkspaceIndex workspaceIndex,
                                            double startX) const {
  ...
  return limits.first <= startX && startX <= limits.second && startX < m_fitDomains[domainIndex].m_endX;
}

bool FitScriptGeneratorModel::isEndXValid(std::string const &workspaceName, WorkspaceIndex workspaceIndex,
                                          double endX) const {
  auto const domainIndex = findDomainIndex(workspaceName, workspaceIndex);

  auto const limits = xLimits(workspaceName, workspaceIndex);
  return limits.first <= endX && endX <= limits.second && endX > m_fitDomains[domainIndex].m_startX;
}

std::pair<double, double> FitScriptGeneratorModel::xLimits(std::string const &workspaceName,
                                                           WorkspaceIndex workspaceIndex) const {
  auto &ads = AnalysisDataService::Instance();
  if (ads.doesExist(workspaceName))
    return xLimits(ads.retrieveWS<MatrixWorkspace>(workspaceName), workspaceIndex);

  throw std::invalid_argument("The domain '" + workspaceName + " (" + std::to_string(workspaceIndex.value) +
                              ")' could not be found.");
}

std::pair<double, double> FitScriptGeneratorModel::xLimits(MatrixWorkspace_const_sptr const &workspace,
                                                           WorkspaceIndex workspaceIndex) const {
  if (workspace) {
  ...
  throw std::invalid_argument("The workspace '" + workspace->getName() + "' is not a matrix workspace.");
}

void FitScriptGeneratorModel::updateStartX(std::string const &workspaceName, WorkspaceIndex workspaceIndex,
                                           double startX) {
  auto const domainIndex = findDomainIndex(workspaceName, workspaceIndex);
  m_fitDomains[domainIndex].m_startX = startX;
}

void FitScriptGeneratorModel::updateEndX(std::string const &workspaceName, WorkspaceIndex workspaceIndex, double endX) {
  auto const domainIndex = findDomainIndex(workspaceName, workspaceIndex);
  m_fitDomains[domainIndex].m_endX = endX;
}

void FitScriptGeneratorModel::removeCompositeAtPrefix(std::string const &prefix) {
  removeCompositeAtIndex(getPrefixIndexAt(prefix, 0));
}

void FitScriptGeneratorModel::addEmptyCompositeAtPrefix(std::string const &prefix) {
  if (!m_function) {
    m_function = createMultiDomainFunction(1);
  } else {
    addEmptyCompositeAtPrefix(getTopPrefix(prefix), getPrefixIndexAt(prefix, 0));
  }
}

void FitScriptGeneratorModel::addEmptyCompositeAtPrefix(std::string const &compositePrefix,
...
void FitScriptGeneratorModel::removeCompositeAtIndex(std::size_t const &compositeIndex) {
  if (compositeIndex > numberOfDomains())
...
bool FitScriptGeneratorModel::hasCompositeAtPrefix(std::string const &compositePrefix) const {
  return static_cast<bool>(toComposite(getFunctionAtPrefix(compositePrefix, m_function)));
}

std::string FitScriptGeneratorModel::nextAvailablePrefix() const { return "f" + std::to_string(numberOfDomains()); }
