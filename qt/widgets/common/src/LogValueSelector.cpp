 ...
 */
LogValueSelector::LogValueSelector(QWidget *parent) : API::MantidWidget(parent) {
  m_ui.setupUi(this);
 ...
 */
void LogValueSelector::doConnect() { connect(m_ui.chkUseLog, SIGNAL(stateChanged(int)), this, SLOT(setEnabled(int))); }
 ...
 */
QString LogValueSelector::getFunctionText() const { return m_ui.function->currentText(); }
 ...
 */
bool LogValueSelector::isCheckboxShown() const { return m_ui.chkUseLog->isVisible(); }

/**
 * Control whether checkbox is shown
 * @param visible :: [input] Whether checkbox should be visible
 */
void LogValueSelector::setCheckboxShown(bool visible) { m_ui.chkUseLog->setVisible(visible); }
