...
public:
  ...
  MOCK_CONST_METHOD3(isEndXValid, bool(std::string const &workspaceName, WorkspaceIndex workspaceIndex, double endX));

  MOCK_METHOD3(updateStartX, void(std::string const &workspaceName, WorkspaceIndex workspaceIndex, double startX));
  MOCK_METHOD3(updateEndX, void(std::string const &workspaceName, WorkspaceIndex workspaceIndex, double endX));
};
...
public:
  ...
  void test_that_changing_a_start_x_will_update_its_value_in_the_model_when_the_x_value_is_valid() {
    ...
    ON_CALL(*m_model, isStartXValid(m_wsName, m_wsIndex, m_startX)).WillByDefault(Return(true));

    EXPECT_CALL(*m_view, selectedRows()).Times(1).WillOnce(Return(selectedRows));
    EXPECT_CALL(*m_view, workspaceName(selectedRow)).Times(1).WillOnce(Return(m_wsName));
    EXPECT_CALL(*m_view, workspaceIndex(selectedRow)).Times(1).WillOnce(Return(m_wsIndex));
    EXPECT_CALL(*m_view, startX(selectedRow)).Times(1).WillOnce(Return(m_startX));
    EXPECT_CALL(*m_model, isStartXValid(m_wsName, m_wsIndex, m_startX)).Times(1).WillOnce(Return(true));
  ...
  void test_that_changing_a_start_x_will_reset_the_view_if_its_new_value_is_invalid() {
    ...
    ON_CALL(*m_view, workspaceIndex(selectedRow)).WillByDefault(Return(m_wsIndex));
    ON_CALL(*m_view, startX(selectedRow)).WillByDefault(Return(m_startX));
    ON_CALL(*m_model, isStartXValid(m_wsName, m_wsIndex, m_startX)).WillByDefault(Return(false));

    EXPECT_CALL(*m_view, selectedRows()).Times(1).WillOnce(Return(selectedRows));
    EXPECT_CALL(*m_view, workspaceName(selectedRow)).Times(1).WillOnce(Return(m_wsName));
    EXPECT_CALL(*m_view, workspaceIndex(selectedRow)).Times(1).WillOnce(Return(m_wsIndex));
    EXPECT_CALL(*m_view, startX(selectedRow)).Times(1).WillOnce(Return(m_startX));
    EXPECT_CALL(*m_model, isStartXValid(m_wsName, m_wsIndex, m_startX)).Times(1).WillOnce(Return(false));
  ...
  void test_that_changing_a_end_x_will_update_its_value_in_the_model_when_the_x_value_is_valid() {
    ...
    ON_CALL(*m_model, isEndXValid(m_wsName, m_wsIndex, m_endX)).WillByDefault(Return(true));

    EXPECT_CALL(*m_view, selectedRows()).Times(1).WillOnce(Return(selectedRows));
    EXPECT_CALL(*m_view, workspaceName(selectedRow)).Times(1).WillOnce(Return(m_wsName));
    EXPECT_CALL(*m_view, workspaceIndex(selectedRow)).Times(1).WillOnce(Return(m_wsIndex));
    EXPECT_CALL(*m_view, endX(selectedRow)).Times(1).WillOnce(Return(m_endX));
    EXPECT_CALL(*m_model, isEndXValid(m_wsName, m_wsIndex, m_endX)).Times(1).WillOnce(Return(true));
  ...
  void test_that_changing_a_end_x_will_reset_the_view_if_its_new_value_is_invalid() {
    ...
    ON_CALL(*m_view, workspaceIndex(selectedRow)).WillByDefault(Return(m_wsIndex));
    ON_CALL(*m_view, endX(selectedRow)).WillByDefault(Return(m_endX));
    ON_CALL(*m_model, isEndXValid(m_wsName, m_wsIndex, m_endX)).WillByDefault(Return(false));

    EXPECT_CALL(*m_view, selectedRows()).Times(1).WillOnce(Return(selectedRows));
    EXPECT_CALL(*m_view, workspaceName(selectedRow)).Times(1).WillOnce(Return(m_wsName));
    EXPECT_CALL(*m_view, workspaceIndex(selectedRow)).Times(1).WillOnce(Return(m_wsIndex));
    EXPECT_CALL(*m_view, endX(selectedRow)).Times(1).WillOnce(Return(m_endX));
    EXPECT_CALL(*m_model, isEndXValid(m_wsName, m_wsIndex, m_endX)).Times(1).WillOnce(Return(false));
    EXPECT_CALL(*m_view, resetSelection()).Times(1);
