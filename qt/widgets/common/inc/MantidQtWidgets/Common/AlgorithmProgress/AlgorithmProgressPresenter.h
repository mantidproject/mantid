 ...
 * It will update the progress bar with the first algorithm that has been run. If there are two or more
 * running simultaneously, only the first one's progress will be displayed. 
 * 
 * When the algorithm finishes, the ID is cleared, and the next algorithm that starts will be tracked.
 * 
 * Tracking only 1 algorithm at a time was done to allow the progress bar to completely fill up,
 * rather than be constantly reset by newly started algorithms.
 * 
