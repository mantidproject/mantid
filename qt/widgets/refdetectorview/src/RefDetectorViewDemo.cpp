 ...
 *                     into
 */
std::vector<float> makeTestData(double total_xmin, double total_xmax, double total_ymin, double total_ymax,
  ...
  for (size_t row = 0; row < total_rows; row++)
    for (size_t col = 0; col < total_cols; col++) {
      ...
      data[row * total_cols + col] = (float)(1000.0 * cos((x * x + y * y) * 20.0));
    }
  // mark a row 1/4 way up
  double point = (total_ymax - total_ymin) / 4 + total_ymin;
  double mark_row = 0;
  SpectrumView::SVUtils::Interpolate(total_ymin, total_ymax, point, 0.0, (double)total_rows, mark_row);
  ...
  double mark_col = 0;
  SpectrumView::SVUtils::Interpolate(total_xmin, total_xmax, point, 0.0, (double)total_cols, mark_col);
