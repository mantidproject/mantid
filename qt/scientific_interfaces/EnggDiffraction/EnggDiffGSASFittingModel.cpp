...
std::string refinementMethodToString(const MantidQt::CustomInterfaces::GSASRefinementMethod &method) {
  switch (method) {
  ...
  default:
    throw std::invalid_argument("Invalid refinement method: please contact the development team");
  }
}
...
EnggDiffGSASFittingModel::EnggDiffGSASFittingModel() {
  qRegisterMetaType<MantidQt::CustomInterfaces::GSASIIRefineFitPeaksOutputProperties>(
...
void EnggDiffGSASFittingModel::addFitResultsToMaps(const RunLabel &runLabel, const double rwp, const double sigma,
...
void EnggDiffGSASFittingModel::addLatticeParams(const RunLabel &runLabel, const API::ITableWorkspace_sptr &table) {
  m_latticeParamsMap.add(runLabel, table);
}

void EnggDiffGSASFittingModel::addGamma(const RunLabel &runLabel, const double gamma) {
  m_gammaMap.add(runLabel, gamma);
}

void EnggDiffGSASFittingModel::addRwp(const RunLabel &runLabel, const double rwp) { m_rwpMap.add(runLabel, rwp); }

void EnggDiffGSASFittingModel::addSigma(const RunLabel &runLabel, const double sigma) {
...
std::string generateFittedPeaksWSName(const RunLabel &runLabel) {
  return runLabel.runNumber + "_" + std::to_string(runLabel.bank) + "_gsasii_fitted_peaks";
}

std::string generateLatticeParamsName(const RunLabel &runLabel) {
  return runLabel.runNumber + "_" + std::to_string(runLabel.bank) + "_lattice_params";
}
} // namespace

std::pair<API::IAlgorithm_sptr, GSASIIRefineFitPeaksOutputProperties>
EnggDiffGSASFittingModel::doGSASRefinementAlgorithm(const GSASIIRefineFitPeaksParameters &params) {
  auto gsasAlg = API::AlgorithmManager::Instance().create("GSASIIRefineFitPeaks");

  gsasAlg->setProperty("RefinementMethod", refinementMethodToString(params.refinementMethod));
  gsasAlg->setProperty("InputWorkspace", params.inputWorkspace);
  gsasAlg->setProperty("InstrumentFile", params.instParamsFile);
  gsasAlg->setProperty("PhaseInfoFiles", boost::algorithm::join(params.phaseFiles, ","));
  ...
  const auto latticeParams = ADS.retrieveWS<API::ITableWorkspace>(latticeParamsName);
  return std::make_pair(
      gsasAlg, GSASIIRefineFitPeaksOutputProperties(rwp, sigma, gamma, fittedPeaks, latticeParams, params.runLabel));
}

void EnggDiffGSASFittingModel::doRefinements(const std::vector<GSASIIRefineFitPeaksParameters> &params) {
  m_workerThread = std::make_unique<QThread>(this);
  EnggDiffGSASFittingWorker *worker = new EnggDiffGSASFittingWorker(this, params);
  worker->moveToThread(m_workerThread.get());

  connect(m_workerThread.get(), SIGNAL(started()), worker, SLOT(doRefinements()));
  connect(
      worker, SIGNAL(refinementSuccessful(Mantid::API::IAlgorithm_sptr, GSASIIRefineFitPeaksOutputProperties)), this,
      SLOT(processRefinementSuccessful(Mantid::API::IAlgorithm_sptr, const GSASIIRefineFitPeaksOutputProperties &)));
  connect(worker,
          SIGNAL(refinementsComplete(Mantid::API::IAlgorithm_sptr, std::vector<GSASIIRefineFitPeaksOutputProperties>)),
          this,
          SLOT(processRefinementsComplete(Mantid::API::IAlgorithm_sptr,
  ...
  connect(worker, SIGNAL(refinementCancelled()), this, SLOT(processRefinementCancelled()));
  connect(m_workerThread.get(), SIGNAL(finished()), m_workerThread.get(), SLOT(deleteLater()));
  connect(worker, SIGNAL(refinementSuccessful(Mantid::API::IAlgorithm_sptr, GSASIIRefineFitPeaksOutputProperties)),
          worker, SLOT(deleteLater()));
  connect(worker, SIGNAL(refinementFailed(const std::string &)), worker, SLOT(deleteLater()));
  m_workerThread->start();
}

boost::optional<API::ITableWorkspace_sptr> EnggDiffGSASFittingModel::getLatticeParams(const RunLabel &runLabel) const {
  return getFromRunMapOptional(m_latticeParamsMap, runLabel);
}

boost::optional<double> EnggDiffGSASFittingModel::getGamma(const RunLabel &runLabel) const {
  return getFromRunMapOptional(m_gammaMap, runLabel);
}

boost::optional<double> EnggDiffGSASFittingModel::getRwp(const RunLabel &runLabel) const {
  return getFromRunMapOptional(m_rwpMap, runLabel);
}

boost::optional<double> EnggDiffGSASFittingModel::getSigma(const RunLabel &runLabel) const {
  return getFromRunMapOptional(m_sigmaMap, runLabel);
}

bool EnggDiffGSASFittingModel::hasFitResultsForRun(const RunLabel &runLabel) const {
  return m_rwpMap.contains(runLabel) && m_sigmaMap.contains(runLabel) && m_gammaMap.contains(runLabel);
}

Mantid::API::MatrixWorkspace_sptr EnggDiffGSASFittingModel::loadFocusedRun(const std::string &filename) const {
...
void EnggDiffGSASFittingModel::processRefinementFailed(const std::string &failureMessage) {
  if (m_observer) {
...
void EnggDiffGSASFittingModel::processRefinementSuccessful(
    API::IAlgorithm_sptr successfulAlgorithm, const GSASIIRefineFitPeaksOutputProperties &refinementResults) {
  addFitResultsToMaps(refinementResults.runLabel, refinementResults.rwp, refinementResults.sigma,
                      refinementResults.gamma, refinementResults.latticeParamsWS);
  if (m_observer) {
    m_observer->notifyRefinementSuccessful(std::move(successfulAlgorithm), refinementResults);
  }
}
...
void EnggDiffGSASFittingModel::saveRefinementResultsToHDF5(
    ...
    const std::vector<GSASIIRefineFitPeaksOutputProperties> &refinementResultSets, const std::string &filename) const {
  ...
  const std::string refinementMethod = successfulAlg->getProperty("RefinementMethod");
  ...
  if (refinementMethod == "Pawley refinement") {
    saveAlg->setProperty("PawleyDMin", successfulAlg->getPropertyValue("PawleyDMin"));
    saveAlg->setProperty("PawleyNegativeWeight", successfulAlg->getPropertyValue("PawleyNegativeWeight"));
  }
...
void EnggDiffGSASFittingModel::setObserver(std::shared_ptr<IEnggDiffGSASFittingObserver> observer) {
  m_observer = observer;
}
