...
std::string addRunNumberToGSASIIProjectFile(const std::string &filename,
                                            const MantidQt::CustomInterfaces::RunLabel &runLabel) {
  const auto dotPosition = filename.find_last_of(".");
  return filename.substr(0, dotPosition) + "_" + runLabel.runNumber + "_" + std::to_string(runLabel.bank) +
...
EnggDiffGSASFittingPresenter::EnggDiffGSASFittingPresenter(
    std::unique_ptr<IEnggDiffGSASFittingModel> model, IEnggDiffGSASFittingView *view,
    std::shared_ptr<IEnggDiffMultiRunFittingWidgetPresenter> multiRunWidget,
    std::shared_ptr<IEnggDiffractionParam> mainSettings)
    : m_model(std::move(model)), m_multiRunWidget(std::move(multiRunWidget)), m_mainSettings(std::move(mainSettings)),
      m_view(view), m_viewHasClosed(false) {}

EnggDiffGSASFittingPresenter::~EnggDiffGSASFittingPresenter() {}

void EnggDiffGSASFittingPresenter::notify(IEnggDiffGSASFittingPresenter::Notification notif) {
...
std::vector<GSASIIRefineFitPeaksParameters> EnggDiffGSASFittingPresenter::collectAllInputParameters() const {
  ...
  if (runLabels.size() == 1) {
    ...
    for (const auto &runLabel : runLabels) {
      GSASIIProjectFiles.emplace_back(addRunNumberToGSASIIProjectFile(GSASIIProjectFile, runLabel));
    }
  }
  ...
  for (size_t i = 0; i < runLabels.size(); i++) {
    ...
    inputParams.emplace_back(inputWS, runLabel, refinementMethod, instParamFile, phaseFiles, pathToGSASII,
                             GSASIIProjectFiles[i], dMin, negativeWeight, xMin, xMax, refineSigma, refineGamma);
  }
  return inputParams;
}

GSASIIRefineFitPeaksParameters
EnggDiffGSASFittingPresenter::collectInputParameters(const RunLabel &runLabel,
  ...
  return GSASIIRefineFitPeaksParameters(inputWS, runLabel, refinementMethod, instParamFile, phaseFiles, pathToGSASII,
                                        GSASIIProjectFile, dMin, negativeWeight, xMin, xMax, refineSigma, refineGamma);
}
...
void EnggDiffGSASFittingPresenter::displayFitResults(const RunLabel &runLabel) {
  ...
  if (!latticeParams || !rwp || !sigma || !gamma) {
    m_view->userError("Invalid run identifier", "Unexpectedly tried to display fit results for invalid "
                                                "run, run number = " +
                                                    runLabel.runNumber +
...
void EnggDiffGSASFittingPresenter::doRefinements(const std::vector<GSASIIRefineFitPeaksParameters> &params) {
  m_model->doRefinements(params);
}

void EnggDiffGSASFittingPresenter::notifyRefinementsComplete(
    Mantid::API::IAlgorithm_sptr alg, const std::vector<GSASIIRefineFitPeaksOutputProperties> &refinementResultSets) {
  if (!m_viewHasClosed) {
...
void EnggDiffGSASFittingPresenter::notifyRefinementFailed(const std::string &failureMessage) {
  if (!m_viewHasClosed) {
    m_view->setEnabled(true);
...
void EnggDiffGSASFittingPresenter::notifyRefinementSuccessful(
  ...
  if (!m_viewHasClosed) {
    ...
    const auto filename = m_mainSettings->userHDFRunFilename(refinementResults.runLabel.runNumber);

    try {
      m_model->saveRefinementResultsToHDF5(successfulAlgorithm, {refinementResults}, filename);
    } catch (std::exception &e) {
      m_view->userWarning("Could not save refinement results",
    ...
    m_multiRunWidget->addFittedPeaks(refinementResults.runLabel, refinementResults.fittedPeaksWS);
...
void EnggDiffGSASFittingPresenter::processDoRefinement() {
  ...
  if (!runLabel) {
    m_view->userWarning("No run selected", "Please select a run to do refinement on");
    return;
  }

  const auto inputWSOptional = m_multiRunWidget->getFocusedRun(*runLabel);
  if (!inputWSOptional) {
    m_view->userError("Invalid run selected for refinement",
                      "Tried to run refinement on invalid focused run, run number " + runLabel->runNumber +
  ...
  const auto refinementParams = collectInputParameters(*runLabel, *inputWSOptional);
...
void EnggDiffGSASFittingPresenter::processRefineAll() {
  ...
  if (refinementParams.size() == 0) {
    m_view->userWarning("No runs loaded", "Please load at least one run before refining");
    return;
  }
  m_view->showStatus("Refining run");
  m_view->setEnabled(false);
