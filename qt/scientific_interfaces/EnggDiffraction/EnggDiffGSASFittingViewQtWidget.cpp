...
EnggDiffGSASFittingViewQtWidget::EnggDiffGSASFittingViewQtWidget(
  ...
  auto multiRunWidgetModel = std::make_unique<EnggDiffMultiRunFittingWidgetModel>();
  m_multiRunWidgetView = std::make_unique<EnggDiffMultiRunFittingQtWidget>(pythonRunner);

  auto multiRunWidgetPresenter = std::make_shared<EnggDiffMultiRunFittingWidgetPresenter>(
  ...
  m_presenter =
      std::make_shared<EnggDiffGSASFittingPresenter>(std::move(model), this, multiRunWidgetPresenter, mainSettings);
...
void EnggDiffGSASFittingViewQtWidget::addWidget(IEnggDiffMultiRunFittingWidgetView *widget) {
...
void EnggDiffGSASFittingViewQtWidget::browseFocusedRun() {
  const auto filenames(QFileDialog::getOpenFileNames(this, tr("Find focused run files")));
  setFocusedRunFileNames(filenames);
}

void EnggDiffGSASFittingViewQtWidget::browseGSASHome() {
  auto directoryName(QFileDialog::getExistingDirectory(this, tr("GSAS-II installation directory")));
  m_ui.lineEdit_gsasHome->setText(directoryName);
}

void EnggDiffGSASFittingViewQtWidget::browseGSASProj() {
  auto filename(QFileDialog::getSaveFileName(this, tr("Output GSAS-II project file"), "", "GSAS-II Project (*.gpx)"));
...
void EnggDiffGSASFittingViewQtWidget::browseInstParams() {
  const auto filename(QFileDialog::getOpenFileName(this, tr("Instrument parameter file"), "",
...
void EnggDiffGSASFittingViewQtWidget::browsePhaseFiles() {
  const auto filenames(QFileDialog::getOpenFileNames(this, tr("Phase files"), "", "Phase files (*.cif)"));
  m_ui.lineEdit_phaseFiles->setText(filenames.join(tr(",")));
}

void EnggDiffGSASFittingViewQtWidget::disableLoadIfInputEmpty() { setLoadEnabled(!runFileLineEditEmpty()); }
...
void EnggDiffGSASFittingViewQtWidget::displayLatticeParams(
  ...
  row >> length_a >> length_b >> length_c >> angle_alpha >> angle_beta >> angle_gamma;
...
std::vector<std::string> EnggDiffGSASFittingViewQtWidget::getFocusedFileNames() const {
...
boost::optional<double> EnggDiffGSASFittingViewQtWidget::getPawleyDMin() const {
  ...
  if (!conversionSuccessful) {
    userWarning("Invalid Pawley DMin",
                "Invalid entry for Pawley DMin \"" + pawleyDMinString.toStdString() + "\". Using default");
    return boost::none;
  }
...
boost::optional<double> EnggDiffGSASFittingViewQtWidget::getPawleyNegativeWeight() const {
  ...
  bool conversionSuccessful(false);
  const auto pawleyNegWeight = pawleyNegWeightString.toDouble(&conversionSuccessful);
  if (!conversionSuccessful) {
    userWarning("Invalid Pawley negative weight",
                "Invalid entry for negative weight \"" + pawleyNegWeightString.toStdString() + "\". Using default");
    return boost::none;
  }
...
std::vector<std::string> EnggDiffGSASFittingViewQtWidget::getPhaseFileNames() const {
...
bool EnggDiffGSASFittingViewQtWidget::getRefineGamma() const { return m_ui.checkBox_refineGamma->isChecked(); }

bool EnggDiffGSASFittingViewQtWidget::getRefineSigma() const { return m_ui.checkBox_refineSigma->isChecked(); }

GSASRefinementMethod EnggDiffGSASFittingViewQtWidget::getRefinementMethod() const {
  const auto refinementMethod = m_ui.comboBox_refinementMethod->currentText().toStdString();
  if (refinementMethod == "Pawley") {
    ...
    userError("Unexpected refinement method", "Unexpected refinement method \"" + refinementMethod +
...
boost::optional<double> EnggDiffGSASFittingViewQtWidget::getXMax() const {
  ...
  if (!conversionSuccessful) {
    userWarning("Invalid XMax", "Invalid entry for XMax \"" + xMaxString.toStdString() + "\". Using default");
    return boost::none;
  }
...
boost::optional<double> EnggDiffGSASFittingViewQtWidget::getXMin() const {
  ...
  if (!conversionSuccessful) {
    userWarning("Invalid XMin", "Invalid entry for XMin \"" + xMinString.toStdString() + "\". Using default");
    return boost::none;
  }
...
void EnggDiffGSASFittingViewQtWidget::loadFocusedRun() { m_presenter->notify(IEnggDiffGSASFittingPresenter::LoadRun); }

void EnggDiffGSASFittingViewQtWidget::refineAll() { m_presenter->notify(IEnggDiffGSASFittingPresenter::RefineAll); }

bool EnggDiffGSASFittingViewQtWidget::runFileLineEditEmpty() const { return m_ui.lineEdit_runFile->text().isEmpty(); }

void EnggDiffGSASFittingViewQtWidget::selectRun() { m_presenter->notify(IEnggDiffGSASFittingPresenter::SelectRun); }
...
void EnggDiffGSASFittingViewQtWidget::setFocusedRunFileNames(const QStringList &filenames) {
...
void EnggDiffGSASFittingViewQtWidget::setLoadEnabled(const bool enabled) {
  if (enabled) {
    ...
    m_ui.pushButton_loadRun->setToolTip(tr("Please specify a file to load via the browse menu or by typing the "
...
void EnggDiffGSASFittingViewQtWidget::setupUI() {
  m_ui.setupUi(this);
  connect(m_ui.pushButton_browseRunFile, SIGNAL(clicked()), this, SLOT(browseFocusedRun()));
  connect(m_ui.pushButton_loadRun, SIGNAL(clicked()), this, SLOT(loadFocusedRun()));
  connect(m_ui.lineEdit_runFile, SIGNAL(textChanged(const QString &)), this, SLOT(disableLoadIfInputEmpty()));

  connect(m_ui.pushButton_browseInstParams, SIGNAL(clicked()), this, SLOT(browseInstParams()));
  connect(m_ui.pushButton_browsePhaseFiles, SIGNAL(clicked()), this, SLOT(browsePhaseFiles()));
  connect(m_ui.pushButton_gsasProjPath, SIGNAL(clicked()), this, SLOT(browseGSASProj()));
  connect(m_ui.pushButton_browseGSASHome, SIGNAL(clicked()), this, SLOT(browseGSASHome()));

  connect(m_ui.pushButton_doRefinement, SIGNAL(clicked()), this, SLOT(doRefinement()));
  connect(m_ui.pushButton_refineAll, SIGNAL(clicked()), this, SLOT(refineAll()));

  connect(m_multiRunWidgetView.get(), SIGNAL(runSelected()), this, SLOT(selectRun()));

  QSettings settings(tr(SETTINGS_NAME));
  if (settings.contains(tr(GSAS_HOME_SETTING_NAME))) {
    m_ui.lineEdit_gsasHome->setText(settings.value(tr(GSAS_HOME_SETTING_NAME)).toString());
  }
}

void EnggDiffGSASFittingViewQtWidget::showStatus(const std::string &status) const {
  m_userMessageProvider->showStatus(status);
}

void EnggDiffGSASFittingViewQtWidget::userError(const std::string &errorTitle,
                                                const std::string &errorDescription) const {
  m_userMessageProvider->userError(errorTitle, errorDescription);
}

void EnggDiffGSASFittingViewQtWidget::userWarning(const std::string &warningTitle,
                                                  const std::string &warningDescription) const {
  m_userMessageProvider->userWarning(warningTitle, warningDescription);
}

const char EnggDiffGSASFittingViewQtWidget::GSAS_HOME_SETTING_NAME[] = "GSAS_HOME";
const char EnggDiffGSASFittingViewQtWidget::SETTINGS_NAME[] = "EnggGUIGSASTabSettings";
