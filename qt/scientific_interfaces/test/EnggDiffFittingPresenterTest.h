...
public:
  EnggDiffFittingPresenterNoThread(IEnggDiffFittingView *view)
      : EnggDiffFittingPresenterNoThread(view, std::make_unique<testing::NiceMock<MockEnggDiffFittingModel>>()) {}

  EnggDiffFittingPresenterNoThread(IEnggDiffFittingView *view, std::unique_ptr<IEnggDiffFittingModel> model)
...
private:
  // not async at all
  void startAsyncFittingWorker(const std::vector<RunLabel> &runLabels, const std::string &ExpectedPeaks) override {
...
public:
  ...
  static EnggDiffFittingPresenterTest *createSuite() { return new EnggDiffFittingPresenterTest(); }

  static void destroySuite(EnggDiffFittingPresenterTest *suite) { delete suite; }
  ...
  void setUp() override {
    ...
    m_presenter.reset(
        new MantidQt::CustomInterfaces::EnggDiffFittingPresenter(m_view.get(), std::move(mockModel), nullptr, nullptr));
    ...
    m_basicCalibSettings.m_pixelCalibFilename = "ENGINX_full_pixel_calibration.csv";
  ...
  void tearDown() override { TS_ASSERT(testing::Mock::VerifyAndClearExpectations(m_view.get())); }

  void test_load_with_missing_param() {
    testing::NiceMock<MockEnggDiffFittingView> mockView;
    auto mockModel = std::make_unique<testing::NiceMock<MockEnggDiffFittingModel>>();
    auto *mockModel_ptr = mockModel.get();
    MantidQt::CustomInterfaces::EnggDiffFittingPresenter pres(&mockView, std::move(mockModel), nullptr, nullptr);
    ...
    TSM_ASSERT("View mock not used as expected. Some EXPECT_CALL conditions were not "
               "satisfied.",
               testing::Mock::VerifyAndClearExpectations(&mockView))
    TSM_ASSERT("Model mock not used as expected. Some EXPECT_CALL conditions were not "
               "satisfied.",
  ...
  void test_fitting_with_missing_param() {
    ...
    EXPECT_CALL(mockView, getFittingListWidgetCurrentValue()).Times(1).WillOnce(Return(boost::none));
    ...
    TSM_ASSERT("Mock not used as expected. Some EXPECT_CALL conditions were not "
               "satisfied.",
               testing::Mock::VerifyAndClearExpectations(&mockView))
  }
  ...
  void test_fitting_with_invalid_expected_peaks() {
    ...
    EXPECT_CALL(mockView, getFittingListWidgetCurrentValue())
        .Times(1)
        .WillOnce(Return(boost::optional<std::string>(boost::optional<std::string>("123_1"))));
    EXPECT_CALL(*mockModel_ptr, getWorkspaceFilename(testing::_)).Times(1).WillOnce(ReturnRef(EMPTY));

    EXPECT_CALL(mockView, getExpectedPeaksInput()).Times(1).WillOnce(Return(",3.5,7.78,r43d"));
  ...
  void test_fit_all_runno_valid_single_run() {
    ...
    EXPECT_CALL(mockView, getExpectedPeaksInput()).Times(1).WillOnce(Return("2.3445,3.3433,4.5664"));

    const RunLabel runLabel("123", 1);
    EXPECT_CALL(*mockModel_ptr, getRunLabels()).Times(1).WillOnce(Return(std::vector<RunLabel>({runLabel})));

    EXPECT_CALL(*mockModel_ptr, getWorkspaceFilename(runLabel)).Times(1).WillOnce(ReturnRef(EMPTY));
  ...
  void test_browse_peaks_list() {
    testing::NiceMock<MockEnggDiffFittingView> mockView;
    const auto paramMock = std::make_shared<testing::NiceMock<MockEnggDiffractionParam>>();
    EnggDiffFittingPresenterNoThread pres(&mockView, std::make_unique<testing::NiceMock<MockEnggDiffFittingModel>>(),
                                          paramMock);

    const auto &userDir(Poco::Path::home());
    EXPECT_CALL(*paramMock, outFilesUserDir("")).Times(1).WillOnce(Return(userDir));
  ...
  void test_browse_peaks_list_with_warning() {
    ...
    EXPECT_CALL(mockView, getOpenFile(userDir)).Times(1).WillOnce(Return(dummyDir));
  ...
  void test_save_peaks_list_with_warning() {
    ...
    EXPECT_CALL(mockView, getSaveFile(userDir)).Times(1).WillOnce(Return(dummyDir));
  ...
  void test_add_valid_peaks_to_list_with_comma() {
    ...
    EXPECT_CALL(mockView, getExpectedPeaksInput()).Times(1).WillOnce(Return("1.7906,2.0684,1.2676,"));
  ...
  void test_add_customised_valid_peaks_to_list_without_comma() {
    ...
    EXPECT_CALL(mockView, getExpectedPeaksInput()).Times(1).WillOnce(Return("2.0684,1.2676"));
  ...
  void test_shutDown() {
    ...
    MantidQt::CustomInterfaces::EnggDiffFittingPresenter pres(
        &mockView, std::make_unique<testing::NiceMock<MockEnggDiffFittingModel>>(), nullptr, nullptr);
  ...
  void test_removeRun() {
    ...
    EXPECT_CALL(*mockModel_ptr, getRunLabels())
        .Times(1)
        .WillOnce(Return(std::vector<RunLabel>({RunLabel("123", 2), RunLabel("456", 1)})));
    EXPECT_CALL(mockView, updateFittingListWidget(std::vector<std::string>({"123_2", "456_1"})));
  ...
  void test_updatePlotFittedPeaksValidFittedPeaks() {
    ...
    EXPECT_CALL(*mockModel_ptr, hasFittedPeaksForRun(runLabel)).Times(1).WillOnce(Return(true));
    EXPECT_CALL(*mockModel_ptr, getAlignedWorkspace(runLabel))
        .Times(1)
        .WillOnce(Return(WorkspaceCreationHelper::create2DWorkspace(10, 10)));
    EXPECT_CALL(mockView, plotFittedPeaksEnabled()).Times(1).WillOnce(Return(true));
    EXPECT_CALL(*mockModel_ptr, getFittedPeaksWS(runLabel))
        .Times(1)
        .WillOnce(Return(WorkspaceCreationHelper::create2DWorkspace(10, 10)));
    EXPECT_CALL(mockView, setDataVector(testing::_, testing::_, testing::_, testing::_)).Times(2);
  ...
  void test_updatePlotFittedPeaksNoFittedPeaks() {
    ...
    EXPECT_CALL(mockView, getFittingListWidgetCurrentValue())
        .Times(1)
        .WillOnce(Return(boost::optional<std::string>("123_1")));
    EXPECT_CALL(*mockModel_ptr, hasFittedPeaksForRun(runLabel)).Times(1).WillOnce(Return(false));
    EXPECT_CALL(*mockModel_ptr, getFocusedWorkspace(runLabel))
        .Times(1)
        .WillOnce(Return(WorkspaceCreationHelper::create2DWorkspace(10, 10)));
    EXPECT_CALL(mockView, plotFittedPeaksEnabled()).Times(1).WillOnce(Return(true));
    EXPECT_CALL(*mockModel_ptr, getFittedPeaksWS(runLabel)).Times(0);
    EXPECT_CALL(mockView, setDataVector(testing::_, testing::_, testing::_, testing::_)).Times(1);
    EXPECT_CALL(mockView, userWarning("Cannot plot fitted peaks", testing::_)).Times(1);
  ...
  void test_updatePlotSuccessfulFitPlotPeaksDisabled() {
    ...
    EXPECT_CALL(*mockModel_ptr, getAlignedWorkspace(runLabel))
        .Times(1)
        .WillOnce(Return(WorkspaceCreationHelper::create2DWorkspace(10, 10)));
    EXPECT_CALL(mockView, plotFittedPeaksEnabled()).Times(1).WillOnce(Return(false));
    EXPECT_CALL(*mockModel_ptr, getFittedPeaksWS(runLabel)).Times(0);
    EXPECT_CALL(mockView, setDataVector(testing::_, testing::_, testing::_, testing::_)).Times(1);
...
private:
  ...
  std::unique_ptr<MantidQt::CustomInterfaces::EnggDiffFittingPresenter> m_presenter;
...
const std::string EnggDiffFittingPresenterTest::g_focusedRun = "focused_texture_bank_1";

const std::string EnggDiffFittingPresenterTest::g_validRunNo = "228061";

const std::string EnggDiffFittingPresenterTest::g_focusedBankFile = "ENGINX_241395_focused_texture_bank_1";

const std::string EnggDiffFittingPresenterTest::g_focusedFittingRunNo = "241391-241394";
