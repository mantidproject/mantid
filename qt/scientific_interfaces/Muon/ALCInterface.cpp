...
void ALCInterface::importResults() {
  ...
  if (currentStep == 0) {
    ...
    if (AnalysisDataService::Instance().doesExist(wsData)) {
      ...
      if (ws->getNumberHistograms() != 1) {
        QMessageBox::critical(this, "Error",
                              "Workspace " + QString::fromStdString(wsData) + " must contain one spectrum only");
        return;
      }
      ...
      // Error message
      QMessageBox::critical(this, "Error", "Workspace " + QString::fromStdString(wsData) + " was not found");
    }
    ...
    if (AnalysisDataService::Instance().doesExist(wsData)) {
      ...
      MatrixWorkspace_sptr dataWs = AnalysisDataService::Instance().retrieveWS<MatrixWorkspace>(wsData);

      // Check that ws contains three spectra
      if (dataWs->getNumberHistograms() != 3) {
        QMessageBox::critical(this, "Error",
                              "Workspace " + QString::fromStdString(wsData) + " must contain three spectra");
        return;
      }
    ...
    if (AnalysisDataService::Instance().doesExist(wsData)) {
      ...
      if (dataWs->getNumberHistograms() < 3) {
        QMessageBox::critical(this, "Error",
                              "Workspace " + QString::fromStdString(wsData) + " must contain at least three spectra");
        return;
      }
