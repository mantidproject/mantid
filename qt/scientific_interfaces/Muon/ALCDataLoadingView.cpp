...
void ALCDataLoadingView::setAvailableLogs(const std::vector<std::string> &logs) {
  ...
  if (!logs.empty()) {
    auto index = m_ui.logValueSelector->getLogComboBox()->findText("run_number");
    if (index >= 0)
...
void ALCDataLoadingView::help() { MantidQt::API::HelpWindow::showCustomInterface(nullptr, QString("Muon ALC")); }
