...
public:
	...
	void emitStartXChanged(double startX) {
		emit startXChanged(startX);
	}

	void emitEndXChanged(double endX) {
	...
	void emitBackgroundChanged(double value) {
...
public:
	...
	void tearDown() override {
		...
		TS_ASSERT(Mock::VerifyAndClearExpectations(m_fittingModel));

		delete m_presenter;
		delete m_fittingModel;
		delete m_view;
	}
	...
	void test_that_the_selectedFitDataChanged_signal_will_plot_the_input_when_there_is_only_an_input_workspace() {
		...
		EXPECT_CALL(*m_view, removeFromBottomPreview(QString("Difference"))).Times(1);
	...
	void test_that_the_selectedFitDataChanged_signal_will_set_the_minimum_and_maximum_of_the_fit_range() {
		...
		ON_CALL(*m_fittingModel, getFittingRange(index, 0)).WillByDefault(Return(range));

		EXPECT_CALL(*m_fittingModel, getFittingRange(index, 0)).Times(2).WillRepeatedly(Return(range));
	...
	void test_that_the_selectedFitDataChanged_signal_will_enable_PlotGuess_when_there_is_a_fit_function_and_workspace() {
		std::size_t const index(0);
	...
	void test_that_the_selectedFitDataChanged_signal_will_disable_the_guess_plot_when_there_is_no_fit_function() {
		std::size_t const index(0);
		ON_CALL(*m_fittingModel, getWorkspace(index)).WillByDefault(Return(m_ads->retrieveWorkspace("WorkspaceName")));
	...
	void test_that_the_plotGuessChanged_signal_will_clear_the_guess_plot_when_passed_false() {
		std::size_t const index(0);
		ON_CALL(*m_fittingModel, getWorkspace(index)).WillByDefault(Return(m_ads->retrieveWorkspace("WorkspaceName")));
	...
	void test_that_the_startXChanged_signal_will_set_the_fitting_models_startX() {
		...
		ON_CALL(*m_fittingModel, getFittingRange(0, 0)).WillByDefault(Return(range));
	...
	void test_that_the_backgroundChanged_signal_will_set_the_functions_background() {
		...
		Expectation setDefault = EXPECT_CALL(*m_fittingModel, setDefaultParameterValue("A0", background, 0)).Times(1);
...
private:
	...
	MockIndirectFittingModel *m_fittingModel;
