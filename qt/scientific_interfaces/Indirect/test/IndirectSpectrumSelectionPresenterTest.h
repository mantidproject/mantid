...
public:
  ...
  void emitSelectedSpectraChanged(IDAWorkspaceIndex minimum,
                                  IDAWorkspaceIndex maximum) {
    emit selectedSpectraChanged(minimum, maximum);
  }

  void emitMaskSpectrumChanged(IDAWorkspaceIndex spectrum) {
    emit maskSpectrumChanged(spectrum);
  }

  /// Public methods
  MOCK_CONST_METHOD0(minimumSpectrum, IDAWorkspaceIndex());
  MOCK_CONST_METHOD0(maximumSpectrum, IDAWorkspaceIndex());
  ...
  /// Public slots
  MOCK_METHOD1(setMinimumSpectrum, void(IDAWorkspaceIndex spectrum));
  MOCK_METHOD1(setMaximumSpectrum, void(IDAWorkspaceIndex spectrum));
...
public:
  ...
  void test_that_the_model_and_view_have_been_instantiated_correctly() {
    IDAWorkspaceIndex const maxSpectrum{3};
  ...
  test_that_invoking_a_presenter_method_will_call_the_relevant_methods_in_the_model_and_view() {
    ...
    ON_CALL(*m_model, getExcludeRegion(DatasetIndex{0}, IDAWorkspaceIndex{0}))
        .WillByDefault(Return(excludeRegion));

    Expectation getMask =
        EXPECT_CALL(*m_model,
                    getExcludeRegion(DatasetIndex{0}, IDAWorkspaceIndex{0}))
            .Times(1)
  ...
  test_that_the_selectedSpectraChanged_signal_will_not_display_an_error_label_when_the_range_provided_is_in_range() {
    ...
    m_view->emitSelectedSpectraChanged(IDAWorkspaceIndex{0},
                                       IDAWorkspaceIndex{2});
  }
  ...
  test_that_the_selectedSpectraChanged_signal_will_display_an_error_label_when_the_range_provided_is_out_of_range() {
    ...
    m_view->emitSelectedSpectraChanged(IDAWorkspaceIndex{0},
                                       IDAWorkspaceIndex{11});
  }

  void
  test_that_the_maskSpectrumChanged_signal_will_change_the_mask_by_calling_displayBinMask() {
    IDAWorkspaceIndex const maskSpectrum{0};

    Expectation getMask =
        EXPECT_CALL(*m_model, getExcludeRegion(DatasetIndex{0}, maskSpectrum))
            .Times(1)
            .WillOnce(Return("0"));
  ...
  test_that_the_maskSpectrumChanged_signal_will_change_the_mask_to_an_empty_string_if_the_index_provided_is_out_of_range() {
    IDAWorkspaceIndex const maskSpectrum{11};
  ...
  test_that_minimumSpectrum_returns_the_spectrum_number_that_it_is_set_as() {
    IDAWorkspaceIndex const minSpectrum{3};
  ...
  void test_that_setSpectraRange_will_set_the_minimum_and_maximum_spectrums() {
    ...
    EXPECT_CALL(*m_view, minimumSpectrum())
        .Times(1)
        .WillOnce(Return(IDAWorkspaceIndex{2}));
    EXPECT_CALL(*m_view, maximumSpectrum())
        .Times(1)
        .WillOnce(Return(IDAWorkspaceIndex{2}));
  ...
  void test_that_clear_will_set_the_minimum_and_maximum_spectrums_to_be_zero() {
    m_view->setMinimumSpectrum(IDAWorkspaceIndex{2});
    m_view->setMaximumSpectrum(IDAWorkspaceIndex{4});

    EXPECT_CALL(*m_view, clear()).Times(1);
    EXPECT_CALL(*m_view, minimumSpectrum())
        .Times(1)
        .WillOnce(Return(IDAWorkspaceIndex{0}));
    EXPECT_CALL(*m_view, maximumSpectrum())
        .Times(1)
        .WillOnce(Return(IDAWorkspaceIndex{0}));
