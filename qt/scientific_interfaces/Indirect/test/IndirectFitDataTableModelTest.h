...
public:
  ...
  void test_hasWorkspace_returns_true_for_ws_in_model() { TS_ASSERT(m_fitData->hasWorkspace("data workspace")); }
  ...
  void test_getWorkspace_returns_ws_in_range() {
    TS_ASSERT_EQUALS(m_fitData->getWorkspace(TableDatasetIndex{0})->getName(), "data workspace");
  }
  ...
  void test_isMultiFit_returns_false_for_more_single_ws() { TS_ASSERT(!m_fitData->isMultiFit()); }

  void test_isMultiFit_returns_true_for_more_than_one_ws() {
    auto dataWorkspace = Mantid::IndirectFitDataCreationHelper::createWorkspace(4, 5);
    Mantid::API::AnalysisDataService::Instance().addOrReplace("data workspace 2", std::move(dataWorkspace));
  ...
  void test_getNumberOfSpectra_raise_error_when_out_of_ws_range() {
    TS_ASSERT_EQUALS(m_fitData->getNumberOfSpectra(TableDatasetIndex{0}), 4);
    TS_ASSERT_THROWS(m_fitData->getNumberOfSpectra(TableDatasetIndex{1}), const std::runtime_error &);
  }
