...
class MANTIDQT_INDIRECT_DLL DataReductionSettings {
public:
  explicit DataReductionSettings(QString instrumentName = {}, QString analyserName = {}, QString reflectionName = {});

  [[nodiscard]] const QString &instrumentName() const;
  [[nodiscard]] const QString &analyserName() const;
  [[nodiscard]] const QString &reflectionName() const;

  /// Query persistent storage without changing widgets or writing settings.
  [[nodiscard]] static DataReductionSettings readSettings(const QSettings &settings);
  /// Persist only the instrument configuration values from a snapshot.
  static void saveSettings(QSettings &settings, const DataReductionSettings &values);

private:
  QString m_instrumentName;
  QString m_analyserName;
  QString m_reflectionName;
};
...
private:
  ...
  /// Refresh ConfigService-backed directory state without persistent QSettings I/O.
  void updateDirectories();
  /// Apply an immutable settings snapshot without accessing persistent storage.
  void restoreSettings(const DataReductionSettings &settings);
  /// Capture current widget state without accessing persistent storage.
  [[nodiscard]] DataReductionSettings captureSettings() const;
