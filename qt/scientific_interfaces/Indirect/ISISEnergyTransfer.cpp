...
void convertSpectrumAxis(std::string const &inputWorkspace, std::string const &outputWorkspace,
                         std::string const &target = "ElasticQ", std::string const &eMode = "Indirect") {
...
void rebin(std::string const &inputWorkspace, std::string const &outputWorkspace, std::string const &params) {
...
ISISEnergyTransfer::ISISEnergyTransfer(IndirectDataReduction *idrUI, QWidget *parent)
  ...
  connect(m_uiForm.pbSaveCustomGrouping, SIGNAL(clicked()), this, SLOT(handleSaveCustomGroupingClicked()));
  ...
  connect(m_uiForm.dsCalibrationFile, SIGNAL(dataReady(QString const &)), this, SLOT(handleDataReady(QString const &)));
  ...
  if (!m_uiForm.ckDoNotRebin->isChecked()) {
    if (m_uiForm.cbRebinType->currentText() == "Single") {
      ...
      if (rebinWidth < 0) {
        ...
        int result = QMessageBox::question(nullptr, tr("Logarithmic Binning"), tr(text), QMessageBox::Yes,
      ...
      bool rebinValid = !uiv.checkBins(m_uiForm.spRebinLow->value(), rebinWidth, m_uiForm.spRebinHigh->value());
...
bool ISISEnergyTransfer::numberInCorrectRange(std::size_t const &spectraNumber) const {
  if (hasInstrumentDetail("spectra-min") && hasInstrumentDetail("spectra-max")) {
    auto const spectraMin = static_cast<std::size_t>(getInstrumentDetail("spectra-min").toInt());
    auto const spectraMax = static_cast<std::size_t>(getInstrumentDetail("spectra-max").toInt());
...
QString
ISISEnergyTransfer::checkCustomGroupingNumbersInRange(std::vector<std::size_t> const &customGroupingNumbers) const {
  if (std::any_of(customGroupingNumbers.cbegin(), customGroupingNumbers.cend(),
                  [this](auto number) { return !this->numberInCorrectRange(number); })) {
...
QString ISISEnergyTransfer::validateDetectorGrouping() const {
  if (m_uiForm.cbGroupingOptions->currentText() == "File") {
    ...
    const std::string customString = m_uiForm.leCustomGroups->text().toStdString();
    if (customString.empty())
      return "Please supply a custom grouping for detectors.";
    else
      return checkCustomGroupingNumbersInRange(getCustomGroupingNumbers(customString));
  }
  return "";
}
...
void ISISEnergyTransfer::run() {
  ...
  std::pair<std::string, std::string> grouping = createMapFile(m_uiForm.cbGroupingOptions->currentText().toStdString());
  ...
  m_outputGroupName = instName.toLower().toStdString() + m_uiForm.dsRunFiles->getText().toStdString() + "_" +
...
void ISISEnergyTransfer::includeExtraGroupingOption(bool includeOption, QString const &option) {
...
void ISISEnergyTransfer::handleSaveCustomGroupingClicked() {
  ...
  if (doesExistInADS(GROUPING_WS_NAME)) {
    ...
    props["OutputFile"] = QString::fromStdString(saveDirectory + DEFAULT_GROUPING_FILENAME);
 ...
 */
void ISISEnergyTransfer::setInstrumentDefault(QMap<QString, QString> const &instDetails) {
  ...
  includeExtraGroupingOption(allowDefaultGroupingInstruments.contains(instrumentName), "Default");
  ...
  m_uiForm.spEfixed->setValue(
      hasInstrumentDetail(instDetails, "Efixed") ? getInstrumentDetail(instDetails, "Efixed").toDouble() : 0.0);
  ...
  setInstrumentCheckBoxProperty(m_uiForm.ckFold, instDetails, "fold-frames-choice");
}

void ISISEnergyTransfer::setInstrumentCheckBoxProperty(QCheckBox *checkbox, QMap<QString, QString> const &instDetails,
 ...
 * created.
 */
std::pair<std::string, std::string> ISISEnergyTransfer::createMapFile(const std::string &groupType) {
  if (groupType == "File") {
...
void ISISEnergyTransfer::plotRaw() {
  ...
  auto const instName = getInstrumentConfiguration()->getInstrumentName().toStdString();
...
void ISISEnergyTransfer::setFileExtensionsByName(bool filter) {
  ...
  m_uiForm.dsCalibrationFile->setFBSuffixes(filter ? getCalibrationFBSuffixes(tabName)
                                                   : getCalibrationExtensions(tabName));
  m_uiForm.dsCalibrationFile->setWSSuffixes(filter ? getCalibrationWSSuffixes(tabName) : noSuffixes);
}
...
void ISISEnergyTransfer::setRunEnabled(bool enable) { m_uiForm.pbRun->setEnabled(enable); }
...
void ISISEnergyTransfer::updateRunButton(bool enabled, std::string const &enableOutputButtons, QString const &message,
