...
void ReflSettingsTabPresenter::onReductionResumed() { m_view->disableAll(); }
 ...
 */
bool ReflSettingsTabPresenter::hasPerAngleOptions() const {
...
void ReflSettingsTabPresenter::setTransmissionOption(
    MantidWidgets::DataProcessor::OptionsQMap &options, const QString &key,
...
void ReflSettingsTabPresenter::addIfNotEmpty(OptionsQMap &options,
                                             const QString &key,
                                             const QString &value) const {
  if (!value.isEmpty())
...
bool ReflSettingsTabPresenter::hasReductionTypes(
    const std::string &summationType) const {
  return summationType == "SumInQ";
}

QString ReflSettingsTabPresenter::asAlgorithmPropertyBool(bool value) {
  return value ? "1" : "0";
}
 ...
 */
QString ReflSettingsTabPresenter::getProcessingInstructions() const {
...
*/
OptionsQMap ReflSettingsTabPresenter::getTransmissionOptions() const {
...
*/
OptionsQMap ReflSettingsTabPresenter::getReductionOptions() const {
  OptionsQMap options;
  if (m_view->experimentSettingsEnabled()) {
...
*/
OptionsQMap ReflSettingsTabPresenter::getDefaultOptions() const {
  if (!m_view->experimentSettingsEnabled())
    return OptionsQMap();
...
Mantid::API::IAlgorithm_sptr ReflSettingsTabPresenter::createReductionAlg() {
...
ReflSettingsTabPresenter::createEmptyInstrument(const std::string &instName) {
  auto loadInst =
  ...
  loadInst->execute();
  Mantid::API::MatrixWorkspace_const_sptr ws = loadInst->getProperty("OutputWorkspace");
  return ws->getInstrument();
}

void ReflSettingsTabPresenter::notifyExperimentDefaultsRequested() {
...
void ReflSettingsTabPresenter::addTransmissionOptions(
    OptionsQMap &options, std::initializer_list<QString> keys) const {
  for (auto &&key : keys)
    setTransmissionOption(options, key, QString());
}

OptionsQMap ReflSettingsTabPresenter::transmissionOptionsMap() const {
  OptionsQMap options;
...
void ReflSettingsTabPresenter::notifyInstrumentDefaultsRequested() {
  auto alg = createReductionAlg();
  auto instrument = createEmptyInstrument(m_currentInstrumentName);
  auto parameters = InstrumentParameters(instrument);
...
void ReflSettingsTabPresenter::notifySettingsChanged() {
  m_mainPresenter->settingsChanged();
}

void ReflSettingsTabPresenter::notifySummationTypeChanged() {
  auto summationType = m_view->getSummationType();
