...
/** Constructor
 */
SqwView::SqwView(QWidget *parent) {
  m_uiForm.setupUi(parent);

  m_regionSelector = std::make_unique<MantidQt::Widgets::RegionSelector>(nullptr, m_uiForm.gridLayout);
...
std::string SqwView::getDataName() { return m_uiForm.dsInput->getCurrentDataName().toStdString(); }

void SqwView::setFBSuffixes(QStringList suffix) { m_uiForm.dsInput->setFBSuffixes(suffix); }

void SqwView::setWSSuffixes(QStringList suffix) { m_uiForm.dsInput->setWSSuffixes(suffix); }
...
void SqwView::updateRunButton(bool enabled, std::string const &enableOutputButtons, QString const &message,
...
void SqwView::setRunEnabled(bool enabled) { m_uiForm.pbRun->setEnabled(enabled); }

void SqwView::setSaveEnabled(bool enabled) { m_uiForm.pbSave->setEnabled(enabled); }

void SqwView::plotRqwContour(MatrixWorkspace_sptr rqwWorkspace) {
  m_regionSelector->updateWorkspace(rqwWorkspace);
...
std::tuple<double, double> SqwView::getQRangeFromPlot() {
  return m_uiForm.rqwPlot2D->getAxisRange(MantidWidgets::AxisID::YLeft);
}

std::tuple<double, double> SqwView::getERangeFromPlot() {
