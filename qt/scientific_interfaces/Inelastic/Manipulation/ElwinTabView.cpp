...
void ElwinTabView::subscribePresenter(IElwinPresenter *presenter) { m_presenter = presenter; }

void ElwinTabView::notifyRunClicked() { m_presenter->handleRunClicked(); }

void ElwinTabView::notifySaveClicked() { m_presenter->handleSaveClicked(); }

void ElwinTabView::notifyPlotPreviewClicked() { m_presenter->handlePlotPreviewClicked(); }

void ElwinTabView::notifySelectedSpectrumChanged(int index) { m_presenter->handlePreviewSpectrumChanged(index); }

void ElwinTabView::notifyPreviewIndexChanged(int index) { m_presenter->handlePreviewIndexChanged(index); }

void ElwinTabView::notifyRowModeChanged() { m_presenter->handleRowModeChanged(); }

void ElwinTabView::notifyRemoveDataClicked() { m_presenter->handleRemoveSelectedData(); }

void ElwinTabView::notifyAddWorkspaceDialog() { showAddWorkspaceDialog(); }

void ElwinTabView::showAddWorkspaceDialog() {
...
void ElwinTabView::notifyAddData(MantidWidgets::IAddWorkspaceDialog *dialog) { addData(dialog); }

/** This method checks whether a Workspace or a File is being uploaded through the AddWorkspaceDialog
 *
 */
void ElwinTabView::addData(MantidWidgets::IAddWorkspaceDialog const *dialog) {
  try {
...
OutputPlotOptionsView *ElwinTabView::getPlotOptions() const { return m_uiForm.ipoPlotOptions; }

void ElwinTabView::setHorizontalHeaders() {
  QStringList headers;
  headers << "Workspace"
          << "WS Index";
...
void ElwinTabView::clearDataTable() { m_uiForm.tbElwinData->setRowCount(0); }

void ElwinTabView::addTableEntry(int row, std::string const &name, std::string const &wsIndexes) {
...
void ElwinTabView::setCell(std::unique_ptr<QTableWidgetItem> cell, int row, int column) {
  m_uiForm.tbElwinData->setItem(static_cast<int>(row), column, cell.release());
}

QModelIndexList ElwinTabView::getSelectedData() { return m_uiForm.tbElwinData->selectionModel()->selectedIndexes(); }

void ElwinTabView::setDefaultSampleLog(const Mantid::API::MatrixWorkspace_const_sptr &ws) {
 ...
 */
void ElwinTabView::newInputDataFromDialog(std::vector<std::string> const &names) {
...
void ElwinTabView::clearPreviewFile() { m_uiForm.cbPreviewFile->clear(); }

void ElwinTabView::setPreviewToDefault() {
 ...
 *                    input workspace.
 */
void ElwinTabView::plotInput(MatrixWorkspace_sptr inputWS, int spectrum) {
...
void ElwinTabView::notifyCheckboxValueChanged(QtProperty *prop, bool enabled) {
...
void ElwinTabView::notifyMinChanged(double val) {
...
void ElwinTabView::notifyMaxChanged(double val) {
  MantidWidgets::RangeSelector *from = qobject_cast<MantidWidgets::RangeSelector *>(sender());
...
void ElwinTabView::notifyDoubleValueChanged(QtProperty *prop, double val) {
  auto integrationRangeSelector = m_uiForm.ppPlot->getRangeSelector("ElwinIntegrationRange");
  auto backgroundRangeSelector = m_uiForm.ppPlot->getRangeSelector("ElwinBackgroundRange");
...
void ElwinTabView::disconnectSignals() {
...
void ElwinTabView::connectSignals() {
  connect(m_dblManager, SIGNAL(valueChanged(QtProperty *, double)), this,
          SLOT(notifyDoubleValueChanged(QtProperty *, double)));
...
void ElwinTabView::setIntegrationStart(double value) {
  m_dblManager->setValue(m_properties["IntegrationStart"], value);
}

double ElwinTabView::getIntegrationStart() { return m_dblManager->value(m_properties["IntegrationStart"]); }

void ElwinTabView::setIntegrationEnd(double value) { m_dblManager->setValue(m_properties["IntegrationEnd"], value); }

double ElwinTabView::getIntegrationEnd() { return m_dblManager->value(m_properties["IntegrationEnd"]); }

void ElwinTabView::setBackgroundStart(double value) { m_dblManager->setValue(m_properties["BackgroundStart"], value); }

double ElwinTabView::getBackgroundStart() { return m_dblManager->value(m_properties["BackgroundStart"]); }

void ElwinTabView::setBackgroundEnd(double value) { m_dblManager->setValue(m_properties["BackgroundEnd"], value); }

double ElwinTabView::getBackgroundEnd() { return m_dblManager->value(m_properties["BackgroundEnd"]); }
 ...
 */
void ElwinTabView::setRangeSelector(RangeSelector *rs, QtProperty *lower, QtProperty *upper,
 ...
 */
void ElwinTabView::setRangeSelectorMin(QtProperty *minProperty, QtProperty *maxProperty, RangeSelector *rangeSelector,
                                       double newValue) {
 ...
 */
void ElwinTabView::setRangeSelectorMax(QtProperty *minProperty, QtProperty *maxProperty, RangeSelector *rangeSelector,
                                       double newValue) {
...
void ElwinTabView::setRunIsRunning(const bool running) {
  m_uiForm.pbRun->setText(running ? "Running..." : "Run");
  setButtonsEnabled(!running);
  m_uiForm.ppPlot->watchADS(!running);
}

void ElwinTabView::setButtonsEnabled(const bool enabled) {
  setRunEnabled(enabled);
  setSaveResultEnabled(enabled);
}

void ElwinTabView::setRunEnabled(const bool enabled) { m_uiForm.pbRun->setEnabled(enabled); }

void ElwinTabView::setSaveResultEnabled(const bool enabled) { m_uiForm.pbSave->setEnabled(enabled); }

void ElwinTabView::setAvailableSpectra(WorkspaceIndex minimum, WorkspaceIndex maximum) {
...
void ElwinTabView::setAvailableSpectra(const std::vector<WorkspaceIndex>::const_iterator &from,
...
std::string ElwinTabView::getPreviewWorkspaceName(int index) const {
  return m_uiForm.cbPreviewFile->itemText(index).toStdString();
}

std::string ElwinTabView::getPreviewFilename(int index) const {
  return m_uiForm.cbPreviewFile->itemData(index).toString().toStdString();
}

int ElwinTabView::getPreviewSpec() const {
...
std::string ElwinTabView::getCurrentPreview() const { return m_uiForm.cbPreviewFile->currentText().toStdString(); }

bool ElwinTabView::isGroupInput() const { return m_uiForm.ckGroupOutput->isChecked(); }
bool ElwinTabView::isRowCollapsed() const { return m_uiForm.ckCollapse->isChecked(); }
bool ElwinTabView::isTableEmpty() const { return m_uiForm.tbElwinData->rowCount() == 0; }
bool ElwinTabView::getNormalise() { return m_blnManager->value(m_properties["Normalise"]); }

bool ElwinTabView::getBackgroundSubtraction() { return m_blnManager->value(m_properties["BackgroundSubtraction"]); }

std::string ElwinTabView::getLogName() { return m_uiForm.leLogName->text().toStdString(); }

std::string ElwinTabView::getLogValue() { return m_uiForm.leLogValue->currentText().toStdString(); }

void ElwinTabView::showMessageBox(std::string const &message) const {
