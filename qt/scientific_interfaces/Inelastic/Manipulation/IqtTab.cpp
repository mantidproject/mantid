...
IqtTab::IqtTab(QWidget *parent, IIqtView *view)
    : DataManipulationTab(parent), m_view(view), m_model(std::make_unique<IqtTabModel>()), m_selectedSpectrum(0) {
  m_view->subscribePresenter(this);
  setOutputPlotOptionsPresenter(
      std::make_unique<OutputPlotOptionsPresenter>(m_view->getPlotOptions(), PlotWidget::SpectraTiled));
}

void IqtTab::setup() { m_view->setup(); }

void IqtTab::handleSampDataReady(const std::string &wsname) {
...
void IqtTab::handleResDataReady(const std::string &resWorkspace) {
  m_view->updateDisplayedBinParameters();
  m_model->setResWorkspace(resWorkspace);
}

void IqtTab::handleIterationsChanged(int iterations) { m_model->setNIterations(std::to_string(iterations)); }

void IqtTab::handleRunClicked() {
 ...
 */
void IqtTab::handlePlotCurrentPreview() {
...
void IqtTab::handleErrorsClicked(int state) { m_model->setCalculateErrors(state); }

void IqtTab::handleNormalizationClicked(int state) { m_model->setEnforceNormalization(state); }

void IqtTab::handleValueChanged(std::string const &propName, double value) {
  if (propName == "ELow") {
...
void IqtTab::handlePreviewSpectrumChanged(int spectra) {
 ...
 */
void IqtTab::runComplete(bool error) {
 ...
 */
bool IqtTab::validate() { return m_view->validate(); }

void IqtTab::setFileExtensionsByName(bool filter) {
...
void IqtTab::setButtonsEnabled(bool enabled) {
  m_view->setRunEnabled(enabled);
  m_view->setSaveResultEnabled(enabled);
}

void IqtTab::setRunIsRunning(bool running) {
 ...
 */
int IqtTab::getSelectedSpectrum() const { return m_selectedSpectrum; }
 ...
 */
void IqtTab::setSelectedSpectrum(int spectrum) { m_selectedSpectrum = spectrum; }
 ...
 */
MatrixWorkspace_sptr IqtTab::getInputWorkspace() const { return m_inputWorkspace; }
 ...
 */
void IqtTab::setInputWorkspace(MatrixWorkspace_sptr inputWorkspace) {
 ...
 *          the preview plot.
 */
MatrixWorkspace_sptr IqtTab::getPreviewPlotWorkspace() { return m_previewPlotWorkspace.lock(); }
 ...
 */
void IqtTab::setPreviewPlotWorkspace(const MatrixWorkspace_sptr &previewPlotWorkspace) {
