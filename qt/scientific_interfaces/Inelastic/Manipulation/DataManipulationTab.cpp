...
namespace {
Mantid::Kernel::Logger g_log("DataManipulationTab");
}

namespace MantidQt::CustomInterfaces {

DataManipulationTab::DataManipulationTab(QObject *parent) : IndirectTab(parent), m_tabRunning(false) {
  connect(m_batchAlgoRunner, SIGNAL(batchComplete(bool)), this, SLOT(tabExecutionComplete(bool)));
}

DataManipulationTab::~DataManipulationTab() = default;

void DataManipulationTab::setOutputPlotOptionsPresenter(std::unique_ptr<OutputPlotOptionsPresenter> presenter) {
  m_plotOptionsPresenter = std::move(presenter);
}

void DataManipulationTab::clearOutputPlotOptionsWorkspaces() { m_plotOptionsPresenter->clearWorkspaces(); }

void DataManipulationTab::setOutputPlotOptionsWorkspaces(std::vector<std::string> const &outputWorkspaces) {
  m_plotOptionsPresenter->setWorkspaces(outputWorkspaces);
}

void DataManipulationTab::runTab() {
  if (validate()) {
 ...
 * @param error Unused
 */
void DataManipulationTab::tabExecutionComplete(bool error) {
  UNUSED_ARG(error);
  if (m_tabRunning) {
    m_tabRunning = false;
    runComplete(error);
 ...
 */
void DataManipulationTab::filterInputData(bool filter) { setFileExtensionsByName(filter); }
