...
using namespace IDA;
ElwinTab::ElwinTab(QWidget *parent, IElwinView *view)
    : DataManipulationTab(parent), m_view(view), m_model(std::make_unique<ElwinTabModel>()),
 ...
 */
void ElwinTab::runComplete(bool error) {
...
void ElwinTab::handleValueChanged(std::string const &propName, double value) {
...
void ElwinTab::handleValueChanged(std::string const &propName, bool value) {
 ...
 */
void ElwinTab::newInputDataFromDialog() {
...
void ElwinTab::handlePreviewIndexChanged(int index) {
...
void ElwinTab::newPreviewFileSelected(const std::string &workspaceName, const std::string &filename) {
...
void ElwinTab::newPreviewWorkspaceSelected(const std::string &workspaceName) {
  auto const workspace = WorkspaceUtils::getADSWorkspace(workspaceName);
  setInputWorkspace(workspace);
  updateAvailableSpectra();
  m_view->plotInput(getInputWorkspace(), getSelectedSpectrum());
...
void ElwinTab::handlePreviewSpectrumChanged(int spectrum) {
  if (m_view->getPreviewSpec())
    setSelectedSpectrum(spectrum);
  m_view->plotInput(getInputWorkspace(), getSelectedSpectrum());
}

void ElwinTab::updateIntegrationRange() {
...
void ElwinTab::handleRunClicked() {
 ...
 */
void ElwinTab::handleSaveClicked() {
  for (auto const &name : getOutputWorkspaceNames())
    addSaveWorkspaceToQueue(name);
  m_batchAlgoRunner->executeBatchAsync();
}

std::vector<std::string> ElwinTab::getOutputWorkspaceNames() {
...
std::string ElwinTab::getOutputBasename() { return WorkspaceUtils::getWorkspaceBasename(m_pythonExportWsName); }

void ElwinTab::handleAddData(MantidWidgets::IAddWorkspaceDialog const *dialog) {
  try {
...
void ElwinTab::addDataToModel(MantidWidgets::IAddWorkspaceDialog const *dialog) {
...
void ElwinTab::updateTableFromModel() {
...
void ElwinTab::handleRemoveSelectedData() {
...
void ElwinTab::handleRowModeChanged() { updateTableFromModel(); }

void ElwinTab::updateAvailableSpectra() {
  auto spectra = m_dataModel->getSpectra(findWorkspaceID());
  m_view->setAvailableSpectra(spectra.begin(), spectra.end());
}

size_t ElwinTab::findWorkspaceID() {
 ...
 */
int ElwinTab::getSelectedSpectrum() const { return m_selectedSpectrum; }
 ...
 */
void ElwinTab::setSelectedSpectrum(int spectrum) { m_selectedSpectrum = spectrum; }
 ...
 */
MatrixWorkspace_sptr ElwinTab::getInputWorkspace() const { return m_inputWorkspace; }
 ...
 */
void ElwinTab::setInputWorkspace(MatrixWorkspace_sptr inputWorkspace) {
 ...
 */
void ElwinTab::handlePlotPreviewClicked() {
 ...
 *          the preview plot.
 */
MatrixWorkspace_sptr ElwinTab::getPreviewPlotWorkspace() { return m_previewPlotWorkspace.lock(); }
 ...
 */
void ElwinTab::setPreviewPlotWorkspace(const MatrixWorkspace_sptr &previewPlotWorkspace) {
