...
/** Constructor
 */
MomentsTabView::MomentsTabView(QWidget *parent) : m_presenter() {
...
/** Destructor
 */
MomentsTabView::~MomentsTabView() { m_propTrees["MomentsPropTree"]->unsetFactoryForManager(m_dblManager); }

/**Subscribes the presenter to the view.
 *@param presenter: A pointer to the presenter to which the view is subscribed.
 */
void MomentsTabView::subscribePresenter(IMomentsPresenter *presenter) { m_presenter = presenter; }

void MomentsTabView::notifyDataReady(const QString &dataName) { m_presenter->handleDataReady(dataName.toStdString()); }
/**
 ...
 */
void MomentsTabView::notifyRangeChanged(double min, double max) {
  m_dblManager->setValue(m_properties["EMin"], min);
  m_dblManager->setValue(m_properties["EMax"], max);
}

void MomentsTabView::notifyScaleChanged(int const scale) { m_presenter->handleScaleChanged(scale == Qt::Checked); }

void MomentsTabView::notifyScaleValueChanged(double const value) { m_presenter->handleScaleValueChanged(value); }

void MomentsTabView::notifyValueChanged(QtProperty *prop, double value) {
  m_presenter->handleValueChanged(prop->propertyName().toStdString(), value);
}

void MomentsTabView::notifyRunClicked() { m_presenter->handleRunClicked(); }

void MomentsTabView::notifySaveClicked() { m_presenter->handleSaveClicked(); }

void MomentsTabView::setupProperties() {
...
void MomentsTabView::setFBSuffixes(QStringList const &suffix) { m_uiForm.dsInput->setFBSuffixes(suffix); }

void MomentsTabView::setWSSuffixes(QStringList const &suffix) { m_uiForm.dsInput->setWSSuffixes(suffix); }

OutputPlotOptionsView *MomentsTabView::getPlotOptions() const { return m_uiForm.ipoPlotOptions; }

std::string MomentsTabView::getDataName() const { return m_uiForm.dsInput->getCurrentDataName().toStdString(); }

bool MomentsTabView::validate() {
 ...
 */
void MomentsTabView::plotNewData(std::string const &filename) {
 ...
 */
void MomentsTabView::setPlotPropertyRange(const QPair<double, double> &bounds) {
 ...
 */
void MomentsTabView::setRangeSelector(const QPair<double, double> &bounds) {
  disconnect(m_dblManager, SIGNAL(valueChanged(QtProperty *, double)), this,
             SLOT(notifyValueChanged(QtProperty *, double)));
 ...
 */
void MomentsTabView::setRangeSelectorMin(double newValue) {
 ...
 */
void MomentsTabView::setRangeSelectorMax(double newValue) {
...
void MomentsTabView::replot() { m_uiForm.ppRawPlot->replot(); }

MantidWidgets::RangeSelector *MomentsTabView::getRangeSelector() {
  return m_uiForm.ppRawPlot->getRangeSelector("XRange");
}

void MomentsTabView::plotOutput(std::string const &outputWorkspace) {
  // Plot each spectrum
  m_uiForm.ppMomentsPreview->clear();
...
void MomentsTabView::showMessageBox(std::string const &message) const {
