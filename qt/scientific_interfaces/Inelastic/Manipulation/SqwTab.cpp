...
/** Constructor
 */
SqwTab::SqwTab(QWidget *parent, ISqwView *view)
    : DataManipulationTab(parent), m_model(std::make_unique<SqwTabModel>()), m_view(view) {
 ...
 *
 */
void SqwTab::handleDataReady(std::string const &dataName) {
  if (m_view->validate()) {
 ...
 */
void SqwTab::runComplete(bool error) {
  if (!error) {
...
void SqwTab::setFileExtensionsByName(bool filter) {
...
void SqwTab::handleRunClicked() { runTab(); }

void SqwTab::handleSaveClicked() {
...
void SqwTab::handleQLowChanged(double const value) { m_model->setQMin(value); }

void SqwTab::handleQWidthChanged(double const value) { m_model->setQWidth(value); }

void SqwTab::handleQHighChanged(double const value) { m_model->setQMax(value); }

void SqwTab::handleELowChanged(double const value) { m_model->setEMin(value); }

void SqwTab::handleEWidthChanged(double const value) { m_model->setEWidth(value); }

void SqwTab::handleEHighChanged(double const value) { m_model->setEMax(value); }

void SqwTab::handleRebinEChanged(int const value) { m_model->setRebinInEnergy(value != 0); }
} // namespace MantidQt::CustomInterfaces
