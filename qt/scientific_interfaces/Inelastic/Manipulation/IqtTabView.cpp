...
IqtTabView::IqtTabView(QWidget *parent) : m_presenter(), m_iqtTree(nullptr) {
  m_uiForm.setupUi(parent);
  m_dblEdFac = new DoubleEditorFactory(this);
  m_dblManager = new QtDoublePropertyManager();
}

IqtTabView::~IqtTabView() { m_iqtTree->unsetFactoryForManager(m_dblManager); }

OutputPlotOptionsView *IqtTabView::getPlotOptions() const { return m_uiForm.ipoPlotOptions; }

void IqtTabView::setup() {
...
void IqtTabView::subscribePresenter(IIqtPresenter *presenter) { m_presenter = presenter; }

void IqtTabView::notifySampDataReady(const QString &filename) {
  m_presenter->handleSampDataReady(filename.toStdString());
}

void IqtTabView::notifyResDataReady(const QString &resFilename) {
  m_presenter->handleResDataReady(resFilename.toStdString());
}

void IqtTabView::notifyIterationsChanged(int iterations) { m_presenter->handleIterationsChanged(iterations); }

void IqtTabView::notifyRunClicked() { m_presenter->handleRunClicked(); }

void IqtTabView::notifySaveClicked() { m_presenter->handleSaveClicked(); }

void IqtTabView::notifyPlotCurrentPreview() { m_presenter->handlePlotCurrentPreview(); }

void IqtTabView::notifyErrorsClicked(int state) {
  m_uiForm.spIterations->setEnabled(state);
  m_presenter->handleErrorsClicked(state);
}

void IqtTabView::notifyPreviewSpectrumChanged(int spectra) { m_presenter->handlePreviewSpectrumChanged(spectra); }

void IqtTabView::notifyUpdateEnergyRange(int state) {
  if (state != 0) {
...
void IqtTabView::notifyValueChanged(QtProperty *prop, double value) {
  m_presenter->handleValueChanged(prop->propertyName().toStdString(), value);
}

void IqtTabView::notifyEnableNormalizationClicked(int state) { m_presenter->handleNormalizationClicked(state); }
 ...
 */
void IqtTabView::notifyRangeChanged(double min, double max) {
 ...
 */
void IqtTabView::notifyUpdateRangeSelector(QtProperty *prop, double val) {
  auto xRangeSelector = m_uiForm.ppPlot->getRangeSelector("IqtRange");
...
void IqtTabView::setPreviewSpectrumMaximum(int value) { m_uiForm.spPreviewSpec->setMaximum(value); }
...
void IqtTabView::setSampleFBSuffixes(const QStringList &suffix) { m_uiForm.dsInput->setFBSuffixes(suffix); }

void IqtTabView::setSampleWSSuffixes(const QStringList &suffix) { m_uiForm.dsInput->setWSSuffixes(suffix); }

void IqtTabView::setResolutionFBSuffixes(const QStringList &suffix) { m_uiForm.dsResolution->setFBSuffixes(suffix); }

void IqtTabView::setResolutionWSSuffixes(const QStringList &suffix) { m_uiForm.dsResolution->setWSSuffixes(suffix); }

void IqtTabView::setRunEnabled(bool enabled) { m_uiForm.pbRun->setEnabled(enabled); }

void IqtTabView::setSaveResultEnabled(bool enabled) { m_uiForm.pbSave->setEnabled(enabled); }

void IqtTabView::setRunText(bool running) { m_uiForm.pbRun->setText(running ? "Running..." : "Run"); }

void IqtTabView::setWatchADS(bool watch) { m_uiForm.ppPlot->watchADS(watch); }

/**
 * Plots the selected spectrum of the input workspace.
 */
void IqtTabView::plotInput(MatrixWorkspace_sptr inputWS, int spectrum) {
...
void IqtTabView::setRangeSelectorDefault(const MatrixWorkspace_sptr workspace, const QPair<double, double> &range) {
  auto xRangeSelector = m_uiForm.ppPlot->getRangeSelector("IqtRange");
  try {
 ...
 */
void IqtTabView::setRangeSelectorMin(QtProperty *minProperty, QtProperty *maxProperty, RangeSelector *rangeSelector,
                                     double newValue) {
 ...
 */
void IqtTabView::setRangeSelectorMax(QtProperty *minProperty, QtProperty *maxProperty, RangeSelector *rangeSelector,
                                     double newValue) {
 ...
 */
void IqtTabView::updateDisplayedBinParameters() {
...
std::string IqtTabView::getSampleName() const { return m_uiForm.dsInput->getCurrentDataName().toStdString(); }

void IqtTabView::showMessageBox(const std::string &message) const {
