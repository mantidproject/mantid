...
/** Constructor
 */
MomentsTabModel::MomentsTabModel() { m_scale = false; }

IAlgorithm_sptr MomentsTabModel::setupAlgorithm() {
...
void MomentsTabModel::setInputWorkspace(const std::string &workspace) {
  m_inputWorkspace = workspace;
  m_outputWorkspaceName = m_inputWorkspace.substr(0, m_inputWorkspace.length() - 4) + "_Moments";
}

void MomentsTabModel::setEMin(double eMin) { m_eMin = eMin; }

void MomentsTabModel::setEMax(double eMax) { m_eMax = eMax; }

void MomentsTabModel::setScale(bool scale) { m_scale = scale; }

void MomentsTabModel::setScaleValue(double scaleValue) { m_scaleValue = scaleValue; }

std::string MomentsTabModel::getOutputWorkspace() { return m_outputWorkspaceName; }
