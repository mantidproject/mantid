...
IndirectDataAnalysisTab *DataAnalysisTabFactory::makeMSDFitTab(int const index) const {
  auto tab = new IndirectDataAnalysisTab(new MSDFitModel, new MSDTemplateBrowser, new IndirectFitDataView, "MSDFit",
                                         false, MSDFit::HIDDEN_PROPS, m_tabWidget->widget(index));
...
IndirectDataAnalysisTab *DataAnalysisTabFactory::makeIqtFitTab(int const index) const {
  auto tab = new IndirectDataAnalysisTab(new IqtFitModel, new IqtTemplateBrowser, new IndirectFitDataView, "IqtFit",
                                         false, IqtFit::HIDDEN_PROPS, m_tabWidget->widget(index));
  tab->setupFitDataPresenter<IndirectFitDataPresenter>();
  return tab;
}

IndirectDataAnalysisTab *DataAnalysisTabFactory::makeConvFitTab(int const index) const {
  auto tab = new IndirectDataAnalysisTab(new ConvFitModel, new ConvTemplateBrowser, new ConvFitDataView, "ConvFit",
                                         true, ConvFit::HIDDEN_PROPS, m_tabWidget->widget(index));
...
IndirectDataAnalysisTab *DataAnalysisTabFactory::makeFqFitTab(int const index) const {
  auto tab = new IndirectDataAnalysisTab(new FqFitModel, new FqTemplateBrowser, new FqFitDataView, "FQFit", false,
                                         FqFit::HIDDEN_PROPS, m_tabWidget->widget(index));
