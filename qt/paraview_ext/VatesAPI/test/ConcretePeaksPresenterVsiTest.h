...
public:
  void testSetupPresenterCorrectly() {
    ...
    std::shared_ptr<MockPeaksWorkspaceConcrete> pw_ptr = std::make_shared<MockPeaksWorkspaceConcrete>();
    // Act
  ...
  void testCorrectPeaksInfoIsExtractedForValidRow() {
    ...
    Mantid::Geometry::PeakShape_sptr shape(
        new Mantid::DataObjects::PeakShapeSpherical(peakRadius, coordinateSystem, "test", 1));
