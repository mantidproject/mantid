...
private:
  ...
  vtkSmartPointer<vtkDataSet> doExecute(std::string filename, bool performAsserts = true) {
    // Setup view
    ...
    EXPECT_CALL(*mockView, getLoadInMemory()).Times(AtLeast(1)).WillRepeatedly(testing::Return(true));
    ...
    EXPECT_CALL(factory, create(_)).WillOnce(testing::Return(vtkSmartPointer<vtkUnstructuredGrid>::New()));
...
public:
  ...
  void testConstructWithNullViewThrows() {
    TSM_ASSERT_THROWS("Should throw if an empty file string is given.", MDHWNexusLoadingPresenter(nullptr, "some_file"),
                      const std::invalid_argument &);
  }

  void testConstruct() {
    TSM_ASSERT_THROWS_NOTHING("Object should be created without exception.",
                              MDHWNexusLoadingPresenter(std::make_unique<MockMDLoadingView>(), getSuitableFile()));
  }

  void testCanReadFile() {
    MDHWNexusLoadingPresenter presenter(std::make_unique<MockMDLoadingView>(), getUnhandledFile());
  ...
  void testExecutionWithLegacyFile() {
    ...
    auto filename = Mantid::API::FileFinder::Instance().getFullPath("test_non_orthogonal.nxs");
    ...
    EXPECT_CALL(*view, getLoadInMemory()).Times(AtLeast(0)).WillRepeatedly(testing::Return(true));
    ...
    auto factory = std::make_shared<vtkMDHistoHex4DFactory<TimeToTimeStep>>(normalizationOption, time);

    factory->setSuccessor(std::make_unique<vtkMDHistoHexFactory>(normalizationOption))
        .setSuccessor(std::make_unique<vtkMDHistoQuadFactory>(normalizationOption))
        .setSuccessor(std::make_unique<vtkMDHistoLineFactory>(normalizationOption))
        .setSuccessor(std::make_unique<vtkMD0DFactory>());

    presenter.executeLoadMetadata();
    auto product = presenter.execute(factory.get(), mockLoadingProgressAction, mockDrawingProgressAction);

    // Set the COB
    try {
      auto workspaceProvider = std::make_unique<ADSWorkspaceProvider<Mantid::API::IMDWorkspace>>();
      presenter.makeNonOrthogonal(product, std::move(workspaceProvider), &mockDrawingProgressAction);
    } catch (...) {
    ...
    double expectedElements[16] = {1.0,
                                   ...
                                   0.0,
                                   1.0};
    vtkSmartPointer<vtkMatrix4x4> cobMatrix = vtkPVChangeOfBasisHelper::GetChangeOfBasisMatrix(product);

    TS_ASSERT_DELTA(cobMatrix->GetElement(0, 0), expectedElements[0], std::numeric_limits<double>::epsilon());

    TS_ASSERT_DELTA(cobMatrix->GetElement(0, 1), expectedElements[1], std::numeric_limits<double>::epsilon());
    TS_ASSERT_DELTA(cobMatrix->GetElement(0, 2), expectedElements[2], std::numeric_limits<double>::epsilon());
    TS_ASSERT_DELTA(cobMatrix->GetElement(0, 3), expectedElements[3], std::numeric_limits<double>::epsilon());

    TS_ASSERT_DELTA(cobMatrix->GetElement(1, 0), expectedElements[4], std::numeric_limits<double>::epsilon());
    TS_ASSERT_DELTA(cobMatrix->GetElement(1, 1), expectedElements[5], std::numeric_limits<double>::epsilon());
    TS_ASSERT_DELTA(cobMatrix->GetElement(1, 2), expectedElements[6], std::numeric_limits<double>::epsilon());
    TS_ASSERT_DELTA(cobMatrix->GetElement(1, 3), expectedElements[7], std::numeric_limits<double>::epsilon());

    TS_ASSERT_DELTA(cobMatrix->GetElement(2, 0), expectedElements[8], std::numeric_limits<double>::epsilon());
    TS_ASSERT_DELTA(cobMatrix->GetElement(2, 1), expectedElements[9], std::numeric_limits<double>::epsilon());
    TS_ASSERT_DELTA(cobMatrix->GetElement(2, 2), expectedElements[10], std::numeric_limits<double>::epsilon());
    TS_ASSERT_DELTA(cobMatrix->GetElement(2, 3), expectedElements[11], std::numeric_limits<double>::epsilon());

    TS_ASSERT_DELTA(cobMatrix->GetElement(3, 0), expectedElements[12], std::numeric_limits<double>::epsilon());
    TS_ASSERT_DELTA(cobMatrix->GetElement(3, 1), expectedElements[13], std::numeric_limits<double>::epsilon());
    TS_ASSERT_DELTA(cobMatrix->GetElement(3, 2), expectedElements[14], std::numeric_limits<double>::epsilon());
    TS_ASSERT_DELTA(cobMatrix->GetElement(3, 3), expectedElements[15], std::numeric_limits<double>::epsilon());
  }

  void testCallHasTDimThrows() {
    MDHWNexusLoadingPresenter presenter(std::make_unique<MockMDLoadingView>(), getSuitableFile());
  ...
  void testTimeLabel() {
    ...
    presenter.executeLoadMetadata();
    auto product = presenter.execute(&factory, mockLoadingProgressAction, mockDrawingProgressAction);
    TSM_ASSERT_EQUALS("Time label should be exact.", presenter.getTimeStepLabel(), "DeltaE (DeltaE)");
  ...
  void testAxisLabels() {
    ...
    TSM_ASSERT_EQUALS("X Label should match exactly", getStringFieldDataValue(product, "AxisTitleForX"),
                      "[H,0,0] ($in$ $1.992$ $\\AA^{-1}$)");
    TSM_ASSERT_EQUALS("Y Label should match exactly", getStringFieldDataValue(product, "AxisTitleForY"),
                      "[0,K,0] ($in$ $1.992$ $\\AA^{-1}$)");
    TSM_ASSERT_EQUALS("Z Label should match exactly", getStringFieldDataValue(product, "AxisTitleForZ"),
                      "[0,0,L] ($in$ $1.087$ $\\AA^{-1}$)");
