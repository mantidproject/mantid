...
public:
  ...
  void testConstructWithNullViewThrows() {
    TSM_ASSERT_THROWS("Should throw if a null view is given.", MDEWEventNexusLoadingPresenter(nullptr, "some_file"),
                      const std::invalid_argument &);
  }

  void testConstruct() {
    TSM_ASSERT_THROWS_NOTHING("Object should be created without exception.",
                              MDEWEventNexusLoadingPresenter(std::make_unique<MockMDLoadingView>(), getSuitableFile()));
  }

  void testCanReadFile() {
    MDEWEventNexusLoadingPresenter presenter(std::make_unique<MockMDLoadingView>(), getUnhandledFile());
  ...
  void testCallHasTDimThrows() {
    MDEWEventNexusLoadingPresenter presenter(std::make_unique<MockMDLoadingView>(), getSuitableFile());
  ...
  void testTimeLabel() {
    ...
    vtkSmartPointer<vtkDataSet> product =
        presenter.execute(&factory, mockLoadingProgressAction, mockDrawingProgressAction);
    TSM_ASSERT_EQUALS("Time label should be exact.", presenter.getTimeStepLabel(), "D (En)");
  ...
  void testAxisLabels() {
    ...
    TSM_ASSERT_EQUALS("X Label should match exactly", getStringFieldDataValue(product, "AxisTitleForX"), "A ($\\AA$)");
    TSM_ASSERT_EQUALS("Y Label should match exactly", getStringFieldDataValue(product, "AxisTitleForY"), "B ($\\AA$)");
    TSM_ASSERT_EQUALS("Z Label should match exactly", getStringFieldDataValue(product, "AxisTitleForZ"), "C ($\\AA$)");
