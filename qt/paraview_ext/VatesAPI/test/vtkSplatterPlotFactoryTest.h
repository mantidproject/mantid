...
public:
  ...
  void testCreateWithoutInitializeThrows() {
    ...
    TSM_ASSERT_THROWS("Have NOT initalized object. Should throw.", factory.create(progressUpdate),
  ...
  void test_3DWorkspace() {
    ...
    */
    ...
    TSM_ASSERT_EQUALS("No signal Array", "signal", std::string(product->GetPointData()->GetArray(0)->GetName()));
    TSM_ASSERT_EQUALS("Wrong sized signal Array", expected_n_signals, product->GetPointData()->GetArray(0)->GetSize());
  }
  ...
  void test_MetadataIsAddedCorrectly() {
    ...
    TSM_ASSERT_THROWS_NOTHING("There is XML metadata!",
                              xmlOut = converterFtoM(fd, XMLDefinitions::metaDataId().c_str()));
    TSM_ASSERT_THROWS_NOTHING("There is JSON metadata!",
                              jsonOut = converterFtoM(fd, vatesConfigurations.getMetadataIdJson().c_str()));
