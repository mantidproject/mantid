...
private:
  ...
  */
  ...
  public:
    ...
    ConcreteMDEWLoadingPresenter(std::unique_ptr<MDLoadingView> view) : MDEWLoadingPresenter(std::move(view)) {}
    ...
    bool canLoadFileBasedOnExtension(const std::string &filename, const std::string &expectedExtension) const override {
      // Forwarding method.
...
public:
  ...
  void testTimeChanged() {
    ...
    EXPECT_CALL(*mockView, getRecursionDepth()).Times(2);
    EXPECT_CALL(*mockView, getLoadInMemory()).Times(2);
    EXPECT_CALL(*mockView, getTime()).Times(2).WillOnce(Return(0)).WillOnce(Return(1)); // Time has changed on 2nd call
    EXPECT_CALL(*mockView, updateAlgorithmProgress(_, _)).Times(0);
  ...
  void testhasTDimensionWhenIntegrated() {
    // Setup view
    ConcreteMDEWLoadingPresenter presenter(std::make_unique<MockMDLoadingView>());

    // Test that it does work when setup.
    Mantid::API::Workspace_sptr ws = get3DWorkspace(true, true); // Integrated T Dimension
    presenter.extractMetadata(*std::dynamic_pointer_cast<IMDEventWorkspace>(ws));
