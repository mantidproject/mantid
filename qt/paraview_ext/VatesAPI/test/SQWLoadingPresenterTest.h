...
public:
  ...
  void testConstructWithEmptyFileThrows() {
    ...
    TSM_ASSERT_THROWS("Should throw if an empty file string is given.", SQWLoadingPresenter(std::move(view), ""),
                      const std::invalid_argument &);
  }

  void testConstructWithNullViewThrows() {
    TSM_ASSERT_THROWS("Should throw if an empty file string is given.", SQWLoadingPresenter(nullptr, "some_file"),
                      const std::invalid_argument &);
  }
  ...
  void testCallHasTDimThrows() {
    SQWLoadingPresenter presenter(std::make_unique<MockMDLoadingView>(), getSuitableFileNamePath());
  ...
  void testTimeLabel() {
    ...
    presenter.executeLoadMetadata();
    auto product = presenter.execute(&factory, mockLoadingProgressAction, mockDrawingProgressAction);
    TSM_ASSERT_EQUALS("Time label should be exact.", presenter.getTimeStepLabel(), "en (meV)");
