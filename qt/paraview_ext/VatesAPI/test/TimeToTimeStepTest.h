...
public:
  void testWithMaxMinGreaterThanZero() {
    ...
    TSM_ASSERT_EQUALS("The timeStep index has not been calculated properly. Wrong result.", nBins,
                      converter(dimensionMax))
  }
  ...
  void testBadTimeRangeThrows() {
    ...
    TSM_ASSERT_THROWS("Range is negative, should throw.", TimeToTimeStep::construct(dimensionMin, dimensionMax, nBins),
                      const std::runtime_error &);
  }

  void testUseWithDefaultConstructorThrows() {
    Mantid::VATES::TimeToTimeStep converter;
    TSM_ASSERT_THROWS("Constructing via default constructor should throw when used.", converter(1),
                      const std::runtime_error &);
  }
};
