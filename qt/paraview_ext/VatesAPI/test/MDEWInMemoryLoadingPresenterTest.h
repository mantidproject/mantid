...
public:
  ...
  void testConstructWithNullRepositoryThrows() {
    TSM_ASSERT_THROWS("Should throw with null repository.",
                      MDEWInMemoryLoadingPresenter(std::make_unique<MockMDLoadingView>(), nullptr, "_"),
                      const std::invalid_argument &);
  }
  ...
  void testConstructWithEmptyWsNameThrows() {
    ...
    TSM_ASSERT_THROWS(
        ...
        MDEWInMemoryLoadingPresenter(std::make_unique<MockMDLoadingView>(), new MockWorkspaceProvider, emptyName),
        const std::invalid_argument &);
  }

  void testConstruction() {
    TS_ASSERT_THROWS_NOTHING(
        MDEWInMemoryLoadingPresenter(std::make_unique<MockMDLoadingView>(), new MockWorkspaceProvider, "_"));
  }
  ...
  void testCanLoadWithInvalidName() {
    ...
    MDEWInMemoryLoadingPresenter presenter(std::make_unique<MockMDLoadingView>(), repository.release(), "_");
  ...
  void testExecution() {
    ...
    MDEWInMemoryLoadingPresenter presenter(std::move(view), repository.release(), "_");
    presenter.executeLoadMetadata();
    vtkSmartPointer<vtkDataSet> product =
        presenter.execute(&factory, mockLoadingProgressAction, mockDrawingProgressAction);
    ...
    TSM_ASSERT_EQUALS("One array expected on field data, one for XML and one for JSON!", 2,
  ...
  void testCallHasTDimThrows() {
    MDEWInMemoryLoadingPresenter presenter(std::make_unique<MockMDLoadingView>(), new MockWorkspaceProvider, "_");
