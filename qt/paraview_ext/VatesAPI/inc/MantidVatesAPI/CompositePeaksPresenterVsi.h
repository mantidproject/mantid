...
public:
  Mantid::API::IPeaksWorkspace_sptr getPeaksWorkspace() const override {
    throw std::runtime_error("The composite peaks presenter has no single peaks workspace.");
  }
  ...
  std::string getPeaksWorkspaceName() const override {
    throw std::runtime_error("The composite peaks presenter has no peaks workspace");
  }
  ...
  void sortPeaksWorkspace(const std::string &columnToSortBy, const bool sortedAscending,
