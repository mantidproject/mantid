 ...
 */
vtkSmartPointer<vtkPVClipDataSet> getClippedDataSet(const vtkSmartPointer<vtkDataSet> &dataSet) {
...
void applyCOBMatrixSettingsToVtkDataSet(Mantid::VATES::MDLoadingPresenter *presenter, vtkDataSet *dataSet,
  ...
  try {
    presenter->makeNonOrthogonal(dataSet, std::move(workspaceProvider), nullptr);
  } catch (std::invalid_argument &e) {
    std::string error = e.what();
    g_log_presenter_utilities.warning() << "PresenterUtilities: Workspace does not have correct "
                                           "information to "
 ...
 */
std::unique_ptr<vtkMDHexFactory> createFactoryChainForEventWorkspace(VisualNormalization normalization, double time) {
...
createFactoryChainForHistoWorkspace(VisualNormalization normalization, double time) {
  auto factory = std::make_unique<vtkMDHistoHex4DFactory<TimeToTimeStep>>(normalization, time);
