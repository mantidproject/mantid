 ...
 */
std::vector<bool> CompositePeaksPresenterVsi::getViewablePeaks() const { return std::vector<bool>(); }
 ...
 */
void CompositePeaksPresenterVsi::getPeaksInfo(Mantid::API::IPeaksWorkspace_sptr peaksWorkspace, int row,
  ...
  for (const auto &presenter : m_peaksPresenters) {
    if (presenter->getPeaksWorkspace() == peaksWorkspace) {
      presenter->getPeaksInfo(peaksWorkspace, row, position, radius, specialCoordinateSystem);
    }
  }
}
 ...
 */
std::vector<Mantid::API::IPeaksWorkspace_sptr> CompositePeaksPresenterVsi::getPeaksWorkspaces() const {
 ...
 */
std::map<std::string, std::vector<bool>> CompositePeaksPresenterVsi::getInitializedViewablePeaks() const {
  std::map<std::string, std::vector<bool>> viewablePeaks;
  for (const auto &presenter : m_peaksPresenters) {
    viewablePeaks.emplace(presenter->getPeaksWorkspace()->getName(),
                          std::vector<bool>(presenter->getPeaksWorkspace()->getNumberPeaks(), true));
  }
 ...
 */
void CompositePeaksPresenterVsi::removePresenter(const std::string &peaksWorkspaceName) {
  m_peaksPresenters.erase(std::remove_if(m_peaksPresenters.begin(), m_peaksPresenters.end(),
 ...
 */
void CompositePeaksPresenterVsi::updateWorkspaces(const std::vector<std::string> &peaksWorkspaceNames) {
  std::vector<std::string> storedPeaksWorkspaces = getPeaksWorkspaceNames();
  for (const auto &ws : storedPeaksWorkspaces) {
    size_t count = std::count(peaksWorkspaceNames.begin(), peaksWorkspaceNames.end(), ws);
    if (count == 0) {
      removePresenter(ws);
    }
  }
 ...
 */
bool CompositePeaksPresenterVsi::hasPeaks() { return !m_peaksPresenters.empty(); }
 ...
 */
void CompositePeaksPresenterVsi::sortPeaksWorkspace(const std::string &columnToSortBy, const bool sortedAscending,
