...
class EXPORT_OPT_MANTIDVATES_SIMPLEGUI_VIEWWIDGETS PeaksTabWidget : public QWidget {
  Q_OBJECT
public:
  PeaksTabWidget(std::vector<Mantid::API::IPeaksWorkspace_sptr> ws, const std::string &coordinateSystem,
...
private:
  ...
  void updateTab(const std::vector<bool> &visiblePeaks, const QColor &color, int index);
  /// Adds a new tab to the tab widget.
  void addNewTab(Mantid::API::IPeaksWorkspace_sptr peaksWorkspace, const std::string &tabName,
