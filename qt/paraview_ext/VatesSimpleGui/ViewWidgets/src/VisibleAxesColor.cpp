...
void safeSetProperty(vtkSMProxy *gridAxis, std::initializer_list<const char *> pnames,
...
std::array<double, 3> getContrastingColor(const std::vector<double> &color) {
  ...
  double criteria = 1. - (0.299 * color[0] + 0.587 * color[1] + 0.114 * color[2]);
...
void VisibleAxesColor::setOrientationAxesLabelColor(pqView *view, const std::array<double, 3> &color) {
  safeSetProperty(view->getProxy(), {"OrientationAxesLabelColor"}, color);
}

void VisibleAxesColor::setGridAxesColor(pqView *view, const std::array<double, 3> &color) {
  vtkSMProxy *gridAxes3DActor = vtkSMPropertyHelper(view->getProxy(), "AxesGrid", true).GetAsProxy();
  safeSetProperty(
      gridAxes3DActor,
      {"XTitleColor", "YTitleColor", "ZTitleColor", "XLabelColor", "YLabelColor", "ZLabelColor", "GridColor"}, color);
}

void VisibleAxesColor::setScalarBarColor(pqView *view, const std::array<double, 3> &color) {
  ...
  for (pqPipelineSource *source : sources) {
    ...
    for (pqDataRepresentation *rep : reps) {
      vtkSMProxy *ScalarBarProxy =
          vtkSMTransferFunctionProxy::FindScalarBarRepresentation(rep->getLookupTableProxy(), view->getProxy());
...
unsigned long VisibleAxesColor::observe(pqView *view) {
  return view->getViewProxy()
      ...
      ->AddObserver(vtkCommand::ModifiedEvent, this, &VisibleAxesColor::backgroundColorChangeCallback);
}

void VisibleAxesColor::backgroundColorChangeCallback(vtkObject *caller, unsigned long, void *) {
