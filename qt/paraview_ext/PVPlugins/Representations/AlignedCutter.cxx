...
vtkCxxSetObjectMacro(AlignedCutter,CutFunction,vtkImplicitFunction);
vtkCxxSetObjectMacro(AlignedCutter,Locator,vtkIncrementalPointLocator)

//----------------------------------------------------------------------------
// Construct with user-specified implicit function; initial value of 0.0; and
// generating cut scalars turned off.
AlignedCutter::AlignedCutter(vtkImplicitFunction *cf)
{
...
vtkMTimeType AlignedCutter::GetMTime()
{
...
//----------------------------------------------------------------------------
void AlignedCutter::StructuredPointsCutter(vtkDataSet *dataSetInput,
...
//----------------------------------------------------------------------------
void AlignedCutter::StructuredGridCutter(vtkDataSet *dataSetInput,
                                     vtkPolyData *thisOutput)
{
...
//----------------------------------------------------------------------------
void AlignedCutter::RectilinearGridCutter(vtkDataSet *dataSetInput,
                                      vtkPolyData *thisOutput)
{
...
//----------------------------------------------------------------------------
void AlignedCutter::GetCellTypeDimensions(unsigned char* cellTypeDimensions)
{
...
//----------------------------------------------------------------------------
void AlignedCutter::DataSetCutter(vtkDataSet *input, vtkPolyData *output)
{
  vtkIdType cellId;
  int iter;
  vtkPoints *cellPts;
  ...
  newVerts->Allocate(estimatedSize);
  newLines = vtkCellArray::New();
  newLines->Allocate(estimatedSize);
  newPolys = vtkCellArray::New();
  newPolys->Allocate(estimatedSize);
  ...
  outPD->InterpolateAllocate(inPD,estimatedSize);
  outCD->CopyAllocate(inCD,estimatedSize);
  ...
  vtkContourHelper helper(this->Locator, newVerts, newLines, newPolys,inPD, inCD, outPD,outCD, static_cast<int>(estimatedSize),this->GenerateTriangles!=0);
  ...
  else // VTK_SORT_BY_VALUE:
  {
    ...
    AlignedCutter::GetCellTypeDimensions(cellTypeDimensions);
    int dimensionality;
...
//----------------------------------------------------------------------------
void AlignedCutter::UnstructuredGridCutter(vtkDataSet *input, vtkPolyData *output)
{
  vtkIdType i;
  int iter;
  vtkDoubleArray *cellScalars;
...
void AlignedCutter::CreateDefaultLocator()
{
  if ( this->Locator == NULL )
  {
...
//----------------------------------------------------------------------------
int AlignedCutter::RequestUpdateExtent(
  vtkInformation *,
  vtkInformationVector **inputVector,
...
//----------------------------------------------------------------------------
int AlignedCutter::FillInputPortInformation(int, vtkInformation *info)
{
