...
//----------------------------------------------------------------------------
AlignedThreeSliceFilter::AlignedThreeSliceFilter()
{
  ...
  for (int i = 0; i < 3; ++i)
  {
    ...
    this->Slices[i] = AlignedCutter::New();
    this->Slices[i]->SetAxisNumber(i);
...
//----------------------------------------------------------------------------
void AlignedThreeSliceFilter::SetToDefaultSettings()
{
...
//----------------------------------------------------------------------------
vtkMTimeType AlignedThreeSliceFilter::GetMTime()
{
...
//----------------------------------------------------------------------------
void AlignedThreeSliceFilter::PrintSelf(ostream& os, vtkIndent indent)
{
...
//----------------------------------------------------------------------------
void AlignedThreeSliceFilter::SetCutNormal(int cutIndex, double normal[3])
{
  this->Planes[cutIndex]->SetNormal(normal);
}

//----------------------------------------------------------------------------
void AlignedThreeSliceFilter::SetCutOrigin(int cutIndex, double origin[3])
{
  this->Planes[cutIndex]->SetOrigin(origin);
}

//----------------------------------------------------------------------------
void AlignedThreeSliceFilter::SetCutOrigins(double origin[3])
{
...
//----------------------------------------------------------------------------
void AlignedThreeSliceFilter::SetCutValue(int cutIndex, int index, double value)
{
  AlignedCutter* slice = this->Slices[cutIndex];
...
//----------------------------------------------------------------------------
void AlignedThreeSliceFilter::SetNumberOfSlice(int cutIndex, int size)
{
  AlignedCutter* slice = this->Slices[cutIndex];
...
//-----------------------------------------------------------------------
int AlignedThreeSliceFilter::FillInputPortInformation(int, vtkInformation* info)
{
  info->Set(vtkAlgorithm::INPUT_REQUIRED_DATA_TYPE(), "vtkDataSet");
  return 1;
}
//-----------------------------------------------------------------------
int AlignedThreeSliceFilter::RequestData(
...
{
public:
  CutWorker(AlignedCutter* slices[3])
    : Slices(slices)
  {
  }
...
private:
  AlignedCutter** Slices;
};
}

//----------------------------------------------------------------------------
void AlignedThreeSliceFilter::Process(
  vtkDataSet* input, vtkPolyData* outputs[4], unsigned int compositeIndex)
{
  ...
  if (input->GetCellData()->HasArray(vtkSliceOriginalCellIds) == 1) {
    vtkIdTypeArray *originalCellIds = vtkIdTypeArray::FastDownCast(
        input->GetCellData()->GetArray(vtkSliceOriginalCellIds));
...
//----------------------------------------------------------------------------
void AlignedThreeSliceFilter::EnableProbe(int enable)
{
...
//----------------------------------------------------------------------------
bool AlignedThreeSliceFilter::GetProbedPointData(const char* arrayName, double& value)
{
