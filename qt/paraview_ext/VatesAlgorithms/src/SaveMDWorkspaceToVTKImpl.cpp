...
bool has_suffix(const std::string &stringToCheck, const std::string &suffix) {
  ...
  if (isStringLargerThanSuffix) {
    isSuffixInString = stringToCheck.compare(stringToCheck.size() - suffix.size(), suffix.size(), suffix) == 0;
  }
  return isSuffixInString;
}

bool isNDWorkspace(const Mantid::API::IMDWorkspace &workspace, const size_t dimensionality) {
...
SaveMDWorkspaceToVTKImpl::SaveMDWorkspaceToVTKImpl(SaveMDWorkspaceToVTK *parent) : m_progress(parent, 0.0, 1.0, 101) {
  setupMembers();
}
 ...
 */
void SaveMDWorkspaceToVTKImpl::saveMDWorkspace(const Mantid::API::IMDWorkspace_sptr &workspace,
                                               const std::string &filename, VisualNormalization normalization,
                                               int recursionDepth, const std::string &compressorType) {
  auto isHistoWorkspace = std::dynamic_pointer_cast<Mantid::API::IMDHistoWorkspace>(workspace) != nullptr;
  ...
  const vtkXMLWriter::CompressorType compressor = [&compressorType] {
    if (compressorType == "NONE") {
      ...
      g_log.warning("Incorrect CompressorType: " + compressorType + ". Using CompressorType=NONE.");
      return vtkXMLWriter::NONE;
    }
  }();
  // Define a time slice.
  ...
  auto factoryChain = getDataSetFactoryChain(isHistoWorkspace, normalization, time);
  ...
  auto writeSuccessFlag = writeDataSetToVTKFile(writer, dataSet, fullFilename, compressor);
  if (!writeSuccessFlag) {
 ...
 */
std::unique_ptr<vtkDataSetFactory> SaveMDWorkspaceToVTKImpl::getDataSetFactoryChain(bool isHistoWorkspace,
 ...
 */
std::unique_ptr<MDLoadingPresenter> SaveMDWorkspaceToVTKImpl::getPresenter(bool isHistoWorkspace,
...
void SaveMDWorkspaceToVTKImpl::progressFunction(vtkObject *caller, long unsigned, void *) {
 ...
 */
int SaveMDWorkspaceToVTKImpl::writeDataSetToVTKFile(vtkXMLWriter *writer, vtkDataSet *dataSet,
                                                    const std::string &filename,
                                                    vtkXMLWriter::CompressorType compressor) {
  writer->AddObserver(vtkCommand::ProgressEvent, this, &SaveMDWorkspaceToVTKImpl::progressFunction);
...
VisualNormalization
SaveMDWorkspaceToVTKImpl::translateStringToVisualNormalization(const std::string &normalization) const {
...
void SaveMDWorkspaceToVTKImpl::setupMembers() {
  ...
  m_normalizations.emplace("NoNormalization", VisualNormalization::NoNormalization);
  m_normalizations.emplace("NumEventsNormalization", VisualNormalization::NumEventsNormalization);
  m_normalizations.emplace("VolumeNormalization", VisualNormalization::VolumeNormalization);
}
 ...
 */
double SaveMDWorkspaceToVTKImpl::selectTimeSliceValue(const Mantid::API::IMDWorkspace &workspace) const {
  double time = 0.0;
 ...
 */
bool SaveMDWorkspaceToVTKImpl::is4DWorkspace(const Mantid::API::IMDWorkspace &workspace) const {
 ...
 */
bool SaveMDWorkspaceToVTKImpl::is3DWorkspace(const Mantid::API::IMDWorkspace &workspace) const {
...
std::string SaveMDWorkspaceToVTKImpl::getFullFilename(std::string filename, bool isHistoWorkspace) const {
  const auto extension = isHistoWorkspace ? structuredGridExtension : unstructuredGridExtension;
 ...
 */
vtkSmartPointer<vtkDataSet> SaveMDWorkspaceToVTKImpl::getDataSetWithOrthogonalCorrection(
    vtkSmartPointer<vtkDataSet> dataSet, MDLoadingPresenter *presenter, Mantid::API::IMDWorkspace_sptr workspace,
