...
SaveMDWorkspaceToVTK::SaveMDWorkspaceToVTK() : saver(std::make_unique<SaveMDWorkspaceToVTKImpl>(this)) {}

SaveMDWorkspaceToVTK::~SaveMDWorkspaceToVTK() {}

const std::string SaveMDWorkspaceToVTK::name() const { return "SaveMDWorkspaceToVTK"; }

int SaveMDWorkspaceToVTK::version() const { return 1; }

const std::string SaveMDWorkspaceToVTK::category() const { return "MDAlgorithms"; }
...
void SaveMDWorkspaceToVTK::init() {
  declareProperty(std::make_unique<API::WorkspaceProperty<API::IMDWorkspace>>("InputWorkspace", "",
  ...
  declareProperty(std::make_unique<API::FileProperty>("Filename", "", Mantid::API::FileProperty::Save, extensions,
                                                      Mantid::Kernel::Direction::Input),
                  "Save location.");

  auto normalizations = saver->getAllowedNormalizationsInStringRepresentation();
  declareProperty("Normalization", "AutoSelect", std::make_shared<Mantid::Kernel::StringListValidator>(normalizations),
                  "The visual normalization option. The automatic default will choose a "
                  "normalization based on your data type and instrument.");

  std::shared_ptr<Mantid::Kernel::BoundedValidator<int>> mustBePositive(new Mantid::Kernel::BoundedValidator<int>());
  ...
  declareProperty("CompressorType", "NONE",
                  std::make_shared<Mantid::Kernel::StringListValidator>(std::vector<std::string>{"NONE", "ZLIB"}),
...
void SaveMDWorkspaceToVTK::exec() {
  ...
  auto normalization = saver->translateStringToVisualNormalization(normalizationInStringRepresentation);
  ...
  saver->saveMDWorkspace(inputWS, filename, normalization, recursionDepth, compressorType);
}
