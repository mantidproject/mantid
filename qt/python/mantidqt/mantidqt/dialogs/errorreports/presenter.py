...
class ErrorReporterPresenter(object):
    ...
    def __init__(self, view, exit_code: str, application: str, traceback: Optional[str] = None):
        ...
        self._async_recovery_worker = GDBAsync(parent_presenter=self)
        ...
        if not traceback:
            ...
            try:
                if os.path.isfile(traceback_file_path):
                    ...
                    self._view.block_by_async(True)
                    self._async_recovery_worker.run_gdb_recovery()
            except OSError:
                pass
    ...
    def async_completion_call_back(self):
        self._view.block_by_async(False)
