...
def retrieve_thread_traces_from_coredump_file(ppid: str) -> bytes:
    ...
    core_file = _get_most_recent_core_dump_file(core_dumps_path, ppid)
...
def _get_most_recent_core_dump_file(core_dumps_dir: Path, ppid: str) -> Path | None:
    ...
    if files_sorted_by_latest:
        for latest_core_dump_file in files_sorted_by_latest:
            ...
            if age.seconds < CORE_DUMP_RECENCY_LIMIT:
                ...
                if _check_core_file_is_the_workbench_process(latest_core_dump_file, ppid):
...
def _check_core_file_is_the_workbench_process(core_dump_file: Path, ppid: str) -> bool:
    ...
    search_result = re.search(r"ppid: (\d+) ", stdout)
    if search_result is not None:
        return search_result.group(1) == ppid
    return False
