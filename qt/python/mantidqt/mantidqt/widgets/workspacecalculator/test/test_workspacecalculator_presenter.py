...
class WorkspaceCalculatorTest(unittest.TestCase):
    def setUp(self):
        ...
        self.model.updateParameters = mock.Mock(
            return_value=(False, False, ["Input1: failed during update", "Input2: failed during update"])
        )
    ...
    def test_workspaceCalculator_onPressedGo_update_fail(self):
        ...
        # check calls
        presenter.model.updateParameters.assert_called_with(
            lhs_scale=1.0, lhs_ws="ws1", operation="*", output_ws="output", rhs_scale=1.0, rhs_ws="ws2"
        )
        ...
        presenter.view.setValidationLabel.assert_called_with(
            tooltip=["Input1: failed during update", "Input2: failed during update"], validationValue=False, ws="RHS"
        )
    ...
    def test_workspaceCalculator_onPressedGo_execution_fail(self):
        ...
        presenter.view.setValidationLabel.assert_called_with(
            tooltip=["Input1: execution failed", "Input2: execution failed"], validationValue=False, ws="RHS"
        )
        # here one can only catch the second call, thus ws is RHS, while both fail
    ...
    def test_workspaceCalculator_onPressedGo_success(self):
        ...
        self.assertEqual(presenter.model.performOperation.call_count, 1)
        self.assertEqual(presenter.view.setValidationLabel.call_count, 4)
        presenter.view.setValidationLabel.assert_called_with(tooltip=["", ""], validationValue=True, ws="RHS")
