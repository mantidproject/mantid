...
# local imports
from mantidqt.widgets.sliceviewer.peaksviewer.representation.ellipsoid import EllipsoidalIntegratedPeakRepresentation, slice_ellipsoid

from mantidqt.widgets.sliceviewer.peaksviewer.representation.test.shapetesthelpers import (
    FuzzyMatch,
    create_ellipsoid_info,
    draw_representation,
    create_sphere_info,
)
...
def create_test_ellipsoid(bg_shell=True, do_translate=False):
    ...
    return create_ellipsoid_info(
        radii=radii, axes=("1 0 0", "0 1 0", "0 0 1"), bkgd_radii=((bg_inner), (bg_outer)), translation=translation
    )
...
class EllipsoidalIntergratedPeakRepresentationTest(unittest.TestCase):
    def test_draw_creates_circle_with_expected_properties_with_nonzero_alpha_and_background(self, compute_alpha_mock):
        peak_center = [1, 2, 3]
        ...
        painted = draw_representation(
            EllipsoidalIntegratedPeakRepresentation, peak_center, sphere, painter, fg_color, bg_color, shape_name="spherical"
        )
    ...
    def test_draw_creates_circle_with_expected_properties_with_nonzero_alpha_and_no_background_in_JSON_shape(self, compute_alpha_mock):
        peak_center = [1, 2, 3]
        ...
        self.assertTrue(painted is not None)
        self._assert_painter_calls(
            painter, peak_center[:2], cross_width=0.08, signal_width=0.8, signal_height=0.8, angle=0, alpha=fake_alpha, fg_color=fg_color
        )
    ...
    def test_draw_creates_nothing_when_alpha_lt_zero(self, compute_alpha_mock):
        ...
        painted = draw_representation(EllipsoidalIntegratedPeakRepresentation, [1, 2, 3], ellipsoid, painter, "r", "g")
    ...
    def test_draw_creates_ellipse_with_expected_properties_with_nonzero_alpha_and_background(self, compute_alpha_mock):
        ...
        painted = draw_representation(EllipsoidalIntegratedPeakRepresentation, peak_center, ellipsoid, painter, fg_color, bg_color)
    ...
    def test_draw_creates_ellipse_with_expected_properties_with_nonzero_alpha_no_background(self, compute_alpha_mock):
        ...
        self.assertTrue(painted is not None)
        self._assert_painter_calls(
            painter, peak_center[:2], cross_width=0.3, signal_width=3.0, signal_height=2.6, angle=0, alpha=fake_alpha, fg_color=fg_color
        )
    ...
    def test_draw_respects_transform(self, compute_alpha_mock):
        ...
        painted = draw_representation(
            EllipsoidalIntegratedPeakRepresentation, peak_center, ellipsoid, painter, fg_color, bg_color, slice_transform
        )
        ...
        fractional_thickness = (
            (major_outer_radius - major_inner_radius) / major_outer_radius,
    ...
    def test_draw_varying_background_thickness(self, compute_alpha_mock):
        ...
        ellipsoid = create_ellipsoid_info(
            radii=(0.5, 0.5, 0.5), axes=("1 0 0", "0 1 0", "0 0 1"), bkgd_radii=((0.5, 0.6, 0.6), (0.6, 1.0, 1.0))
        )
        painter = MagicMock()
        fg_color, bg_color = "r", "g"
        fake_alpha = 0.5
    ...
    def test_draw_with_nonzero_translation(self, compute_alpha_mock):
        ...
        painted = draw_representation(
            EllipsoidalIntegratedPeakRepresentation, list(peak_center), ellipsoid, painter, fg_color, bg_color, slice_transform
        )
