...
# local imports
from mantidqt.widgets.sliceviewer.peaksviewer.representation.noshape import NonIntegratedPeakRepresentation
from mantidqt.widgets.sliceviewer.peaksviewer.test.modeltesthelpers import create_slice_info
from mantidqt.widgets.sliceviewer.peaksviewer.representation.test.shapetesthelpers import FuzzyMatch
...
class NonIntegratedPeakRepresentationTest(unittest.TestCase):
    ...
    def test_draw_creates_cross_with_expected_properties_when_alpha_gt_zero(self, compute_alpha_mock):
        ...
        slice_info = create_slice_info(lambda x: x, slice_value=3.0, slice_width=10.0)

        painted = NonIntegratedPeakRepresentation.draw(peak_origin, peak_shape, slice_info, painter, fg_color, "unused")

        width = 0.15
        painter.cross.assert_called_with(peak_origin[0], peak_origin[1], FuzzyMatch(width, atol=1e-2), alpha=alpha, color=fg_color)
