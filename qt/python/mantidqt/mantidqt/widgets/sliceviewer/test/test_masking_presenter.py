...
class SliceViewerBasePresenterTest(unittest.TestCase):
    ...
    def test_new_selector_no_active_selector(self, mock_masking_model_fn):
        mock_model = Mock()
        mock_masking_model_fn.return_value = mock_model
    ...
    def test_export_selectors(self, mock_masking_model_fn):
        ...
        with (
            patch(
                "mantidqt.widgets.sliceviewer.presenters.masking.TOOL_ITEM_TEXT_TO_SELECTOR",
                {ToolItemText.RECT_MASKING: mocks["selector_cls"]},
            ),
            patch.object(Masking, "_reset_active_selector", autospec=True, wraps=Masking._reset_active_selector) as reset_spy,
        ):
            presenter = Masking(dataview=mocks["dataview"], ws_name=Mock())
    ...
    def test_clear_and_disconnect(self):
        pass
