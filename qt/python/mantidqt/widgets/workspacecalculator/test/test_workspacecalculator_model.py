...
#
#
from mantid.simpleapi import CloneWorkspace, CreateMDHistoWorkspace, \
    CreateSingleValuedWorkspace, CreateSampleWorkspace, GroupWorkspaces
...
class WorkspaceCalculatorModelTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.md_lhs = 'md_lhs'
        CreateMDHistoWorkspace(Dimensionality=2, Extents='-3,3,-10,10',
                               SignalInput=S1, ErrorInput=ERR,
                               NumberOfBins='2,5', Names='Dim1,Dim2',
                               Units='MomentumTransfer,EnergyTransfer',
                               OutputWorkspace=cls.md_lhs)
        cls.md_rhs = 'md_rhs'
        CreateMDHistoWorkspace(Dimensionality=2, Extents='-3,3,-10,10',
                               SignalInput=S2, ErrorInput=ERR,
                               NumberOfBins='2,5', Names='Dim1,Dim2',
                               Units='MomentumTransfer,EnergyTransfer',
    ...
    def test_model_single(self):
        ...
        for operation in self.operations:
            ...
            self.check_validity(lhs_validation=valid_lhs, rhs_validation=valid_rhs, err_msg=err_msg,
                                passed=True)
            if operation == "+":
    ...
    def test_model_matrix(self):
        ...
        for operation in self.operations:
            ...
            model = WorkspaceCalculatorModel(lhs_ws=self.matrix_lhs,
    ...
    def test_model_event_groups(self):
        ...
        for operation in self.operations:
            ...
            model = WorkspaceCalculatorModel(lhs_ws=self.event_group_lhs,
                                             rhs_ws=self.event_group_rhs,
                                             output_ws=output_ws,
                                             operation=operation)
            valid_lhs, valid_rhs, err_msg = model.performOperation()
            self.assertTrue(mtd[output_ws])
    ...
    def test_model_md(self):
        ...
        for operation in self.operations:
            ...
            model = WorkspaceCalculatorModel(lhs_ws=self.md_lhs,
                                             rhs_ws=self.md_rhs,
                                             output_ws=output_ws,
                                             operation=operation)
            valid_lhs, valid_rhs, err_msg = model.performOperation()
