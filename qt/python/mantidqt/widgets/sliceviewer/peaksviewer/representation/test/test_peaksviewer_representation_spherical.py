...
class SphericallyIntergratedPeakRepresentationTest(unittest.TestCase):
    def test_draw_creates_nothing_when_alpha_lt_zero(self, compute_alpha_mock):
        ...
        painted = draw_representation(SphericallyIntergratedPeakRepresentation, (1, 2, 3), sphere, painter, 'r', 'g')
    ...
    def test_draw_creates_circle_with_expected_properties_with_nonzero_alpha_and_no_background(
        ...
        painted = draw_representation(SphericallyIntergratedPeakRepresentation, peak_center, sphere, painter, fg_color,
                                      bg_color)
    ...
    def test_draw_respects_slice_offset(self, compute_alpha_mock):
        ...
        bkgd_slice_radius = np.cos(np.arcsin(
            slice_diff / sphere['background_outer_radius'])) * sphere['background_outer_radius']
        bkgd_slice_radius_inner = np.cos(np.arcsin(
            slice_diff / sphere['background_inner_radius'])) * sphere['background_inner_radius']
    ...
    def _assert_painter_calls(self,
        ...
        # signal circle
        painter.circle.assert_called_once_with(x,
                                               y,
        ...
        if bkgd_slice_radius is None:
            ...
            painter.shell.assert_called_once_with(x,
                                                  y,
