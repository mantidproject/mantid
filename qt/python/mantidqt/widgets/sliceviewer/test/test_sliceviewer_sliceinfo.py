...
class SliceInfoTest(unittest.TestCase):
    def test_construction_with_named_fields(self):
        ...
        info = SliceInfo(frame=frame, point=point, transpose=transpose, range=dimrange, qflags=spatial)
    ...
    def test_non_HKL_workspace_cannot_support_nonorthogonal_axes(self):
        self._assert_can_support_nonorthogonal_axes(False, SpecialCoordinateSystem.QLab, qflags=[True] * 3)

    def test_HKL_workspace_with_one_slice_spatial_dimension_cannot_support_nonorthogonal_axes(self):
        self._assert_can_support_nonorthogonal_axes(False, SpecialCoordinateSystem.HKL, qflags=[True, False, False])

    def test_HKL_workspace_with_no_slice_spatial_dimension_cannot_support_nonorthogonal_axes(self):
        self._assert_can_support_nonorthogonal_axes(False, SpecialCoordinateSystem.HKL, qflags=[False] * 3)

    def test_HKL_workspace_with_two_slice_spatial_dimensions_can_support_nonorthogonal_axes(self):
        self._assert_can_support_nonorthogonal_axes(True, SpecialCoordinateSystem.HKL, qflags=[True, True, False])
