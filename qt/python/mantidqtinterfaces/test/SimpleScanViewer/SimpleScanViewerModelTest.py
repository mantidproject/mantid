...
class SimpleScanViewerModelTest(unittest.TestCase):
    ...
    def create_scan_ws(name, number_two_theta_points=50, number_omega_points=10):
        ...
        y_values = np.linspace(start=0, stop=number_of_points - 1, num=number_of_points)
    ...
    def test_roi_integration(self):
        ...
        self.create_scan_ws(name="roi_ws", number_two_theta_points=number_two_theta_points, number_omega_points=number_omega_points)
        ws = mtd["roi_ws"]

        rois = [
            Rectangle((0, 0), 50, 10),  # the entire workspace
        ...
        self.create_scan_ws(name="bg_ws", number_two_theta_points=number_two_theta_points, number_omega_points=1)
        bg_ws = mtd["bg_ws"]
        ...
        expected_corrected = [
            500 * 499 / 2 - 10 * 50 * 49 / 2,
            5 * 50 + 5 - 5,
            sum([50 * i + j - j for i in range(5, 5 + 3) for j in range(10, 10 + 2)]),
            0,
            50 * 10 * 9 / 2,
        ]  # first background column is zero so no correction
