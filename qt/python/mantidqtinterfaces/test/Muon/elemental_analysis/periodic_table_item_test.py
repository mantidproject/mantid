...
from mantidqtinterfaces.Muon.GUI.ElementalAnalysis.PeriodicTable.periodic_table import PeriodicTableItem, ColoredPeriodicTableItem
...
class PeriodicTableItemTest(unittest.TestCase):
    ...
    def test_that_given_list_of_elements_contains_all_the_required_fields(self):
        ...
        for i in range(len(periodic_table._elements) - 1):
            ...
            self.assertTrue(isinstance(periodic_table._elements[i][5], float) or isinstance(periodic_table._elements[i][5], int))
    ...
    @mock.patch("mantidqtinterfaces.Muon.GUI.ElementalAnalysis.PeriodicTable.periodic_table._logger.warning")
