...
import mantidqtinterfaces.Muon.GUI.ElementalAnalysis.PeriodicTable.periodic_table as periodic_table
from mantidqtinterfaces.Muon.GUI.ElementalAnalysis.PeriodicTable.periodic_table import PeriodicList
...
class PeriodicListTest(unittest.TestCase):
    ...
    @mock.patch("mantidqtinterfaces.Muon.GUI.ElementalAnalysis.PeriodicTable.periodic_table.QtWidgets.QTreeWidgetItem")
    ...
    def test_all_tree_items_have_correct_name(self):
        ...
        assert all([self.plist.tree_items[i].text(0) == str(el.Z) for i, el in enumerate(periodic_table._default_table_items)])
        assert all([self.plist.tree_items[i].text(1) == el.symbol for i, el in enumerate(periodic_table._default_table_items)])
    ...
    @mock.patch(
        "mantidqtinterfaces.Muon.GUI.ElementalAnalysis.PeriodicTable.periodic_table.QtWidgets.QTreeWidgetItem.isSelected",
    ...
    def test_that_getSelection_returns_correct_number_of_elements(self, mock_isSelected):
        ...
        self.assertEqual(
            ret, [periodic_table._default_table_items[0], periodic_table._default_table_items[1], periodic_table._default_table_items[2]]
        )
