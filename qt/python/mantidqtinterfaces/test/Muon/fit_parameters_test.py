...
from mantidqtinterfaces.Muon.GUI.Common.contexts.fitting_contexts.fitting_context import FitParameters
...
class FitParametersTest(unittest.TestCase):
    ...
    def test_length_returns_all_params_with_no_globals(self):
        test_parameters = OrderedDict([("Height", (10.0, 0.4)), ("A0", (1, 0.01)), ("Cost function", (0.1, 0.0))])
    ...
    def test_names_value_error_returns_all_expected_values_with_no_globals(self):
        test_parameters = OrderedDict([("f0.Height", (10.0, 0.4)), ("f0.A0", (1, 0.01)), ("Cost function", (0.1, 0.0))])
        fit_params = create_test_fit_parameters(test_parameters)

        self.assertEqual(list(test_parameters.keys()), fit_params.names())
        self.assertEqual(3, len(fit_params))
        for index, name in enumerate(fit_params.names()):
            self.assertEqual(test_parameters[name][0], fit_params.value(name), msg="Mismatch in error for parameter" + name)
            self.assertEqual(test_parameters[name][1], fit_params.error(name), msg="Mismatch in error for parameter" + name)
    ...
    def test_names_return_globals_first_with_simultaneous_prefixes_stripped_for_composite_fn(self):
        ...
        expected_keys = ["f0.A0", "f0.f0.A1", "f0.f1.A0", "f0.f1.A1", "f1.f0.A1", "f1.f1.A0", "f1.f1.A1", "Cost function"]
        self.assertEqual(expected_keys, fit_params.names())

    def test_names_value_error_returns_all_expected_values_with_globals(self):
        test_parameters = OrderedDict(
            [
                ("f0.Height", (10.0, 0.4)),  # global
                ("f0.A0", (1, 0.01)),
                ("f1.Height", (10.0, 0.4)),  # global
                ("f1.A0", (2, 0.05)),
                ("Cost function", (0.1, 0.0)),
            ]
        )
        ...
        for index, name in enumerate(fit_params.names()):
            if name == "Height":
                ...
                orig_name = name
            self.assertEqual(test_parameters[orig_name][0], fit_params.value(name), msg="Mismatch in error for parameter" + name)
            self.assertEqual(test_parameters[orig_name][1], fit_params.error(name), msg="Mismatch in error for parameter" + name)
