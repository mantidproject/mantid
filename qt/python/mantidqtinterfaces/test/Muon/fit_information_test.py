...
def create_test_workspace(ws_name=None, time_series_logs=None, string_value_logs=None):
    """
    ...
    if string_value_logs is not None:
        for name, value in string_value_logs:
            run.addProperty(name, StringPropertyWithValue(name, value), replace=True)
...
class FitInformationTest(unittest.TestCase):
    ...
    def test_equality_with_no_globals(self):
        fit_info = FitInformation(mock.MagicMock(), "MuonGuassOsc", mock.MagicMock(), mock.MagicMock(), mock.MagicMock())
        self.assertEqual(fit_info, fit_info)

    def test_equality_with_globals(self):
        fit_info = FitInformation(mock.MagicMock(), "MuonGuassOsc", mock.MagicMock(), mock.MagicMock(), mock.MagicMock(), ["A"])
        self.assertEqual(fit_info, fit_info)

    def test_inequality_with_globals(self):
        fit_info1 = FitInformation(mock.MagicMock(), "MuonGuassOsc", mock.MagicMock(), mock.MagicMock(), ["A"])
        fit_info2 = FitInformation(mock.MagicMock(), "MuonGuassOsc", mock.MagicMock(), mock.MagicMock(), ["B"])
        self.assertNotEqual(fit_info1, fit_info2)

    def test_empty_global_parameters_if_none_specified(self):
        fit_information_object = FitInformation(mock.MagicMock(), mock.MagicMock(), mock.MagicMock(), mock.MagicMock(), mock.MagicMock())

        self.assertEqual([], fit_information_object.parameters.global_parameters)

    def test_global_parameters_are_captured(self):
        fit_information_object = FitInformation(
            mock.MagicMock(), mock.MagicMock(), mock.MagicMock(), mock.MagicMock(), mock.MagicMock(), ["A"]
        )

        self.assertEqual(["A"], fit_information_object.parameters.global_parameters)

    def test_parameters_are_readonly(self):
        fit_info = FitInformation(mock.MagicMock(), mock.MagicMock(), mock.MagicMock(), mock.MagicMock(), mock.MagicMock())

        self.assertRaises(AttributeError, setattr, fit_info, "parameters", mock.MagicMock())

    def test_logs_from_workspace_without_logs_returns_emtpy_list(self):
        fake_ws = create_test_workspace()
        fit = FitInformation(
            mock.MagicMock(), "func1", [StaticWorkspaceWrapper(fake_ws.name(), fake_ws)], mock.MagicMock(), mock.MagicMock()
        )
    ...
    def test_logs_for_single_workspace_return_all_time_series_logs(self):
        time_series_logs = (("ts_1", (1.0,)), ("ts_2", (3.0,)))
        ...
        for name, _ in time_series_logs:
            self.assertTrue(name in log_names, msg="{} not found in log list".format(name))
        for name, _ in single_value_logs:
            self.assertFalse(name in log_names, msg="{} found in log list".format(name))

    def test_log_names_from_list_of_workspaces_gives_combined_set(self):
        time_series_logs = (("ts_1", (1.0,)), ("ts_2", (3.0,)), ("ts_3", [2.0]), ("ts_4", [3.0]))

        fake1 = create_test_workspace(ws_name="fake1", time_series_logs=time_series_logs[:2])
        fake2 = create_test_workspace(ws_name="fake2", time_series_logs=time_series_logs[2:])
        fit = FitInformation(
            mock.MagicMock(),
            "func1",
            [StaticWorkspaceWrapper(fake1.name(), fake1), StaticWorkspaceWrapper(fake2.name(), fake2)],
            mock.MagicMock(),
            mock.MagicMock(),
        )
    ...
    def test_log_names_uses_filter_fn(self):
        time_series_logs = (("ts_1", (1.0,)), ("ts_2", (3.0,)), ("ts_3", [2.0]), ("ts_4", [3.0]))
        fake1 = create_test_workspace(ws_name="fake1", time_series_logs=time_series_logs)
    ...
    def test_has_log_returns_true_if_all_workspaces_have_the_log(self):
        time_series_logs = (("ts_1", (1.0,)), ("ts_2", (3.0,)))
        fake1 = create_test_workspace(ws_name="fake1", time_series_logs=time_series_logs)
        fake2 = create_test_workspace(ws_name="fake2", time_series_logs=time_series_logs)
        fit = FitInformation(
            mock.MagicMock(),
            "func1",
            [StaticWorkspaceWrapper(fake1.name(), fake1), StaticWorkspaceWrapper(fake2.name(), fake2)],
    ...
    def test_has_log_returns_false_if_all_workspaces_do_not_have_log(self):
        ...
        self.assertFalse(fit.has_log("ts_1"), msg="All input workspaces should have the requested log")

    def test_string_log_value_from_fit_with_single_workspace(self):
        single_value_logs = [("sv_1", "5")]
        fake1 = create_test_workspace(ws_name="fake1", string_value_logs=single_value_logs)
        fit = FitInformation(mock.MagicMock(), "func1", [StaticWorkspaceWrapper(fake1.name(), fake1)], mock.MagicMock(), mock.MagicMock())

        self.assertEqual(float(single_value_logs[0][1]), fit.log_value(single_value_logs[0][0]))

    def test_time_series_log_value_from_fit_with_single_workspace_uses_time_average(self):
        time_series_logs = [("ts_1", (("2000-05-01T12:00:00", 5.0), ("2000-05-01T12:00:10", 20.0), ("2000-05-01T12:05:00", 30.0)))]
    ...
    def test_time_series_log_value_from_fit_with_multiple_workspaces_uses_average_of_time_average(self):
        time_series_logs1 = [("ts_1", (("2000-05-01T12:00:00", 5.0), ("2000-05-01T12:00:10", 20.0), ("2000-05-01T12:05:00", 30.0)))]
        fake1 = create_test_workspace("fake1", time_series_logs1)
        time_series_logs2 = [("ts_1", (("2000-05-01T12:00:30", 10.0), ("2000-05-01T12:01:45", 30.0), ("2000-05-01T12:05:00", 40.0)))]
