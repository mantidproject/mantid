...
from mantidqtinterfaces.Muon.GUI.Common.results_tab_widget.results_tab_model import DEFAULT_TABLE_NAME, ResultsTabModel, TableColumnType
...
def create_test_fits(input_workspaces, function_name, parameters, output_workspace_names=None, global_parameters=None):
    ...
    output_workspace_names = (
        output_workspace_names if output_workspace_names is not None else [StaticWorkspaceWrapper("test-output-ws", workspace)]
    )
    ...
    for name in input_workspaces:
        ...
        fits.append(FitInformation([name], function_name, output_workspace_names, parameter_workspace, mock.Mock(), global_parameters))

    return fits


def create_test_model(input_workspaces, function_name, parameters, output_workspace_names=None, logs=None, global_parameters=None):
    """
...
class ResultsTabModelTest(unittest.TestCase):
    def setUp(self):
        ...
        self.logs = [(self.log_names[0], (50.0, 60.0)), (self.log_names[1], (2.0, 3.0))]
    ...
    def test_create_results_table_with_no_logs_or_global_parameters(self):
        ...
        self._assert_table_matches_expected(zip(expected_cols, expected_types), expected_content, table, model.results_table_name())
    ...
    def test_create_results_table_raises_error_if_number_params_different(self):
        ...
        self.assertRaises(IndexError, model.create_results_table, [], selected_results)

    def test_create_results_table_with_mixed_global_non_global_raises_error(self):
        parameters = OrderedDict([("f0.Height", (100, 0.1)), ("f1.Height", (90, 0.001)), ("Cost function value", (1.5, 0))])
        fits_func1 = create_test_fits(("ws1",), "func1", parameters)
    ...
    def test_create_results_table_with_logs_missing_from_some_workspaces_raises(self):
        ...
        logs = [("log1", (1.0, 2.0)), ("log2", (3.0, 4.0)), ("log3", (4.0, 5.0)), ("log4", (5.0, 6.0))]
        fits_logs1 = create_test_fits(
            ("ws1",), "func1", parameters, output_workspace_names=[StaticWorkspaceWrapper("test-ws1-ws", workspace)]
        )
        add_logs(fits_logs1[0].input_workspaces[0], logs[:2])

        fits_logs2 = create_test_fits(
            ("ws2",), "func1", parameters, output_workspace_names=[StaticWorkspaceWrapper("test-ws2-ws", workspace)]
        )
        ...
        self.assertRaises(IndexError, model.create_results_table, selected_logs, selected_results)
