...
class MuonFreqContextTest(unittest.TestCase):
    ...
    def test_get_freq_names_maxEnt(self):
        output = self.context.get_frequency_workspace_names(run_list=[["62260", "62261", "62262"]],
                                                            group=["fwd", "top"], pair=[],
                                                            frequency_type="MaxEnt", x_label=FIELD)
        self.assertEqual(output, [get_name("MUSR62260_raw_data FD; MaxEnt", GAUSS)])

        output2 = self.context.get_frequency_workspace_names(run_list=[["62260", "62261", "62262"]],
                                                             group=["fwd", "top"], pair=[],
                                                             frequency_type="MaxEnt", x_label=FREQ)
    ...
    def test_get_freq_names_FFT_no_Im_all_parts(self):
        output = self.context.get_frequency_workspace_names(run_list=[["62260"]], group=["fwd", "top"], pair=[],
                                                            frequency_type="FFT All", x_label=FIELD)
    ...
    def test_get_freq_names_FFT_no_Im_all_parts_freq(self):
        output = self.context.get_frequency_workspace_names(run_list=[["62260"]], group=["fwd", "top"], pair=[],
                                                            frequency_type="FFT All", x_label=FREQ)
    ...
    def test_get_freq_names_FFT_run(self, field, rename):
        ...
        output = self.context.get_frequency_workspace_names(run_list=[["62261", "62262"]], group=["fwd", "top"],
                                                            pair=[], frequency_type="FFT All", x_label=FIELD)
        self.assertCountEqual(output,
    ...
    def test_get_freq_names_FFT_run_freq(self, field, rename):
        ...
        output = self.context.get_frequency_workspace_names(run_list=[["62261", "62262"]], group=["fwd", "top"],
                                                            pair=[], frequency_type="FFT All", x_label=FREQ)
        self.assertCountEqual(output,
    ...
    def test_get_freq_names_all(self, field, rename):
        ...
        output = self.context.get_frequency_workspace_names(run_list=[[62260], [62261]], group=["fwd"], pair=[],
                                                            frequency_type="All", x_label=FIELD)
        self.assertCountEqual(output,
                              [get_name(FFT_NAME_COMPLEX_RE, GAUSS),
                               get_name(FFT_NAME_COMPLEX_IM, GAUSS),
                               get_name(FFT_NAME_COMPLEX_MOD,GAUSS),
    ...
    def test_get_freq_names_all_freq(self, field, rename):
        ...
        output = self.context.get_frequency_workspace_names(run_list=[[62260], [62261]], group=["fwd"], pair=[],
                                                            frequency_type="All", x_label=FREQ)
        self.assertCountEqual(output,
                              [get_name(FFT_NAME_COMPLEX_RE, MHz),
                               get_name(FFT_NAME_COMPLEX_IM, MHz),
                               get_name(FFT_NAME_COMPLEX_MOD,MHz),
