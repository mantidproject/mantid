...
def fetch_correction_workspaces(vanadium_path: str, instrument: str):
    ...
    if not check_workspaces_exist(van_run_no):
        van_integration_ws, van_processed_inst_ws = create_vanadium_corrections(vanadium_path, instrument)
    else:
...
def create_vanadium_corrections(vanadium_path: str, instrument: str):  # -> Workspace, Workspace
    ...
    try:
        ...
        logger.error(
            ...
            "Could not run Load algorithm with vanadium run number: " + str(vanadium_path) + ". Error description: " + str(e)
        )
        raise RuntimeError
