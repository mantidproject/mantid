...
def add_histograms(data_filenames, project, instruments, number_regions):
    if number_regions > len(data_filenames):  # many regions in one data and one instrument file
        if len(data_filenames) != 1:
            raise ValueError("There must be one region/bank per focused data file"
                             "or many regions/banks in one focused data file")
        for loop_region_index in range(1, number_regions+1):
            project.add_powder_histogram(datafile=os.path.join(data_filenames[0]),
        ...
        if len(instruments) == len(data_filenames):
            for loop_index, loop_data_filename in enumerate(data_filenames):
                project.add_powder_histogram(datafile=os.path.join(loop_data_filename),
                                             iparams=os.path.join(instruments[loop_index]),
                                             phases=project.phases()
                                             )
            ...
            raise ValueError("Calling GSASII from Mantid with multiple instrument files and one focused data file"
...
def export_refinement_to_csv(temp_save_directory, name_of_project, project):
    for histogram_index, loop_histogram in enumerate(project.histograms()):
        loop_histogram.Export(os.path.join(temp_save_directory, name_of_project + f"_{histogram_index+1}.csv"),
