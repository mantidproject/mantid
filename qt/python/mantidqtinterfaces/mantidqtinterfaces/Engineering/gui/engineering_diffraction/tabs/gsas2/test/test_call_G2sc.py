...
class GSAS2ViewTest(unittest.TestCase):
    ...
    def test_add_histograms_more_datafiles_one_instrument(self):
        ...
        expected_calls = [
            ...
            # File 2, banks 1-3
            mock.call(datafile="data_file_2", iparams="instr_1", phases=["phase_1"], databank=1, instbank=1),
            mock.call(datafile="data_file_2", iparams="instr_1", phases=["phase_1"], databank=2, instbank=2),
            mock.call(datafile="data_file_2", iparams="instr_1", phases=["phase_1"], databank=3, instbank=3),
        ]
    ...
    def test_add_histograms_throws_for_single_bank_per_file(self):
        ...
        with self.assertRaises(ValueError) as context:
            add_histograms(data_filenames, self.project, instruments, number_of_regions, banks_per_file)
        self.assertIn("Each data file must contain multiple banks", str(context.exception))
    ...
    def test_add_histograms_throws_for_inconsistent_region_count(self):
        """Test that inconsistent total region count is rejected"""
        data_filenames = ["data_file_1", "data_file_2"]
        instruments = ["instr_1"]
        number_of_regions = 5  # Should be 6 for 2 files * 3 banks
        banks_per_file = 3
        with self.assertRaises(ValueError) as context:
            add_histograms(data_filenames, self.project, instruments, number_of_regions, banks_per_file)
        self.assertIn("Expected 6 total regions, but found 5", str(context.exception))
