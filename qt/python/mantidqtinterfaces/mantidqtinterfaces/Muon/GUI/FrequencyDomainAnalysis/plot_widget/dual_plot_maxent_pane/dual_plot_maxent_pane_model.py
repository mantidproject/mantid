...
class DualPlotMaxentPaneModel(BasePaneModel):
    def __init__(self, context, time_group_model, raw_model):
        ...
        self.context.plot_panes_context[self.name].set_defaults([0.0, 10.0], [0.0, 1.0e6])
    ...
    def get_workspace_list_and_indices_to_plot(self):
        ...
        if self._is_groups:
            ...
            workspace_list, indices = self._raw_model.get_workspace_list_and_indices_to_plot(
                True, "Counts", self._selection, self._run, self._period
            )
