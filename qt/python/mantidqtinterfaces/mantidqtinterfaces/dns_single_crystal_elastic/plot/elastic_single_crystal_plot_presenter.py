...
class DNSElasticSCPlotPresenter(DNSObserver):
    def __init__(self, name=None, parent=None, view=None, model=None):
        ...
        self.view.sig_update_omegaoffset.connect(self._update_omega_offset)
    ...
    def _plot(self, initial_values=None):
        ...
        self._set_initial_omega_offset_dxdy()
        self.view.single_crystal_plot.connect_ylim_change()
        self.view.draw()

    def _set_initial_omega_offset_dxdy(self):
        omega_offset = self.model.get_omegaoffset()
        dx, dy = self.model.get_dx_dy()
        self.view.set_initial_oof_dxdy(omega_offset, dx, dy)
    ...
    def _plot_triangulation(self, interpolate, axis_type, switch):
        ...
        triangulation, z = self.model.get_interpolated_triangulation(
            interpolate, axis_type, switch)
        self.view.single_crystal_plot.plot_triangulation(
            triangulation, z, color_map, edge_colors, shading)
    ...
    def _want_plot(self, plot_type):
        ...
        if plot_type == 'quadmesh':
            self._plot_quadmesh(axis_type['interpolate'], axis_type['type'],
                                axis_type['switch'])
        if plot_type == 'triangulation':
            self._plot_triangulation(
                axis_type['interpolate'], axis_type['type'], axis_type['switch'])
    ...
    def _set_axis_labels(self):
        axis_type = self.view.get_axis_type()
        own_dict = self.view.get_state()
        x_label, y_label = self.model.get_axis_labels(axis_type['type'],
    ...
    def _get_current_xy_lim(self, zoom=False):
        own_dict = self.view.get_state()
        axis_type = self.view.get_axis_type()
        xlim, ylim = self.model.get_mlimits(own_dict['x_range'], own_dict['y_range'])
        if zoom:
    ...
    def _get_current_z_lim(self, xlim, ylim, zoom):
        ...
        if axis_type['switch']:
            xlim, ylim = ylim, xlim
        dz_min, dz_max, dpz_min = self.model.get_data_zmin_max(xlim, ylim)
        zlim = self.model.get_mzlimit(own_dict['z_range'])
        if zlim[0] is None:
            zlim = [dz_min, dz_max]
            manual_z = False
