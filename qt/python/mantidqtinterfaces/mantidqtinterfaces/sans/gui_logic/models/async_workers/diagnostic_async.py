...
class DiagnosticsAsync(IQtAsync):
    ...
    def run_integral(self, integral_ranges, mask, integral, detector, state):
        ...
        for input_workspace in input_workspaces:
            ...
            output_workspace = self._integrate_ranges(ranges, integral, mask, detector, input_workspace_name,
    ...
    def _integrate_ranges(self, ranges, integral, mask, detector, input_workspace_name, input_workspace, x_dim, y_dim,
                          is_multi_range):
        for integral_range in ranges:
            output_workspace = self._generate_output_workspace_name(integral_range, integral, mask, detector,
                                                                    input_workspace_name)
            output_workspace = self._run_algorithm(input_workspace, integral_range, integral, output_workspace, x_dim,
                                                   y_dim)
    ...
    def _run_algorithm(self, input_workspace, range, integral, output_workspace, x_dim, y_dim):
        ...
        if integral == IntegralEnum.Horizontal:
            output_workspace = ConvertAxesToRealSpace(InputWorkspace=input_workspace, OutputWorkspace=output_workspace,
                                                      VerticalAxis='x',
                                                      HorizontalAxis='y', NumberVerticalBins=int(x_dim),
                                                      NumberHorizontalBins=int(y_dim))
            output_workspace = SumSpectra(InputWorkspace=output_workspace, OutputWorkspace=output_workspace,
                                          StartWorkspaceIndex=hv_min,
                                          EndWorkspaceIndex=hv_max)
        elif integral == IntegralEnum.Vertical:
            output_workspace = ConvertAxesToRealSpace(InputWorkspace=input_workspace, OutputWorkspace=output_workspace,
                                                      VerticalAxis='y',
                                                      HorizontalAxis='x', NumberVerticalBins=int(x_dim),
                                                      NumberHorizontalBins=int(y_dim))
            output_workspace = SumSpectra(InputWorkspace=output_workspace, OutputWorkspace=output_workspace,
                                          StartWorkspaceIndex=hv_min,
                                          EndWorkspaceIndex=hv_max)
