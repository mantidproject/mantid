...
public:
  ...
  static HermitePolynomialsTest *createSuite() {
    return new HermitePolynomialsTest();
  }
  static void destroySuite(HermitePolynomialsTest *suite) { delete suite; }

  void
  test_hermitePoly_With_SingleValue_Returns_Expected_Values_For_First_Few_Terms() {
    using namespace Mantid::Kernel;
    static const unsigned int npoly(6);

    const double xValue = 3.98753;
    const double expectedValues[npoly] = {1, 7.97506, 61.6015820036,
                                          459.376072573630, 3293.942249317455,
                                          22594.378494252622};

    const double tolerance(1e-12);
    for (unsigned int i = 0; i < npoly; ++i) {
      TS_ASSERT_DELTA(expectedValues[i], Math::hermitePoly(i, xValue),
                      tolerance);
    }
  }

  void
  test_hermitePoly_With_Array_Values_Returns_Expected_Values_For_First_Few_Terms() {
    using namespace Mantid::Kernel;
    ...
    const double expectedValues[npoly][nx] = {
        ...
        {2, 27.856406460551, 77.712812921102, 151.569219381653,
         249.425625842204},
    };
    const double tolerance(1e-12);
    for (unsigned int i = 0; i < npoly; ++i) {
