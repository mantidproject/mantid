...
public:
  ...
  void test_correct_point_detector_to_current_position() {
    ...
    const double currentThetaInRad =
    ...
    const double sampleToDetectorBeamOffsetCorrected =
        offsetTupleCorrected.get<1>();

    // Positions should be identical to original. No correction
    TSM_ASSERT_DELTA("Vertical position should be unchanged",
                     sampleToDetectorVerticalOffsetCorrected,
                     sampleToDetectorVerticalOffset, 1e-6);
    TSM_ASSERT_DELTA("Beam position should be unchanged",
                     sampleToDetectorBeamOffsetCorrected,
  ...
  void do_test_correct_point_detector_position(
    ...
    const double sampleToDetectorVerticalOffsetExpected =
        std::tan(thetaInRad) * sampleToDetectorBeamOffsetExpected;
  ...
  void
  test_correct_point_detector_position_using_defaults_for_specifying_detector() {
    do_test_correct_point_detector_position();
  }

  void
  test_correct_point_detector_position_using_name_for_specifying_detector() {
    do_test_correct_point_detector_position("DetectorComponentName",
                                            "point-detector");
  }

  void
  test_correct_point_detector_position_using_spectrum_number_for_specifying_detector() {
