...
ImplicitFunctionParameterParserFactoryImpl::createImplicitFunctionParameterParserFromXML(
  ...
  for (unsigned long i = 0; i < parameters->length(); i++) {
    ...
    std::string paramParserName =
        parameter->getChildElement("Type")->innerText() + "Parser"; // Append parser to the name. Fixed convention
