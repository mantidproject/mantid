...
DeprecatedAlgorithm::DeprecatedAlgorithm() : m_replacementAlgorithm(), m_replacementVersion(-1), m_deprecatedDate() {}
...
std::string DeprecatedAlgorithm::deprecationMsg(const IAlgorithm *algo) {
  ...
  if (this->m_replacementAlgorithm.empty()) {
    ...
    if (!AlgorithmFactory::Instance().exists(this->m_replacementAlgorithm, this->m_replacementVersion)) {
      ...
      invalidReplacementMsg << "Invalid replacement algorithm '" + this->m_replacementAlgorithm + "'";
