...
template <>
MANTID_API_DLL std::vector<Mantid::API::MatrixWorkspace_sptr>
IPropertyManager::getValue<std::vector<Mantid::API::MatrixWorkspace_sptr>>(
    const std::string &name) const {
  PropertyWithValue<std::vector<Mantid::API::MatrixWorkspace_sptr>> *prop =
      dynamic_cast<
          PropertyWithValue<std::vector<Mantid::API::MatrixWorkspace_sptr>> *>(
          getPointerToProperty(name));
  if (prop) {
    return *prop;
  } else {
