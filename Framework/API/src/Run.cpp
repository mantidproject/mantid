...
Run::Run(const Run &other)
    : LogManager(other), m_goniometer(std::make_unique<Geometry::Goniometer>(*other.m_goniometer)),
...
bool Run::operator==(const Run &other) {
  return *m_goniometer == *other.m_goniometer && LogManager::operator==(other) &&
...
std::shared_ptr<Run> Run::clone() {
  ...
  clone->m_goniometer = std::make_unique<Geometry::Goniometer>(*this->m_goniometer);
 ...
 */
const Mantid::Kernel::DblMatrix &Run::getGoniometerMatrix() const { return m_goniometer->getR(); }
