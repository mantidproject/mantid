...
AlgoTimeRegister::Dump::Dump(AlgoTimeRegister &atr, const std::string &nm) : m_algoTimeRegister(atr), m_name(nm) {
...
AlgoTimeRegister::Dump::~Dump() {
  ...
  clock_gettime(CLOCK_MONOTONIC, &regFinish);
  {
    ...
    m_algoTimeRegister.m_info.emplace_back(m_name, std::this_thread::get_id(), m_regStart, regFinish);
  }
}
