...
void IFunctionGeneral::function(const FunctionDomain &domain, FunctionValues &values) const {
  ...
  if (actualValuesSize > 0 && requiredValuesSize > 0 && actualValuesSize != requiredValuesSize) {
  ...
  try {
    ...
    throw std::invalid_argument("Provided domain is not of type FunctionDomainGeneral.");
  }
}

void IFunctionGeneral::functionDeriv(const FunctionDomain &domain, Jacobian &jacobian) {
