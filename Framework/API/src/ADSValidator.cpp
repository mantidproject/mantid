...
ADSValidator::ADSValidator(const bool allowMultiSelection, const bool isOptional)
    : TypedValidator<std::vector<std::string>>(), m_AllowMultiSelection(allowMultiSelection), m_isOptional(isOptional) {
}

/// Clone the validator
Kernel::IValidator_sptr ADSValidator::clone() const { return std::make_shared<ADSValidator>(*this); }

bool ADSValidator::isMultipleSelectionAllowed() { return m_AllowMultiSelection; }

void ADSValidator::setMultipleSelectionAllowed(const bool isMultiSelectionAllowed) {
 ...
 * workspace list"
 */
std::string ADSValidator::checkValidity(const std::vector<std::string> &value) const {
