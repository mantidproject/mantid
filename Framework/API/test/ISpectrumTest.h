...
public:
  void test_empty_constructor() {
    SpectrumTester s(Histogram::Histogram::XMode::Points);
    TS_ASSERT_EQUALS(s.getDetectorIDs().size(), 0);
    TS_ASSERT_EQUALS(s.getSpectrumNo(), 0);
  }

  void test_constructor() {
    SpectrumTester s(1234, Histogram::Histogram::XMode::Points);
    TS_ASSERT_EQUALS(s.getDetectorIDs().size(), 0);
    TS_ASSERT_EQUALS(s.getSpectrumNo(), 1234);
  }

  void test_copyInfoFrom() {
    SpectrumTester a(1234, Histogram::Histogram::XMode::Points);
    a.addDetectorID(678);
    a.addDetectorID(789);
    SpectrumTester b(456, Histogram::Histogram::XMode::Points);
  ...
  void test_use_dx_flag_being_set_when_accessing_dx_with_non_const() {
    ...
    // non-const ptrDx()
    SpectrumTester s2(Histogram::Histogram::XMode::Points);
    s2.ptrDx();
    TS_ASSERT(s2.hasDx());

    // setDX vesion 1
    SpectrumTester s3(Histogram::Histogram::XMode::Points);
    Mantid::MantidVec Dx;
    s3.setDx(Dx);
    TS_ASSERT(s3.hasDx());

    // setDX vesion 2
    SpectrumTester s4(Histogram::Histogram::XMode::Points);
    Mantid::MantidVecPtr::ptr_type Dx_vec_ptr_type;
    s4.setDx(Dx_vec_ptr_type);
    TS_ASSERT(s4.hasDx());

    // setDX version 3
    SpectrumTester s5(Histogram::Histogram::XMode::Points);
  ...
  void test_use_dx_flag_not_being_set_when_accessing_dx_with_const() {
    // const dataDx()
    const SpectrumTester s(Histogram::Histogram::XMode::Points);
    s.dataDx();
    TS_ASSERT(!s.hasDx());
  }
