...
public:
  ...
  static TransformScaleFactoryTest *createSuite() { return new TransformScaleFactoryTest(); }
  ...
  void test_create() {
    ...
    std::vector<std::string> scales = TransformScaleFactory::Instance().getKeys();
    for (auto it = scales.begin(); it != scales.end(); it++) {
      TS_ASSERT_THROWS_NOTHING(l = TransformScaleFactory::Instance().create(*it));
    }
  }
