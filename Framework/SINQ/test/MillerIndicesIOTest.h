...
public:
  ...
  static MillerIndicesIOTest *createSuite() { return new MillerIndicesIOTest(); }
  ...
  void testFromString() {
    ...
    TS_ASSERT_THROWS(MillerIndicesIO::fromString(hklInvalidStringLong), const std::runtime_error &);

    std::string hklInvalidStringShort("2 3");
    TS_ASSERT_THROWS(MillerIndicesIO::fromString(hklInvalidStringShort), const std::runtime_error &);

    std::string hklInvalidStringGarbage("q43tn rufninc");
    TS_ASSERT_THROWS(MillerIndicesIO::fromString(hklInvalidStringGarbage), const boost::bad_lexical_cast &);
  }

  void testComplementarity() {
    MillerIndices hkl(1, 2, 3);
    MillerIndices hklCopy = MillerIndicesIO::fromString(MillerIndicesIO::toString(hkl));
    ...
    TS_ASSERT_EQUALS(MillerIndicesIO::toString(MillerIndicesIO::fromString(hklString)), "1 3 4");
  }
};
