...
public:
  ...
  static PoldiPeakSearchTest *createSuite() {
    return new PoldiPeakSearchTest();
  }
  ...
  void testfindPeaksRecursive() {
    ...
    double testListRaw[] = {-3.0, -2.0, 12.0,  3.0,  5.0, 7.0, 12.0, 34.0, 13.0,
                            18.0, 1.0,  -10.0, 12.0, 3.0, 4.0, 6.0,  7.0};
    std::vector<double> testList(testListRaw, testListRaw + 17);

    std::list<std::vector<double>::const_iterator> maxima =
        poldiPeakSearch.findPeaksRecursive(testList.begin(), testList.end());
  ...
  void testfindPeaks() {
    ...
    std::list<std::vector<double>::const_iterator> maxima =
        poldiPeakSearch.findPeaks(testList.begin(), testList.end());
  ...
  void testgetPeakCoordinates() {
    ...
    double testListRaw[] = {2.0,  -3.0, -2.0, 12.0, 3.0, 5.0,   7.0,
                            12.0, 34.0, 13.0, 18.0, 1.0, -10.0, 12.0,
                            3.0,  4.0,  6.0,  7.0,  3.0};
    ...
    std::list<std::vector<double>::const_iterator> maxima =
        poldiPeakSearch.findPeaksRecursive(baseData.begin(), baseData.end());

    maxima.sort();

    Unit_sptr qUnit = UnitFactory::Instance().create("MomentumTransfer");
    std::vector<PoldiPeak_sptr> peaks = poldiPeakSearch.getPeaks(
        baseData.begin(), baseData.end(), maxima, testXData, qUnit);
  ...
  void testmapPeakPositionsToCorrelationData() {
    ...
    std::list<std::vector<double>::const_iterator> secondIterators =
        poldiPeakSearch.mapPeakPositionsToCorrelationData(
            firstIterators, firstVector.begin(), secondVector.begin());
  ...
  void testgetNumberOfBackgroundPoints() {
    ...
    TS_ASSERT_EQUALS(poldiPeakSearch.getNumberOfBackgroundPoints(
                         peakPositions, correlationCounts),
                     8);
  ...
  void testgetMedianFromSortedVector() {
    ...
    TS_ASSERT_EQUALS(poldiPeakSearch.getMedianFromSortedVector(testList.begin(),
                                                               testList.end()),
                     4.0);
    ...
    TS_ASSERT_EQUALS(poldiPeakSearch.getMedianFromSortedVector(
                         testListEven.begin(), testListEven.end()),
                     3.5);
  }
  ...
  void testgetBackground() {
    ...
    double rawTestList[] = {1.0, 2.0, 1.0, 3.0, 1.0, 0.0, 4.0,
  ...
  void testgetBackgroundWithSigma() {
    ...
    TS_ASSERT_EQUALS(
        poldiPeakSearch.getNumberOfBackgroundPoints(peaks, testList), 6);

    UncertainValue bgSigma =
        poldiPeakSearch.getBackgroundWithSigma(peaks, testList);
  ...
  void testminimumPeakHeightFromBackground() {
    ...
    TS_ASSERT_EQUALS(poldiPeakSearch.minimumPeakHeightFromBackground(
                         UncertainValue(3.0, 3.5)),
                     13.5);
  }
};
