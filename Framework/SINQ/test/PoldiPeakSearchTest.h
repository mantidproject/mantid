...
public:
  ...
  void testgetNeighborSums() {
    ...
    TS_ASSERT_THROWS(poldiPeakSearch.getNeighborSums(input), const std::runtime_error &);
  }
  ...
  void testgetPeakCoordinates() {
    ...
    double testListRaw[] = {2.0,  -3.0, -2.0,  12.0, 3.0, 5.0, 7.0, 12.0, 34.0, 13.0,
                            18.0, 1.0,  -10.0, 12.0, 3.0, 4.0, 6.0, 7.0,  3.0};
    ...
    std::vector<PoldiPeak_sptr> peaks =
        poldiPeakSearch.getPeaks(baseData.begin(), baseData.end(), maxima, testXData, qUnit);
  ...
  void testmapPeakPositionsToCorrelationData() {
    ...
    std::list<std::vector<double>::const_iterator> secondIterators =
        poldiPeakSearch.mapPeakPositionsToCorrelationData(firstIterators, firstVector.begin(), secondVector.begin());
  ...
  void testgetNumberOfBackgroundPoints() {
    ...
    TS_ASSERT_THROWS(poldiPeakSearch.getNumberOfBackgroundPoints(tooManyPeaks, correlationCounts),
