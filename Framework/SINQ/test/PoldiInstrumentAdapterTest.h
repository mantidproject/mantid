...
public:
  ...
  static PoldiInstrumentAdapterTest *createSuite() {
    return new PoldiInstrumentAdapterTest();
  }
  ...
  PoldiInstrumentAdapterTest() {
    ...
    m_run.addProperty<std::vector<int>>("chopperspeed_target_int_vector",
                                        chopperSpeedTargetsInt);

    // add string property, for which there is no extractor
    m_stringRun.addProperty<std::string>(getChopperSpeedPropertyName(),
                                         "10000.0");
    m_stringRun.addProperty<std::string>(getChopperSpeedTargetPropertyName(),
                                         "10000.0");

    // run with correct chopperspeed property
    m_correctRun.addProperty<double>(getChopperSpeedPropertyName(), 10000.0);
    m_correctRun.addProperty<double>(getChopperSpeedTargetPropertyName(),
                                     10000.0);
  }

  void testVectorDoubleValueExtractor() {
    // Extract vector value with vector value extractor - this should work.
    AbstractDoubleValueExtractor_sptr extractorGood(
        new VectorDoubleValueExtractor);
    TS_ASSERT_THROWS_NOTHING((*extractorGood)(const_cast<const Run &>(m_run),
                                              "chopperspeed_vector"));

    // this should not work, because it's a "number" property (see constructor
    // above)
    AbstractDoubleValueExtractor_sptr extractorBad(
        new VectorDoubleValueExtractor);
    TS_ASSERT_THROWS(
    ...
    TS_ASSERT_EQUALS(
        (*extractorGood)(const_cast<const Run &>(m_run), "chopperspeed_vector"),
        10000.0);
  }
  ...
  void testVectorIntValueExtractor() {
    // Extract vector value with vector value extractor - this should work.
    AbstractDoubleValueExtractor_sptr extractorGood(
        new VectorIntValueExtractor);
    TS_ASSERT_THROWS_NOTHING((*extractorGood)(
        const_cast<const Run &>(m_run), "chopperspeed_target_int_vector"));
    ...
    TS_ASSERT_EQUALS((*extractorGood)(const_cast<const Run &>(m_run),
                                      "chopperspeed_target_int_vector"),
                     10000.0);
  }
  ...
  void testNumberDoubleValueExtractor() {
    ...
    // check that the value comes out correctly
    TS_ASSERT_EQUALS(
        (*extractorGood)(const_cast<const Run &>(m_run), "chopperspeed_double"),
        10000.0);
  }
  ...
  void testGetChopperSpeedFromRun() {
    ...
    TS_ASSERT_EQUALS(instrumentAdapter.getChopperSpeedFromRun(m_correctRun),
                     10000.0);
  }
  ...
  void testGetChopperSpeedTargetFromRun() {
    ...
    TS_ASSERT_EQUALS(
        instrumentAdapter.getChopperSpeedTargetFromRun(m_correctRun), 10000.0);
  }
  ...
  void testExtractPropertyFromRun() {
    ...
    TS_ASSERT_THROWS_NOTHING(
        instrumentAdapter.extractPropertyFromRun(m_correctRun, propertyName));
    TS_ASSERT_EQUALS(
        instrumentAdapter.extractPropertyFromRun(m_correctRun, propertyName),
        10000.0);
  }
  ...
  void testChopperSpeedMatchesTarget() {
    ...
    // This throws, because there is no extractor and the exception is not
    ...
    TS_ASSERT_THROWS_NOTHING(
        instrumentAdapter.chopperSpeedMatchesTarget(m_run, 10000.0));
    TS_ASSERT_EQUALS(
        instrumentAdapter.chopperSpeedMatchesTarget(m_run, 10000.0), true);
    TS_ASSERT_EQUALS(instrumentAdapter.chopperSpeedMatchesTarget(m_run, 100.0),
                     true);

    // Otherwise, the values are compared with a tolerance of 1e-4
    TS_ASSERT_EQUALS(
        instrumentAdapter.chopperSpeedMatchesTarget(m_correctRun, 10000.0),
        true);
    TS_ASSERT_EQUALS(
        instrumentAdapter.chopperSpeedMatchesTarget(m_correctRun, 10000.00009),
        true);
    TS_ASSERT_EQUALS(
        instrumentAdapter.chopperSpeedMatchesTarget(m_correctRun, 10000.0002),
        false);
    TS_ASSERT_EQUALS(
        instrumentAdapter.chopperSpeedMatchesTarget(m_correctRun, 9000.0),
        false);
  }
  ...
  void testGetExtractorForProperty() {
    ...
    AbstractDoubleValueExtractor_sptr extractor =
        instrumentAdapter.getExtractorForProperty(
            m_run.getProperty("chopperspeed_double"));
    TS_ASSERT(extractor);
    ...
    extractor = instrumentAdapter.getExtractorForProperty(
        m_stringRun.getProperty(getChopperSpeedPropertyName()));
    TS_ASSERT(!extractor);
  }
