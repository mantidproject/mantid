...
public:
  ...
  void testVectorDoubleValueExtractor() {
    ...
    TS_ASSERT_THROWS((*extractorBad)(const_cast<const Run &>(m_run), "chopperspeed_double"),
  ...
  void testNumberDoubleValueExtractor() {
    ...
    TS_ASSERT_THROWS((*extractorBad)(const_cast<const Run &>(m_run), "chopperspeed_vector"),
                     const std::invalid_argument &);
  ...
  void testExtractPropertyFromRun() {
    ...
    TS_ASSERT_THROWS(instrumentAdapter.extractPropertyFromRun(m_run, "DOESNOTEXIST"), const std::runtime_error &);

    // Throws, because there is no extractor for supplied type
    const std::string propertyName = getChopperSpeedPropertyName();
    TS_ASSERT_THROWS(instrumentAdapter.extractPropertyFromRun(m_stringRun, propertyName),
                     const std::invalid_argument &);
  ...
  void testChopperSpeedMatchesTarget() {
    ...
    TS_ASSERT_THROWS(instrumentAdapter.chopperSpeedMatchesTarget(m_stringRun, 10000.0), const std::invalid_argument &);
  ...
  void testGetExtractorForProperty() {
    ...
    TS_ASSERT_THROWS(instrumentAdapter.getExtractorForProperty(nullptr), const std::invalid_argument &);
    ...
    AbstractDoubleValueExtractor_sptr extractor =
        instrumentAdapter.getExtractorForProperty(m_run.getProperty("chopperspeed_double"));
    TS_ASSERT(extractor);
...
private:
  ...
  std::string getChopperSpeedPropertyName() { return m_testableChopperSpeedPropertyName; }

  std::string getChopperSpeedTargetPropertyName() { return m_testableChopperSpeedTargetPropertyName; }
