...
PoldiHeliumDetector::PoldiHeliumDetector()
    : PoldiAbstractDetector(), m_radius(0.0), m_elementCount(0), m_centralElement(0), m_elementWidth(0.0),
      m_angularResolution(0.0), m_totalOpeningAngle(0.0), m_availableElements(), m_efficiency(0.0),
      m_calibratedPosition(0.0, 0.0), m_vectorAngle(0.0), m_distanceFromSample(0.0), m_calibratedCenterTwoTheta(0.0),
...
double PoldiHeliumDetector::distanceFromSample(int elementIndex) {
  return sqrt(pow(m_radius, 2.0) + pow(m_distanceFromSample, 2.0) -
              2.0 * m_radius * m_distanceFromSample * cos(phi(elementIndex) - m_vectorAngle));
}
...
const std::vector<int> &PoldiHeliumDetector::availableElements() { return m_availableElements; }
