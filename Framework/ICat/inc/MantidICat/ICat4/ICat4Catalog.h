...
public:
  ...
  void search(const CatalogSearchParam &inputs, Mantid::API::ITableWorkspace_sptr &outputws, const int &offset,
...
private:
  ...
  template <class T> bool isAccessAllowed(ICat4::ns1__accessType accessType, T &bean);
   ...
   */
  template <class T> void savetoTableWorkspace(T *input, Mantid::API::TableRow &table) {
    if (input) {
      table << *input;
    } else {
      table << "";
