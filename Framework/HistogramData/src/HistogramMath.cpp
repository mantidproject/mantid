 ...
 * uncertainties remain unchanged. */
Histogram &operator/=(Histogram &histogram, const double factor) { return histogram *= 1.0 / factor; }

/** Scales data in histogram by constant (non-negative) factor.
 *
 * Uncertainties are scaled by the same factor, such that the *relative*
 *uncertainties remain unchanged. */
Histogram operator*(Histogram histogram, const double factor) { return histogram *= factor; }

/** Scales data in histogram by constant (non-negative) factor.
 *
 * Uncertainties are scaled by the same factor, such that the *relative*
 *uncertainties remain unchanged. */
Histogram operator*(const double factor, Histogram histogram) { return histogram *= factor; }
...
Histogram &operator+=(Histogram &histogram, const Histogram &other) {
  ...
  std::transform(histogram.e().cbegin(), histogram.e().cend(), other.e().begin(), histogram.mutableE().begin(),
                 [](const double &lhs, const double &rhs) -> double { return std::sqrt(lhs * lhs + rhs * rhs); });
  return histogram;
}
...
/// uncertainties.
Histogram &operator*=(Histogram &histogram, const Histogram &other) {
  if (histogram.yMode() == Histogram::YMode::Counts && other.yMode() == Histogram::YMode::Counts)
...
/// uncertainties.
Histogram &operator/=(Histogram &histogram, const Histogram &other) {
  if (histogram.yMode() == Histogram::YMode::Frequencies && other.yMode() == Histogram::YMode::Counts)
    throw std::runtime_error("Invalid operation: Cannot divide histogram with YMode::Frequencies by "
