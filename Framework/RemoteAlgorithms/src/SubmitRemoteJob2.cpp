...
void SubmitRemoteJob2::init() {
  ...
  declareProperty("NumNodes", 1, mustBePositive, "The number of compute nodes the job requires", Direction::Input);
  declareProperty("CoresPerNode", 1, mustBePositive, "The number of processes to start on each compute node",
                  Direction::Input);
  ...
  declareProperty("TaskName", std::string(""), "A short name for the job (optional).", Direction::Input);
  ...
  declareProperty("ScriptParams", "", requireValue,
...
void SubmitRemoteJob2::exec() {
  ...
  Mantid::API::IRemoteJobManager_sptr jm = Mantid::API::RemoteJobManagerFactory::Instance().create(comp);
