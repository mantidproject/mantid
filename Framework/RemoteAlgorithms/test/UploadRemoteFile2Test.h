...
public:
  ...
  static UploadRemoteFile2Test *createSuite() { return new UploadRemoteFile2Test(); }
  static void destroySuite(UploadRemoteFile2Test *suite) { delete suite; }

  void test_algorithm() {
    testAlg = Mantid::API::AlgorithmManager::Instance().create("UploadRemoteFile" /*, 2*/);
    TS_ASSERT(testAlg);
  ...
  void test_castAlgorithm() {
    // can create
    std::shared_ptr<UploadRemoteFile2> a;
    TS_ASSERT(a = std::make_shared<UploadRemoteFile2>());

    // can cast to inherited interfaces and base classes
    TS_ASSERT(dynamic_cast<Mantid::RemoteAlgorithms::UploadRemoteFile2 *>(a.get()));
