...
public:
  ...
  static StartRemoteTransaction2Test *createSuite() { return new StartRemoteTransaction2Test(); }
  static void destroySuite(StartRemoteTransaction2Test *suite) { delete suite; }

  void test_algorithm() {
    testAlg = Mantid::API::AlgorithmManager::Instance().create("StartRemoteTransaction", 2);
    TS_ASSERT(testAlg);
  ...
  void test_castAlgorithm() {
    ...
    TS_ASSERT(dynamic_cast<Mantid::RemoteAlgorithms::StartRemoteTransaction2 *>(a.get()));
