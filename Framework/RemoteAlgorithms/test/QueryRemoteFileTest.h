...
public:
  ...
  static QueryRemoteFileTest *createSuite() { return new QueryRemoteFileTest(); }
  static void destroySuite(QueryRemoteFileTest *suite) { delete suite; }

  void test_algorithm() {
    testAlg = Mantid::API::AlgorithmManager::Instance().create("QueryRemoteFile", 1);
    TS_ASSERT(testAlg);
  ...
  void test_castAlgorithm() {
    ...
    TS_ASSERT(dynamic_cast<Mantid::RemoteAlgorithms::QueryRemoteFile *>(a.get()));
