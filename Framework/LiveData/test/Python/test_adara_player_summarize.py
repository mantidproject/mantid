...
class Test_Player_summarize(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.Config = {
            ...
            "summarize": {
                # Default: do not log per-type timestamp ranges
    ...
    def test_per_type_timestamp_ranges_respect_flag(self):
        ...
        self.Config["summarize"]["timestamp_per_type"] = False
        with (
            patch("packet_player.Config", self.Config),
            patch.object(Player, "SUMMARIZE_TIMESTAMP_PER_TYPE", False),
            patch("packet_player._logger") as mock_logger,
        ...
        self.Config["summarize"]["timestamp_per_type"] = True
        with (
            patch("packet_player.Config", self.Config),
            patch.object(Player, "SUMMARIZE_TIMESTAMP_PER_TYPE", True),
