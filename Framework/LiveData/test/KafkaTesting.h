...
public:
  using IKafkaStreamSubscriber_uptr = std::unique_ptr<Mantid::LiveData::IKafkaStreamSubscriber>;
  ...
  MOCK_CONST_METHOD2(subscribe_,
                     IKafkaStreamSubscriber_ptr(std::vector<std::string>, Mantid::LiveData::SubscribeAtOption));
  IKafkaStreamSubscriber_uptr subscribe(std::vector<std::string> s,
                                        Mantid::LiveData::SubscribeAtOption option) const override {
    return std::unique_ptr<Mantid::LiveData::IKafkaStreamSubscriber>(this->subscribe_(s, option));
  }
  MOCK_CONST_METHOD3(subscribe_, IKafkaStreamSubscriber_ptr(std::vector<std::string>, int64_t,
                                                            Mantid::LiveData::SubscribeAtOption));
  IKafkaStreamSubscriber_uptr subscribe(std::vector<std::string> s, int64_t offset,
                                        Mantid::LiveData::SubscribeAtOption option) const override {
    return std::unique_ptr<Mantid::LiveData::IKafkaStreamSubscriber>(this->subscribe_(s, offset, option));
  }
...
// -----------------------------------------------------------------------------
class FakeExceptionThrowingStreamSubscriber : public Mantid::LiveData::IKafkaStreamSubscriber {
public:
  void subscribe() override {}
  void subscribe(int64_t offset) override { UNUSED_ARG(offset) }
  void consumeMessage(std::string *buffer, int64_t &offset, int32_t &partition, std::string &topic) override {
    buffer->clear();
    UNUSED_ARG(offset);
  ...
  std::unordered_map<std::string, std::vector<int64_t>> getOffsetsForTimestamp(int64_t timestamp) override {
    ...
    return {std::pair<std::string, std::vector<int64_t>>("topic_name", {1, 2, 3})};
  }
...
// -----------------------------------------------------------------------------
class FakeEmptyStreamSubscriber : public Mantid::LiveData::IKafkaStreamSubscriber {
public:
  void subscribe() override {}
  void subscribe(int64_t offset) override { UNUSED_ARG(offset) }
  void consumeMessage(std::string *buffer, int64_t &offset, int32_t &partition, std::string &topic) override {
...
void fakeReceiveAnISISEventMessage(std::string *buffer, int32_t nextPeriod) {
  ...
  auto messageFlatbuf = CreateEventMessage(
      builder, builder.CreateString("KafkaTesting"), 0, frameTime, builder.CreateVector(tof),
      builder.CreateVector(spec), FacilityData::ISISData,
      CreateISISData(builder, static_cast<uint32_t>(nextPeriod), RunState::RUNNING, protonCharge).Union());
  FinishEventMessageBuffer(builder, messageFlatbuf);

  // Copy to provided buffer
  buffer->assign(reinterpret_cast<const char *>(builder.GetBufferPointer()), builder.GetSize());
}
...
void fakeReceiveAnEventMessage(std::string *buffer) {
  ...
  auto messageFlatbuf = CreateEventMessage(builder, builder.CreateString("KafkaTesting"), 0, frameTime,
...
void fakeReceiveHistoMessage(std::string *buffer) {
  ...
  auto bin_metadata =
      HistoSchema::CreateDimensionMetaData(builder, 3, builder.CreateString("TOF"), builder.CreateString("TOF"),
                                           HistoSchema::Array::ArrayDouble, xbins.Union());
  auto unit_metadata = HistoSchema::CreateDimensionMetaData(builder, 1, builder.CreateString("Counts"));

  auto dim_metadata = builder.CreateVector(
      std::vector<flatbuffers::Offset<HistoSchema::DimensionMetaData>>{bin_metadata, unit_metadata});

  // Data values are nspectra*nbins
  auto data_values = builder.CreateVector(std::vector<double>{100, 140, 210, 100, 110, 70, 5, 3, 20, 4});
  auto data = HistoSchema::CreateArrayDouble(builder, data_values);

  auto messageFlatBuf = HistoSchema::CreateEventHistogram(builder, builder.CreateString("KafkaTesting"), 0,
                                                          dim_metadata, 0, builder.CreateVector(current_shape), 0,
                                                          HistoSchema::Array::ArrayDouble, data.Union());
...
void fakeReceiveASampleEnvMessage(std::string *buffer) {
  ...
  auto logDataMessage = LogSchema::CreateLogData(builder, builder.CreateString("fake source"), LogSchema::Value::Int,
...
void fakeReceiveARunStartMessage(std::string *buffer, int32_t runNumber, const std::string &startTime,
  ...
  auto startTimestamp = static_cast<uint64_t>(mantidTime.to_time_t() * 1000000000);
  ...
  auto spdet = CreateSpectraDetectorMapping(builder, specVector, detIdsVector, static_cast<int32_t>(m_spec.size()));

  auto runStart =
      CreateRunStart(builder, startTimestamp, 0, builder.CreateString(std::to_string(runNumber)),
                     builder.CreateString(instName), builder.CreateString(nexusStructure), 0, builder.CreateString(""),
                     builder.CreateString(""), builder.CreateString(""), static_cast<uint32_t>(nPeriods), spdet);
  FinishRunStartBuffer(builder, runStart);
  // Copy to provided buffer
  buffer->assign(reinterpret_cast<const char *>(builder.GetBufferPointer()), builder.GetSize());
}

void fakeReceiveARunStopMessage(std::string *buffer, const std::string &stopTime) {
  // Convert date to time_t
  auto mantidTime = Mantid::Types::Core::DateAndTime(stopTime);
  auto stopTimestamp = static_cast<uint64_t>(mantidTime.to_time_t() * 1000000000);
...
// -----------------------------------------------------------------------------
class FakeISISEventSubscriber : public Mantid::LiveData::IKafkaStreamSubscriber {
public:
  explicit FakeISISEventSubscriber(int32_t nperiods) : m_nperiods(nperiods), m_nextPeriod(0) {}
  void subscribe() override {}
  void subscribe(int64_t offset) override { UNUSED_ARG(offset) }
  void consumeMessage(std::string *message, int64_t &offset, int32_t &partition, std::string &topic) override {
    assert(message);
...
public:
  ...
  void consumeMessage(std::string *message, int64_t &offset, int32_t &partition, std::string &topic) override {
    ...
    switch (m_nextOffset) {
    case 0:
      fakeReceiveARunStartMessage(message, 1000, "2016-08-31T12:07:42", "HRPDTEST", 1);
      break;
    case 2:
...
// -----------------------------------------------------------------------------
class FakeSampleEnvironmentSubscriber : public Mantid::LiveData::IKafkaStreamSubscriber {
public:
  void subscribe() override {}
  void subscribe(int64_t offset) override { UNUSED_ARG(offset) }
  void consumeMessage(std::string *message, int64_t &offset, int32_t &partition, std::string &topic) override {
...
// -----------------------------------------------------------------------------
class FakeRunInfoStreamSubscriber : public Mantid::LiveData::IKafkaStreamSubscriber {
public:
  explicit FakeRunInfoStreamSubscriber(int32_t nperiods) : m_nperiods(nperiods) {}
  void subscribe() override {}
  void subscribe(int64_t offset) override { UNUSED_ARG(offset) }
  void consumeMessage(std::string *buffer, int64_t &offset, int32_t &partition, std::string &topic) override {
    assert(buffer);

    fakeReceiveARunStartMessage(buffer, m_runNumber, m_startTime, m_instName, m_nperiods);
...
// -----------------------------------------------------------------------------
class FakeRunInfoStreamSubscriberVaryingNPeriods : public Mantid::LiveData::IKafkaStreamSubscriber {
public:
  void subscribe() override {}
  void subscribe(int64_t offset) override { UNUSED_ARG(offset) }
  void consumeMessage(std::string *buffer, int64_t &offset, int32_t &partition, std::string &topic) override {
...
// -----------------------------------------------------------------------------
class FakeVariablePeriodSubscriber : public Mantid::LiveData::IKafkaStreamSubscriber {
public:
  ...
  void consumeMessage(std::string *buffer, int64_t &offset, int32_t &partition, std::string &topic) override {
    ...
    switch (m_nextOffset) {
    case 0:
      fakeReceiveARunStartMessage(buffer, 1000, m_startTime, m_instName, m_nperiods);
      break;
    case 2:
...
// -----------------------------------------------------------------------------
class FakeDataStreamSubscriber : public Mantid::LiveData::IKafkaStreamSubscriber {
public:
  explicit FakeDataStreamSubscriber(int64_t stopOffset) : m_stopOffset(stopOffset) {}
  void subscribe() override {}
  void subscribe(int64_t offset) override { UNUSED_ARG(offset) }
  void consumeMessage(std::string *buffer, int64_t &offset, int32_t &partition, std::string &topic) override {
    assert(buffer);
  ...
  std::unordered_map<std::string, std::vector<int64_t>> getOffsetsForTimestamp(int64_t timestamp) override {
    UNUSED_ARG(timestamp);
    return {std::pair<std::string, std::vector<int64_t>>(m_topicName, {m_stopOffset})};
  }

  std::unordered_map<std::string, std::vector<int64_t>> getCurrentOffsets() override {
    return {std::pair<std::string, std::vector<int64_t>>(m_topicName, {m_nextOffset - 1})};
  }
