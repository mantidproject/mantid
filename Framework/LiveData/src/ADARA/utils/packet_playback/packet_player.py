...
def get_log_file_path_for_PID(pid: int) -> Path | None:
    """Return the log file path for a given PID, or None if unusable."""
    _LOG_FILE_TEMPLATE = Config["logging"]["filename"]
    if not _LOG_FILE_TEMPLATE:
        return None
    path = Path(_LOG_FILE_TEMPLATE.format(PID=pid))
    return path if path.parent.exists() else None


def _configure_logger_for_pid(pid: int) -> None:
    """Configure this module's logger for the given PID."""
    # Remove any existing handlers (e.g. from parent process)
...
def init_process_logger() -> None:
    ...
    _configure_logger_for_pid(os.getpid())


# Initial configuration for the server process at import time
init_process_logger()
...
class _PacketFileSummary:
    ...
    TIMESTAMP_RESOLUTION: ClassVar[float] = Config["summarize"]["timestamp_resolution"]
...
class Player:
    # ------ `Config` values which are never overridden from the commandline. ------
    ...
    SUMMARIZE_TIMESTAMP_PER_TYPE = Config["summarize"]["timestamp_per_type"]
    ...
    def __init__(self, *, source_address=None, packet_ordering_scheme=None):
        ...
        self._file_output_mode = Player.FileOutputMode(Config["record"]["file_output"])
        self._buffer_MB = Config["server"]["buffer_MB"]
        self._packet_ordering_scheme = packet_ordering_scheme if packet_ordering_scheme else Config["playback"]["packet_ordering"]
