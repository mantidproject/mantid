...
# Duplicate the server's console-log messages to the `Player/packet_player` log file for this PID.
player_logger = logging.getLogger("packet_player")  # same name used for packet_player._logger
for h in player_logger.handlers:
    if h not in console_logger.handlers:
...
class SessionServer:
    ...
    def parse_args() -> argparse.Namespace:
        ...
        # IMPORTANT: the default config-file location is overridden using the environment, NOT here:
        ...
        parser = argparse.ArgumentParser(
            ...
            usage="%(prog)s [-h] [-r] [-s SOURCE_ADDRESS] [-a SERVER_ADDRESS] [-d] [-z] [-f FILES] [glob]",
        )
...
def main():
    ...
    if getattr(args, "summarize", False):
        ...
        if sessions is None:
            console_logger.warning("No sessions/files found to summarize.")
        else:
            console_logger.info("Summarizing content of ADARA-packet files ...")
            for idx, session_files in enumerate(sessions, start=1):
                console_logger.info(f"Session {idx:04d}:")
                player.summarize(session_files)

        console_logger.info(f"Summary available at:\n    '{get_log_file_path_for_PID(os.getpid())}'.")
