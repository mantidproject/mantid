...
void saveInstrument(const Geometry::ComponentInfo &compInfo, const Geometry::DetectorInfo &detInfo,
  ...
  H5::H5File file;
  if (append) {
    file = H5::H5File(fullPath, H5F_ACC_RDWR, NeXus::H5Util::defaultFileAcc()); // open file
    parentGroup = file.openGroup(parentGroupName);
  } else {
    file = H5::H5File(fullPath, H5F_ACC_TRUNC, NeXus::H5Util::defaultFileAcc()); // open file
...
void saveInstrument(const Mantid::API::MatrixWorkspace &ws, const std::string &filePath,
  ...
  if (Poco::File(filePath).exists() && append) {
    file = H5::H5File(filePath, H5F_ACC_RDWR, NeXus::H5Util::defaultFileAcc()); // open existing file
    ...
    file = H5::H5File(filePath, H5F_ACC_TRUNC,
                      NeXus::H5Util::defaultFileAcc()); // create a new file (or overwrite an existing one)
...
void saveInstrument(const Mantid::API::MatrixWorkspace &ws, const std::string &filePath,
  ...
  H5::H5File file = H5::H5File(filePath, append ? H5F_ACC_RDWR : H5F_ACC_TRUNC, NeXus::H5Util::defaultFileAcc());
  H5::Group rootGroup = file.openGroup("/");
