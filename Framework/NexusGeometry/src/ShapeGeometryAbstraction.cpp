...
objectHolder ShapeGeometryAbstraction::createCylinder(Eigen::Matrix<double, 3, 3> &pointsDef)
{
    ...
    for (int i = 0; i < 3; ++i)
    {
        ...
        if (max == centre[i]) { max += radius; }
        if (min == centre[i]) { min -= radius; }
...
///Finalise shape
objectHolder ShapeGeometryAbstraction::createShape(std::map<int, surfaceHolder> &surfaces, std::string &algebra, std::vector<double> &boundingBox){
    ...
    shape->defineBoundingBox(boundingBox[0], boundingBox[2], boundingBox[4], boundingBox[1], boundingBox[3], boundingBox[5]);
    return shape;
}
