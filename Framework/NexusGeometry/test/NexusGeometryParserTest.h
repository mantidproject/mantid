...
public:
  ...
  std::pair<std::unique_ptr<Geometry::ComponentInfo>, std::unique_ptr<Geometry::DetectorInfo>>  extractBeamline(){
    auto instrument = extractInstrument();
    Geometry::ParameterMap pmap;
    return {std::move(std::get<0>(instrument->makeBeamline(pmap))),std::move(std::get<1>(instrument->makeBeamline(pmap)))};
  }
  ...
  void test_basic_instrument_information(){
      ...
      TSM_ASSERT_EQUALS("Detectors + 1 monitor", detectorInfo->size(), 128 * 2 + 1); // TODO, currently NOT counting monitor. Needs fixing.
      TSM_ASSERT_EQUALS("Detectors + 3 non-detector components", componentInfo->size(), detectorInfo->size()  + 3); // TODO, currently NOT counting source + sample + root!
  }

  void test_simple_translation() {
    auto detectorInfo = extractDetectorInfo();
    // First pixel in bank 2
    auto det0Postion = Kernel::toVector3d(detectorInfo->position(detectorInfo->indexOf(1100000)));
    Eigen::Vector3d unitVector(0, 0, 1); // Fixed in file location vector attribute
    const double magnitude = 4.0; // Fixed in file location value
    Eigen::Vector3d offset(-0.498, -0.200, 0.00); // All offsets for pixel x, and y specified separately, z defaults to 0
  ...
  void test_complex_translation(){
    ...
    Eigen::Vector3d unitVectorTranslation(0.2651564830210424, 0.0, 0.9642053928037905);// Fixed in file location vector attribute
    ...
    // Rotation of bank
    affine *= Eigen::Quaterniond(Eigen::AngleAxisd(rotation * M_PI/180, rotationAxis));
