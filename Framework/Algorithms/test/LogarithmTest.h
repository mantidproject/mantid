...
public:
  ...
  void testExec1D(void) {
    ...
    MatrixWorkspace_sptr work_in1 = WorkspaceCreationHelper::create1DWorkspaceFib(sizex, true);
    ...
    TS_ASSERT_THROWS_NOTHING(alg.setPropertyValue("InputWorkspace", "test_inLn");
                             alg.setPropertyValue("OutputWorkspace", "test_inLn"); alg.setPropertyValue("Filler", "10");
    ...
    TS_ASSERT_THROWS_NOTHING(AnalysisDataService::Instance().remove("test_outLn");
  ...
  void testExec2D(void) {
    ...
    alg.initialize();
    TS_ASSERT_THROWS_NOTHING(alg.setPropertyValue("InputWorkspace", "test_inLn2");
  ...
  void testEvents() {
    ...
    TS_ASSERT_THROWS_NOTHING(
        evout = std::dynamic_pointer_cast<EventWorkspace>(AnalysisDataService::Instance().retrieve("test_ev_log_out")));

    TS_ASSERT(!evout); // should not be an event workspace

    MatrixWorkspace_sptr histo_out;
    TS_ASSERT_THROWS_NOTHING(histo_out =
                                 AnalysisDataService::Instance().retrieveWS<MatrixWorkspace>("test_ev_log_out"));
    TS_ASSERT(histo_out); // this should be a 2d workspace

    TS_ASSERT_DELTA(histo_out->y(0)[0], 123, 1e-10);
    for (size_t i = 1; i < 5; ++i) {
      TS_ASSERT_DELTA(histo_out->y(i)[0], std::log(static_cast<double>(i)), 1e-10);
    }
