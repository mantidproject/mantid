...
public:
  ...
  static DetectorEfficiencyCorUserTest *createSuite() { return new DetectorEfficiencyCorUserTest(); }
  static void destroySuite(DetectorEfficiencyCorUserTest *suite) { delete suite; }

  // constructor
  DetectorEfficiencyCorUserTest() : m_Efs(m_numBins), m_inWSName("input_workspace"), m_outWSName("output_workspace") {
...
private:
  ...
  static double efficiencyBank1(const double e) {
    return std::exp(-1.0 / std::sqrt(e)) * (1.0 - std::exp(-1.0 / std::sqrt(e)));
  }

  static double efficiencyBank2(const double e) {
    return std::exp(-2.0 / std::sqrt(e)) * (1.0 - std::exp(-2.0 / std::sqrt(e)));
  }
  ...
  void createInputWorkSpace() const {
    ...
    DataObjects::Workspace2D_sptr dataws =
        WorkspaceCreationHelper::create2DWorkspaceWithRectangularInstrument(numBanks, numPixels, m_numBins);
    ...
    dataws->instrumentParameters().addString(bank.get(), "formula_eff", "exp(-1/sqrt(e))*(1-exp(-1/sqrt(e)))");
