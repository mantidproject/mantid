...
public:
  ...
  void testNonNumericExponent() {
    Power power;
    power.initialize();
    TSM_ASSERT_THROWS("Exponent cannot be non-numeric", power.setPropertyValue("Exponent", "x"),
  ...
  void testEvents() {
    ...
    TS_ASSERT_THROWS_NOTHING(
        evout = std::dynamic_pointer_cast<EventWorkspace>(AnalysisDataService::Instance().retrieve("test_ev_pow_out")));

    TS_ASSERT(!evout); // should not be an event workspace

    MatrixWorkspace_sptr histo_out;
    TS_ASSERT_THROWS_NOTHING(histo_out =
                                 AnalysisDataService::Instance().retrieveWS<MatrixWorkspace>("test_ev_pow_out"));
