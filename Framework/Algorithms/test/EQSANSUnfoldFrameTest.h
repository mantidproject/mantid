...
void testWidth(){
  TS_ASSERT_EQUALS(EQSANSWBand(0.0, 1.5).width(), 1.5);
}
...
// This pair of boilerplate methods prevent the suite being created statically
// This means the constructor isn't called when running other tests
static EQSANSTransWBandsTest *createSuite() { return new EQSANSTransWBandsTest(); }
...
public:
    ...
    void testConstructor() {
        TS_ASSERT_THROWS_NOTHING(EQSANSDiskChopper());
    }

    void testPeriod(){
        TS_ASSERT_DELTA(m_d.period(), 16666.6, 1.0);
    }
    ...
    void testOpeningPhase(){
        TS_ASSERT_DELTA(m_d.openingPhase(), -413.3, 1.0);
    }

    void testClosingPhase(){
        TS_ASSERT_DELTA(m_d.closingPhase(), 1253.3, 1.0);
    }
...
// This pair of boilerplate methods prevent the suite being created statically
// This means the constructor isn't called when running other tests
static EQSANSUnfoldFrameTest *createSuite() { return new EQSANSUnfoldFrameTest(); }
...
// constructor
EQSANSUnfoldFrameTest(){
    createInputWorkspace();
}

void testConstructor() {
    TS_ASSERT_THROWS_NOTHING(EQSANSUnfoldFrame());
}
