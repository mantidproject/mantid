    ...
    for (size_t col = 0; col < 9; ++col) {
      ...
      for (size_t row = 0; row < nSpectra; ++row) {
        if (colName == "WorkspaceIndex") {
          ...
          const double peakCentre = Kernel::UnitConversion::run("Energy", "TOF", Ei, l1, l2, 0.0, Kernel::DeltaEMode::Direct, Ei);
          ...
          TS_ASSERT_EQUALS(outputWS->getRef<std::string>(colName, row), "success")
        } else {
...
private:
  bool hasCorrectColumns(const std::vector<std::string> &names) {
    ...
    const std::array<std::string, 9> columnNames = {
      "WorkspaceIndex", "PeakCentre", "PeakCentreError", "Sigma", "SigmaError", "Height", "HeightError", "chiSq", "FitStatus"
    };
