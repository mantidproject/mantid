...
private:
  MatrixWorkspace_sptr createWorkspace() {
    ...
    const std::vector<double> x = {1.1, 2.5, 3.2, 4.5, 6.7, 8.9, 10.3, 12.4, 13.9, 14.1, 15.3, 16.8};
    const std::vector<double> y = {7, 23, 54, 34, 23, 64, 34, 23, 58, 63, 34, 25};
    const std::vector<double> e = {3.2, 2.1, 8.4, 3.5, 6.3, 4.7, 4.9, 3.6, 4.1, 6.7, 5.1, 3.2};
    const std::vector<double> dx = {0.1, 0.2, 0.4, 0.7, 0.9, 1.3, 1.5, 1.7, 1.9, 1.2, 4.5, 2.3};
...
public:
  static MaskBinsIfTestPerformance *createSuite() { return new MaskBinsIfTestPerformance(); }
  ...
  void setUp() override {
    ...
    creator.execute();
    Mantid::API::MatrixWorkspace_sptr ws = creator.getProperty("OutputWorkspace");
    m_alg.initialize();
    m_alg.setChild(true);
