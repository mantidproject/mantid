...
public:
  ...
  void testEvents() {
    ...
    TS_ASSERT(alg.isInitialized());
    TS_ASSERT_THROWS_NOTHING(
        alg.setPropertyValue("InputWorkspace", "test_ev_exp"));
    TS_ASSERT_THROWS_NOTHING(
        alg.setPropertyValue("OutputWorkspace", "test_ev_out"));
    alg.execute();
    ...
    for (size_t i = 0; i < 5; ++i) {
      TS_ASSERT_DELTA(histo_out->readY(i)[0], exp(static_cast<double>(i)),
                      1e-10);
    }
...
private:
  ...
  void checkData(MatrixWorkspace_sptr work_in1,
    ...
    for (size_t i = 0; i < work_out1->size(); i++) {
      ...
      double sig3 = work_out1->dataY(
          i / work_in1->blocksize())[i % work_in1->blocksize()];
      TS_ASSERT_DELTA(
          work_in1->dataX(i / work_in1->blocksize())[i % work_in1->blocksize()],
          work_out1->dataX(i /
                           work_in1->blocksize())[i % work_in1->blocksize()],
          1.e-10);
