...
public:
  void testInit() {
    ...
    TS_ASSERT_THROWS(alg.setPropertyValue("InputWorkspace", "test_in21"), const std::invalid_argument &)
  ...
  void testExec1D() {
    ...
    alg.initialize();
    TS_ASSERT_THROWS_NOTHING(alg.setPropertyValue("InputWorkspace", "test_in11");
  ...
  void testEvents() {
    ...
    TS_ASSERT_THROWS_NOTHING(
        evout = std::dynamic_pointer_cast<EventWorkspace>(AnalysisDataService::Instance().retrieve("test_ev_out")));
