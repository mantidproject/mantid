...
public:
  ...
  void test_flat_plate_sample_vs_analytic_result() {
    ...
    Mantid::API::MatrixWorkspace_sptr inputWorkspace = WorkspaceCreationHelper::create2DWorkspaceBinned(46, 1, 0.5);
    inputWorkspace->getAxis(0)->unit() = UnitFactory::Instance().create("Wavelength");
    Mantid::API::MatrixWorkspace_sptr SofQWorkspace = WorkspaceCreationHelper::create2DWorkspace(1, 1);
    ...
    double totalXSection = mat.totalScatterXSection() + mat.absorbXSection(WAVELENGTH);
    ...
    double secangle = 1 / cos(inputWorkspace->spectrumInfo().twoTheta(SPECTRUMINDEXTOTEST));
