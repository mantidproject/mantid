...
public:
  ...
  makeTinyInputWorkspace(const bool monitorNormalization = true) {
    // Prepare workspace
    //-----------------
    ...
    detData.access().resize(1, 10); // Filled with 10s
    monData.access().resize(1, 5);  // Filled with 5s
    ...
    conv->setProperty("OutputWorkspace", "dummy");
    conv->setProperty("Target", "TOF");
    conv->execute();
    TS_ASSERT(conv->isExecuted());
    MatrixWorkspace_sptr tofWS = conv->getProperty("OutputWorkspace");
    return tofWS;
  }
  ...
  void test_normalize_by_detmon() {
    ...
    alg->setChild(true);
    TS_ASSERT_THROWS_NOTHING(alg->initialize());
    TS_ASSERT_THROWS_NOTHING(alg->setProperty("InputWorkspace", tofWS));
    TS_ASSERT_THROWS_NOTHING(
  ...
  void test_i0_monitor_index_not_required() {
    ...
    auto tofWS = makeTinyInputWorkspace(false /*no monitor index*/);
