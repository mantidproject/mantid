...
public:
  ...
  void Ntest_singlePeakMultiSpectra() {
    ...
    TS_ASSERT_THROWS_NOTHING(fitpeaks.setProperty("InputWorkspace", m_inputWorkspaceName));
    TS_ASSERT_THROWS_NOTHING(fitpeaks.setProperty("StartWorkspaceIndex", 19990));
    ...
    // about the parameters
    API::MatrixWorkspace_sptr peak_params_ws =
        std::dynamic_pointer_cast<API::MatrixWorkspace>(AnalysisDataService::Instance().retrieve("PeakParametersWS3"));
    TS_ASSERT(peak_params_ws);
  ...
  void Ntest_singlePeakMultiSpectraPseudoVoigt() {
    ...
    TS_ASSERT_THROWS_NOTHING(fitpeaks.setProperty("StartWorkspaceIndex", 19990));
    TS_ASSERT_THROWS_NOTHING(fitpeaks.setProperty("StopWorkspaceIndex", 20000));
    TS_ASSERT_THROWS_NOTHING(fitpeaks.setProperty("PeakFunction", "PseudoVoigt"));
    TS_ASSERT_THROWS_NOTHING(fitpeaks.setProperty("PeakCenters", "1.0758"));
    TS_ASSERT_THROWS_NOTHING(fitpeaks.setProperty("FitWindowLeftBoundary", "1.05"));
    TS_ASSERT_THROWS_NOTHING(fitpeaks.setProperty("FitWindowRightBoundary", "1.15"));
    TS_ASSERT_THROWS_NOTHING(fitpeaks.setProperty("PeakRanges", "0.02"));
  ...
  void Ntest_SingleSpectrum3Peaks() {
    ...
    TS_ASSERT_THROWS_NOTHING(fitpeaks.setProperty("PeakCenters", "1.0758, 0.89198"));
    TS_ASSERT_THROWS_NOTHING(fitpeaks.setProperty("FitWindowLeftBoundary", "1.05, 0.87"));
    TS_ASSERT_THROWS_NOTHING(fitpeaks.setProperty("FitWindowRightBoundary", "1.15, 0.92"));
    ...
    TS_ASSERT(API::AnalysisDataService::Instance().doesExist("PeakPositionsWS2"));

    TS_ASSERT(API::AnalysisDataService::Instance().doesExist("FittedPeaksWS2"));
    API::MatrixWorkspace_sptr fitted_data_ws = std::dynamic_pointer_cast<API::MatrixWorkspace>(
  ...
  void test_multiple_peak_profiles() {
    ...
    TS_ASSERT_THROWS_NOTHING(fitpeaks.setProperty("PeakCenters", "0.6867, 0.728299, 0.89198, 1.0758"));
    TS_ASSERT_THROWS_NOTHING(
        fitpeaks.setProperty("FitWindowBoundaryList", "0.67, 0.709, 0.71, 0.76, 0.87, 0.92, 1.05, 1.15"));
    TS_ASSERT_THROWS_NOTHING(fitpeaks.setProperty("PeakParameterNames", peakparnames));
    TS_ASSERT_THROWS_NOTHING(fitpeaks.setProperty("PeakParameterValues", peakparvalues));
    TS_ASSERT_THROWS_NOTHING(fitpeaks.setProperty("FitFromRight", true));
    TS_ASSERT_THROWS_NOTHING(fitpeaks.setProperty("HighBackground", false));
    ...
    API::MatrixWorkspace_sptr peak_pos_ws = CheckAndRetrieveMatrixWorkspace("PeakPositionsWS2", &peak_pos_ws_exist);
    API::MatrixWorkspace_sptr fitted_ws = CheckAndRetrieveMatrixWorkspace("FittedPeaksWS2", &fitted_ws_exist);
    API::ITableWorkspace_sptr peak_param_ws = CheckAndRetrieveTableWorkspace("PeakParametersWS2", &peak_param_ws_exist);
    ...
    if (fitted_ws_exist) {
      ...
      TS_ASSERT_EQUALS(fitted_ws->getNumberHistograms(), input_ws->getNumberHistograms());
    }
  ...
  std::string genPeakCenterWorkspace(const std::vector<int> &peak_index_vec, const std::string &workspace_name) {
    ...
    std::cout << "Center workspace has " << center_ws->readX(0).size() << " bins"
              << "\n";
