...
public:
  ...
  void test_visWS_noNormalization() {
    ...
    Mantid::HistogramData::HistogramX const &X = testVisWS->x(0);
    Mantid::HistogramData::HistogramY const &Y = testVisWS->y(0);
    TS_ASSERT_EQUALS(X.size(), 201);
    TS_ASSERT_EQUALS(Y.size(), 200);
    ...
    for (size_t i = 0; i < testVisWS->getNumberHistograms(); ++i) {
      Mantid::HistogramData::HistogramY const &Y = testVisWS->y(i);
  ...
  void test_visWS_Normalized() {
    ...
    for (size_t i = 1; i < testVisWS->getNumberHistograms() - 1; ++i) {
      ...
      // Mantid::HistogramData::HistogramY const &Y = testVisWS->y(i); // -- better for debugging as
