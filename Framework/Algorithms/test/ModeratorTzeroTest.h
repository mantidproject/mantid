...
namespace {
void AddToIndirectInstrument(MatrixWorkspace_sptr &testWS, const bool &add_t0_formula = false,
                             const bool &add_Efixed = false) {

  if (add_t0_formula)
    testWS->instrumentParameters().addString(testWS->getInstrument()->getComponentID(), "t0_formula",
...
public:
  ...
  void TestExecEventsIndirect() {
    ...
    const bool add_Efixed = true;
    const bool add_t0_formula = true;
    MatrixWorkspace_sptr mtestWS = std::dynamic_pointer_cast<MatrixWorkspace>(testWS);
...
public:
  // This pair of boilerplate methods prevent the suite being created statically
  // This means the constructor isn't called when running other tests
  static ModeratorTzeroTestPerformance *createSuite() { return new ModeratorTzeroTestPerformance(); }
  ...
  ModeratorTzeroTestPerformance() {
    ...
    output = WorkspaceCreationHelper::create2DWorkspaceWithFullInstrument(10000, 1000, true);
    output->getAxis(0)->unit() = Mantid::Kernel::UnitFactory::Instance().create("TOF");
    ...
    outputEvent = WorkspaceCreationHelper::createEventWorkspaceWithFullInstrument(10, 100, true);
    outputEvent->getAxis(0)->unit() = Mantid::Kernel::UnitFactory::Instance().create("TOF");
  ...
  void testExecDirect() {
    input->instrumentParameters().addString(input->getInstrument()->getComponentID(), "t0_formula",
                                            "101.9*incidentEnergy^(-0.41)*exp(-incidentEnergy/282.0)");
  ...
  void testExecDirectEvent() {
    inputEvent->instrumentParameters().addString(inputEvent->getInstrument()->getComponentID(), "t0_formula",
                                                 "101.9*incidentEnergy^(-0.41)*exp(-incidentEnergy/282.0)");
