...
IAlgorithm_sptr setUpAlgorithm(const MatrixWorkspace_sptr &inputWorkspace, const MatrixWorkspace_sptr &destWorkspace,
                               int const &specMin, int const &specMax, double const &xMin, double const &xMax,
                               int const &yInsertionIndex, int const &xInsertionIndex, std::string const &outputName) {
...
IAlgorithm_sptr setUpAlgorithm(std::string const &inputName, std::string const &destName, int const &specMin,
                               int const &specMax, double const &xMin, double const &xMax, int const &yInsertionIndex,
                               int const &xInsertionIndex, std::string const &outputName) {
  return setUpAlgorithm(getADSMatrixWorkspace(inputName), getADSMatrixWorkspace(destName), specMin, specMax, xMin, xMax,
                        yInsertionIndex, xInsertionIndex, outputName);
}

void populateSpectrum(const MatrixWorkspace_sptr &workspace, std::size_t const &spectrum,
                      std::vector<double> const &yData, std::vector<double> const &xData,
...
void populateWorkspace(const MatrixWorkspace_sptr &workspace, std::vector<double> const &yData,
                       std::vector<double> const &xData, std::vector<double> const &eData) {
  for (auto index = 0u; index < workspace->getNumberHistograms(); ++index)
    populateSpectrum(workspace, index, yData, xData, eData);
}

std::vector<double> constructHistogramData(Mantid::MantidVec::const_iterator fromIterator,
...
void populateOutputWorkspace(const MatrixWorkspace_sptr &workspace, std::vector<double> const &yData,
                             std::vector<double> const &eData) {
  ...
  for (auto i = 0u; i < workspace->getNumberHistograms(); ++i) {
    ...
    populateSpectrum(workspace, i, constructHistogramData(yData.begin() + startIndex, yData.begin() + endIndex), xData,
                     constructHistogramData(eData.begin() + startIndex, eData.begin() + endIndex));
  }
}

ITableWorkspace_sptr compareWorkspaces(const MatrixWorkspace_sptr &workspace1, const MatrixWorkspace_sptr &workspace2,
...
public:
  ...
  void test_that_the_algorithm_does_not_throw_when_given_valid_properties() {
    auto algorithm = setUpAlgorithm(INPUT_NAME, DESTINATION_NAME, 0, 3, 2.1, 2.4, 0, 0, OUTPUT_NAME);

    TS_ASSERT_THROWS_NOTHING(algorithm->execute());
  }

  void test_that_the_algorithm_produces_an_output_workspace_with_the_correct_data() {
    auto algorithm = setUpAlgorithm(INPUT_NAME, DESTINATION_NAME, 0, 3, 2.1, 2.41, 0, 0, OUTPUT_NAME);
    ...
    populateOutputWorkspace(expectedOutput, {1.1, 1.2,  1.3, 1.4, 29.0, 1.1, 1.2,  1.3,  1.4,  29.0, 1.1,  1.2, 1.3,
  ...
  test_that_the_algorithm_produces_an_output_workspace_with_the_correct_data_when_the_start_indices_are_not_zero() {
    auto algorithm = setUpAlgorithm(INPUT_NAME, DESTINATION_NAME, 2, 3, 2.21, 2.41, 2, 2, OUTPUT_NAME);
    ...
    populateOutputWorkspace(expectedOutput,
                            {25.0, 26.0, 27.0, 28.0, 29.0, 25.0, 26.0, 27.0, 28.0, 29.0, 25.0, 26.0, 1.2,
                             1.3,  1.4,  25.0, 26.0, 1.2,  1.3,  1.4,  25.0, 26.0, 27.0, 28.0, 29.0},
  ...
  test_that_the_algorithm_produces_an_output_workspace_with_the_correct_data_when_transfering_a_block_which_is_a_single_line() {
    auto algorithm = setUpAlgorithm(INPUT_NAME, DESTINATION_NAME, 2, 2, 2.11, 2.51, 0, 0, OUTPUT_NAME);
    ...
    auto const expectedOutput = create2DWorkspace(5, 5);
    populateOutputWorkspace(expectedOutput,
                            {1.1,  1.2,  1.3,  1.4,  1.5,  25.0, 26.0, 27.0, 28.0, 29.0, 25.0, 26.0, 27.0,
                             28.0, 29.0, 25.0, 26.0, 27.0, 28.0, 29.0, 25.0, 26.0, 27.0, 28.0, 29.0},
  ...
  test_that_the_algorithm_changes_the_input_workspace_with_the_correct_data_when_the_output_and_destination_workspaces_are_the_same() {
    auto algorithm = setUpAlgorithm(INPUT_NAME, DESTINATION_NAME, 0, 3, 2.11, 2.41, 0, 0, DESTINATION_NAME);
  ...
  void test_that_the_algorithm_throws_when_provided_a_StartWorkspaceIndex_which_is_larger_than_the_EndWorkspaceIndex() {
    auto algorithm = setUpAlgorithm(INPUT_NAME, DESTINATION_NAME, 2, 1, 2.1, 2.4, 0, 0, OUTPUT_NAME);
  ...
  test_that_the_algorithm_throws_when_provided_an_EndWorkspaceIndex_which_is_larger_than_the_number_of_histograms_in_the_input_workspace() {
    auto algorithm = setUpAlgorithm(INPUT_NAME, DESTINATION_NAME, 0, 5, 2.1, 2.4, 0, 0, OUTPUT_NAME);
    TS_ASSERT_THROWS(algorithm->execute(), const std::runtime_error &);
  }

  void test_that_the_algorithm_throws_when_provided_a_xMin_which_comes_later_on_than_larger_than_xMax() {
    auto algorithm = setUpAlgorithm(INPUT_NAME, DESTINATION_NAME, 0, 3, 2.4, 2.1, 0, 0, OUTPUT_NAME);
    TS_ASSERT_THROWS(algorithm->execute(), const std::runtime_error &);
  }
  ...
  test_that_the_algorithm_throws_when_provided_a_block_of_data_which_will_not_fit_in_the_destination_workspace_in_the_y_direction() {
    auto algorithm = setUpAlgorithm(INPUT_NAME, DESTINATION_NAME, 0, 3, 2.1, 2.4, 4, 0, OUTPUT_NAME);
    TS_ASSERT_THROWS(algorithm->execute(), const std::runtime_error &);
  }
  ...
  test_that_the_algorithm_throws_when_provided_a_block_of_data_which_will_not_fit_in_the_destination_workspace_in_the_x_direction() {
    auto algorithm = setUpAlgorithm(INPUT_NAME, DESTINATION_NAME, 0, 3, 2.1, 2.4, 0, 4, OUTPUT_NAME);
    TS_ASSERT_THROWS(algorithm->execute(), const std::runtime_error &);
  }

private:
  void setUpWorkspaces(std::string const &inputName, std::string const &destName, int inputNumberOfSpectra = 5,
                       int destNumberOfSpectra = 5, int inputNumberOfBins = 5, int destNumberOfBins = 5,
    ...
    auto const inputWorkspace = create2DWorkspace(inputNumberOfSpectra, inputNumberOfBins);
    auto const destWorkspace = create2DWorkspace(destNumberOfSpectra, destNumberOfBins);
