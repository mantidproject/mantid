...
namespace Mantid {
  namespace Algorithms {
    ...
    */
    void XrayAbsorptionCorrection::init() {
      ...
      declareProperty(std::make_unique<WorkspaceProperty<>>(
        "MuonImplantationProfile", "", Direction::Input, wsValidator),
    ...
    XrayAbsorptionCorrection::validateInputs() {
     ...
     * @param muonIntensity A MantidVec which contains the intensity of
     * muons as a function  of depth
     * @return A vector of doubles which contains the normalised muon
     * intensity
     */
    Kernel::V3D XrayAbsorptionCorrection::calculateDetectorPos(
    double detectorAngle,double detectorDistance) {
   ...
   * @param muonIntensity A MantidVec which contains the intensity of muons as a function  of depth
   ...
   * @param detectorDistance a double representing the distance from the sample to detector
  ...
  std::vector<Kernel::V3D> XrayAbsorptionCorrection::calculateMuonPos(
    ...
    if (muonPath.count() == 0) {
      throw std::runtime_error("No valid solution, check shape parameters, Muon depth profile and detector distance");
      }
  ...
  void XrayAbsorptionCorrection::exec() {
    ...
    MatrixWorkspace_sptr muonProfile =
      getProperty("MuonImplantationProfile");
    ...
    for (size_t j = 0; j < inputWS->getNumberHistograms(); j++) {
      std::vector<double> energyVector = calculateMidPointOfBin(inputWS->readX(j));
      ...
      for (size_t i = 0; i < energyVector.size();i++) {
        ...
        for (size_t k = 0; k < normalisedMuonIntensity.size(); k++) {
          ...
          double factor{1.0};
          if (xrayPath.count() == 0) {
            throw std::runtime_error("No valid solution, check shape parameters , detector disatance and angle");
            }
          for (auto &link : xrayPath) {
            double distInObject = link.distInsideObject;
            factor = factor * link.object->material().xRayAttenuation(distInObject, energyVector[i]);
            }
