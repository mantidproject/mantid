...
void BinaryOperateMasks::init() {
  ...
  declareProperty(
      std::make_unique<WorkspaceProperty<DataObjects::MaskWorkspace>>("InputWorkspace1", "", Direction::Input),
  ...
  declareProperty("OperationType", "AND", std::make_shared<StringListValidator>(operators),
                  "Operator for Workspace1 and Workspace2");
  declareProperty(
      std::make_unique<WorkspaceProperty<DataObjects::MaskWorkspace>>("OutputWorkspace", "", Direction::Output),
...
void BinaryOperateMasks::exec() {
  ...
  if (op == "NOT") {
    ...
    unsigned int binop;
    if (op == "AND") {
      binop = static_cast<unsigned int>(Mantid::DataObjects::BinaryOperator::AND);
    } else if (op == "OR") {
      binop = static_cast<unsigned int>(Mantid::DataObjects::BinaryOperator::OR);
    } else if (op == "XOR") {
      binop = static_cast<unsigned int>(Mantid::DataObjects::BinaryOperator::XOR);
    } else {
      binop = 1000;
    }
