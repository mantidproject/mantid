...
std::map<std::string, std::string> EstimatePeakIntensities::validateInputs() {
  ...
  const size_t nx = windowWS->x(0).size();
  if (nx == 0 || nx % 2 != 0)
    issues["PeakWindowWorkspace"] =
        "each spectrum must hold a non-zero even number of X values ([min, max] pairs, one per peak).";
  return issues;
}
...
void EstimatePeakIntensities::exec() {
  ...
  const size_t nPeaks = windowWS->x(0).size() / 2;
