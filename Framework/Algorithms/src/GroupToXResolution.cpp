...
const std::string GroupToXResolution::category() const { return "Transforms\\Rebin"; }
...
void GroupToXResolution::init() {
  ...
  inputValidator->add(std::make_shared<API::HistogramValidator>(acceptHistograms));
  declareProperty(
      std::make_unique<API::WorkspaceProperty<>>(Prop::INPUT_WS, "", Kernel::Direction::Input, inputValidator),
  ...
  declareProperty(Prop::FRACTION, 0.2, positive, "A fraction of Dx to group the points to.");
}
...
void GroupToXResolution::exec() {
  ...
  while (true) {
    ...
    if (pickSize > 0) {
      ...
      throw std::out_of_range("Failed to group. Is the X data sorted in ascending order?");
    }
    begin = end;
    if (begin > inXs.back()) {
      break;
