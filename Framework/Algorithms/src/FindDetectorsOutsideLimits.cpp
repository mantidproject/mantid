...
const std::string FindDetectorsOutsideLimits::category() const { return "Diagnostics"; }
...
void FindDetectorsOutsideLimits::exec() {
  ...
  if (minIndex > inputLength || maxIndex > inputLength) {
    std::ostringstream os;
    os << "Workspace indices must be lower than workspace size. Size=" << inputLength << ", Min=" << minIndex
       << ",Max=" << maxIndex;
    throw std::invalid_argument(os.str());
  }
  ...
  MatrixWorkspace_sptr countsWS = integrateSpectra(inputWS, minIndex, maxIndex, rangeLower, rangeUpper, true);
...
#pragma omp parallel for if (countsWS->threadSafe() && outputWS->threadSafe()), reduction(+ : numFailed)
