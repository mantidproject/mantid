...
void FitPeaks::init() {
  ...
  declareProperty(PropertyNames::STOP_WKSP_INDEX, EMPTY_INT(), "Last workspace index to fit (which is included)");
...
std::map<std::string, std::string> FitPeaks::validateInputs() {
  ...
  if (!error_table_name.empty()) {
    ...
    if (!use_raw_params) {
      const std::string msg = "FitPeaks must output RAW peak parameters if fitting error "
...
void FitPeaks::processInputPeakCenters() {
  ...
  if ((!m_peakCenters.empty()) && peakcenterws == nullptr) {
    ...
    g_log.warning() << "Input peak center workspace: " << m_peakCenterWorkspace->x(0).size() << ", "
...
void FitPeaks::fitSpectrumPeaks(size_t wi, const std::vector<double> &expected_peak_centers,
  ...
  // Clone the function
  IPeakFunction_sptr peakfunction = std::dynamic_pointer_cast<API::IPeakFunction>(m_peakFunction->clone());
  ...
  for (size_t fit_index = 0; fit_index < m_numPeaksToFit; ++fit_index) {
    ...
    bool foundAnyPeak = (lastGoodPeakParameters[fit_index].size() >
                         static_cast<size_t>(std::count_if(lastGoodPeakParameters[fit_index].begin(),
    ...
    if (expected_peak_pos <= x0 || expected_peak_pos >= xf) {
      ...
      bool observe_peak_params = decideToEstimatePeakParams(!foundAnyPeak, peakfunction);

      if (observe_peak_params && m_peakWidthEstimateApproach == EstimatePeakWidth::NoEstimation) {
      ...
      // point)
      cost = fitIndividualPeak(wi, peak_fitter, expected_peak_pos, peak_window_i, observe_peak_params, peakfunction,
                               bkgdfunction);
    ...
    processSinglePeakFitResult(wi, peak_index, cost, expected_peak_centers, fit_function,
 ...
 * result to
 */
void FitPeaks::processSinglePeakFitResult(size_t wsindex, size_t peakindex, const double cost,
 ...
 */
void FitPeaks::estimateBackground(const Histogram &histogram, const std::pair<double, double> &peak_window,
  ...
  this->estimateLinearBackground(histogram, peak_window.first, peak_window.second, bkgd_a0, bkgd_a1);
 ...
 */
int FitPeaks::estimatePeakParameters(const Histogram &histogram, const std::pair<double, double> &peak_window,
                                     const API::IPeakFunction_sptr &peakfunction,
                                     const API::IBackgroundFunction_sptr &bkgdfunction, bool observe_peak_width) {

  // get the range of start and stop to construct a function domain
  const auto &vector_x = histogram.points();
  auto start_iter = std::lower_bound(vector_x.begin(), vector_x.end(), peak_window.first);
  auto stop_iter = std::lower_bound(vector_x.begin(), vector_x.end(), peak_window.second);
  size_t start_index = static_cast<size_t>(start_iter - vector_x.begin());
  size_t stop_index = static_cast<size_t>(stop_iter - vector_x.begin());
  ...
  int result =
      observePeakCenter(histogram, bkgd_values, start_index, stop_index, peak_center, peak_center_index, peak_height);
  ...
  if (observe_peak_width && m_peakWidthEstimateApproach != EstimatePeakWidth::NoEstimation) {
    double peak_fwhm = observePeakFwhm(histogram, bkgd_values, peak_center_index, start_index, stop_index);
    if (peak_fwhm > 0.0) {
 ...
 */
int FitPeaks::observePeakCenter(const Histogram &histogram, FunctionValues &bkgd_values, size_t start_index,
  ...
  const size_t searchBox = max(static_cast<size_t>(.3 * static_cast<double>(windowSize)), static_cast<size_t>(5));
 ...
 */
double FitPeaks::observePeakFwhm(const Histogram &histogram, FunctionValues &bkgd_values, size_t ipeak, size_t istart,
                                 size_t istop) {
  double peak_fwhm(-0.);
...
double FitPeaks::fitIndividualPeak(size_t wi, const API::IAlgorithm_sptr &fitter, const double expected_peak_center,
  ...
  if (m_highBackground) {
    ...
    cost = fitFunctionHighBackground(fitter, fitwindow, wi, expected_peak_center, observe_peak_params, peakfunction,
                                     bkgdfunc);
  } else {
    // fit peak and background
    cost = fitFunctionSD(fitter, peakfunction, bkgdfunc, m_inputMatrixWS, wi, fitwindow.first, fitwindow.second,
...
double FitPeaks::fitFunctionSD(const IAlgorithm_sptr &fit, const API::IPeakFunction_sptr &peak_function,
                               const API::IBackgroundFunction_sptr &bkgd_function,
                               const API::MatrixWorkspace_sptr &dataws, size_t wsindex, double xmin, double xmax,
                               const double &expected_peak_center, bool observe_peak_shape, bool estimate_background) {
  ...
  int result = estimatePeakParameters(histogram, peak_window, peak_function, bkgd_function, observe_peak_shape);
  if (result != GOOD) {
...
double FitPeaks::fitFunctionHighBackground(const IAlgorithm_sptr &fit, const std::pair<double, double> &fit_window,
  ...
  bkgdfunc->setParameter(1, bkgdfunc->getParameter(1) + high_bkgd_function->getParameter(1));
...
void FitPeaks::getRangeData(size_t iws, const std::pair<double, double> &fit_window, std::vector<double> &vec_x,
  ...
  size_t left_index = std::lower_bound(orig_x.begin(), orig_x.end(), fit_window.first) - orig_x.begin();
  size_t right_index = std::lower_bound(orig_x.begin(), orig_x.end(), fit_window.second) - orig_x.begin();
 ...
 */
void FitPeaks::estimateLinearBackground(const Histogram &histogram, double left_window_boundary,
  ...
  if (iback_start < iback_stop) {
    double bg2, chisq;
    HistogramData::estimateBackground(1, histogram, istart, istop, iback_start, iback_stop, bkgd_a0, bkgd_a1, bg2,
                                      chisq);
  }
}
