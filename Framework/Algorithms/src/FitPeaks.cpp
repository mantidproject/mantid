...
void FitPeaks::fitSpectrumPeaks(size_t wi, const std::vector<double> &expected_peak_centers,
  ...
  for (size_t fit_index = 0; fit_index < m_numPeaksToFit; ++fit_index) {
    ...
    if (expected_peak_pos <= x0 || expected_peak_pos >= xf) {
      ...
      bool observe_peak_width = decideToEstimatePeakParams(!samePeakCrossSpectrum, peakfunction);
      //
 ...
 */
int FitPeaks::estimatePeakParameters(const Histogram &histogram, const std::pair<size_t, size_t> &peak_window,
  ...
  if (observe_peak_width && m_peakWidthEstimateApproach != EstimatePeakWidth::NoEstimation) {
    const double peak_fwhm =
        observePeakFwhm(histogram, bkgd_values, peak_center_index, peak_window.first, peak_window.second);
    if (peak_fwhm > 0.0) {
