...
void RebinToWorkspace::init() {
  ...
  declareProperty(std::make_unique<WorkspaceProperty<>>("WorkspaceToRebin", "", Kernel::Direction::Input,
  ...
  declareProperty(std::make_unique<WorkspaceProperty<>>("WorkspaceToMatch", "", Kernel::Direction::Input,
                                                        std::make_unique<API::HistogramValidator>()),
                  "The workspace to match the bin boundaries against. "
                  "This must be a Histogram workspace, not Point data. "
                  "If this is a problem try ConvertToHistogram.");
  declareProperty(std::make_unique<WorkspaceProperty<>>("OutputWorkspace", "", Kernel::Direction::Output),
...
bool RebinToWorkspace::needToRebin(const MatrixWorkspace_sptr &left, const MatrixWorkspace_sptr &rght) {
...
void RebinToWorkspace::rebin(MatrixWorkspace_sptr &toRebin, MatrixWorkspace_sptr &toMatch) {
  ...
  const bool matchingX = (toRebin->getNumberHistograms() != toMatch->getNumberHistograms());
...
void RebinToWorkspace::histogram(API::MatrixWorkspace_sptr &toRebin, API::MatrixWorkspace_sptr &toMatch) {
  const auto &inputWS = std::dynamic_pointer_cast<const EventWorkspace>(toRebin);
  const auto numHist = static_cast<int>(toRebin->getNumberHistograms());
  // everything gets the same bin boundaries as the first spectrum
  const bool matchingX = (toRebin->getNumberHistograms() != toMatch->getNumberHistograms());
