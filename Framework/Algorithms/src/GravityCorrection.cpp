 ...
 */
double coordinate(const DetectorInfo &detectorInfo, size_t i, PointingAlong direction) {
 ...
 * index 1 (unit metres)
 */
double coordinate(const SpectrumInfo &spectrumInfo, size_t i, PointingAlong direction) {
...
double coordinate(const string &componentName, PointingAlong direction,
  ...
  double position{0.};
  IComponent_const_sptr component = instrument->getComponentByName(componentName);
  if (component == nullptr)
    throw runtime_error("Cannot get instrument component with name " + componentName);
  else {
...
void GravityCorrection::init() {
  ...
  this->declareProperty(
      make_unique<WorkspaceProperty<MatrixWorkspace>>("InputWorkspace", "", Direction::Input, wsValidator),
      "The name of the input workspace. Values of X and Y must be "
      "TOF and counts, respectively.");
  this->declareProperty(make_unique<WorkspaceProperty<MatrixWorkspace>>("OutputWorkspace", "", Direction::Output),
 ...
 */
string GravityCorrection::componentName(const string &inputName, Mantid::Geometry::Instrument_const_sptr &instr) {
  const string propName = this->getPropertyValue(inputName);
  const string compName = instr->getParameterAsString("Workflow." + propName);
  if (!compName.empty()) {
    g_log.information("Found " + propName + " in Parameter File. " + inputName + ": " + compName);
    return compName;
  }
  return propName;
}
...
map<string, string> GravityCorrection::validateInputs() {
  ...
  Mantid::Geometry::Instrument_const_sptr instrument = this->m_ws->getInstrument();
  ...
  for (auto mapit = slits.begin(); mapit != slits.end(); ++mapit) {
    if (mapit->second.empty())
      ...
      if (slit == nullptr)
        result[mapit->first] = "Instrument component with name " + (mapit->second) + " does not exist. ";
      else {
        last = general::coordinate(mapit->second, m_beamDirection, instrument);
        iposition += last;
        if ((mapit->first == "SecondSlitName") && (iposition / 2 == last && last > 0.))
...
void GravityCorrection::slitCheck() {
  ...
  const string slit1{this->componentName("FirstSlitName", this->m_virtualInstrument)};
  const string slit2{this->componentName("SecondSlitName", this->m_virtualInstrument)};
  // initial guess
  this->m_slit1Name = slit1;
  this->m_slit2Name = slit2;
  // in beam directions
  const double sourceD = general::coordinate(sourceName, this->m_beamDirection, this->m_virtualInstrument);
  const double sampleD = general::coordinate(sampleName, this->m_beamDirection, this->m_virtualInstrument);
  const double slit1D = general::coordinate(slit1, this->m_beamDirection, this->m_virtualInstrument);
  const double slit2D = general::coordinate(slit2, this->m_beamDirection, this->m_virtualInstrument);
...
double GravityCorrection::finalAngle(const double k, const double theta) {
  ...
  double beamShift = (k * (pow(m_beam1, 2.) - pow(m_beam2, 2.)) + (beamDiff * tanAngle)) / (2. * k * beamDiff);
...
void GravityCorrection::virtualInstrument() {
  ...
  MatrixWorkspace_sptr ws = create<Workspace2D>(instrument, this->m_ws->indexInfo().globalSize(), BinEdges(2));
  ...
  if (instrument->isParametrized()) {
    ...
    vector<IComponent_const_sptr> comps = {instrument->getComponentByName(sourceName),
    ...
    if (samplePos.distance(V3D(0., 0., 0.)) > 1e-10) {
      ...
      for (vector<IComponent_const_sptr>::iterator compit = comps.begin(); compit != comps.end(); ++compit) {
        const auto compID1 = (*compit)->getComponentID();
        componentInfo.setPosition(componentInfo.indexOf(compID1), (*compit)->getPos() - samplePos);
      }
    ...
    const double sampleY = general::coordinate(sampleName, this->m_upDirection, ws->getInstrument());
    ...
    if (sampleY != 0.) {
       ...
       *         |/___|____> z
       */
      tanAngle = sampleY / general::coordinate(sourceName, m_beamDirection, ws->getInstrument());
      auto &componentInfo = ws->mutableComponentInfo();
      for (auto compit = comps.begin(); compit != comps.end(); ++compit) {
        const auto compID2 = (*compit)->getComponentID();
        V3D position = (*compit)->getPos();
        double coordUp = general::coordinate((*compit)->getName(), this->m_upDirection, ws->getInstrument());
        general::setCoordinate(position, this->m_beamDirection, coordUp / tanAngle);
      ...
      for (size_t i = 0; i < detectorInfo.size(); ++i) {
        ...
        general::setCoordinate(position, this->m_beamDirection,
                               general::coordinate(detectorInfo, i, this->m_upDirection) / tanAngle);
    ...
    const double sampleX = general::coordinate(sampleName, this->m_horizontalDirection, ws->getInstrument());
    if (sampleX != 0.) {
       ...
       *         |/_______> x
       */
      tanAngle = sampleX / general::coordinate(sampleName, this->m_beamDirection, ws->getInstrument());
      auto &componentInfo = ws->mutableComponentInfo();
      for (auto compit = comps.begin(); compit != comps.end(); ++compit) {
        const auto compID2 = (*compit)->getComponentID();
        V3D position = (*compit)->getPos();
        double coordHori = general::coordinate((*compit)->getName(), this->m_horizontalDirection, ws->getInstrument());
        general::setCoordinate(position, this->m_beamDirection, coordHori / tanAngle);
        componentInfo.setPosition(componentInfo.indexOf(compID2), position);
      }
      auto &detectorInfo = ws->mutableDetectorInfo();
      for (size_t i = 0; i < detectorInfo.size(); ++i) {
        V3D position = detectorInfo.position(i);
        general::setCoordinate(position, this->m_beamDirection,
                               general::coordinate(detectorInfo, i, this->m_horizontalDirection) / tanAngle);
        detectorInfo.setPosition(i, position);
        // rotate detectors
        V3D hvector{0., 0., 0.};
        double hangle = atan(tanAngle);
        general::setCoordinate(hvector, this->m_horizontalDirection, sin(hangle));
 ...
 */
size_t GravityCorrection::spectrumNumber(const double angle, const double theta) {
  ...
  auto lval = at->first;
  while (this->m_smallerThan((1.5 * (*(++at)).first) - (0.5 * lval), angle) && (at != this->m_finalAngles.cend())) {
    lval = at->first;
    n = at->second;
  }
  return n;
 ...
 */
double GravityCorrection::parabolaArcLength(const double arg, double constant) const {
...
void GravityCorrection::exec() {
  this->m_progress = make_unique<Progress>(this, 0.0, 1.0, 10 + this->m_ws->size());
  this->m_progress->report("Create virtual instrument ...");
  this->virtualInstrument();
  this->m_progress->report("Checking slits ...");
  this->slitCheck();
  m_beam1 = general::coordinate(this->m_slit1Name, this->m_beamDirection, this->m_virtualInstrument);
  ...
  for (size_t i = 0; i < spectrumInfo.size(); ++i) {
    ...
    this->m_finalAngles.emplace_hint(this->m_finalAngles.end(), std::make_pair(finalAngleValue, i));
    this->m_progress->report();
  }
  if (this->m_finalAngles.empty())
    this->g_log.error("Map of initial final angles and its corresponding spectrum "
  ...
  for (size_t i = 0; i < spectrumInfo.size(); ++i) {
    ...
    for (HistogramX::const_iterator tofit = tof.cbegin(); tofit < tof.cend(); ++tofit) {
      // this velocity should take the real flight path into account
      if (*tofit == 0.) {
        this->g_log.debug("Zero tof detected. Cannot divide by it, skip bin " + to_string(i));
        continue;
      }

      double v{((spectrumInfo.l1() + spectrumInfo.l2(i)) / *tofit) * 1.e6}; // unit: m/s
    ...
    if (this->m_ws->hasMaskedBins(i)) {
      ...
      for (auto &maskit : maskIn) {
        ...
        HistogramX::iterator t =
            find_if(tof2.begin(), tof2.end(), [&](const double &ii) { return ii > this->m_ws->x(i)[maskit.first]; });
