...
void SpectrumAlgorithm::declareWorkspaceIndexSetProperties(const std::string &indexMinPropertyName,
  ...
  declareProperty(m_indexMinPropertyName, 0, mustBePositive, "The first Workspace index to be included in the summing");
  declareProperty(m_indexMaxPropertyName, EMPTY_INT(), mustBePositive,
                  "The last Workspace index to be included in the summing");

  declareProperty(std::make_unique<Kernel::ArrayProperty<size_t>>(m_indexRangePropertyName),
 ...
 * range. */
Kernel::IndexSet SpectrumAlgorithm::getWorkspaceIndexSet(const API::MatrixWorkspace &workspace) const {
