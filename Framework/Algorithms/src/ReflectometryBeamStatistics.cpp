...
const std::string ReflectometryBeamStatistics::name() const {
  return "ReflectometryBeamStatistics";
}
...
const std::string ReflectometryBeamStatistics::category() const {
...
void ReflectometryBeamStatistics::init() {
  ...
  declareProperty(Prop::FIRST_SLIT_NAME, "", mandatoryString,
                  "Name of the first slit component.");
  declareProperty(Prop::FIRST_SLIT_SIZE_LOG, "", mandatoryString,
                  "The sample log entry for the first slit opening.");
  declareProperty(Prop::SECOND_SLIT_NAME, "", mandatoryString,
...
void ReflectometryBeamStatistics::exec() {
  ...
  statistics.bentSample = bentSample(setup, statistics.sampleWaviness,
                                     statistics.firstSlitAngularSpread);
...
double ReflectometryBeamStatistics::beamRMSVariation(
  ...
  if (ws->run().hasProperty(LogEntry::BEAM_RMS_VARIATION)) {
    try {
      rmsVariation = ws->run().getPropertyValueAsType<double>(
          LogEntry::BEAM_RMS_VARIATION);
    } catch (std::invalid_argument &) {
      m_log.warning()
          << "Cannot convert '" + LogEntry::BEAM_RMS_VARIATION +
  ...
  if (!rmsVariation) {
    ...
    rmsVariation =
        2. * std::sqrt(2. * std::log(2.)) * pixelSize * std::sqrt(variance);
  }
  return *rmsVariation;
}
 ...
 */
bool ReflectometryBeamStatistics::bentSample(
    const Setup &setup, const double sampleWaviness,
    const double firstSlitAngularSpread) {
  return sampleWaviness > 0 &&
         setup.detectorResolution / setup.l2 > firstSlitAngularSpread;
}
...
ReflectometryBeamStatistics::createSetup(const API::MatrixWorkspace &ws,
  ...
  const std::vector<int> reflectedForeground =
      getProperty(Prop::REFLECTED_FOREGROUND);
  ...
  const std::vector<int> directForeground =
      getProperty(Prop::DIRECT_FOREGROUND);
 ...
 */
double ReflectometryBeamStatistics::detectorAngularResolution(
 ...
 */
double
ReflectometryBeamStatistics::interslitDistance(const API::MatrixWorkspace &ws) {
  const std::string slit1Name = getProperty(Prop::FIRST_SLIT_NAME);
  const std::string slit2Name = getProperty(Prop::SECOND_SLIT_NAME);
  auto instrument = ws.getInstrument();
 ...
 */
void ReflectometryBeamStatistics::rmsVariationToLogs(API::MatrixWorkspace &ws,
 ...
 * @return the waviness
 */
double ReflectometryBeamStatistics::sampleWaviness(const Setup &setup,
 ...
 */
double
ReflectometryBeamStatistics::secondSlitAngularSpread(const Setup &setup) {
  // s3_fwhm in COSMOS.
  const auto slit2Detector = setup.slit2SampleDistance + setup.l2;
 ...
 */
void ReflectometryBeamStatistics::statisticsToLogs(
