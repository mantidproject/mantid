...
std::map<std::string, std::string> CalculateEfficiency2::validateInputs() {
  ...
  if (inputWS->blocksize() > 1)
    result[PropertyNames::INPUT_WORKSPACE] = "Input workspace must have only one bin";
...
void CalculateEfficiency2::exec() {
  ...
  childAlg->setPropertyValue("WorkspaceToRebin", getPropertyValue(PropertyNames::INPUT_WORKSPACE));
  childAlg->setPropertyValue("WorkspaceToMatch", getPropertyValue(PropertyNames::INPUT_WORKSPACE));
