...
void ConvertSpectrumAxis2::createElasticQMap(API::Progress &progress,
  ...
  for (size_t i = 0; i < nHist; i++) {
    ...
    if (!spectrumInfo.isMonitor(i)) {
      ...
      efixed = getEfixed(spectrumInfo, i, inputWS, emode); // get efixed
    } else {
      twoTheta = 0.0;
      efixed = DBL_MIN;
    }
...
double ConvertSpectrumAxis2::getEfixed(const API::SpectrumInfo spectrumInfo,
                                       size_t i,
  ...
  if (efixedProp != EMPTY_DBL()) {
    ...
    if (emode == 1) {
      ...
      efixed = spectrumInfo.eFixed(i);

      if (efixed == 0.0) {
        throw std::invalid_argument(
            "Could not retrieve Efixed from the spectrum " + std::to_string(i) +
