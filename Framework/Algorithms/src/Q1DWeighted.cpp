...
void Q1DWeighted::exec() {
  ...
  // to calculate the wavelength width per frame
  auto wavelengthMin =
      inputWS->run().getPropertyValueAsType<double>("wavelength_min");
  auto wavelengthMax =
      inputWS->run().getPropertyValueAsType<double>("wavelength_max");
  auto wavelengthBinWidth = (wavelengthMax - wavelengthMin) / (xLength - 1);
  ...
  for (int iTOF = 0; iTOF < xLength - 1; iTOF++) {
    ...
    auto wl_min = wavelengthMin + wavelengthBinWidth * iTOF;
    tof_ws->mutableRun().addProperty("wavelength_min", wl_min, "Angstrom",
                                     true);
    auto wl_max = wavelengthMin + wavelengthBinWidth * (iTOF + 1);
    ...
    PARALLEL_CRITICAL(iq) /* Write to shared memory - must protect */
    {
      for (int k = 0; k < sizeOut - 1; k++) {
        if (XNorm[k] > 0) {
          ...
          tofYOut[k] += lambdaIq[k] / XNorm[k];
          tofEOut[k] += lambdaIqErr[k] / XNorm[k] / XNorm[k];
        }
  ...
  std::string outputWSGroupName = getPropertyValue("IQLambdaWorkspace");
  if (outputWSGroupName.empty()) {
    std::string outputWSName = getPropertyValue("OutputWorkspace");
