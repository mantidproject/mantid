...
void CrossCorrelate::exec() {
  ...
  if (maxDSpaceShift != EMPTY_DBL()) {
    ...
    shiftCorrection = static_cast<int>(0.5 * std::max(0.0, abs((-numReferenceY + 2) - (numReferenceY - 2)) - maxBins));
  }
  ...
  for (size_t i = 0; i < outX.size(); ++i) {
    outX[i] = static_cast<double>(i - static_cast<size_t>(numReferenceY - shiftCorrection) + 2);
  }
