...
void EQSANSTofStructure::exec() {
  ...
  auto const frequencyLog = dynamic_cast<TimeSeriesProperty<double> *>(inputWS->run().getLogData("frequency"));
  if (!frequencyLog) {
  ...
  auto const chopper_speedLog = dynamic_cast<TimeSeriesProperty<double> *>(inputWS->run().getLogData("Speed1"));
  if (!chopper_speedLog) {
