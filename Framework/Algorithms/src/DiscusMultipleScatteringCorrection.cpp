...
/*std::tuple<std::vector<double>, std::vector<double>> DiscusMultipleScatteringCorrection::simulatePaths(
    ...
    const Kernel::V3D &detPos, bool specialSingleScatterCalc, const Mantid::Geometry::BoundingBox &detectorBbox)*/
 ...
 */
/* std::tuple<bool, std::vector<double>> DiscusMultipleScatteringCorrection::scatter(const int nScatters,
   Kernel::PseudoRandomNumberGenerator &rng, const ComponentWorkspaceMappings &componentWorkspaces, const double kinc,
                                            const std::vector<double> &wValues, const Kernel::V3D &detPos,
                                            bool specialSingleScatterCalc, const Mantid::Geometry::BoundingBox
   &detectorBbox)
                                            */
std::tuple<bool, std::vector<double>> DiscusMultipleScatteringCorrection::scatter(
  ...
  /*
  const auto shape = detector.shape();
  if (shape != nullptr) {
    const Mantid::Geometry::BoundingBox box = shape->getBoundingBox();
    auto widthVec = box.width();
    double w = widthVec.norm();
  }
  */
