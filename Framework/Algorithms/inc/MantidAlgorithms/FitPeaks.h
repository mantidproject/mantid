...
private:
  ...
  double fitIndividualPeak(size_t wi, const API::IAlgorithm_sptr &fitter, const double expected_peak_center,
                           const std::pair<double, double> &fitwindow, const bool observe_peak_params,
  ...
  double fitFunctionSD(const API::IAlgorithm_sptr &fit, const API::IPeakFunction_sptr &peak_function,
                       ...
                       size_t wsindex, double xmin, double xmax, const double &expected_peak_center,
                       bool observe_peak_shape, bool estimate_background);

  double fitFunctionMD(API::IFunction_sptr fit_function, const API::MatrixWorkspace_sptr &dataws, size_t wsindex,
                       std::vector<double> &vec_xmin, std::vector<double> &vec_xmax);
  ...
  void estimateBackground(const HistogramData::Histogram &histogram, const std::pair<double, double> &peak_window,
                          const API::IBackgroundFunction_sptr &bkgd_function);
  /// estimate linear background
  void estimateLinearBackground(const HistogramData::Histogram &histogram, double left_window_boundary,
                                double right_window_boundary, double &bkgd_a0, double &bkgd_a1);

  /// Estimate peak parameters by 'observation'
  int estimatePeakParameters(const HistogramData::Histogram &histogram, const std::pair<double, double> &peak_window,
                             const API::IPeakFunction_sptr &peakfunction,
  ...
  void processSinglePeakFitResult(size_t wsindex, size_t peakindex, const double cost,
