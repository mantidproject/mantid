...
public:
  ...
  void test_oauth_token_from_json_stream() {
    ...
    tokenStringSteam << std::string("{\"token_type\": \"Bearer\", \"expires_in\": 3600, "
    ...
    TS_ASSERT_EQUALS(oauthToken.accessToken(), std::string("2KSL5aEnLvIudMHIjc7LcBWBCfxOHZ"));
    TS_ASSERT_EQUALS(oauthToken.scope(), std::string("api:read data:read settings:read"));
    TS_ASSERT_EQUALS(oauthToken.refreshToken(), std::string("eZEiz7LbgFrkL5ZHv7R4ck9gOzXexb"));
