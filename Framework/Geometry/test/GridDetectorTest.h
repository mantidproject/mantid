...
public:
  ...
  void testNameParentValueConstructor() {
    auto parent = std::make_unique<CompAssembly>("Parent");
    parent->setPos(1, 2, 3);

    // name and parent - parent takes ownership
    auto q = new GridDetector("Child", parent.get());
    q->setPos(1, 1, 1);
  ...
  void testFullConstructor() {
    ...
    ParameterMap_sptr pmap(new ParameterMap());
    auto parDet = std::make_unique<GridDetector>(det.get(), pmap.get());
  ...
  void testResizingParameter() {
    ...
    ParameterMap_sptr pmap(new ParameterMap());
    auto parDet = std::make_unique<GridDetector>(det.get(), pmap.get());
    pmap->addDouble(det.get(), "scalex", 3.0);
    pmap->addDouble(det.get(), "scaley", 5.0);
    pmap->addDouble(det.get(), "scalez", 2.0);
...
private:
   ...
   */
  void do_test_on(const GridDetector &det) {
    do_test_basics(det);
    do_test_bounds(det);
    do_test_ids(det);
  ...
  void do_test_basics(const GridDetector &det) {
  ...
  void do_test_positions(const GridDetector &det) {
