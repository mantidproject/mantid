...
public:
  ...
  void test_GeneralFrameFactory_known_units() {
    ...
    TSM_ASSERT("Expect that we have a label unit",
               dynamic_cast<const Mantid::Kernel::InverseAngstromsUnit *>(&heldMDUnit));
  }
  ...
  void test_QLabFrameFactory() {
    ...
    TSM_ASSERT("Should NOT offer to produce QLab products for this frame key", !factory.canInterpret(badArgument));

    const MDFrameArgument argument = MDFrameArgument(QLab::QLabName);
    TSM_ASSERT("Should offer to produce QLab products for this frame key", factory.canInterpret(argument));

    std::unique_ptr<MDFrame> product = factory.create(argument);
    TSM_ASSERT("Should be creating a QLab frame", dynamic_cast<Mantid::Geometry::QLab *>(product.get()));
  }
  ...
  void test_QSampleFrameFactory() {
    ...
    const MDFrameArgument badArgument("any_frame");
    TSM_ASSERT("Should NOT offer to produce QSample products for this frame key", !factory.canInterpret(badArgument));

    const MDFrameArgument argument = MDFrameArgument(QSample::QSampleName);
    TSM_ASSERT("Should offer to produce QSample products for this frame key", factory.canInterpret(argument));

    std::unique_ptr<MDFrame> product = factory.create(argument);
    TSM_ASSERT("Should be creating a QSample frame", dynamic_cast<Mantid::Geometry::QSample *>(product.get()));
  }
  ...
  void test_HKLFrameFactory_interpretation() {
    ...
    TSM_ASSERT("Should NOT offer to produce HKL products as units are incompatible",
  ...
  void test_HKLFrameFactory_create_inverse_angstroms() {
    ...
    std::unique_ptr<MDFrame> product = factory.create(MDFrameArgument(HKL::HKLName, Units::Symbol::InverseAngstrom));
  ...
  void test_HKLFrameFactory_create_rlu() {
    ...
    std::unique_ptr<MDFrame> product = factory.create(MDFrameArgument(HKL::HKLName, Units::Symbol::RLU));
    TSM_ASSERT("Should be creating a HKL frame, in rlu", dynamic_cast<Mantid::Geometry::HKL *>(product.get()));
  ...
  void test_make_standard_chain() {
    ...
    TS_ASSERT(dynamic_cast<UnknownFrame *>(chain->create(MDFrameArgument(UnknownFrame::UnknownFrameName)).get()));
    TS_ASSERT(dynamic_cast<GeneralFrame *>(chain->create(MDFrameArgument("any_frame")).get()));
    TS_ASSERT(dynamic_cast<Mantid::Geometry::QLab *>(chain->create(MDFrameArgument(QLab::QLabName)).get()));
    TS_ASSERT(dynamic_cast<Mantid::Geometry::QSample *>(chain->create(MDFrameArgument(QSample::QSampleName)).get()));
    TS_ASSERT(
        dynamic_cast<Mantid::Geometry::HKL *>(chain->create(MDFrameArgument(HKL::HKLName, Units::Symbol::RLU)).get()));
  }
};
