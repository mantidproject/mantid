...
public:
  ...
  void testValidCenterings() {
    testCenteringGroup(
        "P", CenteringGroup::P, "P",
        SymmetryOperationFactory::Instance().createSymOps("x,y,z"));
    testCenteringGroup("I", CenteringGroup::I, "I",
                       SymmetryOperationFactory::Instance().createSymOps(
    ...
    testCenteringGroup("R", CenteringGroup::Robv, "R",
                       SymmetryOperationFactory::Instance().createSymOps(
                           "x,y,z; x+1/3,y+2/3,z+2/3; x+2/3,y+1/3,z+1/3"));
    testCenteringGroup("Robv", CenteringGroup::Robv, "R",
                       SymmetryOperationFactory::Instance().createSymOps(
                           "x,y,z; x+1/3,y+2/3,z+2/3; x+2/3,y+1/3,z+1/3"));
    testCenteringGroup("Rrev", CenteringGroup::Rrev, "R",
                       SymmetryOperationFactory::Instance().createSymOps(
...
private:
  void
  testCenteringGroup(const std::string &symbol,
                     CenteringGroup::CenteringType expectedCenteringType,
                     const std::string &expectedSymbol,
                     const std::vector<SymmetryOperation> &expectedOperations) {
    TSM_ASSERT_THROWS_NOTHING("Exception when trying to create " + symbol,
                              CenteringGroup rawGroup(symbol));
    ...
    TSM_ASSERT_EQUALS("CenteringString did not match for " + symbol,
                      centeringGroup->getSymbol(), expectedSymbol);
  }

  bool
  symOpExistsInCollection(const SymmetryOperation &op,
                          const std::vector<SymmetryOperation> &collection) {
    return std::find(collection.begin(), collection.end(), op) !=
           collection.end();
  }
};
