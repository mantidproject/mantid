...
public:
  ...
  void test_Intersection_Type_With_Second_Edge() {
    const PolygonEdge edge(V2D(0.1, 0.1), V2D(2.0, 2.0));
    double t(0.0);
    TS_ASSERT_EQUALS(orientation(edge, PolygonEdge(V2D(0.0, 1.0), V2D(2.0, 1.0)), t), PolygonEdge::Skew);
    TS_ASSERT_EQUALS(orientation(edge, PolygonEdge(V2D(0.2, 0.2), V2D(2.0, 2.0)), t), PolygonEdge::Collinear);
    TS_ASSERT_EQUALS(orientation(edge, PolygonEdge(V2D(0.2, 0.3), V2D(2.0, 2.1)), t), PolygonEdge::Parallel);
  }

  void test_A_Valid_Intersection_Gives_A_Valid_Crossing_Pt() {
    const PolygonEdge edge(V2D(0.1, 0.1), V2D(2.0, 2.0));
    V2D crossPt;
    PolygonEdge::Orientation orient = crossingPoint(edge, PolygonEdge(V2D(0.0, 1.0), V2D(2.0, 1.0)), crossPt);
    TS_ASSERT_EQUALS(orient, PolygonEdge::SkewCross);
    TS_ASSERT_EQUALS(crossPt.X(), 1.0);
    TS_ASSERT_EQUALS(crossPt.Y(), 1.0);

    // Skewed but no cross
    TS_ASSERT_EQUALS(crossingPoint(edge, PolygonEdge(V2D(), V2D(1.5, 0.75)), crossPt), PolygonEdge::SkewNoCross);
    TS_ASSERT_EQUALS(crossingPoint(edge, PolygonEdge(V2D(0.2, 0.2), V2D(2.0, 2.0)), crossPt), PolygonEdge::Collinear);
    TS_ASSERT_EQUALS(crossingPoint(edge, PolygonEdge(V2D(0.2, 0.3), V2D(2.0, 2.1)), crossPt), PolygonEdge::Parallel);
  }
