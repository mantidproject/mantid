...
public:
  ...
  void test_parse_IDF_for_unit_testing() // IDF stands for Instrument Definition File
  {
    ...
    try {
      std::filesystem::remove(vtpFilename);
  ...
  */
  void testWriteCacheFileToTempDirectoryIfNoIDF() {
    ...
    std::filesystem::path path(Mantid::Kernel::ConfigService::Instance().getTempDir());
    path /= (parser.getMangledName() + ".vtp");
