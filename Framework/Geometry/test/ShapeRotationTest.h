...
public:
  ...
  void test_outstanding_is_the_remainder_when_partially_baked() {
    ...
    // Assert against R * B^T as well as the equivalent single rotation, so this cannot pass by
    // the angles happening to subtract.
  ...
  void test_a_definition_frame_rotation_does_not_count_as_baked() {
    ...
    // outstanding - the distinction the marker exists to make.
    const auto rotation = rotationZ(90.0);
    auto meshShape = createCube(2.0);
  ...
  void test_lab_frame_csg_shape_preserves_a_definition_frame_rotation() {
    // <applied-goniometer> says none of <goniometer> is a bake, so its 90 degrees is a rotation
    // within the shape's own frame and must survive the move: 90 + 90 leaves the sphere on -x.
  ...
  void test_a_csg_shape_without_xml_cannot_be_moved_to_the_lab_frame() {
    // createSphere assembles surfaces directly rather than parsing, so there is no XML to rewrite
  ...
  void test_a_csg_shape_without_xml_is_returned_when_nothing_is_outstanding() {
    // nothing has to be expressed, so the missing XML does not matter
    const auto sphere = ShapeFactory::createSphere(V3D(2.0, 0.0, 0.0), 0.5);
  ...
  void test_the_lab_frame_csg_shape_keeps_the_id() {
    // createShape carries over neither the material nor the id, and the id is what the Nexus round
    // trip saves - a shape that lost it would come back anonymous
  ...
  void test_a_shape_that_cannot_be_rotated_is_returned_unchanged() {
    // no way to express a rotation, so it comes back as it stands, plus a warning in the log
