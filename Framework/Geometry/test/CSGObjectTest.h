...
public:
  ...
  */
  ...
  void testTracksForHollowCylinder() {
    ...
    const V3D AXIS_SYMM{0., 1., 0.};
    auto hollowCylinder = ComponentCreationHelper::createHollowCylinder(
        RADIUS_INNER, RADIUS_OUTER, HEIGHT, BOTTOM_CENTRE, AXIS_SYMM,
        "hol-cyl");
    ...
    constexpr double RADIUS_IN_WALL_OFFSET{(.25 * RADIUS_INNER) +
                                           (.75 * RADIUS_OUTER)};
    // point in first wall
    Track in_first_wall_offset(V3D{0., 0., RADIUS_IN_WALL_OFFSET},
                               BEAM_DIRECTION);
    hollowCylinder->interceptSurface(in_first_wall_offset);
    TS_ASSERT_EQUALS(in_first_wall_offset.distInsideObject(),
                     .25 * WALL_THICKNESS); // DOESN'T WORK

    // point in second wall
    Track in_second_wall_offset(V3D{0., 0., -RADIUS_IN_WALL_OFFSET},
                                BEAM_DIRECTION);
    hollowCylinder->interceptSurface(in_second_wall_offset);
    TS_ASSERT_EQUALS(in_second_wall_offset.distInsideObject(),
                     1.75 * WALL_THICKNESS); // DOESN'T WORK
  }
