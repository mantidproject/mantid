...
public:
  ...
  static NearestNeighboursFactoryTest *createSuite() {
    return new NearestNeighboursFactoryTest();
  }
  static void destroySuite(NearestNeighboursFactoryTest *suite) {
    delete suite;
  }
  ...
  void testCreateObject() {
    ...
    TSM_ASSERT_THROWS_NOTHING("Create on Factory should not throw",
                              nn = factory.create(instrument, spectramap););
  ...
  void testCreateObjectAsINearestNeighbourFactory() {
    ...
    // The following will break if NearestNeighbours is not an
    // INearestNeighbours object.
    INearestNeighbours *nn = ifactory.create(instrument, spectramap);
    // For consistency.
    TSM_ASSERT("Product should be a NearestNeighbours object.",
               NULL != dynamic_cast<NearestNeighbours *>(nn));
    delete nn;
  }
};
