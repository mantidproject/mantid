...
public:
  static int makeCNFcopy(std::unique_ptr<Rule> &);         ///< Make Rule into a CNF format (slow)
  static int makeFullDNF(std::unique_ptr<Rule> &);         ///< Make Rule into a full DNF format
...
private:
  ...
  Intersection *doClone() const override; ///< Makes a copy of the whole downward tree
protected:
...
public:
  ...
  std::unique_ptr<Intersection> clone() const;                      ///< Makes a copy of the whole downward tree
  std::string className() const override { return "Intersection"; } ///< Returns class name as string

  Rule *leaf(const int ipt = 0) const override { return ipt ? B.get() : A.get(); } ///< selects leaf component
  ...
  void getBoundingBox(double &xmax, double &ymax, double &zmax, double &xmin, double &ymin,
                      double &zmin) override; /// bounding box
#ifdef ENABLE_OPENCASCADE
...
public:
  ...
  std::string className() const override { return "Union"; } ///< Returns class name as string

  Rule *leaf(const int ipt = 0) const override { return ipt ? B.get() : A.get(); } ///< Select a leaf component
  void setLeaves(std::unique_ptr<Rule>,
                 std::unique_ptr<Rule>) override; ///< set leaves
...
public:
  SurfPoint();
  std::string className() const override { return "SurfPoint"; } ///< Returns class name as string
...
public:
  ...
  std::string className() const override { return "CompObj"; } ///< Returns class name as string
  ...
  int isComplementary() const override { return 1; } ///< Always returns true (1)
...
public:
  ...
  std::string className() const override { return "CompGrp"; } ///< Returns class name as string

  Rule *leaf(const int) const override { return A.get(); } ///< selects leaf component
  void setLeaves(std::unique_ptr<Rule>, std::unique_ptr<Rule>) override;
...
public:
  ...
  std::string className() const override { return "BoolValue"; } ///< Returns class name as string
  ...
  bool isValid(const Kernel::V3D &) const override;
  bool isValid(const std::map<int, int> &) const override; ///< isValue :: Based on a surface status map
  int simplify() override;
