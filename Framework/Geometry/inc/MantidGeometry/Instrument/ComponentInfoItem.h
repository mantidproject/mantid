...
public:
  ComponentInfoItem(T &componentInfo, const size_t index) : m_componentInfo(&componentInfo), m_index(index) {}

  bool isDetector() const { return m_componentInfo->isDetector(m_index); }
  std::vector<size_t> detectorsInSubtree() const { return m_componentInfo->detectorsInSubtree(m_index); }
  std::vector<size_t> componentsInSubtree() const { return m_componentInfo->componentsInSubtree(m_index); }
  const std::vector<size_t> &children() const { return m_componentInfo->children(m_index); }
  ...
  Kernel::V3D scaleFactor() const { return m_componentInfo->scaleFactor(m_index); }
