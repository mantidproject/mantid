  ...
  if (pElemGonio) {
    ...
    for (size_t i = 0; i < 3; ++i) {
      for (size_t j = 0; j < 3; ++j) {
        m_gonioRotateMatrix[i][j] = getDoubleAttribute(pElemGonio, "a" + std::to_string(i) + std::to_string(j));
      }
    }
  }
...
std::string ShapeFactory::addGoniometerTag(Kernel::Matrix<double> rotateMatrix, std::string xml) {
  ...
  for (size_t i = 0; i < rotateMatrix.numRows(); ++i) {
    for (size_t j = 0; j < rotateMatrix.numCols(); ++j) {
      goniometerRotate += matrixElementNames[i][j] + " = '" + std::to_string(rotateMatrix[i][j]) + "' ";
    }
  }
  goniometerRotate += "/>";
