...
double IndexingUtils::Optimize_6dUB(DblMatrix &UB, DblMatrix &ModUB, const std::vector<V3D> &hkl_vectors, const std::vector<V3D> &mnp_vectors, const int &ModDim, const std::vector<V3D> &q_vectors, std::vector<double> &sigabc, std::vector<double> &sigq)
{
...
/**
 STATIC method Optimize_6dUB: Calculates the 6-dimensional matrix that most nearly maps
 the specified hkl_vectors and mnp_vectors to the specified q_vectors.  The calculated
 UB minimizes the sum squared differences between UB|ModUB*(h,k,l,m,n,p) and the
 corresponding (qx,qy,qz) for all of the specified hklmnp and Q vectors.
 The sum of the squares of the residual errors is returned.  This method is
 used to optimize the UB matrix and ModUB matrix once an initial indexing has been found.
 
 ModUB matrix is a 3x3 defines the modulation vectors in Q-sample. each colomn corresponds
 ...
 q_vectors that are indexed by the corresponding hkl
 vectors.
 NOTE: The number of hkl_vectors and mnp_vectors and q_vectors must be the same, and must
 be at least 4.
...
double IndexingUtils::Optimize_6dUB(DblMatrix &UB, DblMatrix &ModUB, const std::vector<V3D> &hkl_vectors, const std::vector<V3D> &mnp_vectors, const int &ModDim, const std::vector<V3D> &q_vectors)
{
    ...
    if (hkl_vectors.size() < 4)
    {
        throw std::invalid_argument("Optimize_6dUB(): Four or more indexed peaks needed to find UB");
    }
    
    if (hkl_vectors.size() != q_vectors.size() || hkl_vectors.size() != mnp_vectors.size())
    {
        throw std::invalid_argument("Optimize_6dUB(): Number of hkl_vectors doesn't match number of mnp_vectors or number of q_vectors");
    }
...
#define HALF_FFT_STEPS 256
  ...
  for (size_t dir_num = 0; dir_num < full_list.size(); dir_num++) {
    ...
    max_mag_fft = GetMagFFT(q_vectors, current_dir, N_FFT_STEPS, projections,
                            index_factor, magnitude_fft);
  ...
  DiscardDuplicates(directions, temp_dirs, q_vectors, required_tolerance,
                    len_tol, ang_tol);
...
double IndexingUtils::GetMagFFT(const std::vector<V3D> &q_vectors,
                                const V3D &current_dir, const size_t N,
 ...
 */
double IndexingUtils::GetFirstMaxIndex(const double magnitude_fft[], size_t N,
                                       double threshold) {
  ...
  while (i < N - 1 && !found_min) {
    val = magnitude_fft[i];
    if (val < threshold && val <= magnitude_fft[i - 1] &&
...
bool IndexingUtils::FormUB_From_abc_Vectors(DblMatrix &UB,
  ...
  while (!c_found && index < directions.size()) {
    ...
    while (!c_found && factor >= -1) // try c in + or - direction
    {
      ...
      if (perp_ang < 90. - epsilon && alpha >= min_deg &&
          (180. - alpha) >= min_deg && beta >= min_deg &&
          (180. - beta) >= min_deg) {
        c_found = true;
      }
      factor -= 2;
...
bool IndexingUtils::ValidIndex(const V3D &hkl, double tolerance) {
  ...
  return (withinTol(hkl[0], tolerance) && withinTol(hkl[1], tolerance) &&
...
*/
int IndexingUtils::NumberOfValidIndexes(const std::vector<V3D> &hkls,
                                        double tolerance,
 ...
 */
int IndexingUtils::NumberIndexed_3D(const V3D &a_dir, const V3D &b_dir,
                                    const V3D &c_dir,
                                    const std::vector<V3D> &q_vectors,
                                    double tolerance) {
...
/**
  ...
  @throws std::invalid_argument exception if the UB matrix is not a 3X3 matrix,
                                or if UB is singular.
 */
int IndexingUtils::CalculateMillerIndices(const DblMatrix &UB,
                                          const std::vector<V3D> &q_vectors,
                                          double tolerance,
...
/**
  ...
  @return The number of q_vectors that are indexed to within the specified
          tolerance, in the specified direction.
 */
int IndexingUtils::GetIndexedPeaks_3D(
    const V3D &direction_1, const V3D &direction_2, const V3D &direction_3,
...
int IndexingUtils::GetModulationVectors(const DblMatrix &UB, const DblMatrix &ModUB, V3D &ModVec1, V3D &ModVec2, V3D &ModVec3)
{
    OrientedLattice o_lattice;
    o_lattice.setUB(UB);
    ...
    if (o_lattice.getdh(2) != 0.0 || o_lattice.getdk(2) != 0.0 || o_lattice.getdl(2) != 0.0)
        ModDim = 2;
    if (o_lattice.getdh(3) != 0.0 || o_lattice.getdk(3) != 0.0 || o_lattice.getdl(3) != 0.0)
        ModDim = 3;
...
std::string IndexingUtils::GetLatticeParameterString(const DblMatrix &UB) {
  ...
  sprintf(buffer,
          ...
          lat_par[0], lat_par[1], lat_par[2], lat_par[3], lat_par[4],
          lat_par[5], lat_par[6]);
