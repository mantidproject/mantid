...
void *NCollection_AccAllocator::Allocate(const size_t theSize) {
  ...
  if (aSize <= mypLastBlock->FreeSize()) {
    ...
    for (aBlock = mypLastBlock->prevBlock; aBlock != 0L && --aBlocksRest; aBlock = aBlock->prevBlock) {
  ...
  Key aKey;
  Standard_ASSERT_VOID(aBlock == findBlock(anAddress, aKey), "improper work of NCollection_AccAllocator::findBlock");
#endif
  return anAddress;
}
...
void NCollection_AccAllocator::Free(void *theAddress) {
  ...
  if (aBlock->IsEmpty()) {
    ...
    // If there are no more blocks, reallocate the block to the default size
      ...
      Standard_Address aNewAddress = Standard::Reallocate(anAddress, myBlockSize);
...
//=======================================================================
NCollection_AccAllocator::Block *NCollection_AccAllocator::findBlock(const Standard_Address theAddress, Key &theKey) {
...
//=======================================================================
NCollection_AccAllocator::Block *NCollection_AccAllocator::allocateNewBlock(const Standard_Size theSize) {
...
#ifdef OCCT_DEBUG_FINDBLOCK
  Key aKey;
  Standard_ASSERT_VOID(mypLastBlock == findBlock((Standard_Byte *)mypLastBlock->allocStart - 1, aKey),
