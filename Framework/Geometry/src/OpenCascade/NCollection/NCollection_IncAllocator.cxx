...
Standard_EXPORT void IncAllocator_SetDebugFlag(const Standard_Boolean theDebug) { IS_DEBUG = theDebug; }
...
Standard_EXPORT void IncAllocator_PrintAlive() {
  ...
  for (NCollection_DataMap<Standard_Address, Standard_Size>::Iterator itMap(StorageIDMap()); itMap.More();
       itMap.Next()) {
    ...
    aTotSize += aSize;
    nbAlloc++;
    aFileOut << std::setw(20) << anID << ' ' << std::setw(20) << (double(aSize) / 1024.0) << '\n';
  }
...
NCollection_IncAllocator::NCollection_IncAllocator(size_t theBlockSize) : myMutex(NULL) {
  ...
  const size_t aSize =
      IMEM_SIZE(sizeof(IBlock)) + IMEM_SIZE((theBlockSize > 2 * sizeof(IBlock)) ? theBlockSize : aDefault);
...
void *NCollection_IncAllocator::Allocate(const size_t aSize) {
  ...
  if (cSize > mySize) {
    ...
    if (aResult)
      ...
      throw Standard_OutOfMemory("NCollection_IncAllocator: out of memory");
  } else if (cSize <= IMEM_FREE(myFirstBlock)) {
...
void *NCollection_IncAllocator::Reallocate(void *theAddress, const size_t oldSize, const size_t newSize) {
