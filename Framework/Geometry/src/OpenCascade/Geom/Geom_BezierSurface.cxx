...
Standard_Integer Geom_BezierSurface::MaxDegree() { return BSplCLib::MaxDegree(); }
...
void Geom_BezierSurface::InsertPoleColAfter(const Standard_Integer VIndex, const TColgp_Array1OfPnt &CPoles,
  ...
  Handle(TColStd_HArray2OfReal) nweights = new TColStd_HArray2OfReal(1, poles->ColLength(), 1, poles->RowLength() + 1);
...
void Geom_BezierSurface::InsertPoleRowAfter(const Standard_Integer UIndex, const TColgp_Array1OfPnt &CPoles,
  ...
  Handle(TColStd_HArray2OfReal) nweights = new TColStd_HArray2OfReal(1, poles->ColLength() + 1, 1, poles->RowLength());
...
void Geom_BezierSurface::Segment(const Standard_Real U1, const Standard_Real U2, const Standard_Real V1,
  ...
  if (urational || vrational) {
    BSplSLib::BuildCache(uparameter_11, vparameter_11, uspanlenght_11, vspanlenght_11, 0, 0, UDegree(), VDegree(), 0, 0,
                         biduflatknots, bidvflatknots, poles->Array2(), &weights->Array2(), Coefs->ChangeArray2(),
                         &WCoefs->ChangeArray2());
  } else {
    BSplSLib::BuildCache(uparameter_11, vparameter_11, uspanlenght_11, vspanlenght_11, 0, 0, UDegree(), VDegree(), 0, 0,
                         biduflatknots, bidvflatknots, poles->Array2(), BSplSLib::NoWeights(), Coefs->ChangeArray2(),
                         BSplSLib::NoWeights());
  }
...
Standard_Real Geom_BezierSurface::UReversedParameter(const Standard_Real U) const { return (1. - U); }
...
Standard_Real Geom_BezierSurface::VReversedParameter(const Standard_Real V) const { return (1. - V); }
...
GeomAbs_Shape Geom_BezierSurface::Continuity() const { return GeomAbs_CN; }
...
void Geom_BezierSurface::D3(const Standard_Real U, const Standard_Real V, gp_Pnt &P, gp_Vec &D1U, gp_Vec &D1V,
  ...
  if (urational || vrational) {
    BSplSLib::D3(U, V, 0, 0, poles->Array2(), &weights->Array2(), biduknots, bidvknots, &bidumults, &bidvmults,
                 UDegree(), VDegree(), urational, vrational, 0, 0, P, D1U, D1V, D2U, D2V, D2UV, D3U, D3V, D3UUV, D3UVV);
  } else {
...
gp_Vec Geom_BezierSurface::DN(const Standard_Real U, const Standard_Real V, const Standard_Integer Nu,
  ...
  bidvmults.Init(VDegree() + 1);
  if (urational || vrational) {
    BSplSLib::DN(U, V, Nu, Nv, 0, 0, poles->Array2(), &weights->Array2(), biduknots, bidvknots, &bidumults, &bidvmults,
                 UDegree(), VDegree(), urational, vrational, 0, 0, Derivative);
  } else {
...
Standard_Integer Geom_BezierSurface::NbUPoles() const { return poles->ColLength(); }
...
Standard_Integer Geom_BezierSurface::NbVPoles() const { return poles->RowLength(); }
...
Standard_Integer Geom_BezierSurface::UDegree() const { return poles->ColLength() - 1; }
...
Standard_Integer Geom_BezierSurface::VDegree() const { return poles->RowLength() - 1; }
...
Standard_Boolean Geom_BezierSurface::IsCNu(const Standard_Integer) const { return Standard_True; }
...
Standard_Boolean Geom_BezierSurface::IsCNv(const Standard_Integer) const { return Standard_True; }
...
Standard_Boolean Geom_BezierSurface::IsURational() const { return urational; }
...
Standard_Boolean Geom_BezierSurface::IsVRational() const { return vrational; }
...
Standard_Boolean Geom_BezierSurface::IsUPeriodic() const { return Standard_False; }
...
Standard_Boolean Geom_BezierSurface::IsVPeriodic() const { return Standard_False; }
...
void Geom_BezierSurface::Resolution(const Standard_Real Tolerance3D, Standard_Real &UTolerance,
  ...
  if (!maxderivinvok) {
    ...
    bidvmults.Init(VDegree() + 1);
    if (urational || vrational) {
      BSplSLib::Resolution(poles->Array2(), &weights->Array2(), biduknots, bidvknots, bidumults, bidvmults, UDegree(),
                           VDegree(), urational, vrational, 0, 0, 1., umaxderivinv, vmaxderivinv);
    } else {
