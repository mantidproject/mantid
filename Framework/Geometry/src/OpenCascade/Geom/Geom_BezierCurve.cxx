...
Geom_BezierCurve::Geom_BezierCurve(const TColgp_Array1OfPnt &Poles, const TColStd_Array1OfReal &Weights) {
  // copy the poles
  Standard_Integer nbpoles = Poles.Length();
  if (nbpoles < 2 || nbpoles > (Geom_BezierCurve::MaxDegree() + 1))
    throw Standard_ConstructionError();
...
void Geom_BezierCurve::Increase(const Standard_Integer Deg) {
  ...
  if (IsRational()) {
    ...
    BSplCLib::IncreaseDegree(Degree(), Deg, 0, poles->Array1(), &weights->Array1(), bidknots, bidmults,
                             npoles->ChangeArray1(), &nweights->ChangeArray1(), bidknots, bidmults);
  } else {
    BSplCLib::IncreaseDegree(Degree(), Deg, 0, poles->Array1(), BSplCLib::NoWeights(), bidknots, bidmults,
                             npoles->ChangeArray1(), BSplCLib::NoWeights(), bidknots, bidmults);
  }
...
Standard_Integer Geom_BezierCurve::MaxDegree() { return BSplCLib::MaxDegree(); }
...
void Geom_BezierCurve::InsertPoleAfter(const Standard_Integer Index, const gp_Pnt &P) { InsertPoleAfter(Index, P, 1.); }
...
void Geom_BezierCurve::InsertPoleAfter(const Standard_Integer Index, const gp_Pnt &P, const Standard_Real Weight) {
...
void Geom_BezierCurve::InsertPoleBefore(const Standard_Integer Index, const gp_Pnt &P, const Standard_Real Weight) {
...
Standard_Real Geom_BezierCurve::ReversedParameter(const Standard_Real U) const { return (1. - U); }
...
void Geom_BezierCurve::Segment(const Standard_Real U1, const Standard_Real U2) {
  ...
  if (IsRational()) {
    ...
    BSplCLib::BuildCache(0.0, 1.0, 0, Degree(), bidflatknots, poles->Array1(), &weights->Array1(), coeffs, &wcoeffs);
    PLib::Trimming(U1, U2, coeffs, &wcoeffs);
    PLib::CoefficientsPoles(coeffs, &wcoeffs, poles->ChangeArray1(), &weights->ChangeArray1());
  } else {
    BSplCLib::BuildCache(0.0, 1.0, 0, Degree(), bidflatknots, poles->Array1(), BSplCLib::NoWeights(), coeffs,
                         BSplCLib::NoWeights());
...
void Geom_BezierCurve::SetPole(const Standard_Integer Index, const gp_Pnt &P, const Standard_Real Weight) {
  SetPole(Index, P);
...
Standard_Boolean Geom_BezierCurve::IsClosed() const { return closed; }
...
Standard_Boolean Geom_BezierCurve::IsCN(const Standard_Integer) const { return Standard_True; }
...
Standard_Boolean Geom_BezierCurve::IsPeriodic() const { return Standard_False; }
...
Standard_Boolean Geom_BezierCurve::IsRational() const { return !weights.IsNull(); }
...
GeomAbs_Shape Geom_BezierCurve::Continuity() const { return GeomAbs_CN; }
...
Standard_Integer Geom_BezierCurve::Degree() const { return poles->Length() - 1; }
...
void Geom_BezierCurve::D0(const Standard_Real U, gp_Pnt &P) const { BSplCLib::D0(U, Poles(), Weights(), P); }
...
gp_Vec Geom_BezierCurve::DN(const Standard_Real U, const Standard_Integer N) const {
  ...
  if (IsRational())
    ...
    BSplCLib::DN(U, N, 0, Degree(), Standard_False, poles->Array1(), &weights->Array1(), bidknots, &bidmults, V);
  else
    //    BSplCLib::DN(U,N,0,Degree(),0.,
    BSplCLib::DN(U, N, 0, Degree(), Standard_False, poles->Array1(), BSplCLib::NoWeights(), bidknots, &bidmults, V);
  return V;
}
...
gp_Pnt Geom_BezierCurve::StartPoint() const { return poles->Value(1); }
...
gp_Pnt Geom_BezierCurve::EndPoint() const { return poles->Value(poles->Upper()); }
...
Standard_Real Geom_BezierCurve::FirstParameter() const { return 0.0; }
...
Standard_Real Geom_BezierCurve::LastParameter() const { return 1.0; }
...
Standard_Integer Geom_BezierCurve::NbPoles() const { return poles->Length(); }
...
const TColgp_Array1OfPnt &Geom_BezierCurve::Poles() const { return poles->Array1(); }
...
void Geom_BezierCurve::Resolution(const Standard_Real Tolerance3D, Standard_Real &UTolerance) {
  if (!maxderivinvok) {
    ...
    if (IsRational()) {
      BSplCLib::Resolution(poles->Array1(), &weights->Array1(), poles->Length(), bidflatknots, Degree(), 1.,
                           maxderivinv);
    } else {
      BSplCLib::Resolution(poles->Array1(), BSplCLib::NoWeights(), poles->Length(), bidflatknots, Degree(), 1.,
                           maxderivinv);
    }
    maxderivinvok = 1;
  }
...
void Geom_BezierCurve::Init(const Handle(TColgp_HArray1OfPnt) & Poles, const Handle(TColStd_HArray1OfReal) & Weights) {
  Standard_Integer nbpoles = Poles->Length();
  // closed ?
