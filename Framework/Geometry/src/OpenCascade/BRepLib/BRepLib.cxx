...
static void InternalUpdateTolerances(const TopoDS_Shape &theOldShape, const Standard_Boolean IsVerifyTolerance,
...
void BRepLib::Precision(const Standard_Real P) { thePrecision = P; }
...
Standard_Real BRepLib::Precision() { return thePrecision; }
...
void BRepLib::Plane(const Handle(Geom_Plane) & P) { thePlane = P; }
...
Standard_Boolean BRepLib::CheckSameRange(const TopoDS_Edge &AnEdge, const Standard_Real Tolerance) {
  ...
  BRep_ListIteratorOfListOfCurveRepresentation an_Iterator((*((Handle(BRep_TEdge) *)&AnEdge.TShape()))->ChangeCurves());
  ...
  while (IsSameRange && an_Iterator.More()) {
    ...
    if (!geometric_representation_ptr.IsNull()) {
      ...
      if (first_time_in) {
        ...
        IsSameRange = (Abs(current_first - first) <= Tolerance) && (Abs(current_last - last) <= Tolerance);
      }
    }
    an_Iterator.Next();
  }
  ...
  while (an_Iterator.More()) {
    ...
    if (!geometric_representation_ptr.IsNull()) {
      ...
      if (has_curve || has_closed_curve) {
        ...
        if (Abs(first - current_first) > Precision::Confusion() || Abs(last - current_last) > Precision::Confusion()) {
          if (has_curve) {
            GeomLib::SameRange(Tolerance, Curve2dPtr, geometric_representation_ptr->First(),
                               geometric_representation_ptr->Last(), current_first, current_last, NewCurve2dPtr);
            geometric_representation_ptr->PCurve(NewCurve2dPtr);
          }
          if (has_closed_curve) {
            GeomLib::SameRange(Tolerance, Curve2dPtr2, geometric_representation_ptr->First(),
                               geometric_representation_ptr->Last(), current_first, current_last, NewCurve2dPtr2);
...
Standard_Boolean BRepLib::BuildCurve3d(const TopoDS_Edge &AnEdge, const Standard_Real Tolerance,
                                       const GeomAbs_Shape Continuity, const Standard_Integer MaxDegree,
  ...
  if (!P.IsNull()) {
    ...
    if (!BRep_Tool::Degenerated(AnEdge)) {
      jj = 0;
      for (ii = 0; ii < 3; ii++) {
        BRep_Tool::CurveOnSurface(TopoDS::Edge(AnEdge), Curve2dPtr, SurfacePtr, LocalLoc, fc, lc, ii);
      ...
      GeomLib::BuildCurve3d(Tolerance, CurveOnSurface, f, l, NewCurvePtr, max_deviation, average_deviation, Continuity,
                            MaxDegree, evaluateMaxSegment(MaxSegment, CurveOnSurface));
      BRep_Builder B;
...
Standard_Boolean BRepLib::BuildCurves3d(const TopoDS_Shape &S, const Standard_Real Tolerance,
                                        const GeomAbs_Shape Continuity, const Standard_Integer MaxDegree,
                                        const Standard_Integer MaxSegment) {
  ...
  while (ex.More()) {
    if (a_counter.Add(ex.Current())) {
      boolean_value = BuildCurve3d(TopoDS::Edge(ex.Current()), Tolerance, Continuity, MaxDegree, MaxSegment);
      ok = ok && boolean_value;
    }
    ex.Next();
  }
...
Standard_Boolean BRepLib::UpdateEdgeTol(const TopoDS_Edge &AnEdge, const Standard_Real MinToleranceRequested,
                                        const Standard_Real MaxToleranceToCheck) {

  Standard_Integer curve_on_surface_index, curve_index, not_done, has_closed_curve, has_curve, jj, ii,
      geom_reference_curve_flag = 0, max_sampling_points = 90, min_sampling_points = 30;

  Standard_Real factor = 100.0e0,
                //     sampling_array[2],
      safe_factor = 1.4e0, current_last, current_first, max_distance, coded_edge_tolerance, edge_tolerance = 0.0e0;
  ...
  BRep_ListIteratorOfListOfCurveRepresentation an_iterator(list_curve_rep), second_iterator(list_curve_rep);
  ...
  if (!C.IsNull()) {
    ...
    a_sampler.Initialize(geom_reference_curve, MinToleranceRequested * factor, current_first, current_last);
  } else {
    not_done = 1;
    ...
    while (not_done && an_iterator.More()) {
      ...
      if (!geometric_representation_ptr.IsNull() && geometric_representation_ptr->IsCurveOnSurface()) {
    ...
    a_sampler.Initialize(curve_on_surface_reference, MinToleranceRequested * factor, current_first, current_last);
  }
  ...
  while (second_iterator.More()) {
    ...
    if (!geometric_representation_ptr.IsNull() && curve_index != curve_on_surface_index) {
      ...
      if (has_curve || has_closed_curve) {
        ...
        Handle(Geom2dAdaptor_Curve) an_adaptor_curve2d_ptr = new Geom2dAdaptor_Curve(an_adaptor_curve2d);
        Handle(GeomAdaptor_Surface) an_adaptor_surface_ptr = new GeomAdaptor_Surface(an_adaptor_surface);
        ...
        if (BRep_Tool::SameParameter(AnEdge)) {
          ...
          GeomLib::EvalMaxParametricDistance(a_curve_on_surface, geom_reference_curve, MinToleranceRequested,
                                             parameters_ptr->Array1(), max_distance);
        } else if (geom_reference_curve_flag) {
          GeomLib::EvalMaxDistanceAlongParameter(a_curve_on_surface, geom_reference_curve, MinToleranceRequested,
                                                 parameters_ptr->Array1(), max_distance);
        } else {

          GeomLib::EvalMaxDistanceAlongParameter(a_curve_on_surface, curve_on_surface_reference, MinToleranceRequested,
                                                 parameters_ptr->Array1(), max_distance);
        }
...
// purpose  :
//=======================================================================
Standard_Boolean BRepLib::UpdateEdgeTolerance(const TopoDS_Shape &S, const Standard_Real MinToleranceRequested,
                                              const Standard_Real MaxToleranceToCheck) {
  TopExp_Explorer ex(S, TopAbs_EDGE);
  TopTools_MapOfShape a_counter;
  ...
  while (ex.More()) {
    if (a_counter.Add(ex.Current())) {
      local_flag = BRepLib::UpdateEdgeTol(TopoDS::Edge(ex.Current()), MinToleranceRequested, MaxToleranceToCheck);
...
// purpose  :
//=======================================================================
static void GetEdgeTol(const TopoDS_Edge &theEdge, const TopoDS_Face &theFace, Standard_Real &theEdTol) {
  TopLoc_Location L;
...
//=======================================================================
static void UpdTolMap(const TopoDS_Shape &theSh, Standard_Real theNewTol, TopTools_DataMapOfShapeReal &theShToTol) {
...
//=======================================================================
static void UpdShTol(const TopTools_DataMapOfShapeReal &theShToTol, const Standard_Boolean IsMutableInput,
                     BRepTools_ReShape &theReshaper, Standard_Boolean theVForceUpdate) {
  BRep_Builder aB;
...
// purpose  :
//=======================================================================
static void InternalSameParameter(const TopoDS_Shape &theSh, BRepTools_ReShape &theReshaper, const Standard_Real theTol,
                                  const Standard_Boolean IsForced, const Standard_Boolean IsMutableInput) {
...
//================================================================
void BRepLib::SameParameter(const TopoDS_Shape &S, const Standard_Real Tolerance, const Standard_Boolean forced) {
...
// purpose  :
//=======================================================================
void BRepLib::SameParameter(const TopoDS_Shape &S, BRepTools_ReShape &theReshaper, const Standard_Real Tolerance,
...
// purpose  :
//=======================================================================
static Standard_Boolean EvalTol(const Handle(Geom2d_Curve) & pc, const Handle(Geom_Surface) & s,
                                const GeomAdaptor_Curve &gac, const Standard_Real tol, Standard_Real &tolbail) {
...
// purpose  :
//=======================================================================
static Standard_Real ComputeTol(const Handle(Adaptor3d_Curve) & c3d, const Handle(Adaptor2d_Curve2d) & c2d,
                                const Handle(Adaptor3d_Surface) & surf, const Standard_Integer nbp)

{
  ...
  Standard_Real uf = surf->FirstUParameter(), ul = surf->LastUParameter(), vf = surf->FirstVParameter(),
...
// purpose  :
//=======================================================================
TopoDS_Edge BRepLib::SameParameter(const TopoDS_Edge &theEdge, const Standard_Real theTolerance,
                                   Standard_Real &theNewTol, Standard_Boolean IsUseOldEdge) {
        ...
        if (GAC2d.GetType() == GeomAbs_BSplineCurve && GAC2d.Continuity() == GeomAbs_C0) {
          ...
          if (goodpc) {
            ...
            if (IsBad) { // if BSpline "bad", try to reparametrize it
              ...
              if (maxdeg == 1)
                maxdeg = 14;
              Approx_CurvilinearParameter AppCurPar(HC2d, HS, Max(1.e-3, theTolerance), cont, maxdeg, 10);
...
// purpose  :
//=======================================================================
void BRepLib::UpdateTolerances(const TopoDS_Shape &S, BRepTools_ReShape &theReshaper,
    ...
    if (!BRep_Tool::Degenerated(anEdge) && EFmap(i).Extent() > 0) {
      ...
      for (Standard_Integer j = 2; j <= theRep.Length(); j++) {
        for (Standard_Integer k = 0; k <= NbSamples; k++) {
          ...
          Standard_Real ParamOnCurve = (BRep_Tool::SameParameter(anEdge))
                                           ? ParamOnCenter
...
public:
  SurfaceProperties(const Handle(Geom_Surface) & theSurface, const gp_Trsf &theSurfaceTrsf,
                    const Handle(Geom2d_Curve) & theCurve2D, const Standard_Boolean theReversed)
      : mySurfaceProps(theSurface, 2, Precision::Confusion()), mySurfaceTrsf(theSurfaceTrsf), myCurve2d(theCurve2D),
  ...
  void Curvature(gp_Dir &thePrincipalDir1, Standard_Real &theCurvature1, gp_Dir &thePrincipalDir2,
...
//=======================================================================
GeomAbs_Shape BRepLib::ContinuityOfFaces(const TopoDS_Edge &theEdge, const TopoDS_Face &theFace1,
                                         const TopoDS_Face &theFace2, const Standard_Real theAngleTol) {
  ...
  if (!theFace1.IsSame(theFace2) && BRep_Tool::IsClosed(theEdge, theFace1) && BRep_Tool::IsClosed(theEdge, theFace2)) {
...
//=======================================================================
static void EncodeRegularity(const TopoDS_Shape &theShape, const Standard_Real theTolAng, TopTools_MapOfShape &theMap,
...
void BRepLib::EncodeRegularity(const TopoDS_Shape &S, const TopTools_ListOfShape &LE, const Standard_Real TolAng) {
...
void BRepLib::EncodeRegularity(TopoDS_Edge &E, const TopoDS_Face &F1, const TopoDS_Face &F2,
...
//=======================================================================
Standard_Boolean BRepLib::EnsureNormalConsistency(const TopoDS_Shape &theShape, const Standard_Real theAngTol,
  ...
  for (Standard_Integer anInd = 1; anInd <= aMapEF.Extent(); anInd++) {
    ...
    const Handle(Poly_PolygonOnTriangulation) &aPTEF1 = BRep_Tool::PolygonOnTriangulation(anEdg, aPT1, aLoc1);
    const Handle(Poly_PolygonOnTriangulation) &aPTEF2 = BRep_Tool::PolygonOnTriangulation(anEdg, aPT2, aLoc2);

    if (aPTEF1->Nodes().Lower() != aPTEF2->Nodes().Lower() || aPTEF1->Nodes().Upper() != aPTEF2->Nodes().Upper())
      continue;

    for (Standard_Integer anEdgNode = aPTEF1->Nodes().Lower(); anEdgNode <= aPTEF1->Nodes().Upper(); anEdgNode++) {
      // Number of node
...
struct EvalDeflection {
  ...
  EvalDeflection(const TopoDS_Face &theFace) : Surface(theFace) {}
...
void BRepLib::UpdateDeflection(const TopoDS_Shape &theShape) {
  ...
  for (; anExpFace.More(); anExpFace.Next()) {
    ...
    for (Standard_Integer aTriIt = 1; aTriIt <= aPT->NbTriangles(); ++aTriIt) {
      ...
      if (aDegNodes.Contains(aNode[0]) || aDegNodes.Contains(aNode[1]) || aDegNodes.Contains(aNode[2])) {
        continue;
      }

      const gp_Pnt aP3d[3] = {aPT->Node(aNode[0]).Transformed(aTrsf), aPT->Node(aNode[1]).Transformed(aTrsf),
                              aPT->Node(aNode[2]).Transformed(aTrsf)};

      const gp_Pnt2d aP2d[3] = {aPT->UVNode(aNode[0]), aPT->UVNode(aNode[1]), aPT->UVNode(aNode[2])};
...
void BRepLib::ReverseSortFaces(const TopoDS_Shape &Sh, TopTools_ListOfShape &LF) {
  ...
  TopTools_ListOfShape LTri(LF.Allocator()), LPlan(LF.Allocator()), LCyl(LF.Allocator()), LCon(LF.Allocator()),
      LSphere(LF.Allocator()), LTor(LF.Allocator()), LOther(LF.Allocator());
  TopExp_Explorer exp(Sh, TopAbs_FACE);
  TopLoc_Location l;
...
// purpose :
//=======================================================================
void BRepLib::BoundingVertex(const NCollection_List<TopoDS_Shape> &theLV, gp_Pnt &theNewCenter,
...
// purpose  :
//=======================================================================
void BRepLib::ExtendFace(const TopoDS_Face &theF, const Standard_Real theExtVal, const Standard_Boolean theExtUMin,
                         const Standard_Boolean theExtUMax, const Standard_Boolean theExtVMin,
                         const Standard_Boolean theExtVMax, TopoDS_Face &theFExtended) {
  // Get face bounds
  BRepAdaptor_Surface aBAS(theF);
  Standard_Real aFUMin = aBAS.FirstUParameter(), aFUMax = aBAS.LastUParameter(), aFVMin = aBAS.FirstVParameter(),
  ...
  if (aType == GeomAbs_Plane || aType == GeomAbs_Sphere || aType == GeomAbs_Cylinder || aType == GeomAbs_Torus ||
      aType == GeomAbs_Cone) {
    // Get basis transformed basis surface
    Handle(Geom_Surface) aSurf = Handle(Geom_Surface)::DownCast(aBAS.Surface().Surface()->Transformed(aBAS.Trsf()));
    ...
    Handle(Geom_BoundedSurface) aSB = Handle(Geom_BoundedSurface)::DownCast(BRep_Tool::Surface(theF));
    if (aSB.IsNull()) {
      theFExtended = theF;
      return;
    }
    ...
    Standard_Boolean isExtUMin = Standard_False, isExtUMax = Standard_False, isExtVMin = Standard_False,
