    ...
    for (; aFreeEdgesIt.More(); aFreeEdgesIt.Next()) {
      ...
      for (Standard_Integer aCurEdgeIdx = 0; aCurEdgeIdx < 3; ++aCurEdgeIdx) {
        ...
        for (Standard_Integer anOtherEdgeIt = 1; anOtherEdgeIt <= 2 && isCanNotBeRemoved; ++anOtherEdgeIt) {
          ...
          const BRepMesh_PairOfIndex &anOtherEdgePair = myMeshData->ElementsConnectedTo(anEdges[anOtherEdgeId]);
...
Standard_Boolean BRepMesh_Delaun::meshLeftPolygonOf(const Standard_Integer theStartEdgeId,
  ...
  while (aPivotNode != aFirstNode) {
    ...
    Standard_Integer aNextLinkId =
        findNextPolygonLink(aFirstNode, aPivotNode, aPivotVertex, aRefLinkDir, aBoxes, aPolygon, theSkipped,
                            isSkipLeprous, aLeprousLinks, aDeadLinks, aNextPivotNode, aNextLinkDir, aNextLinkBndBox);
...
Standard_Integer BRepMesh_Delaun::findNextPolygonLink(
  ...
  for (; aLinkIt.More(); aLinkIt.Next()) {
    ...
    if (theDeadLinks.Contains(aNeighbourLinkId) || (!theSkipped.IsNull() && theSkipped->Contains(aNeighbourLinkId))) {
      continue;
    }
    ...
    if (aNeighbourLink.Movability() == BRepMesh_Free && myMeshData->ElementsConnectedTo(aNeighbourLinkInfo).IsEmpty()) {
...
void BRepMesh_Delaun::cleanupPolygon(const IMeshData::SequenceOfInteger &thePolygon,
  ...
  for (; aPolyVertIt < anUniqueVerticesNum; ++aPolyVertIt) {
    killTrianglesAroundVertex(aPolyVertices(aPolyVertIt), aPolyVertices, aPolyVerticesFindMap, thePolygon, thePolyBoxes,
                              aSurvivedLinks, aLoopEdges);
  }
...
void BRepMesh_Delaun::killTrianglesOnIntersectingLinks(
  ...
  Standard_Boolean isNotIntersect =
      checkIntersection(theLinkToCheck, thePolygon, thePolyBoxes, Standard_False, Standard_False, Standard_False, aBox);
...
void BRepMesh_Delaun::meshPolygon(IMeshData::SequenceOfInteger &thePolygon, IMeshData::SequenceOfBndB2d &thePolyBoxes,
  ...
  for (Standard_Integer aPolyIt = 1; aPolyIt < aPolyLen; ++aPolyIt) {
    ...
    for (; aNextPolyIt <= aPolyLen; ++aNextPolyIt) {
      ...
      gp_Pnt2d anIntPnt;
      BRepMesh_GeomTool::IntFlag aIntFlag = intSegSeg(*aCurEdge, *aNextEdge, Standard_False, Standard_True, anIntPnt);
      ...
      if (aIntFlag == BRepMesh_GeomTool::Cross) {
        ...
        if (Abs(aLoopArea) > aSmallLoopArea) {
          ...
          aNextEdgeId = Abs(createAndReplacePolygonLink(aNextNodes, aNextPnts, aNextPolyIt, BRepMesh_Delaun::Replace,
        ...
        if (isFirstChopping) {
          ...
          for (; aSplitLinkIt < 2; ++aSplitLinkIt) {
            createAndReplacePolygonLink(&aSplitLink[aSplitLinkIt], &aSplitPnts[aSplitLinkIt], aNextPolyIt,
                                        (aSplitLinkIt == 0) ? BRepMesh_Delaun::Replace : BRepMesh_Delaun::InsertAfter,
                                        thePolygon, thePolyBoxes);
          }
      ...
      if (isAddReplacingEdge) {
        aCurEdgeId = Abs(createAndReplacePolygonLink(aCurNodes, aCurPnts, aPolyIt, BRepMesh_Delaun::Replace, thePolygon,
                                                     thePolyBoxes));
...
Handle(IMeshData::MapOfInteger) BRepMesh_Delaun::getEdgesByType(const BRepMesh_DegreeOfFreedom theEdgeType) const {
  ...
  for (; anEdgeIt.More(); anEdgeIt.Next()) {
    ...
    Standard_Boolean isToAdd = (theEdgeType == BRepMesh_Free) ? (myMeshData->ElementsConnectedTo(anEdge).Extent() <= 1)
