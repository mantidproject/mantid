...
void BRepMesh_DelabellaBaseMeshAlgo::buildBaseTriangulation() {
  ...
  try {
    const int aVerticesNb = aTriangulator->Triangulate(static_cast<int>(aPoints.size() / 2), &aPoints[0], &aPoints[1],
    ...
    if (aVerticesNb > 0) {
      ...
      while (aTrianglePtr != NULL) {
        Standard_Integer aNodes[3] = {aTrianglePtr->v[0]->i + 1, aTrianglePtr->v[2]->i + 1, aTrianglePtr->v[1]->i + 1};
