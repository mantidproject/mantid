...
BRepMesh_CurveTessellator::BRepMesh_CurveTessellator(const IMeshData::IEdgeHandle &theEdge,
    ...
    : myDEdge(theEdge), myParameters(theParameters), myEdge(TopoDS::Edge(theEdge->GetEdge().Oriented(theOrientation))),
      myCurve(myEdge, theFace->GetFace()), myMinPointsNb(theMinPointsNb) {
  init();
}
...
void BRepMesh_CurveTessellator::init() {
  ...
  if (myParameters.AdjustMinSize) {
    aMinSize = Min(aMinSize,
                   myParameters.RelMinSize() * GCPnts_AbscissaPoint::Length(myCurve, myCurve.FirstParameter(),
...
// purpose  :
//=======================================================================
Standard_Integer BRepMesh_CurveTessellator::PointsNb() const { return myDiscretTool.NbPoints(); }
