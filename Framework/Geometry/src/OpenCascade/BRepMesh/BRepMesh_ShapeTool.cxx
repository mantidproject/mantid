...
struct EdgeTolerance {
  static Standard_Real Get(const TopoDS_Shape &theEdge) { return BRep_Tool::Tolerance(TopoDS::Edge(theEdge)); }
};

//! Auxiliary struct to take a tolerance of vertex.
struct VertexTolerance {
  static Standard_Real Get(const TopoDS_Shape &theVertex) { return BRep_Tool::Tolerance(TopoDS::Vertex(theVertex)); }
};
...
void BRepMesh_ShapeTool::CheckAndUpdateFlags(const IMeshData::IEdgeHandle &theEdge,
  ...
  if (theEdge->GetSameParam() || theEdge->GetSameRange()) {
    if (theEdge->GetSameRange()) {
      ...
      theEdge->SetSameRange(Abs(aDiffFirst) < Precision::PConfusion() && Abs(aDiffLast) < Precision::PConfusion());
