...
TopoDS_Edge BRepBuilderAPI_Sewing::SameParameterEdge(const TopoDS_Edge &edgeFirst, const TopoDS_Edge &edgeLast,
      ...
      if (surf2 == surf1) {
        ...
        if (!loc2.IsDifferent(loc1)) {
          ...
          if (uclosed || vclosed) {
            ...
            isSeam = ((uclosed && aDist > 0.75 * (fabs(U2 - U1))) || (vclosed && aDist > 0.75 * (fabs(V2 - V1))));
  ...
  if (firstCall && (!isResEdge || !isSamePar || tolReached > myTolerance)) {
    ...
    TopoDS_Edge s_edge =
        SameParameterEdge(edgeFirst, edgeLast, listFacesFirst, listFacesLast, secForward, whichSecn, Standard_False);
    if (!s_edge.IsNull()) {
...
const TopoDS_Shape &BRepBuilderAPI_Sewing::SewedShape() const { return mySewedShape; }
...
Standard_Integer BRepBuilderAPI_Sewing::NbFreeEdges() const { return myFreeEdges.Extent(); }
...
Standard_Integer BRepBuilderAPI_Sewing::NbMultipleEdges() const { return myMultipleEdges.Extent(); }
...
Standard_Integer BRepBuilderAPI_Sewing::NbContigousEdges() const { return myContigousEdges.Extent(); }
...
// purpose  :
//=======================================================================
Standard_Integer BRepBuilderAPI_Sewing::NbDeletedFaces() const { return myLittleFace.Extent(); }
...
Standard_Integer BRepBuilderAPI_Sewing::NbDegeneratedShapes() const { return myDegenerated.Extent(); }
  ...
  if (!wire1.IsNull() && !wire2.IsNull()) {
    if (wire1.IsSame(wire2)) {
      ...
      if (IsClosedShape(wire1, vtx1, vtx2)) {
        ...
        Standard_Boolean isEndVertex = ((V1.IsSame(vtx1) && V2.IsSame(vtx2)) || (V2.IsSame(vtx1) && V1.IsSame(vtx2)));
        if (!isEndVertex)
          Status = 1;
      } else
        Status = 1;
      ...
      for (; its.More(); its.Next()) {
        ...
        if (MergedNearestEdges(section, seqMergedWithSection, seqMergedWithSectionOri)) {
          ...
          while (ii <= seqMergedWithSection.Length()) {
            ...
            // Remove edge if recorded as merged
            Standard_Boolean isRejected = (myMergedEdges.Contains(iedge) || MergedWithSections.Contains(iedge));
            if (!isRejected) {
              if (myBoundSections.IsBound(iedge)) {
                // Edge is split - check sections
                TopTools_ListIteratorOfListOfShape lit(myBoundSections(iedge));
          ...
          if (nbMerged) {
            // Create same parameter edge
            TopTools_MapOfShape ActuallyMerged;
            TopoDS_Edge MergedEdge = SameParameterEdge(section, seqMergedWithSection, seqMergedWithSectionOri,
...
Standard_Boolean BRepBuilderAPI_Sewing::MergedNearestEdges(const TopoDS_Shape &edge,
  ...
  for (Standard_Integer i = 1; i <= mapVert1.Extent(); i++) {
    ...
    for (; ilsec.More(); ilsec.Next()) {
      ...
      if ((mapVert1.Contains(vs1n) && mapVert2.Contains(vs2n)) || (mapVert1.Contains(vs2n) && mapVert2.Contains(vs1n)))
        if (mapEdges.Add(sec)) {
...
// purpose  :
//=======================================================================
const Handle(BRepTools_ReShape) & BRepBuilderAPI_Sewing::GetContext() const { return myReShape; }
