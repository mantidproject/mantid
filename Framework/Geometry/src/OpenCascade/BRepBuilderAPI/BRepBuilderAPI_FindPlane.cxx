...
BRepBuilderAPI_FindPlane::BRepBuilderAPI_FindPlane(const TopoDS_Shape &S, const Standard_Real Tol) { Init(S, Tol); }
    ...
    if (points.Length() > 2) {
      ...
      for (Standard_Integer i = 2; i <= points.Length(); i++) {
        ...
        if (dist > disMax) {
          disMax = dist;
          p1 = points(
              i); // it will be faster to store the point, otherwise it is necessary to find a value in a sequence
        }
      }
...
Standard_Boolean BRepBuilderAPI_FindPlane::Found() const { return !myPlane.IsNull(); }
...
Handle(Geom_Plane) BRepBuilderAPI_FindPlane::Plane() const { return myPlane; }
