...
struct Edge {
  ...
  Edge(const Standard_Integer TheIdx1, const Standard_Integer TheIdx2)
      : Idx1(Min(TheIdx1, TheIdx2)), Idx2(Max(TheIdx1, TheIdx2)) {}
...
struct EdgeHasher {
  ...
  static Standard_Integer HashCode(const Edge &theEdge, const Standard_Integer theUpperBound) {
  ...
  static Standard_Boolean IsEqual(const Edge &theEdge1, const Edge &theEdge2) {
...
void BRepBuilderAPI_MakeShapeOnMesh::Build(const Message_ProgressRange &theRange) {
  ...
  Message_ProgressScope aPS(theRange, "Per-facet shape construction", Standard_Real(aNbNodes + 2 * aNbTriangles));
  ...
  for (Standard_Integer i = 1; i <= aNbTriangles; ++i) {
    ...
    const Standard_Boolean aHasAllEdges = anEdgeToTEgeMap.Contains(aMeshEdge1) &&
                                          anEdgeToTEgeMap.Contains(aMeshEdge2) && anEdgeToTEgeMap.Contains(aMeshEdge3);
    if (!aHasAllEdges)
      continue;
