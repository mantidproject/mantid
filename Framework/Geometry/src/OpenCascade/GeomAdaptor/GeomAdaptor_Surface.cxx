...
void GeomAdaptor_Surface::load(const Handle(Geom_Surface) & S, const Standard_Real UFirst, const Standard_Real ULast,
  ...
  if (mySurface != S) {
    ...
    if (TheType == STANDARD_TYPE(Geom_RectangularTrimmedSurface)) {
      ...
      Handle(Geom_SurfaceOfLinearExtrusion) myExtSurf = Handle(Geom_SurfaceOfLinearExtrusion)::DownCast(mySurface);
      // Create nested adaptor for base curve
...
Standard_Integer GeomAdaptor_Surface::NbUIntervals(const GeomAbs_Shape S) const {
  switch (mySurfaceType) {
  case GeomAbs_BSplineSurface: {
    GeomAdaptor_Curve myBasisCurve(myBSplineSurface->VIso(myBSplineSurface->VKnot(myBSplineSurface->FirstVKnotIndex())),
                                   myUFirst, myULast);
...
Standard_Integer GeomAdaptor_Surface::NbVIntervals(const GeomAbs_Shape S) const {
  switch (mySurfaceType) {
  case GeomAbs_BSplineSurface: {
    GeomAdaptor_Curve myBasisCurve(myBSplineSurface->UIso(myBSplineSurface->UKnot(myBSplineSurface->FirstUKnotIndex())),
                                   myVFirst, myVLast);
    return myBasisCurve.NbIntervals(S);
  }
  case GeomAbs_SurfaceOfRevolution: {
...
void GeomAdaptor_Surface::UIntervals(TColStd_Array1OfReal &T, const GeomAbs_Shape S) const {
  ...
  case GeomAbs_SurfaceOfExtrusion: {
    GeomAdaptor_Curve myBasisCurve(Handle(Geom_SurfaceOfLinearExtrusion)::DownCast(mySurface)->BasisCurve(), myUFirst,
                                   myULast);
    if (myBasisCurve.GetType() == GeomAbs_BSplineCurve) {
      myNbUIntervals = myBasisCurve.NbIntervals(S);
      myBasisCurve.Intervals(T, S);
...
Standard_Boolean GeomAdaptor_Surface::IsUPeriodic() const { return (mySurface->IsUPeriodic()); }
...
Standard_Boolean GeomAdaptor_Surface::IsVPeriodic() const { return (mySurface->IsVPeriodic()); }
...
void GeomAdaptor_Surface::RebuildCache(const Standard_Real theU, const Standard_Real theV) const {
  if (mySurfaceType == GeomAbs_BezierSurface) {
    ...
    if (mySurfaceCache.IsNull())
      mySurfaceCache = new BSplSLib_Cache(aDegU, aBezier->IsUPeriodic(), aFlatKnotsU, aDegV, aBezier->IsVPeriodic(),
                                          aFlatKnotsV, aBezier->Weights());
...
void GeomAdaptor_Surface::D0(const Standard_Real U, const Standard_Real V, gp_Pnt &P) const {
  ...
  case GeomAbs_SurfaceOfExtrusion:
  case GeomAbs_SurfaceOfRevolution:
    Standard_NoSuchObject_Raise_if(myNestedEvaluator.IsNull(), "GeomAdaptor_Surface::D0: evaluator is not initialized");
...
void GeomAdaptor_Surface::D1(const Standard_Real U, const Standard_Real V, gp_Pnt &P, gp_Vec &D1U, gp_Vec &D1V) const {
  ...
  case GeomAbs_SurfaceOfRevolution:
  case GeomAbs_OffsetSurface:
    Standard_NoSuchObject_Raise_if(myNestedEvaluator.IsNull(), "GeomAdaptor_Surface::D1: evaluator is not initialized");
...
void GeomAdaptor_Surface::D2(const Standard_Real U, const Standard_Real V, gp_Pnt &P, gp_Vec &D1U, gp_Vec &D1V,
  ...
  case GeomAbs_SurfaceOfRevolution:
  case GeomAbs_OffsetSurface:
    Standard_NoSuchObject_Raise_if(myNestedEvaluator.IsNull(), "GeomAdaptor_Surface::D2: evaluator is not initialized");
...
void GeomAdaptor_Surface::D3(const Standard_Real U, const Standard_Real V, gp_Pnt &P, gp_Vec &D1U, gp_Vec &D1V,
  ...
  case GeomAbs_BSplineSurface: {
    if ((USide == 0) && (VSide == 0))
      ...
      if (IfUVBound(u, v, Ideb, Ifin, IVdeb, IVfin, USide, VSide))
        myBSplineSurface->LocalD3(u, v, Ideb, Ifin, IVdeb, IVfin, P, D1U, D1V, D2U, D2V, D2UV, D3U, D3V, D3UUV, D3UVV);
      else
        myBSplineSurface->D3(u, v, P, D1U, D1V, D2U, D2V, D2UV, D3U, D3V, D3UUV, D3UVV);
    }
    break;
  ...
  case GeomAbs_SurfaceOfRevolution:
  case GeomAbs_OffsetSurface:
    Standard_NoSuchObject_Raise_if(myNestedEvaluator.IsNull(), "GeomAdaptor_Surface::D3: evaluator is not initialized");
...
gp_Vec GeomAdaptor_Surface::DN(const Standard_Real U, const Standard_Real V, const Standard_Integer Nu,
  ...
  case GeomAbs_SurfaceOfRevolution:
  case GeomAbs_OffsetSurface:
    Standard_NoSuchObject_Raise_if(myNestedEvaluator.IsNull(), "GeomAdaptor_Surface::DN: evaluator is not initialized");
