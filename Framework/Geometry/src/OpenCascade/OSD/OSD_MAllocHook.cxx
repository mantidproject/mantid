...
OSD_MAllocHook::Callback *OSD_MAllocHook::GetCallback() { return MypCurrentCallback; }
...
static long getRequestNum(void * /*pvData*/, long lRequest, size_t & /*theSize*/) { return lRequest; }
...
void OSD_MAllocHook::SetCallback(Callback *theCB) { MypCurrentCallback = theCB; }
...
OSD_MAllocHook::LogFileHandler::~LogFileHandler() { Close(); }
...
Standard_Boolean OSD_MAllocHook::LogFileHandler::MakeReport(const char *theLogFile, const char *theOutFile,
  ...
  for (std::set<StorageInfo>::const_iterator it = aStMap.begin(); it != aStMap.end(); ++it) {
    ...
    aRepFile << std::setw(20) << aInfo.size << ' ' << std::setw(10) << aInfo.nbAlloc << ' ' << std::setw(10) << nbLeft
             << ' ' << std::setw(10) << aInfo.nbLeftPeak << ' ' << std::setw(20) << aSizeAlloc << ' ' << std::setw(20)
             << aSizeLeft << ' ' << std::setw(20) << aSizePeak << std::endl;
...
Standard_Boolean OSD_MAllocHook::CollectBySize::MakeReport(const char *theOutFile) {
  ...
  for (int i = 0; i < MAX_ALLOC_SIZE; i++) {
    if (myArray[i].nbAlloc > 0 || myArray[i].nbFree > 0) {
      ...
      aRepFile << std::setw(10) << aSize << ' ' << std::setw(10) << myArray[i].nbAlloc << ' ' << std::setw(10) << nbLeft
               << ' ' << std::setw(10) << myArray[i].nbLeftPeak << ' ' << std::setw(20) << aSizeAlloc << ' '
