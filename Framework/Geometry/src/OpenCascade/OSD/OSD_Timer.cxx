...
static void timeToHoursMinutesSeconds(Standard_Real theTimeSec, Standard_Integer &theHours,
                                      Standard_Integer &theMinutes, Standard_Real &theSeconds) {
...
Standard_Real OSD_Timer::GetWallClockTime() {
  ...
  return gettimeofday(&aTime, NULL) == 0 ? (aTime.tv_sec - aStartSec) + 0.000001 * aTime.tv_usec : 0.0;
#endif
}
...
OSD_Timer::OSD_Timer(Standard_Boolean theThisThreadOnly)
    : OSD_Chronometer(theThisThreadOnly), myTimeStart(0.0), myTimeCumul(0.0) {
  //
}
...
void OSD_Timer::Show() const { Show(std::cout); }
...
void OSD_Timer::Show(Standard_Real &theSeconds, Standard_Integer &theMinutes, Standard_Integer &theHours,
                     Standard_Real &theCPUtime) const {
  const Standard_Real aTimeCumul = myIsStopped ? myTimeCumul : myTimeCumul + GetWallClockTime() - myTimeStart;
...
void OSD_Timer::Show(Standard_OStream &theOStream) const {
  ...
  theOStream << "Elapsed time: " << anHours << " Hours " << aMinutes << " Minutes " << aSeconds << " Seconds\n";
