...
// purpose  :
//=======================================================================
const Handle(Poly_Triangulation) & BRep_Tool::Triangulation(const TopoDS_Face &theFace, TopLoc_Location &theLocation,
...
// purpose  :
//=======================================================================
const Poly_ListOfTriangulation &BRep_Tool::Triangulations(const TopoDS_Face &theFace, TopLoc_Location &theLocation) {
  theLocation = theFace.Location();
  const BRep_TFace *aTFace = static_cast<const BRep_TFace *>(theFace.TShape().get());
...
const Handle(Geom_Curve) & BRep_Tool::Curve(const TopoDS_Edge &E, TopLoc_Location &L, Standard_Real &First,
                                            Standard_Real &Last) {
...
Handle(Geom2d_Curve) BRep_Tool::CurveOnSurface(const TopoDS_Edge &E, const TopoDS_Face &F, Standard_Real &First,
                                               Standard_Real &Last, Standard_Boolean *theIsStored) {
  TopLoc_Location l;
...
Handle(Geom2d_Curve) BRep_Tool::CurveOnSurface(const TopoDS_Edge &E, const Handle(Geom_Surface) & S,
                                               const TopLoc_Location &L, Standard_Real &First, Standard_Real &Last,
...
//=======================================================================
Handle(Geom2d_Curve) BRep_Tool::CurveOnPlane(const TopoDS_Edge &E, const Handle(Geom_Surface) & S,
                                             const TopLoc_Location &L, Standard_Real &First, Standard_Real &Last) {
  First = Last = 0.;
  ...
  Handle(Geom_Curve) ProjOnPlane = GeomProjLib::ProjectOnPlane(
      new Geom_TrimmedCurve(C3D, f, l, Standard_True, Standard_False), GP, GP->Position().Direction(), Standard_True);
...
void BRep_Tool::CurveOnSurface(const TopoDS_Edge &E, Handle(Geom2d_Curve) & C, Handle(Geom_Surface) & S,
                               TopLoc_Location &L, Standard_Real &First, Standard_Real &Last) {
  // find the representation
  const BRep_TEdge *TE = static_cast<const BRep_TEdge *>(E.TShape().get());
  BRep_ListIteratorOfListOfCurveRepresentation itcr(TE->Curves());
...
Handle(Poly_Polygon2D) BRep_Tool::PolygonOnSurface(const TopoDS_Edge &E, const Handle(Geom_Surface) & S,
...
void BRep_Tool::PolygonOnSurface(const TopoDS_Edge &E, Handle(Poly_Polygon2D) & P, Handle(Geom_Surface) & S,
                                 TopLoc_Location &L) {
  // find the representation
  const BRep_TEdge *TE = static_cast<const BRep_TEdge *>(E.TShape().get());
  BRep_ListIteratorOfListOfCurveRepresentation itcr(TE->Curves());
...
void BRep_Tool::PolygonOnSurface(const TopoDS_Edge &E, Handle(Poly_Polygon2D) & P, Handle(Geom_Surface) & S,
                                 TopLoc_Location &L, const Standard_Integer Index) {
  Standard_Integer i = 0;
...
const Handle(Poly_PolygonOnTriangulation) & BRep_Tool::PolygonOnTriangulation(const TopoDS_Edge &E,
...
void BRep_Tool::PolygonOnTriangulation(const TopoDS_Edge &E, Handle(Poly_PolygonOnTriangulation) & P,
                                       Handle(Poly_Triangulation) & T, TopLoc_Location &L) {
  // find the representation
  const BRep_TEdge *TE = static_cast<const BRep_TEdge *>(E.TShape().get());
  BRep_ListIteratorOfListOfCurveRepresentation itcr(TE->Curves());
  ...
  while (itcr.More()) {
    ...
    if (cr->IsPolygonOnTriangulation()) {
      const BRep_PolygonOnTriangulation *PT = static_cast<const BRep_PolygonOnTriangulation *>(cr.get());
...
Standard_Boolean BRep_Tool::IsClosed(const TopoDS_Edge &E, const Handle(Geom_Surface) & S, const TopLoc_Location &L) {
...
Standard_Boolean BRep_Tool::IsClosed(const TopoDS_Edge &E, const Handle(Poly_Triangulation) & T,
                                     const TopLoc_Location &L) {
  TopLoc_Location l = L.Predivided(E.Location());
...
void BRep_Tool::Range(const TopoDS_Edge &E, const Handle(Geom_Surface) & S, const TopLoc_Location &L,
...
void BRep_Tool::Range(const TopoDS_Edge &E, const TopoDS_Face &F, Standard_Real &First, Standard_Real &Last) {
  TopLoc_Location L;
  const Handle(Geom_Surface) &S = BRep_Tool::Surface(F, L);
...
void BRep_Tool::UVPoints(const TopoDS_Edge &E, const Handle(Geom_Surface) & S, const TopLoc_Location &L,
                         gp_Pnt2d &PFirst, gp_Pnt2d &PLast) {
  TopLoc_Location l = L.Predivided(E.Location());
  Standard_Boolean Eisreversed = (E.Orientation() == TopAbs_REVERSED);
  ...
  while (itcr.More()) {
    ...
    if (cr->IsCurveOnSurface(S, l)) {
      if (cr->IsCurveOnClosedSurface() && Eisreversed) {
        const BRep_CurveOnClosedSurface *CR = static_cast<const BRep_CurveOnClosedSurface *>(cr.get());
...
void BRep_Tool::UVPoints(const TopoDS_Edge &E, const TopoDS_Face &F, gp_Pnt2d &PFirst, gp_Pnt2d &PLast) {
  TopLoc_Location L;
  const Handle(Geom_Surface) &S = BRep_Tool::Surface(F, L);
  TopoDS_Edge aLocalEdge = E;
  if (F.Orientation() == TopAbs_REVERSED) {
...
void BRep_Tool::SetUVPoints(const TopoDS_Edge &E, const Handle(Geom_Surface) & S, const TopLoc_Location &L,
                            const gp_Pnt2d &PFirst, const gp_Pnt2d &PLast) {
  TopLoc_Location l = L.Predivided(E.Location());
  Standard_Boolean Eisreversed = (E.Orientation() == TopAbs_REVERSED);
...
void BRep_Tool::SetUVPoints(const TopoDS_Edge &E, const TopoDS_Face &F, const gp_Pnt2d &PFirst, const gp_Pnt2d &PLast) {
  TopLoc_Location L;
  const Handle(Geom_Surface) &S = BRep_Tool::Surface(F, L);
  TopoDS_Edge aLocalEdge = E;
  if (F.Orientation() == TopAbs_REVERSED) {
...
Standard_Boolean BRep_Tool::HasContinuity(const TopoDS_Edge &E, const TopoDS_Face &F1, const TopoDS_Face &F2) {
...
GeomAbs_Shape BRep_Tool::Continuity(const TopoDS_Edge &E, const TopoDS_Face &F1, const TopoDS_Face &F2) {
  TopLoc_Location l1, l2;
  const Handle(Geom_Surface) &S1 = BRep_Tool::Surface(F1, l1);
  const Handle(Geom_Surface) &S2 = BRep_Tool::Surface(F2, l2);
...
Standard_Boolean BRep_Tool::HasContinuity(const TopoDS_Edge &E, const Handle(Geom_Surface) & S1,
                                          const Handle(Geom_Surface) & S2, const TopLoc_Location &L1,
...
GeomAbs_Shape BRep_Tool::Continuity(const TopoDS_Edge &E, const Handle(Geom_Surface) & S1,
                                    const Handle(Geom_Surface) & S2, const TopLoc_Location &L1,
                                    const TopLoc_Location &L2) {
  ...
  // find the representation
  BRep_ListIteratorOfListOfCurveRepresentation itcr((*((Handle(BRep_TEdge) *)&E.TShape()))->ChangeCurves());
...
Standard_Boolean BRep_Tool::Parameter(const TopoDS_Vertex &theV, const TopoDS_Edge &theE, Standard_Real &theParam) {
...
Standard_Real BRep_Tool::Parameter(const TopoDS_Vertex &V, const TopoDS_Edge &E, const TopoDS_Face &F) {
  TopLoc_Location L;
  const Handle(Geom_Surface) &S = BRep_Tool::Surface(F, L);
...
Standard_Real BRep_Tool::Parameter(const TopoDS_Vertex &V, const TopoDS_Edge &E, const Handle(Geom_Surface) & S,
                                   const TopLoc_Location &L) {
  // Search the vertex in the edge
...
// purpose  :
//=======================================================================
Standard_Real BRep_Tool::MaxTolerance(const TopoDS_Shape &theShape, const TopAbs_ShapeEnum theSubShape) {
