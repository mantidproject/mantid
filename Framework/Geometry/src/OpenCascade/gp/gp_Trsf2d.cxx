...
void gp_Trsf2d::SetMirror(const gp_Ax2d &A) {
  ...
  loc.SetCoord(-2.0 * ((VX * VX - 1.0) * X0 + (VX * VY * Y0)), -2.0 * ((VX * VY * X0) + (VY * VY - 1.0) * Y0));
}
...
Standard_Real gp_Trsf2d::RotationPart() const { return ATan2(matrix.Value(2, 1), matrix.Value(1, 1)); }
...
void gp_Trsf2d::Multiply(const gp_Trsf2d &T) {
  if (T.shape == gp_Identity) {
    ...
    loc.Add(Tloc);
    matrix.Multiply(T.matrix);
  } else if ((shape == gp_CompoundTrsf || shape == gp_Rotation || shape == gp_Ax1Mirror) && T.shape == gp_Translation) {
    gp_XY Tloc(T.loc);
    Tloc.Multiply(matrix);
    if (scale != 1.0)
      Tloc.Multiply(scale);
    ...
    loc.Add(T.loc);
    scale = T.scale;
  } else if ((shape == gp_PntMirror || shape == gp_Scale) && (T.shape == gp_PntMirror || T.shape == gp_Scale)) {
    shape = gp_CompoundTrsf;
    gp_XY Tloc(T.loc);
    Tloc.Multiply(scale);
    loc.Add(Tloc);
...
// purpose  :
//=======================================================================
void gp_Trsf2d::SetValues(const Standard_Real a11, const Standard_Real a12, const Standard_Real a13,
  ...
  if (As < 0)
    As = -As;
  Standard_ConstructionError_Raise_if(As < gp::Resolution(), "gp_Trsf2d::SetValues, null determinant");
