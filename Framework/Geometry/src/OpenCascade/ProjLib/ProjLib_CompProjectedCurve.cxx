...
//=======================================================================
inline Standard_Boolean Comparator(const Standard_Real theA, const Standard_Real theB) { return theA < theB; }
...
static void d2CurvOnSurf(const Standard_Real t, const Standard_Real u, const Standard_Real v, gp_Vec &V1, gp_Vec &V2,
  ...
  V2 = DS2_u * V12d.X() * V12d.X() + DS1_u * V22d.X() + 2 * DS2_uv * V12d.X() * V12d.Y() + DS2_v * V12d.Y() * V12d.Y() +
       DS1_v * V22d.Y();
}
...
static void DichExactBound(gp_Pnt &Sol, const Standard_Real NotSol, const Standard_Real Tol, const Standard_Real TolU,
  ...
  while (fabs(Sol.X() - aNotSol) > Tol) {
    t = (Sol.X() + aNotSol) / 2;
    aPrjPS.Perform(t, U0, V0, gp_Pnt2d(TolU, TolV), gp_Pnt2d(Surface->FirstUParameter(), Surface->FirstVParameter()),
    ...
    while (t <= LastU && !new_part && !FromLastU && !SameDeb) {
      ...
      if (initpoint) {
        ...
        if ((mySurface->IsUPeriodic() &&
          ...
          if ((Abs(U - aLowBorder.X()) < mySurface->UResolution(Precision::PConfusion())) && mySurface->IsUPeriodic()) {
          ...
          if ((Abs(V - aLowBorder.Y()) < mySurface->VResolution(Precision::PConfusion())) && mySurface->IsVPeriodic()) {
            d1(t, U, V, D, myCurve, mySurface);
            if (D.Y() < 0)
              V = aUppBorder.Y();
    ...
    while (t <= LastU && new_part) {
      ...
      // Go further
        ...
        if (!isSplitsComputed) {
          ...
          if (mySurface->IsUPeriodic() && (Abs(Triple.Y() - mySurface->FirstUParameter()) > Precision::PConfusion() &&
          ...
          if (mySurface->IsVPeriodic() && (Abs(Triple.Z() - mySurface->FirstVParameter()) > Precision::PConfusion() &&
...
// purpose  :
//=======================================================================
void ProjLib_CompProjectedCurve::SetTol3d(const Standard_Real theTol3d) { myTol3d = theTol3d; }

//=======================================================================
// function : SetContinuity
// purpose  :
//=======================================================================
void ProjLib_CompProjectedCurve::SetContinuity(const GeomAbs_Shape theContinuity) { myContinuity = theContinuity; }
...
// purpose  :
//=======================================================================
void ProjLib_CompProjectedCurve::SetProj3d(const Standard_Boolean theProj3d) { myProj3d = theProj3d; }

//=======================================================================
// function : SetProj2d
// purpose  :
//=======================================================================
void ProjLib_CompProjectedCurve::SetProj2d(const Standard_Boolean theProj2d) { myProj2d = theProj2d; }
...
void ProjLib_CompProjectedCurve::Load(const Handle(Adaptor3d_Surface) & S) { mySurface = S; }
...
void ProjLib_CompProjectedCurve::Load(const Handle(Adaptor3d_Curve) & C) { myCurve = C; }
...
const Handle(Adaptor3d_Surface) & ProjLib_CompProjectedCurve::GetSurface() const { return mySurface; }
...
const Handle(Adaptor3d_Curve) & ProjLib_CompProjectedCurve::GetCurve() const { return myCurve; }
...
Standard_Integer ProjLib_CompProjectedCurve::NbCurves() const { return myNbCurves; }
//=======================================================================
// function : Bounds
// purpose  :
//=======================================================================
...
void ProjLib_CompProjectedCurve::D0(const Standard_Real U, gp_Pnt2d &P) const {
  ...
  //  Cubic Interpolation
  if (mySequence->Value(i)->Length() < 4 || (Abs(U - mySequence->Value(i)->Value(j).X()) <= Precision::PConfusion())) {
    U0 = mySequence->Value(i)->Value(j).Y();
    V0 = mySequence->Value(i)->Value(j).Z();
...
const Handle(ProjLib_HSequenceOfHSequenceOfPnt) & ProjLib_CompProjectedCurve::GetSequence() const { return mySequence; }
//=======================================================================
// function : FirstParameter
// purpose  :
//=======================================================================

Standard_Real ProjLib_CompProjectedCurve::FirstParameter() const { return myCurve->FirstParameter(); }
...
Standard_Real ProjLib_CompProjectedCurve::LastParameter() const { return myCurve->LastParameter(); }
      ...
      for (j = 1; j < mySequence->Value(i)->Length(); j++) {
        ...
        if (Abs(Ul - CutPntsU(k)) <= myTolU)
          ...
          Solver.Perform((Tl + Tr) / 2, CutPntsU(k), V, gp_Pnt2d(Tol, myTolV),
                         gp_Pnt2d(Tl, mySurface->FirstVParameter()), gp_Pnt2d(Tr, mySurface->LastVParameter()),
                         FuncTol);
          //
          if (Solver.IsDone()) {
      ...
      for (j = 1; j < mySequence->Value(i)->Length(); j++) {
        ...
        if (Abs(Vl - CutPntsV(k)) <= myTolV)
          ...
          Solver.Perform((Tl + Tr) / 2, U, CutPntsV(k), gp_Pnt2d(Tol, myTolV),
                         gp_Pnt2d(Tl, mySurface->FirstUParameter()), gp_Pnt2d(Tr, mySurface->LastUParameter()),
                         FuncTol);
          //
          if (Solver.IsDone()) {
...
GeomAbs_CurveType ProjLib_CompProjectedCurve::GetType() const { return GeomAbs_OtherCurve; }
