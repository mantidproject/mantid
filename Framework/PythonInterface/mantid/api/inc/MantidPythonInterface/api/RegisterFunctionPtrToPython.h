...
// Specialization for shared_ptr<IFunction> and derived types.
template <typename T>
struct DLLExport TypedPropertyValueHandler<std::shared_ptr<T>,
                                           typename std::enable_if<std::is_base_of<API::IFunction, T>::value>::type>
  ...
  void set(Kernel::IPropertyManager *alg, const std::string &name, const boost::python::object &value) const override {
    if (value == boost::python::object())
      ...
      alg->setProperty<HeldType>(name, std::dynamic_pointer_cast<T>(ExtractSharedPtr<API::IFunction>(value)()));
  }
 ...
 */
template <typename IType> struct DLLExport RegisterFunctionPtrToPython {
  using IType_sptr = std::shared_ptr<IType>;
  using IType_wptr = std::weak_ptr<IType>;
  /// Constructor
  RegisterFunctionPtrToPython() {
