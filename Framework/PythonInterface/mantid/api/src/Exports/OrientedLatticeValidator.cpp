...
void export_OrientedLatticeValidator() {
  TypedValidatorExporter<ExperimentInfo_sptr>::define("OrientedLatticeValidator");

  class_<OrientedLatticeValidator, bases<Mantid::Kernel::TypedValidator<ExperimentInfo_sptr>>, boost::noncopyable>(
      "OrientedLatticeValidator", init<>("Checks that the workspace has an orientation matrix defined"));
}
