...
def _init_fn(fields, frozen, has_post_init, self_name):
    ...
    return _create_fn(
        "__init__", [self_name] + [_init_param(f) for f in fields if f.init], body_lines, locals=locals, globals=globals, return_type=None
    )
...
def _get_field(cls, a_name, a_type):
    ...
    if f._field_type is _FIELD and isinstance(f.default, (list, dict, set)):
        raise ValueError(f"mutable default {type(f.default)} for field " f"{f.name} is not allowed: use default_factory")
    ...
    if order:
        ...
        for name, op in [
            ...
            if _set_new_attribute(cls, name, _cmp_fn(name, op, self_tuple, other_tuple)):
                raise TypeError(
                    f"Cannot overwrite attribute {name} " f"in class {cls.__name__}. Consider using " "functools.total_ordering"
                )
