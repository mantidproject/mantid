...
void export_UsageService() {
  ...
  class_<UsageServiceImpl, boost::noncopyable>("UsageServiceImpl", no_init)
    ...
    .def("shutdown", &UsageServiceImpl::shutdown, arg("self"),
      "Sends any pending usage information, and disables the usage service.")

    .def("isEnabled", &UsageServiceImpl::isEnabled,
      arg("self"), "Returns if the usage service is enabled.")
