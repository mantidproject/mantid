...
void export_EnabledWhenProperty() {
  ...
  // Logic enumeration
  enum_<eLogicOperator>("LogicOperator").value("And", AND).value("Or", OR).value("Xor", XOR);

  class_<EnabledWhenProperty, bases<IPropertySettings>>("EnabledWhenProperty", no_init) // no default constructor
      ...
      .def(init<EnabledWhenProperty, EnabledWhenProperty, eLogicOperator>(
          (arg("self"), arg("conditionObjOne"), arg("conditionObjTwo"), arg("operator")),
