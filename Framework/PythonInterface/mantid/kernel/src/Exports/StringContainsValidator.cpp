...
namespace {
StringContainsValidator *createStringContainsValidator() { return new StringContainsValidator(); }
...
void setRequiredStrings(StringContainsValidator &self, const boost::python::list &strs) {
  self.setRequiredStrings(Converters::PySequenceToVector<std::string>(strs)());
}
} // namespace

void export_StringContainsValidator() {
  class_<StringContainsValidator, bases<IValidator>, boost::noncopyable>("StringContainsValidator")
      .def("__init__", make_constructor(&createStringContainsValidator, default_call_policies()))
      .def("__init__",
           make_constructor(&createStringContainsValidatorWithStrings, default_call_policies(), (arg("values"))))
