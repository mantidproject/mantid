...
public:
  static IPeakFunctionAdapterTest *createSuite() { return new IPeakFunctionAdapterTest(); }
  ...
  void testfunctionLocal_Returning_Non_Numpy_Array_Throws() {
    ...
    TS_ASSERT_THROWS_NOTHING(badReturnFunc =
                                 createTestFunction<IPeakFunction_sptr::element_type>("BadReturnFunction", funcImpl));
    TS_ASSERT(badReturnFunc);

    std::array<double, 1> xvalues{{10}}, retvalue{{0}};
    TS_ASSERT_THROWS(badReturnFunc->function1D(retvalue.data(), xvalues.data(), 1), const std::runtime_error &);
  }
  ...
  void testfunctionLocal_Returning_Numpy_Array_With_NonFloat_Type_Throws() {
    ...
    TS_ASSERT_THROWS_NOTHING(badNdArrayFunc =
                                 createTestFunction<IPeakFunction_sptr::element_type>("BadNDArrayFunction", funcImpl));
    TS_ASSERT(badNdArrayFunc);

    std::array<double, 1> xvalues{{10}}, retvalue{{0}};
    TS_ASSERT_THROWS(badNdArrayFunc->function1D(retvalue.data(), xvalues.data(), 1), const std::runtime_error &);
  }
  ...
  void testfunction_Uses_Numerical_Deriv_When_Deriv_NotSupplied() {
    ...
    TS_ASSERT_THROWS_NOTHING(noDerivPeakFunction = createTestFunction<IPeakFunction_sptr::element_type>(
    ...
    noDerivPeakFunction->functionDeriv1D(&jacobian, xvalues.data(), xvalues.size());
  ...
  void testfunction_Uses_Supplied_Deriv() {
    ...
    TS_ASSERT_THROWS_NOTHING(peakFuncWithDeriv = createTestFunction<IPeakFunction_sptr::element_type>(
                                 "IPeakFunctionAdapterWithDeriv", "        return self.getParameterValue(0)*x",
    ...
    std::iota(std::begin(xvalues), std::end(xvalues), 10.0);
    FunctionAdapterTestJacobian jacobian(xvalues.size(), 1);
    peakFuncWithDeriv->functionDeriv1D(&jacobian, xvalues.data(), xvalues.size());
