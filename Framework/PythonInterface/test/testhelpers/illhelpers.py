...
def _convert_unit(source, destination, value, l1, l2):
    parameters = UnitParametersMap()
    parameters[UnitParams.l2] = l2
    parameters[UnitParams.twoTheta] = 0.0
    return UnitConversion.run(source, destination, value, l1, DeltaEModeType.Direct, parameters)
...
def _fillTemplateReflectometryWorkspace(ws, XUnit="TOF"):
    ...
    binWidth = 57.0
    templateXs = numpy.array(numpy.arange(-300.0, 55000.0, binWidth))
...
def _fillTemplateTOFWorkspace(templateWS, bkgLevel):
    ...
    l1 = spectrumInfo.l1()
    l2 = spectrumInfo.l2(0)
    tofElastic = _convert_unit("Energy", "TOF", E_i, l1, l2)
    ...
    tofMonitorDetector = _convert_unit("Energy", "TOF", E_i, monitorSampleDistance, l2)
    ...
    for histogramIndex in range(0, nHistograms - 1):
        ...
        trueTOF = _convert_unit("Energy", "TOF", E_i, l1, trueL2)
    ...
    wavelength = _convert_unit("Energy", "Wavelength", E_i, l1, l2)
    ...
    kwargs = {"Workspace": ws, "LogName": "monitor.monsum", "LogText": str(1000), "LogType": "Number", "NumberType": "Int", "child": True}
...
def create_empty_figaro_workspace():
    kwargs = {"InstrumentName": "FIGARO", "child": True}
...
def default_test_detectors(ws):
    ...
    kwargs = {"Workspace": ws, "StartWorkspaceIndex": 512, "EndWorkspaceIndex": ws.getNumberHistograms() - 2, "child": True}
    run_algorithm("MaskDetectors", **kwargs)
    return ws
...
def refl_rotate_detector(ws, angle):
    ...
    z = r * numpy.cos(angle)
    y = r * numpy.sin(angle)
    args = {"Workspace": ws, "ComponentName": "detector", "X": 0.0, "Y": y, "Z": z, "RelativePosition": False}
