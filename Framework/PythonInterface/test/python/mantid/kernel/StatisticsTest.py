...
class StatisticsTest(unittest.TestCase):
    ...
    def test_getMoments(self):
        ...
        offsetX = mean - (0.5 * deltaX * float(numX))
        # variance about origin
        expVar = mean * mean + sigma * sigma
        # skew about origin
        expSkew = mean * mean * mean + 3.0 * mean * sigma * sigma
        ...
        depend = numpy.exp(-0.5 * weightedDiff * weightedDiff) / sigma / math.sqrt(2.0 * math.pi)
        ...
        self.assertAlmostEqual(1.0, aboutOrigin[0], places=4)
        ...
        self.assertTrue(isinstance(aboutOrigin, numpy.ndarray))
        self.assertEqual(4, aboutOrigin.shape[0])
        self.assertAlmostEqual(1.0, aboutMean[0], places=4)
        self.assertAlmostEqual(0.0, aboutMean[1], places=4)
        self.assertTrue(math.fabs(sigma * sigma - aboutMean[2]) < 0.001 * expVar)
        self.assertTrue(math.fabs(0.0 - aboutMean[3]) < 0.0001 * expSkew)
