...
class SCgapDwaveTest(unittest.TestCase):

	def test_function_has_been_registered(self):
		status, msg = is_registered("SCgapDwave")
		if not status:
			self.fail(msg)
	...
	def test_do_fit(self):
		do_a_fit(np.arange(0.1, 16, 0.2), 'SCgapDwave', guess = dict( = 2.05, Tcritical = 9.1), target = dict( = 2.0, Tcritical = 9.0), atol = 0.01)
