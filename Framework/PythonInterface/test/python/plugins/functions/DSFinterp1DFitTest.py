...
class DSFinterp1DTestTest(unittest.TestCase):
    ...
    '''
        ...
        for iif in range(nf):
            ...
            if e:
                HWHM += 2 * df * (0.5 - random.random()
                                  )  # add uncertainty as big as twice the step between consecutive HWHM!!!
                if HWHM < 0: HWHM = f
            yvalues = 1 / numpy.pi * HWHM / (HWHM * HWHM + xvalues * xvalues) * (1.1 - 0.2 * numpy.random.rand(2 * n)
                                                                                 )  #10% random noise
    ...
    def test_input_exceptions(self):
        ...
        fvalues, InputWorkspaces, xvalues, HWHM = self.generateWorkspaces(
            nf, 0.01, 0.01)  # workspaces sim0 to sim8 (nine workpaces)
