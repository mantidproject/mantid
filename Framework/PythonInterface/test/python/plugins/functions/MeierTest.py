...
class MeierTest(unittest.TestCase):
    ...
    def test_function_output(self):
        ...
        tolerance = 1.0e-05
        status, output = check_output("Meier",
                                      input,
                                      expected,
                                      tolerance,
                                      A0=0.5,
    ...
    def test_do_fit(self):
        do_a_fit(np.arange(0.1, 16, 0.2),
                 'Meier',
                 guess=dict(A0=0.55, FreqD=0.015, FreqQ=0.055, Spin=3.55, Lambda=0.15, Sigma=0.25),
                 target=dict(A0=0.5, FreqD=0.01, FreqQ=0.05, Spin=3.5, Lambda=0.1, Sigma=0.2),
                 atol=0.01)
