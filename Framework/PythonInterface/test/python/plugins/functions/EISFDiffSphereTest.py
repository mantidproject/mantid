...
class EISFDiffSphereTest(unittest.TestCase):
    ...
    def test_function_output(self):
        ...
        tolerance = 1.0e-05
        status, output = check_output("EISFDiffSphere", input, expected, tolerance, A=1.0, R=3.5)
        if not status:
            msg = "Computed output {} from input {} unequal to expected: {}"
            self.fail(msg.format(*[str(a) for a in (output, input, expected)]))

    def test_do_fit(self):
        do_a_fit(np.arange(0.1, 2.2, 0.2), "EISFDiffSphere", guess=dict(A=2.0, R=1.0), target=dict(A=1.0, R=3.5), atol=0.01)
