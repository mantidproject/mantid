...
class EISFDiffSphereAlkylTest(unittest.TestCase):
    ...
    def test_function_output(self):
        ...
        tolerance = 1.0e-05
        status, output = check_output('EISFDiffSphereAlkyl', input, expected, tolerance, A=1.0, Rmin=0.5, Rmax=6.3)
        if not status:
    ...
    def test_do_a_fit(self):
        do_a_fit(np.arange(0.1, 2.2, 0.2),
                 'EISFDiffSphereAlkyl',
                 guess=dict(A=2.0, Rmin=2.0, Rmax=3.0),
                 target=dict(A=1.0, Rmin=0.5, Rmax=6.3),
                 atol=0.01)
