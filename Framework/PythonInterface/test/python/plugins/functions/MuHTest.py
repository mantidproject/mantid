...
class MuHTest(unittest.TestCase):
	...
	def test_function_output(self):
		...
		tolerance = 1.0e-05
		status, output = check_output("MuH", input, expected, tolerance, A0 = 0.5, FreqD = 0.5, Lambda = 0.3, Sigma = 0.05)
		if not status:
			msg = 'Computed output {} from input {} unequal to expected: {}'
			self.fail(msg.format(*[str(i) for i in (output, input, expected)]))

	def test_do_fit(self):
		do_a_fit(np.arange(0.1, 16, 0.2), 'MuH', guess = dict(A0 = 0.55, FreqD = 0.5, Lambda = 0.3, Sigma = 0.05), target = dict(A0 = 0.5, FreqD = 0.5, Lambda = 0.3, Sigma = 0.05), atol = 0.01)
