...
class StaticLorentzianKTTest(unittest.TestCase):
	...
	def test_function_output(self):
		...
		tolerance = 1.0e-05
		status, output = check_output("StaticLorentzianKT", input, expected, tolerance, A0 = 0.2, A = 1, B_L = 1)
		if not status:
			msg = 'Computed output {} from input {} unequal to expected: {}'
			self.fail(msg.format(*[str(i) for i in (output, input, expected)]))

	def test_do_fit(self):
		do_a_fit(np.arange(0.1, 16, 0.2), 'StaticLorentzianKT', guess = dict(A0 = 0.2, A = 1.1, B_L = 0.9), target = dict(A0 = 0.2, A = 1, B_L = 1), atol = 0.01)
