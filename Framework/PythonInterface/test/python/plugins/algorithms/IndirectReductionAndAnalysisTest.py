...
import unittest
from mantid.simpleapi import IndirectReductionAndAnalysis
from mantid.api import mtd


class IndirectReductionAndAnalysisTest(unittest.TestCase):
    def test_IRIS(self):
        IndirectReductionAndAnalysis(InputFiles="IRS26176.RAW, IRS26173.RAW", Instrument='IRIS', Analyser='graphite',
    ...
    def test_OSIRIS(self):
        IndirectReductionAndAnalysis(InputFiles="osi89757.raw", Instrument='OSIRIS', Analyser='graphite',
    ...
    def test_MSDFit(self):
        IndirectReductionAndAnalysis(InputFiles="IRS26176.RAW", Instrument='IRIS', Analyser='graphite',
                                     Reflection='002', SpectraRange='3, 50', ElasticRange='-0.5, 0',
    ...
    def test_diffraction(self):
        IndirectReductionAndAnalysis(InputFiles="IRS26176.RAW", Instrument='IRIS', Analyser='graphite',
                                     Reflection='002', SpectraRange='3, 50', ElasticRange='-0.5, 0',
                                     InelasticRange='0, 0.5', GroupingMethod='Individual', Diffraction=True)
        diffraction_ws = mtd['iris26176_diffspec_red']
        self.assertEqual(round(diffraction_ws.readY(0)[0], 7), 0.5861718)
        self.assertEqual(round(diffraction_ws.readY(0)[1], 7), 0.5547503)
