...
class PDConvertReciprocalSpaceTest(unittest.TestCase):
    def test_simple(self):
        ...
        input_y = np.array([1.0, 2, 3, 2, 1])
        input_e = np.array([0.1, 0.14, 0.17, 0.14, 0.1])
        sq = CreateWorkspace(DataX=input_x, DataY=input_y, DataE=input_e, UnitX="MomentumTransfer")
        SetSampleMaterial(InputWorkspace=sq, ChemicalFormula="Ar")
        fq = PDConvertReciprocalSpace(InputWorkspace=sq, To="F(Q)", From="S(Q)")
        x = fq.readX(0)
        y = fq.readY(0)
        e = fq.readE(0)
        ...
        fkq = PDConvertReciprocalSpace(InputWorkspace=sq, To="FK(Q)", From="S(Q)")
        x = fkq.readX(0)
        y = fkq.readY(0)
        e = fkq.readE(0)
