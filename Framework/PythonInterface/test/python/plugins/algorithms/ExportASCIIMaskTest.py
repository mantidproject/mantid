...
import unittest
import mantid
from mantid.simpleapi import ExportASCIIMask,DeleteWorkspace

class ExportASCIIMaskTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        if not hasattr(self,'write_f'):
           import ExportASCIIMask as amc
           self.test_write_f = amc.writeISISmasks
           self.test_export_f = amc.export_masks

        return super(ExportASCIIMaskTest, self).setUp()
    ...
    def tearDown(self):
        ...
        if os.path.isfile(test_file ):
            os.remove(test_file)
        return super(ExportASCIIMaskTest, self).tearDown()
    ...
    def test_ExportOnlyAlgos(self):
        r_masks = ExportASCIIMask('test_ws',ExportMaskOnly=True)
        self.assertTrue((np.array(self.masks)==r_masks).all())

        test_file = self.test_file + '.msk'
        self.assertFalse(os.path.isfile(test_file))

        r_masks = ExportASCIIMask('test_ws',Filename = test_file)
        self.assertTrue(os.path.isfile(test_file))
        self.assertTrue((np.array(self.masks)==r_masks).all())

        default_tf = os.path.join(mantid.config.getString('defaultsave.directory'),'test_ws.msk')
        ExportASCIIMask('test_ws')
