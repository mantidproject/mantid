...
class AlignComponentsTest(unittest.TestCase):
    def testAlignComponentsPosition(self):
        ...
        def serve_instrument(output_workspace):
            ...
            MoveInstrumentComponent(
                Workspace=output_workspace, ComponentName="bank1", RelativePosition=False, X=0, Y=0.0, Z=0
            )  # move to the origin
            RotateInstrumentComponent(
                Workspace=output_workspace, ComponentName="bank1", X=0, Y=1, Z=0, Angle=scattering_angle, RelativeRotation=False
            )
            ...
            MoveInstrumentComponent(
                Workspace=output_workspace, ComponentName="bank1", RelativePosition=False, X=x, Y=0, Z=z
            )  # translate 2 meters away and center the detector
            return mtd["original"]
        ...
        MoveInstrumentComponent(
            Workspace="perturbed", ComponentName="bank1", RelativePosition=True, X=xyz_shift.X(), Y=xyz_shift.Y(), Z=xyz_shift.Z()
        )

        # calibrate the perturbed bank
        AlignComponents(
            PeakCentersTofTable="table_tofs",
        ...
        ConvertUnits(InputWorkspace="calibrated", Target="dSpacing", EMode="Elastic", OutputWorkspace="calibrated_dspacing")
