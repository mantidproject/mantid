...
class MatchSpectraTest(unittest.TestCase):
    ...
    def __checkValues(self, results, index, scale, offset):
        self.assertEqual(results.Scale[index], scale,
                         'wksp index {} scale exp={} found={}'.format(index, scale, results.Scale[index]))
        self.assertEqual(results.Offset[index], offset,
                         'wksp index {} offset exp={} found={}'.format(index, offset, results.Offset[index]))
        self.assertEqual(results.ChiSq[index], 0., 'wksp index {} chisq'.format(index))

    def __checkReference(self, results, index):
        self.assertEqual(results.Scale[index], 1., 'reference scale')
        self.assertEqual(results.Offset[index], 0., 'reference offset')
        self.assertEqual(results.ChiSq[index], 0., 'reference chisq')
