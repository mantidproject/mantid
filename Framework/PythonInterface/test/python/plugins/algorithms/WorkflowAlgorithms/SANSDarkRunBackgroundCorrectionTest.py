...
class SANSDarkRunBackgroundCorrectionTest(unittest.TestCase):
    ...
    def test_dark_run_correction_with_uniform_and_not_mean_for_workspace2D_input(self):
        ...
        self._provide_workspace2D(bin_boundaries_scatter, y_value_scatter_run, e_value_scatter_run, name_scatter,
                                  spectra)
        ...
        alg = run_algorithm('SANSDarkRunBackgroundCorrection',
    ...
    def test_dark_run_correction_with_non_uniform_and_not_mean_for_workspace2D_input(self):
        ...
        self.assertTrue(AnalysisDataService.doesExist(out_ws_name))
        self._check_output_workspace_non_uniform(mtd[name_scatter], mtd[out_ws_name], mtd[name_dark_run],
    ...
    def test_that_only_monitors_are_corrected_if_only_monitors_should_be_corrected(self):
        ...
        dataY_scatter = 2.
        dataE_scatter = 2.
        scatter_ws = self._load_workspace_with_monitors(monY_scatter,
                                                        monE_scatter,
                                                        dataY_scatter,
                                                        dataE_scatter,
                                                        as_dark_run=False)
        ...
        selected_monitor = []
        # Act
        ws = self._do_run_dark_subtraction(scatter_ws, dark_run, mean, uniform, normalization_ratio, out_ws_name,
                                           applyToMonitors, applyToDetectors, selected_monitor)
        ...
        expected_monitor_Y = monY_scatter - monY_dark * len(dark_run.dataY(0)) / len(
            scatter_ws.dataY(0)) * normalization_ratio
    ...
    def test_that_individual_monitor_is_corrected_if_only_individual_monitor_is_chosen(self):
        ...
        expected_monitor_Y_1 = monY_scatter - monY_dark * len(dark_run.dataY(0)) / len(
            scatter_ws.dataY(0)) * normalization_ratio
    ...
    def test_that_selecting_monitors_and_detectors_is_allowed(self):
        ...
        # Expected value for detectors
        expected_detector_Y = dataY_scatter - dataY_dark * len(dark_run.dataY(0)) / len(
            scatter_ws.dataY(0)) * normalization_ratio
        for index in range(2, ws.getNumberHistograms()):
            comparison(ws.dataY(index), expected_detector_Y)
    ...
    def _do_run_dark_subtraction(self, scatter, dark_run, mean, uniform, normalization_ratio, out_ws_name,
                                 applyToMonitors, applyToDetectors, selected_monitor):
