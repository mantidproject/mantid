...
class CorelliPowderCalibrationCreateTest(unittest.TestCase):
    def setUp(self) -> None:
        ...
        RotateInstrumentComponent(
            Workspace="test_workspace_dSpacing", ComponentName="bank1", X=0.0, Y=1.0, z=0.0, Angle=90, RelativeRotation=True
        )
        MoveInstrumentComponent(Workspace="test_workspace_dSpacing", ComponentName="bank1", X=4.0, y=0.0, z=0.0, RelativePosition=False)
        ...
        # reference values
        ConvertUnits(InputWorkspace="test_workspace_dSpacing", Target="TOF", EMode="Elastic", OutputWorkspace="test_workspace_TOF")
        Rebin(InputWorkspace="test_workspace_TOF", Params=[300, 1.0, 16666.7], OutputWorkspace="test_workspace_TOF")
        ConvertUnits(InputWorkspace="test_workspace_TOF", Target="dSpacing", EMode="Elastic", OutputWorkspace="test_workspace_dSpacing")
    ...
    def spacings_recovered(self, input_workspace, calibrate=True):
        ...
        results = CompareWorkspaces(Workspace1="test_workspace_dSpacing", Workspace2="perturbed_dS", Tolerance=0.001, CheckInstrument=False)
    ...
    def test_exceptions(self):
        ...
        try:
            CorelliPowderCalibrationCreate(
                InputWorkspace="test_workspace_TOF",
                OutputWorkspacesPrefix="cal_",
                TofBinning=[300, 1.0, 16666.7],
                PeakPositions=self.spacings_reference,
                FixSource=True,
    ...
    def test_translation(self):
        ...
        CloneWorkspace(InputWorkspace="test_workspace_TOF", OutputWorkspace="perturbed")
        MoveInstrumentComponent(Workspace="perturbed", ComponentName="bank1", X=0.02, y=0.005, z=0.005, RelativePosition=True)
    ...
    def test_rotation(self):
        CloneWorkspace(InputWorkspace="test_workspace_TOF", OutputWorkspace="perturbed")
        RotateInstrumentComponent(Workspace="perturbed", ComponentName="bank1", X=0, Y=0, z=1, Angle=5, RelativeRotation=True)
        assert self.spacings_recovered("perturbed", calibrate=False) is False
        assert self.spacings_recovered("perturbed", calibrate=True)
        DeleteWorkspaces(["perturbed"])
    ...
    def test_fix_y(self) -> None:
        ...
        MoveInstrumentComponent(Workspace="perturbed", ComponentName="bank1", X=0, y=y, z=0, RelativePosition=False)
