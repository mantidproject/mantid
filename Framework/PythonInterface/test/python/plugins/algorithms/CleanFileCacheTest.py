...
class CleanFileCache(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls._cache_filepath1, _ = CreateCacheFilename(CacheDir=cls._cache_root, OtherProperties=["A=first"])
        cls._cache_filepath2, _ = CreateCacheFilename(CacheDir=cls._cache_root, OtherProperties=["B=second"])
        cls._cache_filepath3, _ = CreateCacheFilename(CacheDir=cls._cache_root, OtherProperties=["C=third"])
    ...
    def test_clean_cache_will_not_clear_non_cache_files(self):
        _write_file(self._cache_filepath1)
        _write_file(self._cache_filepath2)
    ...
    def test_clean_cache_will_not_clear_a_file_older_than_an_age_limit(self):
        ...
        _write_file(self._cache_filepath1, age - 1)
        _write_file(self._cache_filepath2, age)
        _write_file(self._cache_filepath3, age + 1)

        _execute_clean_cache(self._cache_root, age)

        files_remained = glob.glob(os.path.join(self._cache_root, "*"))
        self.assertEqual(set(files_remained), set(self._non_cache_filepaths + [self._cache_filepath1]))
