...
class SaveGEMMAUDParamFileTest(unittest.TestCase):
    ...
    def _test_file_segment_matches(self, segment_header, expected_values, val_type=float):
        ...
        while self.file_contents[line_index]:
            self.assertAlmostEquals(val_type(self.file_contents[line_index]), expected_values[expected_vals_index])
            line_index += 1
    ...
    def test_values_saved_correctly(self):
        ...
        self._test_file_segment_matches("_instrument_counter_bank_ID", ["Bank{}".format(i + 1) for i in range(5)],
                                        val_type=str)
        ...
        prof_coeffs_func_2 = {
            "alpha0", "alpha1", "beta", "switch", "sigma0", "sigma1", "sigma2", "gamma0", "gamma1", "gamma2"
        }
