...
from mantid.api import AlgorithmFactory, CommonBinsValidator, PropertyMode, PythonAlgorithm, MatrixWorkspaceProperty
from mantid.kernel import Direction, FloatBoundedValidator, StringListValidator, StringMandatoryValidator
...
class CalculateEfficiencyCorrection(PythonAlgorithm):
    ...
    def PyInit(self):
        self.declareProperty(
            MatrixWorkspaceProperty(
                "InputWorkspace", "", direction=Direction.Input, optional=PropertyMode.Optional, validator=CommonBinsValidator()
            ),
            doc="Input workspace with wavelength range to calculate for the correction.",
        )

        self.declareProperty(name="WavelengthRange", defaultValue="", doc="Wavelength range to calculate efficiency for.")
        ...
        self.declareProperty(name="Thickness", defaultValue=1.0, validator=FloatBoundedValidator(0.0), doc="Sample thickness (cm).")
    ...
    def PyExec(self):
        ...
        if not self.getProperty("InputWorkspace").isDefault:
            self._output_ws = CloneWorkspace(Inputworkspace=self._input_ws, OutputWorkspace=self._output_ws, StoreInADS=False)
        else:
            self._output_ws = CreateWorkspace(NSpec=1, DataX=[0], DataY=[0], UnitX="Wavelength", Distribution=False, StoreInADS=False)
        ...
        self._output_ws = ConvertUnits(InputWorkspace=self._output_ws, Target="Wavelength", EMode="Elastic", StoreInADS=False)
    ...
    def _calculate_efficiency(self, wavelength):
        ...
        @return efficiency
        """
        efficiency = 1.0 / (1.0 - np.exp(-(self._alpha_scatXS + self._alpha_absXS * wavelength)))
        return efficiency
