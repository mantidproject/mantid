...
import math
import mantid
from OpengenieMaxentTools.multimaxalpha import MULTIMAX
...
class OpengenieMaxent(PythonAlgorithm):
	...
	def PyInit(self):
		self.declareProperty(WorkspaceProperty("InputWorkspace","",direction=Direction.Input,validator=RawCountValidator(True)),doc="Raw muon workspace to process")
		...
		self.declareProperty(WorkspaceProperty("GroupWorkspace","",direction=Direction.Input,optional=PropertyMode.Optional),doc="Group Workspace") # from LoadDetectorsGroupingTable, none=do all spectra individually
		...
		self.declareProperty("DefaultLevel",0.1,validator=valid,doc="Default Level, magic number chitarget?")
		self.declareProperty("Factor",1.04,doc="Sigma looseness factor, magic number A?",direction=Direction.InOut)
		...
		self.declareProperty(WorkspaceProperty("PhaseConvergenceTable","",direction=Direction.Output,optional=PropertyMode.Optional),doc="Convergence of phases (optional)")
	...
	def PyExec(self):
		...
		CHANNELS_itzero=rg0+int(math.floor(-ws.readX(0)[rg0]/RUNDATA_res)) # Bin with t0 in it. note, may be negative for pre-cropped data. Remove +0.5
		TZERO_fine= (ws.readX(0)[CHANNELS_itzero]+ws.readX(0)[CHANNELS_itzero+1])/2.0 # since it's not an exact boundary. Error if t0<0 or t0 is in an odd sized bin
		#TZERO_fine= ws.readX(0)[CHANNELS_itzero] # alternative 1
		mylog.notice("time zero bin has boundaries {} and {} giving tzero={}".format(ws.readX(0)[CHANNELS_itzero],ws.readX(0)[CHANNELS_itzero+1],TZERO_fine))
		t1stgood=self.getProperty("FirstGoodTime").value
		CHANNELS_i1stgood=rg0+max(int(math.floor((t1stgood-ws.readX(0)[rg0])/RUNDATA_res+1.0)),0) # was 1.0. i1stgood is first bin with purely good data in it (and good sized)
		...
		ilast=min(rg0+int(math.floor((tlast-ws.readX(0)[rg0])/RUNDATA_res)),rg9-1) # first bin with some bad data in it, or end (excluding bad sized bins)
		...
		if(not self.getProperty("GroupWorkspace").isDefault):
			# group Workspace (LoadDetectorsGroupingFile format) provided, use it
		...
		if(not self.getProperty("OutputPhaseTable").isDefault):
			...
			for i in range(POINTS_ngroups):
				outPhase.addRow([i+1,AMPS_amp[i],SENSE_phi[i]]) # sign of phase now OK for Mantid 3.12 onwards
		...
		if(not self.getProperty("ReconstructedSpectra").isDefault):
			...
			mylog.notice("i1={} i2={} k1={} k2={} len(srcX)={} len(guess)={}".format(i1,i2,k1,k2,len(ws.dataX(0)),OUTSPEC_guess.shape[0]))
