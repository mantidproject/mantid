...
class SNSPowderReductionAlternativeMPI(DataProcessorAlgorithm):
    COMPRESS_TOL_TOF = .01
    _resampleX = None
    _binning = None
    _bin_in_dspace = None
    ...
    def name(self):
        return "SNSPowderReductionAlternativeMPI"
    ...
    def PyExec(self):
        ...
        if HAVE_MPI and preserveEvents == True:
            ...
            # For comparability, also disable for new MPI model
            self.log().warning("preserveEvents set to False for MPI tasks.")
            preserveEvents = False
        self._info = None
        ...
        for (samRunIndex, sam_ws_name) in enumerate(samwksplist):
            ...
            if van_specified:
                van_run_ws_name = self._process_vanadium_runs(van_run_number_list, timeFilterWall, samRunIndex, calib, preserveEvents=preserveEvents)
            ...
            if van_run_ws_name is not None:
                ...
                self.log().warning('{} {}'.format(sam_ws.readX(0), van_ws.readX(0)))
    ...
    def _process_vanadium_runs(self, van_run_number_list, timeFilterWall, samRunIndex, calib, preserveEvents=True, **focuspos):
...
AlgorithmFactory.subscribe(SNSPowderReductionAlternativeMPI)
