...
class ExaminePowderDiffProfile(PythonAlgorithm):
    ...
    def PyInit(self):
        ...
        self.declareProperty(FileProperty("DataFilename", "", FileAction.OptionalLoad, [".dat"]), "Name of input data file.")
        ...
        self.declareProperty(
            "ProfileType",
            "Back-to-back exponential convoluted with PseudoVoigt",
            StringListValidator(profiletypes),
            "Type of peak profile.",
        )

        # Table workspaces
        self.declareProperty(
            ITableWorkspaceProperty("ProfileWorkspace", "", Direction.InOut), "Name of table workspace containing peak parameters as input."
        )
        self.declareProperty(
            ITableWorkspaceProperty("BraggPeakWorkspace", "", Direction.InOut),
            "Name of table workspace containing reflections (bragg peaks) in form of Miller index.",
        )
        self.declareProperty(FileProperty("ProfileFilename", "", FileAction.OptionalLoad, [".irf"]), "Name of input data file.")
        ...
        # Background
        self.declareProperty(
            ITableWorkspaceProperty("BackgroundParameterWorkspace", "", Direction.InOut),
        ...
        arrvalidator = FloatArrayBoundedValidator(lower=0.0)
        self.declareProperty(
            FloatArrayProperty("BackgroundPoints", values=[], validator=arrvalidator, direction=Direction.Input),
            "User specified X/TOF values of the data points to calculate background.",
        )
        self.declareProperty(
            MatrixWorkspaceProperty("BackgroundWorkspace", "", Direction.Output, PropertyMode.Optional),
    ...
    def mainExec(self):
        ...
        if self.loaddata is True:
            # Load data file
            api.LoadAscii(Filename=self.datafilename, OutputWorkspace=self.datawsname, Unit="TOF")
