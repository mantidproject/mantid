...
class LoadVesuvio(LoadEmptyVesuvio):
    ...
    def PyInit(self):
        ...
        self.declareProperty(
            MODE_PROP, "DoubleDifference", StringListValidator(MODES), doc="The difference option. Valid values: %s" % str(MODES)
        )

        self.declareProperty(
            FileProperty(INST_PAR_PROP, "", action=FileAction.OptionalLoad, extensions=["dat", "par"]),
    ...
    def _load_single_run_spec_and_mon(self, all_spectra, run_str):
        ...
        if mons_in_ws:
            ...
            if self._load_monitors:
                ...
                self._load_monitors_workspace = self._sum_monitors_in_group(monitor_group, self._load_monitors_workspace)
    ...
    def _load_diff_mode_parameters(self, workspace):
        ...
        load_parameter_file.setProperty("Filename", self._get_parameter_filename(self._diff_opt))
    ...
    def _get_parameter_filename(self, diff_opt):
        ...
        if "Difference" not in diff_opt:
            raise RuntimeError(
                "Trying to load parameters for difference mode when not doing differencing! "
    ...
    def _load_and_sum_runs(self, spectra):
        ...
        for index, run in enumerate(runs):
            ...
            # Sum
            if index > 0:
                ms.Plus(LHSWorkspace=SUMMED_WS, RHSWorkspace=out_name, OutputWorkspace=SUMMED_WS, EnableLogging=_LOGGING_)
                ms.Plus(
                    LHSWorkspace=SUMMED_WS + "_monitors",
        ...
        if "Difference" in self._diff_opt:
            ...
            ms.CropWorkspace(Inputworkspace=SUMMED_WS, OutputWorkspace=SUMMED_WS, XMax=x_max, EnableLogging=_LOGGING_)
        ...
        if self._load_monitors:
            ...
            clone.execute()
            self._load_monitors_workspace = clone.getProperty("OutputWorkspace").value
            self._load_monitors_workspace = self._sum_monitors_in_group(summed_mon, self._load_monitors_workspace)
        if "Difference" in self._diff_opt:
    ...
    def _store_results(self):
        ...
        if self._load_monitors:
            ...
            if not self.existsProperty(WKSP_PROP_LOAD_MON):
                mon_out_name = self.getPropertyValue(WKSP_PROP) + "_monitors"
                self.declareProperty(
                    WorkspaceProperty(WKSP_PROP_LOAD_MON, mon_out_name, Direction.Output),
...
class SpectraToFoilPeriodMap(object):
    ...
    def __init__(self, nperiods=6):
        ...
        if nperiods == 2:
            ...
            raise RuntimeError("Unsupported number of periods given: " + str(nperiods) + ". Supported number of periods=2,3,6")
    ...
    def get_foil_periods(self, spectrum_no, state):
        ...
        if spectrum_no < 135:
            foil_periods = [1, 2, 3]
        elif (135 <= spectrum_no <= 142) or (151 <= spectrum_no <= 158) or (167 <= spectrum_no <= 174) or (183 <= spectrum_no <= 190):
