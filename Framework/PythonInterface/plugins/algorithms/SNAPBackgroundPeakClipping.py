...
class SNAPBackgroundPeakClipping(PythonAlgorithm):
	...
	def PyInit(self):
		...
		self.declareProperty(WorkspaceProperty("Input Workspace", "", Direction.Input, PropertyMode.Optional),"The workspace containing the normalization data.")
						
		self.declareProperty("LLS Correction", True,
						"Read live data - requires a saved run in the current IPTS with the same Instrumnet configuration")

		self.declareProperty("Increasing Window", False,
						"Read live data - requires a saved run in the current IPTS with the same Instrumnet configuration")
		...
		self.declareProperty(WorkspaceProperty("Output Workspace", "", Direction.Output),"The workspace containing the normalization data.")
...
############################
	...
	def peak_clip(self, data, win=30, decrese= True, LLS =True, smooth_window = 0 ):
		...
		if decrese == True: scan  = list(range(window+1,0,-1))
		else : scan  = list(range(1,window+1))
...
##############################
	def PyExec(self):		
		...
		for h in range(n_histo):
			...
			peak_clip_WS.setY(h,self.peak_clip(y[h], win=window, decrese= decreasing, LLS =LLS_set, smooth_window = smooth_range ))
