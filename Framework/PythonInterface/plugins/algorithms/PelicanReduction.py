...
from mantid.kernel import (StringListValidator, Direction, StringArrayMandatoryValidator,
                           StringArrayProperty, CompositeValidator)
...
from mantid.simpleapi import (
    Scale, Minus, Divide, ResetNegatives, DeleteWorkspace, CloneWorkspace,
    ReplaceSpecialValues, GroupWorkspaces, FindEPP, Integration, SaveNXSPE, ExtractMonitors,
    SofQW, CorrectKiKf, SumSpectra, Transpose, Rebin, ConvertUnits, LoadPLN, AlgorithmFactory, logger)
...
def scale_and_remove_background(source_ws: str, source_scale: float, empty_ws: str, empty_scale: float,
                                out_ws: str, floor_negatives: bool) -> None:
...
class PelicanReduction(PythonAlgorithm):
    ...
    def PyExec(self) -> None:
        ...
        # Get the list of data files from the runs
        sample_runs = hdf_files_from_runs(
            pval('SampleRuns'), self._search_dirs, self._file_prefix, self._file_extn)
        empty_runs = hdf_files_from_runs(
            pval('EmptyRuns'), self._search_dirs, self._file_prefix, self._file_extn)
        calibration_runs = hdf_files_from_runs(
            pval('CalibrationRuns'), self._search_dirs, self._file_prefix, self._file_extn)
        empty_calib_runs = hdf_files_from_runs(
            pval('EmptyCalibrationRuns'), self._search_dirs, self._file_prefix, self._file_extn)
    ...
    def _nxspe_processing(self, reduced_2D: str) -> None:
        if self._scratch is None:
            ...
            nxspe_file = os.path.join(
                self._scratch.folder, reduced_2D + '.nxspe')
    ...
    def set_efixed(self, sample_path: str) -> None:
        ...
        tags = [HdfKey('wavelength', '/entry1/instrument/crystal/wavelength', None),
                HdfKey('mscor', '/entry1/sample/mscor', [0.0])]
    ...
    def setUp(self) -> None:
        ...
        self._average_peak_width = self._cfg.get_bool(
            'processing', 'average_peak_width', True)
        pixels = self._cfg.get_param(
            str, 'processing', 'active_pixels', '0-63').split('-')
        self._pixel_range = (int(pixels[0]), int(pixels[-1]))

        self._sample_scale = self._cfg.get_param(
            float, 'processing', 'sample_scale', 1.0)
        self._calibration_scale = self._cfg.get_param(
            float, 'processing', 'calibration_scale', 1.0)
        self._cal_background_scale = self._cfg.get_param(
        ...
        self._lo_integ_range = self._cfg.get_param(
            float, 'processing', 'lo_integ_range', -0.18)
        self._hi_integ_range = self._cfg.get_param(
            float, 'processing', 'hi_integ_range', 0.18)
        self._reset_negatives = self._cfg.get_bool(
            'processing', 'reset_negatives', False)
        ...
        self._filter_ws = FilterPixelsTubes(self._analyse_tubes, self._pixel_range,
                                            self._pixels_per_tube, 0)
    ...
    def _load_merge(self, runs : List[str], output_ws : str, lopts : LoaderOptions):
        ...
        load_merge(LoadPLN, runs, output_ws, lopts,
