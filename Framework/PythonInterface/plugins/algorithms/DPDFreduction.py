...
class DPDFreduction(PythonAlgorithm):
    ...
    def _maskDetectors(self, dwn, vwn):
        """Apply vanadium mask to data workspace, first grouping the data if neccessary
        :param dwn: data workspace name
        :param vwn: vanadium workspace name
        :return: nothing
        :except: FileNotFoundError if vanadium was grouped but grouping file not accessible or existent
        """
        # Find out if the vanadium is grouped, and group the data if this is the case
        vanadium = api.mtd[vwn]
        history = vanadium.getHistory()
        for algorithm in history.getAlgorithmHistories():
            if algorithm.name() == "DgsReduction":
                file_name = algorithm.getPropertyValue("GroupingFile")
                if file_name:
                    # Vanadium has been grouped. Apply same grouping to the data workspace
                    if os.path.isfile(file_name):
                        api.GroupDetectors(InputWorkspace=dwn, MapFile=file_name,
                                           OutputWorkspace=dwn, PreserveEvents=True)
                    else:
                        raise FileNotFoundError(errno.ENOENT, os.strerror(errno.ENOENT), file_name)
        api.MaskDetectors(Workspace=dwn, MaskedWorkspace=vwn)
    ...
    def PyExec(self):
        ...
        try:
            ...
            # for every detector we have a histogram versus energy transfer.
        ...
        self._maskDetectors(wn_reduced, wn_van)
        if self._ecruns:
            self._maskDetectors(wn_ec_reduced, wn_van)
