...
class DPDFreduction(api.PythonAlgorithm):
    ...
    def PyInit(self):
        ...
        self.declareProperty(kapi.FloatArrayProperty("EnergyBins", [1.5], validator=e_validator), "Energy transfer binning scheme (in meV)")
        self.setPropertyGroup("EnergyBins", titleConfigurationOptions)

        q_validator = kapi.FloatArrayLengthValidator(0, 3)
        self.declareProperty(
            kapi.FloatArrayProperty("MomentumTransferBins", list(), validator=q_validator),
        ...
        self.declareProperty(api.MatrixWorkspaceProperty("OutputWorkspace", "S_Q_E_sliced", kapi.Direction.Output), "Output workspace")
    ...
    def validateInputs(self):
        ...
        if len(qbins) not in (0, 1, 3):
            issues["MomentumTransferBins"] = "Momentum transfer bins is a list of zero (empty list), one, or three values"
        return issues
    ...
    def PyExec(self):  # noqa
        ...
        if self._ecruns:
            ...
            if abs(Ei - ec_Ei) > Ei_std:
                raise RuntimeError("Empty can runs were obtained at a significant" + " different incident energy than the sample runs")
        ...
        if self._ecruns:
            ...
            sapi.DgsReduction(
                SampleInputWorkspace=wn_ec_data,
        ...
        factor = 1.0 / 5  # a reasonable (heuristic) value
        dtheta *= factor
        ...
        sapi.BinMD(InputWorkspace=wn_sqe, AxisAligned=1, AlignedDim0=Qrange, AlignedDim1=deltaErange, OutputWorkspace=wn_sqeb)

        # Slice the data by transforming to a Matrix2Dworkspace,
        # with deltaE along the vertical axis
        sapi.ConvertMDHistoToMatrixWorkspace(InputWorkspace=wn_sqeb, Normalization="NumEventsNormalization", OutputWorkspace=wn_sqes)
        ...
        if self._clean:
            for name in (wn_van, wn_reduced, wn_ste, wn_van_st, wn_sten, wn_steni, wn_sqe, wn_sqeb, wn_sqesn, "PreprocessedDetectorsWS"):
