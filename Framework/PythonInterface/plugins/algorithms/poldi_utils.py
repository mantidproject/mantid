...
def _get_instrument_settings_from_log(ws: Workspace2D) -> Tuple[float, np.ndarray[float], float, float]:
    """
...
def _get_dspac_limits(tth_min: float, tth_max: float, lambda_min: float, lambda_max: float) -> Tuple[float, float]:
    """
...
def _get_final_dspac_array(bin_width: float, dspac_min: float, dspac_max: float, time_max: float) -> np.ndarray[float]:
    """
...
def _get_max_arrival_time(
    l1: float, l1_chop: float, l2s: Sequence[float], tths: Sequence[float], lambda_max: float, offset: float
) -> float:
    """# return TOF not arrival time?
    Function to calculate arrival time of longest wavelength neutron from first slit opening
    ...
    # calc time of longest wavelength neutron to reach detector with the largest total path from first slit opening
    time_max = UnitConversion.run("Wavelength", "TOF", lambda_max, l1 - l1_chop, DeltaEModeType.Elastic, params) + offset
    return time_max
