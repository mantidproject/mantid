...
import numpy
from mantid.api import PythonAlgorithm, AlgorithmFactory, WorkspaceUnitValidator, IEventWorkspaceProperty, WorkspaceProperty
from mantid.kernel import Direction, CompositeValidator, Property, CompositeRelation, FloatMandatoryValidator
...
class CropWorkspaceForMDNorm(PythonAlgorithm):
    ...
    def summary(self):
        return "Crops an event workspace and store the information" + " about trajectories limits in the run object."
    ...
    def PyInit(self):
        ...
        allowed_units = CompositeValidator(
            [WorkspaceUnitValidator("DeltaE"), WorkspaceUnitValidator("Momentum")], relation=CompositeRelation.OR
        )
        self.declareProperty(
            IEventWorkspaceProperty("InputWorkspace", "", direction=Direction.Input, validator=allowed_units),
            doc="Input workspace. It has to be an event workspace" + " with units of energy transfer or momentum",
        )
        self.declareProperty(
            name="XMin",
    ...
    def PyExec(self):
        ...
        if run_obj.hasProperty("MDNorm_low"):
            ...
            try:
                ...
                raise RuntimeError(
                    "Could not compare old and new values for 'MDNorm_low' log. "
        ...
        if run_obj.hasProperty("MDNorm_high"):
            try:
                ...
                raise RuntimeError(
                    "Could not compare old and new values for 'MDNorm_high' log. "
                    + "Make sure the length of the old data is equal to the number of spectra"
                )
