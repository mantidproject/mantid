...
class ExportExperimentLog(PythonAlgorithm):
    ...
    def PyInit(self):
        ...
        self.declareProperty(FileProperty("OutputFilename", "", FileAction.Save, [".txt, .csv"]), "Output file of the experiment log.")

        filemodes = ["append", "fastappend", "new"]
        self.declareProperty(
            "FileMode",
            "append",
            mantid.kernel.StringListValidator(filemodes),
        ...
        des = (
            "Output file format.  'tab' format will insert a tab between 2 adjacent values; 'comma' will put a , instead. "
    ...
    def _createLogFile(self):
        ...
        try:
            ...
            raise RuntimeError("Unable to write file %s. Check permission. Error message %s." % (self._logfilename, str(err)))
