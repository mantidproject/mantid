...
from mantid.api import (AlgorithmFactory, DistributedDataProcessorAlgorithm, FileProperty, FileAction, Progress)
...
class PowderReduceP2D(DistributedDataProcessorAlgorithm):
    ...
    def PyInit(self):
        # Input files
        self.declareProperty(FileProperty('SampleData', '', action = FileAction.Load, direction = Direction.Input), doc = 'Datafile that should be used.')
        self.declareProperty('DoIntensityCorrection', False, direction = Direction.Input, doc = 'If set to True you have to declare a vanadium measurement for intensity correction.')
        self.declareProperty(FileProperty('VanaData', '', action = FileAction.OptionalLoad, direction = Direction.Input), doc = 'Vanadium measurement for intensity correction.')
        self.declareProperty('DoBackgroundCorrection', False, direction = Direction.Input, doc = 'If set to True you have to declare an empty can measurement for background correction.')
        self.declareProperty(FileProperty('EmptyData', '', action = FileAction.OptionalLoad, direction = Direction.Input), doc = 'Empty measurement of the can for background correction.')
        self.declareProperty(FileProperty('CalFile', '', action = FileAction.OptionalLoad, direction = Direction.Input), doc = 'Calibration file.')
        ...
        self.declareProperty(FileProperty('BinEdgesFile', '', action = FileAction.OptionalLoad, direction = Direction.Input), doc = 'BinEdges file used for edgebinning.')
        ...
        # Data range
        self.declareProperty('TwoThetaMin', 50, validator = IntBoundedValidator(lower = 0), direction = Direction.Input, doc = 'Minimum value for 2 Theta. Everything smaller gets removed.')
        self.declareProperty('TwoThetaMax', 132, validator = IntBoundedValidator(lower = 0), direction = Direction.Input, doc = 'Maximum value for 2 Theta. Everything bigger gets removed.')
        self.declareProperty('WavelengthCenter', 0.7, validator = FloatBoundedValidator(lower = 0.0), direction = Direction.Input, doc = 'Center Wavelength is used to calculate automatic values for lambdaMin and lambdaMax if they are not specified.')
        self.declareProperty('LambdaMin', 0.2, validator = FloatBoundedValidator(lower = 0.0), direction = Direction.Input, doc = 'Minimum value for lambda. Everything smaller gets removed. If zero it is not used and values get calculated from center wavelength.')
        self.declareProperty('LambdaMax', 2.0, validator = FloatBoundedValidator(lower = 0.0), direction = Direction.Input, doc = 'Maximum value for lambda. Everything bigger gets removed. If zero it is not used and values get calculated from center wavelength.')
        self.declareProperty('DMin', 0.5, validator = FloatBoundedValidator(lower = 0.0), direction = Direction.Input, doc = 'Minimum value for d. Everything smaller gets removed. If zero it is not used and values get calculated from 2 theta and lambda.')
        self.declareProperty('DMax', 6.0, validator = FloatBoundedValidator(lower = 0.0), direction = Direction.Input, doc = 'Maximum value for d. Everything bigger gets removed. If zero it is not used and values get calculated from 2 theta and lambda.')
        self.declareProperty('DpMin', 0.2, validator = FloatBoundedValidator(lower = 0.0), direction = Direction.Input, doc = 'Minimum value for dp. Everything smaller gets removed. If zero it is not used and values get calculated from 2 theta and lambda.')
        self.declareProperty('DpMax', 2.0, validator = FloatBoundedValidator(lower = 0.0), direction = Direction.Input, doc = 'Maximum value for dp. Everything bigger gets removed. If zero it is not used and values get calculated from 2 theta and lambda.')
        grp2 = 'Data Ranges'
        ...
        self.declareProperty('OutputParTable', 'Detec', direction = Direction.Input, doc = 'If not empty, a name of a table workspace which will contain the calculated par or phx values for the detectors.')
        ...
        self.declareProperty('LowerCutoff', 99.998, direction = Direction.Input, doc = 'The percentage of the average to use as the lower bound.')
        grp4 = 'FilterBadPulses'
        self.setPropertyGroup('LowerCutoff', grp4)
        # Input for RemovePromptPulse
        self.declareProperty('Width', 150, direction = Direction.Input, doc = 'The width of the time of flight (in microseconds) to remove from the data.')
        ...
        self.declareProperty('WorkspaceName', 'POWTEX', direction = Direction.Input, doc = 'The base of the output workspace names. Names will have _group, _cal, _mask appended to them.')
        self.copyProperties('LoadDiffCal', ['InstrumentName', 'InstrumentFilename', 'MakeGroupingWorkspace', 'MakeCalWorkspace', 'MakeMaskWorkspace', 'TofMin', 'TofMax', 'FixConversionIssues'])
        grp6 = 'LoadDiffCal'
        ...
        self.declareProperty('MaskedWorkspace', self.getProperty('WorkspaceName').value + '_mask', direction = Direction.Input, doc = 'If given but not as a SpecialWorkspace2D, the masking from this workspace will be copied. If given as a SpecialWorkspace2D, the masking is read from its Y values.')
        self.copyProperties('MaskDetectors', ['SpectraList', 'DetectorList', 'WorkspaceIndexList', 'ForceInstrumentMasking', 'StartWorkspaceIndex', 'EndWorkspaceIndex', 'ComponentList'])
        grp7 = 'MaskDetectors'
        ...
        self.declareProperty('AttenuationXSection', 5.08, direction = Direction.Input, doc = 'The ABSORPTION cross-section, at 1.8 Angstroms, for the sample material in barns. Column 8 of a table generated from http://www.ncnr.nist.gov/resources/n-lengths/.')
        self.declareProperty('ScatteringXSection', 5.1, direction = Direction.Input, doc = 'The (coherent + incoherent) scattering cross-section for the sample material in barns. Column 7 of a table generated from http://www.ncnr.nist.gov/resources/n-lengths/.')
        self.declareProperty('SampleNumberDensity', 0.07192, direction = Direction.Input, doc = 'The number density of the sample in number of atoms per cubic angstrom if not set with SetSampleMaterial.')
        self.declareProperty('CylinderSampleHeight', 4, direction = Direction.Input, doc = 'The height of the cylindrical sample in centimetres.')
        self.declareProperty('CylinderSampleRadius', 0.4, direction = Direction.Input, doc = 'The radius of the cylindrical sample in centimetres.')
        self.declareProperty('NumberOfSlices', 10, direction = Direction.Input, doc = 'The number of slices into which the cylinder is divided for the calculation.')
        self.declareProperty('NumberOfAnnuli', 10, direction = Direction.Input, doc = 'The number of annuli into which each slice is divided for the calculation.')
        self.copyProperties('CylinderAbsorption', ['ScatterFrom', 'NumberOfWavelengthPoints', 'ExpMethod', 'EMode', 'EFixed', 'CylinderAxis'])
        ...
        self.declareProperty('FWHM', 2, direction = Direction.Input, doc = 'The number of points covered, on average, by the fwhm of a peak. Passed through to FindPeaks. Default 7.')
        self.declareProperty('Tolerance', 2, direction = Direction.Input, doc = 'A measure of the strictness desired in meeting the condition on peak candidates. Passed through to FindPeaks. Default 4.')
        self.declareProperty('PeakPositionTolerance', 0.05, direction = Direction.Input, doc = 'Tolerance on the found peaks positions against the input peak positions. A non-positive value turns this option off.')
        self.declareProperty('BackgroundType', 'Quadratic', direction = Direction.Input, doc = 'The type of background of the histogram. Present choices include Linear and Quadratic. Allowed values: [Linear, Quadratic]')
        ...
        # Input for FFTSmooth
        self.declareProperty('Filter', 'Butterworth', direction = Direction.Input, doc = 'The type of the applied filter. Allowed values: [Zeroing, Butterworth]')
        self.declareProperty('Params', '20,2', direction = Direction.Input, doc = 'The filter parameters: For Zeroing, 1 parameter: n - an integer greater than 1 meaning that the Fourier coefficients with frequencies outside the 1/n of the original range will be set to zero. For Butterworth, 2 parameters: n and order, giving the 1/n truncation and the smoothing order.')
        self.declareProperty('IgnoreXBins', True, direction = Direction.Input, doc = 'Ignores the requirement that X bins be linear and of the same size. Set this to true if you are using log binning. The output X axis will be the same as the input either way.')
    ...
    def processData(self, filename, wsName):
        ...
        FindDetectorsPar(InputWorkspace = wsName, ReturnLinearRanges = self._returnLinearRanges, ParFile = self._parFile, OutputParTable = self._outputParTable)
        ...
        LoadDiffCal(InputWorkspace = wsName, InstrumentName = self._instrumentName, InstrumentFilename = self._instrumentFilename, Filename = self._filename, MakeGroupingWorkspace = self._makeGroupingWorkspace, MakeCalWorkspace = self._makeCalWorkspace, MakeMaskWorkspace = self._makeMaskWorkspace, WorkspaceName = self._workspaceName, TofMin = self._tofMin, TofMax = self._tofMax, FixConversionIssues = self._fixConversionIssues)
        MaskDetectors(Workspace = wsName, SpectraList = self._spectraList, DetectorList = self._detectorList, WorkspaceIndexList = self._workspaceIndexList, MaskedWorkspace = self._maskedWorkspace, ForceInstrumentMasking = self._forceInstrumentMasking, StartWorkspaceIndex = self._startWorkspaceIndex, EndWorkspaceIndex = self._endWorkspaceIndex, ComponentList = self._componentList)
    ...
    def processVana(self, wsName):
        CylinderAbsorption(InputWorkspace = wsName, OutputWorkspace = 'Atten', AttenuationXSection = self._attenuationXSection, ScatteringXSection = self._scatteringXSection, SampleNumberDensity = self._sampleNumberDensity, NumberOfWavelengthPoints = self._numberOfWavelengthPoints, ExpMethod = self._expMethod, EMode = self._eMode, EFixed = self._eFixed, CylinderSampleHeight = self._cylinderSampleHeight, CylinderSampleRadius = self._cylinderSampleRadius, NumberOfSlices = self._numberOfSlices, NumberOfAnnuli = self._numberOfAnnuli)
        Divide(LHSWorkspace = wsName, RHSWorkspace = 'Atten', OutputWorkspace = wsName)

    def binDataEdge(self, wsName):
        Bin2DPowderDiffraction(InputWorkspace = wsName, OutputWorkspace = wsName, BinEdgesFile = self._binEdgesFile, NormalizeByBinArea = self._normalizeByBinArea)

    def binDataLog(self, wsName, dSpaceBinning, dPerpBinning):
        Bin2DPowderDiffraction(InputWorkspace = wsName, OutputWorkspace = wsName, dSpaceBinning = [self._dMin - dSpaceBinning, dSpaceBinning self._dMax + dSpaceBinning], dPerpendicularBinning = [self._dPMin - dPerpBinning, dPerpBinning self.dPMax + dPerpBinning])

    def postProcessVana(self, wsName):
        StripVanadiumPeaks(InputWorkspace = wsName, OutputWorkspace = wsName, FWHM = self._FWHM, Tolerance = self._tolerance, BackgroundType = self._backgroundType, HighBackground = self._highBackground, PeakPositionTolerance = self._peakPositionTolerance, WorkspaceIndex = self._workspaceIndex)
        FFTSmooth(InputWorkspace = wsName, OutputWorkspace = wsName, WorkspaceIndex = self._workspaceIndexSmooth, Filter = self._filter, Params = self._params, IgnoreXBins = self._ignoreXBins, AllSpectra = self._allSpectra)
    ...
    def PyExec(self):
        # Creating ProgressReporter
        # TODO: Add ProgressReporting
        endrange = 100
        ...
        if self._doEdge:
            ...
            self.binDataLog(self._sampleWS, self._dSpaceBinning, self._dPerpendicularBinning)
        ...
        if self._doEmpty:
            ...
            if self._doEdge:
                ...
                self.binDataLog(self._emptyWS, self._dSpaceBinning, self._dPerpendicularBinning)
        ...
        if self._doVana:
            ...
            if self._doEdge:
                ...
                self.binDataLog(self._vanaWS, self._dSpaceBinning, self._dPerpendicularBinning)
        ...
        SaveP2D(Workspace = self._sampleWS, OutputFile = self._outputFile, RemoveNaN = True, RemoveNegatives = True, CutData = True, TthMin = self._tthMin, TthMax = self._tthMax, lambdaMin = self._lambdaMin, LambdaMax = self._lambdaMax, DMin = self._dMin, DMax = self._dMax, DpMin = self._dpMin, DpMax = self._dpMax)
