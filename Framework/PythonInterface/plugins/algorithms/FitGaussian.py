...
class FitGaussian(PythonAlgorithm):
    ...
    def PyInit(self):
        # input
        self.declareProperty(MatrixWorkspaceProperty("Workspace", "", Direction.Input), doc="input workspace")
        self.declareProperty(
            name="Index", defaultValue=0, validator=IntBoundedValidator(lower=0), doc="workspace index - which spectrum to fit"
        )

        # output
        self.declareProperty(name="PeakCentre", defaultValue=0.0, direction=Direction.Output, doc="the centre of the fitted peak")
        self.declareProperty(
            name="Sigma", defaultValue=0.0, direction=Direction.Output, doc="the sigma of the fitted peak; 0. if fitting was not successful"
        )
    ...
    def PyExec(self):
        ...
        if not "success" == fit_output.OutputStatus:
            self._warning(
                ...
                + " in workspace "
                + workspace.name()
                + "fit was not successful. Input guess parameters were "
                + str(fitFun)
            )
            return
