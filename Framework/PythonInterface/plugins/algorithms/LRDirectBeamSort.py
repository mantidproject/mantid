...
class LRDirectBeamSort(PythonAlgorithm):
    ...
    def PyInit(self):
        ...
        self.declareProperty("UseLowResCut",
                             False,
                             direction=Direction.Input,
                             doc="If True, an x-direction cut will be determined and used")
        self.declareProperty("ComputeScalingFactors",
                             True,
                             direction=Direction.Input,
        ...
        self.declareProperty(FileProperty("ScalingFactorFile", "", action=FileAction.OptionalSave, extensions=['cfg']),
    ...
    def _compute_scaling_factors(self, lr_data_sorted):
        ...
        for g in group_list:
            ...
            for run in g:
                ...
                # Direct beam signal
                workspace = RefRoi(InputWorkspace=run,
                                   ConvertToQ=False,
            ...
            tof_min = source_detector_distance / h * m * (wl + wl_offset * 60.0 / chopper_speed -
                                                          1.7 * 60.0 / chopper_speed) * 1e-4
            tof_max = source_detector_distance / h * m * (wl + wl_offset * 60.0 / chopper_speed +
                                                          1.7 * 60.0 / chopper_speed) * 1e-4
