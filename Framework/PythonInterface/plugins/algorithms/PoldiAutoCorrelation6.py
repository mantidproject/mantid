...
from plugins.algorithms.poldi_utils import (
    _get_instrument_settings_from_log,
    _get_max_arrival_time,
    _get_dspac_limits,
    _get_final_dspac_array,
)
...
class PoldiAutoCorrelation(PythonAlgorithm):
    ...
    def PyExec(self):
        ...
        cycle_time, slit_offsets, t0_const, l1_chop = _get_instrument_settings_from_log(ws)
        ...
        time_max = _get_max_arrival_time(l1, l1_chop, l2s, tths, lambda_max, slit_offsets[0])
        npulses = int(time_max // cycle_time)
        # get final d-spacing array
        dspac_min, dspac_max = _get_dspac_limits(tths.min(), tths.max(), lambda_min, lambda_max)
        bin_width = ws.readX(0)[1] - ws.readX(0)[0]
        dspacs = _get_final_dspac_array(bin_width, dspac_min, dspac_max, time_max)
