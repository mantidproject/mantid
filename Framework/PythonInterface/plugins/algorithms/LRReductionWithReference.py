...
from mantid.simpleapi import AlgorithmFactory, CreateWorkspace, DeleteWorkspace, Divide, LiquidsReflectometryReduction
...
class LRReductionWithReference(DataProcessorAlgorithm):
    ...
    def PyExec(self):
        ...
        model_wksp = CreateWorkspace(
            DataX=q, DataY=model_reflectivity, DataE=np.zeros(len(q)), UnitX=norm_wksp.getAxis(0).getUnit().unitID()
        )
    ...
    def calculate_reflectivity(self, model_description, q, q_resolution=0.025):
        ...
        """
        from refl1d.names import Experiment, Parameter, QProbe, Slab, SLD

        zeros = np.zeros(len(q))
        dq = q_resolution * q
        # The QProbe object represents the beam
        probe = QProbe(q, dq, data=(zeros, zeros))
        sample = Slab(material=SLD(name="back", rho=model_description["back_sld"]), interface=model_description["back_roughness"])

        # Add each layer
        for i, layer in enumerate(model_description["layers"]):
            sample = sample | Slab(
                material=SLD(name="layer%s" % i, rho=layer["sld"], irho=layer["isld"]),
