...
class CalibrateRectangularDetectors(PythonAlgorithm):
    ...
    def PyInit(self):
        self.declareProperty(MultipleFileProperty(name="RunNumber", extensions=EXTENSIONS_NXS), "Event file")
        validator = IntArrayBoundedValidator(lower=0)
        self.declareProperty(IntArrayProperty("Background", values=[0], direction=Direction.Input, validator=validator))
        ...
        self.declareProperty("LowResRef", 0., "Reference DIFC for resolution removal. Zero skips the correction")
        self.declareProperty("MaxOffset", 1.0, "Maximum absolute value of offsets; default is 1")
        self.declareProperty("CrossCorrelation", True, "CrossCorrelation if True; minimize using many peaks if False.")
        ...
        self.declareProperty("PeakWindowMax", 0., "Maximum window around a peak to search for it. Optional.")
        self.declareProperty(
            ITableWorkspaceProperty("FitwindowTableWorkspace", "", Direction.Input, PropertyMode.Optional),
        ...
        self.declareProperty(
            MatrixWorkspaceProperty("DetectorResolutionWorkspace", "", Direction.Input, PropertyMode.Optional),
        ...
        self.declareProperty("PeakFunction", "Gaussian",
                             StringListValidator(["BackToBackExponential", "Gaussian", "Lorentzian"]),
        ...
        self.declareProperty(
            IntArrayProperty("DetectorsPeaks", []),
        ...
        self.declareProperty("FilterBadPulses", True,
    ...
    def _loadData(self, filename, filterWall=None):
        ...
        CompressEvents(InputWorkspace=wkspName, OutputWorkspace=wkspName, Tolerance=COMPRESS_TOL_TOF)  # 100ns
    ...
    def _saveCalibration(self, wkspName, calibFilePrefix):
        ...
        if "dspacemap" in self._outTypes:
            ...
            SaveDspacemap(InputWorkspace=wkspName + "offset", DspacemapFile=outfilename)
        if "calibration" in self._outTypes:
            ...
            SaveCalFile(OffsetsWorkspace=wkspName + "offset",
                        ...
                        Filename=calibFilePrefix + '.cal')
            # the real version
            outfilename = calibFilePrefix + '.h5'
            if os.path.exists(outfilename):
                os.unlink(outfilename)
            ConvertDiffCal(OffsetsWorkspace=wkspName + "offset", OutputWorkspace=wkspName + "cal")
    ...
    def _cccalibrate(self, wksp):
        ...
        CrossCorrelate(InputWorkspace=wksp,
                       OutputWorkspace=wksp + "cc",
        ...
        GetDetectorOffsets(InputWorkspace=wksp + "cc",
                           OutputWorkspace=wksp + "offset",
                           Step=abs(self._binning[1]),
                           DReference=self._peakpos1,
                           XMin=-self._ccnumber,
                           XMax=self._ccnumber,
        ...
        if self._peakpos2 > 0.0:
            ...
            CrossCorrelate(InputWorkspace=wksp,
                           OutputWorkspace=wksp + "cc2",
                           ReferenceSpectra=refpixel,
                           WorkspaceIndexMin=self._lastpixel + 1,
                           WorkspaceIndexMax=self._lastpixel2,
                           XMin=self._peakmin2,
                           XMax=self._peakmax2)
            # Get offsets for pixels using interval around cross correlations center and peak at peakpos (d-Spacing)
            GetDetectorOffsets(InputWorkspace=wksp + "cc2",
                               OutputWorkspace=wksp + "offset2",
                               Step=abs(self._binning[1]),
                               DReference=self._peakpos2,
                               XMin=-self._ccnumber,
                               XMax=self._ccnumber,
                               MaxOffset=self._maxoffset,
                               MaskWorkspace=wksp + "mask2")
            Plus(LHSWorkspace=wksp + "offset", RHSWorkspace=wksp + "offset2", OutputWorkspace=wksp + "offset")
            Plus(LHSWorkspace=wksp + "mask", RHSWorkspace=wksp + "mask2", OutputWorkspace=wksp + "mask")
        ...
        if self._peakpos3 > 0.0:
            ...
            CrossCorrelate(InputWorkspace=wksp,
                           ...
                           XMax=self._peakmax3)
            # Get offsets for pixels using interval around cross correlations center and peak at peakpos (d-Spacing)
            GetDetectorOffsets(InputWorkspace=wksp + "cc3",
                               OutputWorkspace=wksp + "offset3",
                               Step=abs(self._binning[1]),
                               DReference=self._peakpos3,
                               XMin=-self._ccnumber,
                               XMax=self._ccnumber,
                               MaxOffset=self._maxoffset,
                               MaskWorkspace=wksp + "mask3")
            Plus(LHSWorkspace=wksp + "offset", RHSWorkspace=wksp + "offset3", OutputWorkspace=str(wksp) + "offset")
            Plus(LHSWorkspace=wksp + "mask", RHSWorkspace=wksp + "mask3", OutputWorkspace=wksp + "mask")
    ...
    def _multicalibrate(self, wksp):
        ...
        if len(self._smoothGroups) > 0:
            SmoothData(InputWorkspace=wksp,
                       OutputWorkspace=wksp,
                       NPoints=self._smoothGroups,
        ...
        if resws is not None:
            ...
            if reslowf >= resupf:
                raise NotImplementedError("Allowed resolution range factor, lower boundary " +
                                          "(%f) must be smaller than upper boundary (%f)." % (reslowf, resupf))
        else:
            reslowf = 0.0
            resupf = 0.0
        ...
        if self._smoothoffsets and self._xpixelbin * self._ypixelbin > 1:  # Smooth data if it was summed
            SmoothNeighbours(InputWorkspace=wksp + "offset",
                             OutputWorkspace=wksp + "offset",
                             WeightedSum="Flat",
                             AdjX=self._xpixelbin,
                             AdjY=self._ypixelbin)
        Rebin(InputWorkspace=wksp,
              OutputWorkspace=wksp,
              Params=str(self._binning[0]) + "," + str((self._binning[1])) + "," + str(self._binning[2]))
    ...
    def _focus(self, wksp):
        ...
        wksp = AlignDetectors(InputWorkspace=wksp, OutputWorkspace=wksp, CalibrationWorkspace=str(wksp) + "cal")
        # Diffraction focusing using new calibration file with offsets
        if self._diffractionfocus:
            wksp = DiffractionFocussing(InputWorkspace=wksp,
                                        OutputWorkspace=wksp,
    ...
    def PyExec(self):
        ...
        if len(samRuns) != len(backRuns):
            if (len(backRuns) == 1 and backRuns[0] == 0) or (len(backRuns) <= 0):
                ...
                raise RuntimeError("Number of samples and backgrounds must match (%d!=%d)" %
                                   (len(samRuns), len(backRuns)))
        ...
        for (samNum, backNum) in zip(samRuns, backRuns):
            ...
            if backNum > 0:
                ...
                Minus(LHSWorkspace=samRun, RHSWorkspace=backRun, OutputWorkspace=samRun)
                DeleteWorkspace(backRun)
                CompressEvents(samRun, OutputWorkspace=samRun, Tolerance=COMPRESS_TOL_TOF)  # 100ns
            ...
            if DIFCref > 0:
                RemoveLowResTOF(InputWorkspace=samRun, OutputWorkspace=samRun, ReferenceDIFC=DIFCref)
            ...
            if self._xpixelbin * self._ypixelbin > 1:
                SumNeighbours(InputWorkspace=samRun, OutputWorkspace=samRun, SumX=self._xpixelbin, SumY=self._ypixelbin)
            ...
            if self._xpixelbin * self._ypixelbin > 1 or len(self._smoothGroups) > 0:
                ...
                # super special Jason stuff
                if LRef > 0:
                    samRun = UnwrapSNS(InputWorkspace=samRun, OutputWorkspace=samRun, LRef=LRef)
                if DIFCref > 0:
                    samRun = RemoveLowResTOF(InputWorkspace=samRun, OutputWorkspace=samRun, ReferenceDIFC=DIFCref)
            else:
                samRun = ConvertUnits(InputWorkspace=samRun, OutputWorkspace=samRun, Target="TOF")
