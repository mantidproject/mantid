...
class LoadDNSLegacy(PythonAlgorithm):
    ...
    def PyInit(self):
        self.declareProperty(FileProperty("Filename", "", FileAction.Load, [".d_dat"]), "Name of DNS experimental data file.")

        self.declareProperty(
            FileProperty("CoilCurrentsTable", "", FileAction.OptionalLoad, [".txt"]),
        ...
        self.declareProperty("Normalization", "duration", StringListValidator(normalizations), doc="Kind of data normalization.")
    ...
    def PyExec(self):
        ...
        l1 = np.linalg.norm(component_info.samplePosition() - component_info.sourcePosition())
        ...
        if not pol:
            pol = ["0", "undefined"]
            self.log().warning("Failed to determine polarisation for " + filename + ". Values have been set to undefined.")
        ndet = 24
        unitX = "Wavelength"
        ...
        logs["values"].extend(
            [
                metadata.deterota,
                float(metadata.monitor_counts),
                metadata.duration,
                metadata.huber,
                metadata.huber - metadata.deterota,
                metadata.temp1,
                metadata.temp2,
                metadata.tsp,
            ]
        ...
        logs["names"].extend(
            ["flipper_precession", "flipper_z_compensation", "flipper", "C_a", "C_b", "C_c", "C_z", "polarisation", "polarisation_comment"]
        )
        logs["values"].extend(
            [
                metadata.flipper_precession_current,
                metadata.flipper_z_compensation_current,
                flipper_status,
                metadata.a_coil_current,
                metadata.b_coil_current,
                metadata.c_coil_current,
                metadata.z_coil_current,
                str(pol[0]),
                str(pol[1]),
            ]
        )
        ...
        # slits
        logs["names"].extend(
            ["slit_i_upper_blade_position", "slit_i_lower_blade_position", "slit_i_left_blade_position", "slit_i_right_blade_position"]
        )
        logs["values"].extend(
            [
