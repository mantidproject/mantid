...
class GetEiT0atSNS(mantid.api.PythonAlgorithm):
    ...
    def PyInit(self):
        ...
        self.declareProperty(
            mantid.api.WorkspaceProperty("MonitorWorkspace", "", direction=mantid.kernel.Direction.InOut), "Monitor workspace"
        )
        self.declareProperty("IncidentEnergyGuess", -1.0, doc="Incident energy guess")
    ...
    def PyExec(self):
        ...
        if numpy.mean(wm.getRun()["vChTrans"].value) == 2:
            ...
            try:
                ...
                for sp in range(nsp):
                    if wm.getSpectrum(sp).getDetectorIDs()[0] == -int(component_info.getNumberParameter(root, "ei-mon1-spec")[0]):
                        sp1 = sp
                    if wm.getSpectrum(sp).getDetectorIDs()[0] == -int(component_info.getNumberParameter(root, "ei-mon2-spec")[0]):
                        sp2 = sp
                if sp1 == -1:
                ...
                for i in range(1, Nmax + 1):
                    tmin = min(i * period - 30.0, maxtof)
                    tmax = min(i * period + 30.0, maxtof)
                    if tmin < tmax:
