...
class LoadWANDSCD(PythonAlgorithm):
    ...
    def load_and_group(self, runs: List[str]) -> IMDHistoWorkspace:
        ...
        try:
            ...
            UB = np.eye(3) / (2 * np.pi)  # _tmp_ws.sample().getOrientedLattice().getUB().copy()

        if self.getProperty("ApplyGoniometerTilt").value:
            sgl = np.deg2rad(np.mean(sgl_array))
            sgu = np.deg2rad(np.mean(sgu_array))
            sgl_a = np.array([[1, 0, 0], [0, np.cos(sgl), np.sin(sgl)], [0, -np.sin(sgl), np.cos(sgl)]])  # 'HB2C:Mot:sgl.RBV,1,0,0,-1'
            sgu_a = np.array([[np.cos(sgu), np.sin(sgu), 0], [-np.sin(sgu), np.cos(sgu), 0], [0, 0, 1]])  # 'HB2C:Mot:sgu.RBV,0,0,1,-1'
            UB = sgu_a @ (sgl_a @ UB)  # Apply the Goniometer tilts to the UB matrix
