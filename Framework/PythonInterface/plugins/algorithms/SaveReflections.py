...
def has_modulated_indexing(workspace):
    ...
    """
    return num_additional_indices(workspace) > 0


def num_additional_indices(workspace):
    """Check if this workspace has more than 3 indices

    :params: workspace :: the workspace count indices in
    :returns: the number of additional indices present in the workspace
    """
...
class SaveReflections(PythonAlgorithm):
    ...
    def PyExec(self):
        ...
        FORMAT_MAP[output_format]()(file_name, workspace)
...
class FullprofFormat(object):
    ...
    def write_header(self, f_handle, workspace):
        ...
        """
        num_hkl = 3+num_additional_indices(workspace)
...
class JanaFormat(object):
    ...
    def write_peaks(self, f_handle, workspace):
        ...
        column_format = "#{:>4}{:>4}{:>4}"
        column_format += "".join(["{:>4}" for _ in range(num_additional_indices(workspace))])
