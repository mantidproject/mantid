...
import time
import os
# Use sys for script interruption during development
import sys
...
class VelocityAutoCorrelations(PythonAlgorithm):
	...
	def summary(self):
		return "Imports trajectory data from an nMoldyn-generated .nc file and calculates velocity auto-correlations. Analogous in some respects to the algorithm found in nMoldyn. Timestep must be specified in femtoseconds."

	def PyInit(self):
		self.declareProperty(FileProperty('InputFile','',action=FileAction.Load),doc="Input .nc file with an MMTK trajectory")

		self.declareProperty("Timestep","1.0",direction=Direction.Input,doc="Specify the timestep between trajectory points in the simulation in fs")
	...
	def PyExec(self):
		...
		print "Calculating velocity auto-correlations (resource intensive calculation)..."
		start_time=time.time()
		...
		print "Averaging auto-correlations & scaling with the coherent neutron scattering lenghts...????????????????????"
		start_time=time.time()
