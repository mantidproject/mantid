...
class TOFTOFMergeRuns(PythonAlgorithm):
        ...
        for wsname in wsnames:
            ...
            for prop in self.must_have_properties:
                if not run.hasProperty(prop):
                    message = "Error: Workspace " + wsname + " does not have property " + prop + ". Cannot merge."
    ...
    def PyExec(self):
        ...
        if nentries > 0:
            ...
            api.AddSampleLog(Workspace=wsOutput, LogName="temperature", LogText=str(tmean), LogType="Number", LogUnit="K")
        # sum monitor counts
        ...
        fmt = "%Y-%m-%dT%H:%M:%S%z"
        run_start = [datetime.datetime.fromisoformat(entry) for entry in pdict["run_start"]]
        api.AddSampleLog(Workspace=wsOutput, LogName="run_start", LogText=min(run_start).strftime(fmt), LogType="String")
        # get maximal run_end
        run_end = [datetime.datetime.fromisoformat(entry) for entry in pdict["run_end"]]
