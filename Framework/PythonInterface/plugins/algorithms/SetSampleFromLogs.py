...
class SetSampleFromLogs(DataProcessorAlgorithm):
    ...
    def PyInit(self):
        ...
        self.declareProperty("FindGeometry", True, "Whether to look for the 'Height' parameter in the logs")
        self.declareProperty("FindSample", True, "Whether to look for the sample material in the logs")
        self.declareProperty("FindEnvironment", True, "Whether to look for the sample container in the logs")
        ...
        if self.getProperty("FindGeometry").value:
            if not _hasValue(geometry, "Height"):
                ...
                self.log().information(
                    "Looking for sample height from {} and units from {}".format(heightInContainerNames, heightInContainerUnitsNames)
                )
                ...
                if heightKey:
                    ...
                    warningMsg = "No valid height found in sample logs;" + "we will rely on user input for sample density information."
                    self.log().warning(warningMsg)
    ...
    def PyExec(self):
        ...
        instrEnum = ConfigService.getInstrument(wksp.getInstrumentName())
