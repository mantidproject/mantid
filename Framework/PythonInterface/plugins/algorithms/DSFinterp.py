...
class DSFinterp(PythonAlgorithm):
    ...
    def summary(self):
        return (
            "Given a set of parameter values {Ti} and corresponding structure factors S(Q,E,Ti), this algorithm "
    ...
    def PyInit(self):
        ...
        self.declareProperty(
            StringArrayProperty("Workspaces", values=[], validator=arrvalidator, direction=Direction.Input), doc="list of input workspaces"
        )
        self.declareProperty("LoadErrors", True, direction=Direction.Input, doc="Do we load error data contained in the workspaces?")
        self.declareProperty(
            FloatArrayProperty("ParameterValues", values=[], validator=FloatArrayMandatoryValidator(), direction=Direction.Input),
        ...
        self.declareProperty(
            StringArrayProperty("OutputWorkspaces", values=[], validator=arrvalidator),
...
try:
    ...
    logger.debug(
        "Failed to subscribe algorithm DSFinterp; Python package dsfinterp" "may be missing (https://pypi.python.org/pypi/dsfinterp)"
    )
