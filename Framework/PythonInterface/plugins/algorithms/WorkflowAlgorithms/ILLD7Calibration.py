...
class ILLD7Calibration(PythonAlgorithm):
    ...
    _YIG_D = np.array([5.0521,4.3752,3.3074,3.0938,2.9168,2.7671,2.6384,2.526,2.4269,2.2594,2.1876,2.0075,1.9567,1.9095,1.8656,1.8246,1.7862,1.7501,1.7161,1.684,1.6537,1.5716])

    def category(self):
        return 'ILL\\Diffraction'
        
    def summary(self):
        return 'Performs D7 instrument calibration using YIG scan and returns D7 IPF readable by the NeXus data loader.'

    def seeAlso(self):
        return ['LoadILLPolarizedDiffraction']

    def name(self):
        return 'D7Calibration'       

    def validateInputs(self):
        issues = dict()
        if (self.getProperty('Filenames').isDefault and 
            self.getProperty('ScanWorkspace').isDefault):
            issues['Filenames'] = 'Either a list of filenames containing YIG scan or the workspace with the loaded scan is required for calibration.'
    ...
    def PyExec(self):
        ...
        offsets = self._calibrate_detectors(detector_parameters)
        progress.report()
    ...
    def _get_yig_peaks_positions(self, ws, YIG_D):    
        """Returns a list o tuples with peak centre 
        positions and peak height used for further fitting"""

        # remove all peaks that would require scattering angle above 180 degrees
        # and express the peak positions in scattering angle
    ...
    def _fit_bragg_peaks(self, ws, yig_peaks):
        ...
        for scan in range(ws.getNumberHistograms()):
            ...
            fit_output = Fit(Function=function, 
                             ...
                             CreateOutput=True,
                             Output='out')
            param_table = fit_output.OutputParameters  # table containing the optimal fit parameters                    
    ...
    def _fit_detector_positions(self, ws):
        ...
        lambda_initial = float(self.getPropertyValue("ApproximateWavelength"))
    ...
    def _calculate_pixel_positions(self, parameter_table):
        ...
        respective bank"""
        wavelength = parameter_table.column(1)[1]
    ...
    def _print_parameter_file(self, pixel_offsets):
