...
class ReflectometryILLPreprocess(DataProcessorAlgorithm):
    ...
    @staticmethod
    def _calculate_average_background(transposed_ws: MatrixWorkspace, transposed_bkg_ws_name: MatrixWorkspace,
                                      ranges: List[float]) -> MatrixWorkspace:
    ...
    def _check_angles_match(input_files: str) -> None:
        ...
        for file_no in range(1, len(file_list)):
            ...
            if fabs(dan - dan_test) > tolerance:
                logger.warning(
                    'Different detector angles detected! {0}: {1}, {2}: {3}'.format(r, dan, r_test, dan_test))
    ...
    def _recalculate_average_chopper_params(ws: MatrixWorkspace) -> None:
        ...
        logs_to_average = ['chopper1.speed_average', 'chopper1.phase_average', 'chopper2.speed_average',
                           'chopper2.phase_average']
    ...
    def PyInit(self):
        ...
        self.setPropertySettings('DirectBeamDetectorAngle', EnabledWhenProperty('AngleOption', PropertyCriterion.IsEqualTo,
                                                                                'DetectorAngle'))
        ...
        self.declareProperty(IntArrayProperty(Prop.FOREGROUND_HALF_WIDTH, validator=max_two_non_negative_ints),
    ...
    def _convert_to_wavelength(self, ws: MatrixWorkspace) -> MatrixWorkspace:
        ...
        wavelength_ws = ConvertUnits(InputWorkspace=ws,
                                     OutputWorkspace=wavelength_ws_name,
                                     Target='Wavelength',
                                     EMode='Elastic',
                                     EnableLogging=self._subalgLogging)
        self._cleanup.cleanup(ws)
    ...
    def _gravity_correction(self, ws: MatrixWorkspace) -> MatrixWorkspace:
        ...
        new_delay_angle = delay_angle - poffoff / 2.
        new_delay = delay + ((delay_angle - new_delay_angle) / 360.) * period
    ...
    def _input_ws(self) -> MatrixWorkspace:
        ...
        ws = LoadAndMerge(Filename=input_files,
    ...
    def _normalise_to_flux(self, det_ws: MatrixWorkspace, mon_ws: MatrixWorkspace) -> MatrixWorkspace:
        ...
        if method == FluxNormMethod.MONITOR:
            ...
            min_x = mon_xs[0]
            max_x = mon_xs[-1]
            normalised_ws = NormaliseToMonitor(InputWorkspace=det_ws,
    ...
    def _subtract_flat_bkg(self, ws: MatrixWorkspace) -> MatrixWorkspace:
        ...
        if method == BkgMethod.CONSTANT or method == BkgMethod.LINEAR:
            ...
            transposed_bkg_ws = CalculatePolynomialBackground(InputWorkspace=transposed_ws,
    ...
    def _water_calibration(self, ws: MatrixWorkspace) -> MatrixWorkspace:
        ...
        rebinned_water_ws = RebinToWorkspace(WorkspaceToRebin=water_ws,
                                             WorkspaceToMatch=ws,
                                             OutputWorkspace=rebinned_water_ws_name,
                                             EnableLogging=self._subalgLogging)
        calibrated_ws_name = self._names.withSuffix('water_calibrated')
        calibrated_ws = Divide(LHSWorkspace=ws,
                               RHSWorkspace=rebinned_water_ws,
                               OutputWorkspace=calibrated_ws_name,
                               EnableLogging=self._subalgLogging)
