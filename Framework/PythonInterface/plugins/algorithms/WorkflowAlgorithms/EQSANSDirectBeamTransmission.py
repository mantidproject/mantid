...
class EQSANSDirectBeamTransmission(PythonAlgorithm):
    ...
    def PyExec(self):
        ...
        if (
            self.getProperty("FitFramesTogether").value
            or not workspace.getRun().hasProperty("is_frame_skipping")
            or (workspace.getRun().hasProperty("is_frame_skipping") and workspace.getRun().getProperty("is_frame_skipping").value == 0)
        ):
        ...
        if trans_ws is None:
            ...
            def _crop_and_compute(wl_min_prop, wl_max_prop, suffix):
                ...
                trans_ws, raw_ws = TransmissionUtils.calculate_transmission(
                    self, sample_ws, empty_ws, first_det, "__transmission_" + suffix
                )
                alg = TransmissionUtils.simple_algorithm(
                    "RebinToWorkspace",
                    {
            ...
            alg = TransmissionUtils.simple_algorithm(
                "Plus", {"LHSWorkspace": trans_frame_1, "RHSWorkspace": trans_frame_2, "OutputWorkspace": "__transmission"}
            )
            trans_ws = alg.getProperty("OutputWorkspace").value
            ...
            alg = TransmissionUtils.simple_algorithm(
                "Plus", {"LHSWorkspace": raw_frame_1, "RHSWorkspace": raw_frame_2, "OutputWorkspace": "__transmission_unfitted"}
            )
            raw_ws = alg.getProperty("OutputWorkspace").value
