...
class IndirectSampleChanger(DataProcessorAlgorithm):
    ...
    def PyInit(self):
        self.declareProperty(
            name="Instrument", defaultValue="IRIS", validator=StringListValidator(["IRIS", "OSIRIS"]), doc="The name of the instrument."
        )
        self.declareProperty(
            name="Analyser",
            defaultValue="",
        ...
        self.declareProperty(name="FirstRun", defaultValue=-1, validator=IntBoundedValidator(lower=0), doc="First Sample run-number.")
        self.declareProperty(name="LastRun", defaultValue=-1, validator=IntBoundedValidator(lower=0), doc="Last Sample run-number.")
        self.declareProperty(name="NumberSamples", defaultValue=-1, validator=IntBoundedValidator(lower=0), doc="Increment for run-number.")

        self.declareProperty(
            IntArrayProperty(name="SpectraRange", values=[0, 1], validator=IntArrayLengthValidator(2)),
        ...
        self.declareProperty(name="SampleEnvironmentLogName", defaultValue="Position", doc="Name of the sample environment log entry")
        ...
        self.declareProperty(name="MSDFit", defaultValue=False, doc='Perform an MSDFit. Do not use with GroupingMethod as "All"')

        self.declareProperty(name="WidthFit", defaultValue=False, doc='Perform a 2 peak width Fit. Do not use with GroupingMethod as "All"')
    ...
    def validateInputs(self):
        ...
        if self._spectra_range[0] > self._spectra_range[1]:
            ...
            spectra_parameters = get_ipf_parameters_from_run(
                self._run_first, self._instrument_name, self._analyser, self._reflection, ["spectra-min", "spectra-max"]
            )
            if "spectra-min" in spectra_parameters and "spectra-max" in spectra_parameters:
                if self._spectra_range[0] < spectra_parameters["spectra-min"] or self._spectra_range[1] > spectra_parameters["spectra-max"]:
                    issues["SpectraRange"] = "The spectra range must be between {0} and {1} for the {2} instrument".format(
                        str(int(spectra_parameters["spectra-min"])), str(int(spectra_parameters["spectra-max"])), self._instrument_name
                    )
