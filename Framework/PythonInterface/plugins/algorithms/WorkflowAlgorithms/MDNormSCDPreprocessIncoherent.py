...
from mantid.simpleapi import (ConvertUnits, CropWorkspace, LoadInstrument, Minus, Load, DeleteWorkspace, LoadIsawDetCal,
                              LoadMask, GroupDetectors, Rebin, MaskDetectors, SumSpectra, SortEvents, IntegrateFlux,
                              AnvredCorrection, NormaliseByCurrent)
    ...
    def PyInit(self):
        ...
        self.declareProperty(
            WorkspaceProperty("SolidAngleOutputWorkspace",
                              ...
                              direction=Direction.Output), "Output Workspace for Solid Angle")

        self.declareProperty(
            WorkspaceProperty("FluxOutputWorkspace", "", optional=PropertyMode.Mandatory, direction=Direction.Output),
    ...
    def PyExec(self):
        ...
        if _background:
            Load(Filename=self.getProperty("Background").value,
                 ...
                 FilterByTofMax=self.getProperty("FilterByTofMax").value)
            if _norm_current:
                NormaliseByCurrent(InputWorkspace='__bkg', OutputWorkspace='__bkg')
            else:
        ...
        # Create solid angle
        Rebin(InputWorkspace='__van', OutputWorkspace=_SA_name, Params=rebin_param, PreserveEvents=False)
