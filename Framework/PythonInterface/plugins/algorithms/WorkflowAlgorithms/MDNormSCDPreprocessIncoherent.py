...
class MDNormSCDPreprocessIncoherent(DataProcessorAlgorithm):
    ...
    def PyInit(self):
        ...
        # Grouping
        self.declareProperty(
            FileProperty(name="GroupingFile", defaultValue="", action=FileAction.OptionalLoad, extensions=[".xml", ".map"]),
        ...
        # Output
        self.declareProperty(
            WorkspaceProperty("SolidAngleOutputWorkspace", "", optional=PropertyMode.Mandatory, direction=Direction.Output),
    ...
    def PyExec(self):
        ...
        if _background:
            Load(
                Filename=self.getProperty("Background").value,
