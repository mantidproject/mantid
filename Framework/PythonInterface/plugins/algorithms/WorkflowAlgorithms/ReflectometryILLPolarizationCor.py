...
from mantid.api import (AlgorithmFactory, DataProcessorAlgorithm, FileAction, FileProperty, MatrixWorkspace, mtd, WorkspaceGroup,
...
class ReflectometryILLPolarizationCor(DataProcessorAlgorithm):
    ...
    def PyInit(self):
        ...
        self.declareProperty(StringArrayProperty(Prop.INPUT_WS, values=[], validator=mandatory_input_workspaces),
    ...
    def _correct(self, wss: List[str], eff_ws: MatrixWorkspace) -> WorkspaceGroup:
        ...
        corr_wss = PolarizationEfficiencyCor(InputWorkspaces=wss,
                                             OutputWorkspace=corr_wss_name,
                                             Flippers=flippers,
                                             Efficiencies=eff_ws,
    ...
    def _efficiencies(self, ref_ws: MatrixWorkspace) -> MatrixWorkspace:
        ...
        eff_ws = LoadILLPolarizationFactors(Filename=filename,
                                            OutputWorkspace=eff_ws_name,
                                            WavelengthReference=ref_ws,
                                            EnableLogging=self._subalg_logging)
    ...
    def _flipper_configuration(self, wss: List[str]) -> str:
        ...
        if len(wss) == 2:
            ...
            for ws in wss:
                ...
                if analyzer_translation < 0. or analyzer_translation >= 200.:
