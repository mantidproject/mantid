...
class ISISIndirectEnergyTransferSampleChanger(DataProcessorAlgorithm):
    ...
    def PyInit(self):
        ...
        self.declareProperty(name='OutputWorkspaceBaseName', defaultValue='',
                             direction=Direction.Input,
                             doc='Base name for output reduced workspace')
    ...
    def PyExec(self):
        ...
        self._workspace_names, _, masked_detectors = load_files(self._data_files,
                                                                self._ipf_filename,
        ...
        for sample in sample_sorted_lists:
            sample_name_suffix = "_" + SAMP_POSITION[sample] + "_Reduced"
            self._run_workspace_list(sample_sorted_lists[sample], sample_name_suffix, masked_detectors, process_prog)
            output_ws_name.append(self._output_base_name+sample_name_suffix)
    ...
    def _setup(self):
        ...
        self._output_base_name = self.getPropertyValue("OutputWorkspaceBaseName")
    ...
    def _run_workspace_list(self, wksp_list, sample, masked_detectors, process_prog):
        for c_ws_name in wksp_list:
        ...
        output_workspace_names = [rename_reduction(ws_name, self._sum_files) for ws_name in wksp_list]
        ...
        GroupWorkspaces(InputWorkspaces=output_workspace_names,
                        OutputWorkspace=self._output_base_name+sample)

    def _sort_runs_by_sample(self):
        sample_sorted_lists = {}
        for ws_name in self._workspace_names:
            samp_posn = AnalysisDataService.retrieve(ws_name).getRun().getProperty("SAMP_POSN").value[1]
            if samp_posn in sample_sorted_lists.keys():
                sample_sorted_lists[samp_posn].append(ws_name)
            else:
                sample_sorted_lists[samp_posn] = [ws_name]
        return sample_sorted_lists


# Register algorithm with Mantid
AlgorithmFactory.subscribe(ISISIndirectEnergyTransferSampleChanger)
