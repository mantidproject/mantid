...
def load_monitors(self, property_manager):
    ...
    if property_manager.existsProperty("TransmissionNormalisation"):
        if property_manager.getProperty("TransmissionNormalisation").value == "Monitor":
            monitor_det_ID = int(component_info.getNumberParameter(component_info.root(), "default-incident-monitor-spectrum")[0])
        else:
            monitor_det_ID = int(component_info.getNumberParameter(component_info.root(), "default-incident-timer-spectrum")[0])
        ...
        def _normalise(workspace):
            ...
            alg = _execute(
                p.valueAsStr,
                {"InputWorkspace": workspace, "OutputWorkspace": workspace, "ReductionProperties": property_manager_name},
                is_name=False,
            )
            msg = ""
            if alg.existsProperty("OutputMessage"):
    ...
    if abs(sum(spec0) - sum(spec_last)) > 0.000001:
        ...
        alg = _execute(
            "RebinToWorkspace", {"WorkspaceToRebin": empty_ws, "WorkspaceToMatch": reference_ws, "OutputWorkspace": empty_ws_name}
        )
        empty_ws = alg.getProperty("OutputWorkspace").value
        alg = _execute(
            "RebinToWorkspace", {"WorkspaceToRebin": sample_ws, "WorkspaceToMatch": reference_ws, "OutputWorkspace": sample_ws_name}
        )
        sample_ws = alg.getProperty("OutputWorkspace").value

    alg = _execute(
        "GroupDetectors",
        {"InputWorkspace": empty_ws, "OutputWorkspace": empty_mon_ws_name, "DetectorList": det_list, "KeepUngroupedSpectra": True},
    )
    empty_mon_ws = alg.getProperty("OutputWorkspace").value

    alg = _execute(
        "GroupDetectors",
        {"InputWorkspace": sample_ws, "OutputWorkspace": sample_mon_ws_name, "DetectorList": det_list, "KeepUngroupedSpectra": True},
    )
    ...
    alg = _execute(
        "RebinToWorkspace", {"WorkspaceToRebin": empty_mon_ws, "WorkspaceToMatch": sample_mon_ws, "OutputWorkspace": empty_mon_ws_name}
    )
    empty_mon_ws = alg.getProperty("OutputWorkspace").value
...
def apply_transmission(self, workspace, trans_workspace):
    ...
    alg = _execute(
        "RebinToWorkspace",
        {"WorkspaceToRebin": trans_workspace, "WorkspaceToMatch": workspace, "OutputWorkspace": "__trans_rebin", "PreserveEvents": False},
    )
...
def subtract_dark_current(self, workspace, property_manager):
    ...
    def _dark(ws, dark_current_property, dark_current_file=None):
        if property_manager.existsProperty(dark_current_property):
            ...
            alg_props = {"InputWorkspace": ws, "PersistentCorrection": False, "ReductionProperties": property_manager_name}
