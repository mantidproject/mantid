...
class SANSTubeCalibration(PythonAlgorithm):
    ...
    def calibrate(self, ws, tubeSet, knownPositions, funcForm, **kwargs):  # noqa: C901
        ...
        **Define the new position for the detectors**
        ...
        for key in list(kwargs.keys()):
            if key not in [FITPAR, MARGIN, RANGELIST, CALIBTABLE, PLOTTUBE, EXCLUDESHORT, OVERRIDEPEAKS, FITPOLIN, OUTPUTPEAK, OUTPUTC]:
                msg = (
                    "Wrong argument: '%s'! This argument is not defined in the signature of this function. "
        ...
        if not (isinstance(knownPositions, list) or isinstance(knownPositions, tuple) or isinstance(knownPositions, np.ndarray)):
            raise RuntimeError(
        ...
        try:
            ...
            raise RuntimeError(
                (
                    ...
                    "The knownPos has %d elements and the given funcForm has %d." % (nPeaks, len(funcForm))
                )
            )
        ...
        if OVERRIDEPEAKS in kwargs:
            ...
            try:
                ...
                raise RuntimeError(
                    ...
                    "Ex (3 peaks): overridePeaks = {1:[2,5.4,500]}" % OVERRIDEPEAKS
                )
        else:
            overridePeaks = dict()
        ...
        if FITPOLIN in kwargs:
            ...
            if polinFit not in [1, 2, 3]:
                raise RuntimeError(
                    ...
                    "2 for quadratic, or 3 for 3rd polinomial order when fitting the pixels positions agains the known positions" % FITPOLIN
                )
        else:
            polinFit = 2
        ...
        if isinstance(outputPeak, ITableWorkspace):
            if outputPeak.columnCount() < len(idealTube.getArray()):
                raise RuntimeError(
                    ...
                    "for M = number of peaks given in knownPositions" % OUTPUTPEAK
                )
        else:
            if not outputPeak:
        ...
        # RKH added meanCTable
        self.getCalibration_RKH(
            ws, tubeSet, calibTable, fitPar, idealTube, outputPeak, outputC, overridePeaks, excludeShortTubes, plotTube, rangeList, polinFit
        )
