...
class SANSMask(PythonAlgorithm):
    ...
    def PyInit(self):
        facilities = ["SNS", "HFIR"]
        self.declareProperty("Facility", "SNS", StringListValidator(facilities),
                             "Facility to which the SANS instrument belongs")

        self.declareProperty(MatrixWorkspaceProperty("Workspace", "", direction=Direction.InOut),
                             "Workspace to apply the mask to")

        self.declareProperty(IntArrayProperty("MaskedDetectorList", values=[], direction=Direction.Input),
                             "List of detector IDs to be masked")

        self.declareProperty(IntArrayProperty("MaskedEdges", values=[0, 0, 0, 0], direction=Direction.Input),
        ...
        self.declareProperty("MaskedSide", "None", StringListValidator(sides),
    ...
    def PyExec(self):
        ...
        if len(edges) == 4:
            if facility.upper() == "HFIR":
                masked_ids = hfir_instrument.get_masked_ids(edges[0], edges[1], edges[2], edges[3], workspace,
                                                            component_name)
        ...
        if facility.upper() == "HFIR":
            ...
            if component.type() == 'RectangularDetector':
                ...
                ids_at_the_bottom = list(
                    range(side_to_mask * component.idstep() + component.idstart(),
            ...
            elif component.type() == 'CompAssembly' or component.type() == 'ObjCompAssembly' or component.type(
            ) == 'DetectorComponent':
    ...
    def _mask_component(self, workspace, component_name):
        ...
        if component.type() == 'RectangularDetector':
            ...
            Logger("SANSMask").error("Mask not applied. Component not valid: %s of type %s." %
