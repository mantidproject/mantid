...
from mantid.api import (AlgorithmFactory, DataProcessorAlgorithm, FileAction, FileProperty, MultipleFileProperty, Progress, WorkspaceGroup,
                        WorkspaceGroupProperty)
from mantid.kernel import (Direction, EnabledWhenProperty, FloatArrayBoundedValidator, FloatArrayProperty,
...
class PropertyNames(object):
    ...
    PROPERTIES_TO_SIZE_MATCH = [
        ...
        END_WS_INDEX_DIRECT, WAVELENGTH_LOWER, WAVELENGTH_UPPER
    ]
...
class ReflectometryILLAutoProcess(DataProcessorAlgorithm):
    ...
    def seeAlso(self):
        ...
        return [
            'ReflectometryILLConvertToQ', 'ReflectometryILLPolarizationCor', 'ReflectometryILLPreprocess', 'ReflectometryILLSumForeground'
        ]
    ...
    def PyInit(self):
        ...
        non_negative_float_array.setLower(0.)
        string_array_validator = StringArrayLengthValidator()
        string_array_validator.setLengthMin(1)

        # ======================== Main Properties ========================
        self.declareProperty(PropertyNames.POLARIZATION_OPTION, 'NonPolarized', StringListValidator(['NonPolarized', 'Polarized']),
        ...
        self.declareProperty(MultipleFileProperty(PropertyNames.RB00, action=FileAction.OptionalLoad, extensions=['nxs']),
        ...
        self.declareProperty(MultipleFileProperty(PropertyNames.RB01, action=FileAction.OptionalLoad, extensions=['nxs']),
        ...
        self.declareProperty(MultipleFileProperty(PropertyNames.RB10, action=FileAction.OptionalLoad, extensions=['nxs']),
        ...
        self.declareProperty(MultipleFileProperty(PropertyNames.RB11, action=FileAction.OptionalLoad, extensions=['nxs']),
        ...
        self.declareProperty(FileProperty(PropertyNames.EFFICIENCY_FILE, defaultValue='', action=FileAction.OptionalLoad),
        ...
        self.declareProperty(FloatArrayProperty(PropertyNames.MANUAL_SCALE_FACTORS, values=[]),
        ...
        self.declareProperty(StringArrayProperty(PropertyNames.ANGLE_OPTION,
                                                 values=[PropertyNames.DAN],
                                                 validator=string_array_validator,
                                                 direction=Direction.Input),
                             doc='Angle option used for detector positioning')
        self.declareProperty(FloatArrayProperty(PropertyNames.THETA, values=[-1.]), doc='A user-defined angle theta in degree')
        self.declareProperty(StringArrayProperty(PropertyNames.SUM_TYPE,
                                                 values=[PropertyNames.INCOHERENT],
                                                 validator=string_array_validator,
                                                 direction=Direction.Input),
                             doc='Type of summation to perform')
        self.declareProperty(FloatArrayProperty(PropertyNames.WAVELENGTH_LOWER, values=[2.], validator=non_negative_float_array),
                             doc='The lower wavelength bound (Angstrom)')
        self.declareProperty(FloatArrayProperty(PropertyNames.WAVELENGTH_UPPER, values=[30.], validator=non_negative_float_array),
                             doc='The upper wavelength bound (Angstrom)')
        self.declareProperty(FloatArrayProperty(PropertyNames.GROUPING_FRACTION, values=[0.5], validator=non_negative_float_array),
        ...
        self.declareProperty(IntArrayProperty(PropertyNames.LOW_FRG_HALF_WIDTH_DIRECT, values=[2], validator=non_negative_ints),
        ...
        self.declareProperty(IntArrayProperty(PropertyNames.LOW_BKG_OFFSET_DIRECT, values=[5], validator=non_negative_ints),
        ...
        self.declareProperty(IntArrayProperty(PropertyNames.LOW_BKG_WIDTH_DIRECT, values=[5], validator=non_negative_ints),
        ...
        self.declareProperty(IntArrayProperty(PropertyNames.HIGH_FRG_HALF_WIDTH_DIRECT, values=[2], validator=non_negative_ints),
        ...
        self.declareProperty(IntArrayProperty(PropertyNames.HIGH_BKG_OFFSET_DIRECT, values=[5], validator=non_negative_ints),
        ...
        self.declareProperty(IntArrayProperty(PropertyNames.HIGH_BKG_WIDTH_DIRECT, values=[5], validator=non_negative_ints),
        ...
        self.declareProperty(IntArrayProperty(PropertyNames.START_WS_INDEX_DIRECT, values=[0], validator=non_negative_ints),
        ...
        self.declareProperty(IntArrayProperty(PropertyNames.END_WS_INDEX_DIRECT, values=[255], validator=non_negative_ints),
                             doc='Last histogram index used for peak fitting')
        self.setPropertyGroup(PropertyNames.END_WS_INDEX_DIRECT, pre_process_direct)
        self.declareProperty(PropertyNames.XMIN_DIRECT, defaultValue=-1., doc='Minimum x value (unit wavelength) used for peak fitting.')
        self.setPropertyGroup(PropertyNames.XMIN_DIRECT, pre_process_direct)
        self.declareProperty(PropertyNames.XMAX_DIRECT, defaultValue=-1., doc='Maximum x value (unit wavelength) used for peak fitting.')
        ...
        self.declareProperty(IntArrayProperty(PropertyNames.LOW_FRG_HALF_WIDTH, values=[2], validator=non_negative_ints),
                             doc='Number of foreground pixels at lower angles from the centre pixel.')
        self.setPropertyGroup(PropertyNames.LOW_FRG_HALF_WIDTH, pre_process_reflected)
        self.declareProperty(IntArrayProperty(PropertyNames.LOW_BKG_OFFSET, values=[5], validator=non_negative_ints),
                             doc='Distance of flat background region towards smaller detector angles from the '
                             'foreground centre, in pixels.')
        self.setPropertyGroup(PropertyNames.LOW_BKG_OFFSET, pre_process_reflected)
        self.declareProperty(IntArrayProperty(PropertyNames.LOW_BKG_WIDTH, values=[5], validator=non_negative_ints),
                             doc='Width of flat background region towards smaller detector angles from the '
                             'foreground centre, in pixels')
        self.setPropertyGroup(PropertyNames.LOW_BKG_WIDTH, pre_process_reflected)
        self.declareProperty(IntArrayProperty(PropertyNames.HIGH_FRG_HALF_WIDTH, values=[2], validator=non_negative_ints),
                             doc='Number of foreground pixels at higher angles from the centre pixel.')
        self.setPropertyGroup(PropertyNames.HIGH_FRG_HALF_WIDTH, pre_process_reflected)
        self.declareProperty(IntArrayProperty(PropertyNames.HIGH_BKG_OFFSET, values=[5], validator=non_negative_ints),
                             doc='Distance of flat background region towards larger detector angles from the '
                             'foreground centre, in pixels')
        self.setPropertyGroup(PropertyNames.HIGH_BKG_OFFSET, pre_process_reflected)
        self.declareProperty(IntArrayProperty(PropertyNames.HIGH_BKG_WIDTH, values=[5], validator=non_negative_ints),
                             doc='Width of flat background region towards larger detector angles from the '
                             'foreground centre, in pixels.')
        self.setPropertyGroup(PropertyNames.HIGH_BKG_WIDTH, pre_process_reflected)
        self.declareProperty(IntArrayProperty(PropertyNames.START_WS_INDEX, values=[0], validator=non_negative_ints),
                             doc='Start histogram index used for peak fitting')
        self.setPropertyGroup(PropertyNames.START_WS_INDEX, pre_process_reflected)
        self.declareProperty(IntArrayProperty(PropertyNames.END_WS_INDEX, values=[255], validator=non_negative_ints),
                             doc='Last histogram index used for peak fitting')
    ...
    def convert_to_momentum_transfer(self, input_ws_name: str, output_ws_name: str, direct_foreground_name: str,
                                     angle_index: int, theta_correction='') -> None:
    ...
    def crop_theta_correction(self, reflected_beam_name: str, theta_ws_name: str, angle_index: int) -> str:
        ...
        CropWorkspace(InputWorkspace=theta_ws_name_cropped,
    ...
    def preprocess_direct_beam(self, run: str, out_ws: str, angle_index: int) -> None:
        ...
        """
        ReflectometryILLPreprocess(Run=run,
    ...
    def sum_foreground(self, input_ws_name: str, output_ws_name: str, sum_type: str, angle_index: int,
                       direct_foreground_name='') -> Tuple[str, str]:
