...
from mantid.api import AlgorithmFactory, AnalysisDataService, DataProcessorAlgorithm, MatrixWorkspace, NumericAxis, Progress
from mantid.simpleapi import CreateWorkspace, DeleteWorkspace, GroupWorkspaces, mtd, IndirectTwoPeakFit, SaveNexusProcessed
...
class IndirectQuickRun(DataProcessorAlgorithm):
    ...
    def PyInit(self):
        ...
        self.declareProperty(
            FloatArrayProperty(name="ElasticRange", validator=FloatArrayLengthValidator(2)), doc="Energy range for the elastic component."
        )
        self.declareProperty(
            FloatArrayProperty(name="InelasticRange", validator=FloatArrayLengthValidator(2)),
            doc="Energy range for the inelastic component.",
        )
        self.declareProperty(
            FloatArrayProperty(name="TotalRange", validator=FloatArrayLengthValidator(2)),
    ...
    def _group_energy_window_scan_output(self):
        ...
        energy_window_scan_workspaces = [self._scan_ws + suffix for suffix in suffixes if exists_in_ads(self._scan_ws + suffix)]
    ...
    def _plot_output(self):
        workspace_names = [
            self._scan_ws + "_el_eq1",
            self._scan_ws + "_inel_eq1",
            self._scan_ws + "_total_eq1",
            self._scan_ws + "_el_eq2",
            self._scan_ws + "_inel_eq2",
            self._scan_ws + "_total_eq2",
            self._scan_ws + "_eisf",
        ]
        try:
