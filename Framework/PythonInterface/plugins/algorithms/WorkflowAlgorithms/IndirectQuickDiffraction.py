...
class IndirectQuickDiffraction(DataProcessorAlgorithm):
    ...
    def summary(self):
        return ('Runs an energy transfer reduction for an inelastic indirect geometry instrument, '
                'then runs through a Diffraction Reduction')
    ...
    def PyExec(self):
        ...
        process_prog = Progress(self, start=0.1, end=0.9, nreports=2)
        process_prog.report('Running diffraction reduction')
        scan_alg = self.createChildAlgorithm("ISISIndirectDiffractionReduction", 0.05, 0.95)
        scan_alg.setProperty('InputFiles', self._data_files)
        scan_alg.setProperty('CalFile', self._diff_cal)
