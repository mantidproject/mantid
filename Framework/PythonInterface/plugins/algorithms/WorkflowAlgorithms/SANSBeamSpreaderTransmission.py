...
class SANSBeamSpreaderTransmission(PythonAlgorithm):
    ...
    def PyInit(self):
        ...
        self.declareProperty(FileProperty("SampleSpreaderFilename", "", action=FileAction.Load, extensions=["xml", "nxs", "nxs.h5"]))
        self.declareProperty(FileProperty("DirectSpreaderFilename", "", action=FileAction.Load, extensions=["xml", "nxs", "nxs.h5"]))
        self.declareProperty(FileProperty("SampleScatteringFilename", "", action=FileAction.Load, extensions=["xml", "nxs", "nxs.h5"]))
        self.declareProperty(FileProperty("DirectScatteringFilename", "", action=FileAction.Load, extensions=["xml", "nxs", "nxs.h5"]))
    ...
    def PyExec(self):  # noqa: C901
        ...
        if trans_ws is None:
            ...
            ws_names = [
                [sample_spread, sample_spreader_ws],
    ...
    def _apply_transmission(self, workspace, trans_workspace):
        ...
        api.RebinToWorkspace(
            WorkspaceToRebin=trans_workspace, WorkspaceToMatch=workspace, OutputWorkspace=trans_workspace + "_rebin", PreserveEvents=False
        )
        # Apply angle-dependent transmission correction using the zero-angle transmission
        theta_dependent = self.getProperty("ThetaDependent").value

        api.ApplyTransmissionCorrection(
