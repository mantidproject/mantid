...
class DirectILLIntegrateVanadium(DataProcessorAlgorithm):
    ...
    def PyInit(self):
        ...
        self.declareProperty(
            WorkspaceProperty(name=common.PROP_OUTPUT_WS, defaultValue="", direction=Direction.Output), doc="The integrated workspace."
        )
        self.declareProperty(
        ...
        self.declareProperty(
            ITableWorkspaceProperty(name=common.PROP_EPP_WS, defaultValue="", direction=Direction.Input, optional=PropertyMode.Mandatory),
        ...
        self.declareProperty(
            ...
            direction=Direction.Input,
            doc="Vanadium temperature in Kelvin for Debye-Waller correction, " + "overrides the default value from the sample logs.",
        )
        self.setPropertySettings(common.PROP_TEMPERATURE, EnabledWhenProperty(common.PROP_DWF_CORRECTION, PropertyCriterion.IsDefault))
    ...
    def _maskZeros(self, mainWS):
        ...
        mainWS = MaskDetectorsIf(
            InputWorkspace=mainWS, OutputWorkspace=mainWS, Mode="SelectIf", Operator="Equal", Value=0.0, EnableLogging=self._subalgLogging
        )
        return mainWS
