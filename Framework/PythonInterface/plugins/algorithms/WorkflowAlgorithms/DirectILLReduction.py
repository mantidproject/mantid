...
def _paramGroupsToEdges(groups):
    ...
    for params in groups:
        if not params:
            ...
            while params:
                if len(params) < 2:
                    raise RuntimeError("Error in " + common.PROP_REBINNING_W + ": not enough numbers to form the binning.")
                dx = params.pop(0)
                endX = params.pop(0)
                x = beginX
                index = 1
...
def _minMaxQ(ws):
    ...
    maxEf = Ei - minW
    # In Ånströms
    maxQ = numpy.sqrt(2.0 * constants.m_n / constants.hbar**2 * (Ei + maxEf - 2 * numpy.sqrt(Ei * maxEf) * -1.0)) * 1e-10
    minQ = 0.0
    return (minQ, maxQ)
...
class DirectILLReduction(DataProcessorAlgorithm):
    ...
    def PyInit(self):
        ...
        positiveFloat = FloatBoundedValidator(0.0, exclusive=True)
        ...
        self.declareProperty(
            MatrixWorkspaceProperty(
                name=common.PROP_DIAGNOSTICS_WS, defaultValue="", direction=Direction.Input, optional=PropertyMode.Optional
            ),
        ...
        self.declareProperty(
            FloatArrayProperty(name=common.PROP_REBINNING_PARAMS_W, validator=validRebinParams), doc="Manual energy rebinning parameters."
        )
        self.setPropertyGroup(common.PROP_REBINNING_PARAMS_W, PROPGROUP_REBINNING)
        self.declareProperty(
            name=common.PROP_REBINNING_W, defaultValue="", doc="Energy rebinning when mixing manual and automatic binning parameters."
        )
        self.declareProperty(
            FloatArrayProperty(name=common.PROP_BINNING_PARAMS_Q, validator=validRebinParams), doc="Manual q rebinning parameters."
        )
        ...
        self.declareProperty(
            WorkspaceProperty(
                name=common.PROP_OUTPUT_THETA_W_WS, defaultValue="", direction=Direction.Output, optional=PropertyMode.Optional
            ),
    ...
    def _convertTOFToDeltaE(self, mainWS):
        ...
        energyConvertedWS = ConvertUnits(
            InputWorkspace=mainWS, OutputWorkspace=energyConvertedWSName, Target="DeltaE", EMode="Direct", EnableLogging=self._subalgLogging
        )
    ...
    def _correctByDetectorEfficiency(self, mainWS):
        ...
        correctedWS = DetectorEfficiencyCorUser(InputWorkspace=mainWS, OutputWorkspace=correctedWSName, EnableLogging=self._subalgLogging)
        self._cleanup.cleanup(mainWS)
        return correctedWS
    ...
    def _correctByKiKf(self, mainWS):
        ...
        correctedWS = CorrectKiKf(InputWorkspace=mainWS, OutputWorkspace=correctedWSName, EnableLogging=self._subalgLogging)
        self._cleanup.cleanup(mainWS)
        return correctedWS
    ...
    def _groupDetectors(self, mainWS):
        ...
        GenerateGroupingPowder(
            InputWorkspace=mainWS, AngleStep=angleStep, GroupingFilename=path, GenerateParFile=False, EnableLogging=self._subalgLogging
        )
        try:
            groupedWSName = self._names.withSuffix("grouped_detectors")
            groupedWS = GroupDetectors(
    ...
    def _normalizeToVana(self, mainWS):
        ...
        vanaNormalizedWS = Divide(
            LHSWorkspace=mainWS, RHSWorkspace=vanaWS, OutputWorkspace=vanaNormalizedWSName, EnableLogging=self._subalgLogging
        )
        self._cleanup.cleanup(mainWS)
    ...
    def _outputWSConvertedToTheta(self, mainWS):
        ...
        if not self.getProperty(common.PROP_OUTPUT_THETA_W_WS).isDefault:
            ...
            thetaWS = ConvertSpectrumAxis(
                InputWorkspace=mainWS, OutputWorkspace=thetaWSName, Target="Theta", EMode="Direct", EnableLogging=self._subalgLogging
            )
    ...
    def _sOfQW(self, mainWS):
        ...
        sOfQWWS = SofQWNormalisedPolygon(
            InputWorkspace=mainWS,
    ...
    def _transpose(self, mainWS):
        ...
        transposedWS = Transpose(InputWorkspace=mainWS, OutputWorkspace=transposedWSName, EnableLogging=self._subalgLogging)
        self._cleanup.cleanup(mainWS)
        return transposedWS
