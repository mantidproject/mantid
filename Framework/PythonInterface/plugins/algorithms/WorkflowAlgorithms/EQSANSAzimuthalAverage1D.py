...
class EQSANSAzimuthalAverage1D(PythonAlgorithm):
    ...
    def PyInit(self):
        ...
        self.declareProperty("IndependentBinning", True, doc="If true and frame skipping is used, each frame will have its own binning")
        self.declareProperty("ScaleResults", True, doc="If true and frame skipping is used, frame 1 will be scaled to frame 2")
        ...
        self.declareProperty(MatrixWorkspaceProperty("OutputWorkspace", "", direction=Direction.Output), doc="I(q) workspace")
    ...
    def _process_frame(self, workspace, wl_min, wl_max, source_aperture_radius, frame_ID="1", binning=None):
        ...
        (dummy_output_msg, output_iq, output_binning) = self._call_sans_averaging(
            output_ws, binning, nbins, log_binning, property_manager_name, ws_frame
        )
        if compute_resolution:
            sample_aperture_radius = self.getProperty("SampleApertureDiameter").value / 2.0
            alg = AlgorithmManager.create("EQSANSResolution")
