...
class PDToGUDRUN(DataProcessorAlgorithm):
    ...
    def PyInit(self):
        ...
        self.declareProperty(MatrixWorkspaceProperty("OutputWorkspace", "",
                             ...
                             doc="Handle to reduced workspace")
        self.declareProperty(FileProperty(name="GUDRUNFile", defaultValue="", action=FileAction.Save,
                                          extensions=[".getn"]), "Output filename")
        self.setPropertyGroup("OutputWorkspace", group)
        self.setPropertyGroup("GUDRUNFile", group)

        self.declareProperty(FileProperty(name="GroupingFile",
                                          defaultValue="", action=FileAction.OptionalLoad,
                                          extensions=[".xml"]),
    ...
    def _loadCharacterizations(self):
        ...
        results = PDLoadCharacterizations(Filename=charFilename,
                                          ...
                                          startProgress=0.0, endProgress=0.1)
    ...
    def PyExec(self):
        ...
        if len(groupingFile) > 0:
            ...
            LoadDetectorsGroupingFile(InputFile=groupingFile,
                                      OutputWorkspace=instrumentName + '_group')
        ...
        wksp = AlignAndFocusPowder(InputWorkspace=wksp, OutputWorkspace=wksp,
                                   ...
                                   ReductionProperties="__snspowderreduction",
                                   startProgress=0.3, endProgress=0.8)
        wksp = NormaliseByCurrent(InputWorkspace=wksp, OutputWorkspace=wksp,
                                  startProgress=0.8, endProgress=0.9)
        ...
        if len(setUncertainties) > 0:
            wksp = SetUncertainties(InputWorkspace=wksp, OutputWorkspace=wksp,
                                    SetError=setUncertainties)

        SaveNexusPD(InputWorkspace=wksp,
