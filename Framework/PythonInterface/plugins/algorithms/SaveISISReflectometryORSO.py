...
class SaveISISReflectometryORSO(PythonAlgorithm):
    ...
    _MANUAL_METADATA_NOT_SET = "__manual_metadata_not_set__"
    _FACILITY = "ISIS"
    ...
    def PyInit(self):
        ...
        self.declareProperty(
            name=Prop.REDUCTION_TIMESTAMP,
            defaultValue=self._MANUAL_METADATA_NOT_SET,
            doc="When the reduction took place (ISO8601 Standard).",
        )
        ...
        self.declareProperty(Prop.CALIB_FILE, defaultValue=self._MANUAL_METADATA_NOT_SET, doc="Calibration file used.")
    ...
    def _create_single_refl_dataset(self, ws, is_child, metadata_dict=None) -> ReflectometryDatasetBase:
        ...
        def get_manual_property_value(alg_prop: str, default_value="") -> str:
            return default_value if self.getProperty(alg_prop).isDefault else self.getPropertyValue(alg_prop)
        ...
        set_simple_dataset_value_from_property(
            "reduction_timestamp", get_manual_property_value(Prop.REDUCTION_TIMESTAMP, default_value=None), Prop.REDUCTION_TIMESTAMP
        )
        set_simple_dataset_value_from_property("calibration_entry", get_manual_property_value(Prop.CALIB_FILE), Prop.CALIB_FILE)
