...
class IkedaCarpenterConvoluted(IFunction1D):
    ...
    def setPenalizedConstraints(self,
                                A0=None,
                                B0=None,
                                R0=None,
                                T00=None,
    ...
    def function1D(self, t):
        ...
        f_int = Scale * ((1 - R) * np.power((A * (t - T0)), 2) * np.exp(-A * (t - T0)) + 2 * R * A**2 * B / np.power(
            (A - B), 3) * (np.exp(-B * (t - T0)) - np.exp(-A * (t - T0)) * (1 + (A - B) * (t - T0) + 0.5 * np.power(
                (A - B), 2) * np.power((t - T0), 2))))
        f_int[t < T0] = 0
