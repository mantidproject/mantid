...
class SCgapDwave(IFunction1D):

    def category(self):
        return "Muon"

    def init(self):
        self.declareParameter("Delta", 2.0, 'Gap Value')
    ...
    def function1D(self, x):
        ...
        def Integrand(E, phi):
                DeltaDwave = Delta * np.cos(np.pi * phi)
                a = 1.018
                c = 1.82
                return  1 / np.cosh(np.sqrt((Ec * E) ** 2 + (DeltaDwave * np.tanh (c * (a * (Tc / xx - 1)) ** 0.51)) ** 2) / (2 * kb * xx)) ** 2
...
FunctionFactory.subscribe(SCgapDwave)
