...
public:
  ...
  void test_size() {
    ...
    */
    ...
    auto bankSortedComponentIndices = boost::make_shared<const std::vector<size_t>>(std::vector<size_t>{0,1,2});
    auto detectorRanges = boost::make_shared<const std::vector<std::pair<size_t, size_t>>>();
    auto componentRanges = boost::make_shared<const std::vector<std::pair<size_t, size_t>>>(std::vector<std::pair<size_t, size_t>>{});
    auto positions = boost::make_shared<std::vector<Eigen::Vector3d>>();
    auto rotations = boost::make_shared<std::vector<Eigen::Quaterniond>>();

    ComponentInfo info(
        bankSortedDetectorIndices, detectorRanges, bankSortedComponentIndices, componentRanges, positions, rotations,
  ...
  void test_throw_if_positions_rotation_inputs_different_sizes() {
    ...
    auto bankSortedComponentIndices = boost::make_shared<const std::vector<size_t>>(std::vector<size_t>{0});
    auto innerDetectorRanges = std::vector<std::pair<size_t, size_t>>{{0, 0}};// One component with no detectors
    auto detectorRanges = boost::make_shared<const std::vector<std::pair<size_t, size_t>>>(std::move(innerDetectorRanges));

    auto innerComponentRanges = std::vector<std::pair<size_t, size_t>>{{0, 0}};// One component with no sub-components
    auto componentRanges = boost::make_shared<const std::vector<std::pair<size_t, size_t>>>(std::move(innerComponentRanges));
    auto positions = boost::make_shared<std::vector<Eigen::Vector3d>>(
        1); // 1 position provided
    auto rotations = boost::make_shared<std::vector<Eigen::Quaterniond>>(
        0); // 0 rotations provided

    TS_ASSERT_THROWS(ComponentInfo(detectorIndices, detectorRanges, bankSortedComponentIndices, componentRanges, positions,
                                   rotations,
  ...
  void test_throw_if_positions_and_rotations_not_same_size_as_detectorRanges() {
     ...
     * components
     * We should have as many detectorRanges as we have non-detector components too.
     * All vectors should be the same size.
     */
    auto detectorIndices = boost::make_shared<const std::vector<size_t>>();//No detector indices in this example!

    auto componentIndices = boost::make_shared<const std::vector<size_t>>(std::vector<size_t>{0});

    auto detectorRanges = boost::make_shared<const std::vector<std::pair<size_t, size_t>>>(); // Empty detectorRanges
    ...
    auto componentRanges = boost::make_shared<const std::vector<std::pair<size_t, size_t>>>(std::vector<std::pair<size_t, size_t>>{{0,0}});


    TS_ASSERT_THROWS(ComponentInfo(detectorIndices, detectorRanges, componentIndices, componentRanges, positions,
                                   rotations,
                                   boost::make_shared<DetectorInfo>()),
                     std::invalid_argument &);
  }
  ...
  void test_read_positions_rotations() {
      ...
      */
      ...
      auto bankSortedDetectorIndices = boost::make_shared<const std::vector<size_t>>(std::vector<size_t>{0, 2, 1});
      auto bankSortedComponentIndices = boost::make_shared<const std::vector<size_t>>(std::vector<size_t>{0, 1, 3, 2, 4});

      std::vector<std::pair<size_t, size_t>> detectorRanges;
      detectorRanges.push_back(std::make_pair(0, 2)); // sub-assembly (registered first)
      detectorRanges.push_back(std::make_pair(0, 3)); // instrument-assembly (with 3 detectors)

      std::vector<std::pair<size_t, size_t>> componentRanges;
      componentRanges.push_back(std::make_pair(0, 2)); // sub-assembly (contains two detectors)
      componentRanges.push_back(std::make_pair(0, 4)); // instrument assembly (with 4 sub-components)
      ...
      ComponentInfo info(bankSortedDetectorIndices, boost::make_shared<const std::vector<std::pair<size_t, size_t>>>(detectorRanges), bankSortedComponentIndices, boost::make_shared<const std::vector<std::pair<size_t, size_t>>>(componentRanges), compPositions,
  ...
  void test_detector_indexes() {
      ...
      */
      ...
      auto bankSortedDetectorIndices = boost::make_shared<const std::vector<size_t>>(std::vector<size_t>{0, 2, 1});
      auto bankSortedComponentIndices = boost::make_shared<const std::vector<size_t>>(std::vector<size_t>{0, 2, 3, 1, 4});
      std::vector<std::pair<size_t, size_t>> detectorRanges;
      ...
      ComponentInfo info(bankSortedDetectorIndices, boost::make_shared<const std::vector<std::pair<size_t, size_t>>>(detectorRanges), bankSortedComponentIndices, boost::make_shared<const std::vector<std::pair<size_t, size_t>>>(componentRanges), positions,
                         rotations,  boost::make_shared<DetectorInfo>(detPositions, detRotations));
