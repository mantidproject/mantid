 ...
 */
void importDataFromColumnFile(const string &filename, const string &datawsname) {
  // 1. Call LoadAscii
...
API::MatrixWorkspace_sptr createInputDataWorkspace(int option) {
  ...
  default:
    // not supported
    std::cout << "LeBailFitTest.createInputDataWorkspace() Option " << option << " is not supported. \n";
    throw std::invalid_argument("Unsupported option. ");
  }

  // 2. Get workspace
  MatrixWorkspace_sptr dataws =
      std::dynamic_pointer_cast<MatrixWorkspace>(AnalysisDataService::Instance().retrieve(datawsname));
  if (!dataws) {
...
DataObjects::TableWorkspace_sptr createReflectionWorkspace(int option) {
  ...
  std::cout << "Created Table Workspace with " << hklws->rowCount() << " entries of peaks.\n";
...
DataObjects::TableWorkspace_sptr createInstrumentParameterWorkspace(int option) {
  ...
  std::vector<std::string> paramnames{"Zero", "Zerot", "Dtt1", "Dtt1t", "Dtt2t", "Tcross", "Width", "LatticeConstant"};
  ...
  default:
    stringstream errss;
    errss << "Option " << option << " is not supported by createInstrumentParameterWorkspace.\n";
...
public:
  ...
  void test_RobustFitPG3Bank1() {
    ...
    DataObjects::Workspace2D_sptr peakdataws =
        std::dynamic_pointer_cast<DataObjects::Workspace2D>(AnalysisDataService::Instance().retrieve("FittedPeaks"));
    ...
    DataObjects::TableWorkspace_sptr outbraggws =
        std::dynamic_pointer_cast<TableWorkspace>(AnalysisDataService::Instance().retrieve("PeaksParameterTable"));
...
public:
  // This pair of boilerplate methods prevent the suite being created statically
  // This means the constructor isn't called when running other tests
  static FitPowderDiffPeaksTestPerformance *createSuite() { return new FitPowderDiffPeaksTestPerformance(); }
  static void destroySuite(FitPowderDiffPeaksTestPerformance *suite) { delete suite; }
