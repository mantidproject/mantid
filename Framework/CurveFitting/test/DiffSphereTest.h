...
public:
  ...
  void testDiffSphereElastic() {
    ...
    // saveWorkspace( data_workspace, "/tmp/junk.nxs" ); // for debugging
    // purposes only

    /* override the function with new parameters, then do the Fit. The effect of
     * ElasticDiffSphere
     * is to multiply the Gaussian by height*[3*j_1(Q*Radius)/(Q*Radius)]^2.
     * Thus, an increase in
    ...
    */
    ...
    TS_ASSERT_DELTA(fitalg_resolution->getParameter("Height"), 1.0,
                    1.0 * 0.001); // allow for a small percent variation
    TS_ASSERT_DELTA(fitalg_resolution->getParameter("Sigma"), 0.002,
                    0.002 * 0.001); // allow for a small percent variation
    // std::cout << "\nPeakCentre = " <<
    // fitalg_resolution->getParameter("PeakCentre") << "  Height= " <<
    // fitalg_resolution->getParameter("Height") << "  Sigma=" <<
    // fitalg_resolution->getParameter("Sigma") << "\n"; // only for debugging
    // purposes
    ...
    TS_ASSERT_DELTA(fitalg_structure_factor->getParameter("Radius"), 3.567,
                    3.567 * 0.05); // allow for a small percent variation
    // std::cout << "\nGOAL: Height = 47.014,  Radius = 1.567\n"; // only for
    // debugging purposes
    // std::cout << "OPTIMIZED: Height = " <<
    // fitalg_structure_factor->getParameter("Height") << "  Radius = " <<
    // fitalg_structure_factor->getParameter("Radius")<< "\n"; // only for
    // debugging purposes
  }
  ...
  void testNormalization() {
    ...
    double R(0.1); // We vary parameter R while keeping the other constant, same
                   // as varying Q*Radius
    const double dR(0.1);

    const double QR_max(20); // suggested value by Volino for the approximation
  ...
  void testDiffSphere() {
    ...
    // algorithm
    auto data_workspace = generateWorkspaceFromFitAlgorithm(fitalg);
    // saveWorkspace( data_workspace, "/tmp/junk_data.nxs" ); // for debugging
    // purposes only
    ...
    // ="<<boost::lexical_cast<std::string>(R)<<",  Diffusion =
    // "<<boost::lexical_cast<std::string>(D)<<"\n"; // only for debugging
    // purposes
    // std::cout << "GOAL: Intensity =
    // "<<boost::lexical_cast<std::string>(I_0)<<",  Radius =
    // "<<boost::lexical_cast<std::string>(R_0)<<",  Diffusion =
    // "<<boost::lexical_cast<std::string>(D_0)<<"\n"; // only for debugging
    // purposes
    // std::cout << "OPTIMIZED: Intensity = " <<
    // fitalg_structure_factor->getParameter("Intensity") << "  Radius = " <<
    // fitalg_structure_factor->getParameter("Radius") << "  Diffusion = " <<
    // fitalg_structure_factor->getParameter("Diffusion") << "\n"; // only for
    // debugging purposes
  }
...
private:
  void runDiffSphereInelasticTest(const double S,
    ...
    // algorithm
    auto data_workspace = generateWorkspaceFromFitAlgorithm(fitalg);
    // saveWorkspace( data_workspace, "/home/dan/junk_data.nxs" ); // for
    // debugging purposes only
    ...
    funtion_stream << ",Intensity=" << boost::lexical_cast<std::string>(I)
                   ...
                   << ",Shift=" << boost::lexical_cast<std::string>(S) << ")";
    fitalg.setProperty("Function", funtion_stream.str());
    // auto before_workspace = generateWorkspaceFromFitAlgorithm( fitalg ); //
    // for debugging purposes only
    // saveWorkspace( before_workspace, "/tmp/junk_before_fitting.nxs" ); // for
    // debugging purposes only
    ...
    TS_ASSERT_THROWS_NOTHING(TS_ASSERT(fitalg.execute()));
    TS_ASSERT(fitalg.isExecuted());
    // auto after_workspace = generateWorkspaceFromFitAlgorithm( fitalg ); //
    // for debugging purposes only
    // saveWorkspace( after_workspace, "/tmp/junk_after_fitting.nxs" ); // for
    // debugging purposes only
    ...
    TS_ASSERT_DELTA(fitalg_structure_factor->getParameter("Diffusion"), D_0,
                    D_0 * 0.05); // allow for a small percent variation
    TS_ASSERT_DELTA(fitalg_structure_factor->getParameter("Shift"), S,
                    0.0005); // allow for a small percent variation
    // std::cout << "\nINITIAL GUESS: Intensity =
    // "<<boost::lexical_cast<std::string>(I)<<",  Radius
    // ="<<boost::lexical_cast<std::string>(R)<<",  Diffusion =
    // "<<boost::lexical_cast<std::string>(D)<<"\n"; // only for debugging
