...
#include <memory>
using Mantid::CurveFitting::Functions::FickDiffusionSQE;

class FickDiffusionSQETest : public CxxTest::TestSuite {
public:
  void test_categories() {
    FickDiffusionSQE func;
  ...
  void test_parameters() {
    auto func = createTestFickDiffusionSQE();
  ...
  void test_function_gives_expected_value_for_given_input() {
    ...
    TS_ASSERT_DELTA(calculatedValues[0], 1.607544499, 1e-8);
  }
...
private:
  class TestableFickDiffusionSQE : public FickDiffusionSQE {
  public:
    void function1D(double *out, const double *xValues, const size_t nData) const override {
      FickDiffusionSQE::function1D(out, xValues, nData);
    }
  };

  std::shared_ptr<TestableFickDiffusionSQE> createTestFickDiffusionSQE() {
    auto func = std::make_shared<TestableFickDiffusionSQE>();
    func->initialize();
