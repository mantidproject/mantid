...
public:
  ...
  void testAgainstMockData() {
     ...
     * 	Scale^2  ~  \sum_i[ dx_i * P_i * (ln(x[i]))^2 ]  / \sum_i[ dx_i * y[i] ]
     * - Location^2
     * 	Height   ~ \sum_i[ dx_i * y[i] ] / ( Scale * sqrt(2*pi) )
     * These formulas derived from the fact that  logNormal becomes Gaussian
     * with change of variables z = ln(x), that is:
     * 	H/x*exp(-(ln(x)-L)^2/(2*S^2))*dx == LN(x)*dx = exp(-(z-L)^2/(2*S^2))*dz
     * == G(z)*dz
     * 	Example: Location = \integral dz G(z) z = \integral dx LN(x) z =
     * \integral dx LN(x) ln(x),		 *
     *  */
