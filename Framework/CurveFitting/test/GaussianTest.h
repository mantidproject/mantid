...
public:
  ...
  void getHRP38692Peak2Data(Mantid::MantidVec &x, Mantid::MantidVec &y,
                            Mantid::MantidVec &e) {
    // x-values in time-of-flight
    for (size_t i = 0; i < 8; i++)
      x[i] = 79292.4375 + 7.875 * double(i);
    for (size_t i = 8; i < 41; i++)
  ...
  void testAgainstPeak2WithConstraints() {
    ...
    alg.setProperty("Function",
                    boost::dynamic_pointer_cast<IFunction>(fnWithBk));
    ...
    IPeakFunction *pk = dynamic_cast<IPeakFunction *>(
        dynamic_cast<CompositeFunction *>(out.get())->getFunction(1).get());
  ...
  void testAgainstMockData() {
    ...
    // Set which spectrum to fit against and initial starting values
    alg2.setProperty("InputWorkspace",
                     boost::dynamic_pointer_cast<MatrixWorkspace>(ws2D));
  ...
  void testAgainstHRPD_DatasetWithConstraintsSimplex() {
    ...
    // This test will not make sense if the configuration peakRadius is not set
    // correctly.
    const std::string priorRadius =
    ...
    BoundaryConstraint *bc3 =
        new BoundaryConstraint(fn.get(), "Sigma", 20, 100.0);
