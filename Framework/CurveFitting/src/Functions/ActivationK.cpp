...
DECLARE_FUNCTION(ActivationK)

void ActivationK::init() {
...
void ActivationK::function1D(double *out, const double *xValues, const size_t nData) const {
  const double attemptRate = getParameter("AttemptRate");
  const double barrier = getParameter("Barrier");
  const double meVConv = 1.0;
...
void ActivationK::functionDeriv1D(Jacobian *out, const double *xValues, const size_t nData) {
  const double attemptRate = getParameter("AttemptRate");
  const double barrier = getParameter("Barrier");
  const double meVConv = 1.0;
