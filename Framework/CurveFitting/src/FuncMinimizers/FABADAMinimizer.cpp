...
FABADAMinimizer::FABADAMinimizer()
    : m_counter(0), m_ChainIterations(0), m_changes(), m_jump(),
      m_parameters(), m_chain(), m_chi2(0.), m_converged(false),
      m_conv_point(0), m_par_converged(), m_lower(), m_upper(),
      m_bound(), m_criteria(), m_max_iter(0), m_par_changed(),
      m_Temperature(0.), m_counterGlobal(0), m_SimAnnealingItStep(0),
      m_LeftRefrPoints(0), m_TempStep(0.), m_Overexploration(false),
      m_nParams(0), m_InnactConvCriterion(0),
...
void FABADAMinimizer::initialize(API::ICostFunction_sptr function,
  ...
  if (getProperty("SimAnnealingApplied")) {
	 ...
	 if (m_Overexploration) m_LeftRefrPoints = 0;
  }
  else {
	  m_Temperature = 1.0;
	  m_LeftRefrPoints = 0;
...
bool FABADAMinimizer::iterate(size_t) {
  ...
  for (size_t i = 0; i < m; i++) {
    ...
    //Given the new chi2, position, m_changes[ParameterIndex] and chains are updated
    ...
    if (m_LeftRefrPoints == 0 && !m_par_changed[i]
        && m_counter > lowerConvergenceLimit) m_par_converged[i] = true;

    if (m_LeftRefrPoints == 0 && !m_par_converged[i] && m_counter > lowerConvergenceLimit) {
      if (chi2_old != m_chi2) {
...
void FABADAMinimizer::finalize() {
  ...
  if (conv_length > 0) {
	  ...
	  for (size_t j = 0; j < m_nParams; ++j) {
	    ...
	    BestParameters[j] = rc_chain_j[position_min_chi2 - red_conv_chain[m_nParams].begin()];
	    std::sort(rc_chain_j.begin(), rc_chain_j.end());
	    auto pos_par = std::find(rc_chain_j.begin(), rc_chain_j.end(), BestParameters[j]);
  ...
  if (outputParametersTable) {
    ...
    for (size_t j = 0; j < m_nParams; ++j) {
      ...
      row << m_FitFunction->parameterName(j) << BestParameters[j] << error_left[j]
          << error_rigth[j];
    }
	...
	m_leastSquares = boost::dynamic_pointer_cast<CostFunctions::CostFuncLeastSquares>(
  ...
  size_t pdf_length = getProperty("NumberBinsPDF"); // histogram length for the PDF output workspace
  if (pdf_length <= 0) {
  ...
  if (conv_length > 0) {
	  std::sort(red_conv_chain[m_nParams].begin(), red_conv_chain[m_nParams].end());
	  std::vector<double> pdf_y(pdf_length, 0);
	  double start = red_conv_chain[m_nParams][0];
	  double bin =
	      (red_conv_chain[m_nParams][conv_length - 1] - start) / double(pdf_length);
	  size_t step = 0;
  ...
  if (outputCostFunctionTable) {
    ...
    if (conv_length > 0) mostPchi2_red = mostPchi2 / (double(data_number - m_nParams));
    else {
		g_log.warning() << "Most probable chi square not calculated. -1 is returned.\n"
		    << "Most probable reduced chi square not calculated. -1 is returned.\n";
		mostPchi2_red = -1;
	}
...
double FABADAMinimizer::GaussianStep(const double& Jump){
    ...
    mt.seed(123 * (int(m_counter) +
                   45 * int(Jump)) + 14* int(time_t())); // Numbers for the seed
    boost::normal_distribution<double> distr(0.0, std::abs(Jump));
    boost::variate_generator<boost::mt19937,
                             boost::normal_distribution<double>> step(mt,
                                                                     distr);
    return step();
}
...
void FABADAMinimizer::TieApplication(const size_t& ParameterIndex, GSLVector& new_parameters, double& new_value){
	const size_t& i = ParameterIndex;
...
void FABADAMinimizer::JumpUpdate(const size_t& ParameterIndex) {
    ...
    if(m_LeftRefrPoints == 0 && m_changes[i] == m_changesOld[i]) ++m_NumInactiveRegenerations[i];
    else m_changesOld[i] = m_changes[i];
...
void FABADAMinimizer::ConvergenceCheck() {
	if (m_LeftRefrPoints == 0 && m_counter > lowerConvergenceLimit && !m_converged) {
	    size_t t = 0;
	    ...
	    // If all parameters have converged (usually or through observed immobility):
	    ...
	    if (t == m_nParams) {
	      ...
	      if (ImmobilityConv) g_log.warning() << 
...
void FABADAMinimizer::SimAnnealingRefrigeration() {
	...
	for (size_t i = 0; i < m_nParams; ++i) JumpUpdate(i);
	...
	if (m_LeftRefrPoints == 0) m_Temperature = 1.0;
	else m_Temperature /= m_TempStep;
}

bool FABADAMinimizer::IterationContinuation() {
	
  //If still through Simulated Annealing
  if (m_LeftRefrPoints != 0) return true;
  ...
  if (!m_converged) {
    ...
    // convergenceMaxIterations iterations, stop and throw the error.
      ...
      throw std::runtime_error(
          "Convegence NOT reached after " + std::to_string(m_max_iter - m_ChainIterations) +
          " iterations.\n   Try to set better initial values for parameters: " +
          failed + " Or increase the maximum number of iterations (MaxIterations property).");
    }
  } else {
  ...
  // can we even get here? -> Nope (we should not, so we do not want it to continue)
  return false;
}
