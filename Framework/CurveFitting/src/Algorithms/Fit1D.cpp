...
void Fit1D::exec() {
  ...
  const MantidVec &XValues = localworkspace->x(histNumber);
  const MantidVec &YValues = localworkspace->y(histNumber);
  const MantidVec &YErrors = localworkspace->e(histNumber);
  ...
  if (!output.empty()) {
    ...
    const MantidVec &inputX = inputWorkspace->x(iSpec);
    const MantidVec &inputY = inputWorkspace->y(iSpec);
