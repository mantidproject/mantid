...
/// Move constructor
/// @param M :: The other matrix.
ComplexMatrix::ComplexMatrix(ComplexMatrix &&M) : m_matrix(nullptr) { std::swap(m_matrix, M.m_matrix); }
...
ComplexMatrix::ComplexMatrix(const ComplexMatrixMult2 &mult2) : m_matrix(nullptr) { *this = mult2; }

/// Create this matrix from a product of three other matrices
/// @param mult3 :: Matrix multiplication helper object.
ComplexMatrix::ComplexMatrix(const ComplexMatrixMult3 &mult3) : m_matrix(nullptr) { *this = mult3; }
...
void ComplexMatrix::set(size_t i, size_t j, ComplexType value) {
  if (i < m_matrix->size1 && j < m_matrix->size2) {
    gsl_matrix_complex_set(m_matrix, i, j, gsl_complex{{value.real(), value.imag()}});
  } else {
...
ComplexMatrix &ComplexMatrix::operator=(const ComplexMatrixMult2 &mult2) {
  ...
  CBLAS_TRANSPOSE tr1 = mult2.tr1 ? CblasTrans : (mult2.ctr1 ? CblasConjTrans : CblasNoTrans);
  CBLAS_TRANSPOSE tr2 = mult2.tr2 ? CblasTrans : (mult2.ctr2 ? CblasConjTrans : CblasNoTrans);

  // this = m_1 * m_2
  gsl_blas_zgemm(tr1, tr2, gsl_complex{{1.0, 0.0}}, mult2.m_1.gsl(), mult2.m_2.gsl(), gsl_complex{{0.0, 0.0}}, gsl());
...
ComplexMatrix &ComplexMatrix::operator=(const ComplexMatrixMult3 &mult3) {
  ...
  CBLAS_TRANSPOSE tr1 = mult3.tr1 ? CblasTrans : (mult3.ctr1 ? CblasConjTrans : CblasNoTrans);
  CBLAS_TRANSPOSE tr2 = mult3.tr2 ? CblasTrans : (mult3.ctr2 ? CblasConjTrans : CblasNoTrans);
  CBLAS_TRANSPOSE tr3 = mult3.tr3 ? CblasTrans : (mult3.ctr3 ? CblasConjTrans : CblasNoTrans);

  // AB = m_1 * m_2
  gsl_blas_zgemm(tr1, tr2, gsl_complex{{1.0, 0.0}}, mult3.m_1.gsl(), mult3.m_2.gsl(), gsl_complex{{0.0, 0.0}},
                 AB.gsl());

  // this = AB * m_3
  gsl_blas_zgemm(CblasNoTrans, tr3, gsl_complex{{1.0, 0.0}}, AB.gsl(), mult3.m_3.gsl(), gsl_complex{{0.0, 0.0}}, gsl());
