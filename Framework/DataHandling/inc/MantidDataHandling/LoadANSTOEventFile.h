...
#pragma pack(pop)
	...
	*/
	...
	void ReadEventFile(IReader& loader, IEventHandler& handler, IProgress& progress, int def_clock_scale, bool use_tx_chopper)
	{
		...
		// Setup the clock_scale.  In format 0x00010001 this was not part of the headers, hence a command line option -c is provided to allow it to be specified manually.
		// In the current format 0x00010002, clock_scale is written to the header and need not be specified, unless some alternate scale is needed.
		double scale_microsec = hdr_base.clock_scale / 1000.0;
		if (!hdr_base.clock_scale) { // old eventfile format did not have clock_scale...
			scale_microsec = def_clock_scale / 1000.0;;
		}

		// the initial time is not set correctly so wait until primary and auxillary time have been reset before sending events
			...
			if (event_ended) {
				...
				if (oob_en || !frame_start_event) {
					if (oob_event) {
						...
						// if times are ok pass the event trhough the call back, time units in nsec
				...
				if (frame_start_event) {
					// reset timestamp at start of a new frame and pass the absolute time if it is available
					// resolve the logic for the running time so that that the absolute time can be used
					primary_time = 0;
					primary_ok = true;
					handler.newFrame();
				}
