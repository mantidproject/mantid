    ...
    if (info.rank == 1) {
      ...
      if (info.type == NXnumtype::CHAR) {
        ...
        if (!columnTitle.empty()) {
          ...
          int64_t nRows = static_cast<int64_t>(info.dims[0]);
          workspace->setRowCount(nRows);

          const int64_t maxStr = static_cast<int64_t>(info.dims[1]);
          data.load();
...
API::Workspace_sptr LoadNexusProcessed::loadLeanElasticPeaksEntry(const NXEntry &entry) {
  ...
  for (const auto &str : columnNames) {
    if (str == "column_1") {
      ...
      const int64_t maxShapeJSONLength = static_cast<int64_t>(info.dims[1]);
      data.load();
