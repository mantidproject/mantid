 ...
 */
bool LoadRawHelper::hasAlternateDataStream(const Poco::Path &pathToFile) {
#ifdef _WIN32
 ...
 */
std::set<std::string> LoadRawHelper::logFilesFromAlternateDataStream(
  ...
  while (Mantid::Kernel::Strings::extractToEOL(adsStream, line)) {
    if (boost::algorithm::ends_with(line, ".txt") ||
        boost::algorithm::ends_with(line, ".log")) {
