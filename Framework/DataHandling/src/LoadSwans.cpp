...
void LoadSwans::init() {
	...
	declareProperty(
			new WorkspaceProperty<EventWorkspace>("OutputWorkspace", "",
					Direction::Output),
...
void LoadSwans::placeDetectorInSpace() {
	...
	std::string componentName = m_ws->getInstrument()->getStringParameter(
			"detector-name")[0];
	const double distance =
			static_cast<double>(m_ws->getInstrument()->getNumberParameter(
					"detector-sample-distance")[0]);
	const double angle =
			static_cast<double>(m_ws->getInstrument()->getNumberParameter(
					"detector-rotation-angle")[0]);

	g_log.information() << "Moving detector " << componentName << " "
			<< distance << " meters and " << angle << " degrees." << std::endl;
