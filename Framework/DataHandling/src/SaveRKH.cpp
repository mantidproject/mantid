...
void SaveRKH::write1D() {
  ...
  for (size_t i = 0; i < nhist; ++i) {
    ...
    for (size_t j = 0; j < nbins; ++j) {
      ...
      m_outRKH << std::fixed << std::setw(12) << std::setprecision(5) << xval << std::scientific << std::setw(16)
               << std::setprecision(6) << ydata[j] << std::setw(16) << edata[j];
...
void SaveRKH::write2D() {
  ...
  for (size_t i = 0; i < num_hist; ++i) {
    for (const auto &yVal : m_workspace->y(i)) {
      m_outRKH << std::setw(12) << std::scientific << std::setprecision(4) << yVal;
      requireNewLine = true;
  ...
  for (size_t i = 0; i < num_hist; ++i) {
    for (const auto &eVal : m_workspace->e(i)) {
      m_outRKH << std::setw(12) << std::scientific << std::setprecision(4) << eVal;
      if ((itemCount + 1) % LINE_LENGTH == 0)
        m_outRKH << "\n";
      ++itemCount;
    }
