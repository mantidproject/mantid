...
NXstatus SaveToSNSHistogramNexus::copy_file(const char *inFile, int nx__access, const char *outFile,
  ...
  if (NXopen(inFile, nx__access, &inId) != NXstatus::NX_OK) {
    printf("NX_ERROR: Can't open %s\n", inFile);
    return NXstatus::NX_ERROR;
  }

  if (NXopen(outFile, nx_write_access, &outId) != NXstatus::NX_OK) {
    printf("NX_ERROR: Can't open %s\n", outFile);
    return NXstatus::NX_ERROR;
  }

  /* Output global attributes */
  if (WriteAttributes(nx_is_definition) != NXstatus::NX_OK) {
    return NXstatus::NX_ERROR;
  }
  /* Recursively cycle through the groups printing the contents */
  if (WriteGroup(nx_is_definition) != NXstatus::NX_OK) {
    return NXstatus::NX_ERROR;
  }
  /* close input */
  if (NXclose(&inId) != NXstatus::NX_OK) {
    return NXstatus::NX_ERROR;
  }
  ...
  // HDF5 only
  {
    ...
    for (int i = 0; i < links_count; i++) {
      if (NXopenpath(outId, links_to_make[i].to) != NXstatus::NX_OK)
        return NXstatus::NX_ERROR;
      if (NXgetdataID(outId, &link) == NXstatus::NX_OK || NXgetgroupID(outId, &link) == NXstatus::NX_OK) {
        if (NXopenpath(outId, links_to_make[i].from) != NXstatus::NX_OK)
          return NXstatus::NX_ERROR;
        char const *tstr = strrchr(links_to_make[i].to, '/');
        if (!strcmp(links_to_make[i].name, tstr + 1)) {
          if (NXmakelink(outId, &link) != NXstatus::NX_OK)
            return NXstatus::NX_ERROR;
        } else {
          if (NXmakenamedlink(outId, links_to_make[i].name, &link) != NXstatus::NX_OK)
            return NXstatus::NX_ERROR;
        }
      } else {
        return NXstatus::NX_ERROR;
  ...
  if (NXclose(&outId) != NXstatus::NX_OK) {
    return NXstatus::NX_ERROR;
  }
...
NXstatus SaveToSNSHistogramNexus::WriteOutDataOrErrors(const Geometry::RectangularDetector_const_sptr &det,
  ...
  if (m_compress) {
    ...
    if (NXmakedata(outId, field_name, NXnumtype::FLOAT32, dataRank, dataDimensions) != NXstatus::NX_OK)
      return NXstatus::NX_ERROR;
  }
  if (NXopendata(outId, field_name) != NXstatus::NX_OK)
    return NXstatus::NX_ERROR;
  if (WriteAttributes(is_definition) != NXstatus::NX_OK)
    return NXstatus::NX_ERROR;
  if (!doErrors) {
  ...
  // ---- Errors field -----
  if (doBoth) {
    if (NXclosedata(outId) != NXstatus::NX_OK)
      return NXstatus::NX_ERROR;

    if (m_compress) {
      if (NXcompmakedata(outId, errors_field_name, NXnumtype::FLOAT32, dataRank, dataDimensions, NX_COMP_LZW,
                         slabDimensions) != NXstatus::NX_OK)
        return NXstatus::NX_ERROR;
    } else {
      if (NXmakedata(outId, errors_field_name, NXnumtype::FLOAT32, dataRank, dataDimensions) != NXstatus::NX_OK)
        return NXstatus::NX_ERROR;
    }
    if (NXopendata(outId, errors_field_name) != NXstatus::NX_OK)
      return NXstatus::NX_ERROR;
  ...
  for (int x = 0; x < det->xpixels(); x++) {
    ...
    if (!doBoth && (x % x_pixel_slab == x_pixel_slab - 1)) {
      ...
      if (NXputslab(outId, data, slabStartIndices, slabDimensions) != NXstatus::NX_OK)
        return NXstatus::NX_ERROR;
  ...
  if (doBoth) {
    ...
    if (NXopendata(outId, field_name) != NXstatus::NX_OK)
      returnerror = true;
    else if (NXputdata(outId, data) != NXstatus::NX_OK)
      returnerror = true;
    else if (NXclosedata(outId) != NXstatus::NX_OK)
      returnerror = true;
    else {
      ...
      if (NXopendata(outId, errors_field_name) != NXstatus::NX_OK)
        returnerror = true;
      else if (NXputdata(outId, errors) != NXstatus::NX_OK)
        returnerror = true;
      else if (NXclosedata(outId) != NXstatus::NX_OK)
        returnerror = true;
      else {
...
NXstatus SaveToSNSHistogramNexus::WriteDataGroup(const std::string &bank, int is_definition) {
  ...
  if (NXgetinfo(inId, &dataRank, dataDimensions, &dataType) != NXstatus::NX_OK)
    return NXstatus::NX_ERROR;
  ...
  if (!det) {
    ...
    // Just copy that then.
    if (NXmalloc(&dataBuffer, dataRank, dataDimensions, dataType) != NXstatus::NX_OK)
      return NXstatus::NX_ERROR;
    if (NXgetdata(inId, dataBuffer) != NXstatus::NX_OK)
      return NXstatus::NX_ERROR;
    NXname nxName;
    if (NXcompmakedata(outId, nxName, dataType, dataRank, dataDimensions, NX_COMP_LZW, // cppcheck-suppress uninitvar
                       dataDimensions) != NXstatus::NX_OK)
      return NXstatus::NX_ERROR;
    if (NXopendata(outId, nxName) != NXstatus::NX_OK)
      return NXstatus::NX_ERROR;
    if (WriteAttributes(is_definition) != NXstatus::NX_OK)
      return NXstatus::NX_ERROR;
    if (NXputdata(outId, dataBuffer) != NXstatus::NX_OK)
      return NXstatus::NX_ERROR;
    if (NXfree(&dataBuffer) != NXstatus::NX_OK)
      return NXstatus::NX_ERROR;
    if (NXclosedata(outId) != NXstatus::NX_OK)
      return NXstatus::NX_ERROR;
  } else {
    ...
    if (memory_available < memory_required) {
      ...
      if (this->WriteOutDataOrErrors(det, x_slab, "data", "data_errors", false, false, is_definition, bank) !=
          NXstatus::NX_OK)
        return NXstatus::NX_ERROR;
      if (this->WriteOutDataOrErrors(det, x_slab, "errors", "", true, false, is_definition, bank) != NXstatus::NX_OK)
        return NXstatus::NX_ERROR;
    } else {
  ...
  do {
    ...
    if (status == NXstatus::NX_OK) {
      if (!strncmp(theClass, "NX", 2)) {
        if (NXopengroup(inId, nxName, theClass) != NXstatus::NX_OK)
          return NXstatus::NX_ERROR;
        add_path(nxName);

        if (NXgetgroupID(inId, &link) != NXstatus::NX_OK)
          return NXstatus::NX_ERROR;
        if (!strcmp(current_path, link.targetPath)) {
          // Create a copy of the group
          if (NXmakegroup(outId, nxName, theClass) != NXstatus::NX_OK)
            return NXstatus::NX_ERROR;
          if (NXopengroup(outId, nxName, theClass) != NXstatus::NX_OK)
            return NXstatus::NX_ERROR;
          if (WriteAttributes(is_definition) != NXstatus::NX_OK)
            return NXstatus::NX_ERROR;
          if (WriteGroup(is_definition) != NXstatus::NX_OK)
            return NXstatus::NX_ERROR;
          remove_path(nxName);
        } else {
          remove_path(nxName);
          ...
          links_count++;
          if (NXclosegroup(inId) != NXstatus::NX_OK)
            return NXstatus::NX_ERROR;
        }
      } else if (!strncmp(theClass, "SDS", 3)) {
        add_path(nxName);
        if (NXopendata(inId, nxName) != NXstatus::NX_OK)
          return NXstatus::NX_ERROR;
        ...
        if (!strcmp(current_path, link.targetPath)) {
          ...
          //---------------------------------------------------------------------------------------
            ...
            if (WriteAttributes(is_definition) != NXstatus::NX_OK)
              return NXstatus::NX_ERROR;
            if (NXputdata(outId, tof_data.data()) != NXstatus::NX_OK)
              return NXstatus::NX_ERROR;
            if (NXclosedata(outId) != NXstatus::NX_OK)
              return NXstatus::NX_ERROR;
        ...
        if (NXclosedata(inId) != NXstatus::NX_OK)
          return NXstatus::NX_ERROR;
      }
    } else if (status == NXstatus::NX_EOD) {
      if (NXclosegroup(inId) != NXstatus::NX_OK)
        return NXstatus::NX_ERROR;
      if (NXclosegroup(outId) != NXstatus::NX_OK)
        return NXstatus::NX_ERROR;
      return NXstatus::NX_OK;
    }
  } while (status == NXstatus::NX_OK);
  return NXstatus::NX_OK;
}
  ...
  do {
    ...
    if (status == NXstatus::NX_OK) {
      ...
      if (std::none_of(attrs.cbegin(), attrs.cend(),
        ...
        if (NXmalloc(&attrBuffer, 1, &attrLen, attrType) != NXstatus::NX_OK)
          return NXstatus::NX_ERROR;
        if (NXgetattr(inId, attrName, attrBuffer, &attrLen, &attrType) != NXstatus::NX_OK)
          return NXstatus::NX_ERROR;
        if (NXputattr(outId, attrName, attrBuffer, attrLen, attrType) != NXstatus::NX_OK)
          return NXstatus::NX_ERROR;
        if (NXfree(&attrBuffer) != NXstatus::NX_OK)
          return NXstatus::NX_ERROR;
      }
    }
  } while (status != NXstatus::NX_EOD);
  return NXstatus::NX_OK;
}
