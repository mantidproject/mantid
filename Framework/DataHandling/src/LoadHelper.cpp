...
void LoadHelper::addNexusFieldsToWsRun(NXhandle nxfileID, API::Run &runDetails, const std::string &entryName,
  ...
  if (getnextentry_status == NXstatus::OKAY) {
    if ((NXopengroup(nxfileID, nxname, nxclass)) == NXstatus::OKAY) {
      std::string emptyStr;
...
void LoadHelper::recurseAndAddNexusFieldsToWsRun(NXhandle nxfileID, API::Run &runDetails,
  ...
  while (has_entry) {
    ...
    if (getnextentry_status == NXstatus::OKAY) {
      ...
      if (NXopengroup(nxfileID, nxname, nxclass) == NXstatus::OKAY) {
      ...
      else if (NXopendata(nxfileID, nxname) == NXstatus::OKAY) {
        if (parent_class != "NXData" && parent_class != "NXMonitor" &&
          ...
          // Get the value
          if (NXgetinfo(nxfileID, &rank, dims, &type) == NXstatus::OKAY) {
            ...
            if (read_property) {
              ...
              if (NXgetdata(nxfileID, dataBuffer) == NXstatus::OKAY) {
                  ...
                  if ((type == NXnumtype::FLOAT32) || (type == NXnumtype::FLOAT64)) {
                    ...
                    if (dims[0] == 1) {
                      ...
                      if (!runDetails.hasProperty(property_name)) {
                        ...
                        if (units_status != NXstatus::ERROR)
    ...
    } else if (getnextentry_status == NXstatus::EOD) {
