...
void LoadCSNSNexus::init() {
  declareProperty(std::make_unique<PropertyWithValue<std::string>>("Instrument", "GPPD",
                                                         Direction::Input),
  ...
  declareProperty(
      std::make_unique<PropertyWithValue<bool>>("LoadBank", true, Direction::Input),
      "Default true: load bank data, false: load monitor data.");

  declareProperty(
      std::make_unique<ArrayProperty<std::string>>("Bankname", Direction::Input),
...
std::multimap<uint32_t, std::pair<float, int64_t>> LoadCSNSNexus::getEventData(
		const std::vector<std::string> &inputList, const std::vector<uint32_t> &startList,
