...
void LoadCSNSNexus::init() {
  std::vector<std::string> propOptions{"GPPD", "MR",
                                       "SANS", "MPI"};
  declareProperty("Instrument", "GPPD",
  ...
  declareProperty(
	std::make_unique<PropertyWithValue<string>>("NXentryName","csns",Direction::Input),
  ...
  declareProperty(
   ...
   declareProperty(
        make_unique<PropertyWithValue<bool>>("LoadEvent", false, Direction::Input),
   ...
   declareProperty(
	make_unique<PropertyWithValue<bool>>("LoadMonitor", true, Direction::Input),
 ...
 * @return modules
 */
vector<string> LoadCSNSNexus::getModules(string inst, const vector<string> &inputNames)
{
    vector<string> data;
 ...
 * @return pixelId
 */
std::vector<int64_t> LoadCSNSNexus::getPixelId(const vector<string> &inputList)
{
 ...
 */
std::vector<uint32_t> LoadCSNSNexus::getHistData(const vector<string> &inputList)
{
    std::vector<int> tmp;
 ...
 */
void LoadCSNSNexus::loadHistData(boost::shared_ptr<API::MatrixWorkspace> workspace, std::vector<uint32_t> &timeOfFlight, size_t pidNums, std::vector<uint32_t> &histData)
{
 ...
 */
std::multimap<uint32_t, std::pair<float, int64_t>> LoadCSNSNexus::getEventData(const vector<string> &inputList, const vector<uint32_t> &startList, const vector<uint32_t> &endList, const vector<int64_t> &pids)
{
 ...
 */
void LoadCSNSNexus::loadEventData(boost::shared_ptr<DataObjects::EventWorkspace> workspace, const std::vector<uint32_t> &timeOfFlight, size_t pidNums, std::multimap<uint32_t, std::pair<float, int64_t>> evtData)
{
...
{  
    ...
    Mantid::Types::Core::DateAndTime start_time = getExperimentTime("start_time_utc");
	...
	if(m_inputCheck)
	{
		...
		loadEventData(ws_evt, tof_bank, pid_bank.size(), evtData); 
    		...
    		g_log.information()<<"load histogram data "<<endl;	
            	ws_hist = boost::dynamic_pointer_cast<MatrixWorkspace>( WorkspaceFactory::Instance().create("Workspace2D", pid_bank.size(), tof_bank.size(), tof_bank.size()-1));
	...
	vector<uint32_t> tof_mon = getTimeBin("monitor");
        ws_mon = boost::dynamic_pointer_cast<MatrixWorkspace>( WorkspaceFactory::Instance().create("Workspace2D", pid_mon.size(), tof_mon.size(), tof_mon.size()-1));
    ...
    if(m_loadMonitor && m_loadBank){
        ...
        setProperty("OutputWorkspace",boost::dynamic_pointer_cast<Workspace>(wksp_group));
         ...
         if(m_loadBank){
	     if(m_loadEvent){
	         setProperty("OutputWorkspace",boost::dynamic_pointer_cast<Workspace>(ws_evt));
	     }else{
       	         setProperty("OutputWorkspace",boost::dynamic_pointer_cast<Workspace>(ws_hist));
	     }
