...
SaveAscii2::sampleLogValueUnit(const std::string &logName) {
  auto run = m_ws->run();
 ...
 */
void SaveAscii2::writeFileHeader(const std::vector<std::string> &logList,
                                 std::ofstream &outputFile) {
  for (const auto &logName : logList) {
    const std::pair<std::string, std::string> readLog =
        sampleLogValueUnit(logName);
