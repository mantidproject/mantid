...
struct EventFileHeader_Packed { // total content should be 16*int (64 bytes)
  ...
  int evt_stg_nbits_wa;  // number of bits in wa datum // MJL added 5/15 for
                         // format 0x00010002
...
void ReadEventFile(IReader &loader, IEventHandler &handler,
                   TProgressCallback *cbp_progress, bool use_tx_chopper) {
  ...
  /*
  printf(
  "magic_number=0x%08X, format_number=0x%08X, anstohm_version=0x%08X,
  oob_enabled=%d, clock_scale=%d ns/unit\n", hdr_base.magic_number,
  ...
  */
  ...
  unsigned int *ptr_val[NVAL] = {&x, &y, &v, &w,
  ...
  int dt; // , t = 0 dt may be negative occasionally for some DAE types,
          // therefore dt and t are signed ints.

  int nbits_val_oob[NVAL] = {};

  int nbits_val_neutron[NVAL] = {
      hdr_packed.evt_stg_nbits_x, hdr_packed.evt_stg_nbits_y,
      hdr_packed.evt_stg_nbits_v, hdr_packed.evt_stg_nbits_w,
  ...
  int oob_event = 0, c = 0; // For neutron events, oob_event = 0, and for OOB
      ...
      if (oob_en || !frame_start_event) {
        if (oob_event) {
          if (c == -3) {                   // FRAME_AUX_START = -3
            if (!use_tx_chopper && x == 0) // 0 is the reflecting chopper and 1
                                           // is the transmission chopper
              auxillary_time = 0;
          ...
          handler.addEvent(
              x, y, static_cast<int64_t>(primary_time * hdr_base.clock_scale),
