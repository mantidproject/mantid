...
public:
  ...
  void test_rotating_a_mesh_does_not_disturb_a_workspace_sharing_it() {
    // Sample's copy constructor shares the shape pointer, so a copied workspace holds the very same
    // mesh - and the mesh branch used to turn the vertices where they lay.
  ...
  //
  // Everything above asserts only that the algorithm ran and left a tag, so a sign error or a
  // swapped axis order would pass the whole suite. These pin the geometry.
  ...
  void test_run_goniometer_is_ignored() {
    // The rotation asked for and nothing else: 90 against a 90 degree run goniometer gives 90, not
    // 180. Enacting the run's orientation belongs to whoever consumes the shape.
  ...
  void test_two_calls_compose() {
    // Two 90 degree turns leave the sphere on -x. The CSG branch used to replace the tag, discarding
    // the first call; it now composes, which is all a mesh has ever been able to do.
  ...
  void test_rotating_does_not_claim_the_shape_has_been_baked() {
    // A user rotation re-expresses the shape within its own frame, so the marker stays identity for
    // both types - the CSG branch pins <applied-goniometer> alongside the total it writes.
  ...
  void test_a_baked_rotation_survives_a_user_rotation() {
    // The user's turn goes on top of the total, leaving the record of the bake unchanged.
    auto shapeXML = ComponentCreationHelper::sphereXML(0.5, V3D(1.0, 0.0, 0.0), "sphere");
