...
public:
  ...
  void test_copyGroup_same_file() {
    FileResource testInput("copy_group_same_file.h5");
    {
      ...
      Group g1 = input1.createGroup("/one");
      input1.createGroup("/two");
      g1.createGroup("three");
    }
    {
...
private:
  ...
  void _assert_group_structure(H5::H5File &file, const std::vector<std::string> &paths) {
    for (const auto &path : paths) {
      TS_ASSERT(_groupExists(file, path));
    }
  }
};
