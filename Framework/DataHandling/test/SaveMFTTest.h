...
public:
  ...
  void testExec() {
    ...
    TS_ASSERT_EQUALS(columns[2], "1");
    TS_ASSERT_EQUALS(columns[3], "1");
  ...
  void testNoX() {
    ...
    // the first is black due to the leading separator
    TS_ASSERT_EQUALS(columns[0], "");
    TS_ASSERT_EQUALS(columns[1], "0");
    TS_ASSERT_EQUALS(columns[2], "1");
    TS_ASSERT_EQUALS(columns[3], "1");
    TS_ASSERT((columns[4] == "nan") || (columns[5] == "inf"));
    in.close();
  ...
  void test_fail_invalid_workspace() {
    ...
    TS_ASSERT_THROWS_ANYTHING(
        alg.setProperty("InputWorkspace", "NotARealWS"));
...
private:
  ...
  void createWS(bool zeroX = false, bool zeroY = false, bool zeroE = false,
    ...
    // Check if any the data should be zeroed to check for divide by zero or
    // similiar
    if (zeroX)
      ws->mutableX(0) = m_zeros;
    else
      ws->mutableX(0) = m_data;

    if (zeroY)
      ws->mutableY(0) = m_zeros;
    else
      ws->mutableY(0) = m_data;

    if (zeroE)
      ws->mutableE(0) = m_zeros;
    else
      ws->mutableE(0) = m_data;
  }
