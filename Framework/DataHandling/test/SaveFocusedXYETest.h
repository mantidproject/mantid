...
public:
  ...
  void testHeaderTemperature() {
    ...
    TS_ASSERT(std::filesystem::exists(focusfile));
  ...
  void testHistogramFileNotSplit() {
    ...
    filestrm.close();
    std::filesystem::remove(focusfile);
  ...
  void testSaveGSSWorkspaceGroups_dont_multiply_bin_width() {
    ...
    std::ifstream filestrm(focusfile);
    std::string line;
    int bin_no(1);
    while (getline(filestrm, line)) {
      if (line.empty())
...
public:
  ...
  void tearDown() override {
    ...
    if (std::filesystem::exists(focusedFile))
      std::filesystem::remove(focusedFile);
  }
