...
public:
  ...
  void test_load_emu_algorithm() {
	  ...
	  TS_ASSERT(
		  ...
		  0)
		  TS_ASSERT(
			  run.getProperty("end_time")->value().find("2018-07-26T10:17:12.6") ==
			  0)

		  // test some data properties
		  auto logpm = [&run](std::string tag) { return run.getPropertyValueAsType<double>(tag); };
	  ...
	  auto iparam = [&instr](std::string tag) { return instr->getNumberParameter(tag)[0]; };
