...
using Mantid::DataHandling::LoadILLSANS2;
...
class LoadILLSANS2Test : public CxxTest::TestSuite {
public:
  // This pair of boilerplate methods prevent the suite being created statically
  // This means the constructor isn't called when running other tests
  static LoadILLSANS2Test *createSuite() { return new LoadILLSANS2Test(); }
  static void destroySuite(LoadILLSANS2Test *suite) { delete suite; }
  ...
  void test_D11_no_instrument() {
    LoadILLSANS2 alg;
    alg.setChild(true);
    alg.initialize();
    TS_ASSERT_THROWS_NOTHING(alg.setPropertyValue("Filename", "010560.nxs"))
    TS_ASSERT_THROWS_NOTHING(alg.setPropertyValue("LoadInstrument", "0"))
    TS_ASSERT_THROWS_NOTHING(alg.setPropertyValue("OutputWorkspace", "__unused_for_child"))
    TS_ASSERT_THROWS_NOTHING(alg.execute())
    TS_ASSERT(alg.isExecuted())
    MatrixWorkspace_const_sptr outputWS = alg.getProperty("OutputWorkspace");
  ...
  void test_D22B_Cycle211() {
    ...
    TS_ASSERT(!outputWS->isHistogramData())
    TS_ASSERT(!outputWS->isDistribution())
    TS_ASSERT_EQUALS(outputWS->getAxis(0)->unit()->unitID(), "Empty");
    const auto &instrument = outputWS->getInstrument();
    const auto &run = outputWS->run();
  ...
  void test_D16_GAMMA() {
    ...
    TS_ASSERT_EQUALS(xAxis.size(), 1)
    TS_ASSERT_DELTA(xAxis[0], 0, 1E-3)
  ...
  void test_D33_MONO() {
    ...
    TS_ASSERT_EQUALS(xAxis.size(), 1)
    TS_ASSERT_DELTA(xAxis[0], 0.0, 1E-3)
...
class LoadILLSANS2TestPerformance : public CxxTest::TestSuite {
public:
  void setUp() override {
    m_alg.initialize();
    m_alg.setChild(true);
  ...
  void testLoadILLSANS2Performance() {
    for (int i = 0; i < 20; ++i) {
