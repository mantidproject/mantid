...
public:
    void testInit()
    {
        ...
        TS_ASSERT_THROWS_NOTHING(loader.setProperty("InstrumentName", "testInstName"));
    ...
    void testExec()
    {
        ...
        TS_ASSERT_THROWS_NOTHING(testInst1 = API::InstrumentDataService::Instance().retrieve("testInstrument"));
    }
...
private:
    ...
    void setUpAddDetectorWithParent(LoadNexusGeometry &loader, Geometry::Instrument_sptr instrument)
    {
        std::string detName("testDetector");
        Eigen::Vector3d detPos(1.0,1.0,0.5);
        int id(5);
        TS_ASSERT_THROWS_NOTHING(loader.addDetector(detName, detPos, id, instrument));
    }
    void setUpAddSource(LoadNexusGeometry &loader, Geometry::Instrument_sptr instrument)
    {
        std::string name = "testSource";
        Eigen::Vector3d pos(0.0,0.0,-1.0);
        TS_ASSERT_THROWS_NOTHING(loader.addSource(name, pos, instrument));
    }
    void setUpAddSample(LoadNexusGeometry &loader, Geometry::Instrument_sptr instrument)
    {
    ...
    void checkDetectorWithParent(Geometry::InstrumentVisitor &inst, ComponentInfo_sptr compInfo)
    {
