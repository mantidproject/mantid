...
public:
  ...
  //
  // Baked in only when there is no sample environment, because the container is never rotated, so a
  // sample turned inside a stationary can cannot arise. Neither environment path bakes - the two are
  // the same feature for the two shape types. The suite previously had no goniometer in it at all.
  ...
  void test_Setting_The_Same_Environment_Twice_Gives_The_Same_Shape() {
    using Mantid::Kernel::ConfigService;
    // Environment specs live in a process-wide cache and the container hands out its own shape
    // pointer, so anything done to that shape leaks into every later call.
    auto &config = ConfigService::Instance();
    const auto defaultDirs = config.getString("instrumentDefinition.directory");
    config.setString("instrumentDefinition.directory", m_testRoot);
  ...
  Mantid::Geometry::BoundingBox boundingBoxAfterSettingEnvironment() {
    ...
    // This spec's sample is an STL mesh handed straight out of the cache, not rebuilt per call from
    // <samplegeometry>. It rejects a Geometry override, so there is none here.
