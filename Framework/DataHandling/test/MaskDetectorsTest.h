...
public:
    ...
    void setUpWS(bool event, const std::string &name = "testSpace",
        ...
        if (event) {
            ...
            // Does not have connection between instrument and spectra though has to have instrument
            space->setInstrument(instr);
        }
    ...
    void test_MaskWorksForGroupedWSAllDet() {
        ...
        grouper->setPropertyValue("GroupingPattern",
        ...
        for (size_t i = 0; i < inputWS->getNumberHistograms(); ++i) {
            ...
            if (i == 0 || i == 2 || i == 5) {
                TSM_ASSERT_EQUALS("Detector with id: " +
                    boost::lexical_cast<std::string>(det->getID()) + "; Spectra N: " +
