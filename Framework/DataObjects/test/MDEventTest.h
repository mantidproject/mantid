...
public:
  ...
  void test_simple_constructors() {
    ...
    // Ubuntu 10.04 and MacOS as of now.
    // Therefore these tests fail and the events are somewhat too big on these
    // platforms:
    // TS_ASSERT_EQUALS( sizeof(a), sizeof(Mantid::coord_t)*3+8+6);
    // TS_ASSERT_EQUALS( sizeof(b), sizeof(Mantid::coord_t)*4+8+6);
  }

  void test_constructor_noGoniometer() {
   ...
   * in by the compiler */
  void test_CopyConstructor_noGoniometer() {
    Mantid::coord_t coords[3] = {0.125, 1.25, 2.5};
    MDEvent<3> b(2.5, 1.5, 123, 456789, coords);
    MDEvent<3> a(b);
    TS_ASSERT_EQUALS(a.getNumDims(), 3);
  ...
  void test_serialize_deserializeLean() {
    size_t nPoints = 99; // the number should not be nPoints%4=0 to hold test
    ...
    std::vector<Mantid::coord_t> data;
    size_t ncols;
  ...
  void test_serialize_deserializeFat() {
    ...
    // traits are: signal, errorSquared, runIndex, goniometerIndex, detectorId
...
public:
  ...
  void setUp() override {
    ...
    events4.reserve(num);
    lean_events3.clear();
    lean_events3.reserve(num);
    lean_events4.clear();
    lean_events4.reserve(num);
  }
  ...
  void test_create_MDEvent3() {
    ...
    for (size_t i = 0; i < num; i++)
      events3.emplace_back(
          MDEvent<3>(signal, error, runIndex, goniometerIndex, detectorId, center));
  }
  ...
  void test_create_MDEvent4() {
    ...
    for (size_t i = 0; i < num; i++)
      events4.emplace_back(
          MDEvent<4>(signal, error, runIndex, goniometerIndex, detectorId, center));
  }
  ...
  void test_serialize_deserializeFat() {
    ...
    TS_ASSERT_THROWS_NOTHING(
        MDEvent<4>::eventsToData(events, data, ncols, totalSignal, totalErrSq));
    TS_ASSERT_EQUALS(5 + 4, ncols);
    TS_ASSERT_EQUALS((5 + 4) * nPoints, data.size());
