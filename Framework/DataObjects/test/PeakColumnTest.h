...
public:
  PeakColumnTestHelper(std::vector<Peak> &peaks, const std::string &name) : PeakColumn(peaks, name) {}
...
public:
  ...
  void test_constructor_throws_given_unknown_name() {
    TS_ASSERT_THROWS(PeakColumn(m_peaks, "NotPeakColumn"), const std::runtime_error &);
  }
