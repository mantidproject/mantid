...
public:
  ...
  size_t columnCount() const override { return static_cast<int>(columns.size()); }
  ...
  std::vector<std::string> getColumnNames() const override { return this->columnNames; }
...
private:
  ...
  std::unique_ptr<Geometry::IPeak> createPeakQSample(const Kernel::V3D &position) const;
