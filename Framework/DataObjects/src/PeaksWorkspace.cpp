 ...
 */
PeaksWorkspace::PeaksWorkspace() : IPeaksWorkspace(), peaks(), columns(), columnNames(), m_coordSystem(None) {
  initColumns();
...
PeaksWorkspace::PeaksWorkspace(const PeaksWorkspace &other)
    : IPeaksWorkspace(other), peaks(other.peaks), columns(), columnNames(), m_coordSystem(other.m_coordSystem) {
  initColumns();
  // PeaksWorkspace does not use the grouping mechanism of ExperimentInfo.
  setNumberOfDetectorGroups(0);
}
...
void PeaksWorkspace::removePeaks(std::vector<int> badPeaks) {
  ...
  int ip = -1;
  auto it = std::remove_if(peaks.begin(), peaks.end(), [&ip, badPeaks](Peak &pk) {
    (void)pk;
    ip++;
...
void PeaksWorkspace::addPeakColumn(const std::string &name) {
  ...
  columns.emplace_back(std::make_shared<DataObjects::PeakColumn>(this->peaks, name));
  // Cache the names
