...
protected:
  ...
  Poco::Net::HTTPResponse::HTTPStatus lastStatus() const override { return Poco::Net::HTTPResponse::HTTP_NOT_FOUND; }
...
class MockMantidAPIStatusNotFoundWithErrMsg : public MantidWebServiceAPIJobManager {
public:
...
protected:
  ...
  Poco::Net::HTTPResponse::HTTPStatus lastStatus() const override { return Poco::Net::HTTPResponse::HTTP_OK; }
...
public:
  ...
  static MantidWebServiceAPIJobManagerTest *createSuite() { return new MantidWebServiceAPIJobManagerTest(); }
  static void destroySuite(MantidWebServiceAPIJobManagerTest *suite) { delete suite; }
  ...
  void test_constructCasts() {
    ...
    TSM_ASSERT("Job manager constructed dynamically should cast to IRemoteJobManager",
               dynamic_cast<Mantid::API::IRemoteJobManager *>(djm.get()));
    TSM_ASSERT("Job manager constructed statically should cast to IRemoteJobManger",
    ...
    TSM_ASSERT("Job manager constructed dynamically should cast to "
               ...
               dynamic_cast<Mantid::RemoteJobManagers::MantidWebServiceAPIJobManager *>(djm.get()));
    TSM_ASSERT("Job manager constructed statically should cast to "
               "MantidWebServiceAPIJobManager",
               dynamic_cast<Mantid::RemoteJobManagers::MantidWebServiceAPIJobManager *>(&wsapi));
  }
  ...
  void test_createWithFactory() {
    ...
    TSM_ASSERT_THROWS_NOTHING("create() with " + FermiName + " in the facility " + SNSFac + " should not throw",
    ...
    TSM_ASSERT_THROWS("create() with " + FermiName + " in a facility other than " + SNSFac + " should fail",
                      IRemoteJobManager_sptr jobManager = RemoteJobManagerFactory::Instance().create(FermiName),
  ...
  void test_missingOrWrongParamsWithoutLogin() {
    ...
    TSM_ASSERT_THROWS("abort job without job ID should throw", jm.abortRemoteJob(""), const JSONParseException &);
    TSM_ASSERT_THROWS("abort job with wrong job ID should throw", jm.abortRemoteJob("anything_wrong"),
                      const JSONParseException &);

    TSM_ASSERT_THROWS("download with wrong transaction ID should throw",
                      jm.downloadRemoteFile("any_wrong_transID", "remote_fname", "local_fname"),
                      const JSONParseException &);
    ...
    TSM_ASSERT_THROWS("query all jobs without logging in should throw", infos = jm.queryAllRemoteJobs(),
                      const std::runtime_error &);
    TSM_ASSERT_EQUALS("there should not be any job information returned from the remote", infos.size(), 0);

    std::vector<std::string> files;
    TSM_ASSERT_THROWS("query remote files with wrong transaction ID should throw",
                      files = jm.queryRemoteFile("any_wrong_transID"), const JSONParseException &);
    TSM_ASSERT_EQUALS("The file list for a wrong transaction should be empty", files.size(), 0);

    IRemoteJobManager::RemoteJobInfo info;
    TSM_ASSERT_THROWS("query job info should throw for wrong job ID", info = jm.queryRemoteJob("any_wrong_jobID"),
                      const JSONParseException &);

    std::string id;
    TSM_ASSERT_THROWS("start transaction without logging in should throw", id = jm.startRemoteTransaction(),
                      const JSONParseException &);
    TSM_ASSERT_EQUALS("failed start transaction should not return any ID", id, "");

    TSM_ASSERT_THROWS("stop transaction without logging in should throw", jm.stopRemoteTransaction("a_wrong_transID"),
                      const JSONParseException &);

    std::string jobID;
    TSM_ASSERT_THROWS("submit job without logging in should throw",
                      id = jm.submitRemoteJob("a_wrong_transID", "executable", "--params 0", "name_for_job"),
                      const JSONParseException &);
    TSM_ASSERT_EQUALS("failed submit job should not return any ID", jobID, "");

    TSM_ASSERT_THROWS("upload file without logging in should throw",
                      jm.uploadRemoteFile("wrong_transID", "remote_fname", "local_fname"), const JSONParseException &);

    // and failed login at the end
    TSM_ASSERT_THROWS("authenticate with empty credentials should throw", jm.authenticate("", ""),
                      const JSONParseException &);
    TSM_ASSERT_THROWS("mocked authenticate should throw", jm.authenticate("wrong_user", "no_pass"),
                      const JSONParseException &);
  }
...
private:
  ...
  void checkJMOKResponseNoMsg(MantidWebServiceAPIJobManager &jm) {
    TSM_ASSERT_THROWS_NOTHING("abort job with ok response code from server should not throw",
                              jm.abortRemoteJob("anything"));

    TSM_ASSERT_THROWS_NOTHING("authenticate with ok response code from server should not throw",
                              jm.authenticate("any_user", "any_pass"));

    TSM_ASSERT_THROWS_NOTHING("download with ok response code from server should not throw",
    ...
    TSM_ASSERT_EQUALS("The file list for a transaction should be empty", files.size(), 0);
    ...
    TSM_ASSERT_THROWS_NOTHING("stop transaction with ok response code from server should not throw",
    ...
    TSM_ASSERT_THROWS("upload file with ok response code but no content from server should "
                      "throw",
                      jm.uploadRemoteFile("wrong_transID", "remote_fname", "local_fname"), const JSONParseException &);
  }
  ...
  void checkJMWithErrResponse(MantidWebServiceAPIJobManager &jm) {
    TSM_ASSERT_THROWS("abort job with error response code from server should throw", jm.abortRemoteJob("anything"),
                      const JSONParseException &);

    TSM_ASSERT_THROWS("authenticate with error response code from server should throw",
                      jm.authenticate("any_user", "any_pass"), const JSONParseException &);

    TSM_ASSERT_THROWS("download with error response code from server should throw",
                      jm.downloadRemoteFile("any_transID", "remote_fname", "local_fname"), const JSONParseException &);

    std::vector<IRemoteJobManager::RemoteJobInfo> infos;
    TSM_ASSERT_THROWS("query all jobs with error response from server should throw", infos = jm.queryAllRemoteJobs(),
                      const std::runtime_error &);

    std::vector<std::string> files;
    TSM_ASSERT_THROWS("query remote files with error response code from server should throw",
                      files = jm.queryRemoteFile("any"), const JSONParseException &);
    TSM_ASSERT_EQUALS("The file list for a wrong transaction should be empty", files.size(), 0);

    IRemoteJobManager::RemoteJobInfo info;
    TSM_ASSERT_THROWS("query job info with error response from server should throw", info = jm.queryRemoteJob("any"),
                      const JSONParseException &);

    std::string id;
    TSM_ASSERT_THROWS("start transaction with error response from server should throw",
                      id = jm.startRemoteTransaction(), const JSONParseException &);
    TSM_ASSERT_EQUALS("failed start transaction should not return any ID", id, "");

    TSM_ASSERT_THROWS("stop transaction with error response from server should throw",
    ...
    TSM_ASSERT_THROWS("upload file with error response from server should throw",
                      jm.uploadRemoteFile("wrong_transID", "remote_fname", "local_fname"), const JSONParseException &);
  }
  ...
  void checkJMErrWithoutErrMessage(MantidWebServiceAPIJobManager &jm) {
    ...
    TSM_ASSERT_THROWS("authenticate with error response code but no content from server but "
                      "no content should throw",
                      jm.authenticate("any_user", "any_pass"), const JSONParseException &);

    TSM_ASSERT_THROWS("download with error response code but no content from server should "
                      "throw",
                      jm.downloadRemoteFile("any_transID", "remote_fname", "local_fname"), const JSONParseException &);
    ...
    TSM_ASSERT_THROWS("upload file with error response but no content from server should "
                      "throw",
                      jm.uploadRemoteFile("wrong_transID", "remote_fname", "local_fname"), const JSONParseException &);
  }
