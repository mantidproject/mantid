 ...
 */
struct NXInfo {
  NXInfo() : nxname(), rank(0), dims(), type(NXnumtype::BAD), stat(NXstatus::ERROR) {}
  ...
  operator bool() { return stat == NXstatus::OKAY; } ///< returns success of an operation
};

/**  Information about a Nexus class
 */
struct NXClassInfo {
  NXClassInfo() : nxname(), nxclass(), datatype(NXnumtype::BAD), stat(NXstatus::ERROR) {}
  std::string nxname;  ///< name of the object
...
public:
   ...
   *  @param name :: The name of the dataset
   *  @return NXInfo::stat is set to NXstatus::ERROR if the dataset does not exist
   */
