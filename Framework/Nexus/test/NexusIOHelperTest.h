...
public:
  ...
  void test_nexus_io_helper_readNexusVector_v20_ess_integration_2018() {
    const std::string filename = API::FileFinder::Instance().getFullPath(
  ...
  void test_nexus_io_helper_readNexusSlab() {
    ...
    file.openGroup("raw_event_data", "NXevent_data");
    auto event_index =
        readNexusSlab<uint64_t>(file, "event_index", {10}, {200});
    TS_ASSERT_EQUALS(event_index.size(), 200);
    auto event_id = readNexusSlab<uint64_t>(file, "event_id", {100}, {300});
    TS_ASSERT_EQUALS(event_id.size(), 300);
    auto event_time_offset =
        readNexusSlab<float>(file, "event_time_offset", {1000}, {400});
    TS_ASSERT_EQUALS(event_time_offset.size(), 400);
    auto event_time_zero =
        readNexusSlab<double>(file, "event_time_zero", {111}, {501});
  ...
  void test_nexus_io_helper_readNexusSlab_throws_when_downcasting() {
    ...
    file.openGroup("raw_event_data", "NXevent_data");
    auto event_index =
        readNexusSlab<uint64_t>(file, "event_index", {111}, {222});
    TS_ASSERT_EQUALS(event_index.size(), 222);
    TS_ASSERT_THROWS_EQUALS(
        auto event_id = readNexusSlab<uint16_t>(file, "event_id", {222}, {333}),
        std::runtime_error & e, std::string(e.what()),
        "Downcasting is forbidden in NeXusIOHelper::readNexusAnySlab");
    TS_ASSERT_THROWS_EQUALS(
        auto event_time_offset =
            readNexusSlab<uint16_t>(file, "event_time_offset", {333}, {444}),
        std::runtime_error & e, std::string(e.what()),
        "Downcasting is forbidden in NeXusIOHelper::readNexusAnySlab");
    TS_ASSERT_THROWS_EQUALS(
        auto event_time_zero =
            readNexusSlab<float>(file, "event_time_zero", {444}, {555}),
        std::runtime_error & e, std::string(e.what()),
        "Downcasting is forbidden in NeXusIOHelper::readNexusAnySlab");
    file.closeGroup();
    file.closeGroup();
  ...
  void test_nexus_io_helper_readNexusSlab_v20_ess_integration_2018() {
    ...
    auto event_index =
        readNexusSlab<uint64_t>(file, "event_index", {111}, {222});
    TS_ASSERT_EQUALS(event_index.size(), 222);
    auto event_id = readNexusSlab<uint64_t>(file, "event_id", {333}, {444});
    TS_ASSERT_EQUALS(event_id.size(), 444);
    auto event_time_offset =
        readNexusSlab<float>(file, "event_time_offset", {555}, {666});
    TS_ASSERT_EQUALS(event_time_offset.size(), 666);
    auto event_time_zero =
        readNexusSlab<double>(file, "event_time_zero", {777}, {888});
