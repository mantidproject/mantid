...
void NXObject::getAttributes() {
  ...
  while (NXgetnextattra(m_fileID, pName, &rank, dims, &iType) != NXstatus::EOD) {
...
bool NXClass::isValid(const std::string &path) const {
  if (NXopengrouppath(m_fileID, path.c_str()) == NXstatus::OKAY) {
...
void NXClass::open() {
  if (NXopengrouppath(m_fileID, m_path.c_str()) == NXstatus::ERROR) {
...
bool NXClass::openLocal(const std::string &nxclass) {
  ...
  if (NXopengroup(m_fileID, name().c_str(), className.c_str()) == NXstatus::ERROR) {
    // It would be nice if this worked
    // if (NXstatus::ERROR == NXopengrouppath(m_fileID,m_path.c_str()))
    //{
...
void NXClass::close() {
  if (NXclosegroup(m_fileID) == NXstatus::ERROR) {
 ...
 */
bool NXClass::containsDataSet(const std::string &query) const { return getDataSetInfo(query).stat != NXstatus::ERROR; }
...
void NXDataSet::open() {
  ...
  if (NXopenpath(m_fileID, group_path.c_str()) == NXstatus::ERROR) {
    throw std::runtime_error("Cannot open dataset " + m_path);
  }
  if (NXopendata(m_fileID, name().c_str()) != NXstatus::OKAY) {
    throw std::runtime_error("Error opening data in group \"" + name() + "\"");
  }

  if (NXgetinfo(m_fileID, &m_info.rank, m_info.dims, &m_info.type) != NXstatus::OKAY) {
...
void NXDataSet::getData(void *data) {
  ...
  if (NXgetdata(m_fileID, data) != NXstatus::OKAY)
...
void NXDataSet::getSlab(void *data, int start[], int size[]) {
  NXopendata(m_fileID, name().c_str());
  if (NXgetslab(m_fileID, data, start, size) != NXstatus::OKAY)
