...
public:
  ...
  void test_output_produced() {
	 std::string emptyString("");
	  ...
	  TS_ASSERT_THROWS_NOTHING(alg.setProperty("InputWorkspaceGroup", wsGroup->getName()));
	  ...
	  TS_ASSERT_THROWS_NOTHING(alg.setProperty("SummedPeriods", std::to_string(1)));
  ...
  void test_single_group_doesnt_change_data() {
	  ...
	  MatrixWorkspace_sptr ws = WorkspaceCreationHelper::createGroupedWorkspace2D(1, 10, 0.5);
	  ...
	  TS_ASSERT_THROWS_NOTHING(algCompare.setProperty("Workspace1", ws->getName()));
	  TS_ASSERT_THROWS_NOTHING(algCompare.setProperty("Workspace2", wsOut->getName()));
  ...
  void test_grouping_with_counts() {
	  ...
	  auto wsOut = boost::dynamic_pointer_cast<MatrixWorkspace>(wsGroup->getItem("inputGroup; Group; test; Counts; #1_Raw"));
