...
void MuonGroupDetectors::exec() {
  ...
  for (auto rowIt = nonEmptyRows.begin(); rowIt != nonEmptyRows.end(); ++rowIt) {
    ...
    std::vector<size_t> wsIndices = inWS->getIndicesFromDetectorIDs(detectorIDs);
    ...
    for (auto &wsIndex : wsIndices) {
      ...
      // it's
      // elements
      outWS->getSpectrum(groupIndex).addDetectorIDs(inWS->getSpectrum(wsIndex).getDetectorIDs());
    }

    outWS->setHistogram(groupIndex, hist);

    outWS->getSpectrum(groupIndex).setSpectrumNo(static_cast<specnum_t>(groupIndex + 1));
  }
