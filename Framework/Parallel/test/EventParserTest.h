...
namespace anonymous {
template <typename IndexType, typename TimeZeroType, typename TimeOffsetType> class FakeParserDataGenerator {
public:
  FakeParserDataGenerator(size_t numBanks, size_t pixelsPerBank, size_t numPulses, size_t maxEventsPerPixel = 100) {
  ...
  const std::vector<IndexType> &eventIndex(size_t bank) const { return m_event_indices[bank]; }

  const std::vector<TimeZeroType> &eventTimeZero() const { return m_event_time_zero; }

  const std::vector<TimeOffsetType> &eventTimeOffset(size_t bank) const { return m_event_time_offsets[bank]; }

  const std::vector<int32_t> &eventId(size_t bank) const { return m_event_ids[bank]; }
  ...
  std::shared_ptr<EventParser<TimeOffsetType>> generateTestParser() {
    ...
    return std::make_shared<EventParser<TimeOffsetType>>(comm, std::vector<std::vector<int>>{}, m_bank_offsets,
                                                         eventLists);
  }
...
private:
  void generateTestData(const size_t numBanks, const size_t pixelsPerBank, const size_t numPulses,
    ...
    for (size_t bank = 0; bank < numBanks; ++bank) {
      ...
      for (size_t pulse = 0; pulse < numPulses; ++pulse) {
        ...
        for (size_t pixel = 0; pixel < pixelsPerBank; ++pixel) {
          ...
          std::generate_n(std::back_inserter(list), eventSize,
          ...
          std::transform(list.cbegin() + prev_end, list.cend(), std::back_inserter(m_event_time_offsets[bank]),
                         [](const TofEvent &event) { return static_cast<TimeOffsetType>(event.tof()); });
        }
      }
    }
  }
  ...
  size_t getRandEventSize(size_t min = 1, size_t max = 1000) { return static_cast<size_t>(rand()) % (max - min) + min; }

  double getRandomTimeOffset(size_t pulseWidth) { return static_cast<double>(rand() % pulseWidth); }
...
public:
  ...
  void testConstruct() {
    ...
    Parallel::Communicator comm;
    TS_ASSERT_THROWS_NOTHING((EventParser<double>(comm, rankGroups, bankOffsets, eventLists)));
  }
  ...
  void testConvertEventIDToGlobalSpectrumIndex() {
    ...
    detail::eventIdToGlobalSpectrumIndex(eventId.data(), eventId.size(), bankOffsets[0]);
  ...
  void testExtractEventsFull() {
    ...
    detail::eventIdToGlobalSpectrumIndex(event_id.data() + range.eventOffset, range.eventCount, 1000);
    ...
    partitioner.partition(rankData, event_id.data(), event_time_offset.data() + range.eventOffset, range);

    TS_ASSERT(std::equal(rankData[0].cbegin(), rankData[0].cend(), event_time_offset.cbegin(),
                         [](const EventParser<double>::Event &e, const double tof) { return tof == e.tof; }));
  ...
  void testExtractEventsPartial() {
    ...
    TS_ASSERT(std::equal(
        rankData[0].cbegin(), rankData[0].cend(), event_time_offset.cbegin() + range.eventOffset,
        [](const EventParser<double>::Event &e, const double tof) { return static_cast<double>(tof) == e.tof; }));
    doTestRankData(rankData, gen, range);
  }
  ...
  void testParsingFull_1Pulse_1Bank() {
    ...
    parser->setEventDataPartitioner(std::make_unique<EventDataPartitioner<int32_t, int32_t, double>>(
        1, PulseTimeGenerator<int32_t, int32_t>{gen.eventIndex(0), gen.eventTimeZero(), "nanosecond", 0}));
    parser->setEventTimeOffsetUnit("microsecond");
    auto event_id = gen.eventId(0);
    auto event_time_offset = gen.eventTimeOffset(0);

    parser->startAsync(event_id.data(), event_time_offset.data(), gen.generateBasicRange(0));
  ...
  void testParsingFull_1Rank_1Bank() {
    ...
    auto parser = gen.generateTestParser();
    parser->setEventDataPartitioner(std::make_unique<EventDataPartitioner<int32_t, int64_t, float>>(
        1, PulseTimeGenerator<int32_t, int64_t>{gen.eventIndex(0), gen.eventTimeZero(), "nanosecond", 0}));
    parser->setEventTimeOffsetUnit("microsecond");
    auto event_id = gen.eventId(0);
    auto event_time_offset = gen.eventTimeOffset(0);
  ...
  void testParsingFull_1Rank_2Banks() {
    int numBanks = 2;
    anonymous::FakeParserDataGenerator<int32_t, int64_t, double> gen(numBanks, 10, 7);
    auto parser = gen.generateTestParser();

    for (int i = 0; i < numBanks; i++) {
      parser->setEventDataPartitioner(std::make_unique<EventDataPartitioner<int32_t, int64_t, double>>(
          1, PulseTimeGenerator<int32_t, int64_t>{gen.eventIndex(i), gen.eventTimeZero(), "nanosecond", 0}));
      parser->setEventTimeOffsetUnit("microsecond");
      auto event_id = gen.eventId(i);
      auto event_time_offset = gen.eventTimeOffset(i);

      parser->startAsync(event_id.data(), event_time_offset.data(), gen.generateBasicRange(i));
      parser->wait();
    }
    gen.checkEventLists();
  }
  ...
  void testParsingFull_InParts_1Rank_1Bank() {
    ...
    for (int i = 0; i < parts; ++i) {
      ...
      parser->startAsync(event_id.data() + offset, event_time_offset.data() + offset, range);
      parser->wait();
    }
    gen.checkEventLists();
  }
  ...
  void testParsingFull_InParts_1Rank_3Banks() {
    ...
    for (size_t bank = 0; bank < numBanks; bank++) {
      parser->setEventDataPartitioner(std::make_unique<EventDataPartitioner<int32_t, int64_t, double>>(
          1, PulseTimeGenerator<int32_t, int64_t>{gen.eventIndex(bank), gen.eventTimeZero(), "nanosecond", 0}));
      parser->setEventTimeOffsetUnit("microsecond");
  ...
  void test_setEventTimeOffsetUnit() {
    ...
    parser.setEventDataPartitioner(
        std::make_unique<EventDataPartitioner<int32_t, int32_t, double>>(1, std::move(pulseTimes)));
    ...
    TS_ASSERT_THROWS_EQUALS(parser.setEventTimeOffsetUnit("millisecond"), const std::runtime_error &e,
                            std::string(e.what()), "EventParser: unsupported unit `millisecond` for event_time_offset");
  }

private:
  template <typename T, typename IndexType, typename TimeZeroType, typename TimeOffsetType>
  void doTestRankData(const T &rankData,
                      anonymous::FakeParserDataGenerator<IndexType, TimeZeroType, TimeOffsetType> &gen,
                      const Chunker::LoadRange &range) {
    PulseTimeGenerator<IndexType, TimeZeroType> pulseTimes(gen.eventIndex(0), gen.eventTimeZero(), "nanosecond", 0);
...
public:
  // This pair of boilerplate methods prevent the suite being created statically
  // This means the constructor isn't called when running other tests
  static EventParserTestPerformance *createSuite() { return new EventParserTestPerformance(); }
  ...
  void testCompletePerformance() {
    for (size_t i = 0; i < NUM_BANKS; ++i) {
      parser->setEventDataPartitioner(std::make_unique<EventDataPartitioner<int32_t, int64_t, double>>(
          1, PulseTimeGenerator<int32_t, int64_t>{gen.eventIndex(i), gen.eventTimeZero(), "nanosecond", 0}));
      parser->setEventTimeOffsetUnit("microsecond");
      parser->startAsync(event_ids[i].data(), event_time_offsets[i].data(), gen.generateBasicRange(i));
      parser->wait();
    }
  }
  ...
  void testExtractEventsPerformance() {
    for (size_t bank = 0; bank < NUM_BANKS; bank++) {
      ...
      partitioner.partition(rankData, event_ids[bank].data(), event_time_offsets[bank].data(),
