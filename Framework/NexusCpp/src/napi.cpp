...
NXstatus NXclosegroup(NXhandle fid) {
  ...
  if (fileStackDepth(fileStack) == 0) {
    ...
    if (NXsameID(fid, &closeID, &currentID) == NXstatus::OKAY) {
      NXclose(&fid);
...
NXstatus NXputslab(NXhandle fid, const void *data, const int iStart[], const int iSize[]) {
  ...
  if (NXgetinfo64(fid, &rank, iStart64, &iType) != NXstatus::OKAY) {
    return NXstatus::ERROR;
  }
  for (i = 0; i < rank; ++i) {
    iStart64[i] = iStart[i];
...
static int isDataSetOpen(NXhandle hfil) {
   ...
   */
  if (NXgetdataID(hfil, &id) == NXstatus::ERROR) {
    return 0;
  } else {
    return 1;
  }
...
static int isRoot(NXhandle hfil) {
   ...
   */
  if (NXgetgroupID(hfil, &id) == NXstatus::ERROR) {
    return 1;
  } else {
    return 0;
  }
...
static NXstatus stepOneUp(NXhandle hfil, char const *name) {
  ...
  while (NXgetnextentry(hfil, name2, xclass, &datatype) != NXstatus::EOD) {
...
NXstatus NXopengrouppath(NXhandle hfil, CONSTCHAR *path) {
  ...
  } while (pPtr != NULL && status != NXstatus::EOD);
  return NXstatus::OKAY;
}
