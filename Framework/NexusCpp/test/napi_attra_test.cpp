...
NXstatus createAttrs(const NXhandle file) {
  ...
  if (NXputattra(file, "attribute_0d", r4_array, 0, array_dims, NXnumtype::FLOAT32) != NXstatus::NX_OK)
    return NXstatus::NX_ERROR;
  if (NXputattra(file, "attribute_1d", r4_array, 1, array_dims, NXnumtype::FLOAT32) != NXstatus::NX_OK)
    return NXstatus::NX_ERROR;
  if (NXputattra(file, "attribute_2d", r8_array, 2, array_dims, NXnumtype::FLOAT64) != NXstatus::NX_OK)
    return NXstatus::NX_ERROR;

  if (NXputattr(file, "old_style_int_attribute", &i, 1, NXnumtype::INT32) != NXstatus::NX_OK) {
...
int main(int argc, char *argv[]) {
  ...
  NXhandle fileid;
  if (NXopen(filename.c_str(), nx_creation_code, &fileid) != NXstatus::NX_OK) {
  ...
  if (createAttrs(fileid) != NXstatus::NX_OK && nx_creation_code == NXACC_CREATE5) {
  ...
  NXlen = 4;
  if (NXmakedata(fileid, "dataset", NXnumtype::INT32, 1, &NXlen) != NXstatus::NX_OK)
    return 1;
  if (NXopendata(fileid, "dataset") != NXstatus::NX_OK)
    return 1;
  ...
  if (NXopen(filename.c_str(), NXACC_READ, &fileid) != NXstatus::NX_OK)
    return 1;
  ...
  for (level = 0; level < 3; level++) {
    ...
    do {
      ...
      if (attr_status == NXstatus::NX_OK) {
        ...
        // cppcheck-suppress argumentSize
        if (NXgetattrainfo(fileid, name, &NXrank2, NXdims2, &NXtype2) != NXstatus::NX_OK)
          return 1;
        if (NXrank != NXrank2) {
        ...
        if (NXgetattra(fileid, name, data_buffer) != NXstatus::NX_OK) {
        ...
        if (NXrank == 1 && NXtype == NXnumtype::CHAR) {
          ...
          NXlen = sizeof(char_buffer);
          if (NXgetattr(fileid, name, char_buffer, &NXlen, &NXtype) != NXstatus::NX_ERROR)
    ...
    do {
      // cppcheck-suppress argumentSize
      attr_status = NXgetnextattra(fileid, name, &NXrank, NXdims, &NXtype);
      if (attr_status == NXstatus::NX_EOD) {
        fprintf(stderr, "BANG! We've seen no error iterating through array attributes with old api\n");
        break;
      }
    } while (attr_status != NXstatus::NX_ERROR);
  }

  const int result = (NXclose(&fileid) == NXstatus::NX_OK) ? 0 : 1;
