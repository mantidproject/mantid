...
int main() {
  ...
  for (iFile = 0; iFile < nFiles; iFile++) {
    ...
    printf("file %s\n", strFile);
    NXhandle fileid;
    if (NXopen(strFile, access_mode, &fileid) != NXstatus::OKAY) {
    ...
    for (iEntry = 0; iEntry < nEntry; iEntry++) {
      ...
      for (iNXdata = 0; iNXdata < nData; iNXdata++) {
        ...
        for (iData = 0; iData < nData; iData++) {
          ...
          oss3 << "i2_data_" << iData;
          if (NXmakedata(fileid, PSZ(oss3.str()), NXnumtype::INT16, 1, &array_dims[1]) != NXstatus::OKAY) {
          ...
          if (NXputdata(fileid, i2_array) != NXstatus::OKAY) {
