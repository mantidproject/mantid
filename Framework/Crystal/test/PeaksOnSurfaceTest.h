...
public:
  ...
  void test_vertexes_not_square_sided() {
    ...
    alg.setPropertyValue("Vertex1", "0,0,0");
    alg.setPropertyValue("Vertex2", "0,1,0");
    alg.setPropertyValue("Vertex3", "0.5,1.2247,0"); // x^2 + y^2 + z^2 == 1,
                                                     // and is coplanar, but not
                                                     // square sided.
    alg.setPropertyValue("Vertex4", "1,0,0");
    alg.setPropertyValue("OutputWorkspace", "OutWS");
    TS_ASSERT_THROWS(alg.execute(), std::invalid_argument &);
  ...
  Place a point just behind the plane at (0.5, 0.5, 1). It has a radius of 1 so
  it should intersect the plane.
  */
  ...
  Place a point just behind the plane at (0.5, 0.5, 1). It has a radius of 0.999
  so it should just MISS intersecting the plane.
  */
...
Place a point on the plane exactly at 2,0,0, but has a radius of 0.999, and is
therefore outside the surface boundaries.
*/
...
Place a point on the plane exactly at 2,0,0, but has a radius of 1, and is
therefore does cross the surface boundaries.
*/
  ...
  void test_line_intersects_sphere_facility() {
    ...
    const V3D line = lineEnd - lineStart; // Defines a line running horzontal
