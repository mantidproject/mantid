...
public:
  ...
  void test_only_create_faces_for_clusters_corresponding_to_peaks() {
    ...
    TSM_ASSERT_EQUALS("Should have exactly 6 entries in the table. One cluster "
                      "with 6 neighbours. The other cluster should be ignored "
                      "as has no corresponding peak.",
                      6, faces->rowCount());
  ...
  void test_complex_filtering() {
    ...
    TSM_ASSERT_EQUALS("Should have exactly 3+6 entries in the table. One "
                      "cluster with 6 neighbours, another with 3. The other "
                      "cluster should be ignored as has no corresponding peak.",
                      9, faces->rowCount());
  }
  ...
  void test_limit_rows() {
    ...
    TSM_ASSERT_EQUALS("Although there are actually two faces, only one face "
                      "should be identified",
                      outWS->rowCount(), rowMaximumLimit);
  }
};
