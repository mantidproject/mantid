...
public:
  ...
  void test_throw_if_not_experimentinfo_workspace() {
    ...
    TSM_ASSERT_THROWS("Input workspace type is not derived from ExperimentInfo "
                      "or MultipleExperimentInfo, so should throw.",
                      doExecute(wsName), std::invalid_argument &);
    AnalysisDataService::Instance().remove(wsName);
  }
};
