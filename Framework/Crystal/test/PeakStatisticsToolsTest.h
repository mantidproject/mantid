...
namespace {
std::vector<Peak> getPeaksWithIandSigma(const std::vector<double> &intensity, const std::vector<double> &sigma,
                                        const V3D &hkl = V3D(0, 0, 1)) {
  std::vector<Peak> peaks;
  std::transform(intensity.begin(), intensity.end(), sigma.begin(), std::back_inserter(peaks),
...
UniqueReflection getReflectionWithPeaks(const std::vector<double> &intensities, const std::vector<double> &sigmas,
...
UniqueReflectionCollection getUniqueReflectionCollection(double a, const std::string &centering,
                                                         const std::string &pointGroup, double dMin) {
  UnitCell cell(a, a, a);
  PointGroup_sptr pg = PointGroupFactory::Instance().createPointGroup(pointGroup);
  ReflectionCondition_sptr cent = getReflectionConditionBySymbol(centering);

  return UniqueReflectionCollection(cell, std::make_pair(dMin, 100.0), pg, cent);
}
...
public:
  ...
  static PeakStatisticsToolsTest *createSuite() { return new PeakStatisticsToolsTest(); }
  ...
  void test_UniqueReflectionsConstructor() {
    ...
    TSM_ASSERT_EQUALS("Constructed UniqueReflection does not have 0 peaks.", reflection.count(), 0);
    TSM_ASSERT_EQUALS("HKL is not equal to constructor argument in UniqueReflection", reflection.getHKL(), hkl);
  }
  ...
  void test_UniqueReflectionsPeaks() {
    ...
    TSM_ASSERT_EQUALS("UniqueReflection count is not 1 after adding peak.", reflection.count(), 1);
    TSM_ASSERT_EQUALS("UniqueReflection peaks vector size is not 1 after adding peak.", reflection.getPeaks().size(),
                      1);
  }
  ...
  void test_UniqueReflectionsGetIntensitiesAndSigmas() {
    ...
    TSM_ASSERT_EQUALS("Intensity vector from UniqueReflection has wrong size.", intensities.size(), 2);
    TS_ASSERT_EQUALS(intensities[0], 30.0);
    TS_ASSERT_EQUALS(intensities[1], 34.0);

    std::vector<double> sigmas = reflection.getSigmas();
    TSM_ASSERT_EQUALS("Sigma vector from UniqueReflection has wrong size.", sigmas.size(), 2);
  ...
  void test_UniqueReflectionRemoveOutliersFewPeaks() {
    ...
    TSM_ASSERT_EQUALS("Peak was removed as outlier although there's only 1.", outliersRemoved.count(), 1);
    ...
    TSM_ASSERT_EQUALS("Peak was removed as outlier although there's only 2.", outliersRemoved.count(), 2);
  }

  void test_UniqueReflectionRemoveOutliers() {
    UniqueReflection reflection = getReflectionWithPeaks({30.0, 34.0, 32.0, 31.0}, {4.5, 6.5, 10.0, 2.3});

    // standard deviation is 1.70782512765993
    auto cleanReflection = reflection.removeOutliers();
    TSM_ASSERT_EQUALS("UniqueReflection removed outlier although it should not.", cleanReflection.count(), 4);

    cleanReflection = reflection.removeOutliers(2.0);
    TSM_ASSERT_EQUALS("UniqueReflection removed outlier although it should not.", cleanReflection.count(), 4);

    cleanReflection = reflection.removeOutliers(1.0);
    TSM_ASSERT_EQUALS("UniqueReflection did not remove outliers although it should have.", cleanReflection.count(), 2);
  ...
  void test_UniqueReflectionRemoveOutliersWeighted() {
    ...
    TSM_ASSERT_EQUALS("UniqueReflection removed outlier although it should not.", cleanReflection.count(), 3);

    cleanReflection = reflection.removeOutliers(2.0, true);
    TSM_ASSERT_EQUALS("UniqueReflection removed outlier although it should not.", cleanReflection.count(), 2);

    cleanReflection = reflection.removeOutliers(1.0, true);
    TSM_ASSERT_EQUALS("UniqueReflection did not remove outliers although it should have.", cleanReflection.count(), 1);
  ...
  void test_UniqueReflectionSetIntensityAndSigma() {
    ...
    for (auto peak : reflection.getPeaks()) {
      TSM_ASSERT_EQUALS("Incorrect peak intensity after set in UniqueReflection.", peak.getIntensity(), 10.0);
      TSM_ASSERT_EQUALS("Incorrect peak sigma after set in UniqueReflection.", peak.getSigmaIntensity(), 0.1);
    }
  }

  void test_UniqueReflectionCollectionEmpty() {
    UniqueReflectionCollection reflections = getUniqueReflectionCollection(3.0, "P", "m-3m", 1.5);
  ...
  void test_UniqueReflectionCollectionAddObservations() {
    ...
    reflections.addObservations(getPeaksWithIandSigma({1.0, 1.0}, {2.0, 2.0}, V3D(1, 0, 0)));
    ...
    reflections.addObservations(getPeaksWithIandSigma({1.0, 1.0}, {2.0, 2.0}, V3D(0, 5, 0)));
  ...
  void test_UniqueReflectionCollectionReflectionCounts() {
    ...
    reflections.addObservations(getPeaksWithIandSigma({1.0, 1.0}, {2.0, 2.0}, V3D(1, 0, 0)));
    reflections.addObservations(getPeaksWithIandSigma({1.0, 1.0, 2.0}, {2.0, 2.0, 3.0}, V3D(1, 1, 0)));
  ...
  void test_PeaksStatisticsNoObservation() {
    ...
    uniques.insert(std::make_pair(V3D(1, 1, 1), UniqueReflection(V3D(1, 1, 1))));
  ...
  void test_PeaksStatisticsOneObservation() {
    std::map<V3D, UniqueReflection> uniques{{{1, 1, 1}, getReflectionWithPeaks({56.0}, {4.5}, 1.0)}};
    MockUniqueReflectionCollection reflections(uniques);
  ...
  void test_PeaksStatisticsTwoObservationTwoUnique() {
    ...
    std::map<V3D, UniqueReflection> uniques{{{1, 1, 1}, getReflectionWithPeaks({10.0}, {1.0}, 1.0)},
  ...
  void test_PeaksStatisticsTwoObservationOneUnique() {
    std::map<V3D, UniqueReflection> uniques{{{1, 1, 1}, getReflectionWithPeaks({10.0, 20.0}, {0.1, 0.1}, 1.0)}};
    MockUniqueReflectionCollection reflections(uniques);
  ...
  void test_PeaksStatisticsThreeObservationOneUnique() {
    std::map<V3D, UniqueReflection> uniques{
        {{1, 1, 1}, getReflectionWithPeaks({10.0, 20.0, 15.0}, {0.1, 0.1, 0.1}, 1.0)}};
    MockUniqueReflectionCollection reflections(uniques);
