...
namespace Crystal
{
    ...
    void IndexPeakswithSatellites::init()
    {
        ...
        declareProperty(make_unique<PropertyWithValue<double>>
                              ("Tolerance for Satellite", 0.15, mustBePositive, Direction::Input),
        ...
        declareProperty(make_unique<PropertyWithValue<double>>
                              ("MainError", 0.0, Direction::Output),
                              "Gets set with the average HKL indexing error of Main Peaks.");
        
        declareProperty(make_unique<PropertyWithValue<double>>
                              ("SatelliteError", 0.0, Direction::Output),
    ...
    void IndexPeakswithSatellites::exec()
    {
        ...
        for (size_t run_index = 0; run_index < run_numbers.size(); run_index++)
        {
            ...
            original_indexed = IndexingUtils::CalculateMillerIndices(tempUB, q_vectors, tolerance, miller_indices, original_error);
            ...
            {                              // which is usually sufficient
                ...
                num_indexed = IndexingUtils::CalculateMillerIndices(tempUB, q_vectors, tolerance, miller_indices, average_error);
            ...
            for (size_t i = 0; i < n_peaks; i++)
            {
                if (peaks[i].getRunNumber() == run)
                {
                    ...
                    else
                    {
                        ...
                        {
                            if (order == 0) continue; // exclude order 0
                            V3D hkl1(hkl);
        ...
        << main_indexed << " are Main Bragg Peaks, and " << sate_indexed << " are satellite peaks " << '\n';
