...
void PredictPeaks::init() {
  ...
  declareProperty("UseSymmetry", false,
                  "When using PointGroup, this will only generate "
                  "one peak for each point group. ");
  ...
  setPropertySettings("UseSymmetry", makeSet());
...
void PredictPeaks::exec() {
  ...
  for (auto &goniometerMatrix : gonioVec) {
    ...
    bool useSymmetry = getProperty("UseSymmetry");

    if (pointGroupName.empty() || !useSymmetry) {
...
void PredictPeaks::fillPossibleHKLsUsingGenerator(
  ...
  if (!pointGroupName.empty()) {
    ...
    for (auto it = possibleSpaceGroupSymbols.begin();
      ...
      remove_if(spaceGroup.begin(), spaceGroup.end(), isspace);
      std::string rc = refCond->getSymbol();
      std::string pg = pointGroup->getSymbol();
      if (spaceGroup.compare(0, 1, rc) == 0 &&
          spaceGroup.compare(1, pg.length(), pg) == 0) {
        symbolSG = *it;
        break;
      }
    }
