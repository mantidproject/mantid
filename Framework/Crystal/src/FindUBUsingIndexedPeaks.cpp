    ...
    namespace Crystal
    {
        ...
        void FindUBUsingIndexedPeaks::exec() {
            ...
            for (Peak peak : peaks)
            {
                ...
                if (IndexingUtils::ValidIndex(hkl, 1.0) || IndexingUtils::ValidIndex(mnp, 1.0)) // use tolerance == 1 to
            ...
            {                    // from the full list of peaks, and
                    ...
                    for (int run : run_numbers)
                    {
                        ...
                        for (Peak peak : peaks)
                            if (peak.getRunNumber() == run)
                            {
                                V3D hkl(peak.getIntHKL()); // ##### KEEP
                                V3D mnp(peak.getIntMNP());
                                if (IndexingUtils::ValidIndex(hkl, 1.0) || IndexingUtils::ValidIndex(mnp, 1.0))
                                    run_indexed++;
                            }
                        ...
                        {
                            V3D err_sigq = V3D(sigq[0],sigq[1],sigq[2]) * static_cast<double>(num_indexed);
                            errorHKL1 += err_sigq;
                            errorHKL2 += err_sigq;
                            errorHKL3 += err_sigq;
                        }
                ...
                stream << "New UB will index " << num_indexed << " main Peaks with tolerance " << tolerance << " and " << sate_indexed << " Satellite Peaks with tolerance " << satetolerance << " ,out of " << n_peaks << " Peaks \n" ;
                ...
                o_lattice.setErrorModHKL(errorHKL1[0]*ind_count_inv, errorHKL1[1]*ind_count_inv, errorHKL1[2]*ind_count_inv, errorHKL2[0]*ind_count_inv, errorHKL2[1]*ind_count_inv, errorHKL2[2]*ind_count_inv, errorHKL3[0]*ind_count_inv, errorHKL3[1]*ind_count_inv, errorHKL3[2]*ind_count_inv);
        ...
        void FindUBUsingIndexedPeaks::logLattice(OrientedLattice &o_lattice, int &ModDim)
        {
            ...
            for (int i = 1; i < ModDim+1; i++) {
                g_log.notice() << "Modulation Vector " << i << ": " << o_lattice.getModVec(i) << "\n";
                g_log.notice() << "Modulation Vector " << i << " error: " << o_lattice.getVecErr(i)
                << "\n";
            }
        }
    } // namespace Mantid
