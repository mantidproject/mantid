...
namespace Mantid
{
    namespace Crystal
    {
        ...
        void FindUBUsingIndexedPeakswithSatellites::exec() {
            ...
            if (ModDim == 0)
            {
                throw std::runtime_error("No satellite Peaks indexed, run IndexPeakswithSatellites or use FindUBUsingIndexedPeaks");
            }
...
//            g_log.notice() << "Modulation Dimension is: " << ModDim << "\n";
            ...
            if (indexed_count < 4)
            {
                throw std::runtime_error("At least four linearly independent indexed peaks are needed.");
            }
            ...
            {                    // from the full list of peaks, and
                ...
                char logInfo[200];
                sprintf(logInfo,
                        std::string("New UB will index %1d main Peaks out of %1d with tolerance %5.3f")
                        .c_str(),
                ...
                o_lattice.setErrorModHKL(sigq[0], sigq[1], sigq[2], sigq[3], sigq[4], sigq[5], sigq[6], sigq[7], sigq[8]);
