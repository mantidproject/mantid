 ...
 */
CombinedIndexingStats indexPeaks(const std::vector<Peak *> &peaks, DblMatrix ub, const double mainTolerance,
...
void IndexPeaks::init() {
  ...
  // -- inputs --
  this->declareProperty(std::make_unique<WorkspaceProperty<PeaksWorkspace_sptr::element_type>>(Prop::PEAKSWORKSPACE, "",
                                                                                               Direction::InOut),
...
void IndexPeaks::exec() {
  ...
  if (args.commonUB) {
    ...
    std::transform(std::begin(args.workspace->getPeaks()), std::end(args.workspace->getPeaks()),
    ...
    std::for_each(std::begin(allPeaks), std::end(allPeaks),
                  [&peaksPerRun](Peak &peak) { peaksPerRun[peak.getRunNumber()].emplace_back(&peak); });
