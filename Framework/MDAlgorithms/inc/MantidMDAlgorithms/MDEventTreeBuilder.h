...
MDEventTreeBuilder<ND, MDEventType, EventIterator>::doDistributeEvents(
  ...
  if (mdEvents.size() <= m_bc->getSplitThreshold()) {
    ...
    Task tsk{ root,        mdEvents.begin(),        mdEvents.end(), m_mortonMin,
              m_mortonMax, m_bc->getMaxDepth() + 1, 1 };
...
void MDEventTreeBuilder<ND, MDEventType, EventIterator>::distributeEvents(
  ...
  for (size_t i = 0; i < childBoxCount; ++i) {
    ...
    children.emplace_back(RecursionHelper{ { boxEventStart, eventIt },
                                           { boxLower, boxUpper }, newBox });
  }
  ...
  ++tsk.level;
  for (auto &ch : children) {
    Task newTask{ ch.box,                 ch.eventRange.first,
                  ch.eventRange.second,   ch.mortonBounds.first,
                  ch.mortonBounds.second, tsk.maxDepth,
                  tsk.level };
    if (wtp == MASTER &&
