...
/**   Class provides commont interface for various classes performing rebinning
 operations;

 A rebinning class is selected from availible classes which do rebinning
 operations on
 user request and the ability to do the job.

 TODO: A factory should analyse the demands for the job and user requests and
 return the method best suitable for the operations
       but the common interface for the rebinning can be identified and is
 described here.
...
public:
  /** Function identifies the indexes of the sells of the source image that can
   * contribute into the target image
   *  returns number of selected cells and the number of pixels (datapoints,
   * events) contained in these cells which can contribute
   * into the cut.     */
  virtual size_t preselect_cells() = 0;
  /** function takes input multidimensional data points (pixels, events) stored
     in the source data buffer and
     *  rebins these data (adds them) to MD image of the taget workspace;
     * Alternative (USA)vdescription: Identifies the locations of the datapoints
     in the multidimensional grid of the target workspace
  ...
  /** function returns the number of pixels which can contribute into a cut
   * (Number of pixels in selected cells -- become valid after
   *  preselection is done and precelected cells buffer is valid */
  virtual uint64_t getNumPreselectedPixels() const = 0;

  /** Calculates signals and errors of the MD image, obtained as the result of
     one or more rebin_dataset operations
     and (in some implementations) the locations of the points in the final
     MDDatapoints array
