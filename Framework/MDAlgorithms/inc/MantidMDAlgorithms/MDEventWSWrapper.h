...
public:
  ...
  /// initiated
  void addMDData(std::vector<float> &sigErr, std::vector<uint16_t> &runIndex, std::vector<uint32_t> &detId,
...
private:
  ...
  template <size_t nd>
  void addMDDataND(float *sigErr, uint16_t *runIndex, uint32_t *detId, coord_t *Coord, size_t dataSize) const;
  template <size_t nd>
  void addAndTraceMDDataND(float *sig_err, uint16_t *run_index, uint32_t *det_id, coord_t *Coord,
