...
namespace Mantid {
    namespace MDAlgorithms {
         ...
         */
        Integrate3DEvents::Integrate3DEvents(std::vector<std::pair<double, V3D>> const &peak_q_list,DblMatrix const &UBinv, double radius,const bool useOnePercentBackgroundCorrection) : m_UBinv(UBinv), m_radius(radius), maxOrder(0), crossterm(0),  m_useOnePercentBackgroundCorrection(useOnePercentBackgroundCorrection) {
         ...
         * @param   useOnePercentBackgroundCorrection flag if one percent background
         correction should be used.
         */
        Integrate3DEvents::Integrate3DEvents(std::vector<std::pair<double, V3D>> const &peak_q_list,std::vector<V3D> const &hkl_list, std::vector<V3D> const &mnp_list, DblMatrix const &UBinv, DblMatrix const &ModHKL,double radius_m,double radius_s,int MaxO,bool CrossT,const bool useOnePercentBackgroundCorrection) : m_UBinv(UBinv), m_ModHKL(ModHKL), m_radius(radius_m), s_radius(radius_s), maxOrder(MaxO), crossterm (CrossT), m_useOnePercentBackgroundCorrection(useOnePercentBackgroundCorrection) {
            for (size_t it = 0; it != peak_q_list.size(); ++it)
            {
                int64_t hklmnp_key = getHklMnpKey(hkl_list[it][0],hkl_list[it][1],hkl_list[it][2],mnp_list[it][0],mnp_list[it][1],mnp_list[it][2]);
        ...
        std::pair<boost::shared_ptr<const Geometry::PeakShape>, std::tuple<double, double, double>>
        Integrate3DEvents::integrateStrongPeak(const IntegrationParameters &params,const V3D &peak_q, double &inti, double &sigi) {
        ...
        Integrate3DEvents::integrateWeakPeak(const IntegrationParameters &params, PeakShapeEllipsoid_const_sptr shape, const std::tuple<double, double, double> &libPeak, const V3D &center, double &inti, double &sigi) {
        ...
        Integrate3DEvents::ellipseIntegrateEvents(std::vector<Kernel::V3D> E1Vec, V3D const &peak_q, bool specify_size,
        ...
        Mantid::Geometry::PeakShape_const_sptr
        Integrate3DEvents::ellipseIntegrateModEvents(std::vector<Kernel::V3D> E1Vec, V3D const &peak_q, V3D const &hkl, V3D const &mnp, bool specify_size, double peak_radius, double back_inner_radius, double back_outer_radius, std::vector<double> &axes_radii, double &inti, double &sigi)
        {
            inti = 0.0; // default values, in case something
            sigi = 0.0; // is wrong with the peak.
            
            int64_t hkl_key = getHklMnpKey(hkl[0],hkl[1],hkl[2],mnp[0],mnp[1],mnp[2]);
        ...
        void Integrate3DEvents::makeCovarianceMatrix(std::vector<std::pair<double, V3D>> const &events, DblMatrix &matrix, double radius) {
...
//            if (fabs(hkl[0]-boost::math::iround<double>(hkl[0]))<m_radius && fabs(hkl[1]-boost::math::iround<double>(hkl[1]))<m_radius && fabs(hkl[2]-boost::math::iround<double>(hkl[2]))<m_radius)
            {
                int h = boost::math::iround<double>(hkl[0]);
                int k = boost::math::iround<double>(hkl[1]);
                int l = boost::math::iround<double>(hkl[2]);
...
//                        if (fabs(hkl1[0]-boost::math::iround<double>(hkl1[0]))<m_radius && fabs(hkl1[1]-boost::math::iround<double>(hkl1[1]))<m_radius && fabs(hkl1[2]-boost::math::iround<double>(hkl1[2]))<m_radius)
                        {
         ...
         *  and the map key is then formed from those rounded h+-0.5,k+-0.5,l+-0.5 or h,k,l
         *  depending on the offset of satellite peak from main peak.
         *
         *  @param q_vector  The q_vector to be mapped to h,k,l
         */
