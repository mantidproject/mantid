...
*/
template <size_t nd> void MDEventWSWrapper::createEmptyEventWS(const MDWSDescription &description) {

  std::shared_ptr<DataObjects::MDEventWorkspace<DataObjects::MDEvent<nd>, nd>> ws =
  ...
  for (size_t d = 0; d < nd; d++) {
    ...
    if (d < 3 && description.isQ3DMode()) {
      ...
      dim = new Geometry::MDHistoDimension(description.getDimNames()[d], description.getDimIDs()[d], *mdFrame,
                                           Mantid::coord_t(description.getDimMin()[d]),
                                           Mantid::coord_t(description.getDimMax()[d]), nBins);

    } else {
      Mantid::Geometry::GeneralFrame frame(description.getDimNames()[d], description.getDimUnits()[d]);
      dim = new Geometry::MDHistoDimension(description.getDimNames()[d], description.getDimIDs()[d], frame,
                                           Mantid::coord_t(description.getDimMin()[d]),
                                           Mantid::coord_t(description.getDimMax()[d]), nBins);
    }
...
template <> void MDEventWSWrapper::createEmptyEventWS<0>(const MDWSDescription & /*unused*/) {
...
*/
template <size_t nd>
void MDEventWSWrapper::addMDDataND(float *sigErr, uint16_t *runIndex, uint16_t *goniometerIndex, uint32_t *detId,
  ...
  if (pWs) {
    for (size_t i = 0; i < dataSize; i++) {
      pWs->addEvent(DataObjects::MDEvent<nd>(*(sigErr + 2 * i), *(sigErr + 2 * i + 1), *(runIndex + i),
                                             *(goniometerIndex + i), *(detId + i), (Coord + i * nd)));
    }
  } else {
    auto *const pLWs =
        dynamic_cast<DataObjects::MDEventWorkspace<DataObjects::MDLeanEvent<nd>, nd> *>(m_Workspace.get());
    ...
    for (size_t i = 0; i < dataSize; i++) {
      pLWs->addEvent(DataObjects::MDLeanEvent<nd>(*(sigErr + 2 * i), *(sigErr + 2 * i + 1), (Coord + i * nd)));
    }
  }
}
...
template <size_t nd> void MDEventWSWrapper::calcCentroidND() {
  ...
  auto *const pWs =
      dynamic_cast<DataObjects::MDEventWorkspace<DataObjects::MDEvent<nd>, nd> *>(this->m_Workspace.get());
  if (!pWs)
    throw(std::bad_cast());
...
API::IMDEventWorkspace_sptr MDEventWSWrapper::createEmptyMDWS(const MDWSDescription &WSD) {
  ...
  if (WSD.nDimensions() < 1 || WSD.nDimensions() > MAX_N_DIM) {
    std::string ERR = " Number of requested MD dimensions: " + std::to_string(WSD.nDimensions()) +
                      " exceeds maximal number of MD dimensions: " + std::to_string(static_cast<int>(MAX_N_DIM)) +
 ...
 *@param dataSize -- the length of the vector of MD events
 */
void MDEventWSWrapper::addMDData(std::vector<float> &sigErr, std::vector<uint16_t> &runIndex,
  ...
  (this->*(mdEvAddAndForget[m_NDimensions]))(&sigErr[0], &runIndex[0], &goniometerIndex[0], &detId[0], &Coord[0],
                                             dataSize);
}
...
/**constructor */
MDEventWSWrapper::MDEventWSWrapper() : m_NDimensions(0), m_needSplitting(false) {
