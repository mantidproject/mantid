...
*/
template <size_t nd>
void MDEventWSWrapper::addMDDataND(float *sigErr, uint16_t *runIndex, uint32_t *detId, coord_t *Coord,
                                   size_t dataSize) const {

  auto *const pWs = dynamic_cast<DataObjects::MDEventWorkspace<DataObjects::MDEvent<nd>, nd> *>(m_Workspace.get());
  if (pWs) {
    for (size_t i = 0; i < dataSize; i++) {
      pWs->addEvent(DataObjects::MDEvent<nd>(*(sigErr + 2 * i), *(sigErr + 2 * i + 1), *(runIndex + i), *(detId + i),
                                             (Coord + i * nd)));
    }
  } else {
    auto *const pLWs =
