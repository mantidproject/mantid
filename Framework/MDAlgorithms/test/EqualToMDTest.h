...
public:
  ...
  void test_histo_histo() {
    ...
    out = BinaryOperationMDTestHelper::doTest("EqualToMD", "histo_A", "histo_B",
                                              "out");
    TS_ASSERT_DELTA(out->getSignalAt(0), 0.0, 1e-5);
    out = BinaryOperationMDTestHelper::doTest("EqualToMD", "histo_B", "histo_B",
                                              "out");
  ...
  void test_histo_scalar() {
    MDHistoWorkspace_sptr out;
    out = BinaryOperationMDTestHelper::doTest("EqualToMD", "histo_A", "scalar",
                                              "out");
    TS_ASSERT_DELTA(out->getSignalAt(0), 0.0, 1e-5);
    out = BinaryOperationMDTestHelper::doTest("EqualToMD", "scalar", "histo_B",
                                              "out");
    TS_ASSERT_DELTA(out->getSignalAt(0), 1.0, 1e-5);
  }

  void test_event_fails() {
    BinaryOperationMDTestHelper::doTest("EqualToMD", "event_A", "scalar", "out",
                                        false /*fails*/);
    BinaryOperationMDTestHelper::doTest("EqualToMD", "event_A", "event_B",
                                        "out", false /*fails*/);
  }
