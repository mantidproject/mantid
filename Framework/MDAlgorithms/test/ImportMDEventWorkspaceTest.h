...
/*
This builder type provides a convenient way to create and change the contents of
a virtual file of the type expected
by the ImportMDEventWorkspace algorithm.

This type is particularly useful when generating corrupt file contents, as it
allows individual apects of the file contents to be modified independently.
*/
...
private:
  /**
  Helper method runs tests that should throw and invalid argument when the
...
public:
  ...
  void test_mdevent_block_declared_before_dimension_block_throws() {
    // Setup the corrupt file. Notice that the DimensionBlockFlag and the
    // MDEventBlockFlag arguments have been swapped over.
    FileContentsBuilder fileContents;
  ...
  void test_dimension_block_has_corrupted_entries_throws() {
    ...
    std::string dim3 = "b B U\n"; // Ooops, forgot to put in the number of bins
                                  // for this dimension.
  ...
  void test_load_full_mdevents_3d() {
    ...
    const std::string dim3 = "c C U 12\n"; // Ooops, forgot to put in the number
  ...
  void test_ignore_comment_lines() {
    ...
    alg.setPropertyValue("OutputWorkspace", "test_out");
    TS_ASSERT_THROWS_NOTHING(alg.execute());
    // These comment blocks are not being considered if execution completes
    // without throwing
    TS_ASSERT(alg.isExecuted());
