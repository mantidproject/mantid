...
public:
  void test_that_MDHistoWorkspace_throws_exception() {
    ...
    auto mdHistoWorkspace = Mantid::DataObjects::MDEventsTestHelper::makeFakeMDHistoWorkspace(1.0, 1, 10);
    auto emode = Mantid::Kernel::DeltaEMode::Direct;
    Mantid::MDAlgorithms::DisplayNormalizationSetter setter;
    // Act + Assert
    TSM_ASSERT_THROWS("Should throw for MDHistoWorkspace", setter(mdHistoWorkspace, eventWorkspace, isQ, emode),
                      std::runtime_error &);
  }
  ...
  void test_that_direct_energy_mode_creates_a_volume_normalization() {
    ...
    // Assert
    TSM_ASSERT_EQUALS("Should be set to volume normalization", mdEventWorkspace->displayNormalization(),
                      Mantid::API::VolumeNormalization);
    TSM_ASSERT_EQUALS("Should be set to volume normalization", mdEventWorkspace->displayNormalizationHisto(),
                      Mantid::API::VolumeNormalization);
  }
  ...
  void test_that_indirect_energy_mode_with_an_input_event_workspace_creates_no_normalization() {
    ...
    setter(mdEventWorkspace, eventWorkspace, isQ, emode);
    // Assert
    TSM_ASSERT_EQUALS("Should be set to no normalization", mdEventWorkspace->displayNormalization(),
                      Mantid::API::VolumeNormalization);
    TSM_ASSERT_EQUALS("Should be set to no normalization", mdEventWorkspace->displayNormalizationHisto(),
  ...
  void test_that_non_Q_creates_volume_normalization() {
    ...
    setter(mdEventWorkspace, histoWorkspace, isQ, emode);
    // Assert
    TSM_ASSERT_EQUALS("Should be set to number volume normalization", mdEventWorkspace->displayNormalization(),
                      Mantid::API::VolumeNormalization);
    TSM_ASSERT_EQUALS("Should be set to number volume normalization", mdEventWorkspace->displayNormalizationHisto(),
                      Mantid::API::VolumeNormalization);
  }
};
