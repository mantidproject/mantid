...
std::tuple<MDTransfQ3DTestHelper, MDWSDescription> createTestTransform(const double efixed,
                                                                       const DeltaEMode::Type emode) {
  auto indirectInelasticWS = WorkspaceCreationHelper::createProcessedWorkspaceWithCylComplexInstrument(4, 10, true);
  ...
  wsDescription.m_PreprDetTable = WorkspaceCreationHelper::buildPreprocessedDetectorsWorkspace(indirectInelasticWS);
...
public:
  ...
  void testElasticModeWithLorentzCorrection() {
    ...
    auto elasticTestWS = WorkspaceCreationHelper::createProcessedWorkspaceWithCylComplexInstrument(4, 10, true);
    ...
    TSM_ASSERT_THROWS("No detectors yet defined, so should throw run time error: ", Q3DTransf.initialize(wsDescription),
                      const std::runtime_error &)

    // let's preprocess detectors positions to go any further
    wsDescription.m_PreprDetTable = WorkspaceCreationHelper::buildPreprocessedDetectorsWorkspace(elasticTestWS);
    // let's set 2Theta=0 for simplicity and violate const correctness for
    // testing purposes here
    auto &TwoTheta = const_cast<std::vector<double> &>(wsDescription.m_PreprDetTable->getColVector<double>("TwoTheta"));
    for (double &i : TwoTheta) {
      i = 0;
    }

    TSM_ASSERT_THROWS_NOTHING("should initialize properly: ", Q3DTransf.initialize(wsDescription))
    TSM_ASSERT("Should not be lorentz corrected by default ", !Q3DTransf.getLorentzCorr())
    ...
    TSM_ASSERT(" Matrix coordinates should be in range so function return true: ",
  ...
  void testDirectInelasticMode() {
    ...
    std::tie(q3dTransform, wsDescription) = createTestTransform(efixed, DeltaEMode::Direct);
    ...
    // assert
    const auto detDir{wsDescription.m_PreprDetTable->getColVector<V3D>("DetDirections")[0]};
    const double kfinal{sqrt((efixed - deltaE) / Mantid::PhysicalConstants::E_mev_toNeutronWavenumberSq)};
    TS_ASSERT_DELTA(-detDir[0] * kfinal, qOmega[0], 1e-05)
    TS_ASSERT_DELTA(-detDir[1] * kfinal, qOmega[1], 1e-05)
    const double kfixed{sqrt(efixed / Mantid::PhysicalConstants::E_mev_toNeutronWavenumberSq)};
  ...
  void testIndirectInelasticMode() {
    ...
    MDTransfQ3DTestHelper q3dTransform;
    MDWSDescription wsDescription;
    std::tie(q3dTransform, wsDescription) = createTestTransform(efixed, DeltaEMode::Indirect);
    ...
    const double ki{sqrt((efixed + deltaE) / Mantid::PhysicalConstants::E_mev_toNeutronWavenumberSq)};
