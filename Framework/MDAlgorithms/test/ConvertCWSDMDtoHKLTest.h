...
public:
  ...
  void test_convertToHKL() {
    ...
    TS_ASSERT_THROWS_NOTHING(alg.setPropertyValue("InputWorkspace", m_qsampleWS->getName()));
    TS_ASSERT_THROWS_NOTHING(alg.setPropertyValue("UBMatrix", "1.0, 0.5, 0., -0.2, 2.0, 0.4, 0., 1.11, 3.9"));
    ...
    IMDEventWorkspace_sptr hklws =
        std::dynamic_pointer_cast<IMDEventWorkspace>(AnalysisDataService::Instance().retrieve("HKLMD"));
    TS_ASSERT(hklws);

    size_t numevents = hklws->getNEvents();
    TS_ASSERT_EQUALS(numevents, 100);

    TS_ASSERT_EQUALS(hklws->getSpecialCoordinateSystem(), Mantid::Kernel::SpecialCoordinateSystem::HKL);
    // Test the frame type
...
private:
  ...
  void createMDEW() {
    ...
    FrameworkManager::Instance().exec("CreateMDWorkspace", 20, "Dimensions", "3", "EventType", "MDEvent", "Extents",
    ...
    TS_ASSERT_THROWS_NOTHING(m_qsampleWS = AnalysisDataService::Instance().retrieveWS<IMDEventWorkspace>("MDEWS"));
    ...
    mess << num_events / 2 << ", " << peakcentre_x << ", " << peakcentre_y << ", " << peakcentre_z << ", " << radius;
    std::cout << mess.str() << "\n";
    ...
    mess2 << num_events / 2 << ", " << peakcentre_x << ", " << peakcentre_y << ", " << peakcentre_z << ", "
          << radius * 0.25;
    std::cout << mess2.str() << "\n";
    FrameworkManager::Instance().exec("FakeMDEventData", 4, "InputWorkspace", "MDEWS", "PeakParams",
                                      mess2.str().c_str());
    ...
    m_qsampleWS->setCoordinateSystem(Mantid::Kernel::SpecialCoordinateSystem::QSample);
