...
public:
  ...
  IMDEventWorkspace_sptr create_workspace(const std::vector<double> &extents, const std::string &name) {
    // ---- empty MDEW ----
    ...
    std::string frames = Mantid::Geometry::QSample::QSampleName + "," + Mantid::Geometry::QSample::QSampleName + "," +
    ...
    IMDEventWorkspace_sptr out =
        std::dynamic_pointer_cast<IMDEventWorkspace>(AnalysisDataService::Instance().retrieve(name));
    TS_ASSERT(out);
  ...
  void do_test(const std::string &name, std::vector<double> extents) {
    ...
    for (size_t i = 0; i < 3; i++) {
      ...
      if (lamMin != lamMax) {
        ...
        TS_ASSERT_LESS_THAN_EQUALS((Qx * lamMin - extents[0]) * (Qx * lamMin - extents[1]),
                                   1e-7 * (extents[1] - extents[0]))
        TS_ASSERT_LESS_THAN_EQUALS((Qx * lamMax - extents[0]) * (Qx * lamMax - extents[1]),
                                   1e-7 * (extents[1] - extents[0]))
      }
    }
