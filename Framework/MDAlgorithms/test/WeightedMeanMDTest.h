...
private:
  ...
  void do_test_workspace_input_types(const IMDWorkspace_sptr &a, const IMDWorkspace_sptr &b) {
    WeightedMeanMD alg;
    alg.initialize();
...
public:
  ...
  */
  MDHistoWorkspace_sptr create_md_histo_workspace(const std::vector<double> &s, const std::vector<double> &e,
                                                  const std::vector<double> &x, const std::string &name) {
  ...
  void test_compare_to_matrix_weightedmean() {
    ...
    MatrixWorkspace_sptr weighted_mean_matrix =
        run_matrix_weighed_mean(a_matrix_ws, b_matrix_ws, "weighted_mean_matrix");
