...
private:
  ...
  MatrixWorkspace_sptr create_matrix_workspace(const std::vector<double> &s,
...
public:
  ...
  void test_executes_1D() {
    ...
    // Create an MDHisto workspace with signal=1, dimensionality=1, bins=10,
    // max=10, error=1, dimname="A"
    MDHistoWorkspace_sptr a = MDEventsTestHelper::makeFakeMDHistoWorkspace(
        1.0, 1, 10, 10.0, 1.0, "A");
    // Create an MDHisto workspace with signal=1, dimensionality=1, bins=10,
    // max=10, error=1, dimname="A"
    ...
    // Errors = sqrt(0.5) as errors are stored squared and since error is the
    // harmoic mean of e1 and e2. i.e. sqrt(e1 * e2 / (e1 + e2))
    double expectedErrorResults[10] = {0.7071, 0.7071, 0.7071, 0.7071, 0.7071,
  ...
  /**
  This is more of an integration test, but it's quite useful because it executes
  both WeightedMean and WeightedMeanMD on the same data and
  compares the results.
  */
  MDHistoWorkspace_sptr create_md_histo_workspace(const std::vector<double> &s,
                                                  const std::vector<double> &e,
                                                  const std::vector<double> &x,
                                                  const std::string &name) {
  ...
  void test_compare_to_matrix_weightedmean() {
    ...
    MDHistoWorkspace_sptr a_md_histo =
        create_md_histo_workspace(s1, e1, x, "a_md_histo");
    MDHistoWorkspace_sptr b_md_histo =
        create_md_histo_workspace(s2, e2, x, "b_md_histo");

    // Create the equivalent MatrixWorkspaces
    MatrixWorkspace_sptr a_matrix_ws =
        create_matrix_workspace(s1, e1, x, "a_matrix_workspace");
    MatrixWorkspace_sptr b_matrix_ws =
        create_matrix_workspace(s2, e2, x, "b_matrix_workspace");

    // Calculate the weighted mean for the matrix workspace.
    MatrixWorkspace_sptr weighted_mean_matrix = run_matrix_weighed_mean(
        a_matrix_ws, b_matrix_ws, "weighted_mean_matrix");
    ...
    for (size_t j = 0; j < s1.size(); ++j) {
      TS_ASSERT_DELTA(weighted_mean_matrix->readY(0)[j],
                      weighted_mean_md->signalAt(j), 0.0001);
      TS_ASSERT_DELTA(std::pow(weighted_mean_matrix->readE(0)[j], 2),
                      weighted_mean_md->errorSquaredAt(j), 0.0001);
    }
