...
private:
  bool compareWithFullTreeRecursive(FullTree3D3L::PtDistr &distr, size_t id,
    ...
    if (nd->isLeaf()) {
      ...
      for (int i = 0; i < 8; ++i)
        res &=
            compareWithFullTreeRecursive(distr, FullTree3D3L::getChildIdx(id, i), nd->getChild(i));
    }
  ...
  bool compareNode(Mantid::API::IMDNode *nd1, Mantid::API::IMDNode *nd2) {
    if(nd1->getNumChildren() != nd2->getNumChildren()) return false;
    size_t nd = nd1->getNumDims();
    if(nd != nd2->getNumDims()) return false;
    ...
    for(size_t i = 0; i < nd; ++i) {
      ...
      if(ext.outside(center2[i])) return false;
      ext = nd2->getExtents(i);
      if(ext.outside(center1[i])) return false;
    }
    if(nd1->getNPoints() != nd2->getNPoints()) return false;
    return true;
  }
