...
private:
  ...
  void doTestRecursionDepth(const bool bTakeDepthFromInput,
                            const int maxDepth = 0) {
    SliceMD alg;
    alg.initialize();

    IMDEventWorkspace_sptr in_ws =
        MDEventsTestHelper::makeAnyMDEW<MDEvent<3>, 3>(10, 0.0, 10.0, 1);
    ...
    TS_ASSERT_THROWS_NOTHING(
        alg.setProperty("TakeMaxRecursionDepthFromInput", bTakeDepthFromInput));
    ...
    */
    ...
    if (bTakeDepthFromInput) {
      TSM_ASSERT_EQUALS("MaxRecusionDepth property should NOT be enabled",
                        false, p->getSettings()->isEnabled(&alg));
      TSM_ASSERT_EQUALS(
          "Should have passed the maxium depth onto the ouput workspace.",
          in_ws->getBoxController()->getMaxDepth(),
          out->getBoxController()->getMaxDepth());
    } else {
      TSM_ASSERT_EQUALS("MaxRecusionDepth property should be enabled", true,
                        p->getSettings()->isEnabled(&alg));
...
public:
  ...
  void execute_slice(const std::string &name1, const std::string &name2,
    ...
    TSM_ASSERT_EQUALS(
        "Should default to TakeMaxRecursionDepthFromInput == true",
        in_ws->getBoxController()->getMaxDepth(),
        out->getBoxController()->getMaxDepth());
    ...
    MDEventsTestHelper::checkAndDeleteFile(
        alg.getPropertyValue("OutputFilename"));
  ...
  */
  ...
  void test_exec_3D_lean() {
    do_test_exec<MDLeanEvent<3>, 3>("Axis0,2.0,8.0, 3", "Axis1,2.0,8.0, 3",
                                    "Axis2,2.0,8.0, 3", "",
                                    6 * 6 * 6 /*# of events*/, 3 /*dims*/);
  }

  void test_exec_3D_lean_scrambled() {
    do_test_exec<MDLeanEvent<3>, 3>("Axis2,2.0,8.0, 3", "Axis1,2.0,8.0, 3",
                                    "Axis0,2.0,8.0, 3", "",
                                    6 * 6 * 6 /*# of events*/, 3 /*dims*/);
  }
  ...
  void test_exec_2D_lean() {
    do_test_exec<MDLeanEvent<3>, 3>("Axis0,2.0,8.0, 3", "Axis1,2.0,8.0, 3", "",
                                    "", 6 * 6 * 10 /*# of events*/, 2 /*dims*/);
  }

  void test_exec_1D_lean() {
    do_test_exec<MDLeanEvent<3>, 3>("Axis0,2.0,8.0, 3", "", "", "",
                                    6 * 10 * 10 /*# of events*/, 1 /*dims*/);
  }

  void test_exec_3D() {
    do_test_exec<MDEvent<3>, 3>("Axis0,2.0,8.0, 3", "Axis1,2.0,8.0, 3",
                                "Axis2,2.0,8.0, 3", "",
                                6 * 6 * 6 /*# of events*/, 3 /*dims*/);
  }

  void test_exec_4D_to_4D() {
    do_test_exec<MDEvent<4>, 4>("Axis0,2.0,8.0, 3", "Axis1,2.0,8.0, 3",
                                "Axis2,2.0,8.0, 3", "Axis3,2.0,8.0, 3",
                                6 * 6 * 6 * 6 /*# of events*/, 4 /*dims*/);
  }

  void test_exec_4D_to_1D() {
    do_test_exec<MDEvent<4>, 4>("Axis0,2.0,8.0, 3", "", "", "",
                                6 * 10 * 10 * 10 /*# of events*/, 1 /*dims*/);
  }
  ...
  */
  void do_test_transform(double lengthX, double lengthY, double lengthZ,
    ...
    if (false) {
      ...
      IAlgorithm_sptr saver = FrameworkManager::Instance().exec(
          "SaveMD", 4, "InputWorkspace", "SliceMDTest_ws", "Filename",
          "SliceMDTest_ws_rotated.nxs");
    }
  ...
  void test_exec_with_transform() {
    do_test_transform(10, 10, 10, 1000 /*# of events*/);
  }
