...
private:
  void checkFRCorrection(std::string wsName,  double expectedValuePeak1, double expectedValuePeak2) {
    ...
    TS_ASSERT_THROWS_NOTHING(
        out =boost::dynamic_pointer_cast<Mantid::DataObjects::MDHistoWorkspace>(
  ...
  void createMDWorkspace(const std::string &wsName, const std::string &peakParams, const double sampleLog) {
    ...
    alg.execute();
    // Add log
    Mantid::API::IMDEventWorkspace_sptr out = boost::dynamic_pointer_cast<Mantid::API::IMDEventWorkspace>(
