...
public:
  ...
  void test_histo_histo() {
    ...
    out = BinaryOperationMDTestHelper::doTest("OrMD", "histo_A", "histo_zero",
                                              "out");
    TS_ASSERT_DELTA(out->getSignalAt(0), 1.0, 1e-5);
    out = BinaryOperationMDTestHelper::doTest("OrMD", "histo_zero",
                                              "histo_zero", "out");
  ...
  void test_histo_histo_masked() {
    MDHistoWorkspace_sptr out;
    out = BinaryOperationMDTestHelper::doTest("OrMD", "histo_A", "histo_masked",
                                              "out");
    TS_ASSERT_DELTA(out->getSignalAt(0), 1.0, 1e-5);
    out = BinaryOperationMDTestHelper::doTest("OrMD", "histo_masked",
                                              "histo_masked", "out");
    TS_ASSERT_DELTA(out->getSignalAt(0), 0.0, 1e-5);
  }

  void test_scalar_or_event_fails() {
    BinaryOperationMDTestHelper::doTest("OrMD", "histo_A", "scalar", "out",
                                        false /*fails*/);
    BinaryOperationMDTestHelper::doTest("OrMD", "event_A", "event_B", "out",
                                        false /*fails*/);
  }
};
