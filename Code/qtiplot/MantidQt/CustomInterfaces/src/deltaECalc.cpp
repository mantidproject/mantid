  ...
  m_sets(userSettings), pythonCalc()
{
  ...
  if (m_sets.ckSumSpecs->isChecked() || inputFiles.size() == 1)
  ...
  {// we have a base name to use, distingish the multiple output files using numbers
	for( int i = 0, end = inputFiles.size(); ; )
	{
	  ...
	  QString WS = createProcessingScript("'"+inputFiles[i]+"',", 
	    insertNumber(m_sets.leNameSPE->text().toStdString(), i+1));
...
QString deltaECalc::getNormalization() const
{
  ...
  if (m_sets.cbMonitors->currentText().isEmpty()) std::invalid_argument("cbMonitors");
  return m_sets.cbMonitors->currentText();
}
...
std::string deltaECalc::insertNumber(const std::string &filename, const int number) const
{
  ...
  return f.getBaseName() + boost::lexical_cast<std::string>(number) +
    f.getExtension();
}
...
{	
  ...
  QString rebinStr = m_sets.leELow->text()+","+m_sets.leEWidth->text()+","+m_sets.leEHigh->text();
...
void deltaECalc::createNormalizationStatmens(QString &newScr)
{
  ...
  {// inserdt dummy values that Python will accept 
    newScr.replace("|GUI_SET_WBVLow|", "-1");
    newScr.replace("|GUI_SET_WBVHigh|", "-1");
  }
}
