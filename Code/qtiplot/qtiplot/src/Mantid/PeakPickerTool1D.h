...
*/
class PeakPickerTool1D : public QwtPlotPicker, public PlotToolInterface
{
	Q_OBJECT
	public:
    /// Constructor
		PeakPickerTool1D(Graph *graph, ApplicationWindow *app);
    /// Destructor
    ~PeakPickerTool1D();
    ...
    PeakRangeMarker1D* marker()const{return m_range;}
    /// Workspace name
 ...
 *   draw(..) method.
 */
class PeakRangeMarker1D: public QwtPlotItem
{
public:
  // Peak parameter type
  struct PeakParams
  ...
  // Constructor
  PeakRangeMarker1D():m_width(),m_current(-1),m_width_set(false),m_resetting(false){}
