...
unix|macx|win32-msvc2008|win32-msvc2010 { 
    unix|macx|win32-msvc2010:DEFINES += HAVE_STDINT_H
    win32-msvc2008:exists("C:\Program Files\Microsoft Visual Studio 10.0\VC\include\stdint.h")|exists("C:\Program Files (x86)\Microsoft Visual Studio 10.0\VC\include\stdint.h"):DEFINES += HAVE_STDINT_H
}
...
win32:build_pass:CONFIG(debug, debug|release):# Make sure we don't link to the non-debug runtime

# QMAKE_LFLAGS_DEBUG += /NODEFAULTLIB:msvcrt.lib  #NOT SURE IF I SHOULD REMOVE THIS FLAG!!!!

MANTIDPATH = ../../Mantid
MANTIDLIBPATH = $$MANTIDPATH/release
build_pass:CONFIG(debug, debug|release):MANTIDLIBPATH = $$MANTIDPATH/debug
...
unix:!macx { 
    ...
    else:INCLUDEPATH += /usr/include/qwt/
    exists(/usr/include/qwtplot3d-qt4):INCLUDEPATH += /usr/include/qwtplot3d-qt4/
    else:INCLUDEPATH += /usr/include/qwtplot3d/
}
else { 
...
unix { 
    ...
    macx { 
        ...
        exists(/usr/lib64/libqwt-qt4.so):LIBS += -lqwt-qt4
        else { 
            exists(/usr/lib/libqwt-qt4.so):LIBS += -lqwt-qt4
            else:LIBS += -lqwt
        }
        exists(/usr/lib64/libqwtplot3d-qt4.so):LIBS += -lqwtplot3d-qt4
        else { 
            exists(/usr/lib/libqwtplot3d-qt4.so):LIBS += -lqwtplot3d-qt4
            else:LIBS += -lqwtplot3d
        }
...
win32 { 
    ...
    build_pass:CONFIG(debug, debug|release):# Just looks at the place where Visual Studio will put a debug build
    LIBPATH += ../../Mantid/debug
    else:# Look in the right place for both Scons and Visual Studio builds
...
contains(SCRIPTING_LANGS, Python) { 
    ...
    HEADERS += src/PythonSystemHeader.h \
        src/PythonScript.h \
        src/PythonScripting.h
    SOURCES += src/PythonScript.cpp \
