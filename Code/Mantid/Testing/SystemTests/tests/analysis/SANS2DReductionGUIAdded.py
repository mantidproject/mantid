...
class SANS2DReductionGUIAddedEventFiles(sansgui.SANS2DGUIReduction):
    def runTest(self):
        self.initialization()
        ...
        # add files (SAMPLE and CAN)
        import SANSadd2
        #Save the added files as an Event file and enable the overlay. 
        SANSadd2.add_runs(('22048','22048'),'SANS2D', '.nxs', rawTypes=('.add','.raw','.s*'), lowMem=False, saveAsEvent = True, isOverlay = False)
        SANSadd2.add_runs(('22023','22023'),'SANS2D', '.nxs', rawTypes=('.add','.raw','.s*'), lowMem=False, saveAsEvent = True, isOverlay = False)
