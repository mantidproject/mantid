...
public:
    ...
    void testSaveAndLoad()
    {
        SaveAscii save;
        save.initialize();
        save.setPropertyValue("Filename","LoadSaveAsciiTestFile.dat");
        save.setPropertyValue("Workspace","LoadSaveAsciiWS_0");
        TS_ASSERT_THROWS_NOTHING(save.execute());

        LoadAscii load;
        load.initialize();
        load.setPropertyValue("Filename","LoadSaveAsciiTestFile.dat");
        load.setPropertyValue("Workspace","LoadSaveAsciiWS_1");
        TS_ASSERT_THROWS_NOTHING(load.execute());

        Workspace2D_sptr ws = boost::dynamic_pointer_cast<Workspace2D>(AnalysisDataService::Instance().retrieve("LoadSaveAsciiWS_1"));
