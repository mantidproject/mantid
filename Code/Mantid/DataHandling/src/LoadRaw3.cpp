...
void LoadRaw3::init()
{
  ...
  declareProperty("LoadMonitors","Include",new ListValidator(m_monitorOptions),"Use this option to control the loading of monitors.\n"
...
void LoadRaw3::exec()
{
  ...
  {	  
	  ...
	  {
		  fclose(file);
		  throw std::runtime_error("All the selected spectra are monitors and Exclude monitors option selected "); 
	  }
  ...
  for (int period = 0; period < m_numberOfPeriods; ++period)
  {
    ...
    int counter = 0;int monitorCount=0;int counter1=0;
	...
	if (period == 0)
    {
	  ...
	  {	 if(monitorWorkspace)runLoadLog(monitorWorkspace);
		  runLoadInstrument(localWorkspace);
		  runLoadMappingTable(localWorkspace);
		  if(monitorWorkspace) runLoadInstrument(monitorWorkspace);
		  if(monitorWorkspace) runLoadMappingTable(monitorWorkspace);
		  if(monitorWorkspace)monitorWorkspace->getSample()->setProtonCharge(isisRaw->rpb.r_gd_prtn_chrg);
	  }
    ...
    {	if(bLoadlogFiles)
		{	runLoadLog(localWorkspace, period + 1);
			if(monitorWorkspace)runLoadLog(monitorWorkspace, period + 1);
		}
	}
    ...
    {	
		...
		{    	
			if(normalwsSpecs>0)
			{	 if(monitorwsSpecs>0)setWorkspaceProperty(monitorWorkspace,monitorWSGrp,period,true);
			setWorkspaceProperty(localWorkspace,sptrWSGrp,period,false);
			}
			else setWorkspaceProperty(localWorkspace,sptrWSGrp,period,false);
		} 
		else
		{ setWorkspaceProperty(localWorkspace,sptrWSGrp,period,false);
		}
 ...
 */
void LoadRaw3::setWorkspaceProperty(DataObjects::Workspace2D_sptr wsPtr,WorkspaceGroup_sptr wsGrpSptr,const int period,bool bmonitors)
{
	std::string wsName ;
	std::string outws;
 ...
 */
void LoadRaw3::calculateWorkspacesizes(const std::vector<int>& monitorSpecList,const int total_specs,int& normalwsSpecs,int & monitorwsSpecs)
{		
	if(!m_interval && !m_bmspeclist)
	{  normalwsSpecs=total_specs-monitorSpecList.size();
		monitorwsSpecs=monitorSpecList.size();
		g_log.debug()<<"normalwsSpecs   when m_interval  & m_bmspeclist are  false is  "<<normalwsSpecs<<"  monitorwsSpecs is "<< monitorwsSpecs<<std::endl;
	}
	...
	{	
		...
		if(m_interval)
		{
			...
			g_log.debug()<<"normalwsSpecs when  m_interval true is  "<<normalwsSpecs<<"  monitorwsSpecs is "<< monitorwsSpecs<<std::endl;
		}
		if(m_bmspeclist)
		{			
			if(m_interval)
			{
				...
				{	g_log.debug()<<"normalwsSpecs is "<<normalwsSpecs<<"  monitorwsSpecs is "<< monitorwsSpecs<<std::endl;
				}
				else
 ...
 */
void LoadRaw3::getmonitorSpectrumList(DataObjects::Workspace2D_sptr localWorkspace,std::vector<int>& monitorSpecList)
{
	...
	{  
		...
		if(itr!=newVec.end())  newVec.erase(itr);
...
{
	DataObjects::Workspace2D_sptr monitorWorkspace=boost::dynamic_pointer_cast<DataObjects::Workspace2D>(
		  WorkspaceFactory::Instance().create("Workspace2D",nVectors,lengthIn,lengthIn-1));
...
/** This method separates/excludes monitors from output workspace and creates a separate workspace for monitors
  ...
  */
void LoadRaw3::separateOrexcludeMonitors(DataObjects::Workspace2D_sptr localWorkspace,bool bincludeMonitors,bool bexcludeMonitors,bool bseparateMonitors)
{
	std::vector<boost::shared_ptr<MantidVec> > timeChannelsVec;
	std::vector<int> monitorSpecList;
