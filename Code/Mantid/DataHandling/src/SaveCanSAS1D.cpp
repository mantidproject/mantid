...
namespace Mantid
{
	namespace DataHandling
	{
		...
		void SaveCanSAS1D::init()
		{
			...
			declareProperty("Radiation Source", "Spallation Neutron Source", new Kernel::ListValidator(radiation_source));
		}
		...
		*/
		...
		{
			Poco::XML::Element*sasRootElem=
				mDoc->createElement("SASroot");
		...
		*/
		...
		{
			Poco::XML::Element* sasEntryElem=
				mDoc->createElement("SASentry");
		...
		*/
		...
		{
			Poco::XML::Element* titleElem=
				mDoc->createElement("Title");
			throwException(titleElem,"SaveCanSAS1D::createTitleElement","Title");
			Poco::XML::Text* titletext=
				mDoc->createTextNode(m_workspace->getTitle());
		...
		*/
		...
		{
			Poco::XML::Element* runElem=
				mDoc->createElement("Run");
			throwException(runElem,"SaveCanSAS1D::createRunElement","Run");
			Poco::XML::Text* runtext=
				mDoc->createTextNode(m_workspace->getName());
		...
		*/
		...
		{			
			Poco::XML::Element* sasDataElem=
				mDoc->createElement("SASdata");
			...
			for (int i=0;i<m_workspace->getNumberHistograms();++i)
			{
				...
				{
					Poco::XML::Element* iDataElem=
						mDoc->createElement("Idata");
		...
		*/
		void SaveCanSAS1D::createSASInstrument(Poco::XML::Element* parent)
		{
			...
			Poco::XML::Text* radiationText=
				mDoc->createTextNode(radiation_source);
			throwException(radiationText,"SaveCanSAS1D::createSASInstrument","radiation");
			radiationElem->appendChild(radiationText);
			
			Poco::XML::Element* sasCollimationElem=
				mDoc->createElement("SAScollimation");
			...
			Poco::XML::Element* sasDetectorElem=
				mDoc->createElement("SASdetector");
			...
			Poco::XML::Element* detectornameElem=
				mDoc->createElement("name");
			...
			Poco::XML::Text* dectornameText=
				mDoc->createTextNode(detectorName);
			throwException(dectornameText,"SaveCanSAS1D::createSASInstrument","name");
			detectornameElem->appendChild(dectornameText);
			
			Poco::XML::Element* detectorSDDElem=
				mDoc->createElement("SDD");
			...
			sdd<<distance;
			Poco::XML::Text* detectorSDDText=
				mDoc->createTextNode(sdd.str());
		...
		*/
		...
		{
			Poco::XML::Element* sasNoteElem=
				mDoc->createElement("SASnote");
			throwException(sasNoteElem,"SaveCanSAS1D::createSASnote","SASnote");
			parent->appendChild(sasNoteElem);
			Poco::XML::Text* noteText=
				mDoc->createTextNode("Mantid generated CanSAS1d XML");
		 ...
		 */
		void SaveCanSAS1D::throwException(Poco::XML::Element* elem,const std::string & place,const std::string & objectName)
		{
			if(!elem)
			{
