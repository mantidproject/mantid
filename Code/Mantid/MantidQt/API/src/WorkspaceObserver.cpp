...
namespace MantidQt
{
  namespace API
  {
     ...
     */
    void WorkspaceObserver::observeDelete(bool turnOn)
    {
      if( turnOn && !m_del_observed) // Turning it on
      {
      ...
      else if( !turnOn && m_del_observed ) // Turning it off
      {
