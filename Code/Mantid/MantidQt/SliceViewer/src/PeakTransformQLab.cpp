...
namespace MantidQt
{
  ...
  PeakTransformQLab::PeakTransformQLab(const std::string& xPlotLabel, const std::string& yPlotLabel) : PeakTransform(xPlotLabel, yPlotLabel, regex("^Q_lab_x.*$"), regex("^Q_lab_y.*$"), regex("^Q_lab_z.*$")  )
  {
  }

  PeakTransformQLab::PeakTransformQLab(const PeakTransformQLab& other): PeakTransform(other)
  {
  }

  PeakTransformQLab& PeakTransformQLab::operator=(const PeakTransformQLab & other)
  {
    if(this != &other)
    {
  ...
  */
  Mantid::Kernel::V3D PeakTransformQLab::transformPeak(const Mantid::API::IPeak& peak) const
  {
   ...
   */
  Mantid::API::SpecialCoordinateSystem PeakTransformQLab::getCoordinateSystem() const
  {
