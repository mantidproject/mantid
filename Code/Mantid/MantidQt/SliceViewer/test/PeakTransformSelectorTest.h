...
public:
  ...
  { 
    ...
    EXPECT_CALL(*pWrongFactory, createTransform(_, _)).WillOnce(Throw(PeakTransformException())); // Will throw when invoked
    ...
    TSM_ASSERT("Should not have selected the wrong factory", boost::dynamic_pointer_cast<MockPeakTransformFactoryB >(selectedFactory) == NULL);
    TSM_ASSERT("Should have selected the right factory", boost::dynamic_pointer_cast<MockPeakTransformFactoryA >(selectedFactory) != NULL);
