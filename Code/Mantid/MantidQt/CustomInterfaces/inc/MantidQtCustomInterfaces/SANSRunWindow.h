...
private:
  ...
  QString m_sample_no;
  /// List of all validators is searched through before a reduction begins. Where there is a problem focus goes to the widget linked to a validator whose tab is also stored in the pair. Disabling a validator QLabel disables checking that validator
  std::map<QLabel * const, std::pair<QWidget *, QWidget *> > m_validators;
