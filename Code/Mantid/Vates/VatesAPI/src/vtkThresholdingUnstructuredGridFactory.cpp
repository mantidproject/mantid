  ...
  */
  ...
  vtkDataSet* vtkThresholdingUnstructuredGridFactory<TimeMapper>::create() const
  {
    ...
    { 
      ...
      if (!hws)
      {
        return createFromAnyIMDWorkspace4D(m_timestep);
      }
      else
      {
  ...
  template<typename TimeMapper>
  vtkDataSet* vtkThresholdingUnstructuredGridFactory<TimeMapper>::createFromAnyIMDWorkspace4D(const int timestep) const
  {
    validate();
      ...
      for (int i = 0; i < nBinsX; i++)
      {
        for (int j = 0; j < nBinsY; j++)
        {
          for (int k = 0; k < nBinsZ; k++)
          {
            ...
            signalScalar = m_workspace->getSignalNormalizedAt(i, j, k, m_timeMapper(timestep));
