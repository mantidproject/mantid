...
namespace Mantid{
    namespace MDDataObjects{
      /// path-through class which provide transformation of the data from pixel buffer format to data fields format;
      template<class T>
      class MDDataPoint{
      public:
        MDDataPoint(const char * buf, const std::vector<unsigned int> &dim_field_lengths,const std::vector<unsigned int> &img_field_length):
            pBuffer(buf)
        {
           this->field_lengths.reserve(dim_field_lengths.size()+img_field_length.size());
        }
        ~MDDataPoint(void);
        T getDataField(unsigned int nField,size_t n_point)const{return (T)(*(pBuffer+n_point*MDPointStride+field_locations[nField]));}
      private:
        const char *pBuffer;
        unsigned int MDPointStride;
        std::vector<unsigned int> field_locations;
