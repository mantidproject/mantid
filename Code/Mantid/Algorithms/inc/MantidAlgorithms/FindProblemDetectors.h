    ...
    */
    ...
    public:
      ...
      FindProblemDetectors() :
          API::Algorithm(),
          m_Low(0.1), m_High(1.5), m_MinSpec(0), m_MaxSpec(UNSETINT), m_PercentDone(0.0)
// STEVES Allow users to change the failure codes?  m_liveValue(???), double m_deadValue(???)
    ...
    private:
      ...
      API::MatrixWorkspace_sptr getSolidAngles(API::MatrixWorkspace_sptr input);
      ...
      /// Numbers to represent failing and passing detectors
      enum
      {
