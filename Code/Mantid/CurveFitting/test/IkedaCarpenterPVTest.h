...
public:
  ...
  void testAgainstGEM_data()
  {
    ...
    alg2.setPropertyValue("WorkspaceIndex","1");
    alg2.setPropertyValue("StartX","6446");
    alg2.setPropertyValue("EndX","7427");
    ...
    icpv->getParameter("I") = 71967;
    icpv->getParameter("Alpha0") = 0.734079; // numbers for IC params from GEM_ISIS.irf file generated by Laurant 11/2003
    icpv->getParameter("Alpha1") = 2.067249;
    icpv->getParameter("Beta0") = 32.017204;
    icpv->getParameter("Kappa") = 48.734158;
    ...
    icpv->getParameter("SigmaSquared") =10200;
    icpv->getParameter("X0") = 6960;
    icpv->getParameter("Eta") = 0;
    icpv->tie("Eta", "0");
    icpv->getParameter("Gamma") = 0;
    icpv->tie("Gamma", "0");
    ...
    // note this test will never produce good fit because it assumes no background
    double dummy = alg2.getProperty("Output Chi^2/DoF");
    TS_ASSERT_DELTA( dummy, 1.76,0.01);

    TS_ASSERT_DELTA( icpv->getParameter("I"), 71907 ,0.1);
    TS_ASSERT_DELTA( icpv->getParameter("Alpha0"), 0.734079 ,0.1);
    TS_ASSERT_DELTA( icpv->getParameter("Alpha1"), 2.067249 ,0.1);
    TS_ASSERT_DELTA( icpv->getParameter("SigmaSquared"), 10188 ,1);
    TS_ASSERT_DELTA( icpv->getParameter("X0"), 6959.98 ,0.1);
    TS_ASSERT_DELTA( icpv->getParameter("Eta"), 0 ,0.1);
    TS_ASSERT_DELTA( icpv->getParameter("Gamma"), 0 ,0.1);
    TS_ASSERT_DELTA( bk->getParameter("A0"), 87.27 ,0.1);
