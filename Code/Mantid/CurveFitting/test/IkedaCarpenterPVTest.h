...
public:
  ...
  void testAgainstGEM_data()
  {
    ...
    Mantid::DataObjects::Workspace2D_sptr wsToPass = boost::dynamic_pointer_cast<Mantid::DataObjects::Workspace2D>(AnalysisDataService::Instance().retrieve("GEM38370"));
    ...
    // Set general Fit parameters
    alg2.setPropertyValue("InputWorkspace", "GEM38370");
    alg2.setPropertyValue("WorkspaceIndex","1");
    ...
    icpv->setParameter("Alpha0",0.734079); // numbers for IC params from GEM_ISIS.irf file generated by Laurant 11/2003
    icpv->setParameter("Alpha1",2.067249);
    icpv->setParameter("Beta0",32.017204);
    icpv->setParameter("Kappa",48.734158);

    icpv->tie("Alpha0", "0.734079"); // numbers for IC params from GEM_ISIS.irf file generated by Laurant 11/2003
    icpv->tie("Alpha1", "2.067249");
    icpv->tie("Beta0", "32.017204");
    icpv->tie("Kappa", "48.734158");
  ...
  void xtestAgainstGEM_data2()
  {
    ...
    LoadNexus nexus;
    nexus.initialize();
    nexus.setPropertyValue("FileName", "../../../../Test/Nexus/focussedGEM38370_TOF.nxs");
    nexus.setPropertyValue("OutputWorkspace", "GEM38370nexus");
    TS_ASSERT_THROWS_NOTHING(nexus.execute());
    TS_ASSERT( nexus.isExecuted() );
    ...
    //Mantid::DataObjects::Workspace2D_sptr wsToPass2 = boost::dynamic_pointer_cast<Mantid::DataObjects::Workspace2D>(AnalysisDataService::Instance().retrieve("GEM38370nexus"));
    //CheckWorkspacesMatch check;
    //check.setPropertyValue("Workspace1", wsToPass);
    //check.setPropertyValue("Workspace2", wsToPass2);
    //TS_ASSERT_THROWS_NOTHING(check.execute());
    //TS_ASSERT( check.isExecuted() );

    //std::string ffff = check.getProperty("Result");
    //std::cout << "check " << ffff << std::endl;
    ...
    // note this test will never produce good fit because it assumes no background
    double dummy = alg2.getProperty("Output Chi^2/DoF");
    TS_ASSERT_DELTA( dummy, 0.96,0.01);
