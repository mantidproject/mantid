...
/** Calculate the GenericFitting function.
 *  @param in A pointer ot the input active function parameters
 *  @param out A pointer to the output GenericFitting function buffer. The buffer must be large enough to receive nData double values.
 *        The GenericFitting procedure will try to minimise Sum(out[i]^2)
 *  @param xValues The array of nData x-values.
 *  @param nData The size of the GenericFitted data.
 */
  void GenericFit::function(const double* in, double* out, const double* xValues, const int& nData)
  {
  ...
  /** Calculate derivates of GenericFitting function
  *
  * @param in Input GenericFitting parameter values
  * @param out Derivatives
  * @param xValues X values for data points
  * @param nData Number of data points
  */
  void GenericFit::functionDeriv(const double* in, Jacobian* out, const double* xValues, const int& nData)
  {
    if (in) m_function->updateActive(in);
