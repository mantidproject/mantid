...
DECLARE_ALGORITHM(GaussianLinearBG)
...
Logger& GaussianLinearBG::g_log = Logger::get("Gaussian");
...
void GaussianLinearBG::exec()
{
  ...
  l_data.p = 5; // number of gaussian parameters to fit 
  ...
  if ( l_data.n < l_data.p )
  {
    ...
    throw std::runtime_error("Insufficient bins selected (must be at least 5)");
  }
  ...
  gsl_vector_set(initFuncArg, 4, getProperty("bg1"));
  ...
  g_log.information() << "Attempt to fit: bg0+(bg1*x)+height*exp(-0.5*((x-peakCentre)/sigma)^2)\n" <<
    "Iteration = " << iter << "\n" <<
    "Status = " << reportOfFit << "\n" <<
    "Chi^2/DoF = " << chi*chi / dof << "\n" <<
    "bg0 = " << std::setprecision(10) << gsl_vector_get(s->x,0) <<
    "; bg1 = " << std::setprecision(10) << gsl_vector_get(s->x,4) <<
  ...
  setProperty("bg1", gsl_vector_get(s->x,4));
