...
def elwinPlot(eq1,eq2,elf):
    plotElwinWorkspace(eq1,'Elastic Intensity', True)
    plotElwinWorkspace(eq2,'log(Elastic Intensity)', True)
    plotElwinWorkspace(elf,'Temperature(K)')

#Plot a workspace generated by Elwin
def plotElwinWorkspace(ws, axisTitle, setScale=False):
    nBins = mtd[ws].blocksize()
    lastX = mtd[ws].readX(0)[nBins-1]

    nhist = mtd[ws].getNumberHistograms()
    ...
    layer.setAxisTitle(mp.Layer.Bottom, axisTitle)
