...
The algorithm requires a `MDWorkspace <http://www.mantidproject.org/MDWorkspace>`__ of SCD data in
...
The output will be a `Workspace2D <http://www.mantidproject.org/Workspace2D>`__ with one spectrum per
peak, where:
