...
namespace Mantid
{
  namespace NeXus
  {
    ...
    NexusFileIO::NexusFileIO() :
        m_nexuscompression(NX_COMP_LZW), m_progress(0)
    {
    }
    ...
    //
    bool NexusFileIO::writeNxNote(const std::string& noteName, const std::string& author,
        const std::string& date, const std::string& description, const std::string& pairValues) const
    {
    ...
    int NexusFileIO::writeNexusProcessedData2D(const API::MatrixWorkspace_const_sptr& localworkspace,
        const bool& uniformSpectra, const std::vector<int>& spec, const char * group_name,
        bool write2Ddata) const
    {
      ...
      int dims_array[2] =
      { static_cast<int>(nSpect), static_cast<int>(nSpectBins) };
      ...
      if (write2Ddata)
      {
        ...
        {
          int s = spec[i];
          NXputslab(fileID, reinterpret_cast<void*>(const_cast<double*>(&(localworkspace->readY(s)[0]))),
              start, asize);
          start[0]++;
        }
        if (m_progress != 0)
        ...
        NXputattr(fileID, "axes", reinterpret_cast<void*>(const_cast<char*>(axesNames.c_str())),
        ...
        NXputattr(fileID, "units", reinterpret_cast<void*>(const_cast<char*>(yUnits.c_str())),
            static_cast<int>(yUnits.size()), NX_CHAR);
        NXputattr(fileID, "unit_label", reinterpret_cast<void*>(const_cast<char*>(yUnitLabel.c_str())),
        ...
        {
          int s = spec[i];
          NXputslab(fileID, reinterpret_cast<void*>(const_cast<double*>(&(localworkspace->readE(s)[0]))),
              start, asize);
          start[0]++;
        }
        if (m_progress != 0)
        ...
        {
          RebinnedOutput_const_sptr rebin_workspace = boost::dynamic_pointer_cast<const RebinnedOutput>(
              localworkspace);
          name = "frac_area";
          NXcompmakedata(fileID, name.c_str(), NX_FLOAT64, 2, dims_array, m_nexuscompression, asize);
          NXopendata(fileID, name.c_str());
          ...
          for (size_t i = 0; i < nSpect; i++)
          {
            int s = spec[i];
            NXputslab(fileID,
                reinterpret_cast<void*>(const_cast<double*>(&(rebin_workspace->readF(s)[0]))), start,
                asize);
            start[0]++;
          }
          if (m_progress != 0)
      ...
      else
      {
        ...
        for (size_t i = 0; i < nSpect; i++)
        {
          NXputslab(fileID, reinterpret_cast<void*>(const_cast<double*>(&(localworkspace->readX(i)[0]))),
              start, asize);
          start[0]++;
        }
      }
      std::string dist = (localworkspace->isDistribution()) ? "1" : "0";
      NXputattr(fileID, "distribution", reinterpret_cast<void*>(const_cast<char*>(dist.c_str())), 2,
          NX_CHAR);
      NXputattr(fileID, "units", reinterpret_cast<void*>(const_cast<char*>(xLabel.c_str())),
          static_cast<int>(xLabel.size()), NX_CHAR);

      auto label = boost::dynamic_pointer_cast<Mantid::Kernel::Units::Label>(xAxis->unit());
      if (label)
      {
        NXputattr(fileID, "caption",
            reinterpret_cast<void*>(const_cast<char*>(label->caption().c_str())),
            static_cast<int>(label->caption().size()), NX_CHAR);
        auto unitLbl = label->label();
        NXputattr(fileID, "label", reinterpret_cast<void*>(const_cast<char*>(unitLbl.ascii().c_str())),
      ...
      if (!sAxis->isText())
      {
        ...
        NXputattr(fileID, "units", reinterpret_cast<void*>(const_cast<char*>(sLabel.c_str())),
        ...
        const char * group_name) const
    {
      ...
      boost::shared_ptr<const TableWorkspace> tableworkspace = boost::dynamic_pointer_cast<
          const TableWorkspace>(itableworkspace);
      boost::shared_ptr<const PeaksWorkspace> peakworkspace = boost::dynamic_pointer_cast<
          const PeaksWorkspace>(itableworkspace);
      ...
      for (size_t i = 0; i < itableworkspace->columnCount(); i++)
      {
        ...
        // write out title
        NXopendata(fileID, str.c_str());
        NXputattr(fileID, "name", reinterpret_cast<void*>(const_cast<char*>(col->name().c_str())),
    ...
    int NexusFileIO::writeNexusProcessedDataEventCombined(
        const DataObjects::EventWorkspace_const_sptr& ws, std::vector<int64_t> & indices, double * tofs,
        float * weights, float * errorSquareds, int64_t * pulsetimes, bool compress) const
    {
    ...
    void NexusFileIO::NXwritedata(const char * name, int datatype, int rank, int * dims_array,
     ...
     */
    template<class T>
    void NexusFileIO::writeEventListData(std::vector<T> events, bool writeTOF, bool writePulsetime,
        bool writeWeight, bool writeError) const
    {
    ...
    int NexusFileIO::writeEventList(const DataObjects::EventList & el, std::string group_name) const
    {
      ...
      NXputattr(fileID, "sort_type", reinterpret_cast<void*>(const_cast<char*>(sortType.c_str())),
          static_cast<int>(sortType.size()), NX_CHAR);

      // Save an attribute with the type of each event.
      NXputattr(fileID, "event_type", reinterpret_cast<void*>(const_cast<char*>(eventType.c_str())),
    ...
    bool NexusFileIO::writeNexusBinMasking(API::MatrixWorkspace_const_sptr ws) const
    {
      ...
      NXputattr(fileID, "description", reinterpret_cast<void*>(const_cast<char*>(description.c_str())),
    ...
    template<>
    std::string NexusFileIO::logValueType<double>() const
    {
      return "double";
    }

    template<>
    std::string NexusFileIO::logValueType<int>() const
    {
      return "int";
    }

    template<>
    std::string NexusFileIO::logValueType<bool>() const
    {
      return "bool";
    }
