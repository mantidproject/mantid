...
public:
  ...
  // Formatted output to a stream - presumably cout.  (Mostly for debugging purposes)
  ...
  // If the object does not contain the requested type, then the accessor functions
...
{
public:
  JSONCopyException( const std::string & msg)
    : JSONException( msg) { }
};
...
{
public:
  JSONAssignmentException( const std::string & msg)
    : JSONException( msg) { }
};
