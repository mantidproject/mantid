...
JSONValue::JSONValue (const JSONValue &v)
{
  ...
  case JSONValue::STRING:
    ...
    {
      delete mp_string;  // Make sure we don't leak memory in the event of a failure
...
JSONValue &JSONValue::operator=( const JSONValue &v)
{
  ...
  // our own, we need to make sure we don't stomp on any of the existing pointers
  ...
  case JSONValue::STRING:
    ...
    {
      delete tpString;  // Make sure we don't leak memory in the event of a failure
...
// Its job is to delete memory holding the current value (in cases  where it is a
// pointer, ie: strings, arrays & objects);
// It assumed this will be called after the call to getValue() succeeds, but before
...
void initFromStream( JSONObject &obj, istream &istr)
{
  ...
  while (nextChar != '}')  // process the stream
  {
    ...
    istr >> nextChar;
    // nextChar is guaranteed to be either a comma, close brace or close bracket.
    //(If it was anything else, initValueFromStream() would have thrown an exception.)
    // A bracket is an error, a brace means the object is done (and will be checked at
    // the start of the while loop) and a comma needs to be thrown out (along with any
...
void initArrayFromStream( JSONArray &arr, istream &istr)
{
  ...
  while (nextChar != ']')  // process the stream
  {
    ...
    // nextChar is guaranteed to be either a comma, close brace or close bracket.
    //(If it was anything else, initValueFromStream() would have thrown an exception.)
    // A brace is an error, a bracket means the array is done (and will be checked at
    // the start of the while loop) and a comma needs to be thrown out (along with any
...
// and JSONArray are both valid values, and this function may call initFromStream()
// and initArrayFromStream().
// Function expects the stream to be pointing at the first character in the value
JSONValue initValueFromStream( istream &istr)
{
  JSONValue value;
  ...
  {
    // Now it gets a little trickier.  It's either a number or the special values
    ...
    std::transform(val.begin(), val.end(), val.begin(), ::tolower);  // convert to lower case for easy comparing
    if (val == "false")
      value = JSONValue(false);
    else if (val == "true")
      value = JSONValue(true);
    else if (val == "null")
      value = JSONValue();  // actually, this assignment isn't necessary - value is already null
    else
    {
  ...
  {
    string message = "Improperly terminated key/value pair.  Expected comma or closing brace.  Received: ";
    message += nextChar;
    message.append("\n");
    char tempBuf[64];
...
void skipWhiteSpace( istream &istr)
{
  ...
  char next;
  istr >> next;  // take advantage of the fact that >> uses whitespace as a field separator
  if (! istr.eof())
  {
    // If eof() is set, it means we didn't find any tokens after the whitespace.  In this case
...
string readUntilCloseChar( istream &istr)
{
  ...
  while ( (next != ',') && (next != '}') && (next != ']'))
  {
    ...
    if ( (value.size() > 0) || (!isspace(next)) )  // don't add white space to the start of the value string
    {
      value += next;
    }
...
{
  // Prints keys/value pairs.  One pair per line  (Does not print opening or closing braces...)
