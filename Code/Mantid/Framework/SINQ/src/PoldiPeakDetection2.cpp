...
void PoldiPeakDetection2::init()
{
	...
	declareProperty("PeakDetectionThreshold", PeakDetectionThreshold,
	...
	// Data
	declareProperty(new WorkspaceProperty<DataObjects::TableWorkspace>("OutputWorkspace", "", Direction::Output),
...
void PoldiPeakDetection2::exec()
{
	...
	g_log.information() << "                 nb_d_channel = " << this->nb_d_channel << std::endl;


	DataObjects::TableWorkspace_sptr ws_sample_logs = this->getProperty("OutputWorkspace");
	...
	for (size_t ipk = 0; ipk < nb_d_channel; ipk ++){table_dead_wires[ipk]=true;}
	...
	g_log.information() << "_Poldi peak detection  : imax = " << imax << std::endl;
	double vmax = vmax0;
	g_log.information() << "                         vmax = " << vmax << std::endl;
	...
	for (size_t i = 0; i<nb_d_channel; i++){Y2[i] = 0;}

	//peak detection while there is intensity higher than 20% of the max intensity
	while((imax>0) && (vmax > PeakDetectionThreshold*vmax0)){
		count ++; //one peak detected
		g_log.information() << "_Poldi peak detection, search for peak " << count << std::endl;

		//indices and values for the fwhm detection
		size_t ifwhm_min = imax; size_t ifwhm_max = imax;
		double vfwhm_min = vmax; double vfwhm_max = vmax;
		// fwhm detection
		...
		g_log.debug() << "_Poldi peak before   " << center << "\t" << sigma << "\t" << height << std::endl;
		g_log.debug() << "_Poldi peak xmin/max " << X[ipeak_min-1] << "\t" << X[ipeak_max+1] << std::endl;
		double doFit = doFitGaussianPeak(ws_auto_corr, 0, center, sigma, height, X[ipeak_min-2*i_delta_peak], X[ipeak_max+2*i_delta_peak]);
		if(!doFit){
			g_log.error() << "_Poldi peak after    : fit failed"  << std::endl;
		}
		g_log.debug() << "_Poldi peak after    " << center << "\t" << sigma << "\t" << height << std::endl;

		// the information are stored in the ws, fit or not fit...
		TableRow t0 = outputws->appendRow();
		t0 << center << height << 2.35*sigma << int(ipeak_min) << int(imax) << int(ipeak_max) ;
	...
	// keep only the used wires in the correlated ws, should correspond to the sinulated peak at the end
 ...
 *
  ...
  */
bool PoldiPeakDetection2::doFitGaussianPeak(DataObjects::Workspace2D_sptr dataws, int workspaceindex,
