...
void PoldiLoadChopperSlits::init() {
  ...
  declareProperty(new WorkspaceProperty<ITableWorkspace>("PoldiChopperSlits",
                                                         "", Direction::Output),
                  "The output Tableworkspace"
                  "with columns containing key summary information about the "
                  "PoldiDeadWires.");

  declareProperty("nbLoadedSlits", 0, "nb of loaded chopper slits",
                  Direction::Output);
}
...
void PoldiLoadChopperSlits::exec() {
  ...
  try {
    ...
    boost::shared_ptr<const Mantid::Geometry::IComponent> comp =
        localWorkspace->getInstrument()->getComponentByName("chopper");
    ...
    if (bank) {
      ...
      for (unsigned int it = 0; it < children.size(); ++it) {
        ...
        if (tempWire.size() > 0) {
          ...
          ewLine++;
          g_log.debug() << "_poldi : slits " << ewLine << " at  " << val
                        << std::endl;
      ...
      g_log.information() << "_poldi : chopper slits loaded (nb:" << ewLine
                          << ")" << std::endl;
