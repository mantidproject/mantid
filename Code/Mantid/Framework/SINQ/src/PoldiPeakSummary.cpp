...
namespace Poldi
{
  ...
  const std::string PoldiPeakSummary::summary() const { return "The algorithm takes a table with peaks from POLDI analysis algorithms and creates a summary table.";}
  ...
  {
    declareProperty(new WorkspaceProperty<TableWorkspace>("InputWorkspace","",Direction::Input), "Input workspace containing a table with peaks from a POLDI fit routine.");
    declareProperty(new WorkspaceProperty<TableWorkspace>("OutputWorkspace","",Direction::Output), "Output table workspace that contains ");
  }
  ...
  void PoldiPeakSummary::exec()
  {
      ...
      PoldiPeakCollection_sptr peaks = boost::make_shared<PoldiPeakCollection>(poldiPeakTableWorkspace);
  ...
  {
      TableWorkspace_sptr peakResultWorkspace = boost::dynamic_pointer_cast<TableWorkspace>(WorkspaceFactory::Instance().createTable());
