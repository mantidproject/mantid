...
/** Detects peaks in supplied vector
  *
  * This method returns a list of iterators that point to peak positions in the range
  * defined by the arguments (start and end iterators). While the actual peak search is
  * performed by PoldiPeakSearch::findPeaksRecursive, this method does some book keeping
  * such as configuring borders of the vector and after searching, reducing the peak list
  * according to the limit specified by MaximumPeakNumber.
  *
  * @param begin :: Iterator that points to the start of the vector to be searched.
  * @param end :: End iterator.
  * @return List of iterators which indicate peak positions with respect to the input range.
  */
...
/** Actual recursive peak search method
  *
  * The maximum of the vector specified by the two iterators supplied as parameters is designated
  * a peak in the given range and appended to a list. Then, the method is executed on the two sub-ranges
  * of the vector [begin, max - m] and [max + m, end], where m is the user supplied value of MinimumPeakSeparation.
  * The results of these two sub-searches are merged into the list, which is then returned.
  *
  * @param begin :: Starting iterator of currently searched vector.
  * @param end :: End iterator.
  * @return List of iterators which designate peaks in this range and all sub-ranges.
  */
...
/** Returns starting iterator for "left" half of sub-range
  *
  * This method "cleans" the begin-iterator for use in a sub-range during the recursion. Without this,
...
/** Returns end iterator for "right" half of sub-range
  *
  * This method "cleans" the end-iterator for use in a sub-range during the recursion. Without this,
...
/** Maps peak position iterators from one vector to another
  *
  * This method is required because the actual peak search is better performed in the summed neighbor data (see PoldiPeakSearch::getNeighborSums)
  * and the detected peak positions have to be mapped to the original correlation spectrum (shifted by 1).
  *
  * @param peakPositions :: List with peak position iterators.
  * @param baseDataStart :: Starting iterator of the vector in which the peak search was performed.
  * @param originalDataStart :: Starting iterator of the vector with the original correlation data.
  * @returns List with peak positions mapped to the original correlation count data.
  */
...
/** Sets error of workspace to specified value
  *
  * Since an estimation of the error is calculated from background counts, this value is assigned to the workspace via this method.
  *
  * @param correlationWorkspace :: Workspace containing the correlation spectrum on which the peak search was performed.
...
/** Retrieves a vector with all counts that belong to the background
  *
  * In this method, a vector is assembled which contains all count data that is considered to be background.
  * Whether a point is considered background depends on its distance to the given peak positions.
  *
...
/** Checks if the distance of a given point is smaller than the minimum peak separation
  *
  * The method is required for background extraction.
  *
  * @param peakPositions :: Peak positions.
  * @param point :: Point in the spectrum to check.
  * @return Boolean value, true if point is not closer than MinimumPeakSeparation to any peakPosition, false otherwise.
  */
...
/** Returns the minimum height a peak should have.
  *
  * This method is used when no user estimate of minimum required peak height is given. It is set to background
  * plus 3 times Sn.
  *
