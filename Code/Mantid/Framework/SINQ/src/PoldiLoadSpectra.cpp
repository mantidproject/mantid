...
void PoldiLoadSpectra::init()
{
	...
	declareProperty(new WorkspaceProperty<ITableWorkspace>("PoldiSpectra","",Direction::Output),
			"The output Tableworkspace"
			"with columns containing key summary information about the Poldi spectra.");


	declareProperty("nbSpectraLoaded", 0, "nb of loaded chopper slits", Direction::Output);
...
void PoldiLoadSpectra::exec()
{
	...
	try
	{
		...
		boost::shared_ptr<const Mantid::Geometry::IComponent> comp = localWorkspace->getInstrument()->getComponentByName("sourceSp");
		...
		if (bank)
		{
			...
			for (unsigned int it = 0; it < children.size(); ++it)
			{
				...
				ewLine++;
				g_log.debug() << "_poldi : spectra " <<  ewLine << " lmbda  "<< lambda << " : " << intensity << std::endl;
				TableRow t = outputws->appendRow();
				t << lambda << intensity ;
			}
			g_log.information() << "_poldi : spectra loaded (nb:" << ewLine << ")" <<  std::endl;
