...
/** Extracts the source spectrum from an Instrument
  *
  * The spectrum is extracted from an instrument pointer and stored internally as an
  * Interpolation object. In case the spectrum is not present or the Interpolation-object
  * can not be constructed properly, the method throws std::runtime_error.
  *
  * @param poldiInstrument :: Pointer to valid POLDI instrument with configured spectrum.
  */
void PoldiSourceSpectrum::setSpectrumFromInstrument(Instrument_const_sptr poldiInstrument)
{
...
/** Returns the "source" component from the instrument.
  *
  * Tries to extract the source from the given instrument. If it's not present, the method throws
  * std::runtime_error.
  *
  * @param poldiInstrument :: Instrument with valid POLDI definition
  * @return Shared pointer to source component
  */
IComponent_const_sptr PoldiSourceSpectrum::getSourceComponent(Instrument_const_sptr poldiInstrument)
{
...
/** Extracts WavelengthDistribution parameter from source component, given the parameter map of the corresponding instrument.
  *
  * This method extracts the wavelength spectrum of the source and returns the corresponding parameter. If the parameter
  * is not found, the method throws std::runtime_error.
  *
  * @param source :: Shared pointer to source component
  * @param instrumentParameterMap :: Shared pointer to instrument's parameter map
...
/** Sets the spectrum given a parameter
  *
  * This methods actually sets the spectrum from a given fitting parameter that contains a lookup table. If there is no lookup table,
  * the method throws std::runtime_error.
  *
  * @param spectrumParameter :: Shared pointer to fitting parameter with lookup table
  */
