...
PoldiDGrid::PoldiDGrid(boost::shared_ptr<PoldiAbstractDetector> detector, boost::shared_ptr<PoldiAbstractChopper> chopper, double deltaT, std::pair<double, double> wavelengthRange) :
    m_detector(detector),
    m_chopper(chopper),
    m_deltaT(deltaT),
...
void PoldiDGrid::setDetector(boost::shared_ptr<PoldiAbstractDetector> newDetector)
{
    m_detector = newDetector;
}

void PoldiDGrid::setChopper(boost::shared_ptr<PoldiAbstractChopper> newChopper)
{
    m_chopper = newChopper;
}

void PoldiDGrid::setDeltaT(double newDeltaT)
{
    m_deltaT = newDeltaT;
}
...
std::pair<int, int> PoldiDGrid::calculateDRange()
{
    ...
    return std::make_pair(static_cast<int>(Conversions::qToD(qLimits.second) / m_deltaD), static_cast<int>(Conversions::qToD(qLimits.first) / m_deltaD));
}
...
double PoldiDGrid::calculateDeltaD()
{
    ...
    return Conversions::TOFtoD(m_deltaT, m_chopper->distanceFromSample() + m_detector->distanceFromSample(centralElement), sin(m_detector->twoTheta(centralElement) / 2.0));
}

/** Generates an equidistant grid of d-values with a given step size. The result depends on the assigned detector, chopper, wavelength and timing.
  *
  */
void PoldiDGrid::createGrid()
{
    ...
    if(m_wavelengthRange.first <= 0.0 || m_wavelengthRange.second <= 0.0 || m_wavelengthRange.first >= m_wavelengthRange.second) {
