...
PoldiRemoveDeadWires::PoldiRemoveDeadWires() :
    		m_filename(), m_runDeadWires(true), m_runAutoDetectDW(true), m_defautDWThreshold(0.5),
    		m_numberOfSpectra(0), m_channelsPerSpectrum(0)
{}
...
void PoldiRemoveDeadWires::init()
{
	...
	declareProperty("AutoRemoveBadWires", true, "Auto detect and remove the potential bas wires");

	// defaut threshold used to detect dead wires
	double defautDWThreshold = 0.5;
	declareProperty("BadWiresThreshold", defautDWThreshold,
	...
	declareProperty("nbExcludedWires", 0, "nb of excluded wires", Direction::Output);
	// output information about auto-detected dead wires
	declareProperty("nbAuteDeadWires", 0, "nb of auto-detect dead wires", Direction::Output);
...
//	double autoDeadWiresThreshold = 1-0.4;
	...
	while(checkContinue){
		...
		if(!localWorkspace.get()->hasMaskedBins(minPos)){
			...
			while(localWorkspace.get()->hasMaskedBins(left) && left>0){left--;}
		...
		if(!localWorkspace.get()->hasMaskedBins(minPos)){
			...
			//find the first used wires on the left
			while(localWorkspace.get()->hasMaskedBins(right) && right<this->m_numberOfSpectra){right++;}
