...
int main( int argc, char *argv[] ) {
    std::string output_filename = "TEST-PSISINQTest.xml";  
    // Look for an argument giving the suite name (not starting with -) and change the output filename to use it.  
     if (argc > 1) { 
         if (argv[1][0] != '-') { 
            output_filename = "TEST-PSISINQTest." + std::string(argv[1]) + ".xml"; 
        } } 
    // Create the output XML file 
    std::ofstream ofstr( output_filename.c_str() );
    CxxTest::XUnitPrinter tmp(ofstr);
    CxxTest::RealWorldDescription::_worldName = "PSISINQTest";
