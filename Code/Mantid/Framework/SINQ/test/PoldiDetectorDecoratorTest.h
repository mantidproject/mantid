...
public:
    ...
    void testInitialization()
    {
        ...
        decorator.setDecoratedDetector(boost::shared_ptr<PoldiAbstractDetector>(0));
    ...
    void testForwardMethods()
    {
        ...
        EXPECT_CALL(*m_detector, twoTheta(_))
                .WillOnce(Return(1.5));
        TS_ASSERT_EQUALS(decorator.twoTheta(0), 1.5);

        EXPECT_CALL(*m_detector, distanceFromSample(_))
                .WillOnce(Return(1999.9));
        TS_ASSERT_EQUALS(decorator.distanceFromSample(0), 1999.9);

        EXPECT_CALL(*m_detector, elementCount())
                .WillOnce(Return(400));
        TS_ASSERT_EQUALS(decorator.elementCount(), 400);

        EXPECT_CALL(*m_detector, centralElement())
                .WillOnce(Return(199));
