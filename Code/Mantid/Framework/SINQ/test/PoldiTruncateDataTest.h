...
public:
    ...
    void testSetTimeBinWidthFromWorkspace()
    {
        ...
        TS_ASSERT_THROWS(truncate.setTimeBinWidthFromWorkspace(invalidSpectra), std::invalid_argument);

        // matrix workspace with one bin
        MatrixWorkspace_sptr invalidBins = WorkspaceCreationHelper::Create2DWorkspace123(1, 1);

        TS_ASSERT_THROWS(truncate.setTimeBinWidthFromWorkspace(invalidBins), std::invalid_argument);
    }

    void testCalculateBinCount()
    {
        boost::shared_ptr<MockChopper> chopper(new MockChopper);
        EXPECT_CALL(*chopper, cycleTime())
                .Times(1)
                .WillRepeatedly(Return(1500.0));
    ...
    void testGetMaximumTimeValue()
    {
        ...
        TS_ASSERT_THROWS(truncate.getMaximumTimeValue(calculatedBinCount), std::invalid_argument);
    ...
    void testGetMinimumExtraTimeValue()
    {
        ...
        // throws, because actual bin count is smaller than calculated bin count
        TS_ASSERT_THROWS(truncate.getMinimumExtraTimeValue(calculatedBinCount), std::invalid_argument);
    ...
    void testGetCroppedWorkspace()
    {
        ...
        TS_ASSERT_THROWS(truncate.getCroppedWorkspace(smallWorkspace), std::invalid_argument);
    }
    ...
    void testGetExtraCountsWorkspace()
    {
        ...
        TS_ASSERT_THROWS(truncate.getExtraCountsWorkspace(smallWorkspace), std::invalid_argument);
    }
