...
public:
    ...
    void testFromString()
    {
        ...
        TS_ASSERT_THROWS(MillerIndicesIO::fromString(hklInvalidStringLong), std::runtime_error);

        std::string hklInvalidStringShort("2 3");
        TS_ASSERT_THROWS(MillerIndicesIO::fromString(hklInvalidStringShort), std::runtime_error);

        std::string hklInvalidStringGarbage("q43tn rufninc");
        TS_ASSERT_THROWS(MillerIndicesIO::fromString(hklInvalidStringGarbage), boost::bad_lexical_cast);
    }
