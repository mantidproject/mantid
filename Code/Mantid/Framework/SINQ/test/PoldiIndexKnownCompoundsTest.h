...
public:
  ...
  static PoldiIndexKnownCompoundsTest *createSuite() {
    return new PoldiIndexKnownCompoundsTest();
  }
  static void destroySuite(PoldiIndexKnownCompoundsTest *suite) {
    delete suite;
  }
  ...
  void testExecOnePhase() {
    ...
    TS_ASSERT_THROWS_NOTHING(
        alg.setPropertyValue("InputWorkspace", "measured_SI"));
    ...
    TS_ASSERT_THROWS_NOTHING(
        alg.setPropertyValue("ScatteringContributions", "1.0"));
    ...
    ITableWorkspace_sptr si =
        AnalysisDataService::Instance().retrieveWS<ITableWorkspace>("Si");
  ...
  void testSetMeasuredPeaks() {
    PoldiPeakCollection_sptr silicon =
  ...
  void testInitializeIndexedPeaks() {
    ...
    for (size_t i = 0; i < expectedPeaks.size(); ++i) {
      ...
      TS_ASSERT_EQUALS(alg.m_indexedPeaks[i]->getProfileFunctionName(),
                       peaks->getProfileFunctionName());
      TS_ASSERT_EQUALS(alg.m_indexedPeaks[i]->intensityType(),
                       peaks->intensityType());
    }
  }
  ...
  void testGetWorkspaces() {
    ...
    std::vector<std::string> singleWsNames =
        getStringVector("test1,test3,test4");
  ...
  void testGetNormalizedContributions() {
    ...
    std::vector<double> normalized =
        alg.getNormalizedContributions(contributions);
  ...
  void testScaleIntensityEstimates() {
    ...
    PoldiPeakCollection_sptr reference =
        PoldiPeakCollectionHelpers::createTheoreticalPeakCollectionSilicon();

    for (size_t i = 0; i < indexedSilicon->peakCount(); ++i) {
      TS_ASSERT_EQUALS(indexedSilicon->peak(i)->intensity(),
                       2.0 * reference->peak(i)->intensity());
    }
  ...
  void testFwhmSigmaConversion() {
    ...
    TS_ASSERT_EQUALS(PoldiIndexKnownCompounds::sigmaToFwhm(
                         PoldiIndexKnownCompounds::fwhmToSigma(fwhm)),
                     fwhm);
  }
  ...
  void testAssignFwhmEstimates() {
    ...
    for (size_t i = 0; i < indexedSilicon->peakCount(); ++i) {
      TS_ASSERT_EQUALS(indexedSilicon->peak(i)->fwhm(PoldiPeak::Relative),
                       alg.sigmaToFwhm(0.1));
    }
  ...
  void testIsCandidate() {
    ...
    PoldiPeak_sptr candidate1 =
        PoldiPeak::create(Conversions::dToQ(2.02)); // +2 * sigma
    candidate1->setFwhm(UncertainValue(alg.sigmaToFwhm(0.01)),
                        PoldiPeak::AbsoluteD);
    TS_ASSERT(alg.isCandidate(peak, candidate1));

    PoldiPeak_sptr candidate2 =
        PoldiPeak::create(Conversions::dToQ(1.971)); // -2.99 * sigma
    candidate2->setFwhm(UncertainValue(alg.sigmaToFwhm(0.01)),
                        PoldiPeak::AbsoluteD);
    TS_ASSERT(alg.isCandidate(peak, candidate2));

    // More than 3sigma away - no candidate.
    PoldiPeak_sptr candidate3 =
        PoldiPeak::create(Conversions::dToQ(1.97)); // -3 sigma
    candidate3->setFwhm(UncertainValue(alg.sigmaToFwhm(0.01)),
                        PoldiPeak::AbsoluteD);
  ...
  void testGetIndexCandidatePairs() {
    PoldiPeakCollection_sptr measuredSi =
        PoldiPeakCollectionHelpers::createPoldiPeakCollectionMaximum();
    ...
    std::vector<IndexCandidatePair> candidates =
        alg.getIndexCandidatePairs(measuredSi->peak(0), theoreticalSi);
  ...
  void testGetAllIndexCandidatePairs() {
    ...
    std::vector<IndexCandidatePair> candidates =
        alg.getAllIndexCandidatePairs(measuredSi, theoreticalSi);

    // Each peak has two candidates - one from each theoretical Si-collection
    TS_ASSERT_EQUALS(candidates.size(),
                     measuredSi->peakCount() * theoreticalSi.size());
  ...
  void testIndexCandidatePairValues() {
    ...
    candidate1->setFwhm(UncertainValue(alg.sigmaToFwhm(0.1)),
                        PoldiPeak::Relative);

    PoldiPeak_sptr candidate2 = PoldiPeak::create(Conversions::dToQ(1.92), 1.0);
    candidate2->setFwhm(UncertainValue(alg.sigmaToFwhm(0.1)),
                        PoldiPeak::Relative);
  ...
  void testIndexPeaks() {
    std::vector<PoldiPeakCollection_sptr> expected(
        1,
        PoldiPeakCollectionHelpers::createTheoreticalPeakCollectionSilicon());
    PoldiPeakCollection_sptr measured =
        PoldiPeakCollectionHelpers::createPoldiPeakCollectionMaximum();
    ...
    PoldiPeakCollection_sptr indexedSi =
        alg.getIntensitySortedPeakCollection(alg.m_indexedPeaks[0]);
...
private:
  ...
  void setupWorkspaceStructure() {
     ...
     *          +----- test4
     */
    std::vector<std::string> testWorkspaces =
        getStringVector("test1,test2,test3,test4");
