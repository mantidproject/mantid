...
namespace DataHandling
{
  ...
  void PDLoadCharacterizations::init()
  {
    ...
    declareProperty("IParmFilename", std::string(""), "Name of the gsas instrument parameter file.",
                    Direction::Output);
  ...
  void PDLoadCharacterizations::readFocusInfo(std::ifstream &file)
  {
    ...
    for (std::string line = Strings::getLine(file); !file.eof(); line = Strings::getLine(file))
    {
      ...
      {
        this->setProperty("PrimaryFlightPath", boost::lexical_cast<double>(splitted[1]));
        break;
      }
    ...
    if (specIds.size() != l2.size() || specIds.size() != polar.size())
      throw std::runtime_error("Found different number of spectra, L2 and polar angles");
