/*WIKI*
Creates a blank workspace for a given instrument with the option of pulling in detector tables from a RAW/NeXus data file. The histogram sizes are given by binning parameters, see [[Rebin]], rather than explicit data arrays. There are also various options to set axes. 

By default the spectra/detector mapping is 1:1 but there is an option, SpectraMappingFile, to use the mapping from a given file
*WIKI*/
...
namespace Mantid
{
  namespace DataHandling
  {
    ...
    const std::string CreateSimulationWorkspace::category() const { return "General";}
    ...
    void CreateSimulationWorkspace::init()
    {
      ...
      declareProperty(new ArrayProperty<double>("BinParams", boost::make_shared<RebinParamsValidator>(),Direction::Input),
          "A comma separated list of first bin boundary, width, last bin boundary. See Rebin for more details");
      ...
      declareProperty(new FileProperty("DetectorTableFilename", "",FileProperty::OptionalLoad,"", Direction::Input),
                      "An optional filename (currently RAW or ISIS NeXus) that contains UDET & SPEC tables to access hardware grouping");
    }
    ...
    //-----------------------------------------------------------------------------------------------
    /**
     * Create the instrument from the name/file. Runs LoadInstrument with a fake workspace.
     */
    ...
    /**
     * Sets up the detector map. By default a 1:1 map is ensured, however a file can be given to use as a map
    ...
    /**
     * Load the detector mapping from a NeXus file. Throws if the file does not provide the mapping tables
     * @param filename :: The name of the ISIS raw NeXus file to pull the UDET/SPEC tables from
     */
    ...
    void CreateSimulationWorkspace::createGroupingsFromTables(int *specTable, int *udetTable, int ndets)
    {
      ...
      for(int i = 0; i < ndets; ++i)
      {
        ...
        if(m_instrument->isMonitor(detID)) continue; // Skip monitors
    ...
    MantidVecPtr CreateSimulationWorkspace::createBinBoundaries() const
    {
      ...
      if(numBoundaries <= 2)
      {
        throw std::invalid_argument("Error in BinParams - Gave invalid number of bin boundaries: " + boost::lexical_cast<std::string>(numBoundaries));
      }
      return binsPtr;
    }
    ...
    */
    void CreateSimulationWorkspace::applyDetectorMapping()
    {
      ...
      for(auto iter = m_detGroups.begin(); iter != m_detGroups.end(); ++iter)
      {
        ...
        spectrum->setSpectrumNo(static_cast<specid_t>(wsIndex + 1)); // Ensure a contiguous mapping
