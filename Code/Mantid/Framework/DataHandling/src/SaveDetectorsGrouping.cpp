...
namespace DataHandling
{
  ...
  void SaveDetectorsGrouping::createGroupDetectorIDMap(std::map<int, std::vector<detid_t> >& groupwkspmap){
    ...
    for (size_t iws = 0; iws < mGroupWS->getNumberHistograms(); iws ++){
      ...
      std::map<int, std::vector<detid_t> >::iterator it = groupwkspmap.find(groupid);
      if (it == groupwkspmap.end()){
        std::vector<detid_t> tempvector;
        // groupwkspmap.insert(std::pair<int, std::vector<detid_t> >(groupid, tempvector));
      ...
      if (!mspec){
        g_log.error() << "Workspace index " << iws << " has no spectrum.  Impossible!" << std::endl;
        throw;
      }
      std::set<detid_t> detids = mspec->getDetectorIDs();
      if (detids.size() != 1){
        g_log.error() << "Spectrum " << mspec->getSpectrumNo() << " has " << detids.size() << " detectors.  Not allowed situation!" << std::endl;
        throw;
      }
      detid_t detid = 0;
  ...
  void SaveDetectorsGrouping::convertToDetectorsRanges(std::map<int, std::vector<detid_t> > groupdetidsmap,
    ...
    for (std::map<int, std::vector<detid_t> >::iterator it=groupdetidsmap.begin();
