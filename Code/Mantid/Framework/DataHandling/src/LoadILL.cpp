...
void LoadILL::init() {
  ...
  declareProperty(
      new FileProperty("FilenameVanadium", "", FileProperty::OptionalLoad,
          ".nxs"), "File path of the Vanadium file to load (Optional)");
...
void LoadILL::exec() {
  ...
  int calculatedDetectorElasticPeakPosition = getEPPFromVanadium(
      filenameVanadium, vanaWS);
...
void LoadILL::loadInstrumentDetails(NeXus::NXEntry& firstEntry) {
  ...
  if (std::find(m_supportedInstruments.begin(), m_supportedInstruments.end(),
    ...
    std::string message = "The instrument " + m_instrumentName
...
void LoadILL::initWorkSpace(NeXus::NXEntry& entry,
  ...
  m_localWorkspace = WorkspaceFactory::Instance().create("Workspace2D",
      m_numberOfHistograms + numberOfMonitors, m_numberOfChannels + 1,
      m_numberOfChannels);
...
void LoadILL::loadDataIntoTheWorkSpace(NeXus::NXEntry& entry,
  ...
  m_localWorkspace->dataX(firstSpec).assign(detectorTofBins.begin(),
      detectorTofBins.end());
