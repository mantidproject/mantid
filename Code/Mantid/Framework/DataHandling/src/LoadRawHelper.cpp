...
namespace Mantid
{
  namespace DataHandling
  {
     ...
     */
    std::string LoadRawHelper::getThreeColumnName() const
    {
      std::string rawID;
      ...
      if (Poco::File(logfileName).exists())
      {
        ...
        std::string aLine;
        int count = 0;
        kind l_kind(LoadRawHelper::empty);
        while(Mantid::Kernel::extractToEOL(inLogFile,aLine))
        {
          ...
          if ( LoadRawHelper::string != l_kind )
          {
          ...
          if ( LoadRawHelper::string != l_kind && LoadRawHelper::number!=l_kind)
          {
     ...
     */
    LoadRawHelper::kind LoadRawHelper::classify(const std::string& s) const
    {
      if( s.empty() )
      {
        return LoadRawHelper::empty;
      }
      ...
      {
        return LoadRawHelper::string;
      }
      else
      {
        return LoadRawHelper::number;
      }
    }
     ...
     */
    std::string LoadRawHelper::stringToLower(std::string strToConvert)
    {
     ...
     */
    bool LoadRawHelper::isDateTimeString(const std::string& str) const
    {
