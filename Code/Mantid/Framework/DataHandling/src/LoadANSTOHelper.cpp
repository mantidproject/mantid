...
namespace Mantid
{
  namespace DataHandling
  {
    namespace ANSTO
    {
      ...
      bool FastReadOnlyFile::seek(int64_t offset, int whence, int64_t *newPosition) {
        return
          (0 == fseek(m_handle, offset, whence)) &&
          ((newPosition == NULL) || (0 <= (*newPosition = (int64_t)ftell(m_handle))));
      }
#endif
      ...
      namespace Tar {
        ...
        int File::read_byte() {
          ...
          if (m_bufferPosition == m_bufferAvailable) {
            ...
            uint32_t size = (uint32_t)std::min<int64_t>(sizeof(m_buffer), m_size - m_position);
