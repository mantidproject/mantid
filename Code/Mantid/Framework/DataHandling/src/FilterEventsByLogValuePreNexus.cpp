...
namespace DataHandling
{
  ...
  */
  FilterEventsByLogValuePreNexus::FilterEventsByLogValuePreNexus() :
    ...
    eventfile(NULL), m_maxNumEvents(0)
  {
  }
  ...
  void FilterEventsByLogValuePreNexus::init()
  {
    ...
    declareProperty(new WorkspaceProperty<ITableWorkspace>("EventLogTableWorkspace", "", PropertyMode::Optional),
                    "Optional output table workspace containing the event log (pixel) information. ");
    ...
    declareProperty("FunctionMode", "LoadData", boost::make_shared<StringListValidator>(vecfunmode),
    ...
    declareProperty(new ArrayProperty<std::string>("LogPIxelTags"),
  ...
  void FilterEventsByLogValuePreNexus::processProperties()
  {
    ...
    if (m_pulseIDFileName.empty())
    {
      ...
      if (!m_pulseIDFileName.empty())
      {
        ...
        {
          m_pulseIDFileName = "";
          g_log.warning("Generated pulse ID file name does not point to an existing file. ");
        }
      }
      else
      {
  ...
  void FilterEventsByLogValuePreNexus::processEventLogs()
  {
    ...
    if (!evlog.empty())
    {
      ...
      // Set property
      setProperty("EventLogTableWorkspace", boost::dynamic_pointer_cast<ITableWorkspace>(evtablews));
    }
  ...
  void FilterEventsByLogValuePreNexus::doStatToEventLog(size_t mindex)
  {
    ...
    for (size_t i = 0; i < nbins; ++ i)
    {
      ...
      int64_t templogtime = ptime.totalNanoseconds() +
   ...
   */
  void FilterEventsByLogValuePreNexus::runLoadInstrument(const std::string &eventfilename, MatrixWorkspace_sptr localWorkspace)
  {
   ...
   */
  inline void FilterEventsByLogValuePreNexus::fixPixelId(PixelType &pixel, uint32_t &period) const
  {
  ...
  void FilterEventsByLogValuePreNexus::procEvents(DataObjects::EventWorkspace_sptr & workspace)
  {
    ...
    else
    {
      ...
      g_log.information() << (parallelProcessing ? "Using" : "Not using") << " parallel processing." << "\n";
    }

    if (m_functionMode == "ExamineEventLog" && parallelProcessing)
    {
      parallelProcessing = false;
      g_log.notice("In function mode 'ExamineEventLog', processing mode is forced to serial. ");
    }
    ...
    //--------------------------------------------------------------------
    g_log.notice() << "Read " << num_good_events << " events + " << num_error_events << " errors"
                   << ". Shortest TOF: " << shortest_tof << " microsec; longest TOF: "
                   << longest_tof << " microsec." << "\n"
                   << "Bad Events = " << num_bad_events << "  Events of Wrong Detector = "
                                                        ...
                                                        size_t current_event_buffer_size, size_t fileOffset)
  {
    ...
    PARALLEL_CRITICAL( FilterEventsByLogValuePreNexus_global_statistics )
    {
      ...
      for (it = local_wrongdetids.begin(); it != local_wrongdetids.end(); ++it)
      {
        ...
        // Obtain the global map index for this wrong detector ID events entry in local map
        size_t mindex = 0;
        ...
        // Append local (thread) loaded events (pulse + tof) to global wrong detid data structure
  ...
  void FilterEventsByLogValuePreNexus::filterEvents()
  {
    ...
    if (parallelProcessing)
    {
      ...
      // Merge all workspaces, index by index.
      PARALLEL_FOR_NO_WSP_CHECK()
      for (int iwi=0; iwi<int(localWorkspace->getNumberHistograms()); iwi++)
      {
        size_t wi = size_t(iwi);
  ...
  //----------------------------------------------------------------------------------------------
  /** Set up instrument related parameters such as detector map and etc for 'eventws'
    * and create a pixel-spectrum map
    * We want to pad out empty pixels: monitor
    */
  size_t FilterEventsByLogValuePreNexus::padOutEmptyPixels(DataObjects::EventWorkspace_sptr eventws)
  {
    // Obtain detector map
    detid2det_map detector_map;
  ...
  //----------------------------------------------------------------------------------------------
  /** Set up instrument related parameters such as detector map and etc for 'eventws' create a
    * pixel-spectrum map
    */
  void FilterEventsByLogValuePreNexus::setupPixelSpectrumMap(DataObjects::EventWorkspace_sptr eventws)
  {
    // Obtain detector map
    detid2det_map detector_map;
    eventws->getInstrument()->getDetectors(detector_map);
  ...
  int FilterEventsByLogValuePreNexus::findRunFrequency()
  {
    ...
    for (size_t i = 1; i < checksize; ++i)
    {
      ...
      if (curr_event_index != prev_event_index)
      {
        ...
        {
          g_log.notice() << "istart = " << istart << " w/ value = " << m_vecEventIndex[istart]
                         << ", icurr = " << i << " w/ value = " << m_vecEventIndex[i] << "\n";
  ...
  void FilterEventsByLogValuePreNexus::loadPixelMap(const std::string &filename)
  {
    ...
    if (filename.empty())
    {
      g_log.information("Pixel mapping file name is empty. Pixel map is not loaded and thus empty. ");
      return;
    }
...
void FilterEventsByLogValuePreNexus::openEventFile(const std::string &filename)
{
  ...
  else // We are loading part - work out the event number range
  {
    ...
    if ( chunk == totalChunks ) m_maxNumEvents += num_events%totalChunks;
  }
