...
Reads a McStas Nexus file into a Mantid WorkspaceGroup with a user-supplied name. Data generated by McStas monitor components are stored in workspaces of type Workspace2D or Event. All data sets are numbered and nested within the  WorkspaceGroup.



McStas event data are generated as follows. The McStas component monitor_nD must be called with the argument: options ="mantid square x limits=[-0.2 0.2] bins=128 y limits=[-0.2 0.2] bins=128, neutron pixel t, list all neutrons".  Number of bins and limits can be chosen freely. 
...
Linux
Windows
Mac - use either Intel compiler or gcc 4.8. Simulations using Nexus format and event data does not work using the Clang compiler.  
