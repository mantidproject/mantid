...
namespace Mantid
{
  namespace DataHandling
  {
    ...
    ISISRunLogs::ISISRunLogs(const API::Run & icpRun, const int totalNumPeriods)
      : m_logParser(),
        m_numOfPeriods(totalNumPeriods)
    {
      ...
      // If it does not exist then pass in a NULL log to indicate that period 1 should be assumed
    ...
    void ISISRunLogs::addPeriodLogs(const int period, API::Run & exptRun)
    {
      ...
      if(m_logParser->nPeriods() > 1)
      {
        ...
        else maskProp = periodLog;
      }
      // Filter logs if we have anything to filter on
      if(maskProp) exptRun.filterByLog(*maskProp);
      delete logFilter;
