...
namespace Mantid
{
  namespace DataHandling
  {
    ...
    /**
     * Query the ISIS archive for a set of filenames & extensions. The method goes through each extension
     * and checks whether it can find a match with each filename in the filenames list. The first match is returned.
    ...
    * @return The path to the file or an empty string in case of error/non-existing file.
    */
    std::string ISISDataArchive::getPath(const std::string& fName) const
    {
      ...
      if(status == HTTPResponse::HTTP_OK)
      {
        ...
        try
        {
          ...
          if(Poco::File(expectedPath).exists() ) return expectedPath;
        }
