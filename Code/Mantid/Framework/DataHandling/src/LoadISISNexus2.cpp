  ...
  namespace DataHandling
  {
    ...
    */
    void LoadISISNexus2::exec()
    {
      ...
      Mantid::NeXus::NXEntry entry = root.openEntry("raw_data_1");
      ...
      try
      {
        Mantid::NeXus::NXClass det_class = entry.openNXGroup("detector_1");
        Mantid::NeXus::NXInt spectrum_index = det_class.openNXInt("spectrum_index");
        spectrum_index.load();
      ...
      Mantid::NeXus::NXInt nsp1 = entry.openNXInt("isis_vms_compat/NSP1");
      nsp1.load();
      Mantid::NeXus::NXInt udet = entry.openNXInt("isis_vms_compat/UDET");
      udet.load();
      Mantid::NeXus::NXInt spec = entry.openNXInt("isis_vms_compat/SPEC");
      spec.load();

      //Pull out the monitor blocks, if any exist
      int nmons(0);
      for(std::vector<Mantid::NeXus::NXClassInfo>::const_iterator it = entry.groups().begin();
        it != entry.groups().end(); ++it) 
      {
        if (it->nxclass == "NXmonitor") // Count monitors
        {
          Mantid::NeXus::NXInt index = entry.openNXInt(std::string(it->nxname) + "/spectrum_index");
          index.load();
      ...
      if( ndets == 0 )
      {
        ...
        Mantid::NeXus::NXInt chans = entry.openNXInt(m_monitors.begin()->second + "/data");
      ...
      {
        Mantid::NeXus::NXData nxData = entry.openNXData("detector_1");
        Mantid::NeXus::NXInt data = nxData.openIntData();
      ...
      {
        //Get the X data
        Mantid::NeXus::NXFloat timeBins = entry.openNXFloat("detector_1/time_of_flight");
        timeBins.load();
    ...
    */
    void LoadISISNexus2::loadPeriodData(int period, Mantid::NeXus::NXEntry & entry, DataObjects::Workspace2D_sptr local_workspace)
    {
      ...
      {
        Mantid::NeXus::NXData nxdata = entry.openNXData("detector_1");
        Mantid::NeXus::NXDataSetTyped<int> data = nxdata.openIntData();
        data.open();
      ...
      if( !m_monitors.empty() )
      {
        ...
        {
          Mantid::NeXus::NXData monitor = entry.openNXData(it->second);
          Mantid::NeXus::NXInt mondata = monitor.openIntData();
          ...
          Mantid::NeXus::NXFloat timeBins = monitor.openNXFloat("time_of_flight");
          timeBins.load();
    ...
    */
    void LoadISISNexus2::loadBlock(Mantid::NeXus::NXDataSetTyped<int> & data, int blocksize, int period, int start,
    ...
    */
    void LoadISISNexus2::loadRunDetails(DataObjects::Workspace2D_sptr local_workspace, Mantid::NeXus::NXEntry & entry)
    {
      ...
      Mantid::NeXus::NXClass vms_compat = entry.openNXGroup("isis_vms_compat");
      // Run header
      Mantid::NeXus::NXChar char_data = vms_compat.openNXChar("HDR");
      char_data.load();
      ...
      // RPB struct info
      Mantid::NeXus::NXInt rpb_int = vms_compat.openNXInt("IRPB");
      rpb_int.load();
      ...
      // Now double data
      Mantid::NeXus::NXFloat rpb_dbl = vms_compat.openNXFloat("RRPB");
      rpb_dbl.load();
    ...
    *   @param entry :: The Nexus entry
    */
    void LoadISISNexus2::loadSampleData(DataObjects::Workspace2D_sptr local_workspace, Mantid::NeXus::NXEntry & entry)
    {
      /// Sample geometry
      Mantid::NeXus::NXInt spb = entry.openNXInt("isis_vms_compat/SPB");
      ...
      Mantid::NeXus::NXFloat rspb = entry.openNXFloat("isis_vms_compat/RSPB");
    ...
    */
    void LoadISISNexus2::loadLogs(DataObjects::Workspace2D_sptr ws, Mantid::NeXus::NXEntry & entry,int period)
    {

      Mantid::NeXus::NXMainClass runlogs = entry.openNXClass<Mantid::NeXus::NXMainClass>("runlog");
      ...
      for(std::vector<Mantid::NeXus::NXClassInfo>::const_iterator it=runlogs.groups().begin();it!=runlogs.groups().end();it++)
      {
        if (it->nxclass == "NXlog")
        {
          ...
          Mantid::NeXus::NXLog nxLog(runlogs,it->nxname);
          nxLog.openLocal();
      ...
      Mantid::NeXus::NXMainClass selogs = entry.openNXClass<Mantid::NeXus::NXMainClass>("selog");
      for(std::vector<Mantid::NeXus::NXClassInfo>::const_iterator it=selogs.groups().begin();it!=selogs.groups().end();it++)
      {
        if (it->nxclass == "IXseblock")
        {
          Mantid::NeXus::NXMainClass selog(selogs,it->nxname);
          selog.openLocal("IXseblock");
          Mantid::NeXus::NXLog nxLog(selog,"value_log");
          ...
          {
            Mantid::NeXus::NXFloat value = selog.openNXFloat("value");
            value.load();
