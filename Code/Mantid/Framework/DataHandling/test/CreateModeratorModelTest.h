...
public:
  ...
  static CreateModeratorModelTest *createSuite() { return new CreateModeratorModelTest(); }
  static void destroySuite( CreateModeratorModelTest *suite ) { delete suite; }

  CreateModeratorModelTest() :
    m_inputName("CreateModeratorModelTest"), m_testWS(createTestWorkspace())
  {
  }
  ...
  {
    IAlgorithm_sptr alg = createAlgorithm(m_inputName, "IkedaCarpenterModerator");

    TS_ASSERT_THROWS(alg->setPropertyValue("Parameters", ""), std::invalid_argument);
  }

  void test_Setting_Parameter_String_WIth_Some_Values_Leaves_Others_As_Default()
  {
    doParameterCheckTest("TiltAngle=27,TauS=45", 27*M_PI/180.0, 0.0, 45, 0.0);
  }

  void test_Setting_All_Parameters_Attaches_Correct_Moderator_Object()
  {
    doParameterCheckTest("TiltAngle=27,TauF=13.55,TauS=45,R=0.01", 27*M_PI/180.0,13.55, 45,0.01);
  }
...
private:
  ...
  void doParameterCheckTest(const std::string & params, const double tilt,
                            const double tauf, const double taus, const double rmix)
  {
    ...
    if(ws)
    {
      IkedaCarpenterModerator *sourceProps = dynamic_cast<IkedaCarpenterModerator*>(&ws->moderatorModel());
      TSM_ASSERT("Found moderator object but it was not of the expected type", sourceProps);
