...
public:
  ...
  void test_Exec_With_Absolute_Rotation()
  {
    ...
    MatrixWorkspace_sptr testWS = WorkspaceCreationHelper::create2DWorkspaceWithFullInstrument(1,1,false); // no monitors

    double angle = 45.0;
    Mantid::Kernel::V3D axis(0.,1.,1.);
    testWS = runRotateInstrument(testWS, "pixel-0)", angle, axis, false); // Detector 1
  ...
  void test_Exec_With_Relative_Rotation()
  {
    ...
    // Run it twice to check the relative rotation as the instrument starts out with no rotation
    testWS = runRotateInstrument(testWS, "testInst", angle, axis, false); // instrument
    testWS = runRotateInstrument(testWS, "pixel-0)", angle, axis, true); // relative for detector
...
private:
  ...
  Mantid::API::MatrixWorkspace_sptr runRotateInstrument(const Mantid::API::MatrixWorkspace_sptr & testWS, const std::string & compName,
                                                        const double angle, const Mantid::Kernel::V3D & axis,
                                                        const bool relative)
  {
    RotateInstrumentComponent alg;
    alg.initialize();
