  ...
  namespace MDEvents
  {
    ...
    */
    Mantid::API::CoordTransform* CoordTransformDistanceParser::createTransform(Poco::XML::Element* coordTransElement) const
    {
      ...
      Mantid::MDEvents::CoordCenterVectorParam* coordCenterParam = coordCenterParser.createWithoutDelegation(parameter);
      ...
      Mantid::MDEvents::DimensionsUsedVectorParam* dimsUsedVecParm = dimsUsedParser.createWithoutDelegation(parameter);

      ////Generate the coordinate transform and return
      CoordTransformDistance* transform = new CoordTransformDistance(inDimParameter->getValue(), coordCenterParam->getPointerToStart(), dimsUsedVecParm->getPointerToStart());
      return transform;
    }
