...
namespace Mantid
{
  namespace MDEvents
  {
    ...
    MDHistoWorkspaceIterator::MDHistoWorkspaceIterator(MDHistoWorkspace_const_sptr workspace,
        Mantid::Geometry::MDImplicitFunction * function, size_t beginPos, size_t endPos) :
     ...
     */
    MDHistoWorkspaceIterator::MDHistoWorkspaceIterator(MDHistoWorkspace_const_sptr workspace,
        SkippingPolicy* skippingPolicy, Mantid::Geometry::MDImplicitFunction * function, size_t beginPos,
        size_t endPos) :
        ...
        Mantid::Geometry::MDImplicitFunction * function, size_t beginPos, size_t endPos)
    {
      ...
      if (m_max < m_pos)
        throw std::invalid_argument(
            "MDHistoWorkspaceIterator::ctor(): End point given is before the start point.");
    ...
    coord_t * MDHistoWorkspaceIterator::getVertexesArray(size_t & numVertices,
        const size_t outDimensions, const bool * maskDim) const
    {
    ...
    /**
     * Gets indexes of bins/pixels/boxes neighbouring the present iterator location. Return all vertex touching neighbours.
    ...
    std::vector<size_t> MDHistoWorkspaceIterator::findNeighbourIndexes() const
    {
      ...
      for (size_t i = 0; i < m_permutationsVertexTouching.size(); ++i)
      {
        ...
        if (neighbour_index < m_ws->getNPoints()
            && Utils::isNeighbourOfSubject(m_nd, neighbour_index, m_index, m_indexMaker, m_indexMax))
        {
     ...
     *
     * Will return the indexes of neighbours even if they are unreachable from the current iterator.
     * To verify that the indexes are reachable from the current iterator, run isWithinBounds. Note that this is only a concern where the workspace iteration is portioned up amongst >1 iterators.
     * @return
     */
