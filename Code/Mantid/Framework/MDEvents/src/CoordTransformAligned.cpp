...
namespace MDEvents
{
  ...
  : CoordTransform(inD, outD)
  {
    if (!origin || !scaling || !dimensionToBinFrom)
      throw std::runtime_error("CoordTransformAligned::ctor(): at least one of the input arrays is a NULL pointer.");
  ...
  void CoordTransformAligned::apply(const coord_t * inputVector, coord_t * outVector) const
  {
    ...
    {
      // Get the coordinate at the dimension in the INPUT workspace corresponding to this OUTPUT
  ...
  std::string CoordTransformAligned::toXMLString() const
  {
    using namespace Poco::XML;
     ...
     std::string formattedXMLString = boost::str(boost::format(xmlstream.str().c_str())
       % inD_param.toXMLString().c_str() % outD_param.toXMLString().c_str()
       );
