...
namespace Geometry
{
  ...
  */
  ...
  public:
      ...
      static std::pair<Kernel::IntMatrix, V3R> parseIdentifier(const std::string &identifier);
      static std::string getNormalizedIdentifier(const std::pair<Kernel::IntMatrix, V3R> &data);
      static std::string getNormalizedIdentifier(const Kernel::IntMatrix &matrix, const V3R &vector);

  protected:
      SymmetryOperationSymbolParser();

      static std::pair<Kernel::IntMatrix, V3R> parseComponents(const std::vector<std::string> &components);
      static std::string getCleanComponentString(const std::string &componentString);
      static std::pair<std::vector<int>, RationalNumber> parseComponent(const std::string &component);

      static void processMatrixRowToken(const std::string &matrixToken, std::vector<int> &matrixRow);
      static void addToVector(std::vector<int> &vector, const std::vector<int> &add);
      static std::vector<int> getVectorForSymbol(const char symbol, const char sign = '+');
      static int getFactorForSign(const char sign);

      static void processVectorComponentToken(const std::string &rationalNumberToken, RationalNumber &vectorComponent);
