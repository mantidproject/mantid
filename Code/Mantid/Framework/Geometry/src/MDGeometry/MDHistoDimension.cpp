  ...
  namespace Geometry
  {
    ...
    std::string MDHistoDimension::toXMLString() const
    {
      ...
      if(this->getIsIntegrated())
      {
        ...
        AutoPtr<Text> upperLimitText = pDoc->createTextNode(boost::str(boost::format("%.4f") % this->getMaximum())); // Dimension does not yet provide integration ranges.
        ...
        AutoPtr<Text> lowerLimitText = pDoc->createTextNode(boost::str(boost::format("%.4f") % this->getMinimum())); // Dimension does not yet provide integration ranges.
