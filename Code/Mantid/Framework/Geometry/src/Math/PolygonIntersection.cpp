...
namespace Mantid
{
  namespace Geometry
  {
    ...
    namespace
    {
      ...
      unsigned int intersection(const V2D & a, const V2D & b,
                                const V2D & c, const V2D & d,
                                Kernel::V2D & crossPoint);
      /// Calculate intersection type for parallel lines
      unsigned int parallelIntersect(const V2D & a, const V2D & b,
                                     const V2D & c, const V2D & d,
                                     Kernel::V2D & crossPoint);
     ...
     */
    ConvexPolygon chasingEdgeIntersect(const ConvexPolygon &P, const ConvexPolygon &Q)
    {
    ...
    namespace
    {
      ...
      unsigned int intersection(const V2D & a, const V2D & b,
                                ...
                                V2D & crossPoint)
      {
        double denominator = ( a[0]*(d[1] - c[1]) +
                               b[0]*(c[1] - d[1]) +
                               d[0]*(b[1] - a[1]) +
                               c[0]*(a[1] - b[1]) );
        ...
        double numerator = (a[0]*( d[1] - c[1] ) +
                            ...
                            d[0]*( c[1] - a[1] ));
        unsigned int code(0);
        if ( numerator == 0.0 || numerator == denominator ) code = 2;
        double s = numerator/denominator;
        numerator = -( a[0]*( c[1] - b[1] ) +
                       b[0]*( a[1] - c[1] ) +
                       c[0]*( b[1] - a[1] ) );
        if ( numerator == 0.0 || numerator == denominator ) code = 2;
      ...
      /**
       * Calculate the point of intersection for the given parallel line segments, ab-cd
       * @param a :: Starting point of the first segment
       * @param b :: Starting point of the first segment
       * @param c :: Starting point of the second segment
       * @param d :: Starting point of the second segment
      ...
      /// Is point C between point A and point B
      bool isBetween(const V2D & a, const V2D & b,
                     const V2D & c)
      {
        if ( a[0] != b[0] )
