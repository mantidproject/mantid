...
namespace Mantid
{
  namespace Geometry
  {
    ...
    Kernel::V2D & Vertex2DList::operator[](const size_t index)
    {
      ...
      return const_cast<V2D&>(
        static_cast<const Vertex2DList&>(*this)[index]);
    }
    ...
    const Kernel::V2D & Vertex2DList::operator[](const size_t index) const
    {
      ...
      throw Kernel::Exception::IndexError(index, m_vertices.size(), "Vertex2DList::operator[]");
    }
     ...
     */
    const Kernel::V2D& Vertex2DList::front() const
    {
    ...
    int Vertex2DList::indexOf(const Kernel::V2D & point) const
    {
      ...
      {
        if( m_vertices[index] == point ) return static_cast<int>(index);
      }
      return -1;
    }
