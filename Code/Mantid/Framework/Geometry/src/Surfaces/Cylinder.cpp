...
namespace Mantid
{
  ...
  namespace Geometry
  {
    ...
    Cylinder::Cylinder(const Cylinder& A) :
        Quadratic(A), Centre(A.Centre), Normal(A.Normal), Nvec(A.Nvec), Radius(A.Radius)
    /**
     ...
     */
    {
      enum
      {
        errDesc = -1, errAxis = -2, errCent = -3, errRadius = -4
      };
    ...
    void Cylinder::getBoundingBox(double &xmax, double &ymax, double &zmax, double &xmin, double &ymin,
        double &zmin)
    {
      /**
