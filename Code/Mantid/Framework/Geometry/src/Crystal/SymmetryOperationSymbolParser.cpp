 ...
 */
std::pair<Kernel::IntMatrix, V3R> SymmetryOperationSymbolParser::parseIdentifier(const std::string &identifier)
{
...
/// Tries to parse the three components of the symbol, throws std::runtime_error if number of components is not three.
std::pair<Kernel::IntMatrix, V3R> SymmetryOperationSymbolParser::parseComponents(const std::vector<std::string> &components)
{
    ...
    // Each part of the symbol contains one row of the resulting matrix and the magnitude of the translation vector.
...
std::string SymmetryOperationSymbolParser::getCleanComponentString(const std::string &componentString)
{
    return boost::algorithm::erase_all_copy(componentString, " ");
}

/// Tries to parse a single component of the total symbol, throws std::runtime_error if the string can not be parsed.
std::pair<std::vector<int>, RationalNumber> SymmetryOperationSymbolParser::parseComponent(const std::string &component)
{
    ...
    // If the combined length of the matched sub strings is less than the total string length, there was some garbage inbetween.
    ...
    // The matrix may be invalid, this happens when something like x+x+y+z is specified.
...
/// Try to generate a matrix row from the token and add it to the supplied vector, throws std::runtime_error if it fails to parse the input.
void SymmetryOperationSymbolParser::processMatrixRowToken(const std::string &matrixToken, std::vector<int> &matrixRow)
{
...
/// Add a vector to another vector (element wise), throws std::runtime_error if sizes don't match.
void SymmetryOperationSymbolParser::addToVector(std::vector<int> &vector, const std::vector<int> &add)
{
...
/// Returns the vector corresponding to the given symbol (x: (1, 0, 0); y: (0, 1, 0); z: (0, 0, 1)) and adds + or -.
std::vector<int> SymmetryOperationSymbolParser::getVectorForSymbol(const char symbol, const char sign)
{
...
/// Tries to create a RationalNumber from the input and adds it to the supplied RationalNumber.
void SymmetryOperationSymbolParser::processVectorComponentToken(const std::string &rationalNumberToken, RationalNumber &vectorComponent)
{
    std::vector<std::string> components;
...
bool SymmetryOperationSymbolParser::isValidMatrixRow(const std::vector<int> &matrixRow)
{
    int nulls = 0;
