...
namespace Geometry
{
  ...
  std::set<V3D> PointGroup::getEquivalentSet(const Kernel::V3D &hkl) const
  {
      ...
      for(auto op = m_symmetryOperations.begin();
          op != m_symmetryOperations.end(); ++op) {
  ...
  void PointGroup::setSymmetryOperations(const std::vector<SymmetryOperation> &generators)
  {
      m_symmetryOperations.clear();

      std::vector<SymmetryOperation> allSymmetryOperations = generateSymmetryOperations(generators);
      for(auto it = allSymmetryOperations.begin(); it != allSymmetryOperations.end(); ++it) {
          addSymmetryOperation(*it);
      }
  }

  /// Adds a symmetry operation to the point group.
  void PointGroup::addSymmetryOperation(const SymmetryOperation &symmetryOperation)
  {
   ...
   * symmetry operations. It does so by applying the first symmetry operation (order - 1) times
   * to an identity operation which results in a list of (order - 1) matrices. Then it multiplies
   ...
   * @return
   */
  std::vector<SymmetryOperation> PointGroup::generateSymmetryOperations(const std::vector<SymmetryOperation> &symmetryOperations)
  {
      ...
      for(std::vector<SymmetryOperation>::const_iterator symOp = symmetryOperations.begin();
          symOp != symmetryOperations.end();
          ++symOp) {
          std::vector<SymmetryOperation> currentMatrices(allSymmetryOperations);

          for(std::vector<SymmetryOperation>::const_iterator currentMatrix = currentMatrices.begin();
  ...
  void PointGroupLaue1::init()
  {
      ...
      generatingSymmetryOperations.push_back(SymmetryOperationFactory::Instance().createSymOp("-x,-y,-z"));
  ...
  {
      std::vector<SymmetryOperation> generatingSymmetryOperations;
      generatingSymmetryOperations.push_back(SymmetryOperationFactory::Instance().createSymOp("-y,x,z"));
      generatingSymmetryOperations.push_back(SymmetryOperationFactory::Instance().createSymOp("x,y,-z"));
  ...
  {
      std::vector<SymmetryOperation> generatingSymmetryOperations;
      generatingSymmetryOperations.push_back(SymmetryOperationFactory::Instance().createSymOp("-y,x-y,z"));
      generatingSymmetryOperations.push_back(SymmetryOperationFactory::Instance().createSymOp("-x,-y,-z"));
  ...
  void PointGroupLaue8::init()
  {
      ...
      generatingSymmetryOperations.push_back(SymmetryOperationFactory::Instance().createSymOp("-y,x-y,z"));
      generatingSymmetryOperations.push_back(SymmetryOperationFactory::Instance().createSymOp("-x,-y,-z"));
      generatingSymmetryOperations.push_back(SymmetryOperationFactory::Instance().createSymOp("-x,y-x,z"));
  ...
  void PointGroupLaue11::init()
  {
      std::vector<SymmetryOperation> generatingSymmetryOperations;
      generatingSymmetryOperations.push_back(SymmetryOperationFactory::Instance().createSymOp("x-y,x,z"));
      generatingSymmetryOperations.push_back(SymmetryOperationFactory::Instance().createSymOp("x-y,-y,-z"));
      generatingSymmetryOperations.push_back(SymmetryOperationFactory::Instance().createSymOp("x,y,-z"));
  ...
  {
      std::vector<SymmetryOperation> generatingSymmetryOperations;
      generatingSymmetryOperations.push_back(SymmetryOperationFactory::Instance().createSymOp("z,x,y"));
      generatingSymmetryOperations.push_back(SymmetryOperationFactory::Instance().createSymOp("-x,-y,z"));
      generatingSymmetryOperations.push_back(SymmetryOperationFactory::Instance().createSymOp("x,-y,z"));
      generatingSymmetryOperations.push_back(SymmetryOperationFactory::Instance().createSymOp("-x,-y,-z"));
