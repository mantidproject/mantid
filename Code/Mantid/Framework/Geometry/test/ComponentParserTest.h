...
//      }
//      writer.endFragment();
//      std::cout << tim << " to write out simple XML with Poco XMLWriter. " << str.str().size() << " chars." << std::endl;
...
//      }
//      std::cout << tim << " to write out simple XML by manually appending strings (fastest possible). " << xmlStr.str().size() << " chars." << std::endl;
...
//    if (true)
//    {
//      ::NeXus::File * file = new ::NeXus::File("components.nxs", NXACC_CREATE5);
...
//      file->close();
//      std::cout << tim << " to write make a NXS array " << num << " entries." << std::endl;
//    }
//
//    xml = "<root>" + xml + "</root>";
