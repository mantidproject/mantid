...
public:
  ...
  static ComponentHelperTest *createSuite() { return new ComponentHelperTest(); }
  ...
  void test_moveComponent_With_Absolute_Position()
  {
    ...
    TS_ASSERT_THROWS_NOTHING(ComponentHelper::moveComponent(*det, *pmap, absPos, ComponentHelper::Absolute));
  ...
  void test_moveComponent_With_Relative_Position()
  {
    ...
    TS_ASSERT_THROWS_NOTHING(ComponentHelper::moveComponent(*det, *pmap, shift, ComponentHelper::Relative));
  ...
  void test_rotateComponent_With_Absolute_Rotation()
  {
    ...
    TS_ASSERT_THROWS_NOTHING(ComponentHelper::rotateComponent(*det, *pmap, rotation, ComponentHelper::Absolute));
  ...
  void test_rotateComponent_With_Relative_Rotation()
  {
    ...
    // The test instrument starts with zero rotation so rotate first of the relative one will be the same as absolute
    auto inst = createTestInstrument();
    auto pmap = inst->getParameterMap();
    ...
    TS_ASSERT_THROWS_NOTHING(ComponentHelper::rotateComponent(*inst, *pmap, rotation, ComponentHelper::Absolute)); // Absolute for Instrument

    auto det = inst->getDetector(1);
    TS_ASSERT_THROWS_NOTHING(ComponentHelper::rotateComponent(*det, *pmap, rotation, ComponentHelper::Relative)); // Relative for Detector
...
private:
  ...
  {
    using namespace Mantid::Geometry;
    auto baseInst = ComponentCreationHelper::createTestInstrumentCylindrical(1); // 1 bank
