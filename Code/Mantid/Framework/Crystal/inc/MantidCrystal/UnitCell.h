  ...
  class DLLExport UnitCell
  {
    public:
      ...
      // Access private variables
	    const Geometry::Matrix<double>& getG() const {return G;} //Get the metric tensor
	    const Geometry::Matrix<double>& getGstar() const { return Gstar;} //Get the reciprocal metric tensor
	    const Geometry::Matrix<double>& getB() const {return B;} //Get the B-matrix
    ...
    private:	    
      std::vector <double> da; //Lattice parameter a,b,c,alpha,beta,gamma (in $\AA$ and radians)
	    ...
	    Geometry::Matrix<double> G; //Metric tensor
	    Geometry::Matrix<double> Gstar; //Reciprocal lattice tensor
	  	Geometry::Matrix<double> B; //B matrix for a right-handed coordinate system 
...
#endif  /* MANTID_CRYSTAL_UNITCELL_H_ */
