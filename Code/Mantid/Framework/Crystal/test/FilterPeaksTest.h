...
private:
   ...
   */
  PeaksWorkspace_sptr createInputWorkspace(const double h, const double k, const double l,
      const double intensity = 0, const double sigIntensity = 0)
  {
   ...
   */
  IPeaksWorkspace_sptr runAlgorithm(PeaksWorkspace_sptr inWS, const std::string& filterVariable,
      const double filterValue, const std::string& filterOperator)
  {
    ...
    IPeaksWorkspace_sptr outWS = AnalysisDataService::Instance().retrieveWS<IPeaksWorkspace>(
        outputWorkspace);
...
public:
  ...
  FilterPeaksTestPerformance()
  {
    ...
    Mantid::API::FrameworkManagerImpl& manager = Mantid::API::FrameworkManager::Instance();
