...
public:
  ...
  void testSXPeakConstructorThrowsIfNegativeIntensity()
  {
    ...
    TSM_ASSERT_THROWS("SXPeak: Should not construct with a negative intensity", SXPeak(0.001, 0.02, 0.01, intensity, spectra, detectorDistance, 1), std::invalid_argument);
  }
  ...
  void testSXPeakConstructorThrowsIfSpectraSizeZero()
  {
    ...
    double detectorDistance = 3;
    TSM_ASSERT_THROWS("SXPeak: Should not construct with a zero size specral list", SXPeak(0.001, 0.02, 0.01, intensity, spectra, detectorDistance, 1), std::invalid_argument);
  }
  ...
  void testFindNoPeaks()
  {
    ...
    TSM_ASSERT_EQUALS("Nothing above background in input workspace, should have found no peaks!", 0, result->rowCount());
  }
  ...
  void testFindZeroPeaksWithBoostedBackground()
  {
    ...
    alg.setProperty("SignalBackground", theresholdIntensity); //Boost the background intensity threshold level to be the same as that of the peak
    alg.execute();
    TSM_ASSERT("FindSXPeak should have been executed.", alg.isExecuted());
