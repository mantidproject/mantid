...
public:
  ...
  void doTestValid(bool pass,
      double BackgroundInnerFactor, double BackgroundOuterFactor, double BackgroundInnerRadius, double BackgroundOuterRadius)
  {
    PeakIntensityVsRadius alg;
    TS_ASSERT_THROWS_NOTHING( alg.initialize() )
    TS_ASSERT( alg.isInitialized() )
  ...
  MatrixWorkspace_sptr doTest(double BackgroundInnerFactor, double BackgroundOuterFactor, double BackgroundInnerRadius, double BackgroundOuterRadius)
  {
  ...
  void test_VariableBackground()
  {
    ...
    // Points before 0.5 are approximately zero because the background shell is in the peak.
