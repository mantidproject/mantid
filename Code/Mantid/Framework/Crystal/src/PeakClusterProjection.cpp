  ...
  void validate(IMDWorkspace const * const mdWS)
  {
    ...
    if (coordinateSystem == None)
    {
      throw std::invalid_argument(
          "Input image IMDHistoWorkspace must have a known special coordinate system.");
    }
...
namespace Mantid
{
  namespace Crystal
  {
     ...
     */
    PeakClusterProjection::PeakClusterProjection(boost::shared_ptr<Mantid::API::IMDWorkspace>& mdWS) :
        m_mdWS(mdWS)
    {
      validate(mdWS.get());
    ...
    /**
     ...
     @param mdWS: Input image workspace to base the projection on and to take the coordinate system from.
     */
    PeakClusterProjection::PeakClusterProjection(boost::shared_ptr<Mantid::API::IMDHistoWorkspace>& mdWS) :
    ...
    /**
     Constructor
     @param mdWS: Input mdevent workspace to base the projection on and to take the coordinate system from.
     */
    PeakClusterProjection::PeakClusterProjection(boost::shared_ptr<Mantid::API::IMDEventWorkspace>& mdWS) :
        m_mdWS(boost::static_pointer_cast<IMDWorkspace>(mdWS))
    {
      validate(mdWS.get());
     ...
     */
    Mantid::signal_t PeakClusterProjection::signalAtPeakCenter(const IPeak& peak,
