#ifndef MANTID_ALGORITHMS_LEBAILFITTEST_H_
#define MANTID_ALGORITHMS_LEBAILFITTEST_H_
...
public:
  ...
  /*
   * Test the inputs to LeBailFit() and thus the calculation.
   * Test covers
   * (1) Peak parameters
   * (2) Calculated peak positions (d-space) from reflection
   */
  void test_Fit2Peaks()
  {
    ...
    // 4.
    double zero = lbfit.getFittedParameterValue("Zero");
  ...
  void importDataFromColumnFile(std::string filename, std::vector<double>& vecX, std::vector<double>& vecY, std::vector<double>& vecE)
  {
    ...
    if (!ins.is_open())
    {
      std::cout << "Input data file " << filename << " cannot be opened." << std::endl;
      throw std::invalid_argument("Input data file cannot be open. ");
    }
...
#endif /* MANTID_ALGORITHMS_LEBAILFITTEST_H_ */
