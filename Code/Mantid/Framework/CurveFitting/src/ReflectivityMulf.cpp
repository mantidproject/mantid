    ...
    for (size_t j = 0; j < nData; ++j)
    {
      ...
      for (ii=0;ii<nit1;ii++)
      {
         ...
         for (i=1;i<m_nlayer+1;i++)
         {
           btm=betan[i]*ci;
           btm1=-1.0*betan[i]*ci;
           if(imag(btm1) == 0.0)btm=betan[i]*2.0*ci;
           cbtm=exp(btm);
           if(real(btm1) == 0.0)cbtm1=exp(btm1);
           if(imag(btm1) == 0.0)cbtm1=1.0*cr;
           a211=cbtm;
           if((pfn[i]+pfn[i+1]) != c0)a22t=(pfn[i]-pfn[i+1])/(pfn[i]+pfn[i+1]);
           if((pfn[i]+pfn[i+1]) == c0)a22t=c0;
