  ...
  namespace CurveFitting
  {
    ...
    */
      ...
      const DetectorParams & detpar, const ResolutionParams & respar)
    {
      ...
      std::transform(tseconds.begin(), tseconds.end(), tseconds.begin(), std::bind2nd(std::multiplies<double>(),1e-6));
      ...
      std::transform(tseconds.begin(), tseconds.end(), tseconds.begin(), std::bind2nd(std::multiplies<double>(),1e6));
    }
    ...
    */
    void CalculateGammaBackground::retrieveInputs()
    {
      ...
      if(auto composite = boost::dynamic_pointer_cast<CompositeFunction>(profileFunction))
      {
        m_npeaks = composite->nFunctions();
        for(size_t i = 0; i < m_npeaks; ++i)
        {
          auto single = boost::dynamic_pointer_cast<ComptonProfile>(composite->getFunction(i));
          if(!single)
          {
            throw std::invalid_argument("Invalid function. Composite must contain only ComptonProfile functions");
          }
        }
      }
      else
