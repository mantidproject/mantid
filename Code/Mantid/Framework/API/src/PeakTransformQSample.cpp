...
namespace Mantid
{
  ...
  PeakTransformQSample::PeakTransformQSample(const std::string& xPlotLabel, const std::string& yPlotLabel) : PeakTransform(xPlotLabel, yPlotLabel, regex("^Q_sample_x.*$"), regex("^Q_sample_y.*$"), regex("^Q_sample_z.*$")  )
  {
  }

  PeakTransformQSample::PeakTransformQSample(const PeakTransformQSample& other): PeakTransform(other)
  {
  }

  PeakTransformQSample& PeakTransformQSample::operator=(const PeakTransformQSample & other)
  {
    if(this != &other)
    {
  ...
  */
  Mantid::Kernel::V3D PeakTransformQSample::transformPeak(const Mantid::API::IPeak& peak) const
  {
