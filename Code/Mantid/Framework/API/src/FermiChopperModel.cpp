...
namespace Mantid
{
  namespace API
  {
    ...
    {
      if(m_incidentEnergyLog.empty()) return m_incidentEnergy;
      else
      {
     ...
     * @return A time sample from a triangular distribution with peak at min=-1, max=1 and peak at 0
     */
    double FermiChopperModel::sampleTimeDistribution(const double randomNo) const
    {
      ...
      else
      {
        ...
        os << "FermiChopperModel::sampleTimeDistribution - Random number must be flat between [0,1]. Current value=" << randomNo;
   ...
   //----------------------------------------------------------------------------------------
     ...
     */
    void FermiChopperModel::setParameterValue(const std::string & name, const std::string & value)
    {
      ...
      {
        throw std::invalid_argument("FermiChopperModel::setParameterValue - Unknown parameter: " + name);
      }
    }
    ...
    double FermiChopperModel::regimeFactor(const double gamma) const
    {
      ...
      else if(gamma < 4.0)
      {
        ...
        const double numerator = gamma*std::pow((sqrtGamma - 2.0),2)*(sqrtGamma + 8.0);
      ...
      {
        std::ostringstream os;
        os << "FermiChopperModel::regimeFactor - gamma is greater than 4! Behaviour is undefined. Value=" << gamma;
        throw std::invalid_argument(os.str());
      }
    }

    /**
     * Map a flat random number to a triangular distibution of unit area, with max height at zero and [xmin,xmax]=[-1,1]
     * @param randomNo :: A flat random number
     * @returns The number mapped to the triangular distribution
     */
    double FermiChopperModel::sampleFromTriangularDistribution(const double randomNo) const
    {
      double xmin(-1.0);
