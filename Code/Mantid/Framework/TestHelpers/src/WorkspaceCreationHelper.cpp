...
namespace WorkspaceCreationHelper
{
  ...
  Workspace2D_sptr Create2DWorkspaceWithValues(int64_t nHist, int64_t nBins, bool isHist,
      const std::set<int64_t> & maskedWorkspaceIndices, double xVal, double yVal, double eVal)
  {
    MantidVecPtr x1, y1, e1;
   ...
   */
  void addNoise(Mantid::API::MatrixWorkspace_sptr ws, double noise, const double lower,
      const double upper)
  {
      ...
      bool startYNegative, bool isHistogram, const std::string& instrumentName)
  {
    if (includeMonitors && nhist < 2)
    {
      throw std::invalid_argument(
          "Attempting to 2 include monitors for a workspace with fewer than 2 histograms");
    }

    Workspace2D_sptr space;
    if (isHistogram)
      space = Create2DWorkspaceBinned(nhist, nbins); // A 1:1 spectra is created by default
    else
    ...
    Object_sptr pixelShape = ComponentCreationHelper::createCappedCylinder(pixelRadius, 0.02,
        V3D(0.0, 0.0, 0.0), V3D(0., 1.0, 0.), "tube");
    ...
    for (int i = 0; i < ndets; ++i)
    {
      ...
      lexer << "pixel-" << i << ")";
      Detector * physicalPixel = new Detector(lexer.str(), space->getAxis(1)->spectraNo(i), pixelShape,
          testInst.get());
      int ycount(i);
      if (startYNegative)
        ycount -= 1;
    ...
    {
      Detector *monitor1 = new Detector("mon1", space->getAxis(1)->spectraNo(ndets), Object_sptr(),
          testInst.get());
      monitor1->setPos(-9.0, 0.0, 0.0);
      testInst->add(monitor1);
      testInst->markAsMonitor(monitor1);

      Detector *monitor2 = new Detector("mon2", space->getAxis(1)->spectraNo(ndets) + 1, Object_sptr(),
          testInst.get());
   ...
   */
  Mantid::DataObjects::Workspace2D_sptr create2DWorkspaceWithRectangularInstrument(int numBanks,
      int numPixels, int numBins)
  {
   ...
   * @return The EventWorkspace
   */
  Mantid::DataObjects::EventWorkspace_sptr createEventWorkspaceWithFullInstrument(int numBanks,
      int numPixels, bool clearEvents)
  {
    Instrument_sptr inst = ComponentCreationHelper::createTestInstrumentRectangular(numBanks, numPixels);
  ...
  Mantid::DataObjects::EventWorkspace_sptr createEventWorkspaceWithNonUniformInstrument(int numBanks,
      bool clearEvents)
  {
    // Number of detectors in a bank as created by createTestInstrumentCylindrical
    const int DETECTORS_PER_BANK(9);

    Instrument_sptr inst = ComponentCreationHelper::createTestInstrumentCylindrical(numBanks, false,
        0.0025, 0.005);
  ...
  void createInstrumentForWorkspaceWithDistances(MatrixWorkspace_sptr workspace,
      const V3D& samplePosition, const V3D& sourcePosition, const std::vector<V3D>& detectorPositions)
  {
    Instrument_sptr instrument = boost::make_shared<Instrument>();
    instrument->setReferenceFrame(boost::make_shared<ReferenceFrame>(Y, X, Left, "0,0,0"));
   ...
   */
  EventWorkspace_sptr CreateEventWorkspace(int numPixels, int numBins, int numEvents, double x0,
   ...
   */
  EventWorkspace_sptr CreateGroupedEventWorkspace(std::vector<std::vector<int> > groups, int numBins,
      double binDelta)
  {
  ...
  MatrixWorkspace_sptr CreateGroupedWorkspace2D(size_t numHist, int numBins, double binDelta)
  {
    ...
    retVal->setInstrument(
        ComponentCreationHelper::createTestInstrumentCylindrical(static_cast<int>(numHist)));
  ...
  MatrixWorkspace_sptr CreateGroupedWorkspace2DWithRingsAndBoxes(size_t RootOfNumHist, int numBins,
      double binDelta)
  {
  ...
  //
  Mantid::API::MatrixWorkspace_sptr createProcessedWorkspaceWithCylComplexInstrument(size_t numPixels,
      size_t numBins, bool has_oriented_lattice)
  {
  ...
  Mantid::API::MatrixWorkspace_sptr createProcessedInelasticWS(const std::vector<double> &L2,
      const std::vector<double> &polar, const std::vector<double> &azimutal, size_t numBins, double Emin,
      double Emax, double Ei)
  {
    // not used but interface needs it
    std::set<int64_t> maskedWorkspaceIndices;
    size_t numPixels = L2.size();

    Mantid::API::MatrixWorkspace_sptr ws = Create2DWorkspaceWithValues(uint64_t(numPixels),
        uint64_t(numBins), true, maskedWorkspaceIndices, 0, 1, 0.1);

    // detectors at L2, sample at 0 and source at -L2_min
    ws->setInstrument(
        ComponentCreationHelper::createCylInstrumentWithDetInGivenPosisions(L2, polar, azimutal));
  ...
  Mantid::DataObjects::EventWorkspace_sptr createEventWorkspace3(
      Mantid::DataObjects::EventWorkspace_const_sptr sourceWS, std::string wsname, API::Algorithm* alg)
  {
    UNUSED_ARG(wsname);
    // 1. Initialize:use dummy numbers for arguments, for event workspace it doesn't matter
    Mantid::DataObjects::EventWorkspace_sptr outputWS = Mantid::DataObjects::EventWorkspace_sptr(
  ...
  boost::shared_ptr<DataObjects::TableWorkspace> createTableWorkspace(
  ...
  void processDetectorsPositions(const API::MatrixWorkspace_const_sptr &inputWS,
      DataObjects::TableWorkspace_sptr &targWS, double Ei)
  {
    ...
    {
      throw Kernel::Exception::InstrumentDefinitionError(
          "Unable to calculate source-sample distance for workspace", inputWS->getTitle());
    }
    // Instrument name
  ...
  boost::shared_ptr<Mantid::DataObjects::TableWorkspace> buildPreprocessedDetectorsWorkspace(
