...
reduction property manager. It can have the values ''None'', ''ByCurrent'' or
''ByMonitor''. For SNS, monitor workspaces need to be passed. Parameters in
...
name given. All underlined parameters are fixed via other input method. If
normalisation is performed, a sample log called
DirectInelasticReductionNormalisedBy is added to the resulting workspace with
the normalisation procedure used.

=== Workflow ===
[[File:DgsPreprocessDataWorkflow.png]]
