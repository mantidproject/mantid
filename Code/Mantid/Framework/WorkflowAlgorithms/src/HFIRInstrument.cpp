 ...
 */
int getDetectorFromPixel(const int& pixel_x, const int& pixel_y, API::MatrixWorkspace_sptr dataWS)
{
  UNUSED_ARG(dataWS);
...
{
  const int nx_pixels = (int)(readInstrumentParameter("number-of-x-pixels", dataWS));
  const int ny_pixels = (int)(readInstrumentParameter("number-of-y-pixels", dataWS));
