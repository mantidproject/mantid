...
namespace Mantid
{
  namespace DataObjects
  {
    ...
    namespace
    {
       ...
       */
      int64_t calculateCorrectedFullTime(const int64_t& totalNanoseconds, const double& tof,
          const double& tofFactor, const double& tofShift)
      {
    ...
    EventList::EventList(EventWorkspaceMRU * mru, specid_t specNo) :
        IEventList(specNo), eventType(TOF), order(UNSORTED), mru(mru), m_lockedMRU(false)
    {
    }
    ...
    EventList& EventList::operator+=(const std::vector<TofEvent> & more_events)
    {
      ...
      case WEIGHTED:
        ...
        for (std::vector<TofEvent>::const_iterator it = more_events.begin(); it != more_events.end();
            ++it)
    ...
    EventList& EventList::operator+=(const std::vector<WeightedEventNoTime> & more_events)
    {
      ...
      case WEIGHTED_NOTIME:
        ...
        this->weightedEventsNoTime.insert(weightedEventsNoTime.end(), more_events.begin(),
            more_events.end());
        break;
      }
    ...
    void EventList::minusHelper(std::vector<T1> & events, const std::vector<T2> & more_events)
    {
      ...
      for (itev = more_begin; itev != more_end; itev++)
      {
        ...
        events.push_back(
            T1(itev->tof(), itev->pulseTime(), itev->weight() * (-1.0), itev->errorSquared()));
      }
    }
      ...
      case TOF:
        if (eventType != TOF)
          throw std::runtime_error(
              "EventList::switchTo() called on an EventList with weights to go down to TofEvent's. This would remove weight information and therefore is not possible.");
        break;
    ...
    void EventList::switchToWeightedEvents()
    {
      ...
      case WEIGHTED_NOTIME:
        throw std::runtime_error(
            "EventList::switchToWeightedEvents() called on an EventList with WeightedEventNoTime's. It has lost the pulse time information and can't go back to WeightedEvent's.");
        break;
    ...
    const std::vector<TofEvent> & EventList::getEvents() const
    {
      if (eventType != TOF)
        throw std::runtime_error(
            "EventList::getEvents() called for an EventList that has weights. Use getWeightedEvents() or getWeightedEventsNoTime().");
      return this->events;
    }
    ...
    std::vector<WeightedEvent>& EventList::getWeightedEvents()
    {
      if (eventType != WEIGHTED)
        throw std::runtime_error(
            "EventList::getWeightedEvents() called for an EventList not of type WeightedEvent. Use getEvents() or getWeightedEventsNoTime().");
    ...
    std::vector<WeightedEventNoTime>& EventList::getWeightedEventsNoTime()
    {
      if (eventType != WEIGHTED_NOTIME)
        throw std::runtime_error(
            "EventList::getWeightedEvents() called for an EventList not of type WeightedEventNoTime. Use getEvents() or getWeightedEvents().");
    ...
    const std::vector<WeightedEventNoTime>& EventList::getWeightedEventsNoTime() const
    {
      if (eventType != WEIGHTED_NOTIME)
        throw std::runtime_error(
            "EventList::getWeightedEventsNoTime() called for an EventList not of type WeightedEventNoTime. Use getEvents() or getWeightedEvents().");
      return this->weightedEventsNoTime;
    }
    ...
    void EventList::sort(const EventSortType order) const
    {
      ...
      {
        throw std::invalid_argument("sorting by time at sample requires extra parameters. call sortTimeAtSample instead.");
      }
      else
      {
...
//  }
     ...
     *   from a previous run of the same sort type, you need to trigger a full resort using forceResort = true. False by default.
     */
    void EventList::sortTimeAtSample(const double& tofFactor, const double& tofShift, bool forceResort) const
    {
      ...
      case WEIGHTED_NOTIME:
      {
        ...
        std::sort(weightedEventsNoTime.begin(), weightedEventsNoTime.end(), comparitor);
      }
        break;
      }
      //Save the order to avoid unnecessary re-sorting.
    ...
    {
      if (!mru)
        throw std::runtime_error("EventList::constDataY() called with no MRU set. This is not allowed.");
    ...
    {
      if (!mru)
        throw std::runtime_error("EventList::constDataE() called with no MRU set. This is not allowed.");
    ...
    template<class T>
    inline void EventList::compressEventsHelper(const std::vector<T> & events,
        std::vector<WeightedEventNoTime> & out, double tolerance)
    {
    ...
    template<class T>
    void EventList::compressEventsParallelHelper(const std::vector<T> & events,
        std::vector<WeightedEventNoTime> & out, double tolerance)
    {
...
//        else
     ...
     */
    template<class T>
    typename std::vector<T>::const_iterator EventList::findFirstEvent(const std::vector<T> & events,
        const double seek_tof)
    {
     ...
     *
     * @param events :: event vector in which to look.
     * @param seek_time :: seek time to find (typically the first bin X[0]). Seek time in nanoseconds.
    ...
    typename std::vector<T>::const_iterator EventList::findFirstTimeAtSampleEvent(
        const std::vector<T> & events, const double seek_time, const double& tofFactor,
        const double& tofOffset) const
    {
      ...
      //if tof < X[0], that means that you need to skip some events
      while ((itev != itev_end)
          && (calculateCorrectedFullTime(itev->pulseTime().totalNanoseconds(), itev->tof(), tofFactor,
              tofOffset) < seek_time))
        itev++;
      // Better fix would be to use a binary search instead of the linear one used here.
      return itev;
     ...
     */
    template<class T>
    typename std::vector<T>::iterator EventList::findFirstEvent(std::vector<T> & events,
        const double seek_tof)
    {
     ...
     */
    template<class T>
    void EventList::histogramForWeightsHelper(const std::vector<T> & events, const MantidVec & X,
        MantidVec & Y, MantidVec & E)
    {
      ...
      std::transform(E.begin(), E.end(), E.begin(), static_cast<double (*)(double)>(std::sqrt));}
       ...
       */
    void EventList::generateHistogramPulseTime(const MantidVec& X, MantidVec& Y, MantidVec& E,
     ...
     *          events; you can just ignore the returned E vector.
     */
    void EventList::generateHistogramTimeAtSample(const MantidVec& X, MantidVec& Y, MantidVec& E,
        const double& tofFactor, const double& tofOffset, bool skipError) const
    {
      ...
      case WEIGHTED:
        throw std::runtime_error("Cannot histogram by time at sample on Weighted Events currently"); // This could be supported.
     ...
     *        events; you can just ignore the returned E vector.
     */
    void EventList::generateHistogram(const MantidVec& X, MantidVec& Y, MantidVec& E,
        bool skipError) const
    {
    ...
    // --------------------------------------------------------------------------
    /** With respect to Time at Sample, fill a histogram given specified histogram bounds. Does not modify
     * the eventlist (const method).
     * @param X :: The x bins
     * @param Y :: The generated counts histogram
     * @param tofFactor :: time of flight factor
        ...
        const double& tofFactor, const double& tofOffset) const
    {
      ...
      {
        //Iterate through all events (sorted by pulse time)
        std::vector<TofEvent>::const_iterator itev = findFirstTimeAtSampleEvent(this->events, X[0],
            tofFactor, tofOffset);
        std::vector<TofEvent>::const_iterator itev_end = events.end(); //cache for speed
        // The above can still take you to end() if no events above X[0], so check again.
        if (itev == itev_end)
        ...
        while ((itev != itev_end) && (bin < x_size - 1))
        {
          tAtSample = calculateCorrectedFullTime(itev->pulseTime().totalNanoseconds(), itev->tof(),
              tofFactor, tofOffset);
          while (bin < x_size - 1)
          {
            //Within range?
    ...
    void EventList::generateErrorsHistogram(const MantidVec& Y, MantidVec& E) const
    {
      ...
      std::transform(Y.begin(), Y.end(), E.begin(), static_cast<double (*)(double)>(std::sqrt));
     ...
     */
    template<class T>
    void EventList::integrateHelper(std::vector<T> & events, const double minX, const double maxX,
        const bool entireRange, double & sum, double & error)
    {
      sum = 0;
      error = 0;
      //Nothing in the list?
    ...
    void EventList::addPulsetime(const double seconds)
    {
      ...
      case WEIGHTED_NOTIME:
        throw std::runtime_error(
            "EventList::addPulsetime() called on an event list with no pulse times. You must call this algorithm BEFORE CompressEvents.");
        break;
      }
    }
     ...
     */
    template<class T>
    std::size_t EventList::maskTofHelper(std::vector<T> & events, const double tofMin,
        const double tofMax)
    {
    ...
    DateAndTime EventList::getTimeAtSampleMax(const double& tofFactor, const double& tofOffset) const
    {
      ...
      if (this->order == TIMEATSAMPLE_SORT)
      {
        ...
        {
        case TOF:
          return calculateCorrectedFullTime(this->events.rbegin()->pulseTime().totalNanoseconds(),
      ...
      for (size_t i = 0; i < numEvents; i++)
      {
        ...
        {
        case TOF:
          temp = calculateCorrectedFullTime(this->events[i].pulseTime().totalNanoseconds(),
              this->events[i].tof(), tofFactor, tofOffset);
          break;
        case WEIGHTED:
          temp = calculateCorrectedFullTime(this->weightedEvents[i].pulseTime().totalNanoseconds(),
              this->weightedEvents[i].tof(), tofFactor, tofOffset);
          break;
        case WEIGHTED_NOTIME:
          temp = calculateCorrectedFullTime(this->weightedEventsNoTime[i].pulseTime().totalNanoseconds(),
    ...
    DateAndTime EventList::getTimeAtSampleMin(const double& tofFactor, const double& tofOffset) const
    {
      ...
      if (this->order == TIMEATSAMPLE_SORT)
      {
        ...
        {
        case TOF:
          return calculateCorrectedFullTime(this->events.begin()->pulseTime().totalNanoseconds(),
     ...
     * */
    template<class T>
    void EventList::multiplyHistogramHelper(std::vector<T> & events, const MantidVec & X,
        const MantidVec & Y, const MantidVec & E)
    {
      //Validate inputs
      if ((X.size() < 2) || (Y.size() != E.size()) || (X.size() != 1 + Y.size()))
        throw std::invalid_argument(
            "EventList::multiply() was given invalid size or inconsistent histogram arrays.");
     ...
     * */
    template<class T>
    void EventList::divideHistogramHelper(std::vector<T> & events, const MantidVec & X,
        const MantidVec & Y, const MantidVec & E)
    {
      //Validate inputs
      if ((X.size() < 2) || (Y.size() != E.size()) || (X.size() != 1 + Y.size()))
        throw std::invalid_argument(
            "EventList::divide() was given invalid size or inconsistent histogram arrays.");
     ...
     */
    template<class T>
    void EventList::filterByTimeAtSampleHelper(std::vector<T> & events, DateAndTime start,
        DateAndTime stop, double tofFactor, double tofOffset, std::vector<T> & output)
    {
      typename std::vector<T>::iterator itev = events.begin();
      typename std::vector<T>::iterator itev_end = events.end();
      //Find the first event with m_pulsetime >= start
      while ((itev != itev_end)
          && (calculateCorrectedFullTime(itev->m_pulsetime.totalNanoseconds(), itev->tof(), tofFactor,
    ...
    void EventList::filterByPulseTime(DateAndTime start, DateAndTime stop, EventList & output) const
    {
      ...
      case WEIGHTED_NOTIME:
        throw std::runtime_error(
            "EventList::filterByPulseTime() called on an EventList that no longer has time information.");
        break;
      }

    }

    void EventList::filterByTimeAtSample(Kernel::DateAndTime start, Kernel::DateAndTime stop,
        double tofFactor, double tofOffset, EventList & output) const
    {
      if (this == &output)
      {
        throw std::invalid_argument("In-place filtering is not allowed");
      ...
      case TOF:
        ...
        break;
      case WEIGHTED:
        filterByTimeAtSampleHelper(this->weightedEvents, start, stop, tofFactor, tofOffset,
            output.weightedEvents);
        break;
      case WEIGHTED_NOTIME:
        throw std::runtime_error(
            "EventList::filterByTimeAtSample() called on an EventList that no longer has full time information.");
        break;
      }
    }
    ...
    void EventList::filterInPlace(Kernel::TimeSplitterType & splitter)
    {
      ...
      case WEIGHTED_NOTIME:
        throw std::runtime_error(
            "EventList::filterInPlace() called on an EventList that no longer has time information.");
        break;
      }
    }
    ...
    void EventList::splitByTimeHelper(Kernel::TimeSplitterType & splitter,
        std::vector<EventList *> outputs, typename std::vector<T> & events) const
    {
        ...
        std::vector<EventList *> outputs) const
    {
      if (eventType == WEIGHTED_NOTIME)
        throw std::runtime_error(
            "EventList::splitByTime() called on an EventList that no longer has time information.");
    ...
    void EventList::splitByFullTimeHelper(Kernel::TimeSplitterType & splitter,
        std::map<int, EventList *> outputs, typename std::vector<T> & events, bool docorrection,
        double toffactor, double tofshift) const
    {
      ...
      while (itspl != itspl_end)
      {
        ...
        while (itev != itev_end)
        {
          int64_t fulltime;
          if (docorrection)
            fulltime = calculateCorrectedFullTime(itev->m_pulsetime.totalNanoseconds(), itev->m_tof,
                toffactor, tofshift);
          else
    ...
    void EventList::splitByFullTime(Kernel::TimeSplitterType & splitter,
        std::map<int, EventList *> outputs, bool docorrection, double toffactor, double tofshift) const
    {
      if (eventType == WEIGHTED_NOTIME)
        throw std::runtime_error(
            "EventList::splitByTime() called on an EventList that no longer has time information.");
      ...
      else
      {
        ...
        case TOF:
          ...
          break;
        case WEIGHTED:
          splitByFullTimeHelper(splitter, outputs, this->weightedEvents, docorrection, toffactor,
              tofshift);
          break;
        case WEIGHTED_NOTIME:
          break;
    ...
    std::string EventList::splitByFullTimeVectorSplitterHelper(const std::vector<int64_t>& vectimes,
        ...
        typename std::vector<T> & vecEvents, bool docorrection, double toffactor, double tofshift) const
    {
      ...
      for (eviter = vecEvents.begin(); eviter != vecEvents.end(); ++eviter)
      {
        ...
        // Search in vector
        int index = static_cast<int>(lower_bound(vectimes.begin(), vectimes.end(), evabstimens)
            - vectimes.begin());
        int group;
     ...
     */
    std::string EventList::splitByFullTimeMatrixSplitter(const std::vector<int64_t>& vectimes,
        const std::vector<int>& vecgroups, std::map<int, EventList*> vec_outputEventList,
        bool docorrection, double toffactor, double tofshift) const
    {
      ...
      else
      {
        ...
        {
        case TOF:
          debugmessage = splitByFullTimeVectorSplitterHelper(vectimes, vecgroups, vec_outputEventList,
    ...
    void EventList::convertUnitsViaTofHelper(typename std::vector<T> & events,
        Mantid::Kernel::Unit * fromUnit, Mantid::Kernel::Unit * toUnit)
    {
      typename std::vector<T>::iterator itev = events.begin();
      typename std::vector<T>::iterator itev_end = events.end();
     ...
     */
    template<class T>
    void EventList::convertUnitsQuicklyHelper(typename std::vector<T> & events, const double& factor,
        const double& power)
    {
      typename std::vector<T>::iterator itev = events.begin();
      typename std::vector<T>::iterator itev_end = events.end();
