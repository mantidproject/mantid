...
class DummyCatalog : public ICatalog
{
  public:
    ...
    void search(const CatalogSearchParam&,ITableWorkspace_sptr& ws,const int &,const int &)
    {
      // Each implementation of the ICatalog methods should always append data to the workspace.
      ws->appendRow();
      m_counter++;
    }
    ...
    {
      m_counter++;
      // Return 5 to verify that the NumberOfResults gets added together across catalogs as expected.
      return 5;
    }
...
class CompositeCatalogTest : public CxxTest::TestSuite
{
  public:
    ...
    {
      CatalogSearchParam params;
      // The number of results returned by getNumberOfSearchResults is hard-coded to 5
      int64_t numberOfResults = createCompositeCatalog()->getNumberOfSearchResults(params);
      // As two catalogs are created in createCompositeCatalog we verify that getNumberOfSearchResults functions correctly.
  ...
  private:
    ...
    {
      const boost::shared_ptr<CompositeCatalog>compositeCatalog(new CompositeCatalog());
