...
void EQSANSTofStructure::execEvent(Mantid::DataObjects::EventWorkspace_sptr inputWS, double threshold,
    double frame_offset, double tof_frame_width, double tmp_frame_width, bool frame_skipping)
{
...
double EQSANSTofStructure::getTofOffset(EventWorkspace_const_sptr inputWS, bool frame_skipping)
{
  ...
  if (frame_skipping) g_log.information() << " + " << frameskip_wl_1 << " " << frameskip_wl_2 << std::endl;
  else g_log.information() << std::endl;
