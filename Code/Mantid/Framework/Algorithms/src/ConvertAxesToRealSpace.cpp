...
namespace Algorithms
{
  ...
  const std::string ConvertAxesToRealSpace::summary() const { return "Converts the spectrum and TOF axes to real space values, integrating the data in the process";};
  ...
  void ConvertAxesToRealSpace::init()
  {
    ...
    declareProperty(new WorkspaceProperty<Workspace2D>("OutputWorkspace","",Direction::Output), "An output workspace.");
    ...
    declareProperty("VerticalAxis", "y", boost::make_shared<StringListValidator>(propOptions),
      "What will be the vertical axis ?\n"
       );    
    declareProperty("HorizontalAxis", "2theta", boost::make_shared<StringListValidator>(propOptions),
      "What will be the horizontal axis?\n"
       );

    declareProperty(new Kernel::PropertyWithValue<int>("NumberVerticalBins", 100), "The number of bins along the vertical axis.");
    declareProperty(new Kernel::PropertyWithValue<int>("NumberHorizontalBins", 100), "The number of bins along the horizontal axis.");
  }
  ...
  void ConvertAxesToRealSpace::exec()
  {
    ...
    MatrixWorkspace_sptr outputWs = WorkspaceFactory::Instance().create(inputWs, axisVector[1].bins,axisVector[0].bins,axisVector[0].bins);
    ...
    for (int i = 0; i < nHist; ++i)
    {
      try 
      {
        ...
        for (int axisIndex = 0; axisIndex < 2; ++axisIndex)
        {
          ...
          if (axisValue > axisVector[axisIndex].max) axisVector[axisIndex].max = axisValue;
          if (axisValue < axisVector[axisIndex].min) axisVector[axisIndex].min = axisValue;
        }
      }
    ...
    boost::shared_ptr<Units::Label> xlabel = boost::dynamic_pointer_cast<Units::Label>(xUnit);
    ...
    boost::shared_ptr<Units::Label> ylabel = boost::dynamic_pointer_cast<Units::Label>(UnitFactory::Instance().create("Label"));
    ylabel->setLabel(axisVector[1].label, m_unitMap[axisVector[1].label]);
    yAxis->unit() = ylabel;
    outputWs->replaceAxis(1,yAxis);
    
    //work out where to put the data into the output workspace, but don't do it yet as that needs to be single threaded
    ...
    for (int i = 0; i < nHist; ++i)
    {
      ...
      {
        int xIndex = static_cast<int>(std::distance(x->begin(), std::lower_bound(x->begin(), x->end(), dataVector[i].horizontalValue)));
        if (xIndex > 0) --xIndex;
        int yIndex = static_cast<int>(std::distance(y->begin(), std::lower_bound(y->begin(), y->end(), dataVector[i].verticalValue)));
        if (yIndex > 0) --yIndex;
    ...
    for (int i = 0; i < nOutputHist; ++i)
    {
      ...
      std::transform(errorVec.begin(), errorVec.end(), errorVec.begin(), (double(*)(double)) sqrt);
  ...
  /** Fills the values in an axis linearly from min to max for a given number of steps
    ...
    * @param isHistogram true if the data should be a histogram rather than point data
    */
  ...
  /** Fills the unit map and ordered vector with the same data
    ...
    */
  void ConvertAxesToRealSpace::fillUnitMap(std::vector<std::string>& orderedVector, std::map <std::string,std::string>& unitMap, const std::string& caption, const std::string& unit)
  {
