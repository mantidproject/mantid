...
namespace Algorithms
{
  ...
  void ConvertSpectrumAxis2::init()
  {
    ...
    declareProperty("Target","",boost::make_shared<StringListValidator>(targetOptions),
      "The unit to which spectrum axis is converted to - \"theta\" (for the angle in degrees), Q or Q^2, where elastic Q is evaluated at EFixed. Note that 'theta' and 'signed_theta' are there for compatibility purposes; they are the same as 'Theta' and 'SignedTheta' respectively");
  ...
  void ConvertSpectrumAxis2::exec()
  {
    ...
    if ( isHist ) { m_nxBins = m_nBins+1; }
    else { m_nxBins = m_nBins; }
  ...
  {
    //Set up binding to member funtion. Avoids condition as part of loop over nHistograms.
    boost::function<double(IDetector_const_sptr)> thetaFunction;
    if ( targetUnit.compare("signed_theta") == 0 || targetUnit.compare("SignedTheta") == 0 )
    {
      thetaFunction = boost::bind(&MatrixWorkspace::detectorSignedTwoTheta, m_inputWS, _1);
    }
    else if ( targetUnit == "theta" || targetUnit == "Theta" )
    {
      thetaFunction = boost::bind(&MatrixWorkspace::detectorTwoTheta, m_inputWS, _1);
    }
  ...
  MatrixWorkspace_sptr ConvertSpectrumAxis2::createOutputWorkspace(const std::string & targetUnit)
  {
    // Create the output workspace. Can not re-use the input one because the spectra are re-ordered.
    MatrixWorkspace_sptr outputWorkspace = WorkspaceFactory::Instance().create(m_inputWS, m_indexMap.size(),m_nxBins,m_nBins);
    ...
    for (it = m_indexMap.begin(); it != m_indexMap.end(); ++it)
    {
      ...
      outputWorkspace->getSpectrum(currentIndex)->copyInfoFrom(*m_inputWS->getSpectrum(it->second));
      ++currentIndex;
    }
    return outputWorkspace;
  }
  
  double ConvertSpectrumAxis2::getEfixed(IDetector_const_sptr detector, MatrixWorkspace_const_sptr inputWS, int emode) const
  {
    double efixed(0);
