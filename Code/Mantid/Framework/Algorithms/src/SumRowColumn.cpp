...
void SumRowColumn::init()
{
  ...
  declareProperty("H/V_Min",EMPTY_INT(),mustBePositive);
  declareProperty("H/V_Max",EMPTY_INT(),mustBePositive->clone());
}
...
void SumRowColumn::exec()
{
  ...
  int start = getProperty("H/V_Min");
  int end = getProperty("H/V_Max");
