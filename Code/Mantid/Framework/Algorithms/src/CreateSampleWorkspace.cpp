...
namespace Algorithms
{
  ...
  void CreateSampleWorkspace::init()
  {
    ...
    declareProperty("WorkspaceType","Histogram",boost::make_shared<StringListValidator>(typeOptions),
      "The type of workspace to create (default: Histogram)");
    
    //pre-defined function strings these use $PCx$ to define peak centres values that will be replaced before use
    //$PC0$ is the far left of the data, and $PC10$ is the far right, and therefore will often not be used
    //$PC5$ is the centre of the data
    m_preDefinedFunctionmap.insert(std::pair<std::string,std::string>("One Peak","name=LinearBackground, A0=0.3; name=Gaussian, PeakCentre=$PC5$, Height=10, Sigma=0.7;"));
    m_preDefinedFunctionmap.insert(std::pair<std::string,std::string>("Multiple Peaks","name=LinearBackground, A0=0.3;name=Gaussian, PeakCentre=$PC3$, Height=10, Sigma=0.7;name=Gaussian, PeakCentre=$PC6$, Height=8, Sigma=0.5"));
    m_preDefinedFunctionmap.insert(std::pair<std::string,std::string>("Flat background","name=LinearBackground, A0=1;"));
    m_preDefinedFunctionmap.insert(std::pair<std::string,std::string>("Exp Decay","name=ExpDecay, Height=100, Lifetime=1000;"));
    m_preDefinedFunctionmap.insert(std::pair<std::string,std::string>("User Defined",""));
    std::vector<std::string> functionOptions;
    for(auto iterator = m_preDefinedFunctionmap.begin(); iterator != m_preDefinedFunctionmap.end(); iterator++) {
    ...
    declareProperty("BinWidth",200.0, boost::make_shared<BoundedValidator<double> >(0,100000,true),
  ...
  void CreateSampleWorkspace::exec()
  {
    ...
    Instrument_sptr inst = createTestInstrumentRectangular(numBanks, bankPixelWidth);
    int num_bins = static_cast<int>((xMax-xMin)/binWidth);
    MatrixWorkspace_sptr ws;
    if (wsType=="Event")
    {
      ws = createEventWorkspace(numBanks*bankPixelWidth*bankPixelWidth, num_bins,numEvents, 
                                           ...
                                           const std::string& functionString, bool isRandom)
  {
    ...
    uf dblSqrt = std::sqrt;
    std::transform(y.access().begin(), y.access().end(), e.access().begin(), dblSqrt);
   ...
   */
  EventWorkspace_sptr CreateSampleWorkspace::createEventWorkspace(int numPixels,
                                           ...
                                           const std::string& functionString, bool isRandom)
  {
    ...
    //we need to normalise the results and then multiply by the number of events to find the events per bin
   ...
   * @param noiseScale :: A scaling factor for niose to be added to the data, 0= no noise
   * @returns the calculated values
   */
  std::vector<double> CreateSampleWorkspace::evalFunction(const std::string& functionString, const std::vector<double>& xVal, double noiseScale = 0)
  {
   ...
   */
  Instrument_sptr CreateSampleWorkspace::createTestInstrumentRectangular(int num_banks, int pixels, double pixelSpacing)
  {
    ...
    // One object
    Object_sptr pixelShape = createCappedCylinder(cylRadius, cylHeight, V3D(0.0,-cylHeight/2.0,0.0), V3D(0.,1.0,0.), "pixel-shape");
   ...
   */
  Object_sptr CreateSampleWorkspace::createCappedCylinder(double radius, double height, const V3D & baseCentre, const V3D & axis, const std::string & id)
  {
   ...
   */
  Object_sptr CreateSampleWorkspace::createSphere(double radius, const V3D & centre, const std::string & id)
  {
    ShapeFactory shapeMaker;
    std::ostringstream xml;
    xml << "<sphere id=\"" << id <<  "\">"
