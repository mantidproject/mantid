...
Resolution of a detector in d-spacing is defined as <math>\frac{\Delta d}{d}</math>,
which is constant for an individual detector.

Starting from the Bragg equation for T.O.F. diffractometer,
:<math>d = \frac{t}{252.777\cdot L\cdot2\sin\theta}</math>

as
:<math>\Delta d = \sqrt{(\Delta T)^2 + (\Delta L)^2 + (\Delta\theta)^2}</math>

and thus
:<math>\frac{\Delta d}{d} = \sqrt{(\frac{\Delta T}{T})^2 + (\frac{\Delta L}{L})^2 + (\Delta\theta\cdot\cot(\theta))^2}</math>

where,
* <math>\Delta T</math> is the time resolution from modulator;
* <math>\Delta\theta</math> is the coverage of the detector, and can be approximated from the square root of the solid angle of the detector to sample;
* <math>L</math> is the flight path of the neutron from source to detector.
...
Range of <math>\Delta\theta\cot\theta</math>: <math>(0.00170783, 0.0167497)</math>
