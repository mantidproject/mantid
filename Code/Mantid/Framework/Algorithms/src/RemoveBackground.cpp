    ...
    void RemoveBackground::exec()
    {
      ...
      if(pInstrument)
      {
        if(!pInstrument->getSample())
          throw std::invalid_argument(" Workspace: "+inputWS->getName()+" does not have properly defined instrument. Can not remove background");
      }

      if(!(bkgWksp->getNumberHistograms() == 1 || inputWS->getNumberHistograms()==bkgWksp->getNumberHistograms()))
      {
        throw std::invalid_argument(" Background Workspace: "+bkgWksp->getName()+" should have the same number of spectra as source workspace or be a single histogram workspace");
      }
      ...
      if(!inPlace)
      {
        // make the copy of the output Workspace from the input. Also copies X-vectors and axis
