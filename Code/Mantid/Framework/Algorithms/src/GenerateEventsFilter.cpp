...
namespace Algorithms
{
                                                       ...
                                                       DateAndTime runend)
  {
    ...
    for (size_t i = 0; i < numlogentries; ++i)
    {
      ...
      if (statuschanged) pregroup = currgroup;
    ...
    if (pregroup >= 0)
    {
      ...
      if (splitstarttime.totalNanoseconds() == 0)
        throw runtime_error("Programming logic error (1).");
      makeSplitterInVector(m_vecSplitterTime, m_vecSplitterGroup, splitstarttime, runend, pregroup,
                           timetolns, laststoptime);
    }
  ...
  void GenerateEventsFilter::generateSplittersInMatrixWorkspaceParallel()
  {
    ...
    for (size_t i = 0; i < numThreads; ++i)
    {
      ...
      g_log.debug() << "[DB] Thread " << i << " have " << vecGroupIndexSet[i].size()
                    << " splitter " << "\n";
    }
    ++ numtimes;
