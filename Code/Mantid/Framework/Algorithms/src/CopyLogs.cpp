...
Note that this will not concatenate values or ranges. The algorithm will choose to keep the value of any the log already present in the output workspace, leaving it untouched.
...
namespace Algorithms
{
  ...
  void CopyLogs::init()
  {
    declareProperty(new WorkspaceProperty<>("InputWorkspace","",Direction::Input), "Workspace to copy logs from.");
    declareProperty(new WorkspaceProperty<>("OutputWorkspace","",Direction::InOut), "Workspace to copy logs too.");
    ...
    auto strategiesValidator = boost::make_shared<StringListValidator>(strategies);
    declareProperty("MergeStrategy","MergeReplaceExisting", strategiesValidator, "The type of merge strategy to use on the logs");
  }
