...
void Q1DTOF::init()
{
  ...
  declareProperty(new WorkspaceProperty<>("CorrectionWorkspace","",Direction::Input,wsValidator->clone()));
...
void Q1DTOF::exec()
{
      ...
      for ( int j = 0; j < xLength-1; j++)
      {
        ...
        if (iq>=0 && iq < sizeOut-1 && YCorr[j]>0.0)
        {
          ...
          PARALLEL_CRITICAL(iq)    /* Write to shared memory - must protect */
          {
            ...
            ENorm[iq] += 1.0/(YCorr[j]*YCorr[j]*YCorr[j]*YCorr[j])*ECorr[j]*ECorr[j];
          }
        }
      }
  ...
  {
    YOut[i] /= XNorm[i];
    EOut[i] = sqrt(  EOut[i] + YOut[i]*YOut[i]/(XNorm[i]*XNorm[i])*ENorm[i] ) / XNorm[i];
  }
