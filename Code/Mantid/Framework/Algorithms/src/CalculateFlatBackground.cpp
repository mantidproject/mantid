    ...
    namespace Algorithms
    {
        ...
        void CalculateFlatBackground::exec()
        {
            ...
            for (specIt = specInds.begin(); specIt != specInds.end(); ++specIt)
            {
                ...
                {
                    if (skipMonitors)
                    {  // this will fail in Windows ReleaseWithDebug info mode if the loop above made parallel. MS2012 Compiler bug.
                        // the remedy in this case would be to extract the code within internal true into separate routine. 
                        try
                        {
