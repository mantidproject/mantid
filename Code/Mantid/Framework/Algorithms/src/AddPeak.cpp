...
namespace Algorithms
{
  ...
  void AddPeak::init()
  {
    declareProperty(new WorkspaceProperty<IPeaksWorkspace>("PeaksWorkspace","",Direction::InOut), "A peaks workspace.");
    declareProperty(new WorkspaceProperty<MatrixWorkspace>("RunWorkspace","",Direction::Input),
  ...
  void AddPeak::exec()
  {
    ...
    Mantid::API::IPeak* peak = peaksWS->createPeak(Mantid::Kernel::V3D(Qx,Qy,Qz),l2);
    ...
    if(height > 0.) peak->setSigmaIntensity(std::sqrt(height));
