...
namespace Mantid
{
  namespace Algorithms
  {
    ...
    void UnaryOperation::execEvent()
    {
      ...
      const MatrixWorkspace_const_sptr matrixInputWS = this->getProperty(inputPropName());
      EventWorkspace_const_sptr inputWS= boost::dynamic_pointer_cast<const EventWorkspace>(matrixInputWS);

      // generate the output workspace pointer
      API::MatrixWorkspace_sptr matrixOutputWS = this->getProperty(outputPropName());
      ...
      {
        PARALLEL_START_INTERUPT_REGION
        //switch to weighted events if needed, and use the appropriate helper function
