...
public:
  ...
  {
    using namespace Mantid::API;
    // Instrument as 2 detectors but first spectrum by default only knows about first one
    auto inputWS = createTestWorkspace();
...
private:
  ...
  Mantid::API::MatrixWorkspace_sptr createTestWorkspace()
  {
    ...
    boost::shared_ptr<Object> shape = ShapeFactory().createShape(xmlShape, addTypeTag);
