...
Mantid::API::MatrixWorkspace_sptr createTestWorkspace(const size_t nhist,
  ...
  if (isModerator) {
    ws2d = WorkspaceCreationHelper::Create2DWorkspaceFromFunction(twos(), static_cast<int>(nhist), x0,x1,dx);
  } else {
    ws2d = WorkspaceCreationHelper::Create2DWorkspaceFromFunction(ones(), static_cast<int>(nhist), x0,x1,dx);
  }
...
public:
  ...
  auto testWorkspace = createTestWorkspace(1, 0, 3, 1, "", false,
                                            collimationLengthCorrection,
                                            collimationLengthIncrement,
                                            guideCutoff,
                                            numberOfGuides,
  ...
  auto sigmaModerator = createTestWorkspace(1, 0, 3, 1, "", true,
                                            collimationLengthCorrection,
                                            collimationLengthIncrement,
                                            guideCutoff,
                                            numberOfGuides,
