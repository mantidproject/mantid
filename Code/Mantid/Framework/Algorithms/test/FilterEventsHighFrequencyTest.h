...
public:
  ...
  void test_FilterEvents(){

    std::cout << "\nTest Filter Events" << std::endl;

    // 1. Set up
    DataObjects::EventWorkspace_sptr eventWS(new DataObjects::EventWorkspace());
    AnalysisDataService::Instance().addOrReplace("RawData", eventWS);
    std::string logname = "DummySensor";
    std::string calibfilename = "testcal.dat";
    double ssoffset = 0.0;
    double value_low = 0.9;
    double value_upp = 1.1;
    std::string outputWSname = "TestOutput";
    double t0 = 0.0;
    double tf = 9.9;
    ...
    buildEventWorkspace(eventWS, run_start, numevents);
    addTimeSeriesLog(eventWS, calibfilename, run_start, logname, lenlog);
    ...
    // 3. Set up
    alg.setPropertyValue("InputEventWorkspace", "RawData");
    ...
    */
    ...
    /*
    for (size_t i=0; i<eventsIn0.getNumberEvents()/10; i++){
      DataObjects::WeightedEvent ei0 = eventsIn0.getEvent(i);
      int64_t abstime = ei0.m_pulsetime.total_nanoseconds()+static_cast<int64_t>(ei0.m_tof*1000.0);
      Kernel::DateAndTime abstimex(abstime);
      std::cout << "(In) Event " << i << " = " << ei0.m_pulsetime << ", " << ei0.m_tof << " / " << abstime << " / " << abstimex << std::endl;
    }
    */
  ...
  void buildEventWorkspace(DataObjects::EventWorkspace_sptr iws, Kernel::DateAndTime run_start, size_t numevents){
    ...
    for (detid2det_map::iterator it=detector_map.begin(); it!=detector_map.end(); ++it){
      if (it->second->isMonitor()){
        std::cout << "Detector " << it->first << " is montior" << std::endl;
      }
      ...
      if (it->first > 0){
        ...
        // std::cout << "Spectrum " << wsindex+1 << "  -->  Detector ID = " << detectorid << std::endl;
        wsindex ++;
      }
    }
   ...
   */
  void addTimeSeriesLog(DataObjects::EventWorkspace_sptr iws, std::string calibfilename, Kernel::DateAndTime run_start,
      std::string logname, size_t lenlog){
    ...
    for (size_t i = 0; i < lenlog; i ++){
      ...
      // std::cout << "VZ Here: time added = " << st << ", " << logtime << std::endl;
