...
public:
  ...
  void createLogWksp() {
      ...
      {
        auto alg = FrameworkManager::Instance().createAlgorithm("CreateWorkspace");
  ...
  void addRow(ITableWorkspace_sptr wksp, double freq, double wl, int bank, int van, int can,
              int empty, std::string dmin, std::string dmax, double tofmin, double tofmax) {
  ...
  ITableWorkspace_sptr createTableWksp(bool full) {
    ...
    if (full) {
      addRow(wksp, 60., 0.533, 1, 17702, 17711, 0, "0.05",  "2.20",  0000.00, 16666.67);
      addRow(wksp, 60., 1.333, 3, 17703, 17712, 0, "0.43",  "5.40", 12500.00, 29166.67);
      addRow(wksp, 60., 2.665, 4, 17704, 17713, 0, "1.15",  "9.20", 33333.33, 50000.00);
      addRow(wksp, 60., 4.797, 5, 17705, 17714, 0, "2.00", "15.35", 66666.67, 83333.67);
    }

    return wksp;
  }

  PropertyManager_sptr createExpectedInfo(const double freq, const double wl, const int bank,
      ...
      expectedInfo->declareProperty(new PropertyWithValue<double>("frequency", freq));
      expectedInfo->declareProperty(new PropertyWithValue<double>("wavelength", wl));
      expectedInfo->declareProperty(new PropertyWithValue<int>("bank", bank));
      expectedInfo->declareProperty(new PropertyWithValue<int32_t>("vanadium", van));
      expectedInfo->declareProperty(new PropertyWithValue<int32_t>("container", can));
      expectedInfo->declareProperty(new PropertyWithValue<int32_t>("empty", empty));
      expectedInfo->declareProperty(new ArrayProperty<std::vector<double>>("d_min"));
      expectedInfo->setPropertyValue("d_min", dmin);
      expectedInfo->declareProperty(new ArrayProperty<std::vector<double>>("d_max"));
      expectedInfo->setPropertyValue("d_max", dmax);
      expectedInfo->declareProperty(new PropertyWithValue<double>("tof_min", tofmin));
      expectedInfo->declareProperty(new PropertyWithValue<double>("tof_max", tofmax));

      return expectedInfo;
  }

  void compareResult(PropertyManager_sptr expected, PropertyManager_sptr observed) {
  ...
  void testEmptyChar()
  {
      ...
      TS_ASSERT_THROWS_NOTHING( alg.setPropertyValue("ReductionProperties", m_propertyManagerName) );
      TS_ASSERT_THROWS_NOTHING( alg.execute(); );
      TS_ASSERT( alg.isExecuted() );

      auto expectedInfo = createExpectedInfo(0., 0., 1, 0, 0, 0, "", "", 0., 0.);

      compareResult(expectedInfo,
                    PropertyManagerDataService::Instance().retrieve(m_propertyManagerName));
  }
  ...
  void testFullChar() {
      ...
      auto expectedInfo = createExpectedInfo(60., 0.533, 1, 17702, 17711, 0, "0.05", "2.20", 0000.00, 16666.67);
  ...
  void testFullCharDisableChar() {
      ...
      auto expectedInfo = createExpectedInfo(60., 0.533, 1, 0, 0, 0, "0.05", "2.20", 0000.00, 16666.67);
