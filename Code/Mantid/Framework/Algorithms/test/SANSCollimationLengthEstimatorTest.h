 ...
 */
Mantid::Geometry::Instrument_sptr createTestInstrument(const Mantid::detid_t id,
 ...
 */
void setInstrumentParametersForTOFSANS(const Mantid::API::MatrixWorkspace_sptr ws,
 ...
 */
void addSampleLog(Mantid::API::MatrixWorkspace_sptr workspace, std::string sampleLogName, double value, unsigned int length) {
 ...
 */
Mantid::API::MatrixWorkspace_sptr createTestWorkspace(const size_t nhist,
                                                      const double x0,
                                                      const double x1,
                                                      const double dx,
                                                      std::string methodType = "",
...
void test_that_collimation_length_is_provided_for_simple_instrument_without_guides() {
  ...
  auto testWorkspace = createTestWorkspace(10, 0, 10, 0.1, "",
                                            collimationLengthCorrection,
                                            collimationLengthIncrement,
                                            guideCutoff,
                                            numberOfGuides,
...
void test_that_invalid_collimation_method_throws_an_error() {
  ...
  auto collimationLengthEstimator = SANSCollimationLengthEstimator();
  // Act + Assert
  TSM_ASSERT_THROWS("Should throw an exception since we don't have the requested method implemneted",
...
void test_that_only_3_log_guides_are_all_picked_up_and_contribute() {
  ...
  auto length = collimationLengthEstimator.provideCollimationLength(testWorkspace);
  // Assert
  auto expectedCollimationLength = V3D(sourcePosition - samplePosition).norm() - collimationLengthCorrection + 3*collimationLengthIncrement;
...
void test_that_only_1_log_guides_is_picked_up_and_contributes() {
  ...
  auto length = collimationLengthEstimator.provideCollimationLength(testWorkspace);
  // Assert
  auto expectedCollimationLength = V3D(sourcePosition - samplePosition).norm() - collimationLengthCorrection + 1*collimationLengthIncrement;
  TSM_ASSERT_EQUALS("Should have a collimation length of 5+2*3", length, expectedCollimationLength);
}
