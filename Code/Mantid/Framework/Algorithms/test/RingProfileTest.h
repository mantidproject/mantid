...
public:
  ...
  void test_wrongInputs()
  {
    ...
    TS_ASSERT_THROWS(alg.setProperty("StartAngle",500.0), std::invalid_argument); 
  ...
  /**
    ...
    \/
     ...
     With the limits being: X = [ -0.3, -0.18, -0.06, 0.06, 0.18, 0.3 ] x0=-0.3,delta=0.12
                            Y = [-0.24, -0.12,  0   , 0.12, 0.24]


     To be easy to have a visual interpretation I will put the Y axis growing towards up: 
    Y
     | 0 3 0 4 0      
     | 3 0 4 0 4
     | 0 3 0 1 0
  ...
  void test__profile_of_2d_workspace_startangle_minus45_anticlock(){
    ...
    // set start angle = -45
    // set NumBins = 4
    configure_ring_profile(alg, goodWS, std::vector<double>(2,0.0), 
                           4, -45.0, 0.115, 0.13, true);
  ...
  void test__profile_of_2d_workspace_bigger_ring_and_more_bins(){
    ...
    centre[0] = 0.18; 
    centre[1] = 0.18;
    // selecting 10 bins;  centre 0,0 ; start_angle = -10; low_ring = 0; high_ring=10; anti-clock; 
    configure_ring_profile(alg, goodWS, centre, 
                           num_bins, 0, 0.05, 0.12, true);
...
/**
   This will create the following input 2D workspace with rectangular instrument: 
  ...
  MatrixWorkspace_sptr create_rectangular_instrument_workspace(){
    ...
    for (int i = 0; i<25; i++){
      ...
      std::cout << "ind " << i << " -> theta = " << t << " phi = " << p << " r = " << r << std::endl; 
    }
  ...
  void test__profile_of_rectangular_startangle_minus45_anticlock(){
    MatrixWorkspace_sptr goodWS = create_rectangular_instrument_workspace(); 
    RingProfile alg;         
    // put the centre in the midle of the panel, but in the origin in Z (X=0.016, Y= 0.016, Z=0.0)    
    std::vector<double>centre(3,0.016);
    centre[2] = 0.0; 
    // selecting 4 bins from -45 degrees in anti-clock sense
    // select the minRadius and maxRadius (the thetha limits) small to get only the values 1,2,3,4
