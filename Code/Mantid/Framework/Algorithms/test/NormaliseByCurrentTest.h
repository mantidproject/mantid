...
private:
  ...
  /// Helper method to add necessary log values to simulate multi-period data. The algorithm uses these logs to determien how to normalise by the current.
  void addMultiPeriodLogsTo(MatrixWorkspace_sptr ws, int period, const std::string& protonCharges)
  {
...
public:
  ...
  MatrixWorkspace_const_sptr doTest(MatrixWorkspace_sptr inWS, std::string wsNameOut, double expectedY, double expectedE)
  {
  ...
  void testMultiPeriodData()
  {
    ...
    // Note that CreateWorkspace123 creates uniform signal value of 2.0, and uniform error value of 3.0.
    ...
    // Check that normalisation has used the protonChargeByPeriod data. i.e Yout = Yin/2.0, Yout = Yin/4.0, ... for each period workspace.
  ...
  void testThrowsWithoutCURRENT_PERIOD_Log()
  {
    ...
    TSM_ASSERT("Should not execute to completion without current_period if it's supposed to be multiperiod data.", !alg.execute());
    TSM_ASSERT("Should not execute to completion without current_period if it's supposed to be multiperiod data.", !alg.isExecuted());
  }
  ...
  void testThrowsWithoutPROTON_CHARGE_BY_PERIOD_Log()
  {
    ...
    ws->setYUnit("Counts");
    addMultiPeriodLogsTo(ws, 1, protonChargeByPeriod); // If this worked, we would be normalising by a charge of 2.0
    ws->mutableRun().removeLogData("proton_charge_by_period"); // proton_charge_by_period log data is now gone!

    NormaliseByCurrent alg;
    alg.initialize();
    alg.setProperty("InputWorkspace", ws);
    alg.setPropertyValue("OutputWorkspace", "testws");
    TSM_ASSERT("Should not execute to completion without proton_charge_by_period if it's supposed to be multiperiod data.", !alg.execute());
    TSM_ASSERT("Should not execute to completion without proton_charge_by_period if it's supposed to be multiperiod data.", !alg.isExecuted());
  }
