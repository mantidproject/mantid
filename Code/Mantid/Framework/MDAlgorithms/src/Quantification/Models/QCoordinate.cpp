...
namespace Mantid
{
  namespace MDAlgorithms
  {
     ...
     */
    void QCoordinate::setAttribute(const std::string & name, const API::IFunction::Attribute& attr)
    {
      if(name == ATTR_NAMES[0])
      {
        std::string value = attr.asString();
        if(value == "QcX") m_coord = 0;
        else if(value == "QcY") m_coord = 1;
        else if(value == "QcZ") m_coord = 2;
        else if(value == "H") m_coord = 3;
     ...
     * @param point :: The axis values for the current point in Q-W space: Qx, Qy, Qz, DeltaE. They
    ...
    double QCoordinate::scatteringIntensity(const API::ExperimentInfo & exptSetup, const std::vector<double> & point) const
    {
      ...
      //qh
      if(m_coord == 3) return ((rb11*rb22 - rb12*rb21)*qx + (rb02*rb21 - rb01*rb22)*qy + (rb01*rb12 - rb02*rb11)*qz)/twoPiDet;
      //qk
      else if(m_coord == 4) return ((rb12*rb20 - rb10*rb22)*qx + (rb00*rb22 - rb02*rb20)*qy + (rb02*rb10 - rb00*rb12)*qz)/twoPiDet;
      //ql
      else if(m_coord == 5) return ((rb10*rb21 - rb11*rb20)*qx + (rb01*rb20 - rb00*rb21)*qy + (rb00*rb11 - rb01*rb10)*qz)/twoPiDet;
      else
        throw std::invalid_argument("Logical error. Invalid coord type " + boost::lexical_cast<std::string>(m_coord));
    }
  }
}
