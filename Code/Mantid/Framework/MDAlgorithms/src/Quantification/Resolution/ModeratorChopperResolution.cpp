...
namespace Mantid
{
  namespace MDAlgorithms
  {
    ...
    double ModeratorChopperResolution::energyWidth(const double deltaE) const
    {
      ...
      const double modChopTime = m_observation.moderatorToFirstChopperDistance()/neutronSpeed;
      const double stdDevModPulse = std::sqrt(m_moderator->emissionTimeVariance());
      double moderatorVar = ((stdDevModPulse/modChopTime)*(1.0 + (m_chopSampleDist/sampleToDetDist)*std::pow(ef/ei, 3)));
      moderatorVar *= moderatorVar;

      const double stdDevChopPulse = m_chopper->pulseTimeVariance();
      double chopperVar = ((stdDevChopPulse/modChopTime)*(1.0 +((m_modChopDist+m_chopSampleDist)/sampleToDetDist)*std::pow(ef/ei,3)));
