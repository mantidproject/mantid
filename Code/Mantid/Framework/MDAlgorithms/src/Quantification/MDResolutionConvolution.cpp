...
namespace Mantid
{
  namespace MDAlgorithms
  {
    ...
    /**
     * Construct the object with a ForegroundModel & resolution the function being fitted
     * @param fittingFunction :: A pointer to the function being fitted
     * @param fgModelName :: The name of the foreground model to use to calculate the
     ...
     */
    void MDResolutionConvolution::setFittingFunction(const API::IFunctionMD & fittingFunction)
    {
     ...
     * @param fgModelName :: The name of an existing model. Throws if the model is not known
     */
    void MDResolutionConvolution::setForegroundModel(const std::string & fgModelName)
    {
      if(m_foreground) return; // All ready done
      if(!m_fittingFunction)
      {
        throw std::runtime_error("MDResolutionConvolution::setForegroundModel - Fitting function must be defined first");
      }
      try
      {
        m_foreground = ForegroundModelFactory::Instance().createModel(fgModelName,  getFittingFunction());
      }
      catch(Kernel::Exception::NotFoundError &)
      {
        throw std::invalid_argument("Unknown foreground model selected - \"" + fgModelName + "\"");
      }
    }
    ...
    {
      throw Kernel::Exception::NotImplementedError("Error: Override MDResolutionConvolution::declareAttributes() "
                                                   "and declare convolution parameters");
    }

    /**
     * Set an attribute if the name is known to the convolution. If not, pass it on to the foreground model
     * @param name :: The name of the attribute
     * @param value :: The value of the attribute
     */
    void MDResolutionConvolution::setAttribute(const std::string& name, const API::IFunction::Attribute & value)
    {
