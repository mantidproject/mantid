...
Merge several [[MDWorkspace]]s into one.
...
namespace MDAlgorithms
{
  ...
  const std::string MergeMD::category() const { return "General";}
  ...
  void MergeMD::createOutputWorkspace(std::vector<std::string> & inputs)
  {
    ...
    // Number of dimensions, start with the 0th one. They all have to match # of dims anyway
    ...
    for (size_t i=0; i < m_workspaces.size(); i++)
    {
      ...
      if (ws->getNumDims() != numDims)
        throw std::invalid_argument("Workspace " + ws->name() + " does not match the number of dimensions of the others ("
            + Strings::toString(ws->getNumDims()) + ", expected " + Strings::toString(numDims) + ")" );

      if (ws->getEventTypeName() != ws0->getEventTypeName())
        throw std::invalid_argument("Workspace " + ws->name() + " does not match the MDEvent type of the others ("
      ...
      for (size_t d=0; d<numDims; d++)
      {
        ...
        if (dim->getName() != dim0->getName())
          throw std::invalid_argument("Workspace " + ws->name() + " does not have the same dimension " +
              Strings::toString(d) + " as the others (" + dim->getName() + ", expected " + dim0->getName() + ")" );
    ...
    {
      IMDDimension_const_sptr dim0 = ws0->getDimension(d);
      MDHistoDimension * dim = new MDHistoDimension(dim0->getName(), dim0->getDimensionId(), dim0->getUnits(),
          dimMin[d], dimMax[d], dim0->getNBins());
  ...
  void MergeMD::exec()
  {
    ...
    for (size_t i=0; i<m_workspaces.size(); i++)
    {
      g_log.information() << "Adding workspace " << m_workspaces[i]->name() << std::endl;
