                                                       ...
                                                       Strings &out_dim_units, MDEvents::MDWSDescription &TargWSDescription)
{
   ...
   // get the X axis of input workspace, it has to be there; if not axis throws invalid index
    ...
    bool isPowder = false;
    if(!TargWSDescription.hasLattice()) // No lattice
    { 
          isPowder = true;
    }
                                                   ...
                                                   size_t maxNdim,MDEvents::MDWSDescription &TargWSDescription)
{
   ...
   if(the_algID.find("Unknown")!=std::string::npos){
    ...
    size_t nDims = TargWSDescription.nDimensions();
   ...
   if((nDims<3)&&(QMod==Q3D))
   {
    ...
    if(TargWSDescription.isDetInfoLost() && emode!=-1){
                              ...
                              MDEvents::MDWSDescription &TargWSDescription)const
{
  ...
  {
    ws_dimIDs  = AxisNames.getDefaultDimIDModQ(eeMode);
    ws_dimNames= ws_dimIDs;
    nDims = ws_dimNames.size();
    ws_algoDefined = true;
  }
  // Q3D : default dimension names
  if (getQMode(AlgoID)==Q3D)
  {
    ws_dimIDs  = AxisNames.getDefaultDimIDQ3D(eeMode);    
    ws_dimNames= ws_dimIDs;
    nDims = ws_dimNames.size();
    ws_algoDefined = true;
  }
...
// assign output:
   nDims+=other_dim_names.size();   

   if(nDims != TargWSDescription.nDimensions())
   {
     throw(std::invalid_argument(" n-dims identified througn old parameters: ConvertToMDEventsParams is not equal to the one, constructed from TargetWSDescription"));
   }
