...
namespace Mantid
{
  namespace MDAlgorithms
  {
    ...
    void CreateMDFitWorkspace::init()
    {
      ...
      declareProperty("Formula","","muparser function to generate data in the workspace. Variables are x,y,z,t");
      declareProperty("MaxPoints",1,"Maximum number of points per cell. MDPoints are generated randomly");
    ...
    void CreateMDFitWorkspace::exec()
    {
      ...
      if (nDim <= 2)
      {
        ...
        do
        {
          ...
          {
            fil << it->getCoordinate(0) << ',' << ws->getCell(i).getSignal() << std::endl;
          }
        }while(it->next());
      }
