...
public:
  ...
  void testAddDimensions() {
    // Check that all dimensions from the file are being read-in. These tests
    // characterize what is in the file!
  ...
  /*Even though we have no need for the oriented lattice as part of the MDEW
    yet, test that the functionality
    is there to extract it*/
  void testReadLattice() {
    ExposedLoadSQW alg; // Derived type exposes protected method as public
                        // ::extractLattice.
    alg.initialize();
    alg.setPropertyValue("Filename", "test_horace_reader.sqw");
    alg.setPropertyValue("OutputWorkspace", "testAddDimension");
