...
namespace Mantid { namespace Kernel {
  ...
  {
    /**
    Minimize an objective function using the SLSQP optimization subroutine originally implemented by Dieter Kraft
    and ported to Python by scipy. This implementation is a port of the scipy variant using f2c to translate the Fortran
    code.
    ...
    \frac{1}{2}||Cx - d||^2
    \f]
    where \f$ ||f|| \f$ denotes the 2nd-norm of \f$f\f$. It possible to specify an optional set of constraints such that the function
    ...
    public:
      ...
      SLSQPMinimizer(const size_t nparams, const T & objfunc)
        : m_nparams(nparams), m_neq(0), m_nineq(0), m_objfunc(FunctionWrapper(objfunc)), m_constraintNorms() {}
      ...
      SLSQPMinimizer(const size_t nparams, const T & objfunc,
        ...
        : m_nparams(nparams), m_neq(equality.numRows()), m_nineq(inequality.numRows()),
