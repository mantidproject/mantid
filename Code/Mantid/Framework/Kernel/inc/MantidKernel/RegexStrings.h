...
template <typename T>
MANTID_KERNEL_DLL int StrComp(const char *, const boost::regex &, T &,
                              const int = 0);

/// Find if a pattern matches
template <typename T>
MANTID_KERNEL_DLL int StrComp(const std::string &, const boost::regex &, T &,
                              const int = 0);
...
MANTID_KERNEL_DLL std::vector<std::string> StrParts(std::string,
                                                    const boost::regex &);
...
MANTID_KERNEL_DLL int findPattern(std::istream &, const boost::regex &,
                                  std::string &);
