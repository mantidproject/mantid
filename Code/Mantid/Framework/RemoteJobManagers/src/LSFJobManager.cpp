...
// Do not declare, as this is an abstract class
...
void LSFJobManager::downloadRemoteFile(const std::string &transactionID,
  ...
  const std::string jobId = m_transactions[transactionID].jobIDs.back();
...
LSFJobManager::queryRemoteFile(const std::string &transactionID) const {
  ...
  // ACCEPT_TYPE}
  //  const std::string jobId = m_transactions[transactionID].jobIDs.back();
  const std::string jobId = m_transactions.find(transactionID)->second.jobIDs.back();
...
LSFJobManager::queryRemoteJob(const std::string &jobID) const {
  ...
  if (info.size() != 1)
    throw std::runtime_error(
        "There was an unexpected problem while retrieven status info of job "
        "with Id:" +
 ...
 * automatically.
 * @param numNodes Number of computing nodes
 * @param coresPerNode Number of cores
 *
 * @throw std::invalid_argument if there is an issue with the transaction ID
 * provided
 */
std::string LSFJobManager::submitRemoteJob(const std::string &transactionID,
  ...
  const std::string &body =
      buildSubmitBody(appName, boundary, runnable, param, taskName);
 ...
 */
std::string LSFJobManager::buildSubmitBody(const std::string &appName,
...
void LSFJobManager::addJobInTransaction(std::string &jobID) {
  auto &jobs = m_transactions.end()->second.jobIDs;
