...
class Reducer(object):
    ...
    def set_instrument(self, configuration):
        if issubclass(configuration.__class__, Instrument.Instrument):
            self.instrument = configuration
        else:
            raise RuntimeError, "Reducer.set_instrument expects an Instrument object, found %s" % configuration.__class__
