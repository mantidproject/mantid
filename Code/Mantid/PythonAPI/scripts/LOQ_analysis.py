...
#######################
#Function definition to remove a 'rectangle' of detectors
def detBlock(startID,rows,cols,step=128):
        output = ""
        for j in range(0,cols):
                for i in range(0,rows):
                        output += str(startID+i+(step*j))+","
        output = output.rstrip(",")
        return output

#######################
#Step 1 - Load the data file 
LoadDataAlg = LoadRawDialog(Filename=path+"Data/LOQ sans configuration/LOQ48098.raw",OutputWorkspace="Monitor",spectrummin="2",spectrummax="2")
...
#MarkDeadDetectors("Small_Angle",DetectorList=detBlock(?,?,?))
