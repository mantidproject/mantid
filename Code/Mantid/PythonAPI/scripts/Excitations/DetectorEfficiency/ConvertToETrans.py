...
import EfficiencyLib as efficien
...
#----Calculations start------------------
  ...
  if len(input) > 1:
    for toAdd in input[ 1 : ] :
      ...
      common.LoadNexRaw(toAdd, efficien.tempWS)
      LoadDetectorInfo(efficien.tempWS, toAdd)
      Plus(inOutWS, efficien.tempWS, inOutWS)
    mantid.deleteWorkspace(efficien.tempWS)
  ...
  if |GUI_SET_WBV| != '':
    efficien.NormaliseToWhiteBeam(|GUI_SET_WBV|, inOutWS, mapFile, |GUI_SET_WBVLow|, |GUI_SET_WBVHigh|)
