...
#--Start with Settings but these are not ones that are interesting to change, everything is read in from the MantidPlot GUI
...
try:
  ...
  #--make memory allocations easier by overwriting the workspaces of the same size, although it means that more comments are required to make the code readable
  ...
  if len(dataFiles) > 1 :
    for toAdd in dataFiles[ 1 : ] :
      ...
      Integration(InputWorkspace="_FindBadDetects loading", OutputWorkspace="_FindBadDetects loading" )# |TOFWINDOWBLOCK|)#a Mantid algorithm
...
#  prevTest = MedianDetectorTest( InputWorkspace=downSoFar, OutputWorkspace=NORMA, SignificanceTest=NUMERRORBARS, LowThreshold=-1, HighThreshold=10)
  ...
  lowThres = -1
  if REMOVEZERO == 'true' :
  #--the counts are integer numbers of counts that have been normalised and prehaps have a rounding error (is that true?). A very low threshold should reject all zeros but not allow anything that was 1 prior to normaliation to get through
    lowThres = 1e-40
  
  #--finally find the detectors! again reusing those workspaces
  MDT = MedianDetectorTest( InputWorkspace=SUM, OutputWorkspace=NORMA, SignificanceTest=NUMERRORBARS, LowThreshold=lowThres, HighThreshold=ACCEPT, OutputFile=OMASKFILE )#for usage see www.mantidproject.org/MedianDetectorTest
  ...
  Minus(NORMA, downSoFar, NORMA)																#for usage see www.mantidproject.org/Minus
