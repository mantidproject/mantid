  ...
  namespace API
  {
    ...
    */
	...
	else if( tag == "Cached:" )
	{
	  ...
	  g_log.debug() << "Linux - Cached memory reported: " << value << " KB. Note: Using 80% of this value as additional free memory.\n";
	}
	else continue;
	if( values_found == 3 ) { 
	  mi.freeRatio = static_cast<int>( double(mi.availMemory) / mi.totalMemory * 100 );
	  g_log.debug() << "Linux - Memory taken to be available for use (incl. cache): " << mi.availMemory << " KB.\n";
	  g_log.debug() << "Linux - Percentage of memory taken to be available for use (incl. cache): " << mi.freeRatio << "%.\n";
	  file.close();
	  return true;
	}
      }
