  ...
  */
{
  ...
  if (Stype.first+Stype.second==0)
    throw ColErr::ExBase(-2,"Pair Count wrong in AddComp");
      ...
      else               // Not in a bracket (currently)
	{
	  if (Ln[iu]=='(')
	  ...
	  else if (isalpha(Ln[iu]) || Ln[iu]=='%')
	    {
		  ...
		  if (!Nmove)
		    throw ColErr::InvalidLine(Ln,"Acomp::procIntersection",iu);
		  numItem+=52;
		  iu+=Nmove;
		}
	      else
  ...
  for(int i=0;i<BX.Size();i++)
    {
	  ...
	  if (V>=static_cast<int>(Index.size()))
	    throw ColErr::ExBase(-20,"Error with addUnit::Index");
  ...
  for(uc=Units.begin();uc!=Units.end();uc++)
    {
      ...
      bv=Base.find(V);
      if (bv==Base.end())
	throw ColErr::ExBase(-10,"Base unit not found");
  ...
  while (sPos!=std::string::npos && Ln[sPos+1]=='(')
    {
      ...
      if (bLevel)
	throw ColErr::InvalidLine(Ln,"Acomp::setString",sPos);
