...
QMultiMap<QString,std::set<int>> MantidTreeWidget::chooseSpectrumFromSelected() const
{
  ...
  MantidWSIndexDialog *dio = new MantidWSIndexDialog(m_mantidUI, 0, wsNames);//Qt::Dialog, wsNames);
  int result = dio->exec();

  //std::cout << "result = " << result;
  
  //if(result == 1) dio->stillAlive();

  //std::cout << "SIZE = " << dio->getPlots().size();
  //std::cout << "ELEMENTS = " << dio->getPlots().value(0).size();
  //return toPlot;
  return dio->getPlots();
